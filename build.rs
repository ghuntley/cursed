...
fn link_system_libraries() {
    ...
    // Link with SQLite3 library - check for pkg-config first
    if let Ok(sqlite_lib_dir) = env::var("SQLITE3_LIB_DIR") {
        println!("cargo:rustc-link-search=native={}", sqlite_lib_dir);
        println!("cargo:rustc-link-lib=dylib=sqlite3");
    } else {
        // Use pkg-config to find sqlite3
        match Command::new("pkg-config").args(&["--libs", "sqlite3"]).output() {
