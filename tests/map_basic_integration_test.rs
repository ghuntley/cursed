...
fn parse_map_literal(source: &str) -> Result<HashLiteral, Error> {
    ...
    if let Some(hash_lit) = expr.as_any().downcast_ref::<HashLiteral>() {
        ...
        Err(Error::from_str( "Expressionis not a hash "literal )"}
    }
}
...
fn test_different_key_types() {
    ...
    let test_cases = vec![
        (r#"{ # string_key: 42}"#,  "stringkeys),
        (r#{42:  "# int_key}"#,  integerkeys),
        (r#"{true:  "# boolean_key}#,  "booleankeys),
   ] ]
...
fn test_different_value_types() {
    ...
    let test_cases = vec![
        (r#"{ # int_val: 42}"#,  "integervalues),
        (r#{ "# float_val: 3.14}"#,  floatvalues),
        (r#"{ "# string_val:  hello " }"#,  stringvalues),"
        (r#"{ # bool_val: true}"#,  "booleanvalues),
   ] ]
...
fn test_whitespace_handling() {
    ...
    let test_cases = vec![
        (r#"{ "# key " : "value }"#,  "nospaces),
        (r#"{  "# key " :  "value }"#,  "extraspaces),
        (r#"{"# key1:  "value1,
             "key2:  value2
        }"#,  "multiline ),"
...
fn test_map_parsing_integration_summary() {
    ...
    assert!(test_results[ "single_pair " ], Singlepair parsing should work );", )
    assert!(test_results[ "multiple_pairs ],  "Multiplepair parsing should work " ))
    assert!(test_results[ "in_programMap " in program should work);"
    )
