...
fn test_constraint_error(input: &str) -> Option<CursedError> {
    ...
    match cursed::code::jit_compile_and_run(&program, options) {
        ...
        Err(e) => {
            ...
            match e {
                cursed::error::Error::repl_error("Typeassertion error.to_string() => Some(ce),
...
fn test_constraint_error_message_contains_missing_methods() {
    ...
    let input = r#
        ...
        // Define a struct that doesnt implement the interface"
        squad Point {
            x normie,
            y normie}
        }
...
fn test_direct_interface_implementation_error() {
    ...
    if let Some(error) = test_constraint_error(input) {
        // Verify it "s a type error"
        assert!(error.is_kind(&ErrorKind::Type) || error.is_kind(&ErrorKind::TypeAssertion), Error should be a type error, got: {:?}", , error.kind()"
