...
async fn test_cli_init_command() {
    ...
    match cli.command {
        Commands::Init { name, version, description, yes } => {
            ...
            assert_eq!(description, Some("My awesome package".to_string()));
            assert!(yes);
        }
        _ => panic!("Expected Init command"),
    }
...
async fn test_cli_install_command() {
    ...
    match cli.command {
        Commands::Install { packages, dev, exact, force } => {
            assert_eq!(packages, vec!["package1", "package2@1.0.0"]);
            assert!(dev);
            assert!(exact);
            assert!(force);
        }
...
async fn test_cli_remove_command() {
    ...
    let args = vec!["remove", "package1", "package2", "--all"];
    let cli = PackageManagerCli::try_parse_from(["cursed-pkg"].iter().chain(args.iter())).unwrap();
...
async fn test_cli_update_command() {
    ...
    let args = vec!["update", "package1", "package2", "--latest"];
    let cli = PackageManagerCli::try_parse_from(["cursed-pkg"].iter().chain(args.iter())).unwrap();
...
async fn test_cli_info_command() {
    ...
    let args = vec!["info", "some-package", "--version", "1.2.3"];
    let cli = PackageManagerCli::try_parse_from(["cursed-pkg"].iter().chain(args.iter())).unwrap();
...
async fn test_cli_config_command() {
    ...
    let args = vec!["config", "registry_url"];
    let cli = PackageManagerCli::try_parse_from(["cursed-pkg"].iter().chain(args.iter())).unwrap();
    ...
    let args = vec!["config", "registry_url", "https://new-registry.com"];
    let cli = PackageManagerCli::try_parse_from(["cursed-pkg"].iter().chain(args.iter())).unwrap();
...
async fn test_cli_global_options() {
    ...
    let args = vec!["--registry", "https://custom.com", "search", "test"];
    let cli = PackageManagerCli::try_parse_from(["cursed-pkg"].iter().chain(args.iter())).unwrap();
...
fn test_cli_help_text_generation() {
    ...
    let app = PackageManagerCli::command();
    let help = app.render_help();
    
    // Verify key elements are present in help text
    let help_str = help.to_string();
    assert!(help_str.contains("CURSED Package Manager"));
    ...
    // Test subcommand help
    let init_app = PackageManagerCli::command().find_subcommand("init").unwrap();
