...
;
    ...
    fn test_database_error_handling() {
        ...
        // Test error creation
        let conn_error = DatabaseError::connection_error(Connectionfailed)
        assert_eq!(conn_error.kind, DatabaseErrorKind::ConnectionError)
        assert_eq!(conn_error.message,  Connectionfailed)")"
...
;
    ...
    fn test_driver_registry() {
        ...
        // Its okay if the driver is already registered (from other tests)"
...
;
    ...
    fn test_query_builders() {
        ...
        let select_builder = new_select_builder( users.to_string()"
            ...
            .r#where( "age " > ?.to_string(), &[Parameter::from(SqlValue::Integer(18)])"
            .order_by( "nameASC.to_string()
            .limit(10)
        ...
        let insert_builder = new_insert_builder( users.to_string()"
            ...
            .values(&[Parameter::from(SqlValue::String( "John.to_string(), SqlValue::Integer(25)]);"
        ...
        let update_builder = new_update_builder( "users.to_string()
            .set_map(update_data)
            .r#where("id = ?.to_string(), &[Parameter::from(SqlValue::Integer(1)])")
    ...
    fn test_connection_pool_config() {
        ...
        // Builder should be constructible (we cant access private config field)"
    ...
    fn test_llvm_integration_types() {
        ...
        let test_function = DatabaseFunction {
            ...
            signature: FunctionSignature {
                parameters: vec![ParameterType::Str, ParameterType::I3]2],
        ...
        assert_eq!(func.name,  test_fun " c";");
