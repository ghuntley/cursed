...
fn test_map_literal_creation() {
    let token = Token::Tea;
    let key_type = Box::new(Identifier::new(Token::Identifier("tea".to_string()), "tea".to_string())) as Box<dyn Expression>;
    let value_type = Box::new(Identifier::new(Token::Identifier("normie".to_string()), "normie".to_string())) as Box<dyn Expression>;
    
    let pairs = vec![
        (
            Box::new(StringLiteral::new(Token::String("\"name\"".to_string()), "name".to_string())) as Box<dyn Expression>,
            Box::new(IntegerLiteral::new(Token::Int(42), 42)) as Box<dyn Expression>
        ),
        (
            Box::new(StringLiteral::new(Token::String("\"age\"".to_string()), "age".to_string())) as Box<dyn Expression>,
            Box::new(IntegerLiteral::new(Token::Int(25), 25)) as Box<dyn Expression>
        ),
    ];
    ...
    assert_eq!(map_literal.get_key_type().string(), "tea");
...
fn test_map_literal_clone() {
    ...
    let pairs = vec![
        (
            Box::new(StringLiteral::new(Token::String("\"key\"".to_string()), "key".to_string())) as Box<dyn Expression>,
            Box::new(IntegerLiteral::new(Token::Int(100), 100)) as Box<dyn Expression>
        ),
    ];
...
fn test_map_literal_pairs_iterator() {
    let token = Token::Tea;
    let key_type = Box::new(Identifier::new(Token::Identifier("normie".to_string()), "normie".to_string())) as Box<dyn Expression>;
    let value_type = Box::new(Identifier::new(Token::Identifier("tea".to_string()), "tea".to_string())) as Box<dyn Expression>;
    
    let pairs = vec![
        (
            Box::new(IntegerLiteral::new(Token::Int(1), 1)) as Box<dyn Expression>,
            Box::new(StringLiteral::new(Token::String("\"one\"".to_string()), "one".to_string())) as Box<dyn Expression>
        ),
        (
            Box::new(IntegerLiteral::new(Token::Int(2), 2)) as Box<dyn Expression>,
            Box::new(StringLiteral::new(Token::String("\"two\"".to_string()), "two".to_string())) as Box<dyn Expression>
        ),
    ];
...
fn test_map_type_expression_clone() {
    let token = Token::Tea;
    let key_type = Box::new(Identifier::new(Token::Identifier("CustomKey".to_string()), "CustomKey".to_string())) as Box<dyn Expression>;
    let value_type = Box::new(Identifier::new(Token::Identifier("CustomValue".to_string()), "CustomValue".to_string())) as Box<dyn Expression>;
...
fn test_map_type_expression_with_complex_types() {
    ...
    let value_type = Box::new(Identifier::new(Token::Identifier("SliceOfNormie".to_string()), "SliceOfNormie".to_string())) as Box<dyn Expression>;
...
fn test_map_literal_with_complex_values() {
    let token = Token::Tea;
    let key_type = Box::new(Identifier::new(Token::Identifier("tea".to_string()), "tea".to_string())) as Box<dyn Expression>;
    let value_type = Box::new(Identifier::new(Token::Identifier("ComplexType".to_string()), "ComplexType".to_string())) as Box<dyn Expression>;
    
    // Create a map with nested expressions as values
    let pairs = vec![
        (
            Box::new(StringLiteral::new(Token::String("\"config\"".to_string()), "config".to_string())) as Box<dyn Expression>,
            Box::new(Identifier::new(Token::Identifier("defaultConfig".to_string()), "defaultConfig".to_string())) as Box<dyn Expression>
        ),
    ];
...
fn test_map_literal_debug_format() {
    ...
    let pairs = vec![
        (
            Box::new(StringLiteral::new(Token::String("\"test\"".to_string()), "test".to_string())) as Box<dyn Expression>,
            Box::new(IntegerLiteral::new(Token::Int(1), 1)) as Box<dyn Expression>
        ),
    ];
