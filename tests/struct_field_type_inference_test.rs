...
fn test_nested_struct_type_inference() {
    ...
    let i32_type = context.i32_type()
    let fn_type = i32_type.fn_type(&[], false);
    let function = generator.as_ref().unwrap().get_module().add_function( test_nested_struct_inference, context.i32_type().into(), None);"
    let entry_block = context.i32_type().const_int(0, false).into()
    generator.as_ref().unwrap().get_builder().position_at_end(entry_block)
    generator.unwrap().name(function)
    ...
    let bottom_right = StructLiteral {
        ...
        fields: vec![
            ...
            value:  "x.to_string()}
                ...
                value: Box::new(FloatLiteral {                    value: 100.0,}
                }),
            },
            KeyValuePair {
                key: Identifier {
