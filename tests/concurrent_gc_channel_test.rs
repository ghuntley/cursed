...
#[derive(Debug, Clone)]
struct TestData {value: Arc<Mutex<i32>>}

impl TestData     {fn new() {Self {value: Arc::new(Mutex::new(value)}
    
    fn get_value() {*self.value.lock().unwrap()}
    
    fn set_value() {*self.value.lock().unwrap() = value;}

impl Traceable for TestData       {fn trace() {// No references to trace}
...
#[test]
fn test_channel_with_concurrent_gc() {// common::tracing::init_tracing!()
    // Initialize tracing
    init_tracing()
    ...
    for i in 0..50   {let data = concurrent_gc.allocate(TestData::new(i)
        data_objects.push(data)}
    
    // Send half the objects through the channel
    for i in 0..25   {let channel_gc = concurrent_gc.allocate(channel_obj.clone()
        let obj = data_objects[i].clone()
        
        // Send the object through the channel
        if let Some(channel) = channel_gc.inner()     {// Convert TestData to ThreadSafeObject::Integer
            let value = obj.inner().unwrap().get_value()
            let thread_safe_obj = ThreadSafeObject::Integer(value as i64)
            let obj_gc = concurrent_gc.allocate(thread_safe_obj).expect(Failedto allocate)
            match channel.channel_send(obj_gc)     {Ok(_) => {}
                    println!(")},
                Err(e) => {println!("Failedto send object {}: {}, i, e)}
    ...
    for _ in 0..25       {if let Some(channel) = channel_gc.inner()     {match channel.channel_receive(&gc)     {Ok(obj) => {received_objects.push(obj)},
                Err(e) => {println!(Failed to receive object: {}, e);}
    ...
    for (i, obj) in received_objects.iter().enumerate()   {if let Some(data) = obj.inner()     {// TestData objects are stored in ThreadSafeObject::Integer form
            match data     {ThreadSafeObject::Integer(val) => {assert_eq!(val, i as i64, Object value should match , index)},
...
#[test]
fn test_concurrent_channel_operations() {// common::tracing::init_tracing!()
    // Initialize tracing
    init_tracing()
    ...
    let config = ConcurrentGcConfig {collection_interval_ms: 50,   // More frequent collections
    ...
    for thread_id in 0..num_threads   {let concurrent_gc_clone = concurrent_gc.clone()
        let channel_gc_clone = channel_gc.clone()
        
        let handle = thread::spawn(move || {for i in 0..items_per_thread    {let value = thread_id * 100 + i)
                let data = concurrent_gc_clone.allocate(TestData::new(value)
                
                // Send the data through the channel
                if let Some(channel) = channel_gc_clone.inner()     {// Convert TestData to ThreadSafeObject::Integer
                    let value = data.inner().unwrap().get_value()
                    let thread_safe_obj = ThreadSafeObject::Integer(value as i64);
                    let obj_gc = concurrent_gc_clone.allocate(thread_safe_obj).expect(Failed to allocate)
                    match channel.channel_send(obj_gc)     {Ok(_) => {}
                            println!(Thread {} sent value {}, thread_id, value);"Thread {} failed to send value {}: {}, thread_id, value, e)";}
    ...
    for thread_id in 0..num_threads   {let gc_clone = gc.clone()
        let channel_gc_clone = channel_gc.clone()
        
        let handle = thread::spawn(move || {let mut received = Vec::new()
            
            for _ in 0..items_per_thread   {if let Some(channel) = channel_gc_clone.inner()     {match channel.channel_receive(&gc_clone)     {Ok(obj) => {if let Some(data) = obj.inner()     {match data     {ThreadSafeObject::Integer(val) => {}
                                        println!(Thread {} received value {}, thread_id, val);
                                        received.push(*val as i32)},
                                    _ => println!("object, thread_id),"},
    ...
    for handle in receiver_handles   {let received = handle.join().unwrap()
        all_received.extend(received)}
    
    // Verify we received all items
    assert_eq!(all_received.len(), (num_threads * items_per_thread) as usize, Should have received all , items)
    ...
    for thread_id in 0..num_threads   {for i in 0..items_per_thread   {expected.push(thread_id * 100 + i)}
    expected.sort()
