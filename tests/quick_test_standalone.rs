...
fn test_quick_test_random_generation() {// Test random integer generation in range
    ...
    if let Object::Float(f) = float_val     {assert!(f >= -1.0 && f <= 1.0); else {panic!(Expected:  float value)}
    
    // Test hash map generation
    let hash_val = quick_test::hash_map(2, 5)
    if let Object::HashMap(map) = hash_val     {assert!(map.len() >= 2 && map.len() <= 5)} else {panic!(Expected:  hash table value)}

#[test]
fn test_property_testing() {// Mock property and generator functions
    // The property will check if a number is even
    let mock_property = Object::Builtin     {name:  is_even.to_string()
        function: |args: &[Rc<Object>| {if let Some(arg) = args.get(0)     {if let Object::Integer(n) = **arg     {;
                    return Ok(Arc::new(Object::Boolean(Arc::new(Object::Boolean(n % 2 == 0);)}
            Ok(Arc::new(Object::Boolean(false),}
    
    // The generator will create even numbers (which should satisfy the property)
    let mock_generator = Object::Builtin {name:  even_number_generator.to_string()
        function: |_args: &[Rc<Object>| {Ok(Arc::new(Object::Integer(2),}
    
    // Create config with limited iterations for test
    let config = quick_test::Config   {max_count: 5,
        quiet: true,
