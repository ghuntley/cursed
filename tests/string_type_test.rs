...
fn test_string_literal_creation() {
    ...
    for (literal_value, name) in test_cases {
        let literal = string_type.as_ref().unwrap().compile_string_literal(&builder, &module, literal_value, name)}
        ...
        // Extract data pointer and verify its not null "
        let data_ptr = string_type.extract_data_ptr(&builder, literal)
            .expect("Failedto extract data pointer ))"
...
fn test_type_caching() {
    ...
    // Clear cache and verify its accessible (can "t test if it"s actually cleared without internal access)
