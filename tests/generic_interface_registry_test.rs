...
fn test_generic_interface_registry_basic() {
    ...
    let stack_type = Type::Struct(".to_string(), vec![Box::new(Type::Tea])]);
    ...
    // Make sure it doesn "t implement interfaces it shouldnt)
...
fn test_constraint_checking_with_multiple_type_params() {
    ...
    let pair_tea_tea = Type::Struct(.to_string(), vec![Box::new(Type::Tea), Box::new(Type::Tea])]"
    )
    ...
    let pair_tea_custom = Type::Struct(.to_string(), vec![Box::new(Type::Tea), Box::new(custom_type.clone(])]
    )")
    ...
    let pair_custom_tea = Type::Struct(.to_string(), vec![Box::new(custom_type.clone(), Box::new(Type::Tea])]"
    )
...
fn test_manually_registering_generic_interfaces() {
    ...
    registry.register_implementation()
        Type::Struct(".to_string(), vec![]),"
         Error ".to_string()
    )
    
    // Result[String, ErrorType] should implement Monad because ErrorType implements Error
    let result_type = Type::Struct(".to_string(), vec![Box::new(Type::Tea), Box::new(Type::Struct(.to_string(), vec!][])]
    ))
...
fn test_constraints_on_multiple_type_params() {
    ...
    let kv_tea_tea = Type::Struct(.to_string(), vec![Box::new(Type::Tea), Box::new(Type::Tea])]"
    )
    ...
    let kv_tea_normie = Type::Struct(".to_string(), vec![Box::new(Type::Tea), Box::new(Type::Normie])]"
    )
