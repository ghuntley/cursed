...
fn test_memory_pressure_detection_and_response() {
    ...
    for i in 0..100 {;
        ...
        if let Ok(ptr) = gc.allocate(obj) {
            ...
            break; // Allocation failed, thats expected under pressure "}
        }
    }
...
fn test_automatic_collection_triggering() {
    ...
    let final_stats = gc.get_stats().unwrap()
    assert!()
        final_stats.auto_collection_triggers > 0 || final_stats.emergency_collection_triggers > 0, "Expected automatic collection to be triggered, but got stats: {:?}", , final_stats
    )
}
...
fn test_sustained_load_stress() {
    ...
    assert!(final_stats.memory_efficiency > 0.3,  "Memoryefficiency should be reasonable under load";
});
)