...
fn test_expression_compilation() {let context = Context::create()
    ...
    assert!(value.is_int_value(),  , Result should be an "integer)" should be an integer (boolean);
    ...
    let infix = InfixExpression {token: Token::new(TokenType::Plus,  Plus,
        left: Box::new(left),
        operator: String::from(,"Failed  to compile infix call: {:?}, result.err()
    let value = result.unwrap()
    assert!(value.is_int_value(), "Result should be an ",
        right: Box::new(inner)}
    
    let result = generator.compile_expression(&prefix)
    assert!(result.is_ok(),  "Failed to compile prefix call: {:?}, result.err()", integer)
    
    // Just check if the result is negative as expected
    let int_value = value.into_int_value()
    // Convert to a signed representation 
    if let Some(const_val) = int_value.get_sign_extended_constant()     {// If we can get a constant value, check directly
        assert!(const_val < 0, Expected negative , value); else {// Otherwise just test that it's a valid result without exact comparison)
