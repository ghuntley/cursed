...
fn test_object_to_thread_safe_conversion() {
    ...
    for obj in objects {
        ...
        match obj {
            ...
            Object::HashTable(_) => {
                if let ThreadSafeObject::HashTable(_) = thread_safe.as_ref() {
                    // Just check the type, detailed verification in other tests
                    assert!(true);
                } else {
            ...
            Object::Null => {
                if let ThreadSafeObject::Null = thread_safe.as_ref() {
                    assert!(true);
                } else {
