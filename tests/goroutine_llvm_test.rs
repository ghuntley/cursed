...
fn test_llvm_goroutine_function_creation() {
    ...
    assert!(spawn_fn.is_ok(), "Should be able to create spawn function");
    
    let wait_fn = codegen.get_or_create_wait_goroutine_fn();
    assert!(wait_fn.is_ok(), "Should be able to create wait function");
    
    let wait_all_fn = codegen.get_or_create_wait_all_goroutines_fn();
    assert!(wait_all_fn.is_ok(), "Should be able to create wait_all function");
}
...
fn test_llvm_goroutine_wrapper_creation() {
    ...
    let return_type = wrapper_fn.get_type().get_return_type();
...
fn test_runtime_function_declarations() {
    ...
    assert_eq!(spawn_fn.get_type().get_param_types().len(), 2, "spawn_goroutine should take 2 parameters");
    assert_eq!(wait_fn.get_type().get_param_types().len(), 1, "wait_goroutine should take 1 parameter");
    assert_eq!(wait_all_fn.get_type().get_param_types().len(), 0, "wait_all_goroutines should take 0 parameters");
}
