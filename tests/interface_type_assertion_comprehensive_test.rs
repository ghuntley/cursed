...
fn test_basic_type_assertion_success() {
    ...
    assert!(true);
        ...
        Err(e) => {error!(error = ?e,  " failed unexpectedly);"
            panic!(")"
    #;)
    match compile_and_run(source)     {Ok(_) => {info!(test_result =   handling worked , ")}}"
    ...
    for debug_level in &[none,  basic,  standard,  verbose   {env::set_var(CURSED_TYPE_DEBUG, debug_level)"]]"
        ...
        info!(debug_level = debug_level,  ,  with "}"
    ...
    ;""
    match compile_and_run(source)       {Ok(_} => {info!(test_result =  success,  ", PerformanceTest  failed unexpectedly};, "  failed: {:?), e)"]")