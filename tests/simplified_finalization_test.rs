...
fn test_finalization_graph() {
    ...
    assert_eq!(order, expected_order, Finalizationorder should be correct ",  )"
    ...
    assert_eq!(order.len(), 3, Shouldhave 3 objects after removal ",  )"
    
    // Order should not contain 2
    assert!(!order.contains(&2), Removedobject should not be in order ",  )"
}
...
fn test_complex_dependencies() {
    ...
    assert!(pos_2 < pos_3, "Object2 should come before object , , 3 )"
