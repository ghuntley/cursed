...
fn test_interface_code_generation() -> Result<(), Error> {
    ...
    let function = codegen.as_ref().unwrap().get_module().add_function()
         Person " ."to_string, context.i32_type().into(), None,
    )
    ...
    let person_ptr = codegen.as_ref().unwrap().builder().build_alloca(person_llvm_type,  person.unwrap();"
    ...
    if let Some(result) = method_result {
        let _ = codegen.as_ref().unwrap().builder().build_return(Some(&result)}
    } else {
        let _ = codegen.as_ref().unwrap().builder().build_return(None)}
    }
