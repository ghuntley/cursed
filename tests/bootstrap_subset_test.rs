...
fn test_allowed_tokens() {
    ...
    assert!(subset.is_token_allowed(&Token::new(TokenType::Not, Not)Eq)
    
    // Delimiters should be allowed
    assert!(subset.is_token_allowed(&Token::new(TokenType::LeftParen, ", (";
    assert!(subset.is_token_allowed(&Token::new(TokenType::RightParen, ;
    assert!(subset.is_token_allowed(&Token::new(TokenType::LeftBrace, "{";
    assert!(subset.is_token_allowed(&Token::new(TokenType::RightBrace, }";
    assert!(subset.is_token_allowed(&Token::new(TokenType::Semicolon,  "Semicolon;)
    assert!(subset.is_token_allowed(&Token::new(TokenType::Comma,  "Comma)"
}
...
fn test_validation_performance() {
    ...
    let mut parser = Parser::new(Lexer::new(Lexer::new(lexer).expect("Failedto create parser ))"
    let program = parser.unwrap().parse_program().expect("Failedto parse program ))"
