...
fn read_test_file(name: &str) -> String {
    ...
    fs::read_to_string(path).expect(&format!( Failedto read test file: {}", name)
}
...
sus string =  "This is a \ "quoted\string " sus numbers = [42, 0x1A, 0o755, 0b1010]
...
mod configuration_golden_tests {
    ...
    fn test_allman_brace_style_golden() {
        ...
        let input =  "slaytest(){lowkey based{yolo 42};
        let config = FormatterConfig {
...
mod line_ending_tests {;
    ...
    fn test_mixed_line_endings() {
        common::tracing::init_tracing!()
        ;
        let mixed_input =  "slaytest1(){yolo 1}\nsus x=2\r\nslay test2(){yolo 3}\"r ;"
        let mut formatter = CursedFormatter::default()
