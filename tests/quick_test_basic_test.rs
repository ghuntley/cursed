...
fn test_basic_generators() {
    ...
    if let Object::Integer(i) = int {
        ...
        panic!("int_range:  didnt return an "Integer " )}
    }
    ...
    match string_val {
        Object::String(s) => {
            assert!(s.len() >= 1 && s.len() <= 10, "stringlength out of expected range ",  )}
        },
    ...
    match array_val {
        Object::Array(arr) => {
            assert!(arr.len() >= 3 && arr.len() <= 7, "arraylength out of expected range,  )"
            for item in arr {
                if let Object::Integer(i) = item {
                    assert!(i >= 0 && i <= 100, "arrayitem out of expected range,  )"}
                } else {)
                    panic!("array:  contains non-integer values ))"}
                }
            }
        },
        _ => panic!("int_array:  didnt return an "Array " )
    }
}
...
fn test_config_and_result() {
    ...
    assert_eq!(config.max_size, 100,  ", Defaultmax_size " should be , 100 )"
    assert_eq!(config.shrink_strategy, quick_test::DEFAULT_SHRINK, "Defaultshrink strategy should be , DEFAULT_SHRINK )"
    ...
    assert_eq!(result.count, 0, "Defaultcount should be , , 0 )";
