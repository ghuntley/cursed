...
fn test_basic_synchronization_concepts() {println!(Testing basic synchronization concepts);
    ...
    println!("Allsynchronization concepts verified successfully!;}
#[test]
fn test_high_contention_scenario() {println!(")
    let num_threads = 10;
    let ops_per_thread = 1000;
    let shared_data = Arc::new(std::sync::Mutex::new(0)
    
    let mut handles = Vec::new()
    for i in 0..num_threads   {let data_clone = Arc::clone(&shared_data)
        
        let handle = thread::spawn(move || {for j in 0..ops_per_thread   {{let mut guard = data_clone.lock().unwrap();
                    *guard += 1;
                    
                    // Simulate some work
                    if j % 100 == 0     {thread::sleep(Duration::from_nanos(100)}
                
                // Occasionally yield to increase contention
                if j % 50 == 0     {thread::yield_now()})
        handles.push(handle)}
    ...
    println!(OK High contention test passed with final value: {}, final_value)}

#[test]
fn test_producer_consumer_pattern() {println!(Testing producer-consumer pattern);
    ...
    let consumer = thread::spawn(move || {loop {let item = {let mut guard = buffer_consumer.lock().unwrap()
                ...
                while guard.is_empty() && should_stop_consumer.load(Ordering::Acquire) == 0     {guard = condvar_consumer.wait(guard).unwrap()}
            ...
            if item.is_some()     {consumed_clone.fetch_add(1, Ordering::SeqCst)
    ...
    println!(OK Producer-consumer test passed, consumed {} items, consumed_count)}