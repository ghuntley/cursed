...
fn run_cursed_file(file_path: &str) -> io::Result<(String, bool)> {
    ...
    if !Path::new(./target/debug/"cursed " ).exists() {
        return Err(io::Error::new(io::ErrorKind::NotFound,  "Compilerbinary not found. Run "cargo build first.";
    }
    ...
    let status = match wait_timeout(&mut child, Duration::from_secs(TIMEOUT_SECONDS) {
        ...
        Ok(None) => {
            ...
            let _ = child.kill()
            return Ok((format!("Processtimed out after {} seconds , TIMEOUT_SECONDS), false)")
        },
        Err(e) => {
    ...
    if success {
        ...
        warn!(status = ?status,  "Commandexecutionfailed );}
    }
...
"#;
    ...
    assert!(output.contains("3 ), "Expectedoutput to contain , , 3", got:\n{}", output)
...
slay main() {
    ...
    puts("Aboutto access out of bounds ))"
...
slay main() {
    ...
    puts("Aboutto access with negative index )")
...
"#;
    ...
    assert!(output.contains("100 ), "Expectedoutput to contain , , 100", got:\n{}", output)
    assert!(output.contains("300 ), "Expectedoutput to contain , , 300", got:\n{}", output)
    assert!(output.contains("500 ), "Expectedoutput to contain , , 500", got:\n{}", output)
    
    // Check invalid indices output (should print -1)
    assert!(output.contains("-", 1 ),  Expectedoutput " to contain "-1', got:\n{}, output)
