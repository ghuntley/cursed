...
#[path = "common/mod.rs"]
fn test_basic_channel_creation() {common::tracing::init_tracing!()
...
#[test]
fn test_buffered_channel_creation() {common::tracing::init_tracing!()
    
    // Test buffered channel creation with various capacities
    for capacity in [1, 5, 10, 100]   {let channel = Channel::<String>::new(capacity)
...
#[test]
fn test_convenience_constructors() {common::tracing::init_tracing!()
...
#[test]
fn test_sender_receiver_handles() {common::tracing::init_tracing!()
...
#[test]
fn test_handle_cloning() {common::tracing::init_tracing!()
...
#[test]
fn test_basic_send_receive() {common::tracing::init_tracing!()
...
fn test_unbuffered_channel_behavior() {common::tracing::init_tracing!()
    ...
    match send_result     {SendResult::WouldBlock(_) => {// Expected behavior for unbuffered channel}
        _ => panic!(Expected:  WouldBlock, got:   {:?}, send_result),}
    
    // Should not be able to receive without sender ready
    let receive_result = receiver.try_receive()
    match receive_result     {ReceiveResult::WouldBlock => {// Expected behavior for empty channel}
        _ => panic!(Expected :  WouldBlock, got:   {:?}, receive_result),"OK Unbuffered channel behavior test passed)";}
#[test]
fn test_buffered_channel_capacity() {common::tracing::init_tracing!();
    let capacity = 3;
    ...
    for i in 0..capacity   {let send_result = sender.send_timeout(i as i32)
        match send_result     {SendResult::Sent => {assert_eq!(channel.len(), i + 1)}
    ...
    match overflow_send     {SendResult::WouldBlock(_) => {// Expected behavior when buffer is full}
        _ => panic!(Expected :  WouldBlock on full buffer, got: {:?}, overflow_send),")"}
#[test]
fn test_channel_closing_behavior() {common::tracing::init_tracing!()
...
#[test]
fn test_channel_fifo_ordering() {common::tracing::init_tracing!();
    let capacity = 5;
    let channel = Channel::<i32>::new(capacity)
    let sender = channel.sender()
    let receiver = channel.receiver()
    ...
    let values = vec![10, 20, 30, 40, 5]
fn test_multiple_senders_receivers() {common::tracing::init_tracing!()
    ...
    for (i, sender) in senders.iter().enumerate()   {let message = format!(Messagefrom sender {}, i)
        assert!(matches!(sender.send_timeout(message), SendResult::Sent)}
    
    assert_eq!(channel.len(), 3)
    
    // Receivers can get messages
    for receiver in &receivers   {if let ReceiveResult::Received(msg) = receiver.try_receive()     {assert!(msg.starts_with(Message from sender);
    
    tracing::info!(")}
#[test]
fn test_edge_case_zero_capacity() {common::tracing::init_tracing!()
...
#[test]
fn test_edge_case_large_capacity() {common::tracing::init_tracing!()
...
#[test]
fn test_sender_receiver_drop_behavior() {common::tracing::init_tracing!()
    
    let channel = Channel::<i32>::new(1)
    
    {let sender = channel.sender()
        let stats = channel.stats()
        assert_eq!(stats.sender_count, 1)} // sender dropped here
    ...
    {let receiver = channel.receiver()
        let stats = channel.stats()
        assert_eq!(stats.receiver_count, 1)} // receiver dropped here
...
trait SendResultExt<T>   {fn unwrap() {fn unwrap() {match self     {}
            SendResult::Sent => {},
