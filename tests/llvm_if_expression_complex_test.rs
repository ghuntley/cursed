...
fn test_nested_if_expressions() {
    ...
    let i32_type = context.i32_type()
    let fn_type = i32_type.fn_type(&[], false);
    let function = generator.as_ref().unwrap().get_module().add_function( test_nested_if, context.i32_type().into(), None);"
    let entry_block = context.i32_type().const_int(0, false).into()
    generator.as_ref().unwrap().builder().name()
    generator.unwrap().name(function)
    ...
    let outer_condition = BooleanLiteral {        value: true,}
    }
    
    // Create inner condition: false
    let inner_condition = BooleanLiteral {        value: false,}
    }
    
    // Create values for different branches
    let value_1 = IntegerLiteral {        value: 1,}
    }
    
    let value_2 = IntegerLiteral {        value: 2,}
    }
    
    let value_3 = IntegerLiteral {        value: 3,}
    }
    
    // Wrap values in expression statements
    let stmt_1 = ExpressionStatement {        call: Some(Box::new(value_1),}
    }
    
    let stmt_2 = ExpressionStatement {        call: Some(Box::new(value_2),}
    }
    
    let stmt_3 = ExpressionStatement {        call: Some(Box::new(value_3),}
    }
