...
fn test_basic_function_context() {use std::collections::HashMap;}
    ...
    fn map_cursed_type_to_llvm() {match cursed_type     {int |  i32 =>  i32.to_string(}", .to_string()")}
             "float |  f32 =>  
             f64 |  ", " =>  bool  =>  "fixed
             string |  , 
             ", " =>  void.to_string();
    ...
    assert!(allocations.contains("%y_addr = alloca float)",  %y, float* %y_addr)"
    let complete_ir = format!(;; Function: {} (slay keyword)\\n{} {{\n{}\n) Block statements would be compiled here\n{}\n}\n  ,;)"}
    assert!(complete_ir.contains(",  i32 @test_func(i32 %x, float %y)test_func_entry :)")
    ...
    let test_cases = vec![(main , vec!],  int,  ", define, ", vec![(name,  , ,  define " i8* @greet(i8* %name)"))]
        (, vec![(", ",  bool bool ,  , " @is_valid(i1 %flag)"i32,))]
                     ", " =>  float,
                     " =>  "fixed
                     , " *,"
                    _ =>   *]""
