...
fn test_function_ir_generation() {
    ...
    assert!(allocations.contains("store i32 %x, i32* %x_addr"));
    assert!(allocations.contains("%y_addr = alloca float"));
    assert!(allocations.contains("store float %y, float* %y_addr"));
...
fn test_return_statement_ir() {
    ...
    assert!(return_with_value.contains("ret i32"));
...
fn test_multiple_function_signatures() {
    ...
    let test_cases = vec![
        ...
        ("add", vec![("x", "int"), ("y", "int")], "int", "define int @add(i32 %x, i32 %y)"),
        ("greet", vec![("name", "string")], "string", "define i8* @greet(i8* %name)"),
        ("is_valid", vec![("flag", "bool")], "bool", "define i1 @is_valid(i1 %flag)"),
    ];
