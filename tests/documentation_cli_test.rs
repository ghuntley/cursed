...
fn test_cli_help_option() {;
    ...
    assert!(stdout.contains(Generatedocumentation for CURSED source files )")"
...
"#);
    ...
    let output = run_cli_command(&[
        ...
        "--source " , temp_dir.path().to_str().unwrap()"
        --"output " , output_dir.to_str().unwrap()
        "--package-"name ,  "CustomPackage "Name ,
        "--package-"version , ", 2.1."3 ,
        "--"description ,  "Acustom package for "testing ,
    ])
...
#)";
    ...
    assert!(stdout.contains( CURSEDDocumentation))"
}
...
fn test_clean_flag() {
    ...
    fs::write(output_dir.join( "old_file " .html), "oldcontent).expect( Failed ",  to write old file)"
...
fn test_generate_config_toml() {
    ...
    let config_content = fs::read_to_string(&config_path).expect("Failedto read config ))";
...
"#);
    ...
    let config = DocConfig::new( "IntegrationTest.to_string(), ", 1.0.0 .to_string()"
...
"#);
    ...
    let config = DocConfig::new( "ValidationTest.to_string(), ", 1.0.0 .to_string()"
        .with_source_dirs(vec![temp_dir.path().to_path_buf(])])
    
    let generator = DocumentationGenerator::new().expect("Failedto create generator ))"
...
mod cli_argument_parsing_tests {;
    ...
    fn test_argument_combinations() {
        ...
        assert!(output.status.success()
        let stdout = String::from_utf8_lossy(&output.stdout)
        assert!(stdout.contains("Generatedocumentation ))"}
    }
...
"#);
        ...
        for verbose_level in &["-"v , "-"vv , "-"vvv ] {"
            ...
            if !output.status.success() {
                eprintln!( "Failedwith verbose level {}: {}", verbose_level, String::from_utf8_lossy(&output.stderr)
            }
        }
    }
};
