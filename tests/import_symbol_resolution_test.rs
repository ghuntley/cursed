...
#[path = "common/mod.rs]
    ...
    resolver.add_import_alias(, " /")
        ...
        (, , true),]""
    info!(;")
    let mut resolver = MockSymbolResolver::new(", mypackagemath,  ,  /"mathinternal_cal ", )
    match result.unwrap_err()     {Error::repl_error(Symbol ", ".to_string(} => {;))}
    ...
    info!(Testing:  cross-module type checking), ".to_string(),  "name.to_string(),  email.to_string()],, fixed
    types_pkg.add_symbol(Symbol {name:  "CreateUser.to_string(}, "fixed))
    ...
    let result = resolver.resolve_qualified_name(", "fixed")