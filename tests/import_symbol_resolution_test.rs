...
fn test_private_symbol_access() {
    ...
    let result = resolver.resolve_qualified_name("math", "internal_calc");
...
fn test_unqualified_name_resolution() {
    ...
    let result = resolver.resolve_unqualified_name("local_func");
    assert!(result.is_ok(), "Should resolve local symbol");
...
fn test_symbol_visibility_rules() {
    ...
    let result = resolver.resolve_qualified_name("test", "internalFunc");
