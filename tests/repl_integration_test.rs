...
"#).unwrap()";
    ...
    assert!(workspace.contains("Build Targets"));
}
...
fn test_repl_with_working_directory() {
    ...
    fs::write(&test_file, "facts test_var = 123").unwrap();
...
fn test_session_code_formatting() {
    ...
    session.execute_code("facts y   =   24").unwrap();
...
fn test_function_completion() {
    ...
    session.execute_code("slay test_function() { return 42; }").unwrap();
