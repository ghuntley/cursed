...
#[test]
fn test_type_system_integration() {// Test code with various type system features
    let input = 
        vibe main
        ...
        be_like Container[T] collab {add(item T)
            get(index normie) T
            size() normie}
        
        fr fr Implement Container for a generic list
        be_like List[T] squad  {items []T
            count normie}
        
        fr fr Generic function with multiple type parameters
        slay map[T, U](items []T, transform slay(T) U) []U {sus result = make([]U, len(items)
            sus i = 0
            
            periodt i < len(items) {result[i] = transform(items[i])
                i = i + 1}
    ...
    assert_eq!(interface_count, 1,  "Should have 1 interface "definitions);
