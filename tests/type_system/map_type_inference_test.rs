...
fn test_map_type_inference() {// Parse and type check a simple map literal
    ...
    if let Some(expr_stmt) = stmt.as_any().downcast_ref::<cursed::ast::statements::ExpressionStatement>()   {if let Some(expr) = &expr_stmt.expression   {;
            let result = type_checker.check_expression(expr.as_ref();
            assert!(result.is_ok();
            
            match result.unwrap()   {Type::Map(key_type, value_type) => {assert_eq!(key_type, Type::Tea); // Keys should be tea (string) type
                ...
                other => panic!(Expected map type, got {:?}, other),} else {panic!("Not an expression "statement);}
#[test]
fn test_map_type_inference_int_to_float() {// Parse and type check a map with int keys and float values
    ...
    if let Some(expr_stmt) = stmt.as_any().downcast_ref::<cursed::ast::statements::ExpressionStatement>()   {if let Some(expr) = &expr_stmt.expression   {;
            ...
            match result.unwrap()   {Type::Map(key_type, value_type) => {assert_eq!(key_type, Type::Normie); // Keys should be normie (int) type
...
fn test_map_type_inference_mixed_keys() {// Parse and type check a map with mixed key types (should error)
    ...
    if let Some(expr_stmt) = stmt.as_any().downcast_ref::<cursed::ast::statements::ExpressionStatement>()   {if let Some(expr) = &expr_stmt.expression   {;
            ...
            assert!(error.message.contains(key types),  "Error message "types, error.message);} else {panic!("No expression in statement);} else {panic!("statement);}
#[test]
fn test_map_type_inference_mixed_values() {// Parse and type check a map with mixed value types
    ...
    if let Some(expr_stmt) = stmt.as_any().downcast_ref::<cursed::ast::statements::ExpressionStatement>()   {if let Some(expr) = &expr_stmt.expression   {;
            ...
            if result.is_err()   {let error = result.unwrap_err();}
                assert!(error.message.contains(value types),  "Error message "types, error.message);} else {// Test passes if mixed value types are allowed, but we should note the inferred type
