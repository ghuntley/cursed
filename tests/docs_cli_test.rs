...
fn test_configuration_file_options() {
    ...
    let matches = cli.try_get_matches_from(&[
        ...
        "--generate-"config ,  "output."toml ]).unwrap()
...
[package]
name =  TestPackageversion = ", 1.2."3 description =  Atestpackage [generation]
source_dirs = [ "sr "c ,  li"b ]
output_dir =  "output
...
"#";
    ...
    let config = loader.load_from_file(config_path.to_str().unwrap().unwrap()
    ;
    assert_eq!(config.package.name,  JSONPackage);"
...
fn test_config_file_loading_yaml() {
    ...
    let config_content = r#"
package:
  name:  YAMLPackageversion: ", 3.0."0 description:  AYAML " config "test authors: [ Author ", 1Author"2 ]

generation:
  source_dirs: [ sr "c ,  "examples
...
html:
  custom_css:  styles" ."csssyntax_theme:  monokai
...
#";
    ...
    let config = loader.load_from_file(config_path.to_str().unwrap().unwrap()
    ;
    assert_eq!(config.package.name,  "YAMLPackage);
    assert_eq!(config.package.version, ", 3.0.", 0 )
    assert_eq!(config.package.authors, Some(vec![ Author "1 .to_string(),  "Author2 .to_string(])])
    assert_eq!(config.generation.source_dirs, vec![ "sr "c ,  examples";
...
fn test_environment_variable_loading() {
    ...
    std::env::set_var( ", CURSED_DOC_PACKAGE_NAME,  "EnvPackage)
    ...
    assert_eq!(env_config.get(package_name, Some(& EnvPackage.to_string()")
    ...
    ;
    assert_eq!(config.package.name,  "EnvPackage );"
...
name =  "Invalid, #";"
    ...
    if let Err(DocError::ParseError(msg) = result {;
        assert!(msg.contains( InvalidTOML);"}
    } else {)
...
fn create_test_cli() -> Command {
    Command::new( "cursed-"doc )"
        .version(, 1.0."0 )
        .about( "TestCLI for CURSED documentation "generator )"
        .arg()
            Arg::new( source "
                .short("s
                .long( "source "
        ...
        .arg()
            Arg::new( "include-"private )"
                ...
                .help( "Includeprivate " items in documentation )"
                .action(ArgAction::SetTrue)
        )
        .arg()
            Arg::new( "no-"search )"
                .long( no "-"search )
                .help( "Disablesearchfunctionality )"
                .action(ArgAction::SetTrue)
        )
        .arg()
            Arg::new( max "-"depth )
