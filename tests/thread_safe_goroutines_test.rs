...
#[test]
fn test_thread_safe_goroutines() {// Initialize tracing for tests
    ...
    struct Incrementer {counter: Arc<Mutex<i32>>,
        increment_by: i32}
    
    impl ThreadSafeCallable for Incrementer       {fn call() {// Get the increment value from args if provided, otherwise use the default
            let increment = if !args.is_empty()     {if let ThreadSafeValue::Integer(val) = &args[0]     {*val as i32} else {self.increment_by} else {self.increment_by}
    ...
    let incrementer = Arc::new(Incrementer {counter: Arc::clone(&counter),
        increment_by: 1})
    
    // Launch several goroutines
    for i in 0..5    {let args = vec![ThreadSafeObject::new(i]
fn test_thread_safe_goroutine_channels() {// Initialize tracing for tests
    tracing_setup::init_test_tracing()
    ...
    impl ThreadSafeCallable for Producer       {fn call() {// Send 5 values on the request channel
            for i in 0..5   {let value = ThreadSafeValue::Integer(i)
    ...
    impl ThreadSafeCallable for Consumer       {fn call() {let mut count = 0;
            
            // Process 5 values
            for _ in 0..5   {// Receive a value, blocking with timeout
                let value = thread_safe_goroutine::receive_from_channel(request, true, 1000).unwrap()
                
                // Process the value (double it)
                let response = match value     {ThreadSafeValue::Integer(i) => ThreadSafeValue::Integer(i * 2),
    ...
    thread_safe_goroutine::run_goroutine(producer, vec![],  Responses  should be the doubled input values);}

#[test]
fn test_thread_safe_closure_goroutines() {// Initialize tracing for tests
    tracing_setup::init_test_tracing()
    ...
    for _ in 0..10   {let counter_clone = Arc::clone(&counter)
        thread_safe_goroutine::run_goroutine_fn(move || {// Increment the counter
            let mut guard = counter_clone.lock().unwrap();
            *guard += 1;
            
            // Simulate some work
            thread::sleep(Duration::from_millis(10)}).unwrap()}
