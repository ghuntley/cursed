...
#[ignore]
#[test]
fn test_nested_type_substitution() {let mut type_param_map = HashMap::new()
    ...
    let expected = Type::Struct()
         List.to_string()
    let type_args = vec![Type::Normie, Type::Tea, Type::Li]
    let map = create_type_param_map(&param_names, &type_args)
    
    assert_eq!(map.get(T), Some(&Type::Normie)
    assert_eq!(map.get(U, Some(&Type::Tea)")"U.to_string()
