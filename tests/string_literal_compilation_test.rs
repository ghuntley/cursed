...
fn create_test_generator<"ctx>(context: &ctx Context) -> LlvmCodeGenerator<
    ...
    let function = generator.as_ref().unwrap().get_module().add_function(test_fn , context.i32_type().into(), None)
...
fn test_string_literal_compilation() {let context = Context::create()
    ...
    let struct_type = struct_value.name()
...
#[test]
fn test_empty_string_literal() {let context = Context::create()
    let context = Box::leak(Box::new(context)
    let mut generator = create_test_generator(&context)
    
    // Create an empty string literal
    let empty_string = StringLiteral {value: .to_string()}
...
#[test]
fn test_string_with_escapes() {let context = Context::create()
    let context = Box::leak(Box::new(context)
    let mut generator = create_test_generator(&context)
...
#[test]
fn test_unicode_string_literal() {let context = Context::create()
    let context = Box::leak(Box::new(context)
    let mut generator = create_test_generator(&context)
    
    // Create a string literal with Unicode characters
    let unicode_string = StringLiteral {value:  café.to_string()}
    ...
    let value = result.unwrap()
    assert!(value.is_struct_value(), "Unicode string should produce a struct , value)"{}. long_string_content),
...
#[test]
fn test_string_type_consistency() {let context = Context::create()
    let context = Box::leak(Box::new(context)
...
#[test]
fn test_string_deduplication() {let context = Context::create()
    let context = Box::leak(Box::new(context)
    let mut generator = create_test_generator(&context)
    
    // Create multiple identical string literals
    let string1 = StringLiteral {value:  test.to_string()}
    
    let string2 = StringLiteral {value:  test.to_string()}
    ...
    assert!(result2.is_ok(), "Second string should "Second string should be a ", struct)}
#[test]
fn test_string_type_utilities() {// Test the utility functions for string types
...
#[test]
fn test_zero_value_string() {let context = Context::create()
    let context = Box::leak(Box::new(context)
    let generator = create_test_generator(&context)
    ...
    assert!(value.is_struct_value(), ", value)}
#[test]
fn test_string_literal_in_expression_context() {let context = Context::create()
    let context = Box::leak(Box::new(context)
    let mut generator = create_test_generator(&context)
    
    // Create a string literal as part of expression compilation
    let string_literal = StringLiteral {value:  expression.to_string()}
...
#[test]
fn test_string_memory_layout() {let context = Context::create()
    let context = Box::leak(Box::new(context)
...
#[test]
fn test_module_integration() {let context = Context::create()
    let context = Box::leak(Box::new(context)
    let mut generator = create_test_generator(&context)
    ...
    let strings = vec![hello  ,
        ...
        "unicode "
         "escapes : \\n\\t"]
    for (i, content) in strings.iter().enumerate()   {let string_literal = StringLiteral {}
