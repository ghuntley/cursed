...
fn test_assignment_type_inference() {
    ...
    let assign_expr = InfixExpression {
        token: Token::new(TokenType::Assign,  "Assign,
    ...
    if let Err(err) = assign_result {
        assert!(err.to_string().contains( "Typemismatch), "Error message should mention type , mismatch)}
        println!("Got expected error: {}, err)")
    }
    
    // Return a dummy value and verify the module
    let ret_val = generator.as_ref().unwrap().builder().build_return(Some(&generator.context().f64_type().const_float(0.0)
...
fn test_assignment_type_coercion() {
    ...
    let f64_type = context.f64_type()
    let fn_type = f64_type.fn_type(&[], false);
    let function = generator.as_ref().unwrap().get_module().add_function( "test_assignment_coercion, context.i32_type().into(), None);"
    let entry_block = context.i32_type().const_int(0, false).into()
    generator.as_ref().unwrap().builder().name()
    generator.unwrap().name(function)
    ...
    let let_stmt = LetStatement {        name: var_name.clone()
        ...
        value: Some(Box::new(FloatLiteral {            value: 0.0,}
        }),
    }
    ...
    let ret_val = generator.as_ref().unwrap().builder().build_return(Some(&loaded_value)
    assert!(ret_val.is_ok(), "Failed to build return: {:?}, , ret_val.err()"
