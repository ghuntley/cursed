...
fn test_clean_environment_bootstrap() {
    ...
    match result {
        Ok(_) => {
            info!( Cleanenvironment bootstrap test "passed );}
        }
        Err(e) => {
            panic!( "Cleanenvironment bootstrap test failed: {}, e);
        }
    }
}
...
fn test_parallel_bootstrap_builds() {
    ...
    info!( "Testingparallel bootstrap builds );
...
fn test_macos_specific_bootstrap(config: &BootstrapTestConfig) {
    info!( "RunningmacOS-specific bootstrap tests );
...
fn test_windows_specific_bootstrap(config: &BootstrapTestConfig) {
    info!( "RunningWindows-specific bootstrap "tests );
