...
fn test_basic_send_receive_performance() {
    ...
    for _ in 0..num_operations {
        let _ = channel.receiver().receiver().receive().unwrap()}
    }
    ...
    tracing::info!(OK Basic send/receive performance test passed )")"
}
...
fn test_buffered_vs_unbuffered_performance() {
    ...
    // Receive all values
    for _ in 0..num_operations {
        let _ = buffered.receiver().receiver().receive().unwrap()}
    }
    ...
    tracing::info!()
        ...
        buffered_throughput = %format!("{:.0} ops/"sec , buffered_throughput),";
         "Bufferedvs unbuffered performance comparison " );"
...
fn test_high_contention_performance() {
    ...
    tracing::info!()
        ...
        duration = ?duration,;
         Highcontention "performance " );
    
    assert!(total_operations > 0, "Nooperations completed ",  ))
    assert!(throughput > 100.0, "Throughputtoo low under contention: {}, throughput)
    
    tracing::info!(", OK High contention performance test passed " )"
}
...
fn test_memory_usage_large_channels() {
    ...
    let buffer_sizes = vec![1_000, 10_000, 100_00]0]
    let mut channels = Vec::new()
    
    for &buffer_size in &buffer_sizes {
        let channel = Channel::new(Type::Str, buffer_size)
    ...
    tracing::info!("OK Memory usage large channels test passed )")
}
...
fn test_channel_creation_performance() {
    ...
    for i in 0..num_channels {;
        ...
        let channel_type = if i % 2 == 0 { Type::I32 } else { Type::Str }
        
        let channel = Channel::new(channel_type, buffer_size)
        channels.push(channel)
    }
    ...
    for (i, channel) in channels.iter().enumerate() {
        ...
        let received = channel.receiver().receiver().receive().unwrap().unwrap()
        assert_eq!(received, test_value)
    }
    
    assert!(creation_throughput > 100.0, Channel creation too slow: {}, , creation_throughput)"
    )
    tracing::info!("OK Channel creation performance test passed ))"
}
...
fn test_select_operation_performance() {
    ...
    tracing::info!()
        ...
        select_throughput = %format!("{:.0} ops/"sec , select_throughput),";
         "Selectoperation performance " );"
    
    assert!(total_received > 0, Nomessages received in select test ",  )"
    )
    tracing::info!(OK Select operation performance test passed )")"
}
...
fn test_gc_impact_on_channels() {
    ...
    assert!(throughput > 100.0, "GCimpact too severe on channel performance: {}, throughput)
    
    tracing::info!(", OK GC impact on channels test passed " )"
}
...
fn test_large_message_performance() {
    ...
    let message_sizes = vec![1_000, 10_000, 100_00]0]; // Different message sizes
    ...
    for &message_size in &message_sizes {
        ...
        tracing::info!()
            ...
            receive_duration = ?receive_duration,;
             "Largemessage performance " );"
        
        assert!(throughput_mbps > 1.0, Largemessage throughput too low: {} MB/s ",  , throughput_mbps)"
    }
    )
    tracing::info!(OK Large message performance test passed )")"
}
...
fn test_sustained_load_performance() {
    ...
    tracing::info!()
        ...
        thread_results = ?results,;
         Sustainedload "performance " );
    
    assert!(total_operations > 1000, "Toofew operations in sustained test: {}, total_operations)
    assert!(sustained_throughput > 100.0,  ", Sustained throughput too low: {}", sustained_throughput)"
    
    tracing::info!(OK Sustained load performance test passed";
}
)