...
use cursed::codegen::llvm::main::{LlvmCodeGenerator, InterfaceDefinition, InterfaceMethod, VTableDefinition, VTableEntry};
use cursed::runtime::interface_dispatch::{
    ...
    create_global_interface_value, dispatch_global_method
};
...
#[test]
fn test_interface_method_index_lookup() -> Result<(), CursedError> {
    ...
    // Add interface to local registry
    let interface_def = InterfaceDefinition {
        ...
        methods: vec![
            ...
            InterfaceMethod {
                name: "another_method".to_string(),
                signature: "str(str)".to_string(),
                index: 1,
            },
        ],
    };
    
    codegen.interface_registry.insert("TestInterface".to_string(), interface_def);
    
    // Test successful method lookup
    let index = codegen.get_interface_method_index("test_method")?;
    assert_eq!(index, 0);
    
    let index = codegen.get_interface_method_index("another_method")?;
    assert_eq!(index, 1);
    
    // Test method not found
    let result = codegen.get_interface_method_index("nonexistent_method");
...
fn test_global_interface_dispatch() -> Result<(), CursedError> {
    ...
    let result = dispatch_global_method(&interface_value, "global_method", &args);
    
    // Should not error (but implementation might return None for mock)
    assert!(result.is_ok());
...
fn test_vtable_registry_integration() -> Result<(), CursedError> {
    // Test vtable registry integration with codegen
    let mut codegen = LlvmCodeGenerator::new()?;
    
    // Add vtable to registry
    let vtable_def = VTableDefinition {
        interface_name: "RegistryInterface".to_string(),
        implementation_type: "RegistryStruct".to_string(),
        methods: vec![
            VTableEntry {
                method_name: "registry_method".to_string(),
                function_name: "registry_method_impl".to_string(),
                signature: "i32(i32)".to_string(),
            },
        ],
    };
    
    codegen.vtable_registry.insert("RegistryInterface::RegistryStruct".to_string(), vtable_def);
    
    // Test method index lookup from vtable registry
    let index = codegen.get_interface_method_index("registry_method")?;
    assert!(index < 256); // Should be within reasonable range
