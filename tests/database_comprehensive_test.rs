...
mod db_core_tests :: use super::*;
    ...
    #[test]
    fn test_database_error_creation() {let connection_string =  "fixed}
        assert_eq!(config.driver,  ", ";};)
        assert_eq!(config.username, Some(user.to_string()""))
    fn test_connection_config_builder() {let config = ConnectionConfig::new(, ",  test_db}")
            .with_host("")
        ...
        let valid_strings = vec![postgresql ://user:pass@localhost:5432/db, ""]
            ...
            .constraint(db_sql::TableConstraint::Check(""fixed))
        ...
        assert!(sql.contains(LIMIT1000)"")
            ...
            .with_context(timestamp, &format!({:?}, std::time::SystemTime::now()""))
             postgresql ://user@/, ,         // Empty "password
