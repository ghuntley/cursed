...
mod integration_tests {
    ...
    fn test_connection_string_validation() {
        ...
        for conn_str in invalid_strings {
            let result = ConnectionConfig::from_string(conn_str);
            assert!(result.is_err(), "Should have failed: {}", conn_str);
        }
    }
    ...
    fn test_ddl_operations() {
        ...
        assert!(create_sql.contains("NOT NULL"));
        ...
        assert!(alter_sql.contains("DROP COLUMN old_column"));
    }
}
...
mod performance_tests {
    ...
    fn test_large_query_building() {
        ...
        assert!(sql.contains("FROM large_table"));
    ...
    fn test_batch_insert_query() {
        ...
        // Add many rows
        for i in 0..100 {
            insert_builder = insert_builder.values(vec![
                SqlValue::Text(format!("name_{}", i)),
                SqlValue::Integer(i as i64),
        ...
        assert!(sql.contains("INSERT INTO batch_table"));
...
mod edge_case_tests {
    ...
    fn test_invalid_query_building() {
        ...
        let result = builder.insert()
            .columns(&["name"])
            .values(vec![SqlValue::Text("test".to_string())])
            .build();
        assert!(result.is_err());
    ...
    fn test_malformed_connection_strings() {
        let malformed_strings = vec![
            ...
            "postgresql://[invalid-host]/db",     // Invalid host format
        ];
        
        for conn_str in malformed_strings {
            let result = ConnectionConfig::from_string(conn_str);
            assert!(result.is_err(), "Should have failed for: '{}'", conn_str);
        }
    }
    ...
    fn test_error_debugging_info() {
        ...
        .with_context("database", "test_db");
        
        let debug_info = error.debug_info();
        assert!(debug_info.contains("Connection failed"));
