...
mod cli_options_tests {
    ...
    fn test_help_flag() {
        ...
        assert!(stdout.contains("CURSED Code Formatter"));
    ...
    fn test_write_flag() {
        ...
        assert!(content.contains("slay test() {"));
    ...
    fn test_line_width_option() {
        ...
        let temp_dir = TempDir::new().unwrap();
        let long_line = "slay very_long_function_name(very_long_parameter_one normie, very_long_parameter_two normie) normie { yolo very_long_expression }";
...
mod file_processing_tests {
    ...
    fn test_multiple_file_processing() {
        ...
        assert!(content1.contains("slay test1() {"));
    ...
    fn test_directory_processing() {
        ...
        assert!(main_content.contains("slay main() {"));
    ...
    fn test_file_extension_filtering() {
        ...
        assert!(csd_content.contains("slay test() {"));
...
mod error_handling_tests {
    ...
    fn test_invalid_syntax_file() {
        ...
        assert!(!output.status.success());
        let stderr = String::from_utf8(output.stderr).unwrap();
        assert!(stderr.contains("syntax error") || stderr.contains("parse error"));
    }
    ...
    fn test_permission_denied() {
        ...
        #[cfg(unix)]
        {
            ...
            assert!(!output.status.success());
            let stderr = String::from_utf8(output.stderr).unwrap();
            assert!(stderr.contains("Permission denied") || stderr.contains("permission"));
        }
    }
    ...
    fn test_invalid_command_line_options() {
        ...
        // Invalid indent size
        let output = run_formatter_cli(&["--indent-size", "0", "test.csd"]);
        assert!(!output.status.success());
        
        // Invalid line width
        let output = run_formatter_cli(&["--line-width", "10", "test.csd"]);
        assert!(!output.status.success());
...
"#";
    ...
    fn test_config_file_override() {
        ...
        let config_content = "indent_size = 8\n";
        let config_file = create_temp_file(temp_dir.path(), ".cursed-fmt.toml", config_content);
...
mod parallel_processing_tests {
    ...
    fn test_parallel_file_processing() {
        ...
        for i in 0..20 {
            ...
            assert!(content.contains(&format!("slay test{}() {{", i)));
        }
