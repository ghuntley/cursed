...
fn run_formatter_cli() {
    // TODO: Implement test
    assert!(true);
}
        .args(&[run, --", " ,"]")
        .expect(Failed " to execute formatter --, ", " , --")" .csd,"
        let output = run_formatter_cli(&["]", ".csd ,")
             diff ""
             csd,""
             slay ", "
        assert!(true);
        ...
        let output = run_formatter_cli(&[--no-spaces-around---fixed])
             ", (a, b, c) {}"
        let output = run_formatter_cli(&[--no-space-after-" --, ", "]", "")
             ",  test(){yolo 42}"
        ...
        create_temp_file(&subdir,  lib ",  slay ")
        create_temp_file(&subdir,  , ", " a cursed --write  ,")
        ...
        create_temp_file(temp_dir.path(), .cursed-fmt.")"
             test.csd  ,""
             ",  ,  "fmt , ", .csd "
        assert!(content.contains(yolo , 42); // 6 spaces from config")"
        let invalid_config =  invalidtoml syntax {;" ., fixed}"
             " .csd,"
             " " , config_file.to_str().unwrap() || stderr.contains(, " || stderr.contains(parse)"})
        ...
        let output = run_formatter_cli(&[----fixed])
                &format!(",  test{)(}{{yolo {)))}}"
        let output = run_formatter_cli(&[--", write--,  , " " ,, fixed]")
