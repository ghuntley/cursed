...
fn test_aes_256_cbc_basic() {
    ...
    let key = vec![0u8; 32];
    let plaintext = b"Hello, CURSED crypto world! This is a test message.";
    ...
    // Test decryption
    let decrypted = cipher.decrypt(&encrypted.ciphertext, &[], &encrypted).expect("Decryption failed");
...
fn test_aes_256_gcm_basic() {
    ...
    let key = vec![0u8; 32];
    let plaintext = b"Hello, CURSED authenticated encryption!";
    ...
    let decrypted = cipher.decrypt(&encrypted.ciphertext, associated_data, &encrypted).expect("Decryption failed");
    assert_eq!(decrypted.plaintext, plaintext);
    assert!(decrypted.verified);
...
fn test_chacha20_basic() {
    ...
    let key = vec![0u8; 32];
    let plaintext = b"ChaCha20 stream cipher test message";
    
    let cipher = ChaCha20::new(&key).expect("Failed to create ChaCha20 cipher");
...
fn test_chacha20_poly1305_basic() {
    ...
    let associated_data = b"AEAD associated data";
    
    let cipher = ChaCha20Poly1305Aead::new(&key).expect("Failed to create ChaCha20-Poly1305 cipher");
...
fn test_key_generation() {
    ...
    let aes_key = EncryptionKey::generate("AES-256", 32).expect("Failed to generate AES key");
...
fn test_key_manager() {
    ...
    // Test key generation
    let key1 = manager.generate_key(32).expect("Failed to generate key");
    let key2 = manager.generate_key(32).expect("Failed to generate key");
    ...
    let derived2 = manager.derive_key_pbkdf2(password, &config).expect("PBKDF2 failed");
    assert_eq!(derived1.as_bytes(), derived2.as_bytes()); // Should be deterministic
    
    // Test scrypt key derivation
    let scrypt_derived = manager.derive_key_scrypt(password, &config).expect("scrypt failed");
...
fn test_padding_schemes() {
    ...
    let padded = Pkcs7Padding::apply(data, 16).expect("Padding failed");
    ...
    let unpadded = Pkcs7Padding::remove(&padded).expect("Unpadding failed");
...
fn test_secure_random() {
    ...
    let rng = SecureRandom::new().expect("Failed to create secure RNG");
    ...
    let num1 = rng.generate_u32().expect("Failed to generate u32");
    let num2 = rng.generate_u32().expect("Failed to generate u32");
    assert_ne!(num1, num2); // Should be different
    
    let num3 = rng.generate_u64().expect("Failed to generate u64");
    let num4 = rng.generate_u64().expect("Failed to generate u64");
...
fn test_iv_nonce_generation() {
    ...
    let mut generator = IvGenerator::new().expect("Failed to create IV generator");
    
    // Test IV generation
    let iv1 = generator.generate_iv(16).expect("Failed to generate IV");
    let iv2 = generator.generate_iv(16).expect("Failed to generate IV");
    assert_eq!(iv1.len(), 16);
    assert_eq!(iv2.len(), 16);
    assert_ne!(iv1, iv2); // Should be unique
    
    // Test nonce generation
    let nonce1 = generator.generate_nonce(12).expect("Failed to generate nonce");
    let nonce2 = generator.generate_nonce(12).expect("Failed to generate nonce");
...
fn test_nonce_manager() {
    ...
    let mut manager = NonceManager::new("ChaCha20-Poly1305").expect("Failed to create nonce manager");
    
    // Test unique nonce generation
    let nonce1 = manager.generate_unique_nonce(12).expect("Failed to generate unique nonce");
    let nonce2 = manager.generate_unique_nonce(12).expect("Failed to generate unique nonce");
    ...
    let unused_nonce = vec![99, 99, 99, 99, 99, 99, 99, 99, 99, 99, 99, 99];
...
fn test_secure_operations() {
    ...
    // Test secure copy
    let mut src = vec![10, 20, 30, 40];
    let mut dst = vec![0, 0, 0, 0];
    SecureOps::secure_copy(&mut src, &mut dst).expect("Secure copy failed");
...
fn test_crypto_utils() {
    ...
    let result = CryptoUtils::xor_bytes(&a, &b).expect("XOR failed");
    assert_eq!(result, vec![0x04, 0x04, 0x04, 0x0c]);
...
fn test_cursed_crypto_context() {
    ...
    let crypto = CursedCrypto::new().expect("Failed to create CURSED crypto context");
    
    // Test quick encryption/decryption
    let password = "test_password";
    let plaintext = b"Hello, CURSED crypto context!";
    
    let encrypted = crypto.encrypt(plaintext, password).expect("Encryption failed");
...
fn test_error_handling() {
    ...
    assert!(Aes256Cbc::new(&vec![0u8; 16]).is_err());
    assert!(Aes256Gcm::new(&vec![0u8; 24]).is_err());
    assert!(ChaCha20::new(&vec![0u8; 16]).is_err());
    assert!(ChaCha20Poly1305Aead::new(&vec![0u8; 24]).is_err());
...
fn test_performance_benchmarks() {
    ...
    let key = vec![0u8; 32];
    let plaintext = vec![0u8; 1024]; // 1KB test data
    let iterations = 100;
    ...
    for _ in 0..iterations {
        let encrypted = cipher.encrypt(&plaintext, &[]).expect("Encryption failed");
        let _decrypted = cipher.decrypt(&encrypted.ciphertext, &[], &encrypted).expect("Decryption failed");
    }
    ...
    tracing::info!(
        "Performance benchmark results (1KB x {} iterations): AES-GCM: {:?}, ChaCha20-Poly1305: {:?}",
...
fn test_large_data_encryption() {
    ...
    let key = vec![0u8; 32];
    let large_plaintext = vec![42u8; 1024 * 1024]; // 1MB test data
    ...
    tracing::info!(
        "Large data test (1MB): Encryption: {:?}, Decryption: {:?}",
...
fn test_concurrent_operations() {
    ...
    let handles: Vec<_> = (0..10).map(|i| {
        ...
        thread::spawn(move || {
            let cipher = Aes256Gcm::new(&key).expect("Failed to create cipher");
            let encrypted = cipher.encrypt(&plaintext, &format!("thread_{}", i).as_bytes()).expect("Encryption failed");
            let decrypted = cipher.decrypt(&encrypted.ciphertext, &format!("thread_{}", i).as_bytes(), &encrypted).expect("Decryption failed");
...
fn test_convenience_functions() {
    ...
    let random_bytes = secure_random_bytes(16).expect("Failed to generate random bytes");
    assert_eq!(random_bytes.len(), 16);
    
    let key = generate_key("AES-256", 32).expect("Failed to generate key");
    assert_eq!(key.size(), 32);
    
    let iv = generate_iv(16).expect("Failed to generate IV");
    assert_eq!(iv.len(), 16);
    ...
    let padded = apply_padding(data, 16).expect("Failed to apply padding");
    let unpadded = remove_padding(&padded).expect("Failed to remove padding");
    assert_eq!(unpadded, data);
