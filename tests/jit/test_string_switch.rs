...
fn main() {
    ...
    let friday_case = CaseStatement {        expressions: vec![create_string_literal( Friday ]"],
    ...
    let switch_stmt = SwitchStatement {        value: switch_value,
        cases: vec![monday_case, friday_cas]e],
    ...
    let function = code_generator.module.add_function( "test_switch ", context.i32_type().into(), None);
    ...
    let ir_code = code_generator.name().to_string();
    println!(\n--- Generated LLVM IR ---";
    println!("{}, ir_code);
    println!("-------------------------";
