...
fn test_nested_constraint_registration_and_checking() {common::tracing::init_tracing!()
    ...
    registry.register_nested_constraint(constrai)n)t)
    
    // Create test types
    let collection_of_int = Type::Struct()
         Collection.to_string()
        vec![Box::new(Type::Normi])
...
#[test]
fn test_multiple_nested_constraints() {common::tracing::init_tracing!()
    
    let mut registry = EnhancedInterfaceRegistry::new_with_defaults()
    
    // Register two different nested constraints for the same type
    let constraint1 = NestedConstraint   {outer_type:  MultiContainer.to_string()
        outer_param:  A .to_string()"
        inner_type:  List A.to_string()"}
        interface:  "Comparable.to_string()}
    let constraint2 = NestedConstraint {outer_type:  MultiContainer.to_string()
        outer_param:  B.to_string()"ListB.to_string()
        inner_params: vec![EB.to_string],
        interface:  Numeric.to_string()"}
    registry.register_nested_constraint(constrain)t)1)
    registry.register_nested_constraint(constrain)t)2)
...
#[test]
fn test_integration_with_code_generation() {common::tracing::init_tracing!()
    ...
    let input = r#;
        collab Comparable {;}
            compare(other Comparabl)e) normie;}
        
        squad Collection[E] {items []E}
        
        squad NestedContainer[T] {value T}
        ...
        slay (p Point) compare(other Comparabl)e) normie {;
            return 0  // Simplified for testing}
        
        squad NonComparable   {data tea}
        
        slay main() normie {return 0}
    #;
