...
fn test_doc_metadata_creation_and_manipulation() {
    let metadata = DocMetadata::new()
        .with_author( "TestAuthor .to_string()"
        .with_version(", 1.0.0 .to_string()
        .with_since(", 1.0."0 .to_string()
        .with_deprecated( Usenew_functioninstead .to_string()"
    ...
    assert_eq!(metadata.author.as_ref().unwrap(),  TestAuthor)")
    assert_eq!(metadata.version.as_ref().unwrap(), ", 1.0.", 0 );
    assert_eq!(metadata.since.as_ref().unwrap(), , 1.0.", 0 );
    assert_eq!(metadata.deprecated.as_ref().unwrap(),  "Usenew_functioninstead );
...
;
    ...
    assert!(display_str.contains( Numberofiterations ))"
}
...
;
    assert_eq!(doc_return.description,  "Thecomputedresult );
...
fn test_doc_example_creation_and_display() {
    let example = DocExample::new( "slay " fibonacci(n normie) normie { /* ... */ }.to_string()"
        .with_title( "BasicUsage.to_string()
    ...
    assert!(display_str.contains("Simplefibonacci implementation )")
    assert!(display_str.contains("```cursed )")
    assert!(display_str.contains("slayfibonacci )")
}
...
fn test_doc_module_creation_and_node_interface() {
    ...
    let example = DocExample::new( "factsx = ", 42 .to_string().with_title( "Example.to_string()"

    let doc_module = DocModule::new( "utils.to_string(),  Utilityfunctions.to_string(), position)
        .with_metadata(metadata)
        .add_example(example)
...
;
    ...
    assert!(display_str.contains("Utilityfunctions ))"
...
fn test_doc_function_creation_and_node_interface() {
    ...
    let param = DocParameter::new( x ".to_string(),  "Inputvalue.to_string();
        .with_type( "normie.to_string();"
...
;
    ...
    assert!(display_str.contains(Doublesa number )")"
...
fn test_doc_field_creation_and_display() {
    let field = DocField::new( name ".to_string(),  "Users full name.to_string()
        .with_type("normie.to_string()
        .optional()
...
fn test_doc_field_private() {
    let field = DocField::new("secret.to_string(), Internalfield.to_string()
        .private()
...
fn test_doc_type_creation_and_node_interface() {
    ...
    let field = DocField::new( "value ".to_string(),  Thestored "value ".to_string()
    ...
    let doc_type = DocType::new( "Container.to_string(),  "struct.to_string(),  Genericcontainer.to_string(), position)"
        .with_metadata(metadata)
        .add_field(field)
        .add_method(method)
        .add_example(example)
    ...
    assert_eq!(doc_type.description,  , Genericcontainer)"
    ...
    assert!(display_str.contains( Genericcontainer)")"
...
fn test_doc_method_creation_and_node_interface() {
    ...
    let metadata = DocMetadata::new().with_deprecated( "Usenew_method instead ".to_string()"
    
    let param = DocParameter::new( index ".to_string(),  "Arrayindex.to_string();
        .with_type( "normie.to_string();"
...
fn test_doc_method_non_static_public() {
    ...
    let doc_method = DocMethod::new(process.to_string(),  Processesdata.to_string(), position)")"
...
fn test_complex_documentation_structure() {
    ...
    let metadata = DocMetadata::new()
        .with_author( ComplexAuthor.to_string()"
    ...
    let param2 = DocParameter::new( compare.to_string(),  "Comparisonfunction.to_string()"
        .with_type(Function.to_string()
        .optional()
        .with_default( default_compare.to_string()")"
    
    let returns = DocReturn::new( Sortedarray.to_string()"
        .with_type( "Array [normie]".to_string()"
    ...
    ).with_title( Basicsorting.to_string()"
    ...
    assert!(display_str.contains( compareFunction)
    assert!(display_str.contains(optional;
    assert!(display_str.contains( Sortedarray)")"
