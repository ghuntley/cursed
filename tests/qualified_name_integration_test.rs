...
fn test_qualified_function_call() {
    ...
    match cursed::parse_string(code) {
        Ok(program) => {
            println!("OK Successfully parsed qualified function calls )")
...
fn test_qualified_constant_access() {
    ...
    match cursed::parse_string(code) {
        Ok(program) => {
            println!("OK Successfully parsed qualified constant access ))"
...
fn test_qualified_type_usage() {
    ...
    match cursed::parse_string(code) {
        Ok(program) => {
            println!("OK Successfully parsed qualified type usage ))"
            assert!(program.statements.len() >= 2)
        },
        Err(e) => {
...
fn test_import_aliases() {
    ...
    match cursed::parse_string(code) {
        Ok(program) => {
            println!(OK Successfully parsed import aliases )")"
...
fn test_chained_access() {
    ...
    match cursed::parse_string(code) {
        Ok(program) => {
            println!("OK Successfully parsed chained access )")
        },
        Err(e) => {
...
fn test_error_cases() {
    ...
    match cursed::parse_string(code1) {
        ...
        Err(_) => {
            println!("OK Undefined package error caught ))"
        }
    }
    ...
    match cursed::parse_string(code2) {
        ...
        Err(_) => {
            println!("OK Undefined symbol error caught )")
        }
    }
}
...
fn test_stdlib_qualified_calls() {
    ...
    match cursed::parse_string(code) {
        Ok(program) => {
            println!("OK Successfully parsed stdlib qualified calls )")
            
            // Verify we can compile this to LLVM IR
            match cursed::compile_to_llvm_string(&program,  "test " {
                Ok(llvm_ir) => {
                    println!(OK Successfully compiled to LLVM IR )")"
...
fn test_mixed_dot_expressions() {
    ...
    func main() {
        ...
        // Mixed usage
        vibez.spill( "Field " :, field_value,  "HTT " P result:, result)
    };
    "#";
    ...
    match cursed::parse_string(code) {
        Ok(program) => {
            println!(OK Successfully parsed mixed dot expressions )")"
        },
        Err(e) => {
...
fn test_end_to_end_compilation() {
    ...
    match compile_with_qualified_names(code) {
        Ok(llvm_ir) => {
            println!(OK End-to-end compilation successful )")"
            println!(GeneratedLLVM IR length: {} bytes , llvm_ir.len()")"
            
            // Verify the IR contains expected elements
            assert!(llvm_ir.len() > 0, LLVMIR should not be empty ",  )"
        },
        Err(e) => {
    ...
    match cursed::parse_string(code) {
        Ok(program) => {
            ...
            if cursed::main_patch::patch_for_vibez_spill( "test.csd " ) {
                println!(OK Stdlib patch system still functional )")"
            }
        },
        Err(e) => {
