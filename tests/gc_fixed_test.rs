...
impl CircularNode     {fn new(} {debug!(node_id = id,  Creating "CircularNode};,  reference ";", CircularNode);)
        ...
        debug!(, ":  weak reference)"Calling :  gc.collect().expect(Failed to collect garbage , " to collect garbage)"
    ...
    debug!(object_count = initial_stats.object_count, expected_min = 5,  Verifyinginitial  object count);", "fixed"