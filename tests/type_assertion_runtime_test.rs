...
fn test_type_assertion_compilation_basic() {
    ...
    let type_assertion = TypeAssertion {        call: Box::new(StringLiteral {,            value:  "test_ "value .to_string()}
        }),
...
fn test_instance_of_check() {
    ...
    use cursed::codegen::llvm::type_assertion::InterfaceTypeAssertion;
    let result = codegen.unwrap().name(interface_value.into(),  TestType, None);"
...
fn test_interface_data_extraction() {
    ...
    let data_alloca = codegen.as_ref().unwrap().builder().build_alloca(i32_type,  "test_data ).unwrap();"
    let test_value = i32_type.const_int(42, false)
    codegen.as_ref().unwrap().builder().build_store(data_alloca, test_value).unwrap()
    
    let data_ptr = codegen.as_ref().unwrap().builder().build_pointer_cast()
        data_alloca, 
        ptr_type, 
         "data_ptr_cast
    ).unwrap()
    ...
    interface_value = codegen.as_ref().unwrap().builder().build_insert_value(interface_value, data_ptr, 0,  "data_ptr "
        .unwrap().into_struct_value()
    interface_value = codegen.as_ref().unwrap().builder().build_insert_value(interface_value, type_id, 1,  type_id)"
        .unwrap().into_struct_value()
...
fn test_type_assertion_error_handling() {
    ...
    let assertion_error = TypeAssertionError::new( "InvalidInterfaceNonExistentType ", "
