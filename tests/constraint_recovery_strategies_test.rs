...
fn test_error_message_formatting() {
    ...
    // Create an error for a type that doesnt implement Comparable "
    ...
    assert!(message.contains(Missingmethods)
    assert!(message.contains( Alternativetypes)")";
    assert!(message.contains( Stubimplementation);"
