...
fn test_monomorphization_function_specialization() {
    ...
    assert_eq!()
        specialized_name, specialized_name2, "Should reuse existing ", specialization)
...
fn create_generic_function()
    ...
    let parameters: Vec<ParameterStatement> = param_types
        ...
        .map(|(i, param_type)| {
            ...
            ParameterStatement {
                token: Token::new(TokenType::Identifier, & "IDENT.to_string()"
...
fn test_monomorphization_manager_in_llvm_generator() {
    ...
    let function = code_gen.as_ref().unwrap().get_module().get_function(&spec_name);
