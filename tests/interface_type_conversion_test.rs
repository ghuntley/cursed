...
fn test_basic_interface_type_assertion() {name tea,
        ...
        slay (p Person) toString() tea {return p.name}
        
        slay main() tea {// Create a concrete value}
    ...
    match common::run_jit_test(code)     {Ok(result) => {assert_eq!(result.as_string(), Some("Alice.to_string();":  failed: {}, e),}
#[test]
fn test_interface_type_assertion_failure() {init_test_tracing!()
    
    // Test type assertion with failure case
    let code = r#"        // Define interfaces"#
        collab Stringer {;
            toString() tea;}
        
        collab Reader {read(buf tea[]byte) lit;}
        ...
        slay (f File) read(buf tea[]byte) lit {return 42  // Dummy implementation}
        
        slay main() tea {// Create a Person value}
            sus alice = Person{name:  Alice, age: 30}
            ...
            lowkey ok {}
                return  should  not happen}"assertion failed as "expected}"#    #";
    
    match common::run_jit_test(code)     {Ok(result) => {assert_eq!(result.as_string(), Some("},
        Err(e) => panic!("Test:  failed: {}, e),"        // Define interfaces
        collab Stringer {;
            toString() tea;}
        
        collab Counter {count() lit;}
        
        // Define a type that implements both interfaces
        squad MultiImplementor {name tea,
            value lit}
        
        // Implement both interfaces
        slay (m MultiImplementor) toString() tea {return m.name}
        
        slay (m MultiImplementor) count() lit {return m.value}
        
        slay main() lit {// Create a concrete value}
            ...
            lowkey ok1 && ok2 {return m1.value + m2.value  // Should be 42 + 42 = 84}
            
            return 0}"#    #;
    match common::run_jit_test(code)     {Ok(result) => {assert_eq!(result.as_i64(), Some(84)},
        Err(e) => panic!(Test:  failed: {}, e),"        // Define interface
        collab Shape    {area() normie;}
        
        // Define concrete types
        squad Circle {radius normie}
        
        squad Rectangle {width normie,
            height normie}
        
        squad Triangle {base normie,
            height normie}
        
        // Implement interface methods
        slay (c Circle) area() normie {return 3.14159 * c.radius * c.radius}
        
        slay (r Rectangle) area() normie {return r.width * r.height}
        
        slay (t Triangle) area() normie {return 0.5 * t.base * t.height}
        
        slay getShapeType(shape Shape) tea {// Type switch using multiple type assertions
            sus _, isCircle = shape.(Circle)
            sus _, isRectangle = shape.(Rectangle)
            sus _, isTriangle = shape.(Triangle)
            
            lowkey isCircle {return  circle} elseif isRectangle     {return  rectangle"} elseif isTriangle     {return  "unknown "}
        slay main() tea {// Create different shapes}
...
#[test]
fn test_direct_interface_method_call_with_assertion() {init_test_tracing!()
    ...
    match common::run_jit_test(code)     {Ok(result) => {assert_eq!(result.as_string(), Some(GoldenRetriever.to_string()";},
        Err(e) => panic!("}
#[test]
fn test_nil_interface_assertion() {init_test_tracing!()
    
    // Test asserting on a nil interface value
    let code = r#"        // Define interface"#
        collab Processor {;
            process(data tea) tea;}
        
        // Define concrete type
        squad DataProcessor {name tea}
        
        // Implement interface
        slay (d DataProcessor) process(data tea) tea {return  Processed  :  + data"}
            
            return  nil" or wrong ""#    "nil or wrong type.to_string()")},
