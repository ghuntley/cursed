...
fn test_package_documentation_creation() {
    ...
    pkg = pkg.with_description("A test package for CURSED".to_string());
    
    assert_eq!(pkg.name, "test_package");
    assert_eq!(pkg.version, "1.0.0");
    assert_eq!(pkg.description, Some("A test package for CURSED".to_string()));
...
fn test_doc_config_builder() {
    let config = DocConfig::new("test_pkg".to_string(), "2.0.0".to_string())
        .with_description("Test package description".to_string())
    ...
    assert_eq!(config.package_description, Some("Test package description".to_string()));
...
"#";
    ...
    let config = DocConfig::new("test_package".to_string(), "1.0.0".to_string())
        ...
        .with_description("A test package for documentation generation".to_string())
    ...
    // For now, we'll test that the generator can be created and configured
...
fn test_cross_reference_building() {
    ...
    let items = vec![person_item, display_person_item];
...
fn test_documentation_item_features() {
    ...
    doc_comment.add_tag(cursed::docs::DocTag::Deprecated { 
        reason: Some("Use new_function instead".to_string()) 
    });
