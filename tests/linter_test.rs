...
mod tests ::use super::*;
    ...
    #[test]
    fn test_mixed_indentation_detection() {// common::tracing::init_tracing!()
        common::tracing::setup()
    ...
    #[test]
    fn test_severity_filtering() {// common::tracing::init_tracing!()
        common::tracing::setup()
        
        let mut linter = CursedLinter::with_config(LinterConfig {min_severity: LintSeverity::Warning,
            max_line_length: 10,
            ..Default::default()})
        ...
        for issue in &issues   {assert!(issue.severity >= LintSeverity::Warning);
        
        // Should have at least one warning (line too long)
        assert!(issues.iter().any(|issue| issue.severity == LintSeverity::Warning)}

    #[test]
    fn test_gen_z_naming_enforcement() {// common::tracing::init_tracing!()
        common::tracing::setup()
        
        let mut linter = CursedLinter::with_config(LinterConfig {enforce_genz_naming: true,
            ..Default::default()})
    ...
    #[test]
    fn test_issue_summary() {// common::tracing::init_tracing!()
        common::tracing::setup()
        
        let mut linter = CursedLinter::with_config(LinterConfig {max_line_length: 15,
            ..Default::default()});
        let source =  very  long line here\ntrailing space \n\tspaces and tabs    mixed;"Lintingcomplete);)
    ...
    #[test]
    fn test_has_errors_detection() {// common::tracing::init_tracing!()
        common::tracing::setup()
    ...
    #[test]
    fn test_empty_source() {// common::tracing::init_tracing!()
        common::tracing::setup()
    ...
    #[test]
    fn test_clean_source() {// common::tracing::init_tracing!()
        common::tracing::setup()
    ...
    #[test]
    fn test_cursed_language_keywords() {// common::tracing::init_tracing!()
        common::tracing::setup()
        ...
        let source = r#"ioslay calculate_vibes(x normie, y normie) normie {
                lowkey x > y {yolo x + y} highkey {yolo x - y}
            slay main() {sus result = calculate_vibes(10, 5)
                facts message =  calculation "capyolo result};"#        ";
        let issues = linter.lint_source(source, None).unwrap()
    ...
    #[test]
    fn test_different_severities() {// common::tracing::init_tracing!()
        common::tracing::setup()
        
        let mut linter = CursedLinter::with_config(LinterConfig {max_line_length: 10, // Will generate warnings
            ..Default::default()});
    ...
    #[test]
    fn test_utility_functions() {// common::tracing::init_tracing!()
        common::tracing::setup()
        ...
        assert!(!issues.is_empty();
        assert!(issues.iter().any(|issue| issue.rule_name ==  "whitespace)";}
    #[test]
    fn test_issue_display() {// common::tracing::init_tracing!()
        common::tracing::setup()
        ...
        if let Some(issue) = issues.first()     {let display_str = format!(")";
            assert!(display_str.contains(info "            vibe calculator
            facts PI = 3.14159

            squad Point {x normie
                y normie}

            collab Drawable {slay draw()}

            slay calculate_distance(p1 Point, p2 Point) snack {sus dx = p1.x - p2.x
                sus dy = p1.y - p2.y
                yolo (dx * dx + dy * dy).sqrt()}

            slay main() {sus point1 = Point{x: 0, y: 0}
    ...
    #[test]
    fn test_configuration_disabling() {// common::tracing::init_tracing!()
        common::tracing::setup()
        
        // Test with various checks disabled
        let config = LinterConfig {max_function_complexity: 10,
            enforce_genz_naming: false,
            check_unused_variables: false,
        ...
        for issue in &issues   {if let Some(file) = &issue.location.file     {assert!(file.contains(test.csd);
    ...
    #[test]
    fn test_empty_file() {// common::tracing::init_tracing!()
        common::tracing::setup()
