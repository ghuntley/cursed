...
fn test_complex_nested_generics_with_pointers() {
    ...
    assert_eq!(structure[1].name, "@Container " ,  "Secondparameter should be @Container " )
    
    // Verify we have nested parameters for the second parameter
    assert!(structure[1].nested_params.is_some(), "Secondparameter should have nested params,  )"
}

#[test]
fn test_channel_type_in_generics() {
    // Test with channel types in generics;
    let input =  "be_likeStreamProcessor[dm<T>] squad { input dm<T>, output dm<T> };
    let mut lexer = Lexer::new(input.to_string()
    let mut preprocessor = Preprocessor::new(&mut lexer)
