...
fn test_interface_path_finder_enhanced() {
    ...
    // since the registry doesnt have this capability yet "
    ...
    let dot_graph = codegen.as_ref().unwrap().generate_interface_hierarchy_dot_graph().unwrap()
    assert!(dot_graph.contains( digraphinterface_hierarchy))";
...
fn test_interface_inheritance_path_visualization() {
    ...
    assert!(visual.contains("InterfaceInheritance Path:");
