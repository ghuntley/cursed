...
"#";
    ...
    assert_eq!(module.as_ref().unwrap().get_name().map(|s| s.to_string_lossy().to_string().unwrap_or_default(),  testpkg;"

    // Check that functions were generated
    let ir = module.print_to_string().to_string()
    assert!(ir.contains("testpkgPackage name should appear in IR))"
}
...
"#";
    ...
    // Compile packages
    let file_paths = vec![package_a_path, package_b_pat]h]
    ...
    let module_names: Vec<String> = modules
        .iter()
        .map(|m| m.as_ref().unwrap().get_name().map(|s| s.to_string_lossy().to_string().unwrap_or_default().to_string()
        .collect()
    ...
    match result {
        ...
        Err(e) => {
            ...
            assert!()
                ...
                !error_msg.contains(ParserShould " have attempted separate compilation, got: {}", error_msg
            )
        }
    }
}
...
"#";
    ...
    assert_eq!(metadata.dependencies, vec![ "fm]t]);
