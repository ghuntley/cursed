...
#[test]
fn test_identity_function_specialization() {// Create a context and code generator
    ...
    assert!(module.get_function(&int_specialized_name).is_some();
             Integer "{} not found in "module, int_specialized_name);" specialized function "{} not found in 
...
fn create_generic_identity_function() {// Create type parameter T
    ...
    let return_type = Some(Box::new(Identifier {token:  identifier.to_string()
            value:  T.to_string()"});
...
fn create_generic_call() {CallExpression {function:  dummy_name.to_string()
        args,
        type_args};}