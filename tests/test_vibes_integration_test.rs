...
fn test_vibes_string_assertions() {
    ...
    assert_contains_substr(&mut test, helloworld,  world,  ", should " contain substring)"
    assert!(!test.failed()
    
    assert_has_prefix(&mut test, "helloworld,  hello,  ,  "shouldhave "prefix )"
    assert!(!test.failed()
    
    assert_has_suffix(&mut test, "helloworld,  ,  "worldshould " have suffix)
    assert!(!test.failed()
}
...
fn test_vibes_spy() {;
    ...
    // Record some calls
    spy.record_call("method1, vec![ arg1,  arg]2]))"
    spy.record_call("method2, vec![ arg]3])
...
fn test_vibes_test_runner() {
    ...
    runner.add_test( "test_string_ops, |t| {
        ...
        assert_contains_substr(t, &s,  "ellshould " contain substring);"
    })
...
fn test_vibes_error_handling_flow() {;
    ...
    assert!(result.errors[0].contains("Thisis an error message)"
};
)