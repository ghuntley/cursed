...
"#;
    ...
    let mut person_struct = DocumentationItem::new("Person".to_string(), ItemType::Squad, 25)
        .with_signature("squad Person".to_string())
        .with_visibility("public".to_string());

    person_struct = person_struct.add_field(FieldInfo {
        name: "name".to_string(),
        field_type: "String".to_string(),
        description: Some("The person's name, bestie".to_string()),
...
"#;
    ...
    let mut drawable_interface = DocumentationItem::new("Drawable".to_string(), ItemType::Collab, 50)
        .with_signature("collab Drawable".to_string())
        .with_visibility("public".to_string());
...
"#;
    ...
    PackageDocumentation {
        ...
        description: Some("A comprehensive test package for the CURSED programming language showcasing all features and item types.".to_string()),
        root_module,
...
fn test_readme_markdown_generation() {
    ...
    assert!(result.content.contains("make build"));
    assert!(result.content.contains("make test"));
...
fn test_cursed_syntax_highlighting() {
    ...
    assert!(result_with.content.contains("slay main"));
...
fn test_custom_metadata_handling() {
    ...
    custom_metadata.insert("author".to_string(), "CURSED Team".to_string());
    ...
    assert_eq!(result.metadata.get("author"), Some(&"CURSED Team".to_string()));
}
...
fn test_parameter_and_field_documentation() {
    ...
    assert!(result.content.contains("| `name` | `String` | public | The person's name, bestie |"));
    assert!(result.content.contains("| `age` | `u32` | public | How many years they've been slaying |"));
...
fn test_example_code_formatting() {
    ...
    assert!(example_count >= 2, "Should have multiple code examples");
}
