...
fn test_stan_expression_creation() {// Create a simple expression to be executed as a goroutine
    ...
    let stan_expr = StanExpression {token: Token::new(TokenType::Stan, stan "stan)
    ...
    let stan_expr = StanExpression {token: Token::new(TokenType::Stan,  stan,
        call: Box::new(call_expr)}"stan test_func()";" myFunction()";
    ...
    match result     {Ok(expr) => {// Check if it's a StanExpression
            if let Some(_stan_expr) = expr.as_any().downcast_ref::<StanExpression>()     {assert!(true, Successfully parsed stan , expression); else {)}
                panic!("Expression:  was not parsed as StanExpression: {}, expr.string()"Failed:  to parse stan call: {}, e)")}
