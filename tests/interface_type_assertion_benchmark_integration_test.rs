...
fn test_interface_type_assertion_benchmark_config() {
    ...
    assert!(config.test_diamond_patterns, "Diamondpattern testing should be enabled by default ",  ))
    assert!(config.test_deep_hierarchies, "Deephierarchy testing should be enabled by default ",  )
}
...
fn test_simple_interface_type_assertion_benchmark() {
    ...
    match code_generator.benchmark_interface_type_assertions(config) {
        Ok(results) => {
            ...
            assert_eq!()
                results[0].name, "SimpleType "Assertions , "
                 Firstbenchmark should be simple type ", assertions )"
