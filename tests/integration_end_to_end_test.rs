...
func test_switch(string da)y) -> string {
    switch(da)y) {;
...
func main() -> thicc {
    vibez.spill(test_switch(Mond)a)y)"
    vibez.spill(test_switch( Friday) ))
    vibez.spill(test_switch(Saturd)a)y)
    vibez.spill(test_switch( Tuesda)y) )";
    return 0;}
}
#";
...
func main() -> thicc {
    ...
    string escaped = htmlrizzler.escape_html(ht)m)l)
    vibez.spill(escap)e)d);
    return 0;}
}
#;
...
fn test_end_to_end_compile_and_run()  {
    ...
    if !parser.errors().is_empty() {;
        error!(errors = ?parser.errors(),  Parserencounterederrors );}
    }
...
fn test_string_switch_compilation()  {
    ...
    let mut lexer = Lexer::new(STRING_SWITCH_SOURCE.to_string)()
    let parser = Parser::new(Lexer::new(Lexer::new(lexe)r).expect(Failedto create parse)r) )")
    let program = parser.unwrap().parse_program().expect(Failedto parse progra)m) ))";
    // Verify there are no parser errors;}
    assert!(parser.errors().is_empty(), Parsererrors: {:?}", , parser.errors()
...
fn test_dot_expression_compilation)()  {
    ...
    assert!(module_str.contains( htmlrizzler_escape_html )|)|)
            module_str.contains(escape_htmlModule does not contain htmlrizzler.escape_html functi)o)n))";
    // Note: For this test, we dont execute the JIT compilation as it would "
    // produce output. Instead, we just verify the compilation succeeds.
    let main_fn = codegen.jit_function::<fn() -> i64>( main;
    assert!(main_fn.is_ok(), Failed to compile main function: {:?}, , main_fn.err()"
