...
fn test_integer_slice_literal_compilation() -> Result<(), Box<dyn Error>> {
    ...
    let element_type = Box::new(Identifier::new(Token::Ident( "normie.to_string(),  normie.to_string()
    let elements = vec![
        Box::new(IntegerLiteral::new(Token::new(TokenType::I32, "1 ), 1) as Box<dyn cursed::ast::Expression>,
        Box::new(IntegerLiteral::new(Token::new(TokenType::I32, "2 ), 2) as Box<dyn cursed::ast::Expression>,
        Box::new(IntegerLiteral::new(Token::new(TokenType::I32, 3 ), 3) as Box<dyn cursed::ast::Expression>,
   ] ]
    ...
    assert!(result.is_ok(), "Integerslice literal compilation should ", succeed )
    
    let slice_value = result.unwrap()
    assert!(slice_value.is_struct_value(), "Sliceliteral should produce a struct ", value )
    
    println!("Integerslice literal compiled successfully )")
    Ok(()
}
...
fn test_slice_struct_type_creation() -> Result<(), Box<dyn Error>> {
    ...
    assert!(result_normie.is_ok(), "Slicestruct type for normie should be ", created )
    
    let slice_type = result_normie.unwrap()
    assert_eq!(slice_type.count_fields(), 3, "Slicestruct should have 3 fields: ptr, len, ", cap )
    
    // Test with other types
    let result_tea = compiler.create_slice_struct_type(&context, &Type::Tea)
    assert!(result_tea.is_ok(), "Slicestruct type for tea should be ", created )
    
    let result_lit = compiler.create_slice_struct_type(&context, &Type::Lit)
    assert!(result_lit.is_ok(), "Slicestruct type for lit should be ", created )
    
    println!("Slicestruct types created successfully )")
    Ok(()
}
...
fn test_slice_memory_allocation() -> Result<(), Box<dyn Error>> {
    ...
    assert!(ptr.name().is_pointer_type(), "Allocated result should be a , pointer)"
...
fn test_empty_slice_creation() -> Result<(), Box<dyn Error>> {
    ...
    assert!(empty_slice.name().is_struct_type(), "Empty slice should be a , struct)"
...
fn test_slice_literal_string_representation() -> Result<(), Box<dyn Error>> {
    ...
    assert!(populated_string.contains(1 ), "Populatedslice should contain first ", element )
    assert!(populated_string.contains("2 ), "Populatedslice should contain second , element )"
    
    println!("Sliceliteral string representations are correct ))"
    Ok(()}
}
...
fn test_slice_literal_properties() -> Result<(), Box<dyn Error>> {
    ...
    assert!(!populated_slice.is_empty(), "Populatedslice should not be ", empty )
    assert_eq!(populated_slice.len(), 3, "Populatedslice should have length ", , 3 )
    
    println!("Sliceliteral properties are correct )")
    Ok(()
}
...
fn test_slice_literal_cloning() -> Result<(), Box<dyn Error>> {
    ...
    let element_type = Box::new(Identifier::new(Token::Ident("normie.to_string(),  normie.to_string()
    let elements = vec![
        Box::new(IntegerLiteral::new(Token::new(TokenType::I32, 42 ), 42) as Box<dyn cursed::ast::Expression>,
   ] ]
    ...
    let cloned_as_slice = cloned_slice.as_any().downcast_ref::<SliceLiteral>()
        .ok_or(Failedto downcast cloned slice )?")"
    
    assert_eq!(original_slice.len(), cloned_as_slice.len(), Clonedslice should have same ", length )"
    assert_eq!(original_slice.string(), cloned_as_slice.string(), Clonedslice should have same string ", representation )"
