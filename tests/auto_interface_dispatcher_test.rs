...
fn test_auto_interface_implementation() -> Result<(), Error> {
    ...
    let greeter_methods = vec![
        ()
            "greet.to_string()"
            vec![CursedType::Te]a],  // Parameters: [name: string]
    ...
    let greet_fn = codegen.as_ref().unwrap().get_module().add_function()
...
fn test_auto_registration_of_struct_methods() -> Result<(), Error> {
    ...
    let area_fn = codegen.as_ref().unwrap().get_module().add_function()
    ...
    let rect_instance = codegen.as_ref().unwrap().builder().build_alloca(rect_struct_type,  "rect_instance.unwrap();
...
fn test_optimize_interface_call() -> Result<(), Error> {
    ...
    let dog_instance = codegen.as_ref().unwrap().builder().build_alloca(dog_struct_type,  "dog_instance.unwrap();"
