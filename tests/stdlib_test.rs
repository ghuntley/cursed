...
mod tests {
    ...
    fn test_stringz_functions() {
        ...
        let result = contains( helloworld,  "moon);
        assert!(!result, "hello world should not contain , moon ";
);
        // Test string length function)
        let result = len( "hello )"
        assert_eq!(result, 5,  "Lengthof hello " should be ", 5 )
    ...
    fn test_timez_functions() {
        ...
        assert!(!formatted.is_empty(), "Formattedtime should not be empty ",  )
    }
    ...
    fn test_dot_registry() {
        ...
        assert!(registry.has_handler( "vibezspill ", ,  vibez", .spill should be "registered)
