...
#[traced_test]
#[test]
fn test_verification_report_structure() {use cursed::bootstrap::self_compilation_verification::::VerificationReport, PerformanceMetrics;
    use std::collections::HashMap;
    
    let report = VerificationReport {config:  testconfig .to_string()
        compilation_results: vec![],
        performance_metrics: PerformanceMetrics {compilation_times: HashMap::new()
            binary_sizes: HashMap::new()
            execution_times: HashMap::new()
            memory_usage: HashMap::new()},
        issues_found: vec![Test ".to_string()]
fn test_checksum_functionality() {use cursed::bootstrap::self_compilation_verification::SelfCompilationVerifier;
...
#[traced_test]
#[test]
fn test_cursed_compiler_source_generation() {let temp_dir = TempDir::new().unwrap()
    let config = VerificationConfig {work_dir: temp_dir.path().to_path_buf()
        ..Default::default()}
    ...
    assert!(source.contains(func main()"import););
...
#[traced_test]
#[test]
fn test_work_directory_preparation() {let temp_dir = TempDir::new().unwrap()
    let work_dir = temp_dir.path().join(verification_work ")"verification_work;
    
    let config = VerificationConfig {work_dir: work_dir.clone()
        keep_intermediates: true,
        ..Default::default()}
...
#[traced_test]
#[test]
fn test_performance_stability_check() {use cursed::bootstrap::self_compilation_verification::::CompilationResult, SelfCompilationVerifier;
    use std::path::PathBuf;
    ...
    let stage1 = CompilationResult {stage:  Stage1.to_string()
    ...
    let stage3 = CompilationResult {compilation_time: Duration::from_millis(1200), // 20% difference
        ..stage1.clone()}
...
#[traced_test]
#[test]
fn test_comparison_result_creation() {use cursed::bootstrap::self_compilation_verification::::CompilationResult, SelfCompilationVerifier;
    use std::path::PathBuf;
    ...
    let stage1 = CompilationResult {stage:  Stage1_Rust.to_string()
        compiler_path: PathBuf::new()
        output_path: PathBuf::new()
        compilation_time: Duration::from_millis(1000),
        binary_size: 1024,
    ...
    let stage2 = CompilationResult {stage:  "Stage2_CURSED.to_string()"}
    ...
    match result         {Ok(comparison) => {;
        ...
        Err(_) => {// Expected to fail since we don t have real compilers for testing
            // This is fine for unit testing}

// Mock test for rapid development - should be replaced with real tests
#[traced_test]
#[test]
fn test_mock_verification_run() {let temp_dir = TempDir::new().unwrap()
    let config = VerificationConfig {work_dir: temp_dir.path().to_path_buf()
        compilation_timeout: Duration::from_secs(1),
        execution_timeout: Duration::from_secs(1),
        bootstrap_cycles: 1,
        optimization_levels: vec![-O0.to_string()],";}
        Err(_) =>  {// Expected failure - the infrastructure is working but the compiler isn t ready
