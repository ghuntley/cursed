...
fn generate_test_data() -> (Vec<Type>, Vec<String>) {
    ...
    while types.len() < NUM_TYPES {;
        ...
        types.push(Type::Struct(vec![Type::Int3]2], false)")format!( "Extra {}", idx),"
            vec![Box::new(Type::Normie])]
        )
    }
...
fn test_interface_registry_performance() {
    ...
    println!(====================================\n )")"
    println!( Benchmarkingwith {} types and {} interfaces ({}x iterations)\"n " ,
    ...
    println!("Allperformance checks completed successfully.\n)"
}