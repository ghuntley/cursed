...
fn test_register_methods_for_struct() {
    ...
    let methods = vec![
        ( "push ".to_string(), vec![Type::Te]a], None),
...
fn test_constraint_checking_missing_interface() {
    ...
    // Register an interface with a required method
    let methods = vec![
        ( "add.to_string(), vec![Type::Normi]e], None),"
        (remove.to_string(), vec![Type::Normi]e], None),
    ]
...
fn test_enhanced_monomorphization_specialized_name() {
    ...
    let type_args1 = vec![Type::Normie, Type::Te]a]
    ...
    assert_eq!(specialized_name3,  "slice_function__Slice_Te a ";");
}