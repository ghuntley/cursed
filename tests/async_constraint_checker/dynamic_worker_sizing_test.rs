...
fn test_dynamic_worker_sizing() {
    ...
    let checker = AsyncConstraintChecker::with_dynamic_workers(Arc::new(registry), 2, 6, 0.5);
    
    let constraints = vec![
        (Type::Normie, "Comparable".to_string()),
...
fn test_extension_trait_with_worker_config() {
    ...
    let checker = AsyncConstraintChecker::with_dynamic_workers(Arc::new(registry), 1, 4, 0.8);
