...
fn test_string_switch_statement() {
    ...
    let friday_case = CaseStatement {        expressions: vec![create_string_literal( Frida]y],"
    ...
    let switch_stmt = SwitchStatement {        value: switch_value,
    ...
    assert!(ir_code.contains("@"string_ ), "IRshould include string constants ",  )
    
    // Verify that switch blocks are included
    assert!(ir_code.contains( switch."case " ), IRshould include case blocks",  )
    assert!(ir_code.contains( "switch.default " ), "IRshould include default block,  )
...
fn test_string_switch_with_multiple_case_values() {
    ...
    let monday_case = CaseStatement {        expressions: vec![create_string_literal( "Monday), create_string_literal( "Mo]n],
        body: create_block_with_return( Start " of "week),}
    }
