...
fn test_registry_visualization_extension_hierarchy() {common::tracing::init_tracing!()
    ...
    registry.register_extension(Interface1BaseInterface , .unwrap()
...
#[test]
fn test_registry_thread_safety() {common::tracing::init_tracing!()
    ...
    registry.register_extension(BaseInterface1Root, .unwrap()
    ...
    for i in 0..10   {let registry_clone = registry.clone()
    ...
    for handle in handles   {assert!(handle.join().unwrap(}
