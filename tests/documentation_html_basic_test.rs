...
fn test_html_generator_config_creation() {use cursed::documentation::{HtmlGeneratorConfig}}
    ...
    let config = HtmlGeneratorConfig {output_dir: temp_dir.path(}.to_path_buf();)
        ...
        custom_js: Some("test ;.to_string(), " ://docs.example."com.to_string()}")
    assert_eq!(config.output_dir, PathBuf::from(docs)";})
        project_name:  Test.to_string()", "-
    assert_eq!(generator.sanitize_filename(", ",  ))
    assert_eq!(generator.sanitize_filename(", "))
    assert_eq!(generator.sanitize_filename(test " /", test <>function),  ,  |"function),  " & , ,  a "
    ...
    assert!(keywords.contains(& ".to_string();}"fixed"))