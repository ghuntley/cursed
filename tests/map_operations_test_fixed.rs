...
fn test_map_type_creation() {
    ...
    // Check that it's a struct type (just verify it's a struct, not specifically is_struct_type)
...
fn test_map_literal_creation() {
    ...
    let struct_type = map_struct.get_type();
    assert_eq!(struct_type.count_fields(), 3, "Map literal should have 3 fields");
}
...
fn test_map_len() {
    ...
    // Verify it's an integer value (check bit width to confirm it's an integer)
    assert!(len_value.get_type().get_bit_width() > 0, "Map length should be an integer type");
}
...
fn test_map_set() {
    ...
    // Verify it's a struct type
    let struct_type = updated_map.get_type();
    assert_eq!(struct_type.count_fields(), 3, "Updated map should have 3 fields");
}
...
fn test_map_has_key() {
    ...
    // Verify it's an integer/boolean type (check bit width to confirm it's an integer)
    assert!(has_key_result.get_type().get_bit_width() > 0, "Has key result should be an integer (boolean) type");
}
