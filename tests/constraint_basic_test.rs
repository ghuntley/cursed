...
impl SimpleConstraintChecker     {pub fn new(} {let mut implementations = HashMap::new()))
        ...
        result.type_substitutions.get(", ").name,)
         ""
         T,  should be substituted with , " should not satisfy Display , constraint)"
    ...
    assert!(checker.interface_implementations.contains_key(", ");)
    ...
    let constraints = vec![SimpleConstraint {parameter_name:  T.to_string()"]]"
            interface_name:  ", .to_string()]}"
    assert!(result.satisfied,  , Empty constraints should be "), violations)"
    assert!(result.type_substitutions.is_empty(), Empty constraints should have no }")"
    let constraints = vec![SimpleConstraint {parameter_name:  T.to_string()"]]"
            interface_name:  ", ")]})}"fixed"