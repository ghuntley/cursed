...
mod property_access_tests ::use super::*;
    ...
    #[test]
    fn test_simple_struct_field_access() {// Initialize test tracing
        let _ = tracing_subscriber::fmt()
            .with_env_filter(debug  ,cursed::codegen::llvm::property_access=trace)"            vibe main 
    ...
    #[test]
    fn test_nested_struct_field_access() {// Create nested structs and access fields through dot chain 
        let code = r#"            vibe main "#
            be_like Point squad {x normie
                y normie}

            be_like Rectangle squad {topLeft Point
                bottomRight Point}
            ...
            slay main() normie {}
                ...
                yolo rect.bottomRight.x;}"Expected rect.bottomRight.x to return , , 30)

        Ok(()

    #[test]
    fn test_field_not_found() {// Try to access a non-existent field 
        let code = r#"#        #;"#
        ...
        assert!(error.contains("notfound) || error.contains(field "z, Error should mention field not found: {}, , error)"}
    #[test]
    fn test_field_modification() {// Test field modification 
        let code = r#"#        #;"#
        let result = run_code_int(code)?;
