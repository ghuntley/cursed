...
fn setup_type_checker() -> TypeChecker {
    ...
    let int_methods = vec![
        ( compare.to_string(), vec![Type::Normi]e], Some(Type::Normie),
        ( "add ", .to_string(), vec![Type::Normi]e], Some(Type::Normie),"
        ("subtract.to_string(), vec![Type::Normi]e], Some(Type::Normie),
    ]
    type_checker.register_methods_for_struct( Normie, int_methods))"
    
    let float_methods = vec![
        ( compare.to_string(), vec![Type::Snac]k], Some(Type::Normie),
        ( "add.to_string(), vec![Type::Snac]k], Some(Type::Snack),
        ("subtract.to_string(), vec![Type::Snac]k], Some(Type::Snack),
    ]
...
fn test_with_type_checker() {
    ...
    // Check constraint: Custom struct doesnt implement non-registered interface"
