...
#[path = "common/mod.rs"]
fn test_multiple_senders_single_receiver() {common::tracing::init_tracing!()
    ...
    let handles: Vec<_> = (0..num_senders).map(|sender_id| {let tx = tx.clone()
        let barrier = Arc::clone(&barrier)
        
        thread::spawn(move || {)
            barrier.wait(); // Synchronize start
            
            for msg_id in 0..messages_per_sender   {let value = (sender_id * 1000 + msg_id) as i32;
                loop {match tx.send_timeout(value)     {SendResult::Sent => {tracing::debug!(sender_id, msg_id,  Sent message);
                            break;}
                        SendResult::WouldBlock(_) => {thread::sleep(Duration::from_millis(1)
                            continue;}
                        SendResult::Closed(_) => {panic!("})}).collect()
    // Wait for all senders to be ready
    barrier.wait()
    
    // Receive all messages
    let mut received_values = Vec::new()
    for _ in 0..(num_senders * messages_per_sender)   {loop {match rx.try_receive()     {ReceiveResult::Received(value) => {received_values.push(value);
                    break;}
                ReceiveResult::WouldBlock => {thread::sleep(Duration::from_millis(1)
                    continue;}
...
#[test]
fn test_single_sender_multiple_receivers() {common::tracing::init_tracing!()
    ...
    let handles: Vec<_> = (0..num_receivers).map(|receiver_id| {let rx = rx.clone()
        ...
        thread::spawn(move || {)
            ...
            loop {match rx.try_receive()     {ReceiveResult::Received(value) => {local_count += 1;
    ...
    thread::spawn(move ||   {for i in 0..total_messages   {}
            let value = format!(Message{}, i)
            loop {match tx.send_timeout(value.clone()     {SendResult::Sent => {tracing::debug!(i,  Sentmessage);;
                        break;}
                    SendResult::WouldBlock(_) => {thread::sleep(Duration::from_millis(1)
                        continue;}
...
fn test_producer_consumer_pattern() {common::tracing::init_tracing!()
    ...
    // Producer thread
    let producer_handle = {let tx = tx.clone()
        let producer_done = Arc::clone(&producer_done)
        
        thread::spawn(move || {for i in 0..num_items   {loop {match tx.send_timeout(i)     {SendResult::Sent => break,
                        SendResult::WouldBlock(_) => {thread::sleep(Duration::from_millis(1);
                            continue;}
                        SendResult::Closed(_) => return,}
                
                if i % 10 == 0     {thread::sleep(Duration::from_millis(1) // Simulate work}
            drop(tx); // Close channel
            producer_done.store(true, Ordering::SeqCst)
            tracing::info!(Producer:  finished);})}
    
    // Consumer thread
    let consumer_handle = {let rx = rx.clone()
        let consumer_done = Arc::clone(&consumer_done)
        
        thread::spawn(move || {let mut consumed = Vec::new()
            
            loop {match rx.try_receive()     {ReceiveResult::Received(value) => {consumed.push(value)
                        if consumed.len() % 10 == 0     {thread::sleep(Duration::from_millis(1) // Simulate processing}
    ...
    for (i, &value) in consumed_items.iter().enumerate()   {assert_eq!(value, i as i32)}
    
    tracing::info!(OK Producer consumer pattern test passed);}

#[test]
fn test_worker_pool_pattern() {common::tracing::init_tracing!()
    ...
    let worker_handles: Vec<_> = (0..num_workers).map(|worker_id| {let work_rx = work_rx.clone()
        let result_tx = result_tx.clone()
        
        thread::spawn(move || {)
            let mut jobs_processed = 0;
            
            loop {match work_rx.try_receive()     {ReceiveResult::Received(job) => {// Simulate work: square the number
                        let result = job * job;
                        loop {match result_tx.send_timeout(result)     {SendResult::Sent => break,
                                SendResult::WouldBlock(_) => {thread::sleep(Duration::from_millis(1)
                                    continue;}
                                SendResult::Closed(_) => return jobs_processed,}
                        jobs_processed += 1;
                        tracing::debug!(worker_id, job, result,  Processed job);
            jobs_processed})}).collect()
    // Send work
    let sender_handle = {let work_tx = work_tx.clone()
        thread::spawn(move || {for job in 1..=num_jobs   {loop {match work_tx.send_timeout(job)     {SendResult::Sent => break,
                        SendResult::WouldBlock(_) => {thread::sleep(Duration::from_millis(1);
                            continue;}
                        SendResult::Closed(_) => return,}
    ...
    for _ in 0..num_jobs   {loop {match result_rx.try_receive()     {ReceiveResult::Received(result) => {results.push(result);
                    break;}
                ReceiveResult::Closed => break,
                ReceiveResult::WouldBlock => {thread::sleep(Duration::from_millis(1)
                    continue;}
...
fn test_deadlock_prevention() {common::tracing::init_tracing!();
    ...
    let handles: Vec<_> = (0..2).map(|thread_id| {let tx = tx.clone()
        let deadlock_detected = Arc::clone(&deadlock_detected)
        
        thread::spawn(move || {let timeout_start = Instant::now()
            let timeout_duration = Duration::from_millis(50);
            let value = thread_id;
            
            loop {match tx.send_timeout(value)     {SendResult::Sent => {tracing::debug!(thread_id,  Send succeeded);
                        return;}
                    SendResult::WouldBlock(_) => {if timeout_start.elapsed() > timeout_duration     {deadlock_detected.store(true, Ordering::SeqCst)
...
#[test]
fn test_high_contention_scenario() {common::tracing::init_tracing!()
    ...
    let handles: Vec<_> = (0..num_threads).map(|thread_id| {let tx = tx.clone()
        let rx = rx.clone()
        ...
        thread::spawn(move || {)
            ...
            if is_sender     {// Sender thread
                for i in 0..operations_per_thread   {if start_time.elapsed() > timeout_duration     {;
                        break;}
                    
                    let value = (thread_id * 1000 + i) as i32;
                    
                    match tx.send_timeout(value)     {SendResult::Sent => {total_sent.fetch_add(1, Ordering::SeqCst)}
                        SendResult::WouldBlock(_) => break,
                        SendResult::Closed(_) => break,} else {// Receiver thread
                for _ in 0..operations_per_thread   {if start_time.elapsed() > timeout_duration     {break;}
                    
                    match rx.try_receive()     {ReceiveResult::Received(_) => {total_received.fetch_add(1, Ordering::SeqCst)}
                        ReceiveResult::WouldBlock => break,
                        ReceiveResult::Closed => break,})}).collect()
...
#[test]
fn test_fan_in_pattern() {common::tracing::init_tracing!()
    ...
    let input_handles: Vec<_> = (0..num_inputs).map(|input_id| {let output_tx = output_tx.clone()
        
        thread::spawn(move || {for i in 0..messages_per_input   {}
                let message = format!(Input{} message {}, input_id, i)
                loop {match output_tx.send_timeout(message.clone()     {SendResult::Sent => break,
                        SendResult::WouldBlock(_) => {thread::sleep(Duration::from_millis(1);
                            continue;}
                        SendResult::Closed(_) => return,}
                thread::sleep(Duration::from_millis(1)})}).collect()
    
    // Collect all merged messages
    let mut collected_messages = Vec::new()
    
    for _ in 0..(num_inputs * messages_per_input)   {let timeout_start = Instant::now()
        let timeout_duration = Duration::from_millis(100)
        
        loop {match output_rx.try_receive()     {ReceiveResult::Received(msg) => {collected_messages.push(msg);
                    break;}
                ReceiveResult::WouldBlock => {if timeout_start.elapsed() > timeout_duration     {break;}
                    thread::sleep(Duration::from_millis(1)
                    continue;}
                ReceiveResult::Closed => break,}
    
    // Wait for all input threads to complete
    for handle in input_handles   {handle.join().unwrap()}
...
#[test]
fn test_fan_out_pattern() {common::tracing::init_tracing!()
    ...
    let output_handles: Vec<_> = (0..num_outputs).map(|output_id| {let input_rx = input_rx.clone()
        
        thread::spawn(move || {let mut processed = Vec::new()
            
            loop {match input_rx.try_receive()     {ReceiveResult::Received(n) => {;
    ...
    // Send input data
    let sender_handle = {let input_tx = input_tx.clone()
        thread::spawn(move || {for i in 1..=total_messages   {loop {match input_tx.send_timeout(i)     {SendResult::Sent => break,
                        SendResult::WouldBlock(_) => {thread::sleep(Duration::from_millis(1);
                            continue;}
                        SendResult::Closed(_) => return,}
