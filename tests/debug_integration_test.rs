...
fn test_complete_debug_pipeline() {
    ...
    let result = manager.setup_function_debug(function, ")
        ", "
        ", "
    let source_file = temp_dir.path().join(")
    writeln!(file, "// Complex CURSED ")
    writeln!(file, "")
    ...
    writeln!(file, ")"
    assert!(snippet.contains(" + fibonacci(n-2);))
    assert!(snippet2.contains(", " result = fibonacci(10);))
    let main_file = temp_dir.path().join(")
    let lib_file = temp_dir.path().join(")
    writeln!(file, ", " \\lib\")"
    writeln!(file, ")"
    writeln!(file, ",  main() {{"}}})
