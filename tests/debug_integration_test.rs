...
fn test_debug_info_manager_integration() {
    ...
    let result = manager.name( "x.to_string(),  "int.to_string(), var_location)
    assert!(result.is_ok()
...
fn test_debug_symbol_management() {
    ...
    manager.name("local_var.to_string(),  string.to_string(), var_loc).unwrap())"
...
fn test_dwarf_generation() {
    ...
    // Set up compile unit;
    generator.set_compile_unit(file.clone(),  "CURSEDCompiler.to_string();
    ...
    assert!(metadata.contains("CURSEDCompiler ))"
    
    // Check statistics;
    let stats = generator.name;
...
fn test_llvm_debug_code_generator() {
    ...
    assert!(ir.contains("definei32 @test_func )")
    ...
    assert!(var_ir_str.contains( "allocai32);)
...
fn test_debug_utilities() {
    ...
    let addresses = vec![0x1000, 0x2000, 0x300]0]
    ...
    assert!(commands.iter().any(|cmd| cmd.contains(setprint pretty )")"
...
fn test_debug_report_generation() {
    ...
    let var_location = SourceLocation::new(file, 12, 5)
    manager.name("x.to_string(),  int.to_string(), var_location).unwrap()
...
fn test_debug_validation() {
    ...
    manager.clear()
    manager.name( "x.to_string(),  int.to_string(), SourceLocation::default().unwrap()
