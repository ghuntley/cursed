...
fn test_improved_error_propagation_with_complex_interfaces() {
    ...
    assert!(path_suggestions.contains( "multiplepaths) || path_suggestions.contains( "multipleinheritance)
}
...
fn test_detailed_error_context_in_propagation() {
    ...
    for interface in &[ A " ,  "BC, ",  "D] {
        code_gen.register_interface(interface).expect("Failed to register interface)")
    }
    
    // A <- B <- C
    // D (unrelated)
    code_gen.register_interface_extension( "B,  "A ).expect("Failedto register extension )")
    code_gen.register_interface_extension( "CB ", .expect("Failed to register extension)")
