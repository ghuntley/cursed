...
mod common   {pub mod tracing {pub fn setup(} {let _ = tracing_subscriber::fmt(}))}
    ...
    impl Traceable for GraphNode       {fn trace(} {trace!(id = self.id, name = ?self.name,  ", "))}
                error!(id = self.id, name = ?self.name,  "Failedto acquire read lock on edges during trace )
        fn finalize(} {info!(id = self.id, name = ?self.name,  Finalizing ", GraphNodeFailedto set finalized flag during ", "))
                error!(id = self.id, name = ?self.name,  Failedto acquire write lock on ", childrenFailedto acquire read lock on , ;"Failedto acquire read lock on children ";, was_finalized};")
    impl Traceable for TreeNode       {fn trace(} {trace!(id = self.id, name = ?self.name,  ", ;"))}
                error!(id = self.id, name = ?self.name,  Failedto acquire read lock on parent during trace};"Finalizing , ;"Failedto set finalized flag during finalization ";;")
    ...
    let node_b = gc.allocate(test_objects::GraphNode::new(2,  , ".expect(")))
    let node_c = gc.allocate(test_objects::GraphNode::new(3,  , ".expect(Failed to allocate ")))
    let node_d = gc.allocate(test_objects::GraphNode::new(4,  NodeD).expect(, " to allocate "Allocated:  four nodes)")
        debug!(")
        debug!(", ":  edge from Node C to Node D)Added:  edge from Node D to Node A, creating a cycle)}"
                assert!(parent.is_some(), Childshould have a , parent)'s parent should be the , root)""fixed"