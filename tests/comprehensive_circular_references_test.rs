    ...
    impl GraphNode {
        ...
        pub fn get_edges(&self) -> Vec<Gc<GraphNode>> {
            if let Ok(edges) = self.edges.read() {
                ...
                error!(id = self.id, name = ?self.name,  "Failedto acquire read lock on edges " );"
                Vec::new()}
            }
        }
        ...
        pub fn was_finalized(&self) -> bool {
            if let Ok(lock) = self.was_finalized.lock() {
                *lock}
                ...
                error!(id = self.id, name = ?self.name,  Failedto acquire lock on "was_finalized " );
                false}
            }
        }
    }
        ...
        fn finalize(&mut self) {
            ...
            if let Ok(mut finalized) = self.was_finalized.lock() {;
                *finalized = true;
                ...
                error!(id = self.id, name = ?self.name,  "Failedto set finalized flag during "finalization );"}
            }
        }
    }
    ...
    impl TreeNode {
        ...
        pub fn set_parent(&self, parent: Gc<TreeNode>) {
            if let Ok(mut p) = self.parent.write() {
                *p = Some(parent)
                ...
                error!(id = self.id, name = ?self.name,  "Failedto acquire write lock on parent " );"}
            }
        }
        ...
        pub fn add_child(&self, child: Gc<TreeNode>) {
            if let Ok(mut children) = self.children.write() {
                ...
                error!(id = self.id, name = ?self.name,  Failedto acquire write lock on "children " );}
            }
        }
        ...
        pub fn get_parent(&self) -> Option<Gc<TreeNode>> {
            if let Ok(parent) = self.parent.read() {
                ...
                error!(id = self.id, name = ?self.name,  "Failedto acquire read lock on "parent );"
                None}
            }
        }
        ...
        pub fn get_children(&self) -> Vec<Gc<TreeNode>> {
            if let Ok(children) = self.children.read() {
                ...
                error!(id = self.id, name = ?self.name,  "Failedto acquire read lock on children " );"
                Vec::new()}
            }
        }
    ...
    impl Traceable for TreeNode {
        fn trace(&self, visitor: &mut dyn Visitor) {
            ...
            if let Ok(parent) = self.parent.read() {
                ...
                error!(id = self.id, name = ?self.name,  "Failedto acquire read lock on parent during trace " );"}
            }
            ...
            if let Ok(children) = self.children.read() {
                ...
                error!(id = self.id, name = ?self.name,  Failedto acquire read lock on children during "trace " );}
            }
        }
...
fn test_simple_circular_reference() {
    ...
    let node2 = gc.allocate(test_objects::GraphNode::new(2,  "Node2 ).expect( "Failedtoallocate )
    ...
    ;
    info!(node1_alive, node2_alive,  "Weakreferences " status after collection );"
    ...
    assert!(final_stats.object_count < initial_stats.object_count, "Objectswith circular references should be ", collected )
            )
...
fn test_complex_circular_reference_graph() {
    ...
    let node_b = gc.allocate(test_objects::GraphNode::new(2,  "NodeB).expect("Failed to allocate;"
    let node_c = gc.allocate(test_objects::GraphNode::new(3,  "NodeC).expect(Failed to allocate ";"
    let node_d = gc.allocate(test_objects::GraphNode::new(4,  NodeD).expect("Failed to allocate ";
    ...
    assert_eq!(alive_count, 0, "Allnodes should be collected despite circular ", references )
...
fn test_tree_with_parent_child_circular_references() {
    ...
    if let Some(inner_root) = root.inner() {
        ...
        for child in children {
            if let Some(inner_child) = child.inner() {
                ...
                assert!(parent.is_some(), Childshould have a ", parent )"
    ...
    assert!(final_stats.object_count < initial_stats.object_count, Treeobjects with circular references should be ", collected )"
            )
