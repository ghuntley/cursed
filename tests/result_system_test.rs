...
fn test_result_basic_operations() {
    ...
    assert_eq!(ok_expr.string, "expected")
    ...
    assert_eq!(wildcard_pattern.string, "expected"), ""
    let ok_body = Box::new(NumberLiteral::new(", 42))"
    let ok_pattern = ResultPattern::Ok(Box::new(Identifier::new("test")))
    ...
    assert_eq!(root.error_code, "expected"), , ""
    ...
    let chain_error = parse_and_process(")
        .map_err(|_| CursedError::Parse(",  number))"
            Result::Err(",  by ")
    assert_eq!(bad_result.unwrap_err(), ",  by zero"")