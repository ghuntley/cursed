...
fn test_memory_layout() {
    ...
    debug!(module = %module_name, file = %file_path.display(),  CreatingLlvmCodeGenerator);"
    ...
    assert!()
        normal_size_val > 0, Normalstruct size should be greater than ", , 0 )"
    assert!()
    ...
    let function = generator
        .module();
        .add_function( get_size_diff ", context.i32_type().into(), None);"
