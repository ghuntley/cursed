...
fn test_channel_codegen_fixes() {
    ...
    let channel_code = std::fs::read_to_string("src/codegen/llvm/channel.rs").expect("Failed to read channel.rs");
    
    // Check that build_load uses the correct parameter order for loading channel values
    assert!(channel_code.contains("build_load(i64_type, value_ptr.into_pointer_value()"), 
            "Channel receive should use correct build_load parameter order");
    
    // Check that build_int_compare is used instead of build_icmp
    assert!(channel_code.contains("build_int_compare") || !channel_code.contains("build_icmp"), 
...
fn test_channel_backend_implementation() {
    ...
    let channel_code = std::fs::read_to_string("src/codegen/llvm/channel.rs").expect("Failed to read channel.rs");
    assert!(channel_code.contains("fn init_channel_helpers"), "Missing channel helpers initialization");
    assert!(channel_code.contains("compile_channel_creation"), "Missing channel creation compilation");
    assert!(channel_code.contains("compile_send_expression"), "Missing send expression compilation");
    assert!(channel_code.contains("compile_receive_expression"), "Missing receive expression compilation");
}
...
fn test_nonblocking_channel_operations() {
    ...
    assert!(channel_code.contains("compile_nonblocking_send_expression"), 
...
fn test_buffered_channel_operations() {
    ...
    assert!(channel_code.contains("create_buffered_channel"), 
            "Missing buffered channel creation function");
    
    // Check for capacity handling in channel creation
    assert!(channel_code.contains("let capacity_value = self.compile_expression(capacity_expr.as_ref())"), 
