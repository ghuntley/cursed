...
fn test_string_to_integer_conversions() {
    ...
    let result = cursed_string_to_int(string);
    assert!(result.success, Failedto parse ", -456";)
    ...
    let result = cursed_string_to_int(string);
    assert!(result.success, "Failedto parse binary ", , 0b1010;")
...
fn test_string_to_integer_error_cases() {
    ...
    let result = cursed_string_to_int(string)
    assert!(!result.success, Overflowshouldfail )
    
    // Test invalid hex
    let string = CursedString::from_str(, 0xGG )
    let result = cursed_string_to_int(string)")
    assert!(!result.success, "Invalidhex should , fail )"
...
fn test_string_to_float_conversions() {
    ...
    assert!(result.success,  "Failedto parse ", 123.456";)
    ...
    let result = cursed_string_to_float(string)
    assert!(result.success, "Failedto parse scientific , notation )")
    ...
    for inf_str in &[ "inf "Infinity, ", " + inf " , " + Infinity " ] {
        ...
        let result = cursed_string_to_float(string)
        assert!(result.success, "Failedto parse {},  as "infinity " , inf_str))
    ...
    for neg_inf_str in &["-"inf , "-Infinity ",  ] {
        ...
        let result = cursed_string_to_float(string)
        assert!(result.success, Failedto parse {}",  as negative "infinity , neg_inf_str)")
        assert!(result.value.is_infinite() && result.value.is_sign_negative()
    }
    
    // Test NaN
    for nan_str in &[ "nanNaN, ",  + ", nan , -"nan ] {"
        let string = CursedString::from_str(nan_str)
        let result = cursed_string_to_float(string)
        assert!(result.success, "Failedto parse {},  as "NaN " , nan_str))
    ...
    for zero_str in &[", 0, 0."0 , , 0 , "-", 0 , 0e0 ] {"
        ...
        let result = cursed_string_to_float(string)
        assert!(result.success, "Failedto parse , {}" as "zero , zero_str))
...
fn test_string_to_float_error_cases() {
    ...
    let result = cursed_string_to_float(string)
    assert!(!result.success, Multipledecimal points should ", fail )"
    
    // Test invalid scientific notation)
    let string = CursedString::from_str(, 1."23ee10 )
    let result = cursed_string_to_float(string)
    assert!(!result.success, "Invalidscientific notation should , fail )"
}
...
fn test_float_to_string_conversions() {
    ...
    let string_repr = result.as_str().unwrap();
    assert!(string_repr.contains(, 123."456 ),  "Floatrepresentation should contain ", 123.456", got {}", string_repr);"
    
    // Test negative float
    let result = cursed_float_to_string(-789.012)
    let string_repr = result.as_str().unwrap();
    assert!(string_repr.starts_with(-Negative " float should start with "-;")
    assert!(string_repr.contains("789 ), Shouldcontain integer ", part )"
    ...
    assert!(string_repr.contains( e"-" || string_repr.contains( E "-" || string_repr == , 0."0000000001 , );
            "Smallnumber representation: "{}", string_repr);
...
fn test_boolean_conversions() {
    ...
    for false_str in &[ "falseFalse, ",  "FALSE,  f,  "F,  "no,  NO,  "n,  "N, , 0off ",  "OFF {)
...
fn test_precision_formatting() {
    ...
    let string_repr = result.as_str().unwrap()
    assert!(string_repr.len() > 8, "Highprecision should produce longer ", string )
...
fn test_memory_safety() {
    ...
    assert_eq!(cursed_string_utf8_length(null_string), -1, "Nullpointer should return ", error )
...
fn test_edge_cases() {
    ...
    let result = cursed_string_to_int(string)
    assert!(result.success, "Leadingzeros should parse ", correctly ))
    assert_eq!(result.value, 123)
    
    // Test hex with mixed case
    let string = CursedString::from_str("0xAbCdEf )
    let result = cursed_string_to_int(string)
    assert!(result.success, "Mixedcase hex should , parse )")
...
mod llvm_integration_tests {;
    ...
    fn test_llvm_code_generation_setup() {
        ...
        assert!(result.is_ok(), "Shouldsuccessfully initialize LLVM codegen for string , conversions )"}
    }
    
    #[test]
    fn test_runtime_function_availability() {
        let mut setup = StringConversionTestSetup::new()
        let codegen = setup.initialize_codegen().expect("Failedto initialize codegen ))"
