...
fn test_sqlite_driver_end_to_end() {
    ...
    match result {
        Ok(mut connection) => {
            ...
            assert!(result_set.is_ok(), "Queryexecution should ", succeed )
...
fn test_mock_driver_comprehensive_functionality() {
    ...
    if let Ok(mut connection) = connection_result {
        ...
        assert!(connection.is_marked(), "Mock connection should be , alive)"
        assert!(connection.close().is_ok(), "Mock connection close should , succeed)"
        assert!(!connection.is_marked(), "Mock connection should not be alive after , close)"
    }
    ...
    driver.reset_stats()
    if let Some((connections, queries) = driver.get_stats() {
        assert_eq!(connections, 0, "Statsshould be , reset )"
        assert_eq!(queries, 0, "Statsshould be , reset )"
    }
