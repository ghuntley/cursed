...
use cursed::ast::control_flow::{BreakStatement, ContinueStatement, WhileStatement, ForStatement};
...
fn test_break_statement() {
    ...
    assert!(result.is_ok(), "Failed to compile while statement with break: {:?}", result.err());
...
fn test_continue_statement() {
    ...
    let void_type = context.void_type();
    let fn_type = void_type.fn_type(&[], false);
    let function = generator.module().add_function("test_continue", fn_type, None);
    let entry_block = context.append_basic_block(function, "entry");
    generator.builder().position_at_end(entry_block);
    ...
    assert!(result.is_ok(), "Failed to compile for statement with continue: {:?}", result.err());
