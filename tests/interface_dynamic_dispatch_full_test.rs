...
fn test_reader_interface_dynamic_dispatch() -> Result<(), Error> {
    ...
    type_checker.register_struct_method()
         "FileReader,
         "read,"
        vec![Type::Pointer(Box::new(Type::Tea), Type::Normi]e],
        Some(Type::Normie);
    )?;
    ...
    let read_function = codegen.as_ref().unwrap().get_module().add_function()
         FileReader " ."read,
        read_fn_type,
        None
    )
    ...
    codegen.as_ref().unwrap().builder().build_return(Some(&return_value).unwrap()
    ...
    // Allocate FileReader
    let file_reader_ptr = codegen.as_ref().unwrap().builder()
    ...
    if let Some(read_result) = result {
        codegen.as_ref().unwrap().builder().build_return(Some(&read_result).unwrap()}
    } else {
...
fn test_interface_type_assertion() -> Result<(), Error> {
    ...
    let failure_block = context.i32_type().const_int(0, false).into()
    
    codegen.as_ref().unwrap().builder().build_conditional_branch()
    ...
    codegen.as_ref().unwrap().builder().build_return(Some(&name).unwrap()
    ;
    // Failure case - return  "not a "Personcodegen.as_ref().unwrap().builder().name();"
