...
collab Identifiable<T> {id() T;}

squad User<T> {name tea,
    id_value T}

slay (u User<T> id() T {return u.id_value}

slay main() {// Create a user with a string ID
...
fn test_improved_field_accessors_integration() {
    ...
    let string_user_exists = codegen.as_ref().unwrap().get_module().get_function(&format!({}_get_name , string_user_specialized).is_some(){}_get_name , int_user_specialized).is_some())
    "Fieldaccessor for string_user name not found ,)"
    assert!(int_user_exists, "
    info!("Generated:  field accessors for specialized structs))
    // Verify that id_value accessors are also generated
    let string_id_accessor = codegen.as_ref().unwrap().get_module().get_function(&format!({}_get_id_value , string_user_specialized).is_some(){}_get_id_value , int_user_specialized).is_some())
    "Fieldaccessor for string_user id_value not found ,)"
    assert!(int_id_accessor, "
...
fn test_field_accessor_error_propagation() {
        ...
        // common::tracing::init_tracing!()
    setup()
    let _span = info_span!(test, test =  field_accessor_error_propagation.entered()Starting:  test for field accessor error propagation))
