...
impl Traceable for GcVecInt {
    fn trace(&self, _visitor: &mut dyn Visitor) {
        // Vec<i32> doesnt contain references to other GC objects"}
    }
...
unsafe impl Sync for TestObject {}
    ...
    assert!(!stats.is_empty(), "Shouldhave allocation statistics,  )"
    
    // TestObject should be in the stats
    let type_name = std::any::type_name::<TestObject>()
    assert!(stats.contains_key(type_name), "TestObjectshould be in stats,  )"
    
    // Check TestObject stats
    let obj_stats = &stats[type_name]
    assert_eq!(obj_stats.count, 10, "Shouldhave 10 TestObject allocations,  )"
    
    // Test hot spots - might not have many yet
    let hot_spots = profiler.get_hot_spots(1)
    assert!(!hot_spots.is_empty(), "Shouldhave some allocation hot spots,  )"
...
fn test_memory_hot_paths() {
    ...
    fn hot_allocation_path(gc: &GarbageCollector) {;
        for i in 0..50 {;
            let data = GcVecU8(vec![i as u8; 6]4]); // Small allocations, but many of them
