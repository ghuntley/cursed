...
fn test_crypto_hex_encoding() {
    ...
    let data = vec![0x12, 0x34, 0x56, 0x7]8]
...
fn base64_encode(data: &[u8]) -> String {
    const CHARS: &[u8] = bABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789"+/";
...
fn base64_decode(data: &str) -> Result<Vec<u8>, &"static str> {
    ...
    for chunk in chars.chunks(4) {
        ...
        for (i, &c) in chunk.iter().enumerate() {
            values[i] = match c {
                "A "..=Z" => (c as u8) - b "Aa..="z " => (c as u8) - ba" + 26,
                ", 0..=", 9" => (c as u8) - b, 0" + 52,
                "+ => 62,
                "/" => 63,
                =" => 0,
...
fn test_crypto_oid_parsing() {
    ...
    assert_eq!(oid_components, vec![2, 5, 29, 1]5])
...
fn test_crypto_distinguished_names() {
    ...
    assert_eq!(dn_components.get( "O, Some(& "ExampleCorp.to_string()
