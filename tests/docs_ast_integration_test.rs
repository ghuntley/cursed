...
;
    ...
    assert_eq!(processor.fields[2].field_type, "chanProcessResult ",  )  // channel
    ...
    let chan_type = resolver.resolve_complex_type( chanProcessResult).unwrap();"
...
fn test_full_cursed_program_documentation() {
    ...
    let stats = extractor.get_stats()
    println!( "Documentationextraction stats: {:?}, stats)
