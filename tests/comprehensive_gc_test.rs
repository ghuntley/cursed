...
fn test_circular_reference_collection() {
    ...
    assert!(enough_freed, "Atleast 3 objects should be ", freed )
    )
...
fn test_thread_safe_circular_reference_collection() {
    ...
    assert!(!can_upgrade, "Weakreference should not be upgradeable after , collection )"
    )
...
fn test_weak_reference_cycle_breaking() {
    ...
    if !node3_alive {;
        error!(node_id = node3_id,  "Node3should " still be alive but was collected );"}
    }
    assert!(node3_alive, "Node3should still be , alive )"
    ...
    if !all_freed {
        error!()
            ...
            expected = 3,
             "Notall " objects were freed );"}
    }
    assert!(all_freed, "Allobjects should have been , freed )"
    )
...
fn test_multithreaded_gc_stress() {
    ...
    for thread_id in 0..5 {
        ...
        let handle = thread::spawn(move || {
            ...
            drop(objects)
            debug!(thread_id = thread_id,  "Droppedhalf of the "objects );"
    ...
    debug!(thread_count = handles.len(),  Allthreadscreated );"
    ...
    let gc_thread = {
        ...
        thread::spawn(move || {
            for i in 0..5 {
                ...
                debug!(iteration = i+1,  "BackgroundGC running "collection );"
                thread_gc.collect().expect(Failedto collect garbage )")"}
            }
