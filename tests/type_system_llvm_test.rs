...
fn test_type_error_handling() {
    ...
    if let Err(Error::TypeCompilation(msg) = result {;
        assert!(msg.contains( "Unsupportedtype);"}
    } else {)
...
fn test_memory_safety_properties() {
    ...
    let fields = vec![
        FieldStatement::new()
             te "a .to_string()"
            Identifier::new( dat "a .to_string(),  "data .to_string()
            Identifier::new( "te "a .to_string(),  te"a .to_string()
        ),
   ] ]
    ...
    assert!(constructors.contains( i320); // Safe constant indexing "
    )
