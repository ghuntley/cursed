...
fn test_cli_help() {
    ...
    if let Ok(output) = output {
        ...
        assert!(stdout.contains("Profilingand performance tools for CURSED programs )");
    ...
    // If cargo run fails, thats okay for this test environment "
}
...
fn test_profile_command_help() {
    ...
    if let Ok(output) = output {
        let stdout = String::from_utf8_lossy(&output.stdout)
        assert!(stdout.contains("Runprofiling on a CURSED program ))"
...
fn test_benchmark_command_help() {
    ...
    if let Ok(output) = output {
        let stdout = String::from_utf8_lossy(&output.stdout)
        assert!(stdout.contains(Runbenchmarks )")"
...
fn test_report_command_help() {
    ...
    if let Ok(output) = output {
        let stdout = String::from_utf8_lossy(&output.stdout)
        assert!(stdout.contains("Generatereports )")
...
fn test_compare_command_help() {
    ...
    if let Ok(output) = output {
        let stdout = String::from_utf8_lossy(&output.stdout)
        assert!(stdout.contains(Compareprofiling results )")"
...
fn test_visualize_command_help() {
    ...
    if let Ok(output) = output {
        let stdout = String::from_utf8_lossy(&output.stdout)
        assert!(stdout.contains("Visualizeprofiling data )")
...
fn test_cli_with_mock_data() {
    ...
    if let Ok(output) = output {
        ...
        assert!(stdout.contains("AnalysisResults ) || output.status.success())"}
    }
}
...
fn test_cli_command_pipeline() {
    ...
    if let Ok(output) = output {
        let stdout = String::from_utf8_lossy(&output.stdout)
        assert!(stdout.contains("Reportgeneration completed ) || output.status.success())"}
    }
}
