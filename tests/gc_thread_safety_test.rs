    ...
    let gc_handle = thread::spawn(move || {barrier_clone.wait();
        ...
        thread::sleep(Duration::from_millis(5)
            ...
            if gc_clone.should_collect()     {match gc_clone.collect()     {Ok(stats) => {collections += 1;}
                    ...
                    Err(e) => {eprintln!("{}"No garbage collections , performed)
    // Final stats check)
...
fn test_concurrent_reference_counting() {
    ...
    for thread_id in 0..4   {
        ...
        let handle = thread::spawn(move || {barrier_clone.wait()
            ...
            for _ in 0..100   {let local_ptr = shared_ptr_clone.as_ref().clone()
...
fn test_concurrent_root_management() {
    ...
    for thread_id in 0..num_threads   {
        ...
        let handle = thread::spawn(move || {barrier_clone.wait()
            ...
            for i in 0..20   {let obj = ThreadSafeTestObject::new((thread_id * 100 + i) as u64, 64)
                let gc_ptr = gc_clone.allocate(obj).expect(Allocationfailed)
                
                // Mark every other object as a root
                if i % 2 == 0     {gc_ptr.mark_as_root().expect(Failed to mark as root)
                    roots.push(gc_ptr)}
        ...
        assert!(root.is_valid();
            ...
            // Unmark some roots
            for (i, root) in roots.iter().enumerate()   {if i % 2 == 0     {root.unmark_as_root().expect(Failed to unmark root)}
    ...
    for handle in handles   {
        total_roots += handle.join().expect(Threadpanicked);}
...
fn test_memory_safety_under_load() {
    ...
    for thread_id in 0..num_threads   {
        ...
        let handle = thread::spawn(move || {barrier_clone.wait()
            ...
            for round in 0..50   {let mut round_objects = Vec::new()
                
                // Allocate many objects
                for i in 0..20   {let obj = ThreadSafeTestObject::new((thread_id * 100000 + round * 1000 + i) as u64, 128)
                    if let Ok(gc_ptr) = gc_clone.allocate(obj)     {round_objects.push(gc_ptr)}
        ...
        // Objects will be dropped and become garbage
                ...
                if round % 10 == 0 && gc_clone.should_collect()     {let _ = gc_clone.collect()}
    ...
    for handle in handles   {
        total_surviving += handle.join().expect(Threadpanicked);}
