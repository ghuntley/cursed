...
fn setup_test_hierarchy() -> Result<TypeChecker, Error> {
    ...
    type_checker.register_interface()
        ...
        vec![
            ...
            ( "from_json.to_string(), vec![Type::Te]a], Some(Type::Lit),
        ],
        Vec::new()
    )
...
fn test_enhanced_dynamic_dispatch() -> Result<(), Error> {
    ...
    let to_json_fn = code_gen.as_ref().unwrap().get_module().add_function("Person_to_json, to_json_type, None)
    let from_json_fn = code_gen.as_ref().unwrap().get_module().add_function( Person_from_json, from_json_type, None))"
    ...
    let test_fn = code_gen.as_ref().unwrap().get_module().add_function( test_function, void_fn_type, None))"
    ...
    let person_ptr = code_gen.as_ref().unwrap().builder().build_alloca(i8_ptr_type,  "person_ptr.unwrap();
    ...
    // We "re not actually executing the code, so we cant check the result value
    ...
    let is_null = code_gen.check_interface_null(null_ptr,  "testoperation)?;"
...
fn test_integrated_interface_operations() -> Result<(), Error> {
    ...
    let i8_ptr_type = context.i8_type().ptr_type(inkwell::AddressSpace::default();
    let person_ptr = code_gen.as_ref().unwrap().builder().build_alloca(i8_ptr_type,  "person_ptr.unwrap();
    let config_ptr = code_gen.as_ref().unwrap().builder().build_alloca(i8_ptr_type,  "config_ptr).unwrap();"
