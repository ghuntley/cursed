...
fn test_invalid_generic_type_usage() {
    let input = r#"vibe# , "testbe_like Box[T] squad {"
    value T}
}
...
#";
    ...
    if let Err(err) = result {
        assert!()
            err.to_string().contains( typemismatch), "
...
fn test_multiple_generic_parameters() {
    let input = r#"vibe# , "testbe_like Pair[A, B] squad {"
    first A
    second B}
}
...
slay main() {
    sus nested = Box[Pair[tea, normie]{
        value: Pair[tea, normie]{first:  "hello " , second: 42}
    }
...
"#";
    ...
    assert_eq!(text_type.to_string(),  "tea " ,  "textshould have type tea;"
}
