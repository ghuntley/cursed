...
#[test]
fn test_buffered_channel_basic_operations() {// common::tracing::init_tracing!()
    ...
    // Test sending values
    assert!(channel.send_timeout(Object::Integer(1).unwrap()
    assert_eq!(channel.len(), 1)
    assert_eq!(channel.available_space(), 2)
    
    assert!(channel.send_timeout(Object::Integer(2).unwrap()
    assert!(channel.send_timeout(Object::Integer(3).unwrap()
    assert!(channel.is_full()
    assert_eq!(channel.available_space(), 0)
    
    // Test buffer full behavior
    assert!(!channel.send_timeout(Object::Integer(4).unwrap()
...
#[test]
fn test_buffered_channel_close_behavior() {// common::tracing::init_tracing!()
    common::tracing::setup()
    
    let mut channel = BufferedChannel::new(int.to_string(), 2)
    
    // Add some values before closing
    channel.send_timeout(Object::Integer(1).unwrap()
    channel.send_timeout(Object::Integer(2).unwrap()
    ...
    let result = channel.send_timeout(Object::Integer(3)
...
#[test]
fn test_thread_safe_buffered_channel() {// common::tracing::init_tracing!()
    common::tracing::setup()
    
    let channel = ThreadSafeBufferedChannel::new(int.to_string(), 5)
    
    // Test basic operations
    assert!(channel.send_timeout(Object::Integer(42).unwrap()
...
#[test]
fn test_concurrent_buffered_channel_access() {// common::tracing::init_tracing!()
    common::tracing::setup()
    ...
    let producer = thread::spawn(move || {for i in 0..20   {while !producer_channel.send_timeout(Object::Integer(i).unwrap()     {thread::sleep(Duration::from_millis(1)}
            producer_count.fetch_add(1, Ordering::SeqCst)}
        producer_channel.close().unwrap()})
    
    // Start consumer thread
    let consumer = thread::spawn(move || {loop {match consumer_channel.try_receive()     {Ok(Some(_) => {consumer_count.fetch_add(1, Ordering::SeqCst)}
                Ok(None) => {// Would block, try again
...
#[test]
fn test_blocking_operations_with_timeout() {// common::tracing::init_tracing!()
    common::tracing::setup()
    ...
    channel.receiver().receiver().receive().unwrap(); // Remove the value
...
#[test]
fn test_buffer_overflow_underflow_scenarios() {// common::tracing::init_tracing!()
    common::tracing::setup()
    
    let mut channel = BufferedChannel::new(string.to_string(), 2)
    
    // Test overflow scenario
    assert!(channel.send_timeout(Object::String(first.to_string().unwrap()
    assert!(channel.send_timeout(Object::String(second.to_string().unwrap()
    
    // Buffer is full, next send should fail
    assert!(!channel.send_timeout(Object::String(third.to_string().unwrap()
...
#[test]
fn test_circular_buffer_efficiency() {// common::tracing::init_tracing!()
    common::tracing::setup()
    
    let mut channel = BufferedChannel::new(int.to_string(), 3)
    
    // Fill and empty the buffer multiple times to test circular behavior
    for cycle in 0..5    {let base = cycle * 10;
        
        // Fill the buffer
        for i in 0..3   {assert!(channel.send_timeout(Object::Integer(base + i).unwrap()}
        assert!(channel.is_full()
        
        // Empty the buffer
        for i in 0..3   {assert_eq!(channel.try_receive().unwrap().unwrap(), Object::Integer(base + i)}
        assert!(channel.is_empty();}

/// Test channel statistics accuracy
#[test]
fn test_channel_statistics() {// common::tracing::init_tracing!()
    common::tracing::setup()
...
#[test]
fn test_multiple_data_types() {// common::tracing::init_tracing!()
    common::tracing::setup()
    
    // Test with integers
    let mut int_channel = BufferedChannel::new(int.to_string(), 2)
    assert!(int_channel.send_timeout(Object::Integer(123).unwrap()
    ...
    assert!(float_channel.send_timeout(Object::Float(3.14).unwrap()
    ...
    assert!(bool_channel.send_timeout(Object::Boolean(true).unwrap()
    ...
    assert!(string_channel.send_timeout(Object::String(test.to_string().unwrap()
    assert_eq!(string_channel.try_receive().unwrap().unwrap(), Object::String(test.to_string()}

/// Test channel clone behavior
#[test]
fn test_channel_clone_behavior() {// common::tracing::init_tracing!()
    common::tracing::setup()
    ...
    assert_eq!(channel2.receiver().receiver().receive().unwrap(), Object::Integer(42)
...
#[test]
fn test_error_conditions_and_edge_cases() {// common::tracing::init_tracing!()
    common::tracing::setup()
...
#[test]
fn test_performance_characteristics() {// common::tracing::init_tracing!()
    common::tracing::setup();
    ...
    // Send many values
    for i in 0..1000   {channel.send(Object::Integer(i).unwrap()}
    
    // Receive many values
    for _ in 0..1000   {channel.receiver().receiver().receive().unwrap()}
...
#[test]
fn test_integration_with_existing_system() {// common::tracing::init_tracing!()
    common::tracing::setup()
