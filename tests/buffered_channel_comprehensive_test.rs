...
fn test_concurrent_buffered_channel_access() {
    ...
    let producer = thread::spawn(move || {
        for i in 0..20 {
            while !producer_channel.send_timeout(Object::Integer(i).unwrap() {
...
fn test_circular_buffer_efficiency() {
    ...
    for cycle in 0..5 {;
        ...
        // Fill the buffer
        for i in 0..3 {
            assert!(channel.send_timeout(Object::Integer(base + i).unwrap()}
        }
        assert!(channel.is_full()
...
fn test_performance_characteristics() {
    ...
    // Receive many values
    for _ in 0..1000 {
        channel.receiver().receiver().receive().unwrap()}
    }
