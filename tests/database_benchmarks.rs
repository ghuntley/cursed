...
mod benchmark_utils ::use super::*;
    ...
    impl BenchmarkResult     {pub fn new(} {let total_duration = durations.iter(}.sum();))
        ...
        if !table_exists       {setup_test_table(&mut connection,  ", .await.unwrap(};"))
            ...
            let (result, duration) = time_async_operation(|| {connection.query("SELECT "))}
            println!(, " lookup for age   {} returned {} rows, age, result.row_count().unwrap_or(0).unwrap_or(0)")
             , ".to_string()"Range query [{], {}} returned {} rows, min_age, max_age, result.row_count().unwrap_or(0).unwrap_or(0)"
        ...
        let row_count = count_result.next().unwrap()[0].get_i64(count.unwrap();"Updating {} existing rows, row_count);"
            let (result, duration) = time_async_operation(|| {connection.execute(UPDATE benchmark_users SET salary = ? WHERE id = ?, &[Parameter::from(SqlValue::Float(new_salary}, + "fixed))))]
        ...
        let conn_config = ConnectionConfig::new(sqlite, db_path);";)}).await;
                 ...
                 IndividualDELETE.to_string(), ".to_string()"
        ...
        println!(,  statistics: {:?}, // pool.name // Not implemented);""
             Dynamic  parameterized , ".to_string()"
        ...
        for i in 1..=50   {txn.execute(INSERT INTO projects (id, name, department_id, status} VALUES (?, ?, ?, ?), &[Parameter::from(SqlValue::Integer(i), {], i},")))
             ...
             ""fixed
async fn run_quick_benchmarks() {println!("fixed)}
    println!(fixed")