...
mod benchmark_utils {
    ...
    pub async fn setup_test_table(connection: &mut dyn DatabaseConnection, table_name: &str) -> Result<(), Box<dyn std::error::Error>> {
        ...
        connection.execute(&create_sql, vec![]).await?;
        Ok(())
    }
...
mod sqlite_benchmarks {
    ...
    async fn run_sqlite_insert_benchmark(config: &BenchmarkConfig, db_path: &str, db_type: &str) -> Vec<BenchmarkResult> {
        ...
        let mut connection = SqliteDriver::new().connect(conn_config).await.unwrap();
        ...
        for _ in 0..config.warmup_iterations {
            ...
            let (_, duration) = time_async_operation(|| {
                connection.execute(insert_sql, vec![
                    SqlValue::Text("Warmup User".to_string()),
        ...
        let mut connection = SqliteDriver::new().connect(ConnectionConfig::new("sqlite", db_path)).await.unwrap();
        if db_path != ":memory:" {
            connection.execute("DROP TABLE IF EXISTS benchmark_users", vec![]).await.unwrap();
        }
        setup_test_table(&mut connection, "benchmark_users").await.unwrap();
    ...
    async fn run_sqlite_select_benchmark(config: &BenchmarkConfig, db_path: &str, db_type: &str) -> Vec<BenchmarkResult> {
        ...
        let table_exists = connection.query("SELECT name FROM sqlite_master WHERE type='table' AND name='benchmark_users'", vec![]).await.unwrap().row_count() > 0;
        ...
        if !table_exists {
            ...
            for (i, (name, email, age, salary, active)) in test_data.iter().enumerate() {
                txn.execute("INSERT INTO benchmark_users (name, email, age, salary, active) VALUES (?, ?, ?, ?, ?)", vec![
                    SqlValue::Text(name.clone()),
        ...
        for _ in 0..config.benchmark_iterations {
            let (result, duration) = time_async_operation(|| {
                connection.query("SELECT * FROM benchmark_users", vec![])
            }).await;
            let result = result.unwrap();
            println!("Full scan returned {} rows", result.row_count());
        ...
        connection.execute("CREATE INDEX idx_benchmark_users_age ON benchmark_users(age)", vec![]).await.unwrap();
        
        let mut indexed_durations = Vec::new();
        for i in 0..config.benchmark_iterations {
            let age = 20 + (i % 50) as i64;
            let (result, duration) = time_async_operation(|| {
                connection.query("SELECT * FROM benchmark_users WHERE age = ?", vec![SqlValue::Integer(age)])
            }).await;
            let result = result.unwrap();
            println!("Indexed lookup for age {} returned {} rows", age, result.row_count());
            indexed_durations.push(duration);
        }
        
        results.push(BenchmarkResult::new(
            "Indexed SELECT".to_string(),
            "SQLite".to_string(),
            db_type.to_string(),
            indexed_durations
        ));
        ...
        for i in 0..config.benchmark_iterations {
            ...
            let (result, duration) = time_async_operation(|| {
                connection.query("SELECT * FROM benchmark_users WHERE age BETWEEN ? AND ? ORDER BY age", vec![
                    SqlValue::Integer(min_age),
                    SqlValue::Integer(max_age)
                ])
            }).await;
            let result = result.unwrap();
            println!("Range query [{}, {}] returned {} rows", min_age, max_age, result.row_count());
    ...
    async fn run_sqlite_update_benchmark(config: &BenchmarkConfig, db_path: &str, db_type: &str) -> Vec<BenchmarkResult> {
        ...
        let count_result = connection.query("SELECT COUNT(*) as count FROM benchmark_users", vec![]).await.unwrap();
        let row_count = count_result.rows()[0].get_i64("count").unwrap();
        ...
        for i in 0..config.benchmark_iterations.min(100) {
            ...
            let (result, duration) = time_async_operation(|| {
                connection.execute("UPDATE benchmark_users SET salary = ? WHERE id = ?", vec![
                    SqlValue::Float(new_salary),
                    SqlValue::Integer(id)
                ])
            }).await;
        ...
        for i in 0..config.benchmark_iterations {
            ...
            let (result, duration) = time_async_operation(|| {
                connection.execute("UPDATE benchmark_users SET salary = salary + ? WHERE active = ?", vec![
                    SqlValue::Float(salary_increase),
                    SqlValue::Boolean(true)
                ])
            }).await;
    ...
    async fn run_sqlite_delete_benchmark(config: &BenchmarkConfig, db_path: &str, db_type: &str) -> Vec<BenchmarkResult> {
        ...
        for i in 0..config.benchmark_iterations.min(50) {
            ...
            let find_result = connection.query("SELECT id FROM benchmark_users LIMIT 1 OFFSET ?", vec![
                SqlValue::Integer(i as i64)
            ]).await;
            
            if let Ok(result) = find_result {
                if result.row_count() > 0 {
                    let id = result.rows()[0].get_i64("id").unwrap();
                    
                    let (delete_result, duration) = time_async_operation(|| {
                        connection.execute("DELETE FROM benchmark_users WHERE id = ?", vec![
                            SqlValue::Integer(id)
                        ])
                    }).await;
    ...
    async fn run_sqlite_transaction_benchmark(config: &BenchmarkConfig, db_path: &str, db_type: &str) -> Vec<BenchmarkResult> {
        ...
        connection.execute("DROP TABLE IF EXISTS txn_benchmark", vec![]).await.unwrap();
        ...
        for _ in 0..config.benchmark_iterations {
            let (_, duration) = time_async_operation(|| async {
                ...
                for (i, (name, email, age, salary, active)) in test_data.iter().enumerate() {
                    txn.execute("INSERT INTO txn_benchmark (name, email, age, salary, active) VALUES (?, ?, ?, ?, ?)", vec![
                        SqlValue::Text(name.clone()),
    ...
    async fn run_direct_connection_benchmark(config: &BenchmarkConfig) -> Vec<BenchmarkResult> {
        ...
        for _ in 0..config.benchmark_iterations {
            let (_, duration) = time_async_operation(|| async {
                ...
                connection.execute("INSERT INTO test (value) VALUES (?)", vec![
                    SqlValue::Text("test value".to_string())
                ]).await.unwrap();
                
                let _result = connection.query("SELECT * FROM test", vec![]).await.unwrap();
                connection.close().await.unwrap();
            }).await;
    ...
    async fn run_pool_connection_benchmark(config: &BenchmarkConfig) -> Vec<BenchmarkResult> {
        ...
        for _ in 0..config.benchmark_iterations {
            let (_, duration) = time_async_operation(|| async {
                ...
                conn.execute("INSERT INTO test_pool (value) VALUES (?)", vec![
                    SqlValue::Text("pool test value".to_string())
                ]).await.unwrap();
                
                let _result = conn.query("SELECT * FROM test_pool", vec![]).await.unwrap();
        ...
        let handles: Vec<_> = (0..concurrent_tasks).map(|task_id| {
            ...
            tokio::spawn(async move {
                ...
                for i in 0..5 { // Each task does 5 operations
                    ...
                    conn.execute("INSERT INTO test_concurrent (task_id, iteration) VALUES (?, ?)", vec![
                        SqlValue::Integer(task_id),
                        SqlValue::Integer(i)
                    ]).await.unwrap();
                    
                    let _result = conn.query("SELECT * FROM test_concurrent WHERE task_id = ?", vec![
                        SqlValue::Integer(task_id)
                    ]).await.unwrap();
...
mod query_builder_benchmarks {
    ...
    async fn run_dynamic_vs_prepared_benchmark(config: &BenchmarkConfig) -> Vec<BenchmarkResult> {
        ...
        let mut txn = connection.begin_transaction().await.unwrap();
        for (i, (name, email, age, salary, active)) in test_data.iter().enumerate() {
            txn.execute("INSERT INTO query_benchmark (name, email, age, salary, active) VALUES (?, ?, ?, ?, ?)", vec![
                SqlValue::Text(name.clone()),
                SqlValue::Text(format!("{}_{}", email, i)),
                SqlValue::Integer(*age),
                SqlValue::Float(*salary),
        ...
        for i in 0..config.benchmark_iterations {
            ...
            let (_, duration) = time_async_operation(|| {
                connection.query("SELECT * FROM query_benchmark WHERE age > ? AND active = ? ORDER BY salary DESC", vec![
                    SqlValue::Integer(age_threshold),
                    SqlValue::Boolean(true)
                ])
            }).await;
    ...
    async fn run_query_complexity_benchmark(config: &BenchmarkConfig) -> Vec<BenchmarkResult> {
        ...
        // Departments
        for i in 1..=10 {
            txn.execute("INSERT INTO departments (id, name, budget) VALUES (?, ?, ?)", vec![
                SqlValue::Integer(i),
        ...
        // Users
        for i in 1..=100 {
            txn.execute("INSERT INTO users (id, name, department_id) VALUES (?, ?, ?)", vec![
                SqlValue::Integer(i),
        ...
        // Projects
        for i in 1..=50 {
            txn.execute("INSERT INTO projects (id, name, department_id, status) VALUES (?, ?, ?, ?)", vec![
                SqlValue::Integer(i),
        ...
        for _ in 0..config.benchmark_iterations {
            let (_, duration) = time_async_operation(|| {
                connection.query("SELECT * FROM users WHERE id > 50", vec![])
            }).await;
        ...
        for _ in 0..config.benchmark_iterations {
            let (_, duration) = time_async_operation(|| {
                connection.query("SELECT u.name, d.name as dept_name FROM users u JOIN departments d ON u.department_id = d.id", vec![])
            }).await;
        ...
        for _ in 0..config.benchmark_iterations {
            let (_, duration) = time_async_operation(|| {
                connection.query(complex_sql, vec![])
            }).await;
