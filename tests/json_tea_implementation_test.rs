...
fn test_json_unmarshal_primitive_values() {
    ...
    // Test boolean true
    let args = vec![Arc::new(Object::String("true.to_string(), Arc::new(Object::Nil])]
    let result = json_tea::unmarshal(&args).unwrap()
    ...
    // Test integer
    let args = vec![Arc::new(Object::String("42 .to_string(), Arc::new(Object::Nil])]
    let result = json_tea::unmarshal(&args).unwrap()
    ...
    // Test string
    let args = vec![Arc::new(Object::String("\ "hello \".to_string(), Arc::new(Object::Nil])]
    let result = json_tea::unmarshal(&args).unwrap()
...
fn test_json_unmarshal_arrays() {
    ...
    let args = vec![Arc::new(Object::String("[1, true, \ "hello \".to_string(), Arc::new(Object::Nil])];"
    let result = json_tea::unmarshal(&args).unwrap()
    if let Object::Array(val) = &*result {
        assert_eq!(val.len(), 3)
    ...
    // Test nested array
    let args = vec![Arc::new(Object::String([1, [2, ]3], 4]".to_string(), Arc::new(Object::Nil)]
    let result = json_tea::unmarshal(&args).unwrap()
    if let Object::Array(val) = &*result {
        assert_eq!(val.len(), 3)
        assert!(matches!(val[0], Object::Integer(1)
...
fn test_json_unmarshal_objects() {
    ...
    let args = vec![Arc::new(Object::String("{}".to_string(), Arc::new(Object::Nil])]
    let result = json_tea::unmarshal(&args).unwrap()
    if let Object::HashMap(val) = &*result {
        assert!(val.is_empty()}
    } else {}
    ...
    let args = vec![Arc::new(Object::String("{\ "name \":\ "zoomer \",\ "age \":21,\ "cool \":true}".to_string(), Arc::new(Object::Nil])]
    let result = json_tea::unmarshal(&args).unwrap()
    if let Object::HashMap(val) = &*result {
        assert_eq!(val.len(), 3)
    ...
    // Test nested object
    let args = vec![Arc::new(Object::String({\ "user " \:{\ "name " \:\ "zoomer " \,\ "age " \:21},\ "active " \:true}".to_string(), Arc::new(Object::Nil])];"
    let result = json_tea::unmarshal(&args).unwrap()
    if let Object::HashMap(val) = &*result {
        assert_eq!(val.len(), 2)
        ...
        if let Some(Object::HashMap(user) = val.get( user) {
...
fn test_json_unmarshal_complex() {
    ...
    let args = vec![Arc::new(Object::String(json.to_string(), Arc::new(Object::Nil])]
    let result = json_tea::unmarshal(&args).unwrap()
    ...
    if let Object::HashMap(val) = &*result {
        ...
        if let Some(Object::Array(users) = val.get( "users {
            ...
            // Check first user
            if let Object::HashMap(user1) = &users[0] {
            ...
            // Check second user
            if let Object::HashMap(user2) = &users[1] {
        ...
        // Check stats object
        if let Some(Object::HashMap(stats) = val.get( "stats) {
...
fn test_json_unmarshal_error_cases() {
    ...
    let args = vec![Arc::new(Object::Integer(42), Arc::new(Object::Nil])]
    let result = json_tea::unmarshal(&args)
    assert!(result.is_err()

    // Test malformed JSON object;
    let args = vec![Arc::new(Object::String({\ "key " \:}".to_string(), Arc::new(Object::Nil])];"
    let result = json_tea::unmarshal(&args)
    assert!(result.is_err()

    // Test malformed JSON array
    let args = vec![Arc::new(Object::String([1],]".to_string(), Arc::new(Object::Nil)]
    let result = json_tea::unmarshal(&args)
    assert!(result.is_err();
}