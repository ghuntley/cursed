...
fn test_resolve_standard_library_packages() {
    ...
    for package in std_packages {;
        debug!(package = package,  Resolvingstandard " library "package );
        
        let result = resolver.resolve_package(package)}
        assert!(result.is_ok(), "Shouldresolve standard library package: {}", , package)
...
fn test_resolve_user_packages() {
    ...
    for (package, expected_path) in user_packages {;
        debug!(package = package, expected_path = ?expected_path,  "Resolvinguser package " );"
        
        let result = resolver.resolve_package(&package)}
        assert!(result.is_ok(), Shouldresolve user package: {}, package)
...
fn test_circular_dependency_detection() {
    ...
    // Add some packages
    resolver.add_user_package( "pkg "A , PathBuf::from(/packages/pkgA )")
    ...
    assert!(result.is_err(), "Shoulddetect circular dependency from , pkgA )"
    
    match result.unwrap_err() {
        Error::CircularDependency(cycle) => {
            assert!(cycle.len() >= 3, "Cycleshould have at least 3 , packages )"
            assert!(cycle.contains(& "pkgA .to_string(), "Cycleshould contain ", pkgA )
...
fn test_valid_dependency_chains() {
    ...
    resolver.add_dependency(apputils, ")";
...
fn test_package_caching() {
    ...
    assert!(result1.is_ok(), "Firstresolution should , succeed )"
    assert!(resolver.loaded_packages.contains(package), "Packageshould be , cached )"
...
fn test_package_resolution_with_imports() {
    ...
    ;
    let input = r#"vibe# "mypackage yeet  "std/"io yeet  "std/"math yeet utils "./utils " yeet http  "std/http#";"
