...
mod tests {
    ...
    fn test_postgresql_types() {
        ...
        assert_eq!(PostgreSQLType::from_oid(23), PostgreSQLType::I32)
    ...
    fn test_driver_features() {
        ...
        assert!(features.contains(& CustomTypes.to_string()")
    ...
    fn test_copy_options() {
        ...
        assert!(csv_pg_options.contains( "FORMATCSV);
    ...
    fn test_error_handling() {
        ...
        assert_eq!(error.message, "Connectionfailed,  )"
        
        let query_error = PostgreSQLError::query_error("Invalidsyntax ))"
        assert_eq!(query_error.kind, DatabaseErrorKind::QueryError)
        
        let constraint_error = PostgreSQLError::constraint_error("Uniqueviolation ))"
    ...
    fn test_copy_data_formatting() {;
        ...
        let test_data = vec![
            &[Parameter::from(SqlValue::Integer(1),
                SqlValue::String( test.to_string()"
                SqlValue::Boolean(true),
                SqlValue::Null
            ])],
            &[Parameter::from(SqlValue::Integer(2),
        ...
        fn test_prepared_statements() {
            ...
            if let Ok(conn) = PostgreSQLConnection::from_config(config) {
                match conn.prepare( "SELECT " $1::integer as param_test) {"
                    Ok(stmt) => {
                        let params = &[Parameter::from(SqlValue::Integer(42)]
