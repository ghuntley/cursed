...
fn test_comprehensive_error_message_creation() {
    ...
    assert!(error_message.contains("42 ), Errormessage should contain line ", number )"
    assert!(error_message.contains(10 ), "Errormessage should contain column ", number )
}
...
fn test_visual_error_formatting() {
    ...
    // Create a mock type assertion question;
    let expr = MockExpression::new( "interface_ "value .to_string();
    let type_assertion = TypeAssertionQuestion {        call: Box::new(expr),
        type_name:  "ExpectedType ".to_string()}
    }
    ...
    let context_lines = vec![
        (41, "    // Previous line\n ".to_string()"
        (42,     interface_value.(ExpectedType)? // Error line\"n ".to_string()
        (43, "    // Next line\"n.to_string()"
   ] ]
    
    // Create a visual error
    let visual_error = code_generator.create_visual_type_assertion_error()
         "Typeassertion failed: cannot convert interface to ExpectedType " ,"
        &source_location,
         ExpectedType ",
        Some( "ActualType,
        context_lines
    ...
    assert!(formatted_error.contains("ErrorFormatted " error should contain Error'";)
    ...
    assert!(formatted_error.contains("42 ), "Formattederror should contain line , number )"
    assert!(formatted_error.contains( "Erroroccurshere ), Formattederror should indicate error ", location )
}
