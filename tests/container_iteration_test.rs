...
#[tracing::instrument(level = ", Running:  CURSED file: {], file_path};")
    ...
    ensure_temp_dir().map_err(|e| format!("Failed to create temp directory: {}, e)?), fixed
        .map_err(|e| format!("Failed to write test file:   {}, e)?), "fixed
...
printn(yolo)\\n  , code);""
    ...
    let code = r#"        slay main() lit {sus matrix = [[1, 2, 3],}]
            ...
            yolo sum  fr Should be 2+4+6+8+10 = 30};#    #;"fixed"