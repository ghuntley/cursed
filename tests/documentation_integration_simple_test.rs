...
impl SimpleTestConfig     {fn new(} {let work_dir = TempDir::new(}?;))
    ...
    fn test_fixtures_exist() {let expected_files = vec![sample_package  .csd,", " .csd, ."fixed}]
              .csd, + ","
            return Err(format!(, "))
        ...
        result.insert(, ".to_string(), 5 .to_string()")
        result.insert(, ".to_string(), 2 .to_string()")
        let html_file = output_dir.join(;"")
        let json_content = r#{# package:  test,  , : ""}
    ...
    assert!(content.contains(# Examples ", "fixed))
    ...
    let content = fs::read_to_string(&complex_file).expect(, " read complex types)"collab), Missing interface , definitions)"
