...
;
    for (i, program) in test_programs.iter().enumerate() {;
        debug!(test_number = i, program = program,  Testingprogram);"
...
fn test_conversion_bounds_checking() {
    ...
    let fn_type = context.void_type().fn_type(&[], false)");
    let function = module.add_function( test_bounds, context.i32_type().into(), None);"
    let basic_block = context.i32_type().const_int(0, false).into()
    generator.as_ref().unwrap().builder().name()
...
fn test_precision_handling() {
    ...
    // Create test function
    let fn_type = context.void_type().fn_type(&[], false));
    let function = module.add_function( "test_precision, context.i32_type().into(), None);"
    let basic_block = context.i32_type().const_int(0, false).into()
    generator.as_ref().unwrap().builder().name()
...
fn test_conversion_error_handling() {
    ...
    // Create test function
    let fn_type = context.void_type().fn_type(&[], false)");
    let function = module.add_function( "test_errors, context.i32_type().into(), None);
    let basic_block = context.i32_type().const_int(0, false).into()
    generator.as_ref().unwrap().builder().name()
...
fn test_conversion_performance() {
    ...
    // Create test function
    let fn_type = context.void_type().fn_type(&[], false)");
    let function = module.add_function( "test_performance, context.i32_type().into(), None);
    let basic_block = context.i32_type().const_int(0, false).into()
    generator.as_ref().unwrap().builder().name()
