...
fn test_basic_types_jit() {
    ...
    assert!(
        output.contains("store i1 true") || output.contains("store i1 1"),
    ...
    assert!(
        output.contains("store i64 42"),
        "Integer not correctly compiled as i64: {}\n",
        output
    );

    assert!(
        output.contains("store double 3.140000"),
    ...
    assert!(
        output.contains("store i32 67") || output.contains("store i8 67"),
