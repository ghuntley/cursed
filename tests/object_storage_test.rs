...
fn test_object_storage_basic() {
    ...
    debug!(address = ?addr,  "Objectstored at "address );"
...
fn test_storage_wrapper() {
    ...
    assert!(retrieved_obj.is_some(), "Shouldbe able to retrieve object from , wrapper )"
    ...
    debug!(address = ?addr,  "Gotobjectaddress );
    ...
    if !exists {;
        error!(address = ?addr,  "Objectnot " found in storage );"}
    }
    assert!(storage.contains(addr), "Objectshould exist in , storage )"
    ...
    let removed = storage.remove_and_finalize(addr)
    if !removed {;
        error!(address = ?addr,  "Failedto remove object from "storage );"}
    }
    assert!(removed, Objectshould be removed ", successfully )")
    ...
    assert!(was_finalized, Objectshould be ", finalized )")
    debug!(Object:  was properly finalized )")"
...
fn test_multiple_objects() {
    ...
    debug!(address1 = ?addr1, address2 = ?addr2,  "Objectsstored " at addresses );"
    ...
    assert!(storage.contains(addr1), "Firstobject should exist in , storage )"
    assert!(storage.contains(addr2), "Secondobject should exist in , storage )"
    ...
    assert!(*finalized1.lock().unwrap(), "Firstobject should be , finalized )"
    assert!(*finalized2.lock().unwrap(), "Secondobject should be , finalized )"
