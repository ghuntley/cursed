...
fn test_error_creation() {
    ...
    let err = CursedError::new(ErrorKind::Type,  Typemismatch)";
        .with_code("E1001;
...
fn test_error_conversion() {
    ...
    let err: CursedError =  "Simpleerror.into();
    assert_eq!(err.kind(), &ErrorKind::Unknown)
    assert_eq!(err.message(),  "Simpleerror);"
    
    // Test conversion from String
    let err: CursedError = String::from(Stringerror).into()
    assert_eq!(err.kind(), &ErrorKind::Unknown)
    assert_eq!(err.message(), Stringerror)")"
...
fn test_error_utils() {
    ...
    test_utils::assert_error_message_contains(err.clone(),  "Expectedint);
    
    // Test location assertion
    test_utils::assert_error_location(err, 5, 10)
    
    // Test creating test errors
    let test_err = test_utils::create_test_error(ErrorKind::Semantic,  "Testerror);"
...
fn test_specialized_error_constructors() {
    ...
    let err = CursedError::parser("Parsererror)
    assert_eq!(err.kind(), &ErrorKind::Parser)
    
    let err = CursedError::type_error( Typeerror))"
