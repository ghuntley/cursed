...
fn test_randomness_quality() {
    ...
    let mut random_data = vec![0u8; byte_count];
    fill_random(&mut random_data).expect("Random generation failed");
    ...
    assert!(zeros < byte_count / 2, "Too many zeros in random data");
    assert!(ones < byte_count / 2, "Too many ones in random data");
    
    // Test 2: Frequency test - each byte value should appear roughly equally
    let mut frequency = vec![0usize; 256];
    ...
    assert!(unique_bytes > 200, "Should see diverse byte values: {}", unique_bytes);
...
fn test_constant_time_operations() {
    ...
    assert!(time_ratio > 0.5 && time_ratio < 2.0, 
            "Constant-time comparison timing inconsistent: ratio {}", time_ratio);
...
fn test_key_derivation_security() {
    ...
    assert_eq!(key1a, key1b, "Same PBKDF2 inputs should produce same output");
    ...
    assert_ne!(key_salt1, key_salt2, "Different salts should produce different keys");
    ...
    assert_ne!(key_iter1, key_iter2, "Different iteration counts should produce different keys");
    ...
    assert_ne!(pbkdf2_key, argon2_key, "PBKDF2 and Argon2 should produce different keys");
    assert_ne!(pbkdf2_key, scrypt_key, "PBKDF2 and scrypt should produce different keys");
    assert_ne!(argon2_key, scrypt_key, "Argon2 and scrypt should produce different keys");
...
fn test_authentication_bypass_prevention() {
    ...
    let secret_key = b"super_secret_hmac_key_bestie";
    let original_message = b"authentic message content";
    ...
    assert_ne!(original_hmac, tampered_hmac, "HMAC should differ for different messages");
    ...
    assert_eq!(original_hmac, verification_hmac, "HMAC verification should succeed");
    
    // Test 2: Digital signature verification should detect tampering
    let keypair = KeyGenerator::generate_ed25519_keypair().expect("Key generation failed");
    let signature = keypair.sign(original_message).expect("Signature generation failed");
    
    // Valid signature should verify
    assert!(keypair.verify(original_message, &signature).expect("Signature verification failed"));
    
    // Tampered message should not verify
    assert!(!keypair.verify(tampered_message, &signature).expect("Signature verification failed"));
    
    // Test 3: Authentication with different keys should fail
    let keypair2 = KeyGenerator::generate_ed25519_keypair().expect("Key generation failed");
...
fn test_timing_attack_resistance() {
    ...
    let correct_key = pbkdf2_derive(correct_password, salt, iterations, key_length)
        .expect("Correct key derivation failed");
    ...
    for wrong_password in &wrong_passwords {
        ...
        for _ in 0..10 {
            ...
            let wrong_key = pbkdf2_derive(wrong_password, salt, iterations, key_length)
                .expect("Wrong key derivation failed");
    ...
    for &compare_time in &compare_times {
        ...
        assert!(ratio > 0.1 && ratio < 10.0, 
                "Constant-time comparison too variable: ratio {:.2}", ratio);
    }
...
fn test_side_channel_resistance() {
    ...
    let data_sizes = vec![16, 32, 64, 128, 256, 512, 1024];
    let mut timing_by_size = HashMap::new();
    
    for &size in &data_sizes {
        let test_data = vec![0x33u8; size];
        let mut times = Vec::new();
        
        for _ in 0..100 {
            let start = Instant::now();
            let encrypted = cipher.encrypt(&test_data).expect("Encryption failed");
            let _decrypted = cipher.decrypt(&encrypted).expect("Decryption failed");
    ...
    assert!(scaling_factor > 1.0 && scaling_factor < 100.0, 
            "Unusual timing scaling: {:.2}", scaling_factor);
    ...
    for key in &keys {
        ...
        for _ in 0..50 {
            let start = Instant::now();
            let _encrypted = cipher.encrypt(&test_data).expect("Encryption failed");
            times.push(start.elapsed());
        }
...
fn test_secure_memory_handling() {
    ...
    {
        let _secure_mem = SecureMemory::new(&mut sensitive_data).expect("Secure memory creation failed");
    ...
    let mut key_material = vec![0x33u8; 32];
    let secure_key = SecureMemory::new(&mut key_material);
    assert!(secure_key.is_ok(), "Secure memory should be available");
    ...
    let _derived_key = pbkdf2_derive(&password, salt, 1000, 32).expect("Key derivation failed");
    ...
    // Verify clearing
    assert!(password.iter().all(|&b| b == 0), "Password should be cleared");
...
fn test_crypto_parameter_validation() {
    ...
    let invalid_keys = vec![
        vec![0u8; 0],   // Empty key
        vec![0u8; 15],  // Too short for AES-256
        vec![0u8; 33],  // Invalid size
    ];
    
    for invalid_key in invalid_keys {
        let result = AesGcm256::new(&invalid_key);
        assert!(result.is_err(), "Invalid key size should be rejected: {}", invalid_key.len());
    }
