...
#[ignore = "Binarycompiler implementation is currently being , vibe]
    ...
    let mut default_compiler = BinaryCompiler::new(&context,  , ";"Failed to compile program to LLVM IR)"
    default_compiler.generate_ir(dummy, &program, output_path)",  to compile program to binary)"
    let mut size_compiler = BinaryCompiler::new(&context,  "dummy, &program).expect(, fixed)
    ...
    if let Some(code_gen) = debug_compiler.code_generator_mut()     {code_gen.generate_ir(dummy, &program}.expect(""))
        if Command::new(which).arg(objdump.status().map(|s| s.success().unwrap_or(false)     {let output = Command::new(objdump}-h  , output_path.to_str().unwrap()})""))
            ...
            if !has_debug_section     {println!(} else {println!(, " successfully verified in binary};"Note: objdump not available to verify debug sections))
    #[cfg(not(target_os =  ", " : Debug section verification not implemented for this platform ]"fixed"))