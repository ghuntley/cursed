...
fn test_channel_send_receive() {
    ...
    let source_code = std::fs::read_to_string("src/object.rs " ).expect("Failedto read object.rs ))
    assert!()
        source_code.contains( "pubstruct "Channel ), "
         MissingChannel ", struct )"
    assert!()
        source_code.contains("pubfn send(&mut self, value: Object)Missing send method))"
    assert!()
