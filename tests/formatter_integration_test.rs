...
bestie value flex ch {,   :, value}"};"#.trim();
...
sus a, b = get_values()##.trim();"
        ...
        let source = r#, # } highkey lowkey x < 0 {yolo  negative} highkey {yolo  "zero
,  x > 0}{}};""
...
;"
        ...
        assert!(result.formatted_code.contains(version normi)e)", "(ns NestedStru)c)t) {;}
        let source = r#""
        assert!(result.formatted_code.contains(*/, "  values = [42, 3.14159, 0x1A2B, 0o755, 0b1010]);")
...
sus very_long_variable_name = 3;##.trim();"fixed"