...
fn test_variable_redeclaration() {
    ...
    if let Err(Error::Compile(msg)) = result {
        assert!(msg.contains("already declared"), "Error should mention variable already declared");
    } else {
...
fn test_assignment_to_undefined() {
    ...
    if let Err(Error::Compile(msg)) = result {
        assert!(msg.contains("Undefined variable"), "Error should mention undefined variable");
    } else {
...
fn test_debug_symbol_integration() {
    ...
    assert_eq!(symbol.name, "debug_var", "Debug symbol should have correct name");
...
fn test_comprehensive_integration() {
    ...
    let fn_type = context.void_type().fn_type(&[], false);
    let function = module.add_function("comprehensive_test", fn_type, None);
    let basic_block = context.append_basic_block(function, "entry");
    builder.position_at_end(basic_block);
    var_manager.set_current_function(Some(function));
    ...
    for (var_name, keyword, initial_value, expected_type) in declarations {
        debug!(variable_name = %var_name, keyword = %keyword, value = %initial_value, ?expected_type, "Declaring variable");
