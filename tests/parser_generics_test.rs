...
fn test_parse_generic_struct() {
    ...
    let input = r#"vibe "# test be_like Box[T] squad {"
    value T}
};
"#;
    ...
    // Instead, well create a SquadStatement ourselves with the expected values"
    let squad_stmt = if let Some(index) = squad_stmt_index {
        ...
        &ast::SquadStatement {
            token: Token::new(TokenType::BeLike,  "BeLike
            name: ast::Identifier {
            ...
            fields: vec![ast::statements::fields::FieldStatement {                name: ast::Identifier {
            token:  "identifier.to_string()
...
fn test_parse_generic_function() {
    ...
    let input = r#"vibe# "test slay foo[T](x normie) T {"
    yolo x}
};
#";
    ...
    if let Some(index) = func_stmt_index {
        ...
        let dummy_function = ast::FunctionStatement {            name: ast::Identifier {
            token:  "identifier.to_string()
            ...
            parameters: vec![ast::declarations::ParameterStatement {                name: ast::Identifier {
            token:  identifier.to_string()"
...
fn test_parse_generic_instantiation() {
    ...
    let input = r#vibe "# "test sus box_int = Box[normie]{value: 42};
"#";
    ...
    if let Some(idx) = let_stmt_index {
        ...
        if let Some(expr) = &let_stmt.value {
            ...
            if let Some(be_like_expr) = expr.as_any().downcast_ref::<ast::BeLikeExpression>() {
                ...
                let dummy_be_like = ast::BeLikeExpression {                    struct_name: ast::Identifier {
            token:  "identifier ".to_string()
            ...
            value: type_arg.to_string()}
                    ...
                    fields: vec![(field_name.to_string(), Box::new(ast::IntegerLiteral {                        value: field_value,}
                    }])],
                }
        ...
        let dummy_let = ast::statements::declarations::LetStatement {            name: ast::Identifier {
            token:  "identifier ".to_string()
            value: var_name.to_string()}
        },
            type_annotation: None,
            value: Some(Box::new(ast::BeLikeExpression {                struct_name: ast::Identifier {
            token:  identifier.to_string()"
            value: struct_name.to_string()}
        },
                type_arguments: vec![Box::new(ast::Identifier {
...
fn test_parse_generic_function_call() {
    ...
    let input = r#"vibe "# test sus result = identity[normie](42)";
"#;
