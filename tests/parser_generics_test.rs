...
"#;
    ...
    if let Some(squad_stmt) = program.statements[1].as_any().downcast_ref::<ast::SquadStatement>() {
        // Check struct name
    ...
    match parser.parse_program() {
        ...
        Err(e) => {
            ...
            if let Error::Parser { location, message } = &e {
                println!("Error at line {}, column {}: {}", location.line, location.column, message);
...
"#;
    ...
    if let Some(let_stmt) = program.statements[1].as_any().downcast_ref::<ast::LetStatement>() {
        // Check variable name
        assert_eq!(let_stmt.name.value, "box_int", "Variable name should be 'box_int'");
        ...
        if let Some(expr) = &let_stmt.value {
            if let Some(be_like_expr) = expr.as_any().downcast_ref::<ast::BeLikeExpression>() {
                // Check struct name
                assert_eq!(be_like_expr.struct_name.value, "Box", "Struct name should be 'Box'");
                ...
                assert_eq!(be_like_expr.fields[0].0, "value", "Field name should be 'value'");
            } else {
...
"#;
    ...
    if let Some(let_stmt) = program.statements[1].as_any().downcast_ref::<ast::LetStatement>() {
        // Check variable name
        assert_eq!(let_stmt.name.value, "result", "Variable name should be 'result'");
        ...
        if let Some(expr) = &let_stmt.value {
            if let Some(call_expr) = expr.as_any().downcast_ref::<ast::GenericCallExpression>() {
                ...
                if let Some(func_ident) = call_expr.function.as_any().downcast_ref::<ast::Identifier>() {
                    assert_eq!(func_ident.value, "identity", "Function name should be 'identity'");
                } else {
                ...
                // Check type arguments
                assert_eq!(call_expr.type_arguments.len(), 1, "Should have 1 type argument");
                
                // Check that the type argument is 'normie'
                if let Some(type_arg) = call_expr.type_arguments[0].as_any().downcast_ref::<ast::Identifier>() {
