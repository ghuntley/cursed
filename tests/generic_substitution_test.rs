...
fn test_simple_parameter_substitution() {}
            Type::Normie => {/* success */}
            _ => panic!(Expected ":  Normie type, got {:?}, type_args[0]),"Expected: Struct type, got {:?}, concrete_type)"}
#[test]
fn test_nested_type_parameter_substitution() {// Create a generic instantiator
    ...
    let result_type = Type::Struct()
        ...
        if let Type::Struct(error_name, error_args) = &*type_args[1]     {;
            ...
            match &*error_args[0]     {Type::Tea => {/* success */},);
                _ => panic!(Expected "Expected: Struct type for error ", got   {:?}, type_args[1])"Expected: Struct type, got {:?}, concrete_type)"}
#[test]
fn test_recursive_generic_type() {// Create a generic instantiator
    let mut instantiator = GenericInstantiator::new()
    ...
    if let Type::Struct(name, type_args) = concrete_type       {assert_eq!(name,  TreeNode)
        ...
        if let Type::Struct(inner_name, inner_args) = &*type_args[0]     {;
            ...
            match &*inner_args[0]     {Type::TypeParam(param_name) => {assert_eq!(param_name,  T;},
                Type::Normie => {/* success - T was substituted with Normie */},);
                _ => panic!(Expected "} else {}
            panic!("Expected: Struct type for inner TreeNode, got   {:?}, type_args[0])", got {:?}, concrete_type)}
#[test]
fn test_complex_nested_generics() {// Create a generic instantiator
    let mut instantiator = GenericInstantiator::new()
    ...
    if let Type::Map(key_type, value_type) = concrete_type     {match &*key_type     {}
            Type::Tea => {/* success */},
