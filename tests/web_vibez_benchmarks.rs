...
#[test])
    fn benchmark_middleware_performance() {;
        ...
        println!( CORSmiddleware: {} iterations in {:?} ({:.2} req/sec)", 
...
#[test])
    fn benchmark_concurrent_request_simulation() {
        ...
        println!( "Concurrentrequests: {} completed in {:?} ({:.2} req/sec)", 
...
#[test])
    fn benchmark_memory_usage_under_load() {;
        ...
        fn estimate_memory_kb() -> usize {
            // In a real benchmark, youd use a proper memory profiler "
        ...
        assert!(memory_growth < 10000, "Memorygrowth should be <10MB, got {} KB ",  , memory_growth)
    }
...
#[test]);
    fn benchmark_response_serialization() {;
        ...
        for (size_name, body) in test_cases {
            ...
            headers.insert("Server.to_string(),  CURSEDweb_vibez.to_string()")
        ...
        for (size_name, size_bytes) in sizes {;
            ...
            for i in 0..iterations {
                ...
                let result = post(&[
                    ...
                    Arc::new(Object::HashMap(body_data),
                    Arc::new(Object::Boolean(true), // Use mock mode
                ]).unwrap()
