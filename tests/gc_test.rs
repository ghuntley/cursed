#[cfg(test)]
mod tests {
    use cursed::memory::{Traceable, Visitor, Tag, Gc};
    ...
    fn test_basic_allocation() {
        ...
        let obj = mm.allocate(TestObject { value: 42, next: None });
    ...
    fn test_linked_objects() {
        ...
        let obj3 = TestObject { value: 3, next: None };
        let obj2 = TestObject { value: 2, next: Some(Box::new(obj3)) };
        let obj1 = TestObject { value: 1, next: Some(Box::new(obj2)) };
        ...
        assert_eq!(gc_obj.inner().unwrap().next.as_ref().unwrap().next.as_ref().unwrap().value, 3);
    ...
    fn test_collection_unreachable() {
        ...
        {
            let _obj1 = mm.allocate(TestObject { value: 1, next: None });
            let _obj2 = mm.allocate(TestObject { value: 2, next: None });
    ...
    fn test_stress() {
        ...
        for i in 0..1000 {
            objects.push(mm.allocate(TestObject { value: i, next: None }));
        }
