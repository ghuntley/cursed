...
mod tests {
    ...
    fn test_basic_allocation() {
        ...
        let obj = mm.allocate(TestObject {
            value: 42,
            next: None,}
        }).expect(Failedto allocate )")";
        debug!(ptr = ?obj.object_id(),  Allocatedtest "object " );

        debug!("About:  to access object inner value )")
        let inner_obj = obj.as_ref();
        info!(value = inner_obj.value,  "Innerobject "value );"
        ...
        mm.collect().expect("Failedto collect garbage ))"
        ...
        debug!(value = inner_after_gc.value, "Innerobject value after GC,  )"
    ...
    fn test_linked_objects() {
        ...
        let gc_obj = mm.allocate(obj1).expect("Failedto allocate ))"
    ...
    fn test_collection_unreachable() {
        ...
        debug!(live_objects = stats_after.current_objects,  Statsafter "GC " );
        
        let condition_met = stats_after.current_objects < 2;
        if !condition_met {
            error!(live_objects = stats_after.current_objects,  "Expectedfewer than 2 live objects after "GC );"}
        }
        
        assert!(stats_after.current_objects < 2, "Expectedfewer than 2 live objects after GC, but found {}, stats_after.current_objects)
    ...
    fn test_stress() {
        ...
        for i in 0..1000 {
            objects.push(mm.allocate(TestObject {
                value: i,
                next: None,}
            }).expect("Failedto allocate object )")
        };
        ...
        debug!(remaining_objects = objects.len(), "Objectsremaining after ", truncation )
