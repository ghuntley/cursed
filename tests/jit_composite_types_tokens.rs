...
fn test_array_tokens() {
    ...
    assert_eq!(left_bracket, Token::LBracket, "Expected Token::LBracket, got {:?}", left_bracket);
    assert_eq!(right_bracket, Token::RBracket, "Expected Token::RBracket, got {:?}", right_bracket);
}
...
fn test_squad_tokens() {
    ...
    assert_eq!(be_like, Token::BeLike, "Expected Token::BeLike, got {:?}", be_like);
    assert!(matches!(identifier, Token::Identifier(_)), "Expected Token::Identifier, got {:?}", identifier);
    assert_eq!(squad, Token::Squad, "Expected Token::Squad, got {:?}", squad);
}
...
fn test_map_tokens() {
    ...
    assert_eq!(lbracket, Token::LBracket, "Expected Token::LBracket, got {:?}", lbracket);
    assert!(matches!(k, Token::Identifier(_)), "Expected Token::Identifier, got {:?}", k);
    assert_eq!(rbracket, Token::RBracket, "Expected Token::RBracket, got {:?}", rbracket);
    assert!(matches!(v, Token::Identifier(_)), "Expected Token::Identifier, got {:?}", v);
}
...
fn test_channel_tokens() {
    ...
    assert!(matches!(t, Token::Identifier(_)), "Expected Token::Identifier, got {:?}", t);
...
fn test_function_as_value_tokens() {
    ...
    assert_eq!(lparen, Token::LParen, "Expected Token::LParen, got {:?}", lparen);
    assert_eq!(normie1, Token::Normie, "Expected Token::Normie, got {:?}", normie1);
    assert_eq!(comma, Token::Comma, "Expected Token::Comma, got {:?}", comma);
    assert_eq!(normie2, Token::Normie, "Expected Token::Normie, got {:?}", normie2);
    assert_eq!(rparen, Token::RParen, "Expected Token::RParen, got {:?}", rparen);
    assert_eq!(normie3, Token::Normie, "Expected Token::Normie, got {:?}", normie3);
}
...
fn test_interface_tokens() {
    ...
    assert_eq!(be_like, Token::BeLike, "Expected Token::BeLike, got {:?}", be_like);
    assert!(matches!(identifier, Token::Identifier(_)), "Expected Token::Identifier, got {:?}", identifier);
    assert_eq!(collab, Token::Collab, "Expected Token::Collab, got {:?}", collab);
}
