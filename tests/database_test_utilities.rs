    ...
    impl Drop for DatabaseFixture {
        fn drop(&mut self) {
            if self.cleanup_on_drop {
                ...
                // In real implementation, we'd use a blocking runtime or defer cleanup
...
pub mod mocks {
    ...
    impl DatabaseConnection for MockConnection {
        ...
        async fn execute(&self, sql: &str, parameters: Vec<SqlValue>) -> DbResult<ExecuteResult> {
            // Record the query
        ...
        async fn query(&self, sql: &str, parameters: Vec<SqlValue>) -> DbResult<QueryResult> {
            // Record the query
            self.queries.lock().unwrap().push(format!("{} [params: {:?}]", sql, parameters));
        ...
        async fn close(&mut self) -> DbResult<()> {
            ...
            self.queries.lock().unwrap().push("CLOSE CONNECTION".to_string());
            Ok(())
        }
    }
...
pub mod assertions {
        ...
        match result {
            ...
            Err(e) => {
                ...
                assert!(error_string.contains(expected_error_type), 
                    "Expected error type '{}', but got: {}", expected_error_type, error_string);
            }
        }
    }
    ...
    pub fn assert_row_count(result: &QueryResult, expected_count: usize) {
        assert_eq!(result.row_count(), expected_count, 
            "Expected {} rows, but got {}", expected_count, result.row_count());
    }
    ...
    pub fn assert_rows_affected(result: &ExecuteResult, expected_count: i64) {
        assert_eq!(result.rows_affected(), expected_count,
            "Expected {} rows affected, but got {}", expected_count, result.rows_affected());
    }
    ...
    pub fn assert_pool_stats(stats: &PoolStatistics, expected_total: u32, expected_active: u32) {
        assert_eq!(stats.total_connections(), expected_total,
            "Expected {} total connections, but got {}", expected_total, stats.total_connections());
        assert_eq!(stats.active_connections(), expected_active,
            "Expected {} active connections, but got {}", expected_active, stats.active_connections());
    }
    ...
    pub fn assert_queries_executed(mock: &mocks::MockConnection, expected_queries: &[&str]) {
        ...
        for expected in expected_queries {
            assert!(mock.was_query_executed(expected),
                "Expected query '{}' was not executed. Executed queries: {:?}", 
    ...
    pub async fn assert_table_row_count(connection: &mut dyn DatabaseConnection, table_name: &str, expected_count: usize) {
        ...
        let actual_count = query_result.rows()[0].get_i64("count").expect("count column should exist") as usize;
        assert_eq!(actual_count, expected_count,
            "Expected {} rows in table '{}', but found {}", expected_count, table_name, actual_count);
    }
}
...
mod tests {
    ...
    async fn test_mock_connection() {
        ...
        // Execute operation
        let result = mock.execute("INSERT INTO users VALUES (?, ?)", vec![
