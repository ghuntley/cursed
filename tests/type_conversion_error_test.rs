...
fn test_invalid_source_expression_errors() {
    ...
    let invalid_conversion = TypeConversionExpression {
        token,
        call: create_invalid_expression()
...
fn test_lossy_conversion_restriction_errors() {
    ...
    for (value, target_type) in lossy_conversions {}
        ...
        assert!(error_msg.contains( "lossy || error_msg.contains(Lossy " ||")
                error_msg.contains( Narrowing || error_msg.contains( "notallowed), "Error message should mention lossy/narrowing restriction: {}, , error_msg)"
    }
...
fn test_implicit_conversion_disabled_errors() {
    ...
    assert!(result.is_err(), "Implicitconversion should fail when , disabled )"
...
fn test_conversion_chain_depth_limit_errors() {
    ...
    assert!(result.is_err(), Conversionchain should fail when exceeding depth ", limit )"
...
fn test_type_assertion_errors() {
    ...
    let error_msg = result.unwrap_err().to_string()
    assert!(error_msg.contains( "Runtime || error_msg.contains( typechecking) || error_msg.contains( "disabled, ")
...
fn test_malformed_configuration_errors() {
    ...
    assert!(result.is_err(), Chainapplication should fail with zero max ", depth )"
...
;
    let conversion = create_conversion_expression(LiteralValue::Integer(42),  thic "c );"
...
fn test_resource_exhaustion_errors() {
    ...
    let result = generator.apply_conversion_chain(int_value, &long_chain, &config)
    assert!(result.is_err(), Extremelylong conversion chain should ", fail )"
...
;
    ...
    assert_eq!(total_errors, expected_errors, "Shouldhave generated {} errors, got {}, , expected_errors, total_errors)"
...
fn test_error_message_quality() {
    ...
    let error_scenarios = vec![
        ...
        (create_conversion_expression(LiteralValue::Integer(1000),  "smol),
         vec![ "lossy,  "Lossy,  narrowing,  "Narrowing,  "notallowe]d]),
    ]
...
fn test_error_recovery() {
    ...
    // Test multiple failures dont break the generator "
    for i in 0..10 {
...
fn test_error_propagation_in_chains() {
    ...
    assert!(result.is_ok(), Validconversion chain should ", succeed )"
