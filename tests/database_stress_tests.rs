...
mod pool_stress_tests {
    ...
    async fn perform_database_operation(
        ...
        let connection = {
            ...
            tokio::time::timeout(Duration::from_secs(5), pool_guard.acquire()).await
        ...
        connection.execute("CREATE TEMP TABLE IF NOT EXISTS stress_test (
        ...
        // Insert data
        connection.execute("INSERT INTO stress_test (worker_id, operation, data) VALUES (?, ?, ?)", vec![
            SqlValue::Integer(worker_id as i64),
            SqlValue::Integer(operation as i64),
            SqlValue::Text(format!("Data from worker {} operation {}", worker_id, operation))
        ]).await?;
        
        // Query data
        let result = connection.query("SELECT COUNT(*) as count FROM stress_test WHERE worker_id = ?", vec![
            SqlValue::Integer(worker_id as i64)
        ]).await?;
        
        let count = result.rows()[0].get_i64("count")?;
    ...
    async fn test_pool_manager_under_load() {
        ...
        for (name, min_size, max_size) in &pool_configs {
            ...
            manager.create_pool(name, config).await.unwrap();
            manager.start_pool(name).await.unwrap();
        }
    ...
    async fn perform_pool_manager_operation(
        ...
        // Insert data
        connection.execute(&format!("INSERT INTO {} (worker_id, operation, pool_name) VALUES (?, ?, ?)", table_name), vec![
            SqlValue::Integer(worker_id as i64),
            SqlValue::Integer(operation as i64),
...
mod memory_stress_tests {
    ...
    async fn test_large_result_sets() {
        ...
        for i in 0..record_count {
            txn.execute("INSERT INTO large_data (data, number, flag) VALUES (?, ?, ?)", vec![
                SqlValue::Text(format!("Large data record {} with some additional content to make it bigger", i)),
                SqlValue::Integer(i),
        ...
        let result = connection.query("SELECT * FROM large_data ORDER BY id", vec![]).await.unwrap();
        
        let query_time = query_start.elapsed();
        println!("✅ Queried {} records in {:?} ({:.0} records/sec)", 
                result.row_count(), query_time, result.row_count() as f64 / query_time.as_secs_f64());
        
        assert_eq!(result.row_count(), record_count);
        ...
        // Verify correctness
        let expected_sum = (0..record_count).sum::<usize>() as i64;
    ...
    async fn test_concurrent_large_queries() {
        ...
        setup_conn.execute("CREATE TABLE concurrent_large (
            id INTEGER PRIMARY KEY,
            category INTEGER,
            data TEXT,
            value REAL
        ...
        let mut txn = setup_conn.begin_transaction().await.unwrap();
        for i in 0..data_size {
            txn.execute("INSERT INTO concurrent_large (category, data, value) VALUES (?, ?, ?)", vec![
        ...
        assert!(successful >= (worker_count * 3 / 4), "Too many queries failed");
    ...
    async fn perform_large_query(pool: &Arc<Mutex<ConnectionPool>>, worker_id: usize) -> Result<usize, DatabaseError> {
        ...
        let result = match query_type {
            0 => {
                // Full table scan
                connection.query("SELECT * FROM concurrent_large ORDER BY id", vec![]).await?
            }
            1 => {
                // Filtered query
                let category = (worker_id % 10) as i64;
                connection.query("SELECT * FROM concurrent_large WHERE category = ? ORDER BY value DESC", vec![
                    SqlValue::Integer(category)
                ]).await?
            }
            2 => {
                // Aggregation query
                connection.query("SELECT category, COUNT(*) as count, AVG(value) as avg_value FROM concurrent_large GROUP BY category", vec![]).await?
            }
            _ => {
                // Range query
                let start_id = (worker_id * 1000) as i64;
                let end_id = start_id + 5000;
                connection.query("SELECT * FROM concurrent_large WHERE id BETWEEN ? AND ? ORDER BY id", vec![
                    SqlValue::Integer(start_id),
...
mod timeout_stress_tests {
    ...
    async fn test_connection_timeout_stress() {
        ...
        {
            let pool_guard = pool_arc.lock().unwrap();
            pool_guard.release(held_connection).await.unwrap();
        }
        ...
        let recovery_connection = {
            let pool_guard = pool_arc.lock().unwrap();
            pool_guard.acquire().await.unwrap()
        };
        
        recovery_connection.execute("SELECT 1", vec![]).await.unwrap();
        
        {
            let pool_guard = pool_arc.lock().unwrap();
            pool_guard.release(recovery_connection).await.unwrap();
            pool_guard.stop().await.unwrap();
        }
    ...
    async fn acquire_with_timeout(pool: &Arc<Mutex<ConnectionPool>>) -> Result<Box<dyn DatabaseConnection>, DatabaseError> {
        let pool_guard = pool.lock().unwrap();
        tokio::time::timeout(Duration::from_millis(150), pool_guard.acquire()).await
            .map_err(|_| DatabaseError::connection(
                db_core::ConnectionError::Timeout,
                "Acquisition timeout"
            ))?
    ...
    async fn test_long_running_operations() {
        ...
        let mut txn = connection.begin_transaction().await.unwrap();
        for i in 0..10000 {
            txn.execute("INSERT INTO long_ops (data) VALUES (?)", vec![
                SqlValue::Text(format!("Data item {}", i))
            ]).await.unwrap();
        }
        txn.commit().await.unwrap();
        ...
        println!("✅ Long query completed in {:?} (returned {} rows)", query_time, result.row_count());
        
        assert!(result.row_count() > 0, "Long query should return results");
        ...
        let tasks: Vec<_> = (0..concurrent_ops).map(|op_id| {
            ...
            tokio::spawn(async move {
                let mut conn = SqliteDriver::new().connect(config_clone).await.unwrap();
                let start_time = Instant::now();
...
mod resource_exhaustion_tests {
    ...
    async fn test_file_descriptor_exhaustion() {
        ...
        for i in 0..max_connections {
            ...
            match SqliteDriver::new().connect(config).await {
                Ok(connection) => {
                    ...
                    if connection.execute("CREATE TABLE test (id INTEGER PRIMARY KEY)", vec![]).await.is_ok() {
        ...
        for (connection, _) in &connections {
            if connection.query("SELECT 1 as test", vec![]).await.is_ok() {
        ...
        let mut test_connection = SqliteDriver::new().connect(test_config).await.unwrap();
        test_connection.execute("SELECT 1", vec![]).await.unwrap();
    ...
    async fn test_memory_exhaustion_protection() {
        ...
        let data_sizes = vec![1024, 4096, 16384, 65536, 262144]; // 1KB to 256KB per record
        ...
        for (batch, &size) in data_sizes.iter().enumerate() {
            ...
            for i in 0..batch_size {
                ...
                txn.execute("INSERT INTO memory_test (id, large_data) VALUES (?, ?)", vec![
                    SqlValue::Integer(record_id as i64),
            ...
            let count = result.rows()[0].get_i64("count").unwrap();
            let avg_size = result.rows()[0].get_f64("avg_size").unwrap();
            ...
            // In a real implementation, you'd check actual memory usage here
        ...
        println!("✅ Retrieved {} records in {:?}", result.row_count(), large_query_time);
        ...
        assert!(result.row_count() > 400, "Should have substantial amount of test data");
