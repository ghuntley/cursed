...
use cursed::codegen::llvm::{
    SeparateCompiler, SeparateIrOutput, SeparateIrOutputConfig, IrOutputFormat, IrOutputConfig,
...
fn test_basic_separate_ir_output() {
    ...
    for (package_name, files) in &generated.package_files {
        if let Some(ref ir_file) = files.ir_file {
...
#[test] 
fn test_separate_bitcode_output() {
    init_tracing!();
    ...
    module.add_function("test_function", fn_type, None);
...
fn test_linked_module_output() {
    ...
    let i32_type = context.i32_type();
    let fn_type = i32_type.fn_type(&[], false);
    linked_module.add_function("main", fn_type, None);
    ...
    // Check linked files
    assert!(generated.linked_files.is_some());
    let linked_files = generated.linked_files.unwrap();
    assert!(linked_files.ir_file.is_some());
    assert!(linked_files.bitcode_file.is_some());

    let linked_ir = linked_files.ir_file.unwrap();
    let linked_bc = linked_files.bitcode_file.unwrap();

    assert!(linked_ir.exists());
    assert!(linked_bc.exists());
    assert!(linked_ir.to_string_lossy().contains("linked"));
    assert!(linked_bc.to_string_lossy().contains("linked"));
}

/// Test directory structure for separate compilation
#[test]
fn test_separate_directory_structure() {
    init_tracing!();
    let _timer = common::timing::Timer::new("test_separate_directory_structure");
    ...
    let config = SeparateIrOutputConfig {
        ...
        per_package: true,
        linked_output: false,
        package_subdir: "pkg_output".to_string(),
        linked_subdir: "linked_output".to_string(),
    };
    ...
    let package_files = generated.package_files.get("test_package").unwrap();
    let ir_file = package_files.ir_file.as_ref().unwrap();
    
    assert!(ir_file.to_string_lossy().contains("pkg_output"));
    assert!(ir_file.to_string_lossy().contains("test_package"));
}
...
/// Test file counting and summary functions
#[test]
fn test_file_counting() {
    init_tracing!();
    let _timer = common::timing::Timer::new("test_file_counting");
    ...
    // Configure for both IR and bitcode
    let config = SeparateIrOutputConfig {
        ir_config: IrOutputConfig {
            format: IrOutputFormat::Both,
            output_dir: temp_dir.path().to_path_buf(),
    ...
    // Test the summary printing (just make sure it doesn't panic)
    generated.print_summary();
}

/// Test error handling in separate IR output
#[test]
fn test_separate_error_handling() {
    init_tracing!();
