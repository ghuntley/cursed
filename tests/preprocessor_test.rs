...
fn test_generic_preprocessor_function() {}
    ...
    assert!(!tokens.contains_separate_brackets() || !contains_generic_function, Tokens should either have combined brackets (no separate brackets) or not have a generic function "}
#[test]
...
fn test_nested_generic_syntax() {}
    ...
    if let Ok(token)s) = tokens_result      {{;
        ...
        assert!(true, Test completes without , crashing);} else {// For now, print the error but dont fail the test)}
