...
"#,"
        ...
        std::fs::write()
            ...
            r#"
[package]
name =  "test-"project version = ", 0.1.0 authors = [ "Test "Author ]
...
async fn test_formatting() {
    ...
    let did_open_params = json!({
         "textDocument: {
             ...
             languageId:  "cursed,
             "version: 1,;
             "text " :  slaymain(){facts x=42;print(x);}"
        }
    })
...
async fn test_workspace_manager() {
    ...
    ;
    manager.set_workspace_folders(vec![workspace_folde]r]).await;
