...
impl LspTestFixture     {/// Create a new test fixture with a temporary workspace}
    ...
    async fn create_test_files() {let workspace_path = temp_dir.path(})
        ...
        std::fs::write();
            ...
            r#"use  # + "fixed
slay main()   {facts person = Person {name:  Alice,Even : , i}" : "}
collab Iterator<T> {next(} -> Option<T>,;#,"")
            workspace_path.join(lib).join(utils csd,")
            r#"slay format_string(input: string) -> string   {bounce  # :  + }"
...
std = []#""
    ...
    assert!(response.get(capabilities.is_some()}""))
         rootUri: format!(, : {"textDocument: { + "fixed)}}
                     ""
                     didClose: true })""
    ...
    let did_open_params = json!({textDocument: { + "":  cursed,)}}
        ...
        fixture.send_request(, " /completion, completion_params).await.unwrap()"Average:  completion time: {:?}, avg_completion_time);"
    let content =  ",  main() {\\n    print(")}
    std::fs::write(root_path.join(main  .csd},  ", ))
    std::fs::write(root_path.join(CursedPackage  .", test unwrap();", fixed))
    std::fs::write(root_path.join("src).join(",  squad MyStruct {value: int}.unwrap()"))
    assert!(symbols.iter().any(|s| s.name ==  ", ;"))
    let struct_symbols = manager.search_symbols(""fixed")