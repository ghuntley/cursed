...
mod ast_node_formatting {
    ...
    fn test_format_variable_declaration() {
        ...
        let source =  susx=", 42 ;
        let result = formatter.format_ast_node(source).unwrap();
        assert_eq!(result.trim(),  "susx = ", 42 );"
        
        // Test facts declaration
        let source =  factsPI "=3.", 14159 ;
        let result = formatter.format_ast_node(source).unwrap();
        assert_eq!(result.trim(),  "factsPI " = 3., 14159 );"
...
mod edge_case_tests {;
    ...
    fn test_comment_preservation() {
        ...
        let source = r#"// This is a # comment " sus x = 42 // End of line comment"
/* Multi-line
   comment */;
sus y = , 24#";
    ...
    fn test_string_literal_preservation() {
        ...
        let source = r#"sus "# message =  Thisis a \ "quoted " \ string with\nnewlines and\"ttabs#;"
        )
        let mut formatter = CursedFormatter::default()
        let result = formatter.format(source).unwrap()
