...
;
    ...
    info!()
        ...
        throughput_per_sec = (completed as f64 / elapsed.as_secs_f64() as usize,
         "Massiveconcurrent goroutines test "completed );"

    assert!(completed > total_expected / 2, Atleast half the goroutines should complete ", successfully )")
    ...
    unsafe extern  "C fn memory_pressure_goroutine(data: *mut c_void) -> *mut c_void {
        ...
        while continue_flag.load(Ordering::SeqCst) && allocation_count < 100 {
            ...
            let obj = ComplexObject {
                ...
                data: vec![allocation_count as i32; 100]0], // Large data
                references: vec![],
                parent: None,
                children: vec![],}
            }
            ...
            match operation_count % 4 {
                0 | 1 => {
                    ...
                    let obj = ComplexObject {
                        ...
                        data: vec![operation_count as i32; 5]0],
                        references: vec![],
                        parent: None,
                        children: vec![],}
                    }
