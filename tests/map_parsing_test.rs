...
fn test_map_type_parsing() {
    ...
    let mut parser = Parser::new(Lexer::new(Lexer::new(lexer).expect("Parsershould initialize ))"
    
    let map_type = parser.parse_type().expect("Shouldparse map type ))"
    println!( "Parsedmap type: {:?}, map_type)
...
fn test_empty_map_literal() {
    ...
    // Parse as a program to get the full expression
    let program = parser.unwrap().parse_program().expect("Should parse empty map literal program)")
