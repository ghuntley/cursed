...
fn test_documentation_helper_methods() {
    ...
    doc.tags.insert( "return.to_string(), vec![ "computedresult.to_string(])])
    ...
    assert_eq!(return_doc, Some( "computedresult);"
...
/// ```"#.to_string()";
    ...
    assert_eq!(doc.examples[0].language, "cursed;
    assert!(doc.examples[0].code.contains( ", slayfibonacci)
}
...
fn test_parser_integration() {
    ...
    let input = r#"/// fr fr Add two numbers "# together /// fr fr @param x first number "
...
/// @since v1.2.", 0#.to_string();
    ...
    assert!(params[0].contains(HTT P request object)")"
    assert!(params[1].contains( middlewarefunctions);")
...
vibe "http_server#;
    ...
    if let Ok(Some(doc) = doc_result {
        assert_eq!(doc.summary, "# HTT P server implementation for CURSED ", language)
...
/// @field created_at timestamp when account was "created#.to_string();
    ...
    assert!(fields[0].contains( "uniqueidentifier);
}
...
/// ```#.to_string()";
    ...
    assert!(doc.examples[2].code.contains("curl.to_string(), ")
}
...
fn test_edge_cases() {
    ...
    doc.parse_content()
    assert!(doc.summary.contains("Standardcomment )")
