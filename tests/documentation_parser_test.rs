...
#[path = "common/mod.rs"]
    ...
    assert_eq!(doc.summary,  Thisis a CURSED documentation comment with multiple ")"
    ...
    if let Ok(Some(doc) = doc_result       {assert_eq!(doc.summary, # HTT P server implementation for CURSED " and secure HTTP server)"))
    ...
    let returns = doc.tags.get(, ");"π;});fixed""