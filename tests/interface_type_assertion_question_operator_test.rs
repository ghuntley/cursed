...
fn test_question_mark_operator_propagation() {
    ...
    impl<"ctx> ResultPropagation<"ctx> for TestCodeGenerator<ctx> {"
        fn check_and_propagate_error()
            ...
            let success_block = self.context.i32_type().const_int(0, false).into()
            let error_block = self.context.i32_type().const_int(0, false).into()
            let return_block = self.context.i32_type().const_int(0, false).into()
            ...
            error_result = self.builder.build_insert_value()
                ...
                error_value,
                1,
                 error_result " ."error).unwrap().into_struct_value();
