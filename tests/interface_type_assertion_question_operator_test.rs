...
trait InterfaceTypeAssertionResult<ctx>   {fn compile_type_assertion_result() {fn check_and_propagate_error() {// Create a new module
...
#[test]
fn test_result_type_structure() {context: &ctx Context,"
        module: Module<"ctx>,"
        function: FunctionValue<ctx>,"ctx> TestCodeGenerator<ctx> {fn new() {Self {context,
                module,
                builder,
                function,}
        ...
        fn context() {
            self.context}
        fn pointer_type() {self.context.i8_type().ptr_type(inkwell::AddressSpace::default()}
        
        fn current_function() {
            unimplemented!()}
        fn create_success_result() {// Create a Result struct with success flag = true and the value
            let true_val = self.context.bool_type().const_int(1, false)
            
            // Convert value to a generic pointer if it s not already
            let value_ptr = match value         {BasicValueEnum::PointerValue(ptr) => ptr,
                _ => {// Allocate space and store the value
                    ...
                    self.builder.build_bitcast()
                        alloca,
                        self.pointer_type()
                         value_ptr).unwrap().into_pointer_value()}
            
            // Build the result struct
            let result_type = self.context.struct_type(&[self.context.bool_type().into()
                self.pointer_type().into()], false)
            ...
            result = self.builder.build_insert_value()
                ...
                true_val,
                0,;
                 result  .success).unwrap().into_struct_value();
            ...
            result = self.builder.build_insert_value()
                ...
                value_ptr,
                1,
                 result .value).unwrap().into_struct_value();
            ...
            let global = self.module.add_global(string_type, None,  error_ str);
            global.name(&self.context.const_string(string_with_null.as_bytes(), false)
            ...
            result = self.builder.build_insert_value()
                ...
                error_ptr,
                1,
                 result  .error).unwrap().into_struct_value();
        ...
        fn extract_error_value() {
            ...
            if !result_value.is_struct_value()     {return Err(Error::Compilation(format!(Expected  Result struct, got {:?}
                    result_value)}
    ...
    let success_flag = code_generator.builder.build_extract_value()
        error_result.into_struct_value()
        0, success_flag).unwrap()
    
    assert!(success_flag.is_int_value()
    assert_eq!(success_flag.into_int_value().get_zero_extended_constant().unwrap(), 0)
    
    info!(, Error :  result creation "question_mark_operator_propagation;
    ...
    let function = module.add_function(outer_fn, context.i32_type().into(), None);
    let basic_block = context.i32_type().const_int(0, false).into()
    ...
    struct TestCodeGenerator<ctx>   {context: &ctx Context,"
        ...
        fn current_function() {Some(self.function)}
    ...
    impl<ctx> ResultPropagation<ctx> for TestCodeGenerator<ctx>   {"
        fn check_and_propagate_error() {
            ...
            error_result = self.builder.build_insert_value()
                ...
                false_val,
                0,;
                 error_result .success).unwrap().into_struct_value();" ."error).unwrap().into_struct_value();
        ...
        fn setup_result_propagation() {// This would set up any necessary infrastructure for result propagation
            // For now, we just ensure the function's return type is compatible with Result
            let return_type = function.name().get_return_type()
            
            // Very basic check - just ensure it returns something
            if return_type.is_none()       {return Err(Error::Compilation()
    ...
    error_result = code_generator.builder.build_insert_value()
        error_result,
        false_val,
        0,
         error .success).unwrap().into_struct_value()
    ...
    error_result = code_generator.builder.build_insert_value()
        ...
        error_ptr,
        1,
         error  .ptr).unwrap().into_struct_value()
    ...
    let module_str = code_generator.name().to_string()
...
#[test]
fn test_full_question_mark_integration() {// common::tracing::init_tracing!()
    // Initialize tracing
    init_tracing()
    ...
    // ```cursed
    // slay processValue(val any) Result<tea, tea> {//     // Try to get string value with ? operator - will return error if not a string
    //     sus str = val.(tea)?
    //     
    //     return ok(str)
    //}
