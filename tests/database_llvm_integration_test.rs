...
mod tests {
    ...
    fn test_gc_type_registrations() {
        let gc_registrations = vec![
            ...
            "register_gc_type(result_set, sizeof(ptr), result_destroy)",
            "register_gc_type(query_builder, sizeof(ptr), query_destroy)"
        ];
    ...
    fn test_function_signatures() {
        let function_signatures = vec![
            ("sql_vibes.connect", vec!["string", "parameters"],
            ("sql_vibes.execute", vec!["connection", "string", "parameters"],
        ...
        for (name, params) in &function_signatures {
            ...
            assert!(
                ...
                package == "db_migrate" || package == "db_nosql" || package == "db_query",
                "Package should be a valid database package: {}", package
            );
    ...
    fn test_mutation_functions() {
        let functions = vec![
            "create_table", "update_record", "delete_record", 
            "insert_data", "drop_table", "alter_table"
        ];
