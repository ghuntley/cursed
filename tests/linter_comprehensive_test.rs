...
fn test_severity_filtering() {
    ...
    let source = "this is a very long line\ntrailing space ";
    let issues = engine.lint_source(source, None).unwrap();
...
fn test_reporter_human_format() {
    ...
    let results = vec![(std::path::PathBuf::from("test.csd"), vec![issue])];
...
fn test_directory_linting() {
    ...
    fs::write(&file1, "sus x = 1\n").unwrap();
    fs::write(&file2, "this is a very very very very very very very long line\n").unwrap();
    fs::write(&file3, "this should be ignored").unwrap();
...
fn test_disabled_rules() {
    ...
    let source = "line with trailing space ";
    let issues = engine.lint_source(source, None).unwrap();
...
fn test_multiple_files_with_different_issues() {
    ...
    fs::write(&file1, "this is a very very very very very very very long line").unwrap();
    fs::write(&file2, "line with trailing space ").unwrap();
...
fn test_binary_file_detection() {
    ...
    // Don't assert on specific behavior, just ensure it doesn't panic
    let _ = result;
}
