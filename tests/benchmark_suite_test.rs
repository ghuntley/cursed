...
fn test_benchmark_harness() {// common::tracing::init_tracing!()
    ...
    suite.add_benchmark(cursed::benchmark::harness::Benchmark::new()
        ...
        || {// Sleep for a short time
            ...
            // Return metrics
            vec![Box::new(cursed::benchmark::metrics::TimingMetric   {name:  sleep_time.to_string()
                duration: Duration::from_millis(5)}].name, simple_benchmark;
...
fn test_gc_benchmark_suite() {// Initialize tracing
    tracing_setup::init_test_tracing()
    ...
    for benchmark in &mut suite.benchmarks    {benchmark.config.iterations = 2;
