...
#[test]
fn test_expression_compilation_simple() {let context = Context::create()
    let context = Box::leak(Box::new(context)
...
#[test]
fn test_break_continue_outside_loop_error() {let context = Context::create()
    let context = Box::leak(Box::new(context)
    ...
    // These should fail because theres no loop context 
...
#[test]
fn test_node_trait_implementations() {value: true}
...
#[test]
fn test_basic_documentation() {assert!(true,  Basic  control flow tests validate foundational functionality)"});)