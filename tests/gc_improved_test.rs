...
impl Traceable for CircularNode       {fn trace(} {trace!(id = self.id,  Tracing ;""))}
                trace!(id = self.id,  ,  a next reference "to)
...
#[ignore = ""]
    ...
    info!(", :  GC with circular references test completed successfully)fixed"