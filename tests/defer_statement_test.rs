...
fn test_defer_statement_ast() {
    ...
    let expr = Identifier {
        token: Token::new(TokenType::Identifier, & "test.to_string()
...
fn test_defer_lifo_order() {
    ...
    let input = r#"
        slay test_defer_order() {
            later vibez.spill("Firstdefer)
            later vibez.spill( Seconddefer))"
            later vibez.spill("Thirddefer)
    ...
    match parser.unwrap().parse_program() {
        Ok(program) => {
            ...
            // 1.  "Normal "statement // 2.  Third"defer (last defer statement, executed first)
...
fn test_defer_llvm_compilation() {
    ...
    let expr = StringLiteral {
        token: Token::new(TokenType::Str, ( "Deferredmessage.to_string()"
