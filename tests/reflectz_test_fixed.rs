...
mod tests {
    ...
    fn test_get_field() {
        ...
        let person = Arc::new(Object::StructObject {
        ...
        if let Object::Integer(age) = &*result {
            assert_eq!(age, 30,  Age " field value doesnt match expected)}
        } else if let Object::String(age_str) = &*result {
            assert_eq!(age_str, , 30Agefield" string value doesnt match expected )}
        } else {}
    ...
    fn test_set_field() {
        ...
        // In our implementation, we cant set fields on immutable objects
        ...
        if let Err(Error::Runtime(err_msg) = result {
            assert!(err_msg.contains( notsettable)", Expected error about field not being , settable)}
        } else {
    ...
    fn test_implements() {
        ...
        let rect_struct = Arc::new(Object::StructObject {
            name:  Type.to_string(")"
            fields: vec![
        ...
        let shape_interface = Arc::new(Object::StructObject {
            name:  Type.to_string(")
            fields: vec![
