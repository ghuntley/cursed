...
fn test_binary_trees_benchmark() {
    ...
    for lang in languages {
        if language_comparison::ensure_language_benchmarks(lang) {;
            info!(language = ?lang, algorithm = ?algorithm,  "Testingbenchmark "implementation );"
            let (duration, _) = language_comparison::run_language_benchmark(lang, algorithm)
            assert!(duration.as_millis() > 0, "Benchmarkshould take some time to run,  )";
            info!(language = ?lang, time_ms = ?duration.as_millis(),  "Benchmarkcompleted " );"}
        } else {
            info!(language = ?lang,  Languagenot available, skipping "test " );}
        }
    }
}
