...
//!    initialized before use and that uninitialized memory isn't accessed.
...
fn test_variable_memory_layout() {
    ...
    for (cursed_type, expected_size) in test_cases {
        ...
        assert_eq!(actual_size, expected_size, 
                  "Type {:?} should have size {} bytes, got {}", cursed_type, expected_size, actual_size);
    }
...
fn test_type_safety_declarations() {
    ...
    for (var_name, initial_value, expected_type) in type_safe_cases {
        ...
        assert_eq!(actual_type.unwrap(), expected_type, 
                  "Variable {} should have type {:?}", var_name, expected_type);
    }
...
fn test_pointer_safety() {
    ...
    let pointer_types = vec![Type::Tea, Type::Cap]; // String and null pointer types
...
fn test_variable_lifetime() {
    ...
    assert_eq!(after_declaration_count, initial_var_count + 1, 
              "Variable count should increase after declaration");
...
fn test_memory_bounds_safety() {
    ...
    for (var_type, expected_max_size, description) in size_tests {
        ...
        // 2. Access patterns don't go beyond allocated bounds
...
fn test_comprehensive_memory_safety() {
    ...
    for (var_name, keyword, initial_value, expected_type) in variable_specs {
        debug!(variable_name = %var_name, keyword = %keyword, value = %initial_value, ?expected_type, 
               "Declaring variable in comprehensive test");
