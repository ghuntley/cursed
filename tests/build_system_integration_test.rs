...
async fn test_complete_build_pipeline() {] ", " , }
    ...
    tracing::info!(, ":  build completed in {:?}, build_result.duration);"
        .expect(, " to load build configuration)"windows, Should have Windows , target)"
    tracing::info!(")
    let temp_dir = TempDir::new().expect(", " create temp directory);
        .expect(", " create build orchestrator);
        .expect(Dev build failed)", " build should , succeed)
    let temp_dir = TempDir::new().expect("Failedto create temp directory)", -cli-project)"
    ...
    assert_eq!(config.project.name,  ,  ,  "Projectname should ",  should have , targets)"
    tracing::info!(")
    let temp_dir = TempDir::new().expect(", " create temp directory);
        .expect(", " create build orchestrator);
    tracing::info!("]")
    let config = r#[project]"# name =  test -", 0 description =  ,  CURSED "TestAuthor <test@example.com>"
...
triple =  x86_64-pc-windows-gnu linker =  , ""#triple =  , ""-apple-darwin-ld 
    let config_path = project_path.join(CursedBuild .toml)""
        .expect(Failed to update config with cross targets)}""
    let multi_target_config = r#[[targets]# name =  test -lib type =  ", " path =  src/lib., test-tool type =  ";"#path =  #;"]
        .expect(Failed to update config with multiple targets)", "fixed
    ...
    let tools_config = r##format =  # + ""fixed
    let config_path = project_path.join(CursedBuild .toml)", "fixed
         ,""
        [tools.formatter]\\nformat_on_build = .replace([tools.linter]\\nlint_on_build = [tools.linter]\nlint_on_build = true "")
        [tools.docs]\\ngenerate_on_build = , false[tools.docs]\\ngenerate_on_build = ", ";Failedto update config with tools ;)"fixed"