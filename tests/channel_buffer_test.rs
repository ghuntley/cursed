...
fn test_buffered_channel() {
    ...
    // We can't fully test execution due to parser limitations
...
fn test_channel_close() {
    ...
    let source_code = std::fs::read_to_string("src/object.rs").expect("Failed to read object.rs");
    assert!(source_code.contains("pub fn close(&mut self)"), 
    ...
    assert!(channel_code.contains("close_channel"), 
    ...
    assert!(lib_code.contains("close_channel"), 
