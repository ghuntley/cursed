...
fn test_optimization_manager_creation() {
    ...
    for level in 0..=3 {
        ...
        match level {
            0 => assert!(matches!(config.level, OptimizationLevel::None)),
...
#[test]
fn test_optimization_manager_from_string() {
    init_tracing!();
    ...
    for (level_str, should_succeed) in test_cases {
        let result = create_optimization_manager(level_str);
        
        if should_succeed {
            assert!(result.is_ok(), "Failed to create manager for level {}", level_str);
        } else {
            assert!(result.is_err(), "Unexpectedly succeeded for invalid level {}", level_str);
        }
    }
}

#[test]
fn test_optimization_config_modification() {
    init_tracing!();
    ...
    new_config.level = OptimizationLevel::Default;
    new_config.optimize_size = true;
    new_config.custom_passes.push(OptimizationPass::DeadCodeElimination);
    new_config.custom_passes.push(OptimizationPass::ConstantFolding);
    
    manager.set_config(new_config);
    
    let updated_config = manager.get_config();
    assert!(matches!(updated_config.level, OptimizationLevel::Default));
    assert!(updated_config.optimize_size);
    assert_eq!(updated_config.custom_passes.len(), 2);
}

#[test]
fn test_optimization_pass_configuration() {
    init_tracing!();
...
#[test]
fn test_simple_module_optimization() {
    init_tracing!();
    ...
    for level in 0..=3 {
        let mut manager = OptimizationManager::for_level(level).unwrap();
...
#[test]
fn test_custom_optimization_passes() {
    init_tracing!();
...
fn test_size_optimization() {
    ...
    let function = module.add_function("size_test_function", fn_type, None);
    let basic_block = context.append_basic_block(function, "entry");
    ...
    let add2 = builder.build_int_add(add1, const_1, "add2").unwrap();
    let add3 = builder.build_int_add(add2, const_1, "add3").unwrap();
...
#[test]
fn test_optimization_benchmark_simulation() {
    init_tracing!();
    ...
    let i32_type = context.i32_type();
    let fn_type = i32_type.fn_type(&[i32_type.into()], false);
    let function = module.add_function("benchmark_function", fn_type, None);
    ...
    let final_value = builder.build_load(i32_type, counter_alloca, "final").unwrap();
    builder.build_return(Some(&final_value)).unwrap();
    ...
    for (level, duration, stats) in &results {
        println!(
...
#[test]
fn test_optimization_pass_filtering() {
    init_tracing!();
    ...
    let configs = vec![
        OptimizationConfig {
            ...
            custom_passes: vec![OptimizationPass::DeadCodeElimination],
    ...
    for config in configs {
        let mut manager = OptimizationManager::with_config(config);
        let result = manager.optimize_module(&module);
...
mod cli_integration_tests {
    ...
    #[test]
    fn test_cli_optimization_arg_parsing() {
        init_tracing!();
        
        // Test basic optimization level parsing
        let test_cases = vec![
            (vec!["-O2".to_string(), "test.csd".to_string()], Some("O2")),
            (vec!["-O0".to_string(), "test.csd".to_string()], Some("O0")),
            (vec!["-Os".to_string(), "test.csd".to_string()], Some("Os")),
            (vec!["--optimize".to_string(), "O3".to_string(), "test.csd".to_string()], Some("O3")),
            (vec!["other".to_string(), "test.csd".to_string()], None),
        ];
        ...
        for (args, expected_level) in test_cases {
            ...
            match expected_level {
                Some(level) => {
                    ...
                    assert_eq!(opt_args.level, level);
                }
                None => {
                    assert!(result.is_ok());
                    let opt_args = result.unwrap();
                    assert!(opt_args.is_none());
                }
            }
        }
    }
    ...
    #[test]
    fn test_cli_optimization_flags() {
        init_tracing!();
        ...
        assert!(opt_args.show_stats);
        assert!(opt_args.optimize_size);
        assert_eq!(opt_args.inline_threshold, Some(100));
        assert_eq!(opt_args.input_file.to_string_lossy(), "test.csd");
    }

    #[test]
    fn test_cli_pass_filtering() {
        init_tracing!();
        
        let args = vec![
            "-O2".to_string(),
            "--disable-pass".to_string(),
            "inline".to_string(),
            "--enable-pass".to_string(),
            "dce".to_string(),
            "test.csd".to_string(),
        ];
        ...
        let opt_args = opt_args.unwrap();
        assert_eq!(opt_args.disable_passes, vec!["inline"]);
        assert_eq!(opt_args.enable_passes, vec!["dce"]);
    }
    ...
    fn test_cli_invalid_arguments() {
        ...
        let args = vec!["-O2".to_string(), "--inline-threshold".to_string()];
        let result = parse_optimization_args(&args);
        assert!(result.is_err());
