...
#[test]
fn test_is_type_mismatch_error() {let err = create_test_error()
...
#[test]
fn test_extract_type_info() {let err = create_test_error()
    
    if let Some((source, target) = extract_type_info(&err)     {assert_eq!(source,  Reader);
        assert_eq!(target,  "Failed:  to extract type info)")}
...
#[test]
fn test_error_propagation_with_question_mark() {// Helper functions that use ? operator pattern
    fn returns_type_error() {Err(create_test_error()}
    
    fn calls_function_with_question_mark() {returns_type_error()?;
        Ok(()
    ...
    if let Err(err) = result     {assert!(is_type_mismatch_error(&err)
        if let Some((source, target) = extract_type_info(&err)     {;
            assert_eq!(source,  Reader;
            assert_eq!(target,  Writer;"Error:  info was lost during propagation)"}
// Test conversion between error types
#[test]
fn test_error_conversion() {let assertion_error = TypeAssertionError::new(StringerReader , " convert string to "reader)
    ...
    // Extract back
    if let Some((source, target) = extract_type_info(&error)     {assert_eq!(source,  Stringer);
