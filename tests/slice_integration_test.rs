    ...
    for source in test_sources {
        ...
        match value {
            BasicValueEnum::StructValue(sv) => {
                assert!(sv.get_type().count_fields() >= 2); // Should have data ptr and length
                info!("Successfully compiled slice: {} (struct with {} fields)", 
                      source, sv.get_type().count_fields());
            },
            _ => {
...
fn test_complete_pipeline_simulation() {
    ...
    assert!(module.verify().is_ok(), "Generated LLVM module is invalid");
...
fn test_slice_compilation_performance() {
    ...
    assert!(result.is_ok(), "Large slice compilation failed");
