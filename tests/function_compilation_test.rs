...
fn test_simple_function_declaration() {
    ...
    assert!(ir.contains("ret void"), "Should have return statement");
...
fn test_function_with_parameters() {
    ...
    let func = FunctionStatement {
        ...
        name: Identifier::new("add".to_string(), "add".to_string()),
        parameters: params,
    ...
    assert!(ir.contains("%x_addr = alloca i32"), "Should allocate parameter storage");
    assert!(ir.contains("%y_addr = alloca i32"), "Should allocate parameter storage");
    assert!(ir.contains("store i32 %x, i32* %x_addr"), "Should store parameters");
    assert!(ir.contains("ret i32 0"), "Should have default return");
}
...
fn test_function_with_string_parameters() {
    ...
    let func = FunctionStatement {
        token: "slay".to_string(),
        name: Identifier::new("greet".to_string(), "greet".to_string()),
        parameters: params,
        return_type: Some(return_type),
...
fn test_function_call_compilation() {
    ...
    assert!(ir.contains("@add"), "Should call correct function");
    assert!(ir.contains("%temp"), "Should use temporary variable");
}
...
fn test_void_return_statement() {
    ...
    // Verify void return
    assert!(ir.contains("ret void"), "Should generate void return");
}
...
fn test_function_arguments_generation() {
    ...
    assert_eq!(args, expected, "Should generate correct argument list");
...
fn test_local_variable_allocation() {
    ...
    let result = generator.allocate_local_variable("x", "int");
    assert!(result.is_ok(), "Local variable allocation should succeed");
    
    let ir = result.unwrap();
    tracing::info!("Generated local variable IR: {}", ir);
    
    assert!(ir.contains("%x_addr = alloca i32"), "Should allocate correct type");
    assert!(ir.contains("align 8"), "Should have alignment");
}
...
fn test_recursive_function_compilation() {
    ...
    let func = FunctionStatement {
        token: "slay".to_string(),
        name: Identifier::new("factorial".to_string(), "factorial".to_string()),
        parameters: params,
        return_type: Some(return_type),
    ...
    assert!(call_result.is_ok(), "Recursive call should compile");
...
fn test_multiple_function_compilation() {
    ...
    let functions = vec![
        ("main", vec![], None),
        ("helper", vec![("x", "int")], Some("int")),
        ("printer", vec![("msg", "string")], None),
    ];
    ...
    for (name, params, return_type) in functions {
        ...
        let func = FunctionStatement {
            token: "slay".to_string(),
            name: Identifier::new(name.to_string(), name.to_string()),
    ...
    assert!(generated_ir.contains("@main"), "Should contain main function");
    assert!(generated_ir.contains("@helper"), "Should contain helper function");
    assert!(generated_ir.contains("@printer"), "Should contain printer function");
    
    // Verify different return types
    assert!(generated_ir.contains("define void @main()"), "Main should be void");
...
fn test_function_compilation_memory_management() {
    ...
    let func = FunctionStatement {
        token: "slay".to_string(),
        name: Identifier::new("process_data".to_string(), "process_data".to_string()),
        parameters: params,
    ...
    assert!(ir.contains("%size_addr = alloca i32"), "Should allocate int storage");
    assert!(ir.contains("store i8* %input, i8** %input_addr"), "Should store string parameter");
    assert!(ir.contains("store i32 %size, i32* %size_addr"), "Should store int parameter");
    assert!(ir.contains("align 8"), "Should have proper alignment");
...
fn test_edge_cases_and_error_handling() {
    ...
    let invalid_func = FunctionStatement {
        token: "slay".to_string(),
        name: Identifier::new("test_invalid".to_string(), "test_invalid".to_string()),
...
fn test_complete_cursed_function_workflow() {
    ...
    assert!(call_result.is_ok(), "Function call should compile");
...
fn ir_contains_expected_patterns(ir: &str) -> bool {
    let patterns = vec![
        "; Function: calculate_vibe (slay keyword)",
        "define i1 @calculate_vibe(i8* %mood, i32 %energy)",
        "calculate_vibe_entry:",
        "%mood_addr = alloca i8*",
        "%energy_addr = alloca i32",
        "store i8* %mood, i8** %mood_addr",
        "store i32 %energy, i32* %energy_addr",
        "ret i1",
    ];
