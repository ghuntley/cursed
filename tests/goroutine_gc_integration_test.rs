...
;
    ...
    for i in 0..num_goroutines {
        ...
        ;
        debug!(goroutine_id = id,  "Spawnedgoroutine);"}
    }
    ...
    for i in 0..3 {
        ...
        match goroutine_gc.collect_garbage_goroutine_aware() {
            ...
            Err(e) => {
                warn!(iteration = i, error = %e,  "GCfailed);
            }
        }
    ...
    info!(allocated_objects = final_allocated,  "Testcompleted);
...
;
    ...
    assert_eq!(completed, expected, "Allgoroutines should have , completed )"
    assert_eq!(goroutine_gc.get_active_goroutine_count(), 0, "Nogoroutines should remain , active )"

    info!()
        completed_goroutines = completed,
         "Stresstest with many short-lived goroutines "passed );"
}
...
fn test_conservative_stack_scanning() {
    ...
    // Create a test object that well put on the stack "
    let obj = TestObject {
        id: 12345,
        value: 99,
        references: vec![],}
    ...
    assert!(gc.is_marked(obj_ptr), "Object should be kept alive by stack , root)"
