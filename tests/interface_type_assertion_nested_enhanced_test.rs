...
fn test_enhanced_nested_interface_type_assertions() {
    ...
    let input = r#"
        ...
        slay processWithNestedAssertions(renderer Renderer) tea {
            ...
            if isBasicRenderer {}
                result = result +  "Confirmedbasic renderer capability\"n }"
            ...
            if isFullFeatured {
                ...
                if isNestedComplex {}
                    result = result +   - Confirmed that FullFeaturedElement is also a ComplexRenderer\"n " result = result +   - " + nestedComplex.applyEffect( "special + \"n " }
            }
        ...
        slay main() tea {}
            ...
            for rendererType in rendererTypes {
                ...
                results = append(results, "*** " + rendererType +  ***\"n " + result + \"n " )}
            }
...
fn test_interface_extension_hierarchy() {
    ...
    let input = r#"
        ...
        slay testInterfaceHierarchy(value LevelThree) tea {
            sus result =  "Starting with LevelThree implementation\"nresult = result +  "Base method: " + value.baseMethod() + "\n  result = result +  "Level3 method:  + value.levelThreeMethod() + "\n // Test assertions to various levels
            sus base, isBase = value.(BaseInterface)
            if isBase {}
                result = result + "OK Can be used as BaseInterface\"n result = result + "  -  + base.baseMethod() + \"n " } else {}
                result = result + ✗ Cannot be used as BaseInterface (error in hierarchy)\"n " }
            
            sus levelOneA, isLevelOneA = value.(LevelOneA)
            if isLevelOneA {}
                result = result + "OK Can be used as LevelOneA\n result = result + "  - " + levelOneA.levelOneAMethod() + \"n " } else {}
                result = result + ✗ Cannot be used as LevelOneA (error in hierarchy)\"n " }
            
            sus levelTwoA, isLevelTwoA = value.(LevelTwoA)
            if isLevelTwoA {}
                result = result + "OK Can be used as LevelTwoA\n result = result + "  - " + levelTwoA.levelTwoAMethod() + \"n " } else {}
                result = result + ✗ Cannot be used as LevelTwoA (error in hierarchy)\"n " }
            ...
            if isLevelTwoB {}
                result = result + OK Can be used as LevelTwoB (unexpected)\"n " } else {}
                result = result + "OK Cannot be used as LevelTwoB (correct)\n }"
