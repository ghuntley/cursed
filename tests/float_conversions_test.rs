...
impl<'ctx> FloatConversionTester<'ctx> {
    fn new(context: &'ctx Context) -> Self {
        ...
        let generator = LlvmCodeGenerator::new(context, "float_conversion_test", std::path::PathBuf::from("test.cursed"));
...
fn test_float_to_integer_basic_conversions() {
    ...
    tester.generator.builder().position_at_end(basic_block);
    ...
    for (float_val, target_type, bit_width, is_signed) in test_cases {
        ...
        assert!(result.is_ok(), "Float to {} conversion failed: {:?}", target_type, result.err());
        ...
        if let Ok(converted) = result {
            ...
            assert_eq!(int_result.get_type().get_bit_width(), bit_width, 
                      "Result bit width should match target type");
        }
    }
...
fn test_float_to_float_conversions() {
    ...
    if let Ok(converted) = result {
        ...
        assert_eq!(float_result.get_type(), f64_type, "Result should be f64 type");
    }
    ...
    if let Ok(converted) = result {
        assert!(converted.is_float_value(), "Result should be float value");
        let float_result = converted.into_float_value();
        assert_eq!(float_result.get_type(), f32_type, "Result should be f32 type");
    }
...
fn test_float_to_bool_conversions() {
    ...
    let test_cases = vec![
        (0.0, "zero should be false"),
        (-0.0, "negative zero should be false"),
        (1.0, "positive number should be true"),
        (-1.0, "negative number should be true"),
        (0.000001, "small positive should be true"),
        (-0.000001, "small negative should be true"),
        (f64::INFINITY, "positive infinity should be true"),
        (f64::NEG_INFINITY, "negative infinity should be true"),
        (f64::NAN, "NaN should be false"),
    ];
...
fn test_integer_to_float_conversions() {
    ...
    for (int_val, int_bits, target_type, is_signed) in test_cases {
        ...
        assert!(result.is_ok(), "Integer to {} conversion failed: {:?}", 
                target_type, result.err());
        
        if let Ok(converted) = result {
            assert!(converted.is_float_value(), "Result should be float value");
            let float_result = converted.into_float_value();
            assert_eq!(float_result.get_type(), target_float_type, 
                      "Result should be {} type", target_type);
        }
    }
...
fn test_deterministic_behavior() {
    ...
    for iteration in 0..5 {
        debug!(iteration = iteration, "Testing deterministic conversion");
...
fn test_comprehensive_float_system() {
    ...
    for (source_type, target_type, test_val, should_succeed) in test_matrix {
        debug!(
            ...
            "Testing conversion matrix combination"
        );
