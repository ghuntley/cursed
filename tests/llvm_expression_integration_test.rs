...
fn test_arithmetic_expression_pipeline() {
    ...
    tracing::info!("Generated IR for arithmetic expression:\n{}", ir);
}
...
fn test_boolean_expression_with_slang() {
    ...
    // Create expression: based and not cap (true and not false)
    ...
    assert!(ir.contains("xor i1"));       // not operation
    assert!(ir.contains("and i1"));       // and operation
    
    tracing::info!("Generated IR for Gen Z boolean expression:\n{}", ir);
}
...
fn test_variable_assignment_and_access() {
    ...
    let ir = generator.get_expression_ir();
    assert!(ir.contains("store i64"));  // assignment
    assert!(ir.contains("load i64"));   // access
...
fn test_string_operations() {
    ...
    let complex_string = Literal::string("String with \"quotes\" and\nnewlines");
...
fn test_complex_nested_expressions() {
    ...
    assert!(ir.contains("sub i64"));    // c - d
    assert!(ir.contains("mul i64"));    // (a+b) * (c-d)
    assert!(ir.contains("sdiv i64"));   // e / f
    assert!(ir.contains("and i1"));     // logical &&
    assert!(ir.contains("icmp sgt"));   // comparison >
    ...
    tracing::info!("Generated IR for complex nested expression:\n{}", ir);
}
...
fn test_error_handling_integration() {
    ...
    let undefined_expr = BinaryExpression::new(
        "+".to_string(),
        Box::new(Identifier::from_name("undefined_var")),
        "+".to_string(),
        Box::new(Literal::integer(10))
    );
...
fn test_ir_quality() {
    ...
    assert!(ir.contains("add i64"));        // Final addition
...
fn test_memory_safety() {
    ...
    assert!(ir.contains("icmp eq"));         // Null comparison
