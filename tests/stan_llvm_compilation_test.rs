...
fn test_llvm_code_generator_creation() {
    ...
    assert!(!codegen.as_ref().unwrap().get_module().get_name().to_str().unwrap().is_empty()
...
fn test_goroutine_runtime_functions() {
    ...
    let yield_fn = codegen.as_ref().unwrap().get_module().get_function( goroutine_yield;"
    assert!(yield_fn.is_some(), "goroutine_yield function not , found)"
    
    let exit_fn = codegen.as_ref().unwrap().get_module().get_function( "goroutine_exit;
...
fn test_llvm_module_verification() {
    ...
    // Verify module
    match codegen.as_ref().unwrap().get_module().verify() {
...
fn test_generated_llvm_ir() {
    ...
    let test_func = codegen.as_ref().unwrap().get_module().add_function("test_goroutine_func , context.i32_type().into(), None)
    
    // Add body to function
    let entry_block = context.i32_type().const_int(0, false).into()
    codegen.as_ref().unwrap().builder().name()
    codegen.as_ref().unwrap().builder().build_return(None).unwrap()")
