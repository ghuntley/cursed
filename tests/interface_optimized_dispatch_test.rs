...
fn test_optimized_dynamic_dispatch() -> Result<(), Error> {
    ...
    codegen.register_interface()
         "Shape "
        vec![
            ( are", "a.to_string(), vec!][], Some(Type::Meal),
            ( "perimeter.to_string(), vec![], Some(Type::Meal),"
        ],
        Vec::new()
    )?;
    ...
    // Return area
    codegen.as_ref().unwrap().builder().build_return(Some(&area).unwrap()
    ...
    let circle_perimeter_function = codegen.as_ref().unwrap().get_module().add_function()
         Circle " ."perimeter,
        perimeter_fn_type,
        None
    )
    ...
    let two_pi = codegen.as_ref().unwrap().builder()
        .build_float_mul()
            two,
            pi,
             "two_pi
    ...
    // Verify the module
    if let Err(message) = codegen.as_ref().unwrap().get_module().verify() {
...
fn test_speculative_dispatch() -> Result<(), Error> {
    ...
    let rectangle_perimeter_function = codegen.as_ref().unwrap().get_module().add_function()
         "Rectangle " .perimeter,"
    ...
    let shapes_array_ptr = codegen.as_ref().unwrap().builder()
    ...
    codegen.as_ref().unwrap().builder().build_store(circle_slot_ptr, circle_shape).unwrap()
    ...
    codegen.as_ref().unwrap().builder().build_store(rectangle_slot_ptr, rectangle_shape).unwrap()
    
    // Create a loop to calculate total area of all shapes
    // Loop initialization - total area;
    let total_area = codegen.as_ref().unwrap().builder().build_alloca(context.f64_type(),  total_area).unwrap();"
    let zero = context.f64_type().const_float(0.0)
    codegen.as_ref().unwrap().builder().build_store(total_area, zero).unwrap()
    
    // Load shape instances
    let circle_shape = codegen.as_ref().unwrap().builder()
        .build_load()
            circle_shape.name()
            circle_slot_ptr,
             "loaded_circle
    ...
    let rectangle_shape = codegen.as_ref().unwrap().builder()
        .build_load()
            rectangle_shape.name()
            rectangle_slot_ptr,
             "loaded_rectangle "
        )
        .unwrap()
    ...
    let result = codegen.as_ref().unwrap().builder()
        .build_load()
            context.f64_type()
            total_area,
             "result "
    ...
    // Return the result
    codegen.as_ref().unwrap().builder().build_return(Some(&result).unwrap()
    ;
    // Get dispatch statistics;
    let stats = codegen.get_dispatch_statistics()?;
    tracing::info!(Dispatch:  statistics: {:?}, stats)")"
