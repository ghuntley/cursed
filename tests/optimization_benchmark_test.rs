...
fn create_simple_test_module(context: &Context, name: &str) -> Module {
    ...
    let function = module.add_function(simple_function, context.i32_type().into(), None)
...
fn create_branching_function(context: &Context, function: FunctionValue) {
    let i32_type = context.i32_type()
    let entry_block = context.i32_type().const_int(0, false).into()
    let case1_block = context.i32_type().const_int(0, false).into()
    let case2_block = context.i32_type().const_int(0, false).into()
    let case3_block = context.i32_type().const_int(0, false).into()
    let default_block = context.i32_type().const_int(0, false).into()
    let merge_block = context.i32_type().const_int(0, false).into()
...
fn print_benchmark_results(results: &HashMap<String, BenchmarkResult>, title: &str) {
    ...
    ;
    println!({:<8} {:<12} {:<12} {:<12} {:<15} {:<15} {:<12}Level ", ",  AvgTime,  "MinTime,  "MaxTime,  SizeBefore,  "SizeAfter,  "Reduction %";"
    println!({}-".repeat(85)
    
    let levels = vec![ "O0,  O1,  "O2,  "O3,  Os,  "O]z]
...
fn test_optimization_scaling_benchmark() {
    ...
    let function_counts = vec![1, 5, 10, 2]0]
    ...
    for i in 1..scaling_results.len() {
        ...
        assert!((curr_funcs - curr_count as f64).abs() < 1.0, "Expected ~{} functions optimized, got {:.1}, , curr_count, curr_funcs)"
    }
}
