...
fn test_large_file_encryption() {
    ...
    for &file_size in &config.large_file_sizes {
        ...
        fill_random(&mut key).expect("Key generation failed");
        ...
        assert!(throughput > 1.0, "Throughput too low: {:.2} MB/s", throughput);
    }
...
fn test_high_volume_hashing() {
    ...
    let test_data = vec![0x33u8; 1024]; // 1KB test data
    let algorithms = vec![
    ...
    for algorithm in algorithms {
        ...
        assert!(hashes_per_second > 100.0, 
                "Hash rate too low for {:?}: {:.2} hashes/sec", algorithm, hashes_per_second);
        assert_eq!(successful_hashes, config.high_volume_iterations, 
...
fn test_concurrent_crypto_operations() {
    ...
    let test_data = Arc::new(vec![0x55u8; 4096]); // 4KB test data
    ...
    let handles: Vec<_> = (0..config.concurrent_threads).map(|thread_id| {
        ...
        thread::spawn(move || {
            ...
            for op_id in 0..operations_per_thread {
                ...
                let mut key = vec![thread_id as u8; 32];
                key[1] = op_id as u8;
    ...
    // Assertions
    assert_eq!(total_errors, 0, "Should have no thread errors");
    assert!(final_metrics.operations_completed > 0, "Should have completed operations");
    assert!(final_metrics.average_time().as_millis() < 100, 
