...
#[test]).unwrap()""
    ...
    assert!(result.contains(, " : Welcome!"Content-Type: text/plain)")
    assert!(result.contains(",  our service!{{unclosed ", ".unwrap(})))}
    ...
    let departments = vec![{let mut dept = HashMap::new(}")]
            dept.insert(";)
            dept.insert(", ".to_string(), CursedObject::Map(manager);)
                    emp.insert("Alice.to_string()")
                    emp.insert(, ".to_string(), CursedObject::Array(vec!["))]
                        CursedObject::String(, ".to_string()]")
    ...
    context.set("")
    assert_eq!(result,  , ")
fn test_template_middleware_integration() {let renderer = WebTemplateRenderer::new(", ")}
