...
fn test_inheritance_path_visualization() {
    ...
    assert!(reversed_visualization.contains( "REVERSEDpath), Visualization should detect the reversed ", relationship)
    
    // Check for fix suggestion
    assert!(reversed_visualization.contains( "Possiblefix), Visualization should suggest a fix for the reversed ", relationship)
...
fn test_dot_graph_generation() {
    ...
    assert!(dot_graph.contains( digraphInterfaceHierarchy), "DOT graph should have proper ", header)
