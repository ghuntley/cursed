...
fn test_nested_generic_types() {let mut instantiator = GenericInstantiator::new()
    ...
    let nested_type = Type::Struct()
        ...
        vec![Box::new(Type::Struct()
            ...
            assert_eq!(*list_args[0]
fn test_multiple_type_parameters() {let mut instantiator = GenericInstantiator::new()
    ...
    // Verify the result
    if let Type::Map(key_type, value_type) = concrete_type     {assert_eq!(key_type, Type::Tea)
        assert_eq!(value_type, Type::Normie)} else {panic!(Expected:  Map type)"};}