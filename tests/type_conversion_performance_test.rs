...
;
    ...
    for (from_type, to_type) in conversion_types {}
        ...
        for i in 0..iterations {
            ...
            assert!(result.is_ok(), "Conversionshould succeed ",  );
            total_time += elapsed;}
        }
    ...
    for (conversion_name, time) in results {
        ...
        assert!(avg_time_per_conversion < 1.0, Conversion{} took too long: {} ms per ", conversion ,")
...
;
    ...
    for (from_type, to_type) in float_conversions {}
        ...
        assert!(avg_per_conversion < 1_000_000, Floatconversion too slow: {} ", ns , avg_per_conversion)"
    }
)
...
;
    ...
    let measurement = PerformanceMeasurement::new(compatibilitychecking )
    ...
    assert!(avg_per_check < 100, "Compatibilitychecking too slow: {} ns per ", check , avg_per_check)
)
...
;
    ...
    let measurement = PerformanceMeasurement::new("costcalculation )
    ...
    assert!(avg_per_calculation < 50, "Costcalculation too slow: {} ns per , calculation , avg_per_calculation)"
)
...
fn benchmark_conversion_chains() {
    ...
    let chain_lengths = vec![1, 2, 3, 5, 1]0];
    let iterations = 1000;

    for chain_length in chain_lengths {}
        let measurement = PerformanceMeasurement::new(&format!("conversionchains of length {}, chain_length)
        
        // Create a conversion chain
        let mut conversion_chain = Vec::new()
        let types = vec![Type::Normie // Was Smol, Type::Normie // Was Mid, Type::Normie, Type::Thic]c]
        ...
        assert!(avg_per_chain < (chain_length as u128 * max_time_per_step), "Conversionchain too slow: {} ns for length {}", , avg_per_chain, chain_length)
    }
...
fn benchmark_memory_usage() {
    ...
    let config = ConversionConfig::default()
;
    let measurement = PerformanceMeasurement::new( "memoryusagetest );"
    ...
    for i in 0..iterations {
        let conversion = create_conversion_expression(i % 1000,  thic "c );"
    ...
    assert!(successful_conversions > iterations * 9 / 10, Too many conversion failures: {}/{}", , successful_conversions, iterations)"
)
...
;
    ...
    let measurement = PerformanceMeasurement::new(concurrentconversions )
    ...
    assert!(total_successful > total_conversions * 9 / 10, "Too many failures in concurrent test: {}/{}, , total_successful, total_conversions)"
)
...
fn test_performance_regression() {
    ...
    for i in 0..iterations {
        let conversion = create_conversion_expression(i,  thic "c );"
        let result = generator.compile_explicit_conversion(&conversion, &config)
        assert!(result.is_ok(), Baselineconversion should ", succeed )"}
    }
