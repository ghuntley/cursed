...
fn test_monomorphization_function_specialization() {
    ...
    let generic_func = create_generic_function("max", vec!["T"], vec![Type::TypeParam("T".to_string()), Type::TypeParam("T".to_string())], Type::TypeParam("T".to_string()));
    ...
    assert_eq!(specialized_name, specialized_name2, "Should reuse existing specialization");
...
fn create_generic_function(name: &str, type_params: Vec<&str>, param_types: Vec<Type>, return_type: Type) -> FunctionStatement {
    // Create type parameters
    let type_parameters: Vec<Identifier> = type_params.iter()
    ...
    let parameters: Vec<ParameterStatement> = param_types.iter().enumerate()
        .map(|(i, param_type)| {
...
fn test_monomorphization_manager_in_llvm_generator() {
    ...
    let generic_fn = create_generic_function("map", vec!["T", "U"], 
                                          vec![Type::TypeParam("T".to_string())], 
                                          Type::TypeParam("U".to_string()));
    
    // Specialize with concrete types
    let spec_name = mono_manager.specialize_function(
        &mut code_gen,
        &generic_fn,
        &[Type::Thicc, Type::Tea],
