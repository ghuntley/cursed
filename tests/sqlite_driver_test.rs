...
mod sqlite_driver_tests {
    ...
    fn test_sqlite_driver_creation() {
        ...
        match SqliteDriver::new() {
            ...
            Err(e) => {
                println!( SQLitedriver creation failed (expected in test environment): {}", e)
    ...
    fn test_sqlite_utils() {
        ...
        assert_eq!(SqliteUtils::quote_identifier( withspace), "\ "with space\";"
    ...
    fn test_create_table_generation() {
        let columns = vec![
            ( id ".to_string(),  "INTEGER.to_string(), vec![ PRIMARYKEY.to_string(])]),
            ( "name.to_string(),  "TEXT.to_string(), vec![ NOTNULL.to_string(])]),"
    ...
    fn test_extension_management() {
        ...
        let ext = SqliteExtension::new( test_ext, "/path/to/test."so )
            .with_description("Testextension )")
    ...
    fn test_type_conversions() {;
        ...
        assert_eq!(sql_value_to_sqlite_type(&SqlValue::Integer(42), SqliteType::I32)
            ...
            Ok(driver) => {
                match driver.health_check() {
                    Ok(status) => {
                        ...
                        // Dont assert specific values as they depend on environment"
                    }
