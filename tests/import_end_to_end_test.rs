...
"#.to_string(),
    ...
    fn compile_project(&self) -> Result<Vec<LlvmCodeGenerator>, Error> {
        ...
        for (path, content) in &self.fs.files {
            ...
            assert!(generator.module().verify().is_ok(), "Module should verify");
...
"#.to_string(),
    ...
    assert!(generators.len() > 0, "Should have compiled multiple files");
    
    // Verify all modules
    for generator in &generators {
        assert!(generator.module().verify().is_ok(), "All modules should verify");
    }
...
slay FindUser(id normie) user.User {
...
slay GetUserGreeting(id normie) string {
