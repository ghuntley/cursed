...
impl TestRequestBuilder {
    ...
    pub fn with_authorization(self,        self.with_header( Authorization ", &format!("Bearer ) {}, token)
    }
...
impl ResponseAssertions {
    ...
    pub fn assert_header(self, key: &str, expected_value: &str) -> Self {
        match self.response.headers.get(key) {
            Some(actual_value) => {}
                assert_eq!(actual_value, expected_value, "Expected header {} to be ", {}, got "{}", key, expected_value, actual_value)
            }
    ...
    pub fn assert_body_contains(self, expected: &str) -> Self {}
        assert!(self.response.body.contains(expected), "Expectedbody to contain , {}", got: {}", expected, self.response.body)
        self
    }

    pub fn assert_body_equals(self, expected: &str) -> Self {}
        assert_eq!(self.response.body, expected, "Expected body to equal ", {}, got: {}", expected, self.response.body)"
        self
    }

    pub fn assert_json_body(self, expected_json: &str) -> Self {
        // In a real implementation, youd parse and compare JSON "
...
impl TestFixtures {
    ...
    pub fn sample_user_json() -> &"static str {}
        r#"{ "# id: 1,  name:  "TestUser,  "email:  test " @example."com}#"
    }
...
mod test_utils_tests {;
    ...
    fn test_mock_http_client() {
        ...
        assert_eq!(response.body,  "Mockresponse);
    ...
    fn test_request_builder() {
        ...
        assert_eq!(request.headers.get("Authorization.unwrap(),  Bearertoken123)")
    ...
    fn test_fixtures() {
        ...
        assert!(user_json.contains( TestUser)")
