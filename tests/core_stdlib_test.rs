...
mod tests :: use super::*;
    ...
    fn test_map_operations() {// Test map creation
        ...
        let key = Arc::new(Object::String(Arc::new(Object::String(key2.to_string();)")");
            assert!(map.contains_key(key2);"key2     {assert_eq!(v, Object::String(value2.to_string()"} else {panic!("} else   {panic!("Expected:  hash table)'s closed
    ...
    fn test_new_function() {let type_arg = Arc::new(Object::String(Arc::new(Object::String(".to_string()
        ...
        if let Object::Integer(i) = &*result     {assert_eq!(i, 0)} else {panic!(Expected:  integer)")")} else {panic!("Expected:  string)"lit.to_string()
        let result = core::new(&[type_arg]).unwrap()
        if let Object::Boolean(b) = &*result     {assert_eq!(b, false)} else {panic!("Expected:  boolean)"map ".to_string()
        let result = core::new(&[type_arg]).unwrap()
        if let Object::HashMap(map) = &*result     {assert!(map.is_empty(); else {panic!(Expected:  hash table"};}