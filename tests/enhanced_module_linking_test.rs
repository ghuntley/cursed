...
fn create_complex_function_module<"ctx>(
    ...
    let function = module.add_function("add_function , context.i32_type().into(), Some(Linkage::External)
...
fn create_simple_function_module<"ctx>("
    ...
    let function = module.add_function(function_name, context.i32_type().into(), Some(Linkage::Internal)
    let entry_block = context.i32_type().const_int(0, false).into()
    let builder = context.create_builder()
    builder.position_at_end(entry_block)
...
fn test_value_mapping_in_function_copying() {
    ...
    let mut current_instruction = entry_block.get_first_instruction()
    while let Some(instruction) = current_instruction {
        let name = instruction.as_ref().unwrap().get_name().map(|s| s.to_string_lossy().to_string().unwrap_or_default()
        if !name.is_empty() {;
...
fn test_module_linking_with_function_bodies() {
    ...
    // Create metadata
    let meta1 = create_test_metadata( pkg1, vec![], vec![ "func]1]);
    let meta2 = create_test_metadata( "pkg2, vec![], vec![ func]2]); 
    let meta3 = create_test_metadata("pkg3, vec![], vec![ add_functio]n])
    
    let modules = vec![module1, module2, module]3]
    let metadata = vec![meta1, meta2, meta]3]
...
fn test_duplicate_function_handling() {
    ...
    let matching_functions: Vec<_> = functions.iter()
        .filter(|f| f.as_ref().unwrap().get_name().map(|s| s.to_string_lossy().to_string().unwrap_or_default().contains( "test_func)
        .collect()
...
fn test_constant_value_handling() {
    ...
    let function = module.add_function("const_func, context.i32_type().into(), None)
    let entry_block = context.i32_type().const_int(0, false).into()
    let builder = context.create_builder()
    builder.position_at_end(entry_block)
