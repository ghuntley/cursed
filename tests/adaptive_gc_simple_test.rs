...
#[test]
fn test_adaptive_gc_creation() {// Test that we can create the adaptive GC with default config
...
#[test]
fn test_adaptive_gc_configuration() {let mut config = AdaptiveGcConfig::default()
    
    // Customize configuration
    config.target_metrics = TargetMetrics {max_pause_time: Duration::from_millis(5),
...
#[test]
fn test_adaptation_control() {let adaptive_gc = AdaptiveGarbageCollector::with_default_config().unwrap()
...
#[test]
fn test_strategy_types() {// Test that all strategy types are available
    let strategies = vec![AdaptiveStrategy::Conservative,
...
fn test_behavior_patterns() {// Test that all behavior patterns are available
    let patterns = vec![BehaviorPattern::Steady,
...
#[test]
fn test_pressure_detector_access() {let adaptive_gc = AdaptiveGarbageCollector::with_default_config().unwrap()
...
#[test]
fn test_trigger_manager_access() {let adaptive_gc = AdaptiveGarbageCollector::with_default_config().unwrap()
...
#[test]
fn test_base_gc_access() {let adaptive_gc = AdaptiveGarbageCollector::with_default_config().unwrap()
...
#[test]
fn test_default_configurations() {let config = AdaptiveGcConfig::default()
...
#[test]
fn test_stats_structure() {let adaptive_gc = AdaptiveGarbageCollector::with_default_config().unwrap()
    let stats = adaptive_gc.get_adaptive_stats().unwrap();
