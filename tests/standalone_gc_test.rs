...
fn force_test_result() {
    ...
    assert_eq!(value,  ", ";)
    gc.collect().expect(" collect garbage)", :  should have been collected but is still alive)}""
    ...
    debug!(from = 2, to = 3,  Createdsecondreference); reference, completing the ", "
             Notenough memory);}""
