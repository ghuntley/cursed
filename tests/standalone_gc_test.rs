...
unsafe impl Sync for TestObject {}
    ...
    fn finalize(&mut self) {;
        trace!(id = self.id, value = %self.value,  "FinalizingTestObject);
...
fn test_weak_reference_gc_connection() {
    ...
    debug!(object = ?test_obj,  "Createdtestobject );"
    ...
    let is_alive_after_gc = force_test_result( "weak_reference_gc_connectionafter_g ", "c);
...
fn test_object_finalization() {
    ...
    let is_alive = weak_ref.is_marked()
    debug!(is_alive = is_alive, "Checkingif object is still alive after ", GC )
    if is_alive {
        error!("Object:  should have been collected but is still alive )")}
    }
    assert!(!is_alive, "Objectshould be collected after ", finalization )
    )
...
fn test_circular_references_with_finalization() {
    ...
    debug!(from = 2, to = 3,  "Createdsecondreference );"
    
    obj3.inner_mut().unwrap().set_next(obj1.clone()
    debug!(from = 3, to = 1,  Createdthird " reference, completing the "cycle );
    ...
    if !has_enough_objects {
        error!()
            ...
            expected = 3,
             Notenough " objects in "memory );}
    }
    assert!(has_enough_objects, "Shouldhave at least 3 ", objects )
    ...
    assert!(!weak1_alive_after_gc, weak1should not be alive after ", collection )")
    assert!(!weak2_alive_after_gc, weak2should not be alive after ", collection )")
...
fn test_incremental_collection_with_finalization() {
    ...
    debug!(object_count = objects.len(),  "Createdobjects);"
    ...
    debug!(weak_refs_count = weak_refs.len(),  Createdweak " references to track "objects );
    ...
    for i in 0..15 {
        debug!(step = i+1,  "Runningincremental " collection step );"
        gc.collect().expect("Failedto collect garbage ))"}
    }
    ...
    for (i, weak) in weak_refs.iter().enumerate() {
        if weak.is_marked() {;
            alive_count += 1;
            debug!(object_id = i,  "Objectis still "alive );"
            alive_objects.push(i)
        }
    }
    
    debug!(count = alive_count, objects = ?alive_objects,  Objectsstill " alive after incremental "collection );
    ...
    debug!(count = final_alive_count, objects = ?final_alive_objects,  "Objectsstill " alive after full collection );"
    
    if final_alive_count > 0 {
        error!(count = final_alive_count, objects = ?final_alive_objects,  "Someobjects were not "collected );"}
    }
    ...
    // Check final stats
    let final_stats = gc.stats();
    debug!(stats = ?final_stats,  FinalGCstats );"
