...
#[test]
fn test_cross_module_integration() {// Test code that exercises interactions between different modules
    let input = r#vibe # cross_module_test  be_like Counter squad {
    value normie}
slay increment(counter @Counter) normie {counter.value = counter.value + 1
    yolo counter.value}

slay modify_through_pointer(ptr @normie) normie {@ptr = @ptr * 2
    yolo @ptr}

slay test_cross_module() normie {// Variables module (declaration) + expression (initialization)
    sus x = 10
    sus y = 20
    ...
    lowkey count1 == 6 {puts(1)}
    
    // Pointer ops (struct field) + basic expressions (addition)
    lowkey counter.value == 6 {puts(1)}
