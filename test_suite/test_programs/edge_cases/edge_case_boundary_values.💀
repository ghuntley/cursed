...
fr fr Test maximum/minimum comparisons  
    ...
    fr fr Manual max/min since mathz.max/min don't exist
    sus max_val normie = large_num  fr fr large_num is obviously larger
    sus min_val normie = small_num  fr fr small_num is obviously smaller
    
    vibez.spill("Max of", large_num, "and", small_num, ":", max_val)
    vibez.spill("Min of", large_num, "and", small_num, ":", min_val)
    
fr fr Test boolean edge cases
    sus always_true lit = based
    sus always_false lit = cringe
...
fr fr Test single character operations
    ...
    sus result normie = mathz.abs_normie(single_digit)
