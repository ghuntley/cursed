...
$CURSED_BIN run basic_test.csd > basic_output.txt
...
$CURSED_BIN compile basic_test.csd -o basic_compiled
if [ $? -eq 0 ]; then
...
$CURSED_BIN run comprehensive_test.csd > comprehensive_output.txt
...
EOF
        $CURSED_BIN run stdlib_test_${module}.csd > /dev/null
        if [ $? -eq 0 ]; then
...
$CURSED_BIN run self_hosting_test.csd > self_hosting_output.txt
...
$CURSED_BIN compile self_hosting_test.csd -o self_hosting_compiled
if [ $? -eq 0 ]; then
...
if [ -f "src/bootstrap/stage2/main.csd" ]; then
    ...
    $CURSED_BIN compile src/bootstrap/stage2/main.csd -o cursed_stage2 2>/dev/null
    if [ $? -eq 0 ]; then
...
$CURSED_BIN run production_test.csd > production_output.txt
...
$CURSED_BIN compile production_test.csd -o production_compiled
if [ $? -eq 0 ]; then
