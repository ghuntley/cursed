...
pub struct LoopOptimizationPass<'ctx> {
    ...
    loop_analysis: LoopAnalysis<'ctx>,
}
...
impl<'ctx> LoopOptimizationPass<'ctx> {
    ...
    fn is_value_loop_invariant(&self, value: &BasicValueEnum<'ctx>, loop_info: &LoopInfo) -> bool {
        // Check if this is an instruction value
        if let Some(inst) = value.as_instruction_value() {
            ...
            // Constants, globals, and function arguments are loop invariant
            true
        }
    }
...
pub struct LoopAnalysis<'ctx> {
    analyzed_functions: HashMap<String, Vec<LoopInfo<'ctx>>>,
}

impl<'ctx> LoopAnalysis<'ctx> {
    pub fn new() -> Self {
        Self {
            ...
            if let Some(terminator) = block.get_terminator() {
                match terminator.get_opcode() {
                    inkwell::values::InstructionOpcode::Br => {
                        // Branch instruction (both conditional and unconditional)
                        // For now, skip complex branch analysis due to API limitations
                        // TODO: Implement proper successor analysis when inkwell API is stabilized
                    }
                    _ => {}
                }
            }
    ...
    fn dfs_back_edges<'a>(
        &self,
        block: BasicBlock<'a>,
        cfg: &HashMap<BasicBlock<'a>, Vec<BasicBlock<'a>>>,
        visited: &mut HashSet<BasicBlock<'a>>,
        visiting: &mut HashSet<BasicBlock<'a>>,
        back_edges: &mut Vec<(BasicBlock<'a>, BasicBlock<'a>)>,
    ) {
    ...
    fn analyze_loop<'a>(&self, function: &FunctionValue, header: BasicBlock<'a>, latch: BasicBlock<'a>) -> Result<LoopInfo<'a>> {
        ...
        // Analyze loop body
        let blocks_copy = loop_info.blocks.clone();
    ...
    fn find_loop_blocks<'a>(&self, header: BasicBlock<'a>, latch: BasicBlock<'a>) -> Vec<BasicBlock<'a>> {
    ...
    fn analyze_instruction_for_vectorization<'a>(&self, instruction: &InstructionValue, loop_info: &mut LoopInfo<'a>) {
        match instruction.get_opcode() {
            ...
            inkwell::values::InstructionOpcode::Mul => {
                ...
                if loop_info.primary_data_type.is_none() {
                    // Skip type analysis for now due to API incompatibilities
                    // loop_info.primary_data_type = Some(instruction.get_type());
                }
            }
    ...
    fn analyze_trip_count(&self, header: BasicBlock, latch: BasicBlock) -> Option<u32> {
        ...
        // TODO: Re-implement phi node analysis when inkwell API is stabilized
...
pub struct LoopInfo<'ctx> {
    pub header: Option<BasicBlock<'ctx>>,
    pub latch: Option<BasicBlock<'ctx>>,
    ...
    pub primary_data_type: Option<BasicTypeEnum<'ctx>>,
}
...
impl Default for LoopOptimizationResult {
    fn default() -> Self {
        Self {
            loops_unrolled: 0,
            loops_vectorized: 0,
