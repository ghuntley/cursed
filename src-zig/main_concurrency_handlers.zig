...
pub fn handleStanStatement(variables: *anyopaque, functions: *anyopaque, allocator: Allocator, source_lines: ArrayList([]const u8), line_index: usize, verbose: bool) !void {
    ...
    if (std.mem.indexOf(u8, trimmed, "{")) |_| {
        ...
        while (current_line < source_lines.items.len and brace_count > 0) {
            ...
            if (brace_count > 0) {
                try body_lines.append(self.allocator, block_line);
            }
            current_line += 1;
        }
        ...
        if (global_goroutines) |*goroutines| {
            try goroutines.append(self.allocator, thread);
        }
...
pub fn handleWaitFunction(variables: *anyopaque, allocator: Allocator, line: []const u8, verbose: bool) !void {
    ...
    if (std.mem.startsWith(u8, line, "wait_all()")) {
        ...
        global_concurrency_mutex.lock();
        var threads_to_join = std.ArrayList(u8){};
        defer threads_to_join.deinit();
        
        if (global_goroutines) |*goroutines| {
            // Copy thread handles to avoid holding lock during join
            for (goroutines.items) |thread| {
                try threads_to_join.append(self.allocator, thread);
            }
        ...
        if (std.mem.indexOf(u8, line, "(")) |start| {
            if (std.mem.indexOf(u8, line, ")")) |end| {
                ...
                if (verbose) print("⏳ Waiting for {s}ms (race-safe)...\n", .{bounded_wait});
                std.Thread.sleep(@as(u64, bounded_wait) * 1_000_000); // Convert ms to ns
...
pub fn handleMakeChannel(variables: *anyopaque, allocator: Allocator, var_name: []const u8, verbose: bool) !void {
    ...
    const Variable = struct { value: i64 };
    ...
    if (@intFromPtr(variables) != 0) {
        const variables_map = @as(*struct { value: i64 }Store, @ptrCast(@alignCast(variables)));
        const name_copy = try allocator.dupe(u8, var_name);
        try variables_map.put(name_copy, Variable{ .Integer = @as(i64, @intCast(channel_id)) });
    }
    
    if (verbose) print("✅ Channel created with ID: {s} (runtime integrated)\n", .{channel_id});
}

// Handle send_channel function
pub fn handleSendChannel(variables: *struct { value: i64 }Store, allocator: Allocator, channel_var: []const u8, value_var: []const u8, verbose: bool) !void {
        _ = allocator;
    const concurrency_runtime = @import("concurrency_runtime.zig");
    const Variable = struct { value: i64 };
    ...
    concurrency_runtime.sendToChannel(channel_id, value) catch {
        if (verbose) print("❌ Failed to send to channel {s}\n", .{channel_id});
        return;
    };
    
    if (verbose) print("✅ Sent value to channel {s} successfully\n", .{channel_id});
}

// Handle recv_channel function
pub fn handleRecvChannel(variables: *struct { value: i64 }Store, allocator: Allocator, result_var: []const u8, channel_var: []const u8, verbose: bool) !void {
        _ = allocator;
    const concurrency_runtime = @import("concurrency_runtime.zig");
    ...
    const received = concurrency_runtime.receiveFromChannel(channel_id) catch {
        if (verbose) print("❌ Failed to receive from channel {s}\n", .{channel_id});
        return;
    };
    ...
    if (received) |value| {
        ...
        if (verbose) print("✅ Received value from channel {s} and stored in {s}\n", .{ channel_id, result_var });
    } else {
        if (verbose) print("⚠️ Channel {s} closed or no data available\n", .{channel_id});
    }
}
