...
/// Enhanced error reporting and debugging system for CURSED Zig compiler
/// Provides excellent developer experience with clear, helpful error messages
...
pub const DiagnosticMessage = struct {
    ...
    pub fn setSourceSnippet(self: *DiagnosticMessage, snippet: []const u8) !void {
        self.source_snippet = try self.allocator.dupe(u8, snippet);
    }
...
pub const ErrorReporter = struct {
    ...
    pub fn init(allocator: Allocator, max_errors: usize) ErrorReporter {
        return ErrorReporter{
            .diagnostics = ArrayList(DiagnosticMessage).init(allocator),
            .allocator = allocator,
    ...
    fn reportDiagnostic(
        ...
        var diagnostic = try DiagnosticMessage.init(self.allocator, severity, code, message, location);
...
pub const DebugInfo = struct {
    ...
    pub fn init(allocator: Allocator, level: DebugLevel) DebugInfo {
        return DebugInfo{
            .level = level,
            .line_table = ArrayList(LineInfo).init(allocator),
            .scope_table = ArrayList(ScopeInfo).init(allocator),
            .variable_table = ArrayList(VariableInfo).init(allocator),
            .allocator = allocator,
        };
    }
...
/// Logger for compiler debugging and verbose output
...
// Testing
test "enhanced error reporting system" {
