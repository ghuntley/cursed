...
pub const Variable = union(enum) {
    ...
    pub fn clone(self: Variable, allocator: Allocator) !Variable {
        ...
        switch (self) {
            ...
            .Array => |arr| {
                var new_arr = std.ArrayList(Variable){};
...
fn copyType(allocator: Allocator, original: ast.Type) !ast.Type {
    switch (original) {
        ...
        .Union => |union_type| {
            var new_types = std.ArrayList(u8){};
...
fn interpretProgramWithVariables(allocator: Allocator, source: []const u8, verbose: bool, stdlib_path: ?[]const u8) !void {
        ...
        for (imports.items) |module_name| {
            if (simple_loader.loadModule(module_name) catch |err| {
                ...
                for (module_functions) |func| {
                    ...
                    try func_def.parameters.append(allocator, param_copy);
                    }
    ...
    while (line_index < source_lines.items.len) {
        ...
        if (std.mem.startsWith(u8, trimmed, "stan ") and std.mem.indexOf(u8, trimmed, "{") != null) {
            ...
            if (std.mem.indexOf(u8, trimmed, "}") == null) {
                ...
                try block_lines.append(allocator, trimmed); // Add the opening line
                line_index += 1;
                ...
                while (line_index < source_lines.items.len and brace_depth > 0) {
                    ...
                    try block_lines.append(allocator, block_line);
                ...
                var full_statement = std.ArrayList(u8){};
                defer full_statement.deinit();
                
                for (block_lines.items, 0..) |block_line, i| {
                    if (i > 0) try full_statement.append(allocator, '\n');
        ...
        if (std.mem.startsWith(u8, trimmed, "slay ")) {
            ...
            if (std.mem.indexOf(u8, trimmed, ";")) |semicolon_pos| {
                ...
                var temp_source_lines = std.ArrayList(u8){};
    ...
    if (concurrency.getScheduler()) |scheduler| {
        ...
        if (verbose) {
            ...
            if (remaining > 0) {
                print("⚠️  {s} goroutines still active after wait\n", .{remaining});
            } else {
...
fn processStatements(variables: *VariableStore, functions: *FunctionStore, structs: *StructStore, allocator: Allocator, variable_allocator: Allocator, line: []const u8, verbose: bool) !void {
    ...
    if (final_statement.len > 0) {
        try statements.append(allocator, final_statement);
    }
    ...
    if (std.mem.startsWith(u8, trimmed, "!") and trimmed.len > 1) {
        ...
        if (operand_str.len > 0) {
            ...
            const result = switch (operand) {
                ...
                .String => |str_val| Variable{ .Boolean = str_val.len == 0 },
...
fn evaluateSingleValue(variables: *VariableStore, functions: *FunctionStore, allocator: Allocator, value_str: []const u8, verbose: bool) !Variable {
    ...
    if (std.fmt.parseInt(i64, value_str, 10)) |int_val| {
        if (verbose) print("📊 Parsed as integer: {s}\n", .{int_val});
    ...
    if (value_str.len >= 2 and value_str[0] == '[' and value_str[value_str.len - 1] == ']') {
        ...
        var iterator = std.mem.split(u8, elements_str, ",");
        ...
        while (iterator.next()) |element| {
            ...
            if (trimmed_element.len > 0) {
                ...
                try array.append(allocator, evaluated_element);
                if (verbose) print("📊 Added array element: {any}\n", .{evaluated_element});
            }
        }
        
        if (verbose) print("📊 Created array with {s} elements\n", .{array.items.len});
...
fn handleVariableDeclaration(variables: *VariableStore, functions: *FunctionStore, structs: *StructStore, allocator: Allocator, variable_allocator: Allocator, line: []const u8, verbose: bool) !void {
    ...
    const variable_value = if (std.mem.eql(u8, var_type, "auto")) blk: {
        ...
        if (evaluateExpression(variables, functions, allocator, value_str, verbose)) |result| {
            ...
            if (verbose) print("❌ AUTO TYPE: Failed to evaluate expression '{s}': {s}\n", .{value_str, err});
...
fn handleStanGoroutine(variables: *VariableStore, functions: *FunctionStore, allocator: Allocator, line: []const u8, verbose: bool) !void {
    ...
    if (concurrency.getScheduler() == null) {
        ...
        if (verbose) print("✅ Initialized concurrency scheduler with {s} workers\n", .{config.num_workers});
    }
    ...
    if (std.mem.startsWith(u8, stan_content, "{") and std.mem.endsWith(u8, stan_content, "}")) {
        ...
        const goroutineBlockFn = struct {
            fn run(ctx: ?*anyopaque) void {
                ...
                while (lines.next()) |stmt_line| {
                    ...
                    defer temp_structs.deinit(ctx.allocator);
        ...
        if (verbose) print("✅ Spawned goroutine with ID: {s}\n", .{goroutine_id});
        
        // Check scheduler status
        if (concurrency.getScheduler()) |scheduler| {
            if (verbose) print("📊 Scheduler status: active goroutines = {s}\n", .{scheduler.activeGoroutineCount()});
        }
    ...
    // Handle function call syntax: stan function_name()
        ...
        if (verbose) print("✅ Spawned function goroutine with ID: {s}\n", .{goroutine_id});
    }
    else {
...
fn handleCheckCommand(allocator: Allocator, args: [][:0]u8) !void {
    ...
    while (lines.next()) |line| {
        ...
        if (!isValidCursedStatement(trimmed)) {
            print("❌ Syntax error on line {s}: {s}\n", .{ line_number, trimmed });
            validation_passed = false;
        }
        
        if (verbose) print("✅ Line {s} validated: {s}\n", .{ line_number, trimmed });
        line_number += 1;
    }
...
fn handleTestzFunction(allocator: Allocator, variables: *VariableStore, function_name: []const u8, args: []const u8) !void {
    ...
    if (std.mem.eql(u8, function_name, "assert_true")) {
        ...
        if (std.mem.indexOf(u8, args, ",")) |comma_pos| {
            ...
            if (actual != expected) {
                print("❌ Assertion failed: assert_eq_int({s}, {s}) - actual: {s}, expected: {s}\n", .{ actual, expected, actual, expected });
            }
        }
    }
}
...
fn handleFunctionDeclaration(functions: *FunctionStore, allocator: Allocator, source_lines: ArrayList([]const u8), start_line: usize, verbose: bool) !usize {
    ...
    if (std.mem.indexOf(u8, trimmed, "{")) |open_brace_pos| {
        if (std.mem.lastIndexOf(u8, trimmed, "}")) |close_brace_pos| {
            ...
            if (verbose) print("✅ Single-line function {s} stored with {s} parameters and {s} body lines\n", .{ func_name, func_def.parameters.items.len, func_def.body.items.len });
    ...
    const func_store_key = try allocator.dupe(u8, func_name);
    try functions.put(func_store_key, func_def);
    if (verbose) print("✅ Function {s} stored with {s} parameters and {s} body lines\n", .{ func_name, func_def.parameters.items.len, func_def.body.items.len });
        ...
        if (std.mem.indexOf(u8, trimmed[start_paren..], ")")) |rel_end_paren| {
            ...
            if (std.mem.indexOf(u8, trimmed[end_paren..], "{")) |rel_brace_start| {
                if (std.mem.lastIndexOf(u8, trimmed, "}")) |brace_end| {
                    ...
                    defer { var val = match_value; val.deinit(); }
...
fn handleFamBlock(
    ...
    while (line_index < source_lines.items.len) {
        ...
        if (std.mem.indexOf(u8, line, "{")) |_| {
            if (!in_try_block) {
                ...
                if (verbose) print("📍 Try block starts at line {s}\n", .{try_start_line});
            }
            brace_depth += 1;
        }
        ...
        if (std.mem.indexOf(u8, line, "}")) |_| {
            ...
            if (is_shook_line and in_try_block) {
                ...
                in_try_block = false;
                if (verbose) print("📍 Try block ends at line {s}: '{s}'\n", .{line_index + 1, line});
                break;
            }
        }
    ...
    if (in_catch_block and catch_start_line > 0) {
        if (verbose) print("📍 Processing fam block body starting at line {s}\n", .{catch_start_line});
        ...
        while (line_index < source_lines.items.len and brace_depth > 0) {
            ...
            if (std.mem.indexOf(u8, line, "}")) |_| {
                brace_depth -= 1;
                if (brace_depth == 0) {
                    if (verbose) print("📍 Fam block ends at line {s}\n", .{line_index + 1});
                    break;
                }
            }
        }
    ...
    var try_statements = std.ArrayList(u8){};
    defer try_statements.deinit();
    
    var catch_statements = std.ArrayList(u8){};
    ...
    if (try_start_line > 0) {
        for (try_start_line..line_index) |exec_line_idx| {
            ...
            try try_statements.append(allocator, exec_line);
    ...
    if (catch_start_line > 0) {
        for (catch_start_line..line_index) |exec_line_idx| {
            ...
            try catch_statements.append(allocator, exec_line);
    ...
    if (verbose) print("✅ Fam try-catch block processed, consumed {s} lines\n", .{lines_consumed});
    return lines_consumed;
}
...
fn handleReadyOtherwiseBlock(
    ...
    if (verbose) print("🔍 Starting ready/otherwise block parsing at line {s}\n", .{start_line_index + 1});
    ...
    if (verbose) print("✅ Condition evaluated to: {s}\n", .{condition_is_true});
    ...
    var block_content = std.ArrayList(u8){};
    ...
    if (has_pattern_matching) {
        ...
        if (condition_is_true) {
            if (verbose) print("🟢 Executing if block (lines {s}-{s})\n", .{ if_block_start + 1, if_block_end });
            ...
            if (verbose) print("🔴 Executing else block (lines {s}-{s})\n", .{ else_block_start.? + 1, else_block_end.? });
    ...
    if (verbose) print("✅ Ready/otherwise block processed, consumed {s} lines\n", .{total_lines_consumed});
...
fn matchesPattern(
    ...
    if (std.mem.indexOf(u8, pattern, " when ")) |when_pos| {
        ...
        const condition_result = evaluateExpression(&guard_variables, &guard_functions, allocator, condition_expr, verbose) catch |err| {
            if (verbose) print("❌ Error evaluating when guard condition: {s}\n", .{err});
            return false;
        };
        defer { var cond = condition_result; cond.deinit(); }
        ...
        if (verbose) print("🛡️ When guard condition result: {s}\n", .{matches});
        return matches;
    }
    ...
    switch (value) {
        .Integer => |int_val| {
            ...
            if (std.mem.indexOf(u8, pattern, "..")) |dot_pos| {
                ...
                if (std.fmt.parseInt(i64, start_str, 10)) |start_val| {
                    if (std.fmt.parseInt(i64, end_str, 10)) |end_val| {
                        ...
                        if (verbose) print("📏 Range pattern: {s} <= {s} <= {s} -> {s}\n", .{ start_val, int_val, end_val, matches });
                        return matches;
                    } else |_| {
            ...
            if (std.fmt.parseInt(i64, pattern, 10)) |pattern_int| {
                ...
                if (verbose) print("🔢 Integer pattern: {s} == {s} -> {s}\n", .{ int_val, pattern_int, matches });
                return matches;
            } else |_| {
        ...
        .Float => |float_val| {
            ...
            if (std.mem.indexOf(u8, pattern, "..")) |dot_pos| {
                ...
                if (std.fmt.parseFloat(f64, start_str)) |start_val| {
                    if (std.fmt.parseFloat(f64, end_str)) |end_val| {
                        ...
                        if (verbose) print("📏 Float range pattern: {d} <= {d} <= {d} -> {s}\n", .{ start_val, float_val, end_val, matches });
                        return matches;
                    } else |_| {
            ...
            if (std.fmt.parseFloat(f64, pattern)) |pattern_float| {
                ...
                if (verbose) print("🔢 Float pattern: {d} == {d} -> {s}\n", .{ float_val, pattern_float, matches });
                return matches;
            } else |_| {
        ...
        .Boolean => |bool_val| {
            ...
            if (std.mem.eql(u8, pattern, "based")) {
                ...
                if (verbose) print("🔘 Boolean pattern: based -> {s}\n", .{matches});
                return matches;
            } else if (std.mem.eql(u8, pattern, "cringe")) {
                const matches = bool_val == false;
                if (verbose) print("🔘 Boolean pattern: cringe -> {s}\n", .{matches});
                return matches;
            } else {
...
fn handleInlineBestieLoop(
    ...
    while (iteration_count < max_iterations) {
        ...
        var body_statements = std.ArrayList([]const u8){};
        ...
        for (body_text, 0..) |char, i| {
            if (char == '"' and (i == 0 or body_text[i-1] != '\\')) {
                ...
                if (char == '{') {
                    ...
                    if (stmt.len > 0) {
                        try body_statements.append(allocator, stmt);
                    }
                    start = i + 1;
                }
            }
    ...
    if (iteration_count >= max_iterations) {
        if (verbose) print("⚠️  Loop terminated after {s} iterations (safety limit)\n", .{max_iterations});
    } else if (verbose) {
        print("✅ Loop completed after {s} iterations\n", .{iteration_count});
    }
}
...
fn handleBestieLoop(
    ...
    if (verbose) print("🔍 Starting bestie loop parsing at line {s}\n", .{start_line_index + 1});
    ...
    if (verbose) print("🔄 Loop body: lines {s}-{s}\n", .{ loop_body_start + 1, loop_body_end });
    ...
    while (iteration_count < max_iterations) {
        ...
        if (verbose) print("🐛 DEBUG: Executing loop body lines {s} to {s}\n", .{loop_body_start, loop_body_end});
        ...
        while (body_line_idx < body_lines.items.len) {
            ...
            if (verbose) print("🐛 DEBUG: Loop line {s}: '{s}'\n", .{body_line_idx, exec_line});
    ...
    if (verbose) print("✅ Bestie loop processed, consumed {s} lines\n", .{total_lines_consumed});
    return total_lines_consumed;
}
...
fn executeIfStatement(
    ...
    if (std.mem.indexOf(u8, line, "(")) |open_paren| {
        if (std.mem.indexOf(u8, line[open_paren..], ")")) |close_paren_offset| {
            ...
            if (verbose) print("  🔍 Condition '{s}' evaluated to: {any} (truthy: {s})\n", .{ condition_expr, condition_result, is_true });
        }
    }
}
                ...
                .Array => |*arr| {
                    ...
                    if (std.fmt.parseInt(usize, index_expr, 10)) |index| {
                        if (index < arr.items.len) {
                            ...
                            if (verbose) print("  ✅ Array assignment: {s}[{s}] = {any}\n", .{ array_name, index, value });
                        } else {
                            if (verbose) print("  ❌ Array index out of bounds: {s}\n", .{index});
                        }
                    } else |_| {
