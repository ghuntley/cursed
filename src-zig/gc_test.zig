...
test "gc stress test" {
    ...
    var live_objects = std.ArrayList(*TestObject).init(test_allocator);
...
test "gc heap management" {
    ...
    var objects = std.ArrayList(*TestObject).init(test_allocator);
...
test "gc concurrent safety" {
    ...
    var thread1 = AllocatorThread{
        .gc = gc,
        .objects = std.ArrayList(*TestObject).init(test_allocator),
    };
...
test "gc collection benchmark" {
    ...
    var roots = std.ArrayList(?*anyopaque).init(test_allocator);
    defer roots.deinit();
