...
fn interpretProgramWithVariables(allocator: Allocator, source: []const u8, verbose: bool, stdlib_path: ?[]const u8) !void {
    ...
    defer error_state.deinit(allocator);
    ...
    defer simple_loader.deinit(allocator);
    ...
    while (line_index < source_lines.items.len) {
        ...
        if (std.mem.startsWith(u8, trimmed, "stan ") and std.mem.indexOf(u8, trimmed, "{") != null) {
            ...
            if (std.mem.indexOf(u8, trimmed, "}") == null) {
                ...
                defer full_statement.deinit(allocator);
                ...
                const full_stan = try full_statement.toOwnedSlice(allocator);
                        ...
                        if (struct_instance.fields.get(first_field)) |nested_struct_value| {
                            switch (nested_struct_value) {
                                .Struct => |_| {
                                    ...
                                    defer temp_vars.deinit(allocator);
            ...
            if (content.len > 0) {
                ...
                while (elements.next()) |element| {
                    ...
                    if (std.mem.eql(u8, element_type, "normie") or std.mem.eql(u8, element_type, "drip")) {
                        ...
                        switch (element_var) {
                            .Integer => try array.append(allocator, element_var),
                            .Float => |f| try array.append(allocator, Variable{ .Integer = @as(i64, @intFromFloat(f)) }),
                            else => {
                        ...
                        switch (element_var) {
                            .String => try array.append(allocator, element_var),
                            else => {
                                const str_val = try element_var.toString(allocator);
                                var temp_var = element_var;
                                temp_var.deinit(allocator);
                                try array.append(allocator, Variable{ .String = ManagedString.fromOwned(str_val) });
                            }
                        }
                        ...
                        switch (element_var) {
                            .Float => try array.append(allocator, element_var),
                            .Integer => |i| try array.append(allocator, Variable{ .Float = @as(f64, @floatFromInt(i)) }),
                            else => {
                                if (verbose) print("❌ Array element '{s}' is not numeric, got: {any}\n", .{trimmed_element, element_var});
                                var temp_var = element_var;
                                temp_var.deinit(allocator);
                        ...
                        switch (element_var) {
                            .Boolean => try array.append(allocator, element_var),
                            else => {
...
fn handleFamBlock(
    ...
    var try_statements = .empty;
    defer try_statements.deinit(allocator);
    
    var catch_statements = .empty;
    defer catch_statements.deinit(allocator);
