//! I/O functionality for performance_optimizations
...
impl IOHandler {
    ...
    pub fn read_all<R: Read>(&self, mut reader: R) -> IOResult<Vec<u8>> {
        ...
        reader.read_to_end(&mut buffer)
            .map_err(|e| CursedError::Io(format!("Read error: {}")))?;
        Ok(buffer)
    }
    ...
    pub fn write_all<W: Write>(&self, mut writer: W, data: &[u8]) -> IOResult<()> {
        writer.write_all(data)
            .map_err(|e| CursedError::Io(format!("Write error: {}")))?;
        Ok(())
    }
    ...
    pub fn read_string<R: Read>(&self, reader: R) -> IOResult<String> {
        ...
        String::from_utf8(bytes)
            .map_err(|e| CursedError::Io(format!("UTF-8 decode error: {}")))
    }
...
pub fn init_performance_optimizations() -> IOResult<()> {
    ...
    println!("📁 I/O processing (performance_optimizations) initialized");
    Ok(())
}

/// Test I/O functionality
pub fn test_performance_optimizations() -> IOResult<()> {
    let handler = IOHandler::new();
