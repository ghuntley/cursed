//! Cryptographic functionality for benchmarks
...
pub fn init_benchmarks() -> CryptoResult<()> {
    let handler = CryptoHandler::new();
    let key = handler.generate_key()?;
    if key.len() != 32 {
        return Err(CursedError::runtime_error("Crypto key generation test failed"));
    }
    println!("🔐 Crypto processing (benchmarks) initialized");
    Ok(())
}

/// Test crypto functionality
pub fn test_benchmarks() -> CryptoResult<()> {
    let handler = CryptoHandler::new();
