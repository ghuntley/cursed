//! Simple but Complete CURSED Language Server Protocol Implementation
...
impl CursedLanguageServer {
    ...
    async fn analyze_document(&self, uri: &Url, text: &str, version: i32) -> DocumentData {
        ...
        // Syntax analysis
        let mut lexer_for_parser = Lexer::new(text.to_string());
        let parser_result = Parser::new(lexer_for_parser);
        match parser_result {
            Ok(mut parser) => {
                match parser.parse() {
                    Ok(program) => {
    ...
    fn extract_symbols(&self, program: &crate::ast::Ast, uri: &Url) -> Vec<SymbolInformation> {
        let mut symbols = Vec::new();
        ...
        for statement in &program.statements {
            match statement {
                ...
                Statement::Let(let_stmt) => {
                    // Extract variable name from LetTarget
                    if let crate::ast::LetTarget::Variable(name) = &let_stmt.target {
                        symbols.push(SymbolInformation {
                            name: name.clone(),
...
mod tests {
    ...
    fn create_mock_client() -> Client {
        // This is a placeholder for testing - in real tests you'd create a proper mock client
        unimplemented!("Mock client not implemented for testing")
    }
