...
pub fn main() !void {
    ...
    try advanced_codegen.generateStruct("Rectangle", &[_][]const u8{ "double", "double", "double", "double" });
    
    // Add interface definitions
    try advanced_codegen.generateInterface("Shape", &[_][]const u8{ "area", "perimeter", "draw" });
    try advanced_codegen.generateInterface("Drawable", &[_][]const u8{ "render", "paint" });
    
    // Add advanced functions
    try advanced_codegen.generateFunction("calculate_area", "double", 
        &[_][]const u8{ "%struct.Rectangle* %rect" }, 
        "  ; Calculate rectangle area\n  ret double 0.0\n");
    
    try advanced_codegen.generateFunction("distance_between", "double", 
        &[_][]const u8{ "%struct.Point* %p1", "%struct.Point* %p2" }, 
        "  ; Calculate distance between points\n  ret double 0.0\n");
    
    // Compile the basic program
    try advanced_codegen.compile(basic_source);
