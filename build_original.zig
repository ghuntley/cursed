...
fn addLlvm(b: *std.Build, exe: *std.Build.Step.Compile, target: std.Build.ResolvedTarget) void {
    ...
    // Add LLVM system paths based on detected installation
    const llvm_config_available = b.findProgram(&[_][]const u8{"llvm-config"}, &[_][]const u8{}) catch null;
    
    if (llvm_config_available != null) {
        // Use llvm-config to get proper paths and libraries
        const llvm_prefix_result = b.run(&[_][]const u8{ "llvm-config", "--prefix" }) catch return;
        const llvm_libs_result = b.run(&[_][]const u8{ "llvm-config", "--libs", "core", "native", "orcjit", "mcjit" }) catch return;
        const llvm_cflags_result = b.run(&[_][]const u8{ "llvm-config", "--cflags" }) catch return;
        
        if (llvm_prefix_result.stderr.len == 0 and llvm_libs_result.stderr.len == 0) {
            const llvm_prefix = std.mem.trim(u8, llvm_prefix_result.stdout, " \t\n\r");
            const llvm_libs = std.mem.trim(u8, llvm_libs_result.stdout, " \t\n\r");
            const llvm_cflags = std.mem.trim(u8, llvm_cflags_result.stdout, " \t\n\r");
            
            // Add include paths
            exe.addSystemIncludePath(.{ .path = b.fmt("{s}/include", .{llvm_prefix}) });
            exe.addLibraryPath(.{ .path = b.fmt("{s}/lib", .{llvm_prefix}) });
            
            // Parse and add LLVM libraries
            var lib_iter = std.mem.split(u8, llvm_libs, " ");
            ...
            // Add compiler flags
            var flag_iter = std.mem.split(u8, llvm_cflags, " ");
            while (flag_iter.next()) |flag| {
                if (std.mem.startsWith(u8, flag, "-D")) {
                    const define = flag[2..];
                    if (std.mem.indexOf(u8, define, "=")) |eq_pos| {
                        exe.root_module.addCMacro(define[0..eq_pos], define[eq_pos + 1..]);
                    } else {
        ...
        // Fallback to hardcoded paths for Ubuntu/Debian systems
        exe.addSystemIncludePath(.{ .path = "/usr/include/llvm-c-18" });
        exe.addSystemIncludePath(.{ .path = "/usr/include/llvm-18" });
...
b.installArtifact(complete_exe);
    ...
    // Create syscall-enabled compiler with real file I/O, networking, and process management
    // Only build for native target to avoid cross-compilation LLVM issues
    const syscall_exe: ?*std.Build.Step.Compile = if (!is_cross_compile and !is_wasm) blk: {
        const syscall_exe_binary = b.addExecutable(.{
            .name = "cursed-syscall",
            .root_source_file = b.path("src-zig/main_unified.zig"),
            .target = resolved_target,
            .optimize = optimize,
        });
        syscall_exe_binary.linkLibC();
        addLlvm(b, syscall_exe_binary, resolved_target);
        break :blk syscall_exe_binary;
    } else null;
