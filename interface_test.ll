...
%interface.TestInterface = type { %vtable.TestInterface*, i8* }
%vtable.TestInterface = type { i32 ()* }
@vtable.TestInterface.TestInterfaceImpl = global %vtable.TestInterface { i32 ()* @TestInterfaceImpl.test_method.TestInterface }
...
; Fast dispatch for TestInterface.test_method
define i32 @dispatch_TestInterface_test_method_0(i8* %interface_value) {
entry:
    ; Extract vtable from interface value
    %interface_ptr = bitcast i8* %interface_value to {i8*, i8*}*
    %vtable_ptr_ptr = getelementptr {i8*, i8*}, {i8*, i8*}* %interface_ptr, i32 0, i32 0
    ...
    %vtable_typed = bitcast i8* %vtable_ptr to %vtable.TestInterface*
    
    ; Get method pointer from vtable
    %method_ptr_ptr = getelementptr %vtable.TestInterface, %vtable.TestInterface* %vtable_typed, i32 0, i32 0
...
; Polymorphic dispatch for TestInterface
define i8* @dispatch_TestInterface_polymorphic(i8* %interface_value, i8* %method_name, i8* %args, i32 %arg_count) {
entry:
    ; Create method name comparison blocks
    br label %compare_0

compare_0:
    %method_name_0 = call i8* @strcmp(i8* %method_name, i8* getelementptr inbounds ([12 x i8], [12 x i8]* @str.TestInterface.test_method, i32 0, i32 0))
    %is_method_0 = icmp eq i8* %method_name_0, i8* null
    br i1 %is_method_0, label %dispatch_0, label %method_not_found

dispatch_0:
    %result_0 = call i8* @dispatch_TestInterface_test_method_0(i8* %interface_value, i8* %args, i32 %arg_count)
    ret i8* %result_0
