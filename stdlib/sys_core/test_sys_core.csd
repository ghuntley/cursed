...
fr fr Comprehensive sys_core module tests

test_start("sys_core system information tests")

fr fr Test system information functions
...
assert_true(sys_core.set_memory_limit(268435456)) fr fr 256MB
...
fr fr Test signal handling
assert_true(sys_core.register_signal_handler(15)) fr fr SIGTERM
assert_true(sys_core.ignore_signal(2)) fr fr SIGINT
assert_true(sys_core.send_signal(current_pid, 0)) fr fr Test signal

print_test_summary()

test_start("sys_core resource limit tests")

fr fr Test resource limits
assert_true(sys_core.set_resource_limit(1, 1000000)) fr fr Memory limit
...
fr fr Test hardware information
...
assert_true(sys_core.set_stack_size(16777216)) fr fr 16MB

print_test_summary()

test_start("sys_core network and security tests")

fr fr Test network functions
...
fr fr Should be false for normal user
...
fr fr Test module initialization and cleanup
...
assert_true(sys_core.free(0)) fr fr Should handle null pointer gracefully
assert_false(sys_core.process_exists(999999)) fr fr Non-existent process

fr fr Test with invalid parameters
assert_true(sys_core.set_memory_limit(-1)) fr fr Should handle gracefully
assert_true(sys_core.set_resource_limit(999, 0)) fr fr Invalid resource

print_test_summary()

test_start("sys_core integration tests")

fr fr Test complex operations combining multiple functions
sus initial_memory normie = sys_core.memory_usage()
sus addr1 normie = sys_core.alloc(2048)
sus addr2 normie = sys_core.alloc(4096)

assert_true(addr1 != addr2) fr fr Different addresses
assert_true(sys_core.free(addr1))
assert_true(sys_core.free(addr2))

fr fr Test process and signal integration
sus test_pid normie = sys_core.spawn_process("sleep 1")
assert_true(sys_core.process_exists(test_pid))
assert_true(sys_core.send_signal(test_pid, 15)) fr fr SIGTERM
...
fr fr Test performance of memory operations
sus start_time normie = sys_core.get_system_time()

fr fr Allocate and free multiple blocks
bestie i := 0; i < 100; i++ {
...
fr fr Should complete quickly (less than 1 second for simulation)
...
fr fr Test with large allocations
sus large_addr normie = sys_core.alloc(1048576) fr fr 1MB
assert_true(large_addr > 0)
assert_true(sys_core.free(large_addr))

fr fr Test resource limit boundaries
