...
# Comprehensive Error Management Tests
# Tests all error handling, logging, and recovery functionality

# Test error creation and basic functionality
slay test_error_creation() {
    test_start("Error Creation Tests")
    ...
    assert_true(err.category == error_category.runtime_yikes, "Default category should be runtime")
...
slay test_error_wrapping() {
    test_start("Error Wrapping Tests")
    
    # Create original error
    sus original @managed_error = new_error("Original error", 404)
    ...
    assert_eq_string(wrapped.message, "Database operation failed: Original error", "Wrapped message should include context")
    
    # Test unwrapping
    sus unwrapped @managed_error = unwrap_error(wrapped)
    assert_true(unwrapped == original, "Unwrapped error should be original")
    
    # Test unwrapping nil error
...
slay test_error_context() {
    ...
    sus err @managed_error = new_error("Test error with context", 400)
    
    # Add context
    err.add_context("user_id", "12345")
    err.add_context("request_id", "req-abc-123")
    err.add_context("operation", "user_lookup")
...
slay test_error_formatting() {
    test_start("Error Formatting Tests")
    ...
    assert_true(len(formatted) > 0, "Formatted error should not be empty")
    ...
    assert_true(len(detailed_formatted) > len(formatted), "Detailed error should be longer")
    
    # Test wrapped error formatting
    sus wrapped @managed_error = wrap_error(simple_err, "Wrapper context")
    sus wrapped_formatted tea = format_error(wrapped)
    assert_true(len(wrapped_formatted) > len(formatted), "Wrapped error should include original")
...
# Test severity and category string conversion
slay test_string_conversions() {
    test_start("String Conversion Tests")
    ...
    assert_eq_string(severity_to_string(error_severity.fatal), "FATAL", "Fatal severity string")
...
slay test_logging_basic() {
    test_start("Basic Logging Tests")
    ...
    log_warn("Warning message", yikes.tea{"warning_type": "test"})
...
slay test_error_logging() {
    test_start("Error Logging Tests")
...
slay test_circuit_breaker() {
    test_start("Circuit Breaker Tests")
    ...
    assert_true(cb.state == circuit_state.closed, "Initial state should be closed")
    ...
    assert_true(cb.state == circuit_state.closed, "State should remain closed after success")
...
slay test_retry_backoff() {
    ...
    assert_eq_int(attempts, 3, "Should have taken 3 attempts")
...
slay test_error_classification() {
    test_start("Error Classification Tests")
    ...
    assert_false(is_temporary_error(parse_error), "Parse error should not be temporary")
...
# Test safe execution with panic recovery
slay test_safe_execution() {
    test_start("Safe Execution Tests")
    ...
    assert_true(result == cringe, "Safe execution of successful operation should return nil")
    
    # Test operation that would panic (simulated)
    sus panic_operation slay() @managed_error = slay() @managed_error {
        # In real implementation, this would cause a panic
        # For testing, we simulate by returning a specific error
...
slay test_error_aggregation() {
    test_start("Error Aggregation Tests")
    ...
    assert_true(empty_result == cringe, "Aggregating empty errors should return nil")
    ...
    assert_true(single_result == single_error, "Aggregating single error should return that error")
...
slay test_error_statistics() {
    test_start("Error Statistics Tests")
    
    # Reset statistics
    reset_error_stats()
    sus initial_stats error_stats = get_error_stats()
    assert_eq_int(initial_stats.total_errors, 0, "Initial error count should be 0")
    
    # Create and log various errors to update statistics
    ...
    # Update statistics for each error
    bestie i := 0; i < len(errors); i++ {
        update_error_stats(errors[i])
    }
    
    sus final_stats error_stats = get_error_stats()
    assert_eq_int(final_stats.total_errors, 5, "Should have tracked 5 errors")
...
slay test_complete_workflow() {
    ...
    sus cb @circuit_breaker = new_circuit_breaker("complex_service", 2, 30)
    
    # Wrapper for circuit breaker
    slay protected_operation() @managed_error {
        damn cb.execute(slay() @managed_error {
            damn complex_operation(cap)  # Success
        })
    }
    
    # Test successful operation
    sus success_result @managed_error = protected_operation()
    assert_true(success_result == cringe, "Protected successful operation should succeed")
    ...
    sus retry_operation slay() @managed_error = slay() @managed_error {
        ...
        vibe_check retry_count < 3 {
            damn complex_operation(based)  # Fail first 2 times
        }
        damn complex_operation(cap)  # Succeed on 3rd try
    }
