# Enhanced CURSED Async Core Module Tests
# Comprehensive test suite for task scheduling, cancellation, and error propagation
...
slay test_task_creation() {
    ...
    assert_false(async_is_completed(task))
    ...
    sus task *Task = async_spawn(slay() {
        ...
        bestie i := 0; i < 1000000; i++ {
            yo async_is_cancelled() {
                damn -1  # Return special value on cancellation
            }
        }
        damn 100
    })
    
    # Cancel the task before completion
    sus cancelled lit = async_cancel(task)
    assert_true(cancelled)
    assert_true(async_is_cancelled(task))
    
    # Wait for cancelled task
    sus result normie = async_await(task)
    assert_eq_int(result, -1)  # Should return cancellation value
...
slay test_async_timeout() {
    ...
    sus task *Task = async_spawn_with_timeout(slay() {
        # Simulate long operation
        async_sleep(5000)  # 5 seconds
        damn 123
...
# Test error propagation in async tasks
slay test_error_propagation() {
    test_start("error_propagation")
    
    sus task *Task = async_spawn(slay() {
        yo based {  # Always true - force error
...
# Test task scheduler operations
slay test_task_scheduler() {
    test_start("task_scheduler")
    ...
    assert_true(total_count >= active_count)
    
    # Test scheduler capacity
...
slay test_concurrent_execution() {
    ...
    sus tasks []*Task = make([]*Task, 10)
    
    # Spawn multiple concurrent tasks
    bestie i := 0; i < 10; i++ {
        tasks[i] = async_spawn(slay() {
            async_sleep(100)  # Small delay
            damn i
        })
    }
...
slay test_async_channels() {
    ...
    sus ch *AsyncChannel = async_channel_new(5)  # Buffered channel
    assert_true(ch != cringe)
    assert_eq_int(async_channel_capacity(ch), 5)
    assert_eq_int(async_channel_length(ch), 0)
...
slay test_async_mutex() {
    ...
    assert_eq_int(shared_counter, 50)  # 5 tasks * 10 increments = 50
    
    vibez.spill("✅ Async mutex test passed")
}

# Test async condition variables
...
slay test_async_worker_pool() {
    ...
    sus pool *AsyncWorkerPool = async_worker_pool_new(3)  # 3 workers
    assert_true(pool != cringe)
    assert_eq_int(async_worker_pool_size(pool), 3)
    
    sus results []*Task = make([]*Task, 10)
    ...
    assert_eq_int(sum, 90)  # 0*2 + 1*2 + ... + 9*2 = 90
...
# Test async futures and promises
slay test_async_futures() {
    ...
    assert_false(async_future_is_ready(future))
    
    # Resolve promise in separate task
...
slay test_async_rate_limiting() {
    ...
    sus limiter *AsyncRateLimiter = async_rate_limiter_new(2, 1000)  # 2 tokens per second
    assert_true(limiter != cringe)
    assert_eq_int(async_rate_limiter_capacity(limiter), 2)
    
    # Use all tokens
    assert_true(async_rate_limiter_acquire(limiter))
    assert_true(async_rate_limiter_acquire(limiter))
    assert_false(async_rate_limiter_try_acquire(limiter))  # Should fail
    
    # Wait for token refill
    async_sleep(1100)
    assert_true(async_rate_limiter_try_acquire(limiter))  # Should succeed
    
    vibez.spill("✅ Async rate limiting test passed")
}

# Test async task groups
slay test_async_task_groups() {
    ...
    assert_eq_int(async_task_group_size(group), 0)
    ...
    assert_eq_int(async_task_group_size(group), 3)
