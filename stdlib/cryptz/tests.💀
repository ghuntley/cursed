...
sus random_data1 drip[value] = cryptz.generate_random_bytes(32)
sus random_data2 drip[value] = cryptz.generate_random_bytes(32)
...
sus zero_bytes drip[value] = cryptz.generate_random_bytes(0)
assert_eq_int(len(zero_bytes), 0)

sus single_byte drip[value] = cryptz.generate_random_bytes(1)
...
sus key128 drip[value] = cryptz.generate_secure_key(16)
sus key256 drip[value] = cryptz.generate_secure_key(32)
...
sus sha256_result drip[value] = cryptz.sha256_hash(test_message)
...
sus hash1 drip[value] = cryptz.sha256_hash("test")
sus hash2 drip[value] = cryptz.sha256_hash("test")
...
sus hash_a drip[value] = cryptz.sha256_hash("input_a")
sus hash_b drip[value] = cryptz.sha256_hash("input_b")
...
fr fr Test SHA-512
sus sha512_result drip[value] = cryptz.sha512_hash(test_message)
assert_eq_int(len(sha512_result), cryptz.SHA512_DIGEST_SIZE)
vibez.spill("✅ SHA-512 produces correct digest size")

fr fr Test BLAKE3
sus blake3_result drip[value] = cryptz.blake3_hash(test_message)
...
sus empty_hash drip[value] = cryptz.sha256_hash("")
...
sus hmac_key drip[value] = cryptz.generate_secure_key(32)
sus message tea = "Important message requiring authentication"

sus hmac_result drip[value] = cryptz.hmac_sha256(hmac_key, stringz.bytes(message))
...
sus key1 drip[value] = cryptz.generate_secure_key(32)
sus key2 drip[value] = cryptz.generate_secure_key(32)

sus hmac1 drip[value] = cryptz.hmac_sha256(key1, stringz.bytes("message"))
sus hmac2 drip[value] = cryptz.hmac_sha256(key2, stringz.bytes("message"))
...
sus hmac512_result drip[value] = cryptz.hmac_sha512(hmac_key, stringz.bytes(message))
...
sus aes_key drip[value] = cryptz.generate_secure_key(cryptz.AES256_KEY_SIZE)
sus additional_data tea = "public_metadata"

sus encrypted drip[value] = cryptz.aes_gcm_encrypt(plaintext, aes_key, additional_data)
...
sus decrypted drip[value] = cryptz.aes_gcm_decrypt(encrypted, aes_key, additional_data)
...
sus wrong_key drip[value] = cryptz.generate_secure_key(cryptz.AES256_KEY_SIZE)
sus failed_decrypt drip[value] = cryptz.aes_gcm_decrypt(encrypted, wrong_key, additional_data)
...
sus tampered_decrypt drip[value] = cryptz.aes_gcm_decrypt(encrypted, aes_key, "tampered_metadata")
...
sus encrypted1 drip[value] = cryptz.aes_gcm_encrypt("same plaintext", aes_key, "")
sus encrypted2 drip[value] = cryptz.aes_gcm_encrypt("same plaintext", aes_key, "")
...
sus chacha_key drip[value] = cryptz.generate_secure_key(cryptz.CHACHA20_KEY_SIZE)
sus chacha_nonce drip[value] = cryptz.generate_random_bytes(cryptz.CHACHA20_NONCE_SIZE)
sus plaintext_stream tea = "Stream cipher test message for ChaCha20"

sus encrypted_stream drip[value] = cryptz.chacha20_encrypt(plaintext_stream, chacha_key, chacha_nonce)
...
sus decrypted_stream drip[value] = cryptz.chacha20_encrypt(stringz.from_bytes(encrypted_stream), chacha_key, chacha_nonce)
...
sus invalid_key drip[value] = cryptz.generate_random_bytes(16)  fr fr Wrong size
sus invalid_result drip[value] = cryptz.chacha20_encrypt("test", invalid_key, chacha_nonce)
...
sus signature drip[value] = cryptz.ed25519_sign(document, ed_keypair.private_key)
...
sus salt drip[value] = cryptz.generate_random_bytes(32)
sus derived_key1 drip[value] = cryptz.pbkdf2_derive_key(password, salt, 100000, 32)

assert_eq_int(len(derived_key1), 32)
vibez.spill("✅ PBKDF2 produces correct output length")

fr fr Test PBKDF2 consistency
sus derived_key2 drip[value] = cryptz.pbkdf2_derive_key(password, salt, 100000, 32)
...
sus salt2 drip[value] = cryptz.generate_random_bytes(32)
sus derived_key3 drip[value] = cryptz.pbkdf2_derive_key(password, salt2, 100000, 32)
...
fr fr Test Argon2
sus argon2_key drip[value] = cryptz.argon2_derive_key(password, salt, 65536, 3, 1, 32)
assert_eq_int(len(argon2_key), 32)
vibez.spill("✅ Argon2 key derivation working")

fr fr Test Scrypt
sus scrypt_key drip[value] = cryptz.scrypt_derive_key(password, salt, 16384, 8, 1, 32)
...
sus data1 drip[value] = [1, 2, 3, 4, 5]
sus data2 drip[value] = [1, 2, 3, 4, 5]
sus data3 drip[value] = [1, 2, 3, 4, 6]
...
sus sensitive_data drip[value] = [0xde, 0xad, 0xbe, 0xef, 0xca, 0xfe, 0xba, 0xbe]
...
sus test_bytes drip[value] = [0x01, 0x23, 0x45, 0x67, 0x89, 0xab, 0xcd, 0xef]
...
sus decoded_bytes drip[value] = cryptz.hex_to_bytes(hex_string)
...
sus decoded_base64 drip[value] = cryptz.base64_decode(base64_string)
...
sus key drip[value] = cryptz.generate_secure_key(32)
sus ciphertext drip[value] = cryptz.aes_gcm_encrypt(secret, key, "")
...
sus sig drip[value] = cryptz.ed25519_sign(message, keypair.private_key)
...
sus original_hash drip[value] = cryptz.sha256_hash(original)

sus modified tea = "Important document (modified)"
sus modified_hash drip[value] = cryptz.sha256_hash(modified)
...
sus large_hash drip[value] = cryptz.sha256_hash(large_data)
assert_eq_int(len(large_hash), cryptz.SHA256_DIGEST_SIZE)
vibez.spill("✅ Hash function handles large data")

sus large_key drip[value] = cryptz.generate_secure_key(32)
sus large_encrypted drip[value] = cryptz.aes_gcm_encrypt(large_data, large_key, "")
assert_true(len(large_encrypted) > stringz.length(large_data))

sus large_decrypted drip[value] = cryptz.aes_gcm_decrypt(large_encrypted, large_key, "")
...
bestie i := 0; i < 10; i++ {
    ...
    sus hash_output drip[value] = cryptz.sha256_hash(test_input)
...
sus short_key drip[value] = cryptz.generate_random_bytes(8)  fr fr Too short for AES-256
sus invalid_encrypted drip[value] = cryptz.aes_gcm_encrypt("test", short_key, "")
assert_eq_int(len(invalid_encrypted), 0)
vibez.spill("✅ Encryption rejects invalid key sizes")

fr fr Test corrupted ciphertext
sus valid_key drip[value] = cryptz.generate_secure_key(32)
sus valid_encrypted drip[value] = cryptz.aes_gcm_encrypt("test", valid_key, "")
...
sus corrupted_result drip[value] = cryptz.aes_gcm_decrypt(valid_encrypted, valid_key, "")
...
sus invalid_bytes drip[value] = cryptz.hex_to_bytes("invalid_hex_characters_xyz")
assert_eq_int(len(invalid_bytes), 0)

sus odd_length_bytes drip[value] = cryptz.hex_to_bytes("abc")  fr fr Odd length
...
sus sha256_test drip[value] = cryptz.sha256_hash("test")
sus sha512_test drip[value] = cryptz.sha512_hash("test")
sus blake3_test drip[value] = cryptz.blake3_hash("test")

assert_eq_int(len(sha256_test), 32)
assert_eq_int(len(sha512_test), 64)
assert_eq_int(len(blake3_test), 32)

sus aes_key drip[value] = cryptz.generate_secure_key(32)
sus aes_test drip[value] = cryptz.aes_gcm_encrypt("test", aes_key, "")
assert_true(len(aes_test) > 4)

sus chacha_key drip[value] = cryptz.generate_secure_key(32)
sus chacha_nonce drip[value] = cryptz.generate_random_bytes(12)
sus chacha_test drip[value] = cryptz.chacha20_encrypt("test", chacha_key, chacha_nonce)
assert_eq_int(len(chacha_test), 4)

sus ed_keypair cryptz.KeyPair = cryptz.ed25519_generate_keypair()
sus ed_sig drip[value] = cryptz.ed25519_sign("test", ed_keypair.private_key)
assert_eq_int(len(ed_sig), 64)

sus pbkdf2_test drip[value] = cryptz.pbkdf2_derive_key("password", [1, 2, 3, 4], 1000, 32)
...
sus sensitive_key drip[value] = cryptz.generate_secure_key(32)
sus key_copy drip[value] = []
...
sus password_bytes drip[value] = stringz.bytes("password123")
sus derived_key drip[value] = cryptz.pbkdf2_derive_key("password123", [1, 2, 3, 4], 1000, 32)
...
slay slice(data drip[value], start drip, length drip) drip[value]{
