# Enhanced CURSED Clock Module Tests
# Comprehensive test suite for timezone handling, leap years, date arithmetic edge cases
...
slay test_leap_year_calculations() {
    test_start("leap_year_calculations")
    ...
    assert_false(is_leap_year(2019))
    ...
    # Test leap century years (divisible by 400)
    ...
    assert_true(is_valid_date(2024, 2, 29))  # Valid leap day
    assert_false(is_valid_date(2023, 2, 29))  # Invalid non-leap day
    
    # Test day addition across leap year boundary
...
slay test_timezone_handling() {
    ...
    assert_eq_int(timezone_offset(utc_tz), 0)
    ...
    assert_eq_string(timezone_name(tokyo_tz), "Asia/Tokyo")
    
    # Test timezone conversion
    sus utc_timestamp thicc = Unix(1704067200, 0)  # 2024-01-01 00:00:00 UTC
    ...
    sus tokyo_time thicc = convert_timezone(utc_timestamp, utc_tz, tokyo_tz)
    
    # EST should be 5 hours behind UTC (in winter)
    sus est_offset thicc = Sub(est_time, utc_timestamp)
    assert_true(est_offset <= -4 * HourVibe && est_offset >= -5 * HourVibe)
    
    # PST should be 8 hours behind UTC (in winter)
    sus pst_offset thicc = Sub(pst_time, utc_timestamp)
    assert_true(pst_offset <= -7 * HourVibe && pst_offset >= -8 * HourVibe)
    
    # Tokyo should be 9 hours ahead of UTC
...
slay test_daylight_saving_time() {
    ...
    # Test spring forward (second Sunday in March)
    ...
    assert_true(is_daylight_saving(after_dst, est_tz) == based)
    
    # Test fall back (first Sunday in November)
    sus before_fall thicc = Date(2024, 11, 3, 1, 30, 0).in_timezone(est_tz)
    sus after_fall thicc = Date(2024, 11, 3, 1, 30, 0).in_timezone(est_tz)
    
    # Test DST offset calculation
    ...
    assert_eq_int(winter_offset, -5 * 3600)  # EST is UTC-5
    assert_eq_int(summer_offset, -4 * 3600)  # EDT is UTC-4
...
slay test_date_arithmetic_edge_cases() {
    test_start("date_arithmetic_edge_cases")
    
    # Test month-end overflow
    sus jan_31 thicc = Date(2024, 1, 31).to_unix()
    sus feb_end thicc = add_months(jan_31, 1)
    assert_eq_int(get_month(feb_end), 2)
    assert_eq_int(get_day_of_month(feb_end), 29)  # Feb 29 in leap year
    ...
    assert_eq_int(get_day_of_month(non_leap_feb_end), 28)  # Feb 28 in non-leap year
    ...
    assert_eq_int(get_second(new_year), 0)
    ...
    assert_eq_int(back_to_past, dec_31)
    
    # Test large duration additions
    sus base_time thicc = Date(2000, 1, 1).to_unix()
    sus far_future thicc = Add(base_time, 10000 * DayVibe)  # ~27 years
    assert_eq_int(get_year(far_future), 2027)
    
    vibez.spill("✅ Date arithmetic edge cases test passed")
}

# Test week calculations and business days
slay test_week_calculations() {
    test_start("week_calculations")
    ...
    assert_true(week53 == 52 || week53 == 53)  # Could be either
    
    # Test weekday calculations
    sus monday thicc = Date(2024, 1, 1).to_unix()  # 2024-01-01 was a Monday
    ...
    assert_eq_int(Weekday(sunday), VibeSunday)
    ...
    assert_eq_int(Weekday(prev_business), VibeFriday)
    
    # Test business days between dates
    sus friday thicc = Date(2024, 1, 5).to_unix()  # Friday
    sus next_monday thicc = Date(2024, 1, 8).to_unix()  # Next Monday
    
    sus business_days normie = count_business_days(friday, next_monday)
    assert_eq_int(business_days, 1)  # Only Monday is a business day
...
slay test_date_parsing_edge_cases() {
    test_start("date_parsing_edge_cases")
    ...
    assert_eq_int(get_second(iso_date), 45)
    ...
    assert_eq_int(get_hour(us_date), 14)  # 2:30 PM = 14:30
    ...
    assert_eq_int(get_day_of_month(eu_date), 15)
    
    # Test edge cases
    sus leap_day thicc = parse_date("2024-02-29")
    assert_true(is_valid_time(leap_day))
    
    sus invalid_date thicc = parse_date("2023-02-29")  # Invalid leap day
    assert_false(is_valid_time(invalid_date))
...
# Test time duration precision and overflow
slay test_duration_precision() {
    test_start("duration_precision")
    
    # Test nanosecond precision
    sus nano_duration thicc = 123456789  # nanoseconds
    ...
    assert_eq_int(second_part, 0)
    
    # Test large duration calculations
    sus large_duration thicc = 365 * DayVibe  # One year
    ...
    assert_eq_int(hours, 365 * 24)
    
    # Test duration overflow protection
    sus max_duration thicc = MaxDuration()
    sus overflow_safe thicc = Add(max_duration, SecondVibe)
    assert_eq_int(overflow_safe, max_duration)  # Should not overflow
    
    # Test duration comparison with precision
    sus duration1 thicc = 1500 * MilliBlink  # 1.5 seconds
    sus duration2 thicc = SecondVibe + 500 * MilliBlink  # 1.5 seconds
...
slay test_time_performance() {
    ...
    bestie i := 0; i < iterations; i++ {
        ...
        sus weekday normie = Weekday(offset_time)
    ...
    sus duration thicc = Sub(end_time, start_time)
