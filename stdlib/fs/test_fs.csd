...
slay test_directory_operations() {
    ...
    assert_true(files_after.length == 1)
...
slay test_path_utilities() {
    ...
    assert_eq_string(joined_with_slash, "/home/user/documents")
...
slay test_file_info() {
    ...
    assert_eq_string(info.name, "test_info_file.txt")
...
slay test_recursive_directory_creation() {
    ...
    assert_true(create_recursive_success)
    
    fr fr Test 2: Verify nested directories exist
    assert_true(fs.file_exists("level1"))
    assert_true(fs.file_exists("level1/level2"))
    assert_true(fs.file_exists("level1/level2/level3"))
