...
fr fr Comprehensive tests for ast_mood module

slay test_ast_node_creation() {
    test_start("AST node creation") fr fr Test basic node creation
    sus program_node normie = create_program_node(1, 1)
    assert_true(program_node > 0)
    assert_true(is_program_node(program_node)) fr fr Test function node creation
    sus func_node normie = create_function_node("test_func", 2, 5)
    assert_true(func_node > 0)
    assert_true(is_function_node(func_node)) fr fr Test variable node creation
...
slay test_ast_node_properties() {
    test_start("AST node properties") fr fr Create a test node
    sus test_node normie = create_ast_node(AST_FUNCTION, "test", "value", 5, 15) fr fr Test node type extraction
    sus node_type normie = ast_node_type(test_node)
    assert_true(node_type == AST_FUNCTION) fr fr Test line and column extraction
...
slay test_node_type_checking() {
    test_start("Node type checking") fr fr Create different types of nodes
    ...
    sus binary_node normie = create_binary_op_node("+", 5, 1) fr fr Test type checking functions
    assert_true(is_program_node(prog_node))
    assert_true(is_function_node(func_node))
    assert_true(is_variable_node(var_node))
    assert_true(is_literal_node(literal_node))
    assert_true(is_binary_op_node(binary_node)) fr fr Test expression node detection
...
slay test_ast_node_string_conversion() {
    test_start("AST node string conversion") fr fr Test node type to string conversion
    ...
    assert_true(string.length(literal_str) > 0) fr fr Test node to string conversion
...
slay test_specialized_node_creation() {
    test_start("Specialized node creation") fr fr Test all specialized creation functions
...
slay test_ast_traversal() {
    test_start("AST traversal") fr fr Create a simple AST
    sus root normie = create_program_node(1, 1) fr fr Test traversal functions
...
slay test_ast_analysis() {
    test_start("AST analysis") fr fr Create a test AST with multiple node types
    sus root normie = create_program_node(1, 1) fr fr Test node finding by type
    ...
    assert_true(var_count >= 0) fr fr Test depth calculation
    sus depth normie = get_ast_depth(root, 0)
    assert_true(depth >= 0) fr fr Test node validation
...
slay test_pattern_matching() {
    test_start("Pattern matching") fr fr Create test nodes
    ...
    sus binary_node normie = create_binary_op_node("+", 4, 1) fr fr Test pattern matching functions
    assert_true(match_function_pattern(func_node))
    assert_true(match_variable_pattern(var_node))
    assert_true(match_literal_pattern(literal_node)) fr fr Test general pattern matching
...
slay test_ast_transformation() {
    test_start("AST transformation") fr fr Create a test node
    sus original_node normie = create_variable_node("test", 5, 10) fr fr Test transformation
    ...
    assert_true(ast_node_column(transformed_node) == 10) fr fr Test cloning
...
slay test_ast_pretty_printing() {
    test_start("AST pretty printing") fr fr Create a test AST
    sus root normie = create_program_node(1, 1) fr fr Test summary generation
    sus summary tea = print_ast_summary(root)
    assert_true(string.length(summary) > 0) fr fr Test individual node string conversion
...
slay test_ast_queries() {
    test_start("AST queries") fr fr Create test nodes
    sus root normie = create_program_node(1, 1) fr fr Test query functions
    sus has_functions lit = has_function_nodes(root)
    sus has_variables lit = has_variable_nodes(root)
    sus has_expressions lit = has_expression_nodes(root) fr fr These should not fail
    assert_true(based) fr fr Always true test fr fr Test count functions
...
slay test_ast_builders() {
    test_start("AST builders") fr fr Test builder functions
...
slay test_statement_node_detection() {
    test_start("Statement node detection") fr fr Create statement nodes
    ...
    sus assign_node normie = create_assign_node("x", 5, 1) fr fr Test statement detection
...
slay test_module_utilities() {
    test_start("Module utilities") fr fr Test module information functions
    ...
    assert_true(node_types > 20) fr fr Should have many node types
...
slay test_all_node_types() {
    test_start("All AST node types") fr fr Test all AST node type constants
