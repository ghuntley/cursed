fr fr CURSED RenderZ Module - Comprehensive Graphics Rendering Examples
fr fr Demonstrates professional 2D/3D graphics capabilities
...
slay demo_shader_programming() lit {
    ...
    fr fr Create and compile shader
    ...
    fr fr Use shader and set uniforms
...
fr fr ===== 3D MESH RENDERING EXAMPLE =====
...
slay demo_3d_mesh_rendering() lit {
    ...
    sus camera renderz.Camera = renderz_create_perspective_camera(
        renderz_vec3(0.0, 0.0, 5.0),  fr fr position
        renderz_vec3(0.0, 0.0, 0.0),  fr fr target
        45.0,                         fr fr fov
        16.0 / 9.0,                   fr fr aspect ratio
        0.1,                          fr fr near plane
        100.0                         fr fr far plane
    )
...
slay demo_render_targets() lit {
    ...
    fr fr Create render target for off-screen rendering
    ...
    fr fr Render to off-screen target
    ...
    fr fr Return to main framebuffer
    renderz_unbind_render_target()
    renderz_clear(context)
    
    fr fr Create full-screen quad to display render target
...
slay demo_performance_testing() lit {
    ...
    fr fr Create many meshes for stress testing
    ...
    fr fr Performance test rendering
...
slay main() normie {
    ...
    fr fr Enable debug output for development
...
slay mathz_cos(angle drip) drip {
    fr fr Mock implementation - would calculate cosine
    damn 1.0
}

slay mathz_sin(angle drip) drip {
    fr fr Mock implementation - would calculate sine
    damn 0.0
}

slay mathz_int_to_float(i normie) drip {
    fr fr Mock implementation - would convert int to float
    damn 42.0
}
