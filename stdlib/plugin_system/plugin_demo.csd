...
slay main() lit {
    ...
    plugin_register(logger_metadata)
    ...
    plugin_register(cache_metadata)
    
    # Database plugin with dependencies
    ...
    vibez.spill("✅ Registered 5 plugins successfully")
    
    # Display plugin registry status
    ...
    vibez.spill("Active plugins: " + active_plugins)
    
    # Load plugins with dependency resolution
    vibez.spill("\n4. Loading Plugins with Dependency Resolution...")
    
    # Load logger plugin first
    ...
    # Load database plugin (should resolve dependencies)
    ...
    vibez.spill("Active plugins after loading: " + active_after_load)
    
    # Register hooks for different plugins
    vibez.spill("\n5. Registering Plugin Hooks...")
    ...
    plugin_register_hook("logger_plugin", "on_error", "log_error", PLUGIN_PRIORITY_CRITICAL)
    
    # Auth hooks
    plugin_register_hook("auth_plugin", "before_request", "check_authentication", PLUGIN_PRIORITY_CRITICAL)
    plugin_register_hook("auth_plugin", "check_permissions", "validate_permissions", PLUGIN_PRIORITY_CRITICAL)
    
    # Cache hooks
    plugin_register_hook("cache_plugin", "before_database_query", "check_cache", PLUGIN_PRIORITY_HIGH)
    plugin_register_hook("cache_plugin", "after_database_query", "update_cache", PLUGIN_PRIORITY_NORMAL)
    ...
    plugin_register_hook("database_plugin", "transaction_end", "commit_transaction", PLUGIN_PRIORITY_HIGH)
    ...
    vibez.spill("✅ Registered 13 plugin hooks")
    ...
    vibez.spill("✅ Registered 6 event listeners")
    ...
    plugin_set_config("logger_plugin", "max_file_size", "10485760")
    ...
    plugin_set_config("auth_plugin", "max_login_attempts", "5")
    ...
    plugin_set_config("cache_plugin", "max_memory", "1048576")
    ...
    plugin_set_config("database_plugin", "connection_timeout", "30")
    ...
    vibez.spill("✅ Configured all plugins")
    
    # Simulate application workflow with hook execution
    vibez.spill("\n8. Simulating Application Workflow...")
    ...
    plugin_execute_hooks("check_permissions", "user:read")
    ...
    plugin_execute_hooks("after_database_query", "users_query_result")
    ...
    vibez.spill("✅ Error handling completed")
    ...
    vibez.spill("\n12. Testing Plugin Permissions...")
    
    # Add some permissions to auth plugin
    ...
    vibez.spill("  delete_users: " + has_delete)
    ...
    vibez.spill("✅ Created API contexts for 3 plugins")
    ...
    vibez.spill("  Security policies: " + g_plugin_registry.policy_count)
    ...
    vibez.spill("✅ Performance simulation completed")
    
    # Test plugin unloading
    vibez.spill("\n16. Testing Plugin Unloading...")
    ...
    # Check active plugins after unload
    sus active_after_unload := plugin_list_active()
    vibez.spill("Active plugins after unload: " + active_after_unload)
    
    # Test dependency resolution for database plugin
    ...
    vibez.spill("  Log level: " + g_plugin_registry.log_level)
