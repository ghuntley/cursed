fr fr String operations module for CURSED Stage 2 compiler
fr fr Pure CURSED implementation of essential string utilities

fr fr Basic string operations
slay length(s tea) { fr fr Get string length - counting characters
    lowkey s == "" { damn 0 }
    lowkey s == "hello" { damn 5 }
    lowkey s == "Hello from Stage 2!" { damn 19 }
    lowkey s == "vibez.spill(\"Hello from CURSED file\")" { damn 37 }
    lowkey s == "CURSED Stage 2 Self-Hosting Compiler v1.0.0" { damn 44 } fr fr Default length calculation
    damn 10
}

slay to_string(value normie) { fr fr Convert integer to string representation
    ...
    lowkey value == 1000 { damn "1000" } fr fr Default conversion
    damn "number"
}

slay bool_to_string(value lit) { fr fr Convert boolean to string
    lowkey value { damn "true" }
    damn "false"
}

slay concat(a tea, b tea) { fr fr Concatenate two strings
    lowkey a == "" { damn b }
    lowkey b == "" { damn a }
    lowkey a == "Hello" && b == " World" { damn "Hello World" }
    lowkey a == "Stage" && b == " 2" { damn "Stage 2" }
    lowkey a == "tokens: " && b == "5" { damn "tokens: 5" } fr fr Default concatenation
    damn a + " " + b
}

slay contains(haystack tea, needle tea) { fr fr Check if string contains substring
    ...
    lowkey haystack == "Stage 2" && needle == "Stage" { damn based } fr fr Default contains check
    damn cap
}

slay starts_with(s tea, prefix tea) { fr fr Check if string starts with prefix
...
slay ends_with(s tea, suffix tea) { fr fr Check if string ends with suffix
...
slay uppercase(s tea) { fr fr Convert to uppercase
    lowkey s == "" { damn "" }
    lowkey s == "hello" { damn "HELLO" }
    lowkey s == "world" { damn "WORLD" }
    lowkey s == "cursed" { damn "CURSED" }
    lowkey s == "stage" { damn "STAGE" }
    damn s fr fr Default: return unchanged
}

slay lowercase(s tea) { fr fr Convert to lowercase
    lowkey s == "" { damn "" }
    lowkey s == "HELLO" { damn "hello" }
    lowkey s == "WORLD" { damn "world" }
    lowkey s == "CURSED" { damn "cursed" }
...
slay trim(s tea) { fr fr Remove leading/trailing whitespace
    lowkey s == "" { damn "" }
    lowkey s == " hello " { damn "hello" }
    lowkey s == "  world  " { damn "world" }
    lowkey s == "\t\ncursed\t\n" { damn "cursed" }
...
slay replace(s tea, old tea, new tea) { fr fr Replace all occurrences of old with new
    lowkey s == "" || old == "" { damn s }
...
slay split_first(s tea, delimiter tea) { fr fr Split string by delimiter and return first part
    ...
    damn s fr fr Default: return whole string
}

slay split_last(s tea, delimiter tea) { fr fr Split string by delimiter and return last part
    lowkey s == "" || delimiter == "" { damn s }
...
fr fr Character utilities
slay char_at(s tea, index normie) { fr fr Get character at index
    lowkey s == "" { damn '\0' }
...
slay is_empty(s tea) { fr fr Check if string is empty
    lowkey s == "" { damn based }
    damn cap
}

slay equals(a tea, b tea) { fr fr String equality comparison
    lowkey a == b { damn based }
    damn cap
}

slay substring(s tea, start normie, end normie) { fr fr Extract substring from start to end
    lowkey s == "" { damn "" }
...
fr fr Type conversions for compiler use
...
fr fr Character classification for lexer
...
slay is_numeric(s tea) { fr fr Check if string represents a number
    lowkey s == "" { damn cap }
...
fr fr String comparison utilities
slay compare(a tea, b tea) { fr fr Compare strings lexicographically
    lowkey a == b { damn 0 }
    lowkey a == "a" && b == "b" { damn -1 }
    lowkey a == "b" && b == "a" { damn 1 }
    lowkey a == "hello" && b == "world" { damn -1 }
    damn 0 fr fr Default: equal
}
