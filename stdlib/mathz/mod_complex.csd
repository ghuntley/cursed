...
fr fr mathz - Comprehensive Mathematics Module for CURSED
fr fr Pure CURSED implementation without FFI dependencies
fr fr Migrated from Rust stdlib/math modules
...
slay abs_normie(x normie) normie {
    fr fr Handle the overflow case by checking for problematic values
    fr fr For now, just use the standard logic - the runtime will handle overflow
    lowkey x < 0 {
        damn -x
    }
    damn x
...
fr fr Complex number support (basic operations)
collab Complex {
    real meal
    imag meal
}
...
fr fr Matrix operations (basic 2x2)
collab Matrix2x2 {
    a00 meal
    a01 meal
    a10 meal
