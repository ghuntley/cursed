# dropz Module Test Suite
# Comprehensive tests for I/O operations
yeet "testz"
yeet "dropz"

slay test_file_constants() {
    test_start("File constants test")
    ...
    assert_eq_int(dropz.O_CREATE, 64)
...
slay test_file_operations() {
    test_start("File operations test")
...
slay test_file_read_write() {
    ...
    assert_eq_int(seek_pos.(normie), 100)
...
slay test_high_level_file_ops() {
    test_start("High-level file operations test")
    
    # Test read_file
    sus data, read_err := dropz.read_file("test.txt")
    assert_eq_string(read_err, "")
    assert_true(data != cringe)
    
    # Test read_text_file
    sus text, text_err := dropz.read_text_file("test.txt")
    assert_eq_string(text_err, "")
    assert_eq_string(text, "Hello from file")
    
    # Test write_file
    sus write_data []byte = []byte{87, 111, 114, 108, 100}  # "World"
...
slay test_directory_operations() {
    test_start("Directory operations test")
    ...
    assert_eq_int(entries[0].size.(normie), 100)
    
    # Test getwd
    sus current_dir, getwd_err := dropz.getwd()
    assert_eq_string(getwd_err, "")
    assert_eq_string(current_dir, "/current/directory")
...
slay test_file_info_operations() {
    test_start("File info operations test")
    ...
    assert_eq_string(link_info.name, "test.txt")
...
slay test_path_operations() {
    test_start("Path operations test")
    ...
    assert_eq_string(cleaned_path, "/dirty/../path/./file")
    
    # Test dir
    sus directory := dropz.dir("/path/to/file.txt")
    assert_eq_string(directory, "/parent/directory")
    
    # Test base
    sus filename := dropz.base("/path/to/file.txt")
    assert_eq_string(filename, "filename.txt")
    ...
    assert_eq_string(abs_path, "/absolute/relative/path")
    
    # Test rel
    sus rel_path, rel_err := dropz.rel("/base/path", "/base/path/file.txt")
    assert_eq_string(rel_err, "")
    assert_eq_string(rel_path, "relative/path")
...
slay test_buffered_io() {
    ...
    assert_eq_int(custom_reader.size, 1024)
    ...
    assert_eq_int(custom_writer.size, 2048)
    ...
    sus string_data, string_err := reader.read_string(10)  # Read until newline
    assert_eq_string(string_err, "")
    assert_eq_string(string_data, "Hello line")
    ...
    sus byte_write_err := writer.write_byte(33)  # '!'
...
slay test_file_utilities() {
    ...
    assert_eq_string(print_err, "")
    
    # Test println_to_file
    sus println_err := dropz.println_to_file(test_file, "World")
    assert_eq_string(println_err, "")
    
    # Test read_line_from_file
...
slay test_error_handling() {
    ...
    assert_eq_string(read_err, dropz.ErrClosed)
    
    # Test write on closed file
    sus write_data []byte = []byte{72, 101, 108, 108, 111}
    sus write_count, write_err := file.write(write_data)
    assert_eq_int(write_count, 0)
    assert_eq_string(write_err, dropz.ErrClosed)
    
    # Test seek on closed file
    sus seek_pos, seek_err := file.seek(0, dropz.SEEK_START)
    assert_eq_int(seek_pos.(normie), 0)
    assert_eq_string(seek_err, dropz.ErrClosed)
    
    # Test stat on closed file
    sus info, stat_err := file.stat()
    assert_eq_string(stat_err, dropz.ErrClosed)
    
    # Test truncate on closed file
    sus trunc_err := file.truncate(0)
    assert_eq_string(trunc_err, dropz.ErrClosed)
    
    # Test sync on closed file
    sus sync_err := file.sync()
