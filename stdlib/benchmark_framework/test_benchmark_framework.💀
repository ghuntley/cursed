...
fr fr Comprehensive tests for CURSED Benchmark Framework
fr fr Tests core benchmarking functionality

test_start("CURSED Benchmark Framework Tests")

fr fr Test framework initialization
test_start("Framework initialization tests")
sus init_result = init_benchmark_framework()
assert_true(init_result)

fr fr Test benchmark configuration
...
fr fr Test timestamp functionality
...
fr fr Test single benchmark iteration execution
test_start("Benchmark iteration execution tests")
sus test_function = slay() lit { fr fr Simple computation for testing
    sus sum normie = 0
    bestie i := 0; i < 100; i++ {
        sum = sum + i
    }
...
fr fr Test benchmark execution
test_start("Full benchmark execution tests")
sus benchmark_func = slay() lit { fr fr Simulate some work
    sus result normie = 0
    bestie i := 0; i < 50; i++ {
        result = result + (i * 2)
    }
...
fr fr Test result printing (should not crash)
test_start("Result printing tests")
print_benchmark_results()

fr fr Test benchmark comparison
test_start("Benchmark comparison tests")
compare_benchmark_results(1.0, 1.2) fr fr Should show regression
compare_benchmark_results(1.2, 1.0) fr fr Should show improvement

fr fr Test micro-benchmarking
...
fr fr Test built-in benchmarks
...
fr fr Test regression detection
test_start("Regression detection tests")
sus regression_result = detect_performance_regression("baseline.txt", 5.0)
fr fr Result can be either true or false, just testing it doesn't crash

fr fr Test all benchmarks execution
...
fr fr Test function that does nothing
...
fr fr Test benchmark with zero iterations (edge case)
...
fr fr Test multiple benchmark runs
