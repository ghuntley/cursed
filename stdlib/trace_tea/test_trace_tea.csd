...
slay test_basic_tracing() {
    test_start("basic tracing operations") fr fr Create a buffer for trace output
    sus buffer dropz.Buffer fr fr Start tracing
    err := trace_tea.Start(&buffer)
    assert_eq_string(err, "") fr fr Create a task
    ctx := vibe_context.Background()
    ctx, task := trace_tea.NewTask(ctx, "test-task")
    assert_not_nil(task) fr fr Log some events
    trace_tea.Log(ctx, "info", "test message")
    trace_tea.Logf(ctx, "debug", "formatted message: %d", 42) fr fr End the task
    task.End() fr fr Stop tracing
    err = trace_tea.Stop()
    assert_eq_string(err, "") fr fr Verify trace data was captured
...
slay test_span_management() {
    ...
    ctx := vibe_context.Background() fr fr Create a span
    span := trace_tea.StartSpan(ctx, "test-operation")
    assert_not_nil(span)
    assert_eq_string(span.operation_name, "test-operation")
    assert_false(span.ended) fr fr Set span tags
    span.SetTag("component", "test")
    span.SetTag("version", "1.0") fr fr Log fields
    fields := make(map[tea]interface{})
    fields["key1"] = "value1"
    fields["key2"] = 42
    span.LogFields(fields) fr fr Set baggage items
    span.SetBaggageItem("user-id", "12345")
    span.SetBaggageItem("session-id", "abcdef") fr fr Retrieve baggage
    user_id := span.GetBaggageItem("user-id")
    assert_eq_string(user_id, "12345") fr fr End the span
    span.End()
...
slay test_region_operations() {
    ...
    defer task.End() fr fr Create a region
    region := trace_tea.StartRegion(ctx, "processing")
    assert_not_nil(region)
    assert_eq_string(region.region_type, "processing")
    assert_false(region.ended) fr fr Log to region
    region.LazyLog("Processing step %d", 1) fr fr End region
    region.End()
    assert_true(region.ended) fr fr Test WithRegion helper
...
slay test_event_creation() {
    ...
    sus buffer dropz.Buffer
    trace_tea.Start(&buffer)
    defer trace_tea.Stop() fr fr Create events
    ...
    assert_true(event.timestamp > 0) fr fr Add logging to event
    event.LazyLog("Event details: %s", "test data") fr fr Verify tags
...
slay test_trace_context_propagation() {
    ...
    defer span.End() fr fr Create context with trace information
    span_ctx := vibe_context.WithValue(ctx, "span_id", span.id)
    span_ctx = vibe_context.WithValue(span_ctx, "trace_id", span.trace_id) fr fr Test injection into headers
    headers := make(map[tea]tea)
    trace_tea.InjectTraceContext(span_ctx, headers) fr fr Verify headers contain trace information
    assert_not_eq_string(headers["x-trace-id"], "")
    assert_not_eq_string(headers["x-span-id"], "") fr fr Test extraction from headers
    ...
    assert_eq_string(trace_ctx.span_id, headers["x-span-id"]) fr fr Test context creation from trace context
...
slay test_correlation_ids() {
    test_start("correlation ID management") fr fr Generate correlation ID
    correlation_id := trace_tea.GenerateCorrelationID()
    assert_not_eq_string(correlation_id, "") fr fr Set correlation ID in context
    ctx := vibe_context.Background()
    ctx = trace_tea.SetCorrelationID(ctx, correlation_id) fr fr Retrieve correlation ID from context
...
slay test_filtering() {
    test_start("trace filtering") fr fr Create filter
    filter := trace_tea.NewFilter()
    assert_not_nil(filter) fr fr Configure filter
    filter.IncludeGoroutine("worker-*")
    filter.ExcludeGoroutine("background-*")
    filter.IncludeEvent(trace_tea.EventAPI)
    filter.ExcludeEvent(trace_tea.EventGC) fr fr Verify filter configuration
...
slay test_real_time_analyzer() {
    test_start("real-time analyzer") fr fr Create analyzer
    analyzer := trace_tea.NewRealTimeAnalyzer()
    assert_not_nil(analyzer)
    assert_eq_int(analyzer.high_latency_threshold, 100) fr fr Configure high latency detection
    ...
    assert_not_nil(analyzer.on_high_latency) fr fr Test callback
...
slay test_sampling() {
    test_start("sampling strategies") fr fr Test sampling rate setting
    trace_tea.SetSamplingRate(0.5) fr fr Test sampling decisions
    ...
    } fr fr Should roughly be around 50% with 0.5 sampling rate fr fr Allow for some variance in randomness
...
slay test_metrics_extraction() {
    ...
    assert_not_nil(metrics.event_counts) fr fr Test average latency calculation with empty data
    avg_latency := metrics.AverageLatency("non-existent-operation")
    assert_eq_int(avg_latency, 0) fr fr Test max concurrency with empty data
...
slay test_external_exports() {
    ...
    assert_eq_string(err, "") fr fr Test OpenTelemetry export
...
slay test_distributed_tracing() {
    test_start("distributed tracing helpers") fr fr Create context with trace information
    ctx := vibe_context.Background()
    correlation_id := trace_tea.GenerateCorrelationID()
    ...
    span_ctx := vibe_context.WithValue(ctx, "span_id", span.id)
    span_ctx = vibe_context.WithValue(span_ctx, "trace_id", span.trace_id) fr fr Test propagation to downstream service
    downstream_headers := make(map[tea]tea)
    trace_tea.PropagateTrace(span_ctx, downstream_headers) fr fr Verify headers contain trace information
    assert_not_eq_string(downstream_headers["x-trace-id"], "")
    assert_not_eq_string(downstream_headers["x-span-id"], "")
    assert_eq_string(downstream_headers["x-correlation-id"], correlation_id) fr fr Test receiving trace from upstream service
    received_ctx := trace_tea.ReceiveTrace(downstream_headers)
    assert_not_nil(received_ctx) fr fr Verify correlation ID is preserved
...
slay test_auto_instrumentation() {
    ...
    sus buffer dropz.Buffer
    trace_tea.Start(&buffer)
    defer trace_tea.Stop() fr fr Enable auto-instrumentation for different components
    trace_tea.AutoInstrumentHTTP(based)
    trace_tea.AutoInstrumentDatabase(based)
    trace_tea.AutoInstrumentCache(based) fr fr These should log that auto-instrumentation is enabled fr fr In a real implementation, they would set up hooks
...
slay test_performance_monitoring() {
    ...
    sus buffer dropz.Buffer
    trace_tea.Start(&buffer)
    defer trace_tea.Stop() fr fr Enable various monitoring features
    trace_tea.MonitorGoroutines()
    trace_tea.MonitorMemory()
    trace_tea.MonitorNetworkActivity() fr fr These should log that monitoring is active
...
slay test_high_latency_detection() {
    ...
    sus buffer dropz.Buffer
    trace_tea.Start(&buffer) fr fr Set up analyzer with low threshold for testing
    analyzer := trace_tea.NewRealTimeAnalyzer()
    sus high_latency_detected lit = cap
    sus detected_operation tea = ""
    ...
    trace_tea.RegisterAnalyzer(analyzer) fr fr Create a span that will exceed the threshold
    ctx := vibe_context.Background()
    span := trace_tea.StartSpan(ctx, "slow-operation") fr fr Simulate work that takes longer than threshold
    timez.Sleep(20 * timez.Millisecond)
    
    span.End() fr fr Verify high latency was detected
    assert_true(high_latency_detected)
...
slay test_nested_spans() {
    ...
    ctx := vibe_context.Background() fr fr Create parent span
    parent_span := trace_tea.StartSpan(ctx, "parent-operation")
    parent_ctx := vibe_context.WithValue(ctx, "span_id", parent_span.id)
    parent_ctx = vibe_context.WithValue(parent_ctx, "trace_id", parent_span.trace_id) fr fr Create child span
    child_span := trace_tea.StartSpan(parent_ctx, "child-operation") fr fr Verify parent-child relationship
    assert_eq_string(child_span.trace_id, parent_span.trace_id)
    assert_eq_string(child_span.parent_id, parent_span.id) fr fr Add baggage to parent that should be inherited
...
slay test_concurrent_tracing() {
    ...
    sus buffer dropz.Buffer
    trace_tea.Start(&buffer)
    defer trace_tea.Stop() fr fr Create multiple goroutines that create spans
    done := make(chan lit, 3)
    
    for i := 0; i < 3; i++ {
        stan slay(worker_id normie) {
            ctx := vibe_context.Background()
            span := trace_tea.StartSpan(ctx, vibez.spill_to_tea("worker-%d", worker_id)) fr fr Simulate work
...
slay test_baggage_propagation() {
    ...
    ctx := vibe_context.Background()
    span := trace_tea.StartSpan(ctx, "baggage-test") fr fr Set baggage items
    span.SetBaggageItem("user-id", "user123")
    span.SetBaggageItem("tenant-id", "tenant456")
    span.SetBaggageItem("feature-flag", "new-ui") fr fr Retrieve baggage items
    user_id := span.GetBaggageItem("user-id")
...
slay test_complete_workflow() {
    ...
    sus buffer dropz.Buffer
    trace_tea.Start(&buffer) fr fr Create analyzer for monitoring
    analyzer := trace_tea.NewRealTimeAnalyzer()
    ...
    trace_tea.RegisterAnalyzer(analyzer) fr fr Simulate a complete application workflow
    ctx := vibe_context.Background()
    correlation_id := trace_tea.GenerateCorrelationID()
    ctx = trace_tea.SetCorrelationID(ctx, correlation_id) fr fr Main service operation
    ...
    main_span.SetBaggageItem("user-id", "12345") fr fr Authentication span
    auth_span := trace_tea.StartSpan(ctx, "authenticate")
    timez.Sleep(5 * timez.Millisecond)
    auth_span.SetTag("auth-method", "jwt")
    auth_span.End() fr fr Database operation (simulate slow query)
    db_span := trace_tea.StartSpan(ctx, "database-query")
    db_span.SetTag("query", "SELECT * FROM users")
    timez.Sleep(40 * timez.Millisecond) fr fr This should trigger high latency alert
    db_span.End() fr fr Response preparation
    ...
    trace_tea.Stop() fr fr Verify workflow completed and alerts were triggered
    assert_true(alerts_triggered >= 1) fr fr Database query should trigger alert
    assert_true(buffer.Len() > 0) fr fr Trace data should be captured
