# CURSED Cross-Platform Atomic Operations Test
# Validates hardware atomic operations across all target platforms

yeet "testz"
yeet "atomic_drip"

# Cross-platform atomic feature detection
slay test_platform_atomic_support() {
    test_start("platform_atomic_support")
    
    # Test basic atomic operations work on current platform
    sus atomic *AtomicI32 = atomic_i32_new(1)
    
    # Test compare-and-swap (fundamental atomic operation)
...
# Test x86_64 specific atomic optimizations
slay test_x86_64_atomics() {
    test_start("x86_64_atomics")
    
    # Test operations that benefit from x86_64 LOCK prefix
    sus atomic *AtomicI32 = atomic_i32_new(0)
    
    # Test atomic increment (LOCK INC on x86_64)
    bestie i := 0; i < 1000; i++ {
        atomic_increment_i32(atomic)
    }
    
    sus result normie = atomic_load_i32(atomic)
    assert_eq_int(result, 1000)
    
    # Test atomic decrement (LOCK DEC on x86_64)
    bestie i := 0; i < 500; i++ {
...
# Test ARM64 specific atomic optimizations
slay test_arm64_atomics() {
    test_start("arm64_atomics")
    
    # Test operations that benefit from ARM64 exclusive instructions
    sus atomic *AtomicI64 = atomic_i64_new(0)
    
    # Test large value operations (LDXR/STXR on ARM64)
    sus large_value thicc = 0x123456789ABCDEF0
    atomic_store_i64(atomic, large_value)
    sus loaded thicc = atomic_load_i64(atomic)
    assert_eq_int(loaded, large_value)
    
    # Test compare-and-swap with large values
...
# Test WASM atomic support
slay test_wasm_atomics() {
    test_start("wasm_atomics")
    
    # Test operations that map to WASM atomic instructions
    sus atomic *AtomicI32 = atomic_i32_new(42)
    
    # Test atomic RMW operations (i32.atomic.rmw.* in WASM)
    sus old_add normie = atomic_add_i32(atomic, 8)
    assert_eq_int(old_add, 42)
    
    sus old_and normie = atomic_and_i32(atomic, 0xFF)
    assert_eq_int(old_and, 50)  # 42 + 8 = 50
    
    sus old_or normie = atomic_or_i32(atomic, 0x100)
    assert_eq_int(old_or, 50)  # 50 & 0xFF = 50
    
    sus final normie = atomic_load_i32(atomic)
    assert_eq_int(final, 306)  # 50 | 0x100 = 306
    
    vibez.spill("✅ WASM atomic support test passed")
}

# Test memory ordering across platforms
slay test_cross_platform_memory_ordering() {
    ...
    atomic_store_i32(atomic2, 2)  # Defaults to SEQ_CST
    memory_fence()
...
slay test_weak_vs_strong_cas() {
    ...
    assert_eq_int(strong_result, 200)
    
    # Weak CAS - may fail spuriously, so loop until success
...
# Test atomic flag synchronization patterns
slay test_atomic_flag_patterns() {
    ...
    assert_eq_int(critical_work, 2)
    
    # Test ordered flag operations
...
slay test_atomic_pointer_operations() {
    test_start("atomic_pointer_operations")
    
    # Create test pointers using integer addresses
    ...
    # Test atomic pointer load/store
    ...
    assert_eq_int(stored.(thicc), addr2)
    
    # Test atomic pointer CAS
    ...
    assert_eq_int(cas_result.(thicc), addr3)
    
    # Test atomic pointer swap
...
# Test high-level synchronization primitives
slay test_synchronization_primitives() {
    ...
    assert_false(try_fail)  # Should fail when already locked
    spinlock_unlock(lock)
    
    sus try_success lit = spinlock_try_lock(lock)
    assert_true(try_success)  # Should succeed when unlocked
    spinlock_unlock(lock)
...
slay test_memory_fence_guarantees() {
    ...
    # Test that fences prevent reordering
    atomic_store_i32_ordered(atomic1, 1, MEMORY_ORDER_RELAXED)
    release_fence()  # Prevents later operations from moving before this point
    atomic_store_i32_ordered(atomic2, 2, MEMORY_ORDER_RELAXED)
    
    acquire_fence()  # Prevents earlier operations from moving after this point
    ...
    # Test acquire-release fence
    acq_rel_fence()
    
    # Test compiler fence (should not crash)
    compiler_fence()
...
slay main() {
    ...
    # Platform detection and support tests
    test_platform_atomic_support()
    ...
    test_atomic_performance_characteristics()
    
    # Advanced atomic operation tests
    test_weak_vs_strong_cas()
    test_atomic_flag_patterns()
    test_atomic_pointer_operations()
    
    # High-level synchronization tests
