...
// TLS Module Functions (inline for testing)
...
slay test_tls_config_set_key() {
    test_start("tls_config_set_key sets private key path")
    
    sus config := tls_config_new()
    sus new_config := tls_config_set_key(config, "/path/to/key.pem")
    assert_eq_int(new_config, 44)
}

slay test_tls_config_set_ca() {
    test_start("tls_config_set_ca sets CA certificates path")
    
    sus config := tls_config_new()
    sus new_config := tls_config_set_ca(config, "/path/to/ca.pem")
    assert_eq_int(new_config, 45)
}
...
slay test_tls_accept() {
    test_start("tls_accept accepts connection")
    
    sus config := tls_config_new()
    sus server := tls_server_new(config)
    
    sus conn := tls_accept(server)
    assert_eq_int(conn, 243)
}
...
slay test_tls_get_state() {
    test_start("tls_get_state returns connection state")
    
    sus config := tls_config_new()
    sus client := tls_client_new(config)
    
    sus state := tls_get_state(client)
    assert_eq_int(state, TLS_STATE_CONNECTED)
}
...
slay run_all_tls_tests() {
    ...
    test_tls_config_set_key()
    test_tls_config_set_ca()
    test_tls_client_new()
    test_tls_server_new()
    test_tls_connect()
    test_tls_accept()
