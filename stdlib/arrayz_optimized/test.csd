...
sus arr1 []drip = create_optimized_array(1000)
assert_eq_int(len(arr1), 1000)

sus arr2 []drip = create_array_with_capacity(500)
assert_eq_int(capacity(arr2), 500)

// Test SIMD-optimized operations
sus numbers []drip = [1, 2, 3, 4, 5, 6, 7, 8]
sus doubled []drip = simd_map_multiply(numbers, 2)
...
sus large_array []drip = range(0, 10000)
...
sus sorted []drip = cache_optimized_sort(unsorted)
assert_eq_int(sorted[0], 1)
assert_eq_int(sorted[5], 9)

// Test memory-efficient operations
sus filtered []drip = inplace_filter(large_array, slay(x drip) lit { damn x % 2 == 0 })
assert_true(len(filtered) == 5000)

// Test bulk operations
sus batch_data []drip = range(0, 1000)
...
sus aligned_array []drip = create_aligned_array(1024, 64)
assert_true(is_memory_aligned(aligned_array, 64))

// Performance benchmarks
sus perf_array []drip = range(0, 100000)
