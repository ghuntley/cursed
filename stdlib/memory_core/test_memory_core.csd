...
# Test 3: Memory object retrieval
...
assert_eq_int(stats["heap_size"], 2560)  # 5 * 512
print_test_status("Multiple allocations", stats["live_objects"] == 5)

# Test 7: Reference counting auto-deallocation
sus auto_id normie = allocate_memory(256, ALLOC_HEAP)
inc_ref_count(auto_id)  # ref_count = 2
dec_ref_count(auto_id)  # ref_count = 1
assert_true(memory_object_exists(auto_id))

dec_ref_count(auto_id)  # ref_count = 0, should auto-deallocate
assert_true(!memory_object_exists(auto_id))
print_test_status("Auto-deallocation", based)

# Test 8: GC threshold and triggering
reset_memory_system()
init_memory_system()
set_gc_enabled(based)

# Set low threshold to trigger GC
...
# Test 9: Manual garbage collection
reset_memory_system()
init_memory_system()

# Create objects with zero ref count
sus gc_id1 normie = allocate_memory(512, ALLOC_HEAP)
sus gc_id2 normie = allocate_memory(512, ALLOC_HEAP)
dec_ref_count(gc_id1)  # Make unreachable
dec_ref_count(gc_id2)  # Make unreachable
...
# Objects should be collected during GC
...
# Simulate high memory usage by adjusting heap size
gc_stats.heap_size = MAX_HEAP_SIZE * 85 / 100  # 85% usage
...
gc_stats.heap_size = MAX_HEAP_SIZE * 95 / 100  # 95% usage
...
# Test 15: Memory allocation types
reset_memory_system()
init_memory_system()
