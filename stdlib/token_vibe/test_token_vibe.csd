...
fr fr Comprehensive test suite for token_vibe module
fr fr Essential for validating lexical analysis in self-hosting compiler

fr fr =============================================================================
fr fr Token Type and Constant Tests
fr fr =============================================================================

slay test_token_constants() {
    test_start("Token type constants") fr fr Core token types
    ...
    assert_eq_int(CHAR_TOKEN, 5) fr fr Operators
...
slay test_token_string_conversion() {
    test_start("Token string conversion") fr fr Test token to string conversion
...
fr fr =============================================================================
fr fr Position Tracking Tests
fr fr =============================================================================

slay test_position_functions() {
    test_start("Position tracking functions") fr fr Test position creation
    sus pos normie = create_position("test.csd", 10, 25, 150)
    assert_true(position_is_valid(pos)) fr fr Test position components
    ...
    assert_true(offset >= 0) fr fr Test position string representation
...
slay test_position_edge_cases() {
    test_start("Position edge cases") fr fr Test zero position
    sus zero_pos normie = create_position("", 0, 0, 0)
    assert_true(!position_is_valid(zero_pos)) fr fr Test large position values
...
fr fr =============================================================================
fr fr Token Info Structure Tests
fr fr =============================================================================
...
slay test_token_info_creation() {
    ...
    sus token_info normie = create_token_info(IDENT_TOKEN, "variable", pos, "variable") fr fr Test token type extraction
    sus type normie = token_type(token_info)
    assert_eq_int(type, IDENT_TOKEN) fr fr Test token value hash
    sus value_hash normie = token_value_hash(token_info)
    assert_true(value_hash > 0) fr fr Test position reference
...
slay test_token_classification() {
    ...
    sus pos normie = create_position("test.csd", 1, 1, 0) fr fr Test identifier classification
    ...
    assert_true(!is_keyword(ident_token)) fr fr Test number classification
...
fr fr =============================================================================
fr fr Character Classification Tests
fr fr =============================================================================

slay test_character_classification() {
    test_start("Character classification functions") fr fr Test letter classification
    assert_true(is_letter('a'))
    assert_true(is_letter('Z'))
    assert_true(is_letter('_'))
    assert_true(!is_letter('5'))
    assert_true(!is_letter(' ')) fr fr Test digit classification
    assert_true(is_digit('0'))
    assert_true(is_digit('9'))
    assert_true(!is_digit('a'))
    assert_true(!is_digit(' ')) fr fr Test alphanumeric classification
    assert_true(is_alphanumeric('a'))
    assert_true(is_alphanumeric('5'))
    assert_true(is_alphanumeric('_'))
    assert_true(!is_alphanumeric(' '))
    assert_true(!is_alphanumeric('@')) fr fr Test whitespace classification
    assert_true(is_whitespace(' '))
    assert_true(is_whitespace('\t'))
    assert_true(!is_whitespace('a'))
    assert_true(!is_whitespace('\n')) fr fr Test newline classification
    assert_true(is_newline('\n'))
    assert_true(!is_newline(' '))
    assert_true(!is_newline('a')) fr fr Test hex digit classification
...
fr fr =============================================================================
fr fr Scanner State Machine Tests
fr fr =============================================================================
...
slay test_scanner_creation() {
    ...
    assert_true(scanner > 0) fr fr Test initial scanner state
...
slay test_scanner_advancement() {
    ...
    sus scanner normie = create_scanner("test") fr fr Advance with regular character
    ...
    assert_true(new_col > scanner_column(scanner)) fr fr Advance with newline
...
fr fr =============================================================================
fr fr Keyword Recognition Tests
fr fr =============================================================================

slay test_keyword_recognition() {
    test_start("Keyword recognition") fr fr Test CURSED-specific keywords
    ...
    assert_eq_int(recognize_keyword("defer"), DEFER_TOKEN) fr fr Test type keywords
    ...
    assert_eq_int(recognize_keyword("smol"), SMOL_TOKEN) fr fr Test non-keywords return IDENT_TOKEN
...
slay test_basic_tokenization() {
    test_start("Basic tokenization") fr fr Test simple expression
    sus source1 tea = "sus x normie = 42"
    sus count1 normie = tokenize(source1)
    assert_true(count1 > 0) fr fr Test expression with operators
    sus source2 tea = "x + y * 2"
    sus count2 normie = tokenize(source2)
    assert_true(count2 > 0) fr fr Test function definition
...
slay test_string_tokenization() {
    test_start("String tokenization") fr fr Test string literals
    sus source1 tea = "\"hello world\""
    sus count1 normie = tokenize(source1)
    assert_true(count1 > 0) fr fr Test string with escapes
    sus source2 tea = "\"hello\\nworld\""
    sus count2 normie = tokenize(source2)
    assert_true(count2 > 0) fr fr Test character literals
    sus source3 tea = "'a'"
    sus count3 normie = tokenize(source3)
    assert_true(count3 > 0)
...
slay test_number_tokenization() {
    test_start("Number tokenization") fr fr Test integers
    sus source1 tea = "123"
    sus count1 normie = tokenize(source1)
    assert_true(count1 > 0) fr fr Test floats
    sus source2 tea = "3.14159"
    sus count2 normie = tokenize(source2)
    assert_true(count2 > 0) fr fr Test mixed numbers
...
slay test_comment_tokenization() {
    test_start("Comment tokenization") fr fr Test line comments
    sus source1 tea = "# This is a comment"
    sus count1 normie = tokenize(source1)
    assert_true(count1 > 0) fr fr Test code with comments
    sus source2 tea = "sus x normie = 42 fr fr Variable declaration"
    sus count2 normie = tokenize(source2)
...
slay test_complex_tokenization() {
    test_start("Complex tokenization scenarios") fr fr Test complete function
    sus source tea = "slay fibonacci(n normie) normie {\n    lowkey n <= 1 {\n        damn n\n    }\n    damn fibonacci(n-1) + fibonacci(n-2)\n}"
    sus count normie = tokenize(source)
    assert_true(count > 10) fr fr Should have many tokens
    
    print_test_summary()
}

fr fr =============================================================================
fr fr Token Stream Tests
fr fr =============================================================================
...
slay test_error_recovery() {
    ...
    assert_true(string.contains(error_msg, "error")) fr fr Test error recovery
...
slay test_invalid_tokens() {
    test_start("Invalid token handling") fr fr Test unterminated string
    sus source1 tea = "\"unterminated string"
    sus count1 normie = tokenize(source1)
    assert_true(count1 >= 0) fr fr Should handle gracefully fr fr Test invalid character sequence
...
slay test_utility_functions() {
    test_start("Utility functions") fr fr Test string hashing
    sus hash1 normie = hash_string("test")
    sus hash2 normie = hash_string("test")
    sus hash3 normie = hash_string("different")
    
    assert_eq_int(hash1, hash2) fr fr Same string should have same hash
    assert_true(hash1 != hash3) fr fr Different strings should have different hashes fr fr Test token value extraction
...
fr fr =============================================================================
fr fr Operator and Delimiter Classification Tests
fr fr =============================================================================
...
slay test_large_input_tokenization() {
    test_start("Large input tokenization") fr fr Create a reasonably large input
    ...
    assert_true(count > 20) fr fr Should tokenize successfully
    
    print_test_summary()
}

slay test_edge_case_inputs() {
    test_start("Edge case inputs") fr fr Test empty input
    sus empty_count normie = tokenize("")
    assert_eq_int(empty_count, 0) fr fr Test whitespace only
    sus whitespace_count normie = tokenize("   \t  \n  ")
    assert_eq_int(whitespace_count, 0) fr fr Test single character
