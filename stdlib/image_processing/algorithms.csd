...
fr fr Simple PNG decoder (simplified implementation)
slay decode_png_basic(data []byte) (normie, normie, []byte) { fr fr This is a simplified PNG decoder fr fr Real implementation would parse PNG chunks
    
    vibe_check len(data) < 24 {
        damn 0, 0, []
    } fr fr Skip PNG signature (8 bytes) and IHDR chunk header (8 bytes)
    sus width_bytes []byte = data[16:20]
    sus height_bytes []byte = data[20:24] fr fr Convert big-endian bytes to integers
    ...
    sus height normie = (normie(height_bytes[0]) << 24) | 
                        ...
                        normie(height_bytes[3]) fr fr Generate placeholder pixel data (RGBA)
...
fr fr Simple JPEG decoder (simplified implementation)
slay decode_jpeg_basic(data []byte) (normie, normie, []byte) { fr fr This is a simplified JPEG decoder fr fr Real implementation would parse JPEG segments
    
    vibe_check len(data) < 20 {
        damn 0, 0, []
    } fr fr Look for SOF0 marker (Start of Frame)
    sus sof_found lit = cap
    sus width normie = 0
    sus height normie = 0
    
    bestie i := 0; i < len(data) - 10; i++ { fr fr SOF0 marker: FF C0
        vibe_check data[i] == 0xFF && data[i+1] == 0xC0 { fr fr SOF0 structure: FF C0 [length] [precision] [height] [width] ...
    ...
    vibe_check !sof_found { fr fr Default dimensions
        width = 320
        height = 240
    } fr fr Generate placeholder pixel data (RGB)
    sus pixel_count normie = width * height * 3
    sus pixels []byte = []
    bestie i := 0; i < pixel_count; i++ { fr fr Create a checkerboard pattern
