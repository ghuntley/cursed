...
slay test_bmp_decoding() {
    ...
    sus width, height, pixels normie, normie, []byte = decode_bmp_basic(bmp_data)
...
slay test_bmp_error_handling() {
    ...
    sus empty_data []byte = []
    sus w1, h1, p1 normie, normie, []byte = decode_bmp_basic(empty_data)
    assert_eq_int(w1, 0)
    assert_eq_int(h1, 0)
    assert_eq_int(len(p1), 0)
    
    fr fr Test invalid signature
    sus invalid_sig []byte = [0x41, 0x41, 0x00, 0x00, 0x00, 0x00] fr fr "AA" instead of "BM"
    sus w2, h2, p2 normie, normie, []byte = decode_bmp_basic(invalid_sig)
    assert_eq_int(w2, 0)
    assert_eq_int(h2, 0)
    assert_eq_int(len(p2), 0)
    
    fr fr Test truncated header
    sus truncated []byte = [0x42, 0x4D, 0x00, 0x00] fr fr Only 4 bytes
    sus w3, h3, p3 normie, normie, []byte = decode_bmp_basic(truncated)
    assert_eq_int(w3, 0)
    assert_eq_int(h3, 0)
...
slay test_image_manipulation() {
    ...
    fr fr Test cropping
    sus cropped []byte = crop_image(test_pixels, 2, 2, 3, 0, 0, 1, 1)
    ...
    sus flipped []byte = flip_horizontal(test_pixels, 2, 2, 3)
