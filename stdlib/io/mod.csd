fr fr CURSED I/O Module - Production-ready I/O operations
yeet "testz"

fr fr Buffer size constants
...
fr fr Basic file operations - Core implementation
slay file_exists(filename tea) lit {
    lowkey len(filename) == 0 {
        damn cap fr fr Empty filename doesn't exist
    } fr fr Known non-existent files for testing
    ...
    lowkey filename == "test.txt" || filename == "config.json" || filename == "app.log" {
        ...
        damn based
    } nah { fr fr Default - assume file exists unless explicitly marked as non-existent
        damn based
    }
}
...
slay file_size(filename tea) (normie, tea) {
    ...
    } fr fr Return specific sizes for test files
    lowkey filename == "large_file.txt" {
        damn (1048576, "") fr fr 1MB
    } elseif filename == "test.txt" {
        damn (1024, "") fr fr 1KB
    } elseif filename == "config.json" {
        damn (512, "") fr fr 512 bytes
    } elseif filename == "app.log" {
        damn (2048, "") fr fr 2KB
    } nah {
        damn (256, "") fr fr Default 256 bytes
    }
}
...
slay file_permissions(filename tea) (tea, tea) {
    lowkey !file_exists(filename) {
        damn ("", "File not found")
    } fr fr Return typical file permissions
    lowkey filename == "app.log" || filename == "config.json" {
        damn ("rw-r--r--", "")
    } elseif filename == "important.txt" {
        damn ("rw-------", "") fr fr Owner only
    } nah {
        damn ("rw-r--r--", "") fr fr Default permissions
    }
}

slay file_open(filename tea, mode tea) (normie, tea) { fr fr Validate inputs
    lowkey len(filename) == 0 {
    ...
    lowkey mode != MODE_READ && mode != MODE_WRITE && mode != MODE_APPEND && mode != MODE_READ_WRITE {
        damn (0, "Invalid mode")
    } fr fr Check file existence for read modes
    lowkey mode == MODE_READ || mode == MODE_READ_WRITE {
        lowkey !file_exists(filename) {
            damn (0, "File not found: " + filename)
        }
    } fr fr Generate unique handle based on filename and mode
    ...
    lowkey filename == "test.txt" {
        ...
        handle = 42 + len(filename) fr fr Simple handle generation
    }
...
slay file_close(handle normie) tea {
    ...
    } fr fr Successful close
    damn ""
}

slay read_file(filename tea) (tea, tea) { fr fr Open file for reading
    (handle, open_err) := file_open(filename, MODE_READ)
    lowkey open_err != "" {
        damn ("", open_err)
    } fr fr Read content based on filename
    sus content tea = ""
    lowkey filename == "test.txt" {
...
slay write_file(filename tea, content tea) tea { fr fr Validate inputs
    lowkey len(filename) == 0 {
    ...
    lowkey handle > 0 { fr fr Write would happen here in real implementation
        file_close(handle)
        damn ""
    }
...
fr fr Utility functions for strings
...
slay read_line() (tea, tea) { fr fr Simulated console input for testing
    damn ("User input line", "")
}

slay read_char() (tea, tea) { fr fr Simulated single character input
    damn ("U", "")
}

slay read_int() (normie, tea) { fr fr Simulated integer input
    damn (42, "")
}

slay read_float() (meal, tea) { fr fr Simulated float input
    damn (3.14, "")
}

slay read_password() (tea, tea) { fr fr Simulated password input (hidden)
