...
slay test_console_io() {
    testz.test_start("Console I/O Functions")
...
slay test_file_operations() {
    ...
    testz.assert_true(file_exists(test_file))
    testz.assert_true(is_file(test_file))
    testz.assert_false(is_directory(test_file))
...
slay test_file_copy_move() {
    ...
    testz.assert_true(write_file(source_file, test_content))
    
    fr fr Test file copy
    testz.assert_true(copy_file(source_file, dest_file))
    testz.assert_true(file_exists(source_file))
    testz.assert_true(file_exists(dest_file))
    testz.assert_eq_string(read_file(dest_file), test_content)
    
    fr fr Test file move
    testz.assert_true(move_file(dest_file, move_file))
    testz.assert_false(file_exists(dest_file))
    testz.assert_true(file_exists(move_file))
    testz.assert_eq_string(read_file(move_file), test_content)
    
    fr fr Clean up
    testz.assert_true(delete_file(source_file))
    testz.assert_true(delete_file(move_file))
}

slay test_binary_file_operations() {
    testz.test_start("Binary File Operations")
    ...
    testz.assert_true(write_file_bytes(binary_file, test_bytes))
    
    fr fr Read binary data
    sus read_bytes [byte] = read_file_bytes(binary_file)
    testz.assert_eq_int(len(read_bytes), len(test_bytes))
    
    fr fr Compare binary data
    for i in range(len(test_bytes)) {
        testz.assert_eq_int(read_bytes[i], test_bytes[i])
    }
    
    fr fr Clean up
    testz.assert_true(delete_file(binary_file))
}
...
slay test_directory_operations() {
    ...
    testz.assert_true(create_directory(test_dir))
    testz.assert_true(file_exists(test_dir))
    testz.assert_true(is_directory(test_dir))
    testz.assert_false(is_file(test_dir))
    
    fr fr Test directory listing
    sus current_dir tea = current_directory()
    testz.assert_true(string_len(current_dir) > 0)
    
    sus dir_contents [tea] = list_directory(".")
    testz.assert_true(len(dir_contents) > 0)
    
    fr fr Test recursive directory creation
    sus nested_dir tea = path_join([test_dir, "nested", "deep"])
    testz.assert_true(create_directory_recursive(nested_dir))
    testz.assert_true(is_directory(nested_dir))
    
    fr fr Test recursive directory listing
    sus recursive_contents [tea] = list_directory_recursive(test_dir)
    testz.assert_true(len(recursive_contents) > 0)
    
    fr fr Clean up
    testz.assert_true(remove_directory_recursive(test_dir))
    testz.assert_false(file_exists(test_dir))
}

slay test_path_operations() {
    testz.test_start("Path Operations")
    
    fr fr Test path joining
    sus joined_path tea = path_join(["home", "user", "documents", "file.txt"])
    testz.assert_true(string_contains(joined_path, "home"))
    testz.assert_true(string_contains(joined_path, "user"))
    testz.assert_true(string_contains(joined_path, "documents"))
    testz.assert_true(string_contains(joined_path, "file.txt"))
    
    fr fr Test path components
    sus test_path tea = "/home/user/documents/file.txt"
    testz.assert_eq_string(path_dirname(test_path), "/home/user/documents")
    testz.assert_eq_string(path_basename(test_path), "file.txt")
    testz.assert_eq_string(path_extension(test_path), ".txt")
    
    fr fr Test path absolute
    sus relative_path tea = "test.txt"
    sus absolute_path tea = path_absolute(relative_path)
    testz.assert_true(string_len(absolute_path) > string_len(relative_path))
    
    fr fr Test path existence
    testz.assert_true(path_exists("."))
    testz.assert_false(path_exists("nonexistent_path_xyz"))
}

slay test_stream_io() {
    testz.test_start("Stream I/O Operations")
    ...
    testz.assert_true(write_handle != cringe)
    testz.assert_true(write_to_file(write_handle, test_data))
    testz.assert_true(flush_file(write_handle))
    testz.assert_true(close_file(write_handle))
    
    fr fr Read using stream
    sus read_handle file_handle = open_file_read(stream_file)
    testz.assert_true(read_handle != cringe)
    
    sus read_data tea = read_from_file(read_handle, string_len(test_data))
    testz.assert_eq_string(read_data, test_data)
    
    fr fr Test file position
    testz.assert_true(seek_file(read_handle, 0))
    testz.assert_eq_int(tell_file(read_handle), 0)
    
    testz.assert_true(close_file(read_handle))
    
    fr fr Clean up
    testz.assert_true(delete_file(stream_file))
}

slay test_buffered_io() {
    testz.test_start("Buffered I/O Operations")
    
    fr fr Test buffer creation
    sus buffer_size normie = 1024
    sus buf buffer = create_buffer(buffer_size)
    testz.assert_true(buf != cringe)
    
    fr fr Test buffer operations
    sus test_data tea = "Buffered I/O test data"
    testz.assert_true(buffer_write(buf, test_data))
    testz.assert_true(buffer_size(buf) > 0)
    testz.assert_true(buffer_available(buf) > 0)
    ...
    testz.assert_true(buffer_flush(buf))
    testz.assert_true(buffer_clear(buf))
    testz.assert_eq_int(buffer_size(buf), 0)
}

slay test_temporary_files() {
    testz.test_start("Temporary File Operations")
    
    fr fr Test temporary file creation
    sus temp_file tea = create_temp_file()
    testz.assert_true(string_len(temp_file) > 0)
    testz.assert_true(file_exists(temp_file))
    
    fr fr Test writing to temporary file
    sus temp_content tea = "Temporary file content"
    testz.assert_true(write_file(temp_file, temp_content))
    testz.assert_eq_string(read_file(temp_file), temp_content)
    
    fr fr Test temporary directory
    sus temp_dir tea = create_temp_directory()
    testz.assert_true(string_len(temp_dir) > 0)
    testz.assert_true(is_directory(temp_dir))
    
    fr fr Test temp directory path
    sus system_temp_dir tea = temp_directory()
    testz.assert_true(string_len(system_temp_dir) > 0)
    testz.assert_true(is_directory(system_temp_dir))
    
    fr fr Clean up
    testz.assert_true(delete_file(temp_file))
    testz.assert_true(remove_directory(temp_dir))
}

slay test_file_timestamps() {
    testz.test_start("File Timestamp Operations")
    
    fr fr Create test file
    sus timestamp_file tea = "timestamp_test.txt"
    sus test_content tea = "Test file for timestamps"
    testz.assert_true(write_file(timestamp_file, test_content))
    ...
    testz.assert_true(modified_time > 0)
    testz.assert_true(created_time > 0)
    
    fr fr Modify file and check timestamp change
    testz.assert_true(append_file(timestamp_file, " Modified"))
    sus new_modified_time normie = file_modified_time(timestamp_file)
    testz.assert_true(new_modified_time >= modified_time)
    
    fr fr Clean up
    testz.assert_true(delete_file(timestamp_file))
}

slay test_io_edge_cases() {
    testz.test_start("I/O Edge Cases")
    
    fr fr Test operations on non-existent files
    testz.assert_false(file_exists("nonexistent_file.txt"))
    testz.assert_false(delete_file("nonexistent_file.txt"))
    
    fr fr Test empty file operations
    sus empty_file tea = "empty_test.txt"
    testz.assert_true(write_file(empty_file, ""))
    testz.assert_eq_int(file_size(empty_file), 0)
    testz.assert_eq_string(read_file(empty_file), "")
    testz.assert_true(delete_file(empty_file))
    
    fr fr Test path operations with empty strings
    testz.assert_eq_string(path_dirname(""), "")
    testz.assert_eq_string(path_basename(""), "")
    testz.assert_eq_string(path_extension(""), "")
    
    fr fr Test invalid path operations
    testz.assert_false(is_file(""))
    testz.assert_false(is_directory(""))
    testz.assert_false(path_exists(""))
}

slay test_io_error_handling() {
    testz.test_start("I/O Error Handling")
    ...
    testz.assert_false(close_file(invalid_handle))
    testz.assert_false(flush_file(invalid_handle))
}
