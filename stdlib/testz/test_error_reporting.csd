...
fr fr Test error reporting capabilities of the enhanced testz framework

fr fr ===============================
fr fr Error Reporting Test Suite
fr fr ===============================
...
slay test_assertion_failures() {
    ...
    testz.set_test_suite("Error Reporting Tests") fr fr Test 1: Basic assertion failure
    testz.test_start("Basic assertion failure test")
    vibez.spill("  🔍 Testing basic assertion failure...")
    testz.assert_true(cap) fr fr This should fail
    testz.test_end() fr fr Test 2: String comparison failure
    ...
    testz.test_end() fr fr Test 3: Integer comparison failure
    ...
    testz.test_end() fr fr Test 4: Range assertion failure
    testz.test_start("Range assertion failure test")
    vibez.spill("  🔍 Testing range assertion failure...")
    testz.assert_range_int(150, 0, 100) fr fr This should fail
    testz.test_end() fr fr Test 5: Enhanced context assertion failure
...
slay test_detailed_error_reporting() {
    ...
    testz.set_test_suite("Detailed Error Reporting Tests") fr fr Test 1: Create detailed error report
    ...
    testz.assert_true(based) fr fr This passes to show contrast
    testz.test_end() fr fr Test 2: Error with stack trace
    ...
    testz.assert_true(based) fr fr This passes to show contrast
    testz.test_end() fr fr Test 3: Multiple error context
...
slay test_property_test_failures() {
    ...
    testz.set_test_suite("Property Test Failure Reporting") fr fr Test 1: Property test with intentional failure
    ...
    bestie i := 0; i < 20; i++ {
        ...
        sus b normie = testz.random_int(1, 100) fr fr Intentional failure condition
        fr fr i == 10 {
            testz.property_test_fail("Intentional failure at iteration " + tea(i))
        } fr fr Test property (this should pass for other iterations)
        fr fr (a + b) != (b + a) {
...
slay test_timeout_failure_reporting() {
    ...
    testz.set_test_suite("Timeout Failure Reporting") fr fr Test 1: Timeout failure
    testz.test_start("Timeout failure test")
    enhanced_testz.run_test_with_timeout("timeout_test", 1) fr fr Very short timeout
    testz.test_end() fr fr Test 2: Retry failure
    testz.test_start("Retry failure test")
    enhanced_testz.run_test_with_retry("retry_test", 2) fr fr Will fail after retries
    testz.test_end()
}
...
slay test_benchmark_failure_reporting() {
    ...
    testz.set_test_suite("Benchmark Failure Reporting") fr fr Test 1: Performance validation failure
    ...
    testz.test_end() fr fr Test 2: Approximation failure
    testz.test_start("Approximation failure test")
    enhanced_testz.assert_approximately_equal(100, 200, 50) fr fr This should fail
    testz.test_end()
}
...
slay test_array_and_pattern_failures() {
    ...
    testz.set_test_suite("Array and Pattern Failure Reporting") fr fr Test 1: Array comparison failure
    testz.test_start("Array comparison failure")
    enhanced_testz.assert_array_equals("[1,2,3]", "[1,2,4]") fr fr This should fail
    testz.test_end() fr fr Test 2: Pattern matching failure
    testz.test_start("Pattern matching failure")
    enhanced_testz.assert_matches_pattern("hello world", "xyz") fr fr This should fail
    testz.test_end() fr fr Test 3: Between assertion failure
    testz.test_start("Between assertion failure")
    enhanced_testz.assert_between(150, 0, 100) fr fr This should fail
    testz.test_end()
}
...
slay test_error_report_generation() {
    ...
    testz.set_test_suite("Error Report Generation") fr fr Test 1: Generate reports with errors
    testz.test_start("Error report generation") fr fr Create some failures first
    testz.assert_true(cap) fr fr Failure
    testz.assert_eq_string("expected", "actual") fr fr Failure
    testz.assert_eq_int(42, 24) fr fr Failure fr fr Generate reports
...
slay main_character() {
    ...
    vibez.spill("================================================") fr fr Run all error reporting tests
    ...
    vibez.spill("================================================") fr fr Final summary
