...
assert_eq_int(len(tokens), 5) fr fr sus, x, :=, 42, EOF
...
sus tokens7 := tokenize("sus x := 42 fr fr this is a comment")
...
fr fr Test parser helper functions
...
fr fr Test simple expression parsing
...
fr fr Test binary expression parsing
...
fr fr Test unary expression parsing
...
fr fr Test function call parsing
...
assert_eq_int(len(expr11.children), 1) fr fr callee only
...
assert_eq_int(len(expr12.children), 3) fr fr callee + 2 arguments
...
fr fr Test member access parsing
...
fr fr Test variable declaration parsing
...
fr fr Test function declaration parsing
...
assert_eq_int(len(stmt3.children), 2) fr fr params + body
print_test_summary()
...
fr fr Parameters should be parsed
assert_eq_int(len(stmt4.children[0].children), 2) fr fr 2 parameters
print_test_summary()

fr fr Test return statement parsing
...
fr fr Test constant declaration parsing
...
assert_eq_int(len(program.children), 2) fr fr 2 statements
print_test_summary()

fr fr Test symbol table management
...
assert_eq_string(mixed_type, "meal") fr fr Type promotion to float
print_test_summary()

fr fr Test error handling
test_start("parse error handling")
sus parser31 := create_parser(tokenize("sus := 42")) fr fr Missing identifier
...
assert_true(len(test_nodes) >= 2) fr fr Variable and function both named "test"
print_test_summary()

fr fr Test complex parsing scenarios
...
assert_eq_int(chain_expr.node_type, NodeBinary) fr fr Final member access
...
fr fr Should be parsed as member access followed by function call
...
fr fr Test comprehensive parsing
test_start("parse comprehensive program")
sus comprehensive_source := ` fr fr Variable declarations
    sus x normie := 42
    sus name tea := "test"
    sus flag lit := based fr fr Constant declaration
    facts PI := 3.14159 fr fr Type alias
    be_like MyType = normie fr fr Function declaration
...
assert_true(len(comp_program.children) >= 6) fr fr Multiple statements

fr fr Check symbol table was populated
assert_true(len(parser41.symbols) >= 4) fr fr x, name, flag, PI, calculate

fr fr Should have no parse errors for valid syntax
sus comp_errors := get_errors(parser41)
assert_eq_int(len(comp_errors), 0)

print_test_summary()

fr fr Integration test - full parse and analysis
...
fr fr Verify type checking works
