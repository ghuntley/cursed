...
slay tokenize(source tea) []Token {
    ...
    while i < stringz.length(source) {
        sus ch := stringz.char_at(source, i) fr fr Skip whitespace
        ...
        if ch == '#' {
            ...
            tokens = append(tokens, token)
            continue
        } fr fr Unknown character - create error token
        sus value := stringz.char_to_string(ch)
...
fr fr Check if string is a keyword
...
fr fr Check if character is an operator
...
fr fr Check if character is a delimiter
...
fr fr Type inference for expressions
slay infer_type(node *ASTNode, parser *Parser) tea {
    if node == cringe {
        damn "void"
    }
    
    if node.node_type == NodeLiteral { fr fr Infer type from literal value
