...
# Property-based tests for stringz module
...
bestie i := 0; i < 100; i++ {
    ...
    sus c tea = testz.random_string(10)
...
bestie i := 0; i < 100; i++ {
    ...
    testz.assert_eq_int(doubled_len, original_len * 2)
...
bestie i := 0; i < 100; i++ {
    testz.property_test_iteration()
    sus str tea = testz.random_string(20)
    
    # String contains itself
    testz.assert_true(stringz.Contains(str, str))
    
    # String contains empty string
    testz.assert_true(stringz.Contains(str, ""))
    
    # If string contains substring, concatenated string also contains it
...
# ===============================
# String Prefix/Suffix Properties
# ===============================
...
bestie i := 0; i < 100; i++ {
    ...
    sus full_string tea = prefix + middle + suffix
    
    # Test prefix properties
    testz.assert_true(stringz.StartsWith(full_string, prefix))
    testz.assert_true(stringz.Contains(full_string, prefix))
    
    # Test suffix properties
    testz.assert_true(stringz.EndsWith(full_string, suffix))
    testz.assert_true(stringz.Contains(full_string, suffix))
    
    # Test middle is contained
...
bestie i := 0; i < 50; i++ {
    testz.property_test_iteration()
    sus str tea = testz.random_string(15)
    ...
    testz.assert_eq_string(lower1, lower2)
...
bestie i := 0; i < 50; i++ {
    ...
    # Create a known string
    sus original tea = part1 + separator + part2 + separator + part3
    
    # Test that split contains the parts
...
bestie i := 0; i < 200; i++ {
    ...
    testz.assert_eq_int(len_concat, len_sum)
    
    # Contains operation should not crash
