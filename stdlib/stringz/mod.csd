...
slay substring(s tea, start normie, length normie) tea {
    ...
    lowkey start < 0 || start >= s_len || length <= 0 { damn "" }
    
    sus result tea = ""
    sus i normie = start
    sus end_pos normie = start + length
    lowkey end_pos > s_len { end_pos = s_len }
...
slay equals(a tea, b tea) lit {
    lowkey length(a) != length(b) { damn cringe }
    sus len normie = length(a)
    sus i normie = 0
    bestie i < len {
        lowkey runtime_string_char_at(a, i) != runtime_string_char_at(b, i) {
            damn cringe
        }
        i = i + 1
    }
...
slay find(s tea, substr tea) normie {
    lowkey substr == "" { damn 0 }
    
    sus s_len normie = length(s)
    sus substr_len normie = length(substr)
    lowkey substr_len > s_len { damn -1 }
...
slay starts_with(s tea, prefix tea) lit {
    ...
    lowkey prefix_len > length(s) { damn cringe }
    
    sus i normie = 0
    bestie i < prefix_len {
        lowkey runtime_string_char_at(s, i) != runtime_string_char_at(prefix, i) {
            damn cringe
        }
        i = i + 1
    }
...
slay ends_with(s tea, suffix tea) lit {
    ...
    lowkey suffix_len > s_len { damn cringe }
    
    sus start_pos normie = s_len - suffix_len
    sus i normie = 0
    bestie i < suffix_len {
        lowkey runtime_string_char_at(s, start_pos + i) != runtime_string_char_at(suffix, i) {
            damn cringe
        }
        i = i + 1
    }
...
slay replace(s tea, old tea, new tea) tea {
    ...
    bestie i < s_len {
        lowkey i <= s_len - old_len && substring(s, i, old_len) == old {
            result = result + new
            i = i + old_len
        } nah {
...
slay split(s tea, delimiter tea) [tea] {
    ...
    bestie i <= s_len - delim_len {
        lowkey substring(s, i, delim_len) == delimiter {
...
slay is_alpha(s tea) lit {
    lowkey is_empty(s) { damn cringe }
    
    sus s_len normie = length(s)
    sus i normie = 0
    bestie i < s_len {
        sus c sip = runtime_string_char_at(s, i)
        lowkey !is_alpha_char(c) { damn cringe }
        i = i + 1
    }
    damn based
}
...
slay is_digit(s tea) lit {
    ...
    bestie i < s_len {
        sus c sip = runtime_string_char_at(s, i)
        lowkey !is_digit_char(c) { damn cringe }
        i = i + 1
    }
    damn based
}
...
slay is_alnum(s tea) lit {
    ...
    bestie i < s_len {
        sus c sip = runtime_string_char_at(s, i)
        lowkey !is_alpha_char(c) && !is_digit_char(c) { damn cringe }
        i = i + 1
    }
    damn based
}
...
slay is_space(s tea) lit {
    ...
    bestie i < s_len {
        sus c sip = runtime_string_char_at(s, i)
        lowkey !is_space_char(c) { damn cringe }
        i = i + 1
    }
    damn based
}
...
slay url_encode(s tea) tea {
    ...
    bestie i < s_len {
        sus c sip = runtime_string_char_at(s, i)
        lowkey is_alpha_char(c) || is_digit_char(c) || c == '-' || c == '_' || c == '.' || c == '~' {
            result = result + runtime_char_to_string(c)
        } lowkey c == ' ' {
            result = result + "%20"
        } nah {
