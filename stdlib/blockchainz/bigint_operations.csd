# blockchainz/bigint_operations.csd - Big Integer Operations for Cryptography
# Production-grade arbitrary precision integer arithmetic for blockchain crypto

yeet "mathz"
yeet "stringz"
yeet "vibez"

# ===== BIG INTEGER REPRESENTATION =====
# Big integers stored as arrays of 32-bit words in little-endian format
# This provides sufficient precision for cryptographic operations

slay hex_to_bigint(hex_string tea) []drip {
    fr fr Convert hex string to big integer
    sus cleaned_hex tea = stringz.to_lower(stringz.replace(hex_string, " ", ""))
    
    ready stringz.starts_with(cleaned_hex, "0x") {
        cleaned_hex = stringz.substring(cleaned_hex, 2, stringz.length(cleaned_hex) - 2)
    }
    
    # Pad to multiple of 8 hex chars (32-bit words)
    bestie (stringz.length(cleaned_hex) % 8) != 0 {
        cleaned_hex = "0" + cleaned_hex
    }
    
    sus result []drip = []
    sus hex_len drip = stringz.length(cleaned_hex)
    
    # Process 8 hex chars (32 bits) at a time from right to left
    bestie i := hex_len; i > 0; i -= 8 {
        sus start drip = mathz.max(0, i - 8)
        sus word_hex tea = stringz.substring(cleaned_hex, start, mathz.min(8, i - start))
        sus word_value drip = hex_string_to_int(word_hex)
        result = append(result, word_value)
    }
    
    damn normalize_bigint(result)
}

slay bigint_to_hex(bigint []drip) tea {
    fr fr Convert big integer to hex string
    sus normalized []drip = normalize_bigint(bigint)
    
    ready len(normalized) == 0 {
        damn "0"
    }
    
    sus result tea = ""
    sus started lit = cringe
    
    # Convert from most significant word to least
    bestie i := len(normalized) - 1; i >= 0; i -= 1 {
        sus word_hex tea = int_to_hex_padded(normalized[i], 8)
        
        ready !started {
            # Skip leading zeros in most significant word
            word_hex = stringz.ltrim(word_hex, "0")
            ready word_hex == "" {
                ready i == 0 {
                    damn "0"
                }
                continue
            }
            started = based
        }
        
        result = result + word_hex
    }
    
    ready result == "" {
        damn "0"
    }
    
    damn result
}

slay bytes_to_bigint(bytes []drip) []drip {
    fr fr Convert byte array to big integer (big-endian)
    ready len(bytes) == 0 {
        damn []
    }
    
    sus result []drip = []
    sus current_word drip = 0
    sus bit_count drip = 0
    
    # Process bytes from right to left (big-endian to little-endian)
    bestie i := len(bytes) - 1; i >= 0; i -= 1 {
        current_word = current_word | (bytes[i] << bit_count)
        bit_count = bit_count + 8
        
        ready bit_count == 32 {
            result = append(result, current_word)
            current_word = 0
            bit_count = 0
        }
    }
    
    ready bit_count > 0 {
        result = append(result, current_word)
    }
    
    damn normalize_bigint(result)
}

slay bigint_to_bytes_32(bigint []drip) []drip {
    fr fr Convert big integer to 32-byte array (big-endian)
    sus normalized []drip = normalize_bigint(bigint)
    sus result []drip = make_array(32, 0)
    
    sus byte_index drip = 31  # Start from rightmost byte
    
    bestie i := 0; i < len(normalized) && byte_index >= 0; i++ {
        sus word drip = normalized[i]
        
        # Extract 4 bytes from word (little-endian word to big-endian bytes)
        bestie j := 0; j < 4 && byte_index >= 0; j++ {
            result[byte_index] = word & 0xFF
            word = word >> 8
            byte_index = byte_index - 1
        }
    }
    
    damn result
}

# ===== BASIC ARITHMETIC OPERATIONS =====

slay add_bigint(a []drip, b []drip) []drip {
    fr fr Add two big integers
    sus max_len drip = mathz.max(len(a), len(b)) + 1
    sus result []drip = make_array(max_len, 0)
    sus carry drip = 0
    
    bestie i := 0; i < max_len; i++ {
        sus a_val drip = 0
        sus b_val drip = 0
        
        ready i < len(a) {
            a_val = a[i]
        }
        ready i < len(b) {
            b_val = b[i]
        }
        
        sus sum drip = a_val + b_val + carry
        result[i] = sum & 0xFFFFFFFF
        carry = sum >> 32
    }
    
    damn normalize_bigint(result)
}

slay subtract_bigint(a []drip, b []drip) []drip {
    fr fr Subtract b from a (assumes a >= b)
    ready compare_bigint(a, b) < 0 {
        vibez.spill("ERROR: Cannot subtract larger number from smaller")
        damn []
    }
    
    sus result []drip = make_array(len(a), 0)
    sus borrow drip = 0
    
    bestie i := 0; i < len(a); i++ {
        sus a_val drip = a[i]
        sus b_val drip = 0
        
        ready i < len(b) {
            b_val = b[i]
        }
        
        sus diff drip = a_val - b_val - borrow
        
        ready diff < 0 {
            diff = diff + 0x100000000  # 2^32
            borrow = 1
        } otherwise {
            borrow = 0
        }
        
        result[i] = diff & 0xFFFFFFFF
    }
    
    damn normalize_bigint(result)
}

slay multiply_bigint(a []drip, b []drip) []drip {
    fr fr Multiply two big integers
    ready len(a) == 0 || len(b) == 0 {
        damn []
    }
    
    sus result []drip = make_array(len(a) + len(b), 0)
    
    bestie i := 0; i < len(a); i++ {
        sus carry drip = 0
        
        bestie j := 0; j < len(b); j++ {
            sus product drip = a[i] * b[j] + result[i + j] + carry
            result[i + j] = product & 0xFFFFFFFF
            carry = product >> 32
        }
        
        ready carry > 0 {
            result[i + len(b)] = carry
        }
    }
    
    damn normalize_bigint(result)
}

slay divide_bigint(dividend []drip, divisor []drip) []drip {
    fr fr Divide two big integers (returns quotient)
    ready len(divisor) == 0 || is_zero(divisor) {
        vibez.spill("ERROR: Division by zero")
        damn []
    }
    
    ready compare_bigint(dividend, divisor) < 0 {
        damn []  # Quotient is 0
    }
    
    ready compare_bigint(dividend, divisor) == 0 {
        damn [1]  # Quotient is 1
    }
    
    # Long division algorithm
    sus remainder []drip = copy_bigint(dividend)
    sus quotient []drip = []
    
    # Find the bit length difference
    sus dividend_bits drip = bit_length(dividend)
    sus divisor_bits drip = bit_length(divisor)
    sus bit_diff drip = dividend_bits - divisor_bits
    
    bestie bit_diff >= 0 {
        sus shifted_divisor []drip = left_shift_bigint(divisor, bit_diff)
        
        ready compare_bigint(remainder, shifted_divisor) >= 0 {
            remainder = subtract_bigint(remainder, shifted_divisor)
            quotient = set_bit_bigint(quotient, bit_diff)
        }
        
        bit_diff = bit_diff - 1
    }
    
    damn normalize_bigint(quotient)
}

# ===== MODULAR ARITHMETIC =====

slay mod_bigint(a []drip, m []drip) []drip {
    fr fr Compute a mod m
    ready len(m) == 0 || is_zero(m) {
        vibez.spill("ERROR: Modulus cannot be zero")
        damn []
    }
    
    ready compare_bigint(a, m) < 0 {
        damn normalize_bigint(a)
    }
    
    # Use division to find remainder
    sus quotient []drip = divide_bigint(a, m)
    sus product []drip = multiply_bigint(quotient, m)
    sus remainder []drip = subtract_bigint(a, product)
    
    damn normalize_bigint(remainder)
}

slay mod_add(a []drip, b []drip, m []drip) []drip {
    fr fr Compute (a + b) mod m
    sus sum []drip = add_bigint(a, b)
    damn mod_bigint(sum, m)
}

slay mod_sub(a []drip, b []drip, m []drip) []drip {
    fr fr Compute (a - b) mod m
    ready compare_bigint(a, b) >= 0 {
        sus diff []drip = subtract_bigint(a, b)
        damn mod_bigint(diff, m)
    } otherwise {
        # a < b, so (a - b) mod m = m - ((b - a) mod m)
        sus diff []drip = subtract_bigint(b, a)
        sus mod_diff []drip = mod_bigint(diff, m)
        ready is_zero(mod_diff) {
            damn []
        }
        damn subtract_bigint(m, mod_diff)
    }
}

slay mod_mult(a []drip, b []drip, m []drip) []drip {
    fr fr Compute (a * b) mod m
    sus product []drip = multiply_bigint(a, b)
    damn mod_bigint(product, m)
}

slay mod_exp(base []drip, exponent []drip, modulus []drip) []drip {
    fr fr Modular exponentiation using binary method
    ready is_zero(exponent) {
        damn [1]
    }
    
    sus result []drip = [1]
    sus base_mod []drip = mod_bigint(base, modulus)
    sus exp_copy []drip = copy_bigint(exponent)
    
    bestie !is_zero(exp_copy) {
        ready is_odd(exp_copy) {
            result = mod_mult(result, base_mod, modulus)
        }
        
        base_mod = mod_mult(base_mod, base_mod, modulus)
        exp_copy = right_shift_bigint(exp_copy, 1)
    }
    
    damn result
}

slay mod_inverse(a []drip, m []drip) []drip {
    fr fr Compute modular inverse using extended Euclidean algorithm
    sus original_a []drip = normalize_bigint(a)
    sus original_m []drip = normalize_bigint(m)
    
    ready is_zero(original_a) {
        vibez.spill("ERROR: Cannot find inverse of zero")
        damn []
    }
    
    sus old_r []drip = copy_bigint(original_m)
    sus r []drip = copy_bigint(original_a)
    sus old_s []drip = []  # 0
    sus s []drip = [1]
    
    bestie !is_zero(r) {
        sus quotient []drip = divide_bigint(old_r, r)
        
        # Update remainders
        sus temp_r []drip = copy_bigint(r)
        sus qr_product []drip = multiply_bigint(quotient, r)
        r = subtract_bigint(old_r, qr_product)
        old_r = temp_r
        
        # Update coefficients
        sus temp_s []drip = copy_bigint(s)
        sus qs_product []drip = multiply_bigint(quotient, s)
        
        ready len(old_s) == 0 {  # old_s is 0
            s = negate_bigint(qs_product, original_m)
        } otherwise ready compare_bigint(old_s, qs_product) >= 0 {
            s = subtract_bigint(old_s, qs_product)
        } otherwise {
            sus diff []drip = subtract_bigint(qs_product, old_s)
            s = subtract_bigint(original_m, mod_bigint(diff, original_m))
        }
        
        old_s = temp_s
    }
    
    ready !is_one(old_r) {
        vibez.spill("ERROR: Modular inverse does not exist")
        damn []
    }
    
    # Ensure positive result
    ready len(old_s) == 0 {
        damn []
    }
    
    damn mod_bigint(old_s, original_m)
}

# ===== BIT OPERATIONS =====

slay left_shift_bigint(a []drip, shift drip) []drip {
    fr fr Left shift big integer by shift bits
    ready shift == 0 {
        damn normalize_bigint(a)
    }
    
    sus word_shift drip = shift / 32
    sus bit_shift drip = shift % 32
    
    sus result []drip = make_array(len(a) + word_shift + 1, 0)
    
    ready bit_shift == 0 {
        # Simple word shift
        bestie i := 0; i < len(a); i++ {
            result[i + word_shift] = a[i]
        }
    } otherwise {
        # Shift with bit operations
        sus carry drip = 0
        bestie i := 0; i < len(a); i++ {
            sus shifted drip = (a[i] << bit_shift) | carry
            result[i + word_shift] = shifted & 0xFFFFFFFF
            carry = a[i] >> (32 - bit_shift)
        }
        
        ready carry > 0 {
            result[len(a) + word_shift] = carry
        }
    }
    
    damn normalize_bigint(result)
}

slay right_shift_bigint(a []drip, shift drip) []drip {
    fr fr Right shift big integer by shift bits
    ready shift == 0 {
        damn normalize_bigint(a)
    }
    
    sus word_shift drip = shift / 32
    sus bit_shift drip = shift % 32
    
    ready word_shift >= len(a) {
        damn []  # Result is 0
    }
    
    sus result []drip = make_array(len(a) - word_shift, 0)
    
    ready bit_shift == 0 {
        # Simple word shift
        bestie i := 0; i < len(result); i++ {
            result[i] = a[i + word_shift]
        }
    } otherwise {
        # Shift with bit operations
        bestie i := 0; i < len(result); i++ {
            sus shifted drip = a[i + word_shift] >> bit_shift
            
            ready i + word_shift + 1 < len(a) {
                shifted = shifted | ((a[i + word_shift + 1] & ((1 << bit_shift) - 1)) << (32 - bit_shift))
            }
            
            result[i] = shifted
        }
    }
    
    damn normalize_bigint(result)
}

slay set_bit_bigint(a []drip, bit_index drip) []drip {
    fr fr Set bit at given index in big integer
    sus word_index drip = bit_index / 32
    sus bit_in_word drip = bit_index % 32
    
    sus result []drip = copy_bigint(a)
    
    # Extend array if necessary
    bestie word_index >= len(result) {
        sus new_array []drip = make_array(word_index + 1, 0)
        bestie i := 0; i < len(result); i++ {
            new_array[i] = result[i]
        }
        result = new_array
    }
    
    result[word_index] = result[word_index] | (1 << bit_in_word)
    
    damn normalize_bigint(result)
}

# ===== COMPARISON OPERATIONS =====

slay compare_bigint(a []drip, b []drip) drip {
    fr fr Compare two big integers
    fr fr Returns: -1 if a < b, 0 if a == b, 1 if a > b
    
    sus norm_a []drip = normalize_bigint(a)
    sus norm_b []drip = normalize_bigint(b)
    
    ready len(norm_a) < len(norm_b) {
        damn -1
    }
    ready len(norm_a) > len(norm_b) {
        damn 1
    }
    
    # Same length, compare from most significant word
    bestie i := len(norm_a) - 1; i >= 0; i -= 1 {
        ready norm_a[i] < norm_b[i] {
            damn -1
        }
        ready norm_a[i] > norm_b[i] {
            damn 1
        }
    }
    
    damn 0  # Equal
}

slay is_zero(a []drip) lit {
    sus normalized []drip = normalize_bigint(a)
    damn len(normalized) == 0
}

slay is_one(a []drip) lit {
    sus normalized []drip = normalize_bigint(a)
    damn len(normalized) == 1 && normalized[0] == 1
}

slay is_odd(a []drip) lit {
    ready len(a) == 0 {
        damn cringe
    }
    damn (a[0] & 1) == 1
}

slay bytes_equal(a []drip, b []drip) lit {
    ready len(a) != len(b) {
        damn cringe
    }
    
    bestie i := 0; i < len(a); i++ {
        ready a[i] != b[i] {
            damn cringe
        }
    }
    
    damn based
}

# ===== UTILITY FUNCTIONS =====

slay normalize_bigint(a []drip) []drip {
    fr fr Remove leading zero words
    ready len(a) == 0 {
        damn []
    }
    
    sus last_nonzero drip = -1
    bestie i := len(a) - 1; i >= 0; i -= 1 {
        ready a[i] != 0 {
            last_nonzero = i
            break
        }
    }
    
    ready last_nonzero == -1 {
        damn []  # All zeros
    }
    
    sus result []drip = make_array(last_nonzero + 1, 0)
    bestie i := 0; i <= last_nonzero; i++ {
        result[i] = a[i]
    }
    
    damn result
}

slay copy_bigint(a []drip) []drip {
    sus result []drip = make_array(len(a), 0)
    bestie i := 0; i < len(a); i++ {
        result[i] = a[i]
    }
    damn result
}

slay negate_bigint(a []drip, modulus []drip) []drip {
    fr fr Compute modulus - a
    ready is_zero(a) {
        damn []
    }
    damn subtract_bigint(modulus, a)
}

slay bit_length(a []drip) drip {
    fr fr Return bit length of big integer
    sus normalized []drip = normalize_bigint(a)
    
    ready len(normalized) == 0 {
        damn 0
    }
    
    sus most_significant_word drip = normalized[len(normalized) - 1]
    sus word_bits drip = 0
    
    sus temp drip = most_significant_word
    bestie temp > 0 {
        word_bits = word_bits + 1
        temp = temp >> 1
    }
    
    damn (len(normalized) - 1) * 32 + word_bits
}

slay int_to_bigint(value drip) []drip {
    ready value == 0 {
        damn []
    }
    damn [value]
}

slay bigint_to_int(a []drip) drip {
    sus normalized []drip = normalize_bigint(a)
    ready len(normalized) == 0 {
        damn 0
    }
    damn normalized[0]
}

# ===== HELPER FUNCTIONS FOR HEX CONVERSION =====

slay hex_string_to_int(hex tea) drip {
    fr fr Convert hex string to integer
    sus result drip = 0
    sus hex_len drip = stringz.length(hex)
    
    bestie i := 0; i < hex_len; i++ {
        sus char tea = stringz.char_at(hex, i)
        sus digit drip = hex_char_to_digit(char)
        
        ready digit < 0 {
            vibez.spill("ERROR: Invalid hex character:", char)
            damn 0
        }
        
        result = result * 16 + digit
    }
    
    damn result
}

slay hex_char_to_digit(c tea) drip {
    ready c >= "0" && c <= "9" {
        damn stringz.char_code(c) - stringz.char_code("0")
    }
    ready c >= "a" && c <= "f" {
        damn stringz.char_code(c) - stringz.char_code("a") + 10
    }
    ready c >= "A" && c <= "F" {
        damn stringz.char_code(c) - stringz.char_code("A") + 10
    }
    damn -1
}

slay int_to_hex_padded(value drip, width drip) tea {
    sus hex_chars tea = "0123456789abcdef"
    sus result tea = ""
    sus temp drip = value
    
    bestie width > 0 {
        sus digit drip = temp & 0xF
        result = stringz.char_at(hex_chars, digit) + result
        temp = temp >> 4
        width = width - 1
    }
    
    damn result
}

slay make_array(size drip, value drip) []drip {
    fr fr Create array of given size filled with value
    sus result []drip = []
    bestie i := 0; i < size; i++ {
        result = append(result, value)
    }
    damn result
}

# Export big integer operations
export hex_to_bigint, bigint_to_hex, bytes_to_bigint, bigint_to_bytes_32
export add_bigint, subtract_bigint, multiply_bigint, divide_bigint
export mod_bigint, mod_add, mod_sub, mod_mult, mod_exp, mod_inverse
export left_shift_bigint, right_shift_bigint, set_bit_bigint
export compare_bigint, is_zero, is_one, is_odd, bytes_equal
export normalize_bigint, copy_bigint, bit_length, int_to_bigint, bigint_to_int

# Initialize big integer operations
vibez.spill("Big integer operations module initialized")
vibez.spill("✓ Arbitrary precision arithmetic")
vibez.spill("✓ Modular arithmetic operations")
vibez.spill("✓ Bit manipulation functions")
vibez.spill("✓ Cryptographic-grade precision")
