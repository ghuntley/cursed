...
slay test_hashmap() {
    ...
    sus map := new_hashmap<normie, tea>(10)
    ...
    assert_true(exists2)
    assert_eq_string(value2, "two")
    
    # Test non-existent key
    sus value_none, exists_none := hashmap_get(map, 99)
    assert_false(exists_none)
...
slay test_arraylist() {
    ...
    assert_true(exists4)
    assert_eq_int(val4, 50)
    
    # Test bounds checking
    sus val_out, exists_out := arraylist_get(list, 10)
    assert_false(exists_out)
    
    # Test dynamic resizing
    arraylist_add(list, 60)  # Should trigger resize
...
slay test_linkedlist() {
    ...
    sus list := new_linkedlist<normie>()
...
slay test_queue() {
    ...
    assert_eq_int(queue_size(queue), 0)
...
slay test_set() {
    ...
    assert_false(set_contains(set, 40))
    
    # Test duplicate add (should not affect membership)
    set_add(set, 10)
    assert_true(set_contains(set, 10))
...
slay test_avl_tree() {
    ...
    # Test insert (should maintain balance)
    avl_insert(tree, 10)
    avl_insert(tree, 20)
    avl_insert(tree, 30)
    avl_insert(tree, 40)
    ...
    assert_eq_int(tree.size, 6)
    
    # Test that tree is balanced (height should be reasonable)
    sus root_height := avl_height(tree.root)
    assert_true(root_height <= 4)  # For 6 nodes, height should be <= 4
    
    # Test balance factors are within [-1, 1]
...
slay test_priority_queue() {
    ...
    # Test insert and extract (max heap)
    pq_insert(pq, 10)
    pq_insert(pq, 30)
    pq_insert(pq, 20)
    pq_insert(pq, 40)
...
slay test_collections_integration() {
    test_start("Collections Integration Test")
    
    # Test using multiple collections together
    ...
    arraylist_add(list, "cherry")
    
    # Map fruits to lengths and push lengths to stack
...
slay test_collections_performance() {
    ...
    assert_eq_int(list_val, 1500)
    
    # Verify tree balance after many insertions
    sus tree_height := avl_height(large_tree.root)
    assert_true(tree_height <= 12)  # log2(1000) ≈ 10, so height should be reasonable
    
    vibez.spill("Performance tests passed - 1000 elements processed")
}

slay test_generic_collections() {
    test_start("Generic Collections Test")
    
    # Test collections with different types
    sus string_list := new_arraylist<tea>(5)
    sus int_stack := new_stack<normie>()
    sus string_set := new_set<tea>()
    ...
    assert_eq_string(str_val, "world")
