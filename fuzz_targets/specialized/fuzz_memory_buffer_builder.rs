...
// Targets: builder in src/type_system/test_result_simple.rs:133
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // builder(data, 0);        // Zero size
        // builder(data, 1);        // Single byte
        // builder(data, size);     // Full size
        // builder(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for builder
