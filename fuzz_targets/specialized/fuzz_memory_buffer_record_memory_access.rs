...
// Targets: record_memory_access in src/runtime/gc_race_detector.rs:482
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // record_memory_access(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // record_memory_access(data, 0);        // Zero size
        // record_memory_access(data, 1);        // Single byte
        // record_memory_access(data, size);     // Full size
        // record_memory_access(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for record_memory_access
