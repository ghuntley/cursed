...
// Targets: string_has_prefix in stdlib/stringz/mod_complex.csd:144
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // string_has_prefix(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // string_has_prefix(data, 0);        // Zero size
        // string_has_prefix(data, 1);        // Single byte
        // string_has_prefix(data, size);     // Full size
        // string_has_prefix(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for string_has_prefix
