...
// Targets: string_to_bool in stdlib/string_pure/mod.csd:618
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // string_to_bool(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // string_to_bool(data, 0);        // Zero size
        // string_to_bool(data, 1);        // Single byte
        // string_to_bool(data, size);     // Full size
        // string_to_bool(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for string_to_bool
