...
// Targets: compile_member_access in src/codegen/llvm/expression_compiler.rs:658
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // compile_member_access(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // compile_member_access(data, 0);        // Zero size
        // compile_member_access(data, 1);        // Single byte
        // compile_member_access(data, size);     // Full size
        // compile_member_access(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for compile_member_access
