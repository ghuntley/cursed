...
// Targets: remove_installed_package in src/package_manager/mutable_state_old.rs:100
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // remove_installed_package(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // remove_installed_package(data, 0);        // Zero size
        // remove_installed_package(data, 1);        // Single byte
        // remove_installed_package(data, size);     // Full size
        // remove_installed_package(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for remove_installed_package
