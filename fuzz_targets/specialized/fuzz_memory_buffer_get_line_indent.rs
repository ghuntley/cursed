...
// Targets: get_line_indent in src/formatter/rules.rs:177
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // get_line_indent(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // get_line_indent(data, 0);        // Zero size
        // get_line_indent(data, 1);        // Single byte
        // get_line_indent(data, size);     // Full size
        // get_line_indent(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for get_line_indent
