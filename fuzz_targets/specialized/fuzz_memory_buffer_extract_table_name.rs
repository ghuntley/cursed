...
// Targets: extract_table_name in src/stdlib/database/connection.rs:240
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // extract_table_name(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // extract_table_name(data, 0);        // Zero size
        // extract_table_name(data, 1);        // Single byte
        // extract_table_name(data, size);     // Full size
        // extract_table_name(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for extract_table_name
