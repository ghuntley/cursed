...
// Targets: set_memory_manager in src/runtime/heap_optimizer.rs:872
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // set_memory_manager(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // set_memory_manager(data, 0);        // Zero size
        // set_memory_manager(data, 1);        // Single byte
        // set_memory_manager(data, size);     // Full size
        // set_memory_manager(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for set_memory_manager
