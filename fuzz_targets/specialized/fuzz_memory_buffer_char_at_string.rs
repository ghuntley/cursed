...
// Targets: char_at_string in stdlib/compiler_core/mod.csd:140
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // char_at_string(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // char_at_string(data, 0);        // Zero size
        // char_at_string(data, 1);        // Single byte
        // char_at_string(data, size);     // Full size
        // char_at_string(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for char_at_string
