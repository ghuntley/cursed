...
// Targets: debugPrintStats in src-zig/type_system_runtime.zig:607
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // debugPrintStats(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // debugPrintStats(data, 0);        // Zero size
        // debugPrintStats(data, 1);        // Single byte
        // debugPrintStats(data, size);     // Full size
        // debugPrintStats(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for debugPrintStats
