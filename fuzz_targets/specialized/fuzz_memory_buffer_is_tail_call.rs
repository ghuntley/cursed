...
// Targets: is_tail_call in src/codegen/llvm/passes/tail_call.rs:64
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // is_tail_call(data, 0);        // Zero size
        // is_tail_call(data, 1);        // Single byte
        // is_tail_call(data, size);     // Full size
        // is_tail_call(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for is_tail_call
