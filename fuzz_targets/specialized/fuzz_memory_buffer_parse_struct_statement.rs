...
// Targets: parse_struct_statement in src/parser_main.rs:2802
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // parse_struct_statement(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // parse_struct_statement(data, 0);        // Zero size
        // parse_struct_statement(data, 1);        // Single byte
        // parse_struct_statement(data, size);     // Full size
        // parse_struct_statement(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for parse_struct_statement
