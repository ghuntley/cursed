...
// Targets: test_interface_registry in src/runtime/interface_dispatch.rs:654
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // test_interface_registry(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // test_interface_registry(data, 0);        // Zero size
        // test_interface_registry(data, 1);        // Single byte
        // test_interface_registry(data, size);     // Full size
        // test_interface_registry(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for test_interface_registry
