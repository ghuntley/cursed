...
// Targets: safe_strlen in test_language_features.csd:228
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // safe_strlen(data, 0);        // Zero size
        // safe_strlen(data, 1);        // Single byte
        // safe_strlen(data, size);     // Full size
        // safe_strlen(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for safe_strlen
