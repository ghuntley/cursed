...
// Targets: generate_function_signature in src/type_system/monomorphisation.rs:1105
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // generate_function_signature(buffer, data, size);
...
void test_string_operations(const uint8_t *data, size_t size) {
    ...
    // generate_function_signature(str);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // generate_function_signature(data, 0);        // Zero size
        // generate_function_signature(data, 1);        // Single byte
        // generate_function_signature(data, size);     // Full size
        // generate_function_signature(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for generate_function_signature
