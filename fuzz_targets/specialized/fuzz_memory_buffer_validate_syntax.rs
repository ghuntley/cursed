...
// Targets: validate_syntax in src/repl/advanced_syntax_highlighter.rs:389
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // validate_syntax(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // validate_syntax(data, 0);        // Zero size
        // validate_syntax(data, 1);        // Single byte
        // validate_syntax(data, size);     // Full size
        // validate_syntax(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for validate_syntax
