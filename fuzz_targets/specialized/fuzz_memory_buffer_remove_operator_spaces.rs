...
// Targets: remove_operator_spaces in src/formatter/simple.rs:359
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // remove_operator_spaces(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // remove_operator_spaces(data, 0);        // Zero size
        // remove_operator_spaces(data, 1);        // Single byte
        // remove_operator_spaces(data, size);     // Full size
        // remove_operator_spaces(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for remove_operator_spaces
