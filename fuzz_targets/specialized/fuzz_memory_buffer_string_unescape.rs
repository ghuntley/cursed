...
// Targets: string_unescape in stdlib/string_pure/mod.csd:673
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // string_unescape(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // string_unescape(data, 0);        // Zero size
        // string_unescape(data, 1);        // Single byte
        // string_unescape(data, size);     // Full size
        // string_unescape(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for string_unescape
