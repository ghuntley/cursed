...
// Targets: clear_buffer in src/runtime/debug_output.rs:528
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // clear_buffer(data, 0);        // Zero size
        // clear_buffer(data, 1);        // Single byte
        // clear_buffer(data, size);     // Full size
        // clear_buffer(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for clear_buffer
