...
// Targets: benchmark_memory_operations in benchmarks/cursed/comprehensive_stdlib_benchmark.csd:120
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // benchmark_memory_operations(buffer, data, size);
...
void test_string_operations(const uint8_t *data, size_t size) {
    ...
    // benchmark_memory_operations(str);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // benchmark_memory_operations(data, 0);        // Zero size
        // benchmark_memory_operations(data, 1);        // Single byte
        // benchmark_memory_operations(data, size);     // Full size
        // benchmark_memory_operations(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for benchmark_memory_operations
