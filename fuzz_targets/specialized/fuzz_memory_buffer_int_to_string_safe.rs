...
// Targets: int_to_string_safe in stdlib/vibez/mod_enhanced.csd:97
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // int_to_string_safe(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // int_to_string_safe(data, 0);        // Zero size
        // int_to_string_safe(data, 1);        // Single byte
        // int_to_string_safe(data, size);     // Full size
        // int_to_string_safe(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for int_to_string_safe
