...
// Targets: test_string_length in stdlib/string_pure/test_string_pure.csd:7
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // test_string_length(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // test_string_length(data, 0);        // Zero size
        // test_string_length(data, 1);        // Single byte
        // test_string_length(data, size);     // Full size
        // test_string_length(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for test_string_length
