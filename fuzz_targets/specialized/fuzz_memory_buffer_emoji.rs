...
// Targets: emoji in src/runtime/debug_output.rs:59
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // emoji(data, 0);        // Zero size
        // emoji(data, 1);        // Single byte
        // emoji(data, size);     // Full size
        // emoji(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for emoji
