...
// Targets: extract_condition_from_line in src/coverage/collector.rs:597
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // extract_condition_from_line(buffer, data, size);
...
void test_string_operations(const uint8_t *data, size_t size) {
    ...
    // extract_condition_from_line(str);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // extract_condition_from_line(data, 0);        // Zero size
        // extract_condition_from_line(data, 1);        // Single byte
        // extract_condition_from_line(data, size);     // Full size
        // extract_condition_from_line(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for extract_condition_from_line
