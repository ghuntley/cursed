...
// Targets: get_function_debug_info in src/ffi/debug_tools.rs:513
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // get_function_debug_info(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // get_function_debug_info(data, 0);        // Zero size
        // get_function_debug_info(data, 1);        // Single byte
        // get_function_debug_info(data, size);     // Full size
        // get_function_debug_info(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for get_function_debug_info
