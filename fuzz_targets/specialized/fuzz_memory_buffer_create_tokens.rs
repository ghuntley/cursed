...
// Targets: create_tokens in src/parser/advanced_signature_parser.rs:845
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // create_tokens(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // create_tokens(data, 0);        // Zero size
        // create_tokens(data, 1);        // Single byte
        // create_tokens(data, size);     // Full size
        // create_tokens(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for create_tokens
