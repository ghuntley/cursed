...
// Targets: new_text in src/stdlib/net/websocket/frame.rs:13
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // new_text(data, 0);        // Zero size
        // new_text(data, 1);        // Single byte
        // new_text(data, size);     // Full size
        // new_text(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for new_text
