...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    // Test compile_optimized_ir_to_native with temp file
    // Example: compile_optimized_ir_to_native(temp_path);
    // Example: compile_optimized_ir_to_native(temp_path, data, size);
...
void test_path_operations(const uint8_t *data, size_t size) {
    ...
    // compile_optimized_ir_to_native(path);
}
