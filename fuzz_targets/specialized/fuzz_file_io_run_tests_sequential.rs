...
// Targets: run_tests_sequential in src/bin/cursed_test.rs:146
...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    // Test run_tests_sequential with temp file
    // Example: run_tests_sequential(temp_path);
    // Example: run_tests_sequential(temp_path, data, size);
...
void test_path_operations(const uint8_t *data, size_t size) {
    ...
    // run_tests_sequential(path);
}


// Additional test functions for run_tests_sequential
