...
// Targets: sender in tests/e2e/concurrency/02_channels.csd:13
...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    // Test packet parsing
    // Test sender with network data
    // Example: sender(data, size);
    // Example: sender(sockfd, data, size);
...
void test_socket_operations(const uint8_t *data, size_t size) {
    ...
    // sender(sockfd, data, size);
    
    close(sockfd);
}


// Additional test functions for sender
