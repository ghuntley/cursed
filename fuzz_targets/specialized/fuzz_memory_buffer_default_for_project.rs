...
// Targets: default_for_project in src/build_system/build_pipeline.rs:163
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // default_for_project(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // default_for_project(data, 0);        // Zero size
        // default_for_project(data, 1);        // Single byte
        // default_for_project(data, size);     // Full size
        // default_for_project(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for default_for_project
