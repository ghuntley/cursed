...
// Targets: run in src/codegen/llvm/optimization_pipeline.rs:139
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // run(data, 0);        // Zero size
        // run(data, 1);        // Single byte
        // run(data, size);     // Full size
        // run(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for run
