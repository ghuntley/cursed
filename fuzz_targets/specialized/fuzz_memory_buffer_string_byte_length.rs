...
// Targets: string_byte_length in stdlib/string_simple/mod.csd:25
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // string_byte_length(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // string_byte_length(data, 0);        // Zero size
        // string_byte_length(data, 1);        // Single byte
        // string_byte_length(data, size);     // Full size
        // string_byte_length(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for string_byte_length
