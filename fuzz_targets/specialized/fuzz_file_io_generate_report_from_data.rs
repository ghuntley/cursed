...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    // Test generate_report_from_data with temp file
    // Example: generate_report_from_data(temp_path);
    // Example: generate_report_from_data(temp_path, data, size);
...
void test_path_operations(const uint8_t *data, size_t size) {
    ...
    // generate_report_from_data(path);
}
