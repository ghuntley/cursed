...
// Targets: remove_tag in stdlib/htmlrizzler/mod.csd:376
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // remove_tag(data, 0);        // Zero size
        // remove_tag(data, 1);        // Single byte
        // remove_tag(data, size);     // Full size
        // remove_tag(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for remove_tag
