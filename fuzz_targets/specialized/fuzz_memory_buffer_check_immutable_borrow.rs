...
// Targets: check_immutable_borrow in src/type_system/checker.rs:2587
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // check_immutable_borrow(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // check_immutable_borrow(data, 0);        // Zero size
        // check_immutable_borrow(data, 1);        // Single byte
        // check_immutable_borrow(data, size);     // Full size
        // check_immutable_borrow(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for check_immutable_borrow
