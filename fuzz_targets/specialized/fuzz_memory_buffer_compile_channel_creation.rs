...
// Targets: compile_channel_creation in src/codegen/llvm/expression_compiler.rs:1224
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // compile_channel_creation(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // compile_channel_creation(data, 0);        // Zero size
        // compile_channel_creation(data, 1);        // Single byte
        // compile_channel_creation(data, size);     // Full size
        // compile_channel_creation(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for compile_channel_creation
