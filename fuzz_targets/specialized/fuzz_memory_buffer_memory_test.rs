...
// Targets: memory_test in tests/regression/memory/variable_lifecycle.csd:18
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // memory_test(data, 0);        // Zero size
        // memory_test(data, 1);        // Single byte
        // memory_test(data, size);     // Full size
        // memory_test(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for memory_test
