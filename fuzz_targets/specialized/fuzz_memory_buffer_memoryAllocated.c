...
// Targets: memoryAllocated in src-zig/performance_hooks.zig:637
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // memoryAllocated(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // memoryAllocated(data, 0);        // Zero size
        // memoryAllocated(data, 1);        // Single byte
        // memoryAllocated(data, size);     // Full size
        // memoryAllocated(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for memoryAllocated
