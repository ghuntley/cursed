...
// Targets: generate_debug_info in src/debug/dwarf_comprehensive.rs:129
...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    // Test generate_debug_info with temp file
    // Example: generate_debug_info(temp_path);
    // Example: generate_debug_info(temp_path, data, size);
...
void test_path_operations(const uint8_t *data, size_t size) {
    ...
    // generate_debug_info(path);
}


// Additional test functions for generate_debug_info
