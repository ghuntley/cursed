...
// Targets: build_ge_instruction in src/codegen/llvm/inkwell_expression_compiler.rs:473
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // build_ge_instruction(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // build_ge_instruction(data, 0);        // Zero size
        // build_ge_instruction(data, 1);        // Single byte
        // build_ge_instruction(data, size);     // Full size
        // build_ge_instruction(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for build_ge_instruction
