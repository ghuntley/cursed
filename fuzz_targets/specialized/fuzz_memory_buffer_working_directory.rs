...
// Targets: working_directory in src/stdlib/net/protocols/ssh.rs:113
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // working_directory(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // working_directory(data, 0);        // Zero size
        // working_directory(data, 1);        // Single byte
        // working_directory(data, size);     // Full size
        // working_directory(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for working_directory
