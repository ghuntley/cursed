...
// Targets: contains_string in stdlib/filez/mod.csd:555
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // contains_string(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // contains_string(data, 0);        // Zero size
        // contains_string(data, 1);        // Single byte
        // contains_string(data, size);     // Full size
        // contains_string(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for contains_string
