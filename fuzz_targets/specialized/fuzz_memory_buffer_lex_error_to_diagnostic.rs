...
// Targets: lex_error_to_diagnostic in src/lsp/diagnostics.rs:72
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // lex_error_to_diagnostic(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // lex_error_to_diagnostic(data, 0);        // Zero size
        // lex_error_to_diagnostic(data, 1);        // Single byte
        // lex_error_to_diagnostic(data, size);     // Full size
        // lex_error_to_diagnostic(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for lex_error_to_diagnostic
