...
// Targets: fold_xor in src/codegen/llvm/passes/constant_propagation.rs:531
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // fold_xor(data, 0);        // Zero size
        // fold_xor(data, 1);        // Single byte
        // fold_xor(data, size);     // Full size
        // fold_xor(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for fold_xor
