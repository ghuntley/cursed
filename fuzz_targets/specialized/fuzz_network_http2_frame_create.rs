...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    // Test packet parsing
    // Test http2_frame_create with network data
    // Example: http2_frame_create(data, size);
    // Example: http2_frame_create(sockfd, data, size);
...
void test_socket_operations(const uint8_t *data, size_t size) {
    ...
    // http2_frame_create(sockfd, data, size);
