...
// Targets: benchmark_strings in benchmarks/cursed/minimal_performance_test.csd:6
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // benchmark_strings(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // benchmark_strings(data, 0);        // Zero size
        // benchmark_strings(data, 1);        // Single byte
        // benchmark_strings(data, size);     // Full size
        // benchmark_strings(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for benchmark_strings
