...
// Targets: range_error in src/stdlib/math/mod.rs:134
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // range_error(data, 0);        // Zero size
        // range_error(data, 1);        // Single byte
        // range_error(data, size);     // Full size
        // range_error(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for range_error
