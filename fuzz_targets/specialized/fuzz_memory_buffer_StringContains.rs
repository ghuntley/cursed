...
// Targets: StringContains in stdlib/sus_log/mod.csd:986
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // StringContains(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // StringContains(data, 0);        // Zero size
        // StringContains(data, 1);        // Single byte
        // StringContains(data, size);     // Full size
        // StringContains(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for StringContains
