...
// Targets: compile_with_optimization in benchmarks/cursed/compiler/compilation_speed_bench.csd:458
...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    // Test compile_with_optimization with temp file
    // Example: compile_with_optimization(temp_path);
    // Example: compile_with_optimization(temp_path, data, size);
...
void test_path_operations(const uint8_t *data, size_t size) {
    ...
    // compile_with_optimization(path);
}
