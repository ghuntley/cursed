...
// Targets: load_formatter_config in src/bin/disabled/cursed_tools_original.rs:437
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // load_formatter_config(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // load_formatter_config(data, 0);        // Zero size
        // load_formatter_config(data, 1);        // Single byte
        // load_formatter_config(data, size);     // Full size
        // load_formatter_config(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for load_formatter_config
