...
// Targets: start_collection in src/coverage/simple_collector.rs:41
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // start_collection(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // start_collection(data, 0);        // Zero size
        // start_collection(data, 1);        // Single byte
        // start_collection(data, size);     // Full size
        // start_collection(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for start_collection
