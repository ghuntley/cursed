...
// Targets: test_strict_configuration in tests/disabled/linter_integration_test.rs:246
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // test_strict_configuration(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // test_strict_configuration(data, 0);        // Zero size
        // test_strict_configuration(data, 1);        // Single byte
        // test_strict_configuration(data, size);     // Full size
        // test_strict_configuration(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for test_strict_configuration
