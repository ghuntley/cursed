...
// Targets: string_format in stdlib/runtime_core/mod.csd:346
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // string_format(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // string_format(data, 0);        // Zero size
        // string_format(data, 1);        // Single byte
        // string_format(data, size);     // Full size
        // string_format(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for string_format
