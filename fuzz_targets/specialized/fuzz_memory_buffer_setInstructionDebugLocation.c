...
// Targets: setInstructionDebugLocation in src-zig/debug_info.zig:540
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // setInstructionDebugLocation(buffer, data, size);
...
void test_string_operations(const uint8_t *data, size_t size) {
    ...
    // setInstructionDebugLocation(str);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // setInstructionDebugLocation(data, 0);        // Zero size
        // setInstructionDebugLocation(data, 1);        // Single byte
        // setInstructionDebugLocation(data, size);     // Full size
        // setInstructionDebugLocation(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for setInstructionDebugLocation
