...
// Targets: parse_cpu_list in src/runtime/pal/arm64.rs:459
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // parse_cpu_list(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // parse_cpu_list(data, 0);        // Zero size
        // parse_cpu_list(data, 1);        // Single byte
        // parse_cpu_list(data, size);     // Full size
        // parse_cpu_list(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for parse_cpu_list
