...
// Targets: is_digit in stdlib/json/mod.csd:384
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // is_digit(data, 0);        // Zero size
        // is_digit(data, 1);        // Single byte
        // is_digit(data, size);     // Full size
        // is_digit(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for is_digit
