...
// Targets: endParsing in src-zig/performance_integration.zig:228
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // endParsing(data, 0);        // Zero size
        // endParsing(data, 1);        // Single byte
        // endParsing(data, size);     // Full size
        // endParsing(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for endParsing
