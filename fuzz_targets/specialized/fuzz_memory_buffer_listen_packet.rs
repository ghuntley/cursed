...
// Targets: listen_packet in src/stdlib/vibe_net/mod.rs:142
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // listen_packet(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // listen_packet(data, 0);        // Zero size
        // listen_packet(data, 1);        // Single byte
        // listen_packet(data, size);     // Full size
        // listen_packet(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for listen_packet
