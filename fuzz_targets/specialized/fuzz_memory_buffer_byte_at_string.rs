...
// Targets: byte_at_string in stdlib/vibez/mod_original.csd:608
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // byte_at_string(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // byte_at_string(data, 0);        // Zero size
        // byte_at_string(data, 1);        // Single byte
        // byte_at_string(data, size);     // Full size
        // byte_at_string(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for byte_at_string
