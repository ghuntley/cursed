...
// Targets: count_constraints in src/package_manager/optimized_resolver.rs:778
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // count_constraints(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // count_constraints(data, 0);        // Zero size
        // count_constraints(data, 1);        // Single byte
        // count_constraints(data, size);     // Full size
        // count_constraints(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for count_constraints
