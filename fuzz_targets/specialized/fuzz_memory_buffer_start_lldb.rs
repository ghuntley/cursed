...
// Targets: start_lldb in src/debug/lldb_integration.rs:226
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // start_lldb(data, 0);        // Zero size
        // start_lldb(data, 1);        // Single byte
        // start_lldb(data, size);     // Full size
        // start_lldb(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for start_lldb
