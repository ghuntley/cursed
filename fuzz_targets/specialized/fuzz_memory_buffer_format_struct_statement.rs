...
// Targets: format_struct_statement in src/formatter/mod_complex.rs:444
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // format_struct_statement(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // format_struct_statement(data, 0);        // Zero size
        // format_struct_statement(data, 1);        // Single byte
        // format_struct_statement(data, size);     // Full size
        // format_struct_statement(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for format_struct_statement
