...
// Targets: build_openapi_spec in src/documentation/json_generator.rs:319
...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    // Test build_openapi_spec with temp file
    // Example: build_openapi_spec(temp_path);
    // Example: build_openapi_spec(temp_path, data, size);
...
void test_path_operations(const uint8_t *data, size_t size) {
    ...
    // build_openapi_spec(path);
}


// Additional test functions for build_openapi_spec
