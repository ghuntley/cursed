...
// Targets: count_call_sites in src/optimization/production_llvm_optimization.rs:919
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // count_call_sites(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // count_call_sites(data, 0);        // Zero size
        // count_call_sites(data, 1);        // Single byte
        // count_call_sites(data, size);     // Full size
        // count_call_sites(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for count_call_sites
