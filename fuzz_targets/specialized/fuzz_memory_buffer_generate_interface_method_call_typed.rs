...
// Targets: generate_interface_method_call_typed in src/codegen/llvm/main.rs:5662
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // generate_interface_method_call_typed(buffer, data, size);
...
void test_string_operations(const uint8_t *data, size_t size) {
    ...
    // generate_interface_method_call_typed(str);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // generate_interface_method_call_typed(data, 0);        // Zero size
        // generate_interface_method_call_typed(data, 1);        // Single byte
        // generate_interface_method_call_typed(data, size);     // Full size
        // generate_interface_method_call_typed(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for generate_interface_method_call_typed
