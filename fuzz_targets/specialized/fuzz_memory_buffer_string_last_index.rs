...
// Targets: string_last_index in stdlib/pathz/mod.csd:561
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // string_last_index(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // string_last_index(data, 0);        // Zero size
        // string_last_index(data, 1);        // Single byte
        // string_last_index(data, size);     // Full size
        // string_last_index(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for string_last_index
