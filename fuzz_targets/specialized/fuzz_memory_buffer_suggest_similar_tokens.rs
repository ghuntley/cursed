...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // suggest_similar_tokens(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // suggest_similar_tokens(data, 0);        // Zero size
        // suggest_similar_tokens(data, 1);        // Single byte
        // suggest_similar_tokens(data, size);     // Full size
        // suggest_similar_tokens(NULL, 0);        // NULL pointer
    }
}
