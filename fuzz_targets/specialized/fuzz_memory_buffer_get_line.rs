...
// Targets: get_line in src/linter/mod.rs:283
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // get_line(data, 0);        // Zero size
        // get_line(data, 1);        // Single byte
        // get_line(data, size);     // Full size
        // get_line(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for get_line
