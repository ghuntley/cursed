...
// Targets: get_installed_package in src/package_manager/mod.rs:531
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // get_installed_package(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // get_installed_package(data, 0);        // Zero size
        // get_installed_package(data, 1);        // Single byte
        // get_installed_package(data, size);     // Full size
        // get_installed_package(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for get_installed_package
