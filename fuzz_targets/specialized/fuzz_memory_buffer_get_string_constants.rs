...
// Targets: get_string_constants in src/codegen/llvm/expression_compiler.rs:1119
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // get_string_constants(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // get_string_constants(data, 0);        // Zero size
        // get_string_constants(data, 1);        // Single byte
        // get_string_constants(data, size);     // Full size
        // get_string_constants(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for get_string_constants
