...
// Targets: is_formatted in src/formatter/simple.rs:662
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // is_formatted(data, 0);        // Zero size
        // is_formatted(data, 1);        // Single byte
        // is_formatted(data, size);     // Full size
        // is_formatted(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for is_formatted
