...
// Targets: check_move in src/type_system/checker.rs:2728
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // check_move(data, 0);        // Zero size
        // check_move(data, 1);        // Single byte
        // check_move(data, size);     // Full size
        // check_move(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for check_move
