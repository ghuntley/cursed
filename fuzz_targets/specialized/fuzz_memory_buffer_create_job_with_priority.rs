...
// Targets: create_job_with_priority in examples/parallel_compilation_demo.rs:334
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // create_job_with_priority(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // create_job_with_priority(data, 0);        // Zero size
        // create_job_with_priority(data, 1);        // Single byte
        // create_job_with_priority(data, size);     // Full size
        // create_job_with_priority(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for create_job_with_priority
