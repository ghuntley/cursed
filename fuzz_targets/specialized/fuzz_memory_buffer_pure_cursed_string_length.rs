...
// Targets: pure_cursed_string_length in build_ffi_free.rs:160
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // pure_cursed_string_length(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // pure_cursed_string_length(data, 0);        // Zero size
        // pure_cursed_string_length(data, 1);        // Single byte
        // pure_cursed_string_length(data, size);     // Full size
        // pure_cursed_string_length(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for pure_cursed_string_length
