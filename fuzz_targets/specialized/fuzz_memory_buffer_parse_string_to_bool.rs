...
// Targets: parse_string_to_bool in stdlib/configz/mod_complex_syntax.csd:385
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // parse_string_to_bool(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // parse_string_to_bool(data, 0);        // Zero size
        // parse_string_to_bool(data, 1);        // Single byte
        // parse_string_to_bool(data, size);     // Full size
        // parse_string_to_bool(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for parse_string_to_bool
