...
// Targets: get_callees in src/codegen/llvm/passes/inlining.rs:1086
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // get_callees(data, 0);        // Zero size
        // get_callees(data, 1);        // Single byte
        // get_callees(data, size);     // Full size
        // get_callees(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for get_callees
