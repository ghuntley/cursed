...
// Targets: legacy_string_concat in stdlib/self_hosted_builtins/mod.csd:421
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // legacy_string_concat(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // legacy_string_concat(data, 0);        // Zero size
        // legacy_string_concat(data, 1);        // Single byte
        // legacy_string_concat(data, size);     // Full size
        // legacy_string_concat(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for legacy_string_concat
