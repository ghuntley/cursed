...
// Targets: escape_html in src/bin/cursed_doc_simple.rs:546
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // escape_html(data, 0);        // Zero size
        // escape_html(data, 1);        // Single byte
        // escape_html(data, size);     // Full size
        // escape_html(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for escape_html
