...
// Targets: to_str in src/runtime/value/mod.rs:188
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // to_str(data, 0);        // Zero size
        // to_str(data, 1);        // Single byte
        // to_str(data, size);     // Full size
        // to_str(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for to_str
