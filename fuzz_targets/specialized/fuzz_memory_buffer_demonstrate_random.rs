...
// Targets: demonstrate_random in examples/math_comprehensive_demo.csd:148
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // demonstrate_random(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // demonstrate_random(data, 0);        // Zero size
        // demonstrate_random(data, 1);        // Single byte
        // demonstrate_random(data, size);     // Full size
        // demonstrate_random(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for demonstrate_random
