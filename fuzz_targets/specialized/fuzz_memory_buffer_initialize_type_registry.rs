...
// Targets: initialize_type_registry in src/runtime/type_switch.rs:33
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // initialize_type_registry(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // initialize_type_registry(data, 0);        // Zero size
        // initialize_type_registry(data, 1);        // Single byte
        // initialize_type_registry(data, size);     // Full size
        // initialize_type_registry(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for initialize_type_registry
