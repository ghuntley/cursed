...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    // Test packet parsing
    // Test connection_string with network data
    // Example: connection_string(data, size);
    // Example: connection_string(sockfd, data, size);
...
void test_socket_operations(const uint8_t *data, size_t size) {
    ...
    // connection_string(sockfd, data, size);
