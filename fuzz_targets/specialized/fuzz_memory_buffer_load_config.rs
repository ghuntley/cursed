...
// Targets: load_config in src/documentation/live_server.rs:463
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // load_config(data, 0);        // Zero size
        // load_config(data, 1);        // Single byte
        // load_config(data, size);     // Full size
        // load_config(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for load_config
