...
// Targets: get_error_context in src/execution/execution_context.rs:217
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // get_error_context(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // get_error_context(data, 0);        // Zero size
        // get_error_context(data, 1);        // Single byte
        // get_error_context(data, size);     // Full size
        // get_error_context(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for get_error_context
