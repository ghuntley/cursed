...
// Targets: greet in examples/docs_generation_demo.csd:49
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // greet(data, 0);        // Zero size
        // greet(data, 1);        // Single byte
        // greet(data, size);     // Full size
        // greet(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for greet
