...
// Targets: cursed_create_structured_error in src/execution/runtime_functions.rs:6179
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // cursed_create_structured_error(buffer, data, size);
...
void test_string_operations(const uint8_t *data, size_t size) {
    ...
    // cursed_create_structured_error(str);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // cursed_create_structured_error(data, 0);        // Zero size
        // cursed_create_structured_error(data, 1);        // Single byte
        // cursed_create_structured_error(data, size);     // Full size
        // cursed_create_structured_error(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for cursed_create_structured_error
