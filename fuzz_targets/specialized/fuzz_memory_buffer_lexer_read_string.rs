...
// Targets: lexer_read_string in stdlib/compiler_core/mod.csd:165
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // lexer_read_string(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // lexer_read_string(data, 0);        // Zero size
        // lexer_read_string(data, 1);        // Single byte
        // lexer_read_string(data, size);     // Full size
        // lexer_read_string(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for lexer_read_string
