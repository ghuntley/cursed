...
// Targets: new_fit_buffer_with_data in src/stdlib/bytefit/fitbuffer.rs:136
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // new_fit_buffer_with_data(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // new_fit_buffer_with_data(data, 0);        // Zero size
        // new_fit_buffer_with_data(data, 1);        // Single byte
        // new_fit_buffer_with_data(data, size);     // Full size
        // new_fit_buffer_with_data(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for new_fit_buffer_with_data
