...
// Targets: number_to_string in stdlib/vibez/core_functions.csd:221
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // number_to_string(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // number_to_string(data, 0);        // Zero size
        // number_to_string(data, 1);        // Single byte
        // number_to_string(data, size);     // Full size
        // number_to_string(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for number_to_string
