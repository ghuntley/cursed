...
// Targets: demonstrate_defer_with_error in defer_compilation_demo.csd:17
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // demonstrate_defer_with_error(buffer, data, size);
...
void test_string_operations(const uint8_t *data, size_t size) {
    ...
    // demonstrate_defer_with_error(str);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // demonstrate_defer_with_error(data, 0);        // Zero size
        // demonstrate_defer_with_error(data, 1);        // Single byte
        // demonstrate_defer_with_error(data, size);     // Full size
        // demonstrate_defer_with_error(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for demonstrate_defer_with_error
