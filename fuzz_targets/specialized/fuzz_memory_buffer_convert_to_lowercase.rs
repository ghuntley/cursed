...
// Targets: convert_to_lowercase in stdlib/configz/mod_complex_syntax.csd:617
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // convert_to_lowercase(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // convert_to_lowercase(data, 0);        // Zero size
        // convert_to_lowercase(data, 1);        // Single byte
        // convert_to_lowercase(data, size);     // Full size
        // convert_to_lowercase(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for convert_to_lowercase
