...
// Targets: getTotalUsage in src-zig/arena_allocator.zig:597
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // getTotalUsage(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // getTotalUsage(data, 0);        // Zero size
        // getTotalUsage(data, 1);        // Single byte
        // getTotalUsage(data, size);     // Full size
        // getTotalUsage(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for getTotalUsage
