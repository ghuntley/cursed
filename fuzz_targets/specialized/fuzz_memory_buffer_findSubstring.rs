...
// Targets: findSubstring in stdlib/oglogging/mod_original.csd:353
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // findSubstring(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // findSubstring(data, 0);        // Zero size
        // findSubstring(data, 1);        // Single byte
        // findSubstring(data, size);     // Full size
        // findSubstring(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for findSubstring
