...
// Targets: format_expression_to_string in src/tools/formatter.rs:558
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // format_expression_to_string(buffer, data, size);
...
void test_string_operations(const uint8_t *data, size_t size) {
    ...
    // format_expression_to_string(str);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // format_expression_to_string(data, 0);        // Zero size
        // format_expression_to_string(data, 1);        // Single byte
        // format_expression_to_string(data, size);     // Full size
        // format_expression_to_string(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for format_expression_to_string
