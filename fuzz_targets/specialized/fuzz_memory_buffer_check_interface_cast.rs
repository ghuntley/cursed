...
// Targets: check_interface_cast in src/type_system/checker.rs:1737
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // check_interface_cast(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // check_interface_cast(data, 0);        // Zero size
        // check_interface_cast(data, 1);        // Single byte
        // check_interface_cast(data, size);     // Full size
        // check_interface_cast(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for check_interface_cast
