...
// Targets: convert_value in src/runtime/value/mod.rs:394
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // convert_value(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // convert_value(data, 0);        // Zero size
        // convert_value(data, 1);        // Single byte
        // convert_value(data, size);     // Full size
        // convert_value(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for convert_value
