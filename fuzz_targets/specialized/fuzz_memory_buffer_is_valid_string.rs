...
// Targets: is_valid_string in stdlib/core/mod.csd:159
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // is_valid_string(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // is_valid_string(data, 0);        // Zero size
        // is_valid_string(data, 1);        // Single byte
        // is_valid_string(data, size);     // Full size
        // is_valid_string(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for is_valid_string
