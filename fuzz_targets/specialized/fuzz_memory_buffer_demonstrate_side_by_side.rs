...
// Targets: demonstrate_side_by_side in examples/type_switch_vs_regular_switch.csd:146
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // demonstrate_side_by_side(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // demonstrate_side_by_side(data, 0);        // Zero size
        // demonstrate_side_by_side(data, 1);        // Single byte
        // demonstrate_side_by_side(data, size);     // Full size
        // demonstrate_side_by_side(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for demonstrate_side_by_side
