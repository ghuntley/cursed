...
// Targets: unexpected_token in src/error/structured.rs:760
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // unexpected_token(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // unexpected_token(data, 0);        // Zero size
        // unexpected_token(data, 1);        // Single byte
        // unexpected_token(data, size);     // Full size
        // unexpected_token(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for unexpected_token
