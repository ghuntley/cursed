...
// Targets: read_string in stdlib/serialization/mod.csd:228
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // read_string(data, 0);        // Zero size
        // read_string(data, 1);        // Single byte
        // read_string(data, size);     // Full size
        // read_string(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for read_string
