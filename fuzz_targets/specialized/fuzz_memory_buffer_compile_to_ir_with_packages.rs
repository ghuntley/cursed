...
// Targets: compile_to_ir_with_packages in src/lib.rs:844
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // compile_to_ir_with_packages(buffer, data, size);
...
void test_string_operations(const uint8_t *data, size_t size) {
    ...
    // compile_to_ir_with_packages(str);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // compile_to_ir_with_packages(data, 0);        // Zero size
        // compile_to_ir_with_packages(data, 1);        // Single byte
        // compile_to_ir_with_packages(data, size);     // Full size
        // compile_to_ir_with_packages(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for compile_to_ir_with_packages
