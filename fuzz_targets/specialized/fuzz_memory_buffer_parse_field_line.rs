...
// Targets: parse_field_line in src/documentation/api_extractor.rs:310
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // parse_field_line(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // parse_field_line(data, 0);        // Zero size
        // parse_field_line(data, 1);        // Single byte
        // parse_field_line(data, size);     // Full size
        // parse_field_line(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for parse_field_line
