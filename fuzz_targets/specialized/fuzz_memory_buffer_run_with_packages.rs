...
// Targets: run_with_packages in src/lib.rs:516
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // run_with_packages(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // run_with_packages(data, 0);        // Zero size
        // run_with_packages(data, 1);        // Single byte
        // run_with_packages(data, size);     // Full size
        // run_with_packages(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for run_with_packages
