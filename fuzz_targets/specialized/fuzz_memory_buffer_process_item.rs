...
// Targets: process_item in examples/question_mark_demo.csd:204
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // process_item(data, 0);        // Zero size
        // process_item(data, 1);        // Single byte
        // process_item(data, size);     // Full size
        // process_item(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for process_item
