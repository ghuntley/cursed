...
// Targets: file_write_binary in stdlib/image_processing/mod.csd:535
...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    // Test file_write_binary with temp file
    // Example: file_write_binary(temp_path);
    // Example: file_write_binary(temp_path, data, size);
...
void test_path_operations(const uint8_t *data, size_t size) {
    ...
    // file_write_binary(path);
}


// Additional test functions for file_write_binary
