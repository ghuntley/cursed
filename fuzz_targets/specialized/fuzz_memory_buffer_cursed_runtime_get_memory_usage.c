...
// Targets: cursed_runtime_get_memory_usage in runtime/memory_runtime.c:261
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // cursed_runtime_get_memory_usage(buffer, data, size);
...
void test_string_operations(const uint8_t *data, size_t size) {
    ...
    // cursed_runtime_get_memory_usage(str);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // cursed_runtime_get_memory_usage(data, 0);        // Zero size
        // cursed_runtime_get_memory_usage(data, 1);        // Single byte
        // cursed_runtime_get_memory_usage(data, size);     // Full size
        // cursed_runtime_get_memory_usage(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for cursed_runtime_get_memory_usage
