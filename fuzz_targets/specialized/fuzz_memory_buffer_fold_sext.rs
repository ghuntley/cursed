...
// Targets: fold_sext in src/codegen/llvm/passes/constant_propagation.rs:688
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // fold_sext(data, 0);        // Zero size
        // fold_sext(data, 1);        // Single byte
        // fold_sext(data, size);     // Full size
        // fold_sext(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for fold_sext
