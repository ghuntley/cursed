...
// Targets: link_object_to_executable in src/lib.rs:2419
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // link_object_to_executable(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // link_object_to_executable(data, 0);        // Zero size
        // link_object_to_executable(data, 1);        // Single byte
        // link_object_to_executable(data, size);     // Full size
        // link_object_to_executable(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for link_object_to_executable
