...
// Targets: memory_arena_reset in stdlib/memory/mod.csd:250
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // memory_arena_reset(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // memory_arena_reset(data, 0);        // Zero size
        // memory_arena_reset(data, 1);        // Single byte
        // memory_arena_reset(data, size);     // Full size
        // memory_arena_reset(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for memory_arena_reset
