...
// Targets: protocol_error in src/stdlib/net/error.rs:130
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // protocol_error(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // protocol_error(data, 0);        // Zero size
        // protocol_error(data, 1);        // Single byte
        // protocol_error(data, size);     // Full size
        // protocol_error(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for protocol_error
