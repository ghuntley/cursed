...
// Targets: session_id in src/ffi/profiling.rs:495
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // session_id(data, 0);        // Zero size
        // session_id(data, 1);        // Single byte
        // session_id(data, size);     // Full size
        // session_id(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for session_id
