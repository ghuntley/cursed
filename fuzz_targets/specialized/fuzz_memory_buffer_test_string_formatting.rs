...
// Targets: test_string_formatting in stdlib/clock_bait/test_clock_bait.csd:286
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // test_string_formatting(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // test_string_formatting(data, 0);        // Zero size
        // test_string_formatting(data, 1);        // Single byte
        // test_string_formatting(data, size);     // Full size
        // test_string_formatting(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for test_string_formatting
