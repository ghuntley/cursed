...
// Targets: parse_field in src/ffi/header_parser.rs:418
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // parse_field(data, 0);        // Zero size
        // parse_field(data, 1);        // Single byte
        // parse_field(data, size);     // Full size
        // parse_field(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for parse_field
