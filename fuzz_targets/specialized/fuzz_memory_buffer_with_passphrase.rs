...
// Targets: with_passphrase in src/stdlib/net/protocols/ssh.rs:141
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // with_passphrase(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // with_passphrase(data, 0);        // Zero size
        // with_passphrase(data, 1);        // Single byte
        // with_passphrase(data, size);     // Full size
        // with_passphrase(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for with_passphrase
