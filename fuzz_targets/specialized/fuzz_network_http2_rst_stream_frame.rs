...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    // Test packet parsing
    // Test http2_rst_stream_frame with network data
    // Example: http2_rst_stream_frame(data, size);
    // Example: http2_rst_stream_frame(sockfd, data, size);
...
void test_socket_operations(const uint8_t *data, size_t size) {
    ...
    // http2_rst_stream_frame(sockfd, data, size);
