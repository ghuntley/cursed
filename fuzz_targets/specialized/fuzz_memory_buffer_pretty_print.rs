...
// Targets: pretty_print in stdlib/json/mod.csd:485
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // pretty_print(data, 0);        // Zero size
        // pretty_print(data, 1);        // Single byte
        // pretty_print(data, size);     // Full size
        // pretty_print(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for pretty_print
