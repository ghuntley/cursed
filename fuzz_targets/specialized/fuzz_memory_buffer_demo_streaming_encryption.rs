...
// Targets: demo_streaming_encryption in examples/xchacha20_poly1305_demo.csd:45
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // demo_streaming_encryption(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // demo_streaming_encryption(data, 0);        // Zero size
        // demo_streaming_encryption(data, 1);        // Single byte
        // demo_streaming_encryption(data, size);     // Full size
        // demo_streaming_encryption(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for demo_streaming_encryption
