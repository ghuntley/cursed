...
// Targets: add_builtin_type in src/type_system/mod.rs:1115
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // add_builtin_type(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // add_builtin_type(data, 0);        // Zero size
        // add_builtin_type(data, 1);        // Single byte
        // add_builtin_type(data, size);     // Full size
        // add_builtin_type(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for add_builtin_type
