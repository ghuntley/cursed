...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    // Test profile_guided_optimization with temp file
    // Example: profile_guided_optimization(temp_path);
    // Example: profile_guided_optimization(temp_path, data, size);
...
void test_path_operations(const uint8_t *data, size_t size) {
    ...
    // profile_guided_optimization(path);
}
