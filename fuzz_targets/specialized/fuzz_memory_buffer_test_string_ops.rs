...
// Targets: test_string_ops in src/stdlib/glyph_gang/string_ops.rs:65
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // test_string_ops(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // test_string_ops(data, 0);        // Zero size
        // test_string_ops(data, 1);        // Single byte
        // test_string_ops(data, size);     // Full size
        // test_string_ops(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for test_string_ops
