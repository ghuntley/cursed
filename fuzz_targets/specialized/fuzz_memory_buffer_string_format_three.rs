...
// Targets: string_format_three in stdlib/runtime_core/mod.csd:351
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // string_format_three(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // string_format_three(data, 0);        // Zero size
        // string_format_three(data, 1);        // Single byte
        // string_format_three(data, size);     // Full size
        // string_format_three(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for string_format_three
