...
// Targets: export_data in src/ffi/profiling.rs:299
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // export_data(data, 0);        // Zero size
        // export_data(data, 1);        // Single byte
        // export_data(data, size);     // Full size
        // export_data(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for export_data
