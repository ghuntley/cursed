...
// Targets: adjust_for_calling_convention in src/runtime/platform/dynamic_codegen.rs:492
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // adjust_for_calling_convention(buffer, data, size);
...
void test_string_operations(const uint8_t *data, size_t size) {
    ...
    // adjust_for_calling_convention(str);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // adjust_for_calling_convention(data, 0);        // Zero size
        // adjust_for_calling_convention(data, 1);        // Single byte
        // adjust_for_calling_convention(data, size);     // Full size
        // adjust_for_calling_convention(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for adjust_for_calling_convention
