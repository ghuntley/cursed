...
// Targets: select_frame in src/bin/cursed_debug.rs:482
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // select_frame(data, 0);        // Zero size
        // select_frame(data, 1);        // Single byte
        // select_frame(data, size);     // Full size
        // select_frame(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for select_frame
