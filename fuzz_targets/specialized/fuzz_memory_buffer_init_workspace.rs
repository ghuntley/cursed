...
// Targets: init_workspace in src/package_manager/workspace.rs:56
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // init_workspace(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // init_workspace(data, 0);        // Zero size
        // init_workspace(data, 1);        // Single byte
        // init_workspace(data, size);     // Full size
        // init_workspace(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for init_workspace
