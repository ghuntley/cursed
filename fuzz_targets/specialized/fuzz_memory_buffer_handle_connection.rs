...
// Targets: handle_connection in src/bin/cursed_doc.rs:824
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // handle_connection(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // handle_connection(data, 0);        // Zero size
        // handle_connection(data, 1);        // Single byte
        // handle_connection(data, size);     // Full size
        // handle_connection(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for handle_connection
