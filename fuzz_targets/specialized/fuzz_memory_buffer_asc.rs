...
// Targets: asc in src/stdlib/database/orm/query_builder.rs:496
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // asc(data, 0);        // Zero size
        // asc(data, 1);        // Single byte
        // asc(data, size);     // Full size
        // asc(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for asc
