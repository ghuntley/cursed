...
// Targets: check_package_naming_simplified in src/tools/linter_simplified.rs:278
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // check_package_naming_simplified(buffer, data, size);
...
void test_string_operations(const uint8_t *data, size_t size) {
    ...
    // check_package_naming_simplified(str);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // check_package_naming_simplified(data, 0);        // Zero size
        // check_package_naming_simplified(data, 1);        // Single byte
        // check_package_naming_simplified(data, size);     // Full size
        // check_package_naming_simplified(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for check_package_naming_simplified
