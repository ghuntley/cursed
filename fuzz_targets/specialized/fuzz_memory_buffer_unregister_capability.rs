...
// Targets: unregister_capability in tests/lsp_integration_tests.rs:22
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // unregister_capability(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // unregister_capability(data, 0);        // Zero size
        // unregister_capability(data, 1);        // Single byte
        // unregister_capability(data, size);     // Full size
        // unregister_capability(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for unregister_capability
