...
// Targets: command_exists in examples/simple_process_demo.rs:135
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // command_exists(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // command_exists(data, 0);        // Zero size
        // command_exists(data, 1);        // Single byte
        // command_exists(data, size);     // Full size
        // command_exists(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for command_exists
