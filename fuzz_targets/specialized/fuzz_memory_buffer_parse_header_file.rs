...
// Targets: parse_header_file in src/ffi/mod.rs:176
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // parse_header_file(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // parse_header_file(data, 0);        // Zero size
        // parse_header_file(data, 1);        // Single byte
        // parse_header_file(data, size);     // Full size
        // parse_header_file(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for parse_header_file
