...
// Targets: register_default_strategies in src/ffi/error_handling.rs:205
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // register_default_strategies(buffer, data, size);
...
void test_string_operations(const uint8_t *data, size_t size) {
    ...
    // register_default_strategies(str);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // register_default_strategies(data, 0);        // Zero size
        // register_default_strategies(data, 1);        // Single byte
        // register_default_strategies(data, size);     // Full size
        // register_default_strategies(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for register_default_strategies
