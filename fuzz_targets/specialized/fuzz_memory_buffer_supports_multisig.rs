...
// Targets: supports_multisig in src/stdlib/packages/crypto_signatures/mod.rs:324
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // supports_multisig(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // supports_multisig(data, 0);        // Zero size
        // supports_multisig(data, 1);        // Single byte
        // supports_multisig(data, size);     // Full size
        // supports_multisig(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for supports_multisig
