...
// Targets: highlight_lexical in src/lsp/semantic_highlighting.rs:291
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // highlight_lexical(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // highlight_lexical(data, 0);        // Zero size
        // highlight_lexical(data, 1);        // Single byte
        // highlight_lexical(data, size);     // Full size
        // highlight_lexical(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for highlight_lexical
