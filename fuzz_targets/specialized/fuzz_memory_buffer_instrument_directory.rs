...
// Targets: instrument_directory in src/coverage/instrumentation.rs:27
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // instrument_directory(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // instrument_directory(data, 0);        // Zero size
        // instrument_directory(data, 1);        // Single byte
        // instrument_directory(data, size);     // Full size
        // instrument_directory(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for instrument_directory
