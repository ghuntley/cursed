...
// Targets: get_worker_mut in src/optimization/distributed/worker_node.rs:215
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // get_worker_mut(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // get_worker_mut(data, 0);        // Zero size
        // get_worker_mut(data, 1);        // Single byte
        // get_worker_mut(data, size);     // Full size
        // get_worker_mut(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for get_worker_mut
