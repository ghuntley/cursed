...
// Targets: parse_enum_values in src/ffi/header_parser.rs:502
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // parse_enum_values(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // parse_enum_values(data, 0);        // Zero size
        // parse_enum_values(data, 1);        // Single byte
        // parse_enum_values(data, size);     // Full size
        // parse_enum_values(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for parse_enum_values
