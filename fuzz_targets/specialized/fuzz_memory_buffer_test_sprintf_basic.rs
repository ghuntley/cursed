...
// Targets: test_sprintf_basic in tests/stdlib_vibez_tests.rs:48
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // test_sprintf_basic(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // test_sprintf_basic(data, 0);        // Zero size
        // test_sprintf_basic(data, 1);        // Single byte
        // test_sprintf_basic(data, size);     // Full size
        // test_sprintf_basic(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for test_sprintf_basic
