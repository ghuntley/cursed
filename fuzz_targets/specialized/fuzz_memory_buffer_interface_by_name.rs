...
// Targets: interface_by_name in src/stdlib/vibe_net/mod.rs:378
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // interface_by_name(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // interface_by_name(data, 0);        // Zero size
        // interface_by_name(data, 1);        // Single byte
        // interface_by_name(data, size);     // Full size
        // interface_by_name(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for interface_by_name
