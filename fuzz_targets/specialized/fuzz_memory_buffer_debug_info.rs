...
// Targets: debug_info in src/stdlib/vibez/debug.rs:336
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // debug_info(data, 0);        // Zero size
        // debug_info(data, 1);        // Single byte
        // debug_info(data, size);     // Full size
        // debug_info(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for debug_info
