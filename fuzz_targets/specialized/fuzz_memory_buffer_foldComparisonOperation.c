...
// Targets: foldComparisonOperation in src-zig/constant_folder.zig:187
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // foldComparisonOperation(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // foldComparisonOperation(data, 0);        // Zero size
        // foldComparisonOperation(data, 1);        // Single byte
        // foldComparisonOperation(data, size);     // Full size
        // foldComparisonOperation(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for foldComparisonOperation
