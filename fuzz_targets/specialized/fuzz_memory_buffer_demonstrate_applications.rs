...
// Targets: demonstrate_applications in examples/math_comprehensive_demo.csd:338
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // demonstrate_applications(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // demonstrate_applications(data, 0);        // Zero size
        // demonstrate_applications(data, 1);        // Single byte
        // demonstrate_applications(data, size);     // Full size
        // demonstrate_applications(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for demonstrate_applications
