...
// Targets: validate_command_arg in src/security/input_validation.rs:296
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // validate_command_arg(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // validate_command_arg(data, 0);        // Zero size
        // validate_command_arg(data, 1);        // Single byte
        // validate_command_arg(data, size);     // Full size
        // validate_command_arg(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for validate_command_arg
