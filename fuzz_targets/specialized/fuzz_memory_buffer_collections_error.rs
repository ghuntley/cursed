...
// Targets: collections_error in src/error/mod.rs:230
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // collections_error(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // collections_error(data, 0);        // Zero size
        // collections_error(data, 1);        // Single byte
        // collections_error(data, size);     // Full size
        // collections_error(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for collections_error
