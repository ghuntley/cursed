...
// Targets: io_remove_directory in src/execution/runtime_functions.rs:1009
...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    // Test io_remove_directory with temp file
    // Example: io_remove_directory(temp_path);
    // Example: io_remove_directory(temp_path, data, size);
...
void test_path_operations(const uint8_t *data, size_t size) {
    ...
    // io_remove_directory(path);
}


// Additional test functions for io_remove_directory
