...
// Targets: generate_struct_declaration in self_hosting_compiler/codegen.csd:470
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // generate_struct_declaration(buffer, data, size);
...
void test_string_operations(const uint8_t *data, size_t size) {
    ...
    // generate_struct_declaration(str);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // generate_struct_declaration(data, 0);        // Zero size
        // generate_struct_declaration(data, 1);        // Single byte
        // generate_struct_declaration(data, size);     // Full size
        // generate_struct_declaration(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for generate_struct_declaration
