...
// Targets: is_interface_call in src/codegen/llvm/interface_optimization.rs:514
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // is_interface_call(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // is_interface_call(data, 0);        // Zero size
        // is_interface_call(data, 1);        // Single byte
        // is_interface_call(data, size);     // Full size
        // is_interface_call(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for is_interface_call
