...
// Targets: get_ir in src/codegen/llvm/expression_compiler.rs:1134
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // get_ir(data, 0);        // Zero size
        // get_ir(data, 1);        // Single byte
        // get_ir(data, size);     // Full size
        // get_ir(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for get_ir
