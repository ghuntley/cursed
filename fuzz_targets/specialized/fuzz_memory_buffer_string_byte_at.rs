...
// Targets: string_byte_at in stdlib/asn1_mood/mod.csd:355
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // string_byte_at(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // string_byte_at(data, 0);        // Zero size
        // string_byte_at(data, 1);        // Single byte
        // string_byte_at(data, size);     // Full size
        // string_byte_at(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for string_byte_at
