...
// Targets: deallocate_buffer in variadic_ffi_test.csd:206
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // deallocate_buffer(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // deallocate_buffer(data, 0);        // Zero size
        // deallocate_buffer(data, 1);        // Single byte
        // deallocate_buffer(data, size);     // Full size
        // deallocate_buffer(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for deallocate_buffer
