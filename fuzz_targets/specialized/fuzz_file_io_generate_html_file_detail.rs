...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    // Test generate_html_file_detail with temp file
    // Example: generate_html_file_detail(temp_path);
    // Example: generate_html_file_detail(temp_path, data, size);
...
void test_path_operations(const uint8_t *data, size_t size) {
    ...
    // generate_html_file_detail(path);
}
