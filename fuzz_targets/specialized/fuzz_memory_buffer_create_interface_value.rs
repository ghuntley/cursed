...
// Targets: create_interface_value in src/runtime/interface_dispatch.rs:387
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // create_interface_value(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // create_interface_value(data, 0);        // Zero size
        // create_interface_value(data, 1);        // Single byte
        // create_interface_value(data, size);     // Full size
        // create_interface_value(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for create_interface_value
