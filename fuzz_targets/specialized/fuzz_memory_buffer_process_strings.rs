...
// Targets: process_strings in benchmarks/cursed/string_processing.csd:5
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // process_strings(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // process_strings(data, 0);        // Zero size
        // process_strings(data, 1);        // Single byte
        // process_strings(data, size);     // Full size
        // process_strings(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for process_strings
