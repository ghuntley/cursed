...
// Targets: string_contains_real in stdlib/vibez/enhanced_formatting.csd:257
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // string_contains_real(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // string_contains_real(data, 0);        // Zero size
        // string_contains_real(data, 1);        // Single byte
        // string_contains_real(data, size);     // Full size
        // string_contains_real(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for string_contains_real
