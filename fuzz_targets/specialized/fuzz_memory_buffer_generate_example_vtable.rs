...
// Targets: generate_example_vtable in src/codegen/llvm/interface_dispatch.rs:311
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // generate_example_vtable(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // generate_example_vtable(data, 0);        // Zero size
        // generate_example_vtable(data, 1);        // Single byte
        // generate_example_vtable(data, size);     // Full size
        // generate_example_vtable(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for generate_example_vtable
