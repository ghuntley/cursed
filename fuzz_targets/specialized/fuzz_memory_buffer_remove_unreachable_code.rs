...
// Targets: remove_unreachable_code in src/codegen/llvm/generic_optimization.rs:428
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // remove_unreachable_code(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // remove_unreachable_code(data, 0);        // Zero size
        // remove_unreachable_code(data, 1);        // Single byte
        // remove_unreachable_code(data, size);     // Full size
        // remove_unreachable_code(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for remove_unreachable_code
