...
// Targets: uninstall_package in src/package_manager/installer.rs:209
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // uninstall_package(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // uninstall_package(data, 0);        // Zero size
        // uninstall_package(data, 1);        // Single byte
        // uninstall_package(data, size);     // Full size
        // uninstall_package(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for uninstall_package
