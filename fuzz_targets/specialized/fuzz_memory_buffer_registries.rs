...
// Targets: registries in src/package_manager/mutable_state_old.rs:176
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // registries(data, 0);        // Zero size
        // registries(data, 1);        // Single byte
        // registries(data, size);     // Full size
        // registries(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for registries
