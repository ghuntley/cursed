...
// Targets: create_error_response in src/documentation/live_server.rs:360
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // create_error_response(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // create_error_response(data, 0);        // Zero size
        // create_error_response(data, 1);        // Single byte
        // create_error_response(data, size);     // Full size
        // create_error_response(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for create_error_response
