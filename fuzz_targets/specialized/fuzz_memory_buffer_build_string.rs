...
// Targets: build_string in tests/regression/memory/string_operations.csd:21
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // build_string(data, 0);        // Zero size
        // build_string(data, 1);        // Single byte
        // build_string(data, size);     // Full size
        // build_string(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for build_string
