...
// Targets: infer_applied_kind in src/type_system/higher_kinded_types.rs:271
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // infer_applied_kind(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // infer_applied_kind(data, 0);        // Zero size
        // infer_applied_kind(data, 1);        // Single byte
        // infer_applied_kind(data, size);     // Full size
        // infer_applied_kind(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for infer_applied_kind
