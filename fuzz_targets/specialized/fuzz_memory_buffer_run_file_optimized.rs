...
// Targets: run_file_optimized in src/lib.rs:692
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // run_file_optimized(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // run_file_optimized(data, 0);        // Zero size
        // run_file_optimized(data, 1);        // Single byte
        // run_file_optimized(data, size);     // Full size
        // run_file_optimized(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for run_file_optimized
