...
// Targets: test_memory_ops in src/stdlib/sys_core/memory_ops.rs:64
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // test_memory_ops(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // test_memory_ops(data, 0);        // Zero size
        // test_memory_ops(data, 1);        // Single byte
        // test_memory_ops(data, size);     // Full size
        // test_memory_ops(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for test_memory_ops
