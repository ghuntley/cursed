...
// Targets: benchmark_memory_leaks in benchmarks/cursed/compiler/memory_usage_bench.csd:376
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // benchmark_memory_leaks(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // benchmark_memory_leaks(data, 0);        // Zero size
        // benchmark_memory_leaks(data, 1);        // Single byte
        // benchmark_memory_leaks(data, size);     // Full size
        // benchmark_memory_leaks(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for benchmark_memory_leaks
