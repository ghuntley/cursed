...
// Targets: str_starts_with in stdlib/networkz/simple_mod.csd:270
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // str_starts_with(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // str_starts_with(data, 0);        // Zero size
        // str_starts_with(data, 1);        // Single byte
        // str_starts_with(data, size);     // Full size
        // str_starts_with(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for str_starts_with
