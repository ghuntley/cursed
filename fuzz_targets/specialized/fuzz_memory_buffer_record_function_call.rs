...
// Targets: record_function_call in src/optimization/pgo/profile_collector.rs:84
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // record_function_call(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // record_function_call(data, 0);        // Zero size
        // record_function_call(data, 1);        // Single byte
        // record_function_call(data, size);     // Full size
        // record_function_call(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for record_function_call
