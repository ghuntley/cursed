...
// Targets: build_workspace in src/build_system/build_orchestrator.rs:173
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // build_workspace(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // build_workspace(data, 0);        // Zero size
        // build_workspace(data, 1);        // Single byte
        // build_workspace(data, size);     // Full size
        // build_workspace(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for build_workspace
