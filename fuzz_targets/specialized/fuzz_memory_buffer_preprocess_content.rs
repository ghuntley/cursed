...
// Targets: preprocess_content in src/ffi/header_parser.rs:200
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // preprocess_content(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // preprocess_content(data, 0);        // Zero size
        // preprocess_content(data, 1);        // Single byte
        // preprocess_content(data, size);     // Full size
        // preprocess_content(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for preprocess_content
