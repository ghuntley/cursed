...
// Targets: generate_recommendations in src/optimization/optimization_manager.rs:186
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // generate_recommendations(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // generate_recommendations(data, 0);        // Zero size
        // generate_recommendations(data, 1);        // Single byte
        // generate_recommendations(data, size);     // Full size
        // generate_recommendations(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for generate_recommendations
