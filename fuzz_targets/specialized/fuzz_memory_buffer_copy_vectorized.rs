...
// Targets: copy_vectorized in src/runtime/pal/x86_64.rs:1521
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // copy_vectorized(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // copy_vectorized(data, 0);        // Zero size
        // copy_vectorized(data, 1);        // Single byte
        // copy_vectorized(data, size);     // Full size
        // copy_vectorized(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for copy_vectorized
