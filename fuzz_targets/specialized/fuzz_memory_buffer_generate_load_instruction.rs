...
// Targets: generate_load_instruction in src/codegen/llvm/expression_compiler.rs:1180
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // generate_load_instruction(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // generate_load_instruction(data, 0);        // Zero size
        // generate_load_instruction(data, 1);        // Single byte
        // generate_load_instruction(data, size);     // Full size
        // generate_load_instruction(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for generate_load_instruction
