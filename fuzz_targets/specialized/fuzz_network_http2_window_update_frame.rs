...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    // Test packet parsing
    // Test http2_window_update_frame with network data
    // Example: http2_window_update_frame(data, size);
    // Example: http2_window_update_frame(sockfd, data, size);
...
void test_socket_operations(const uint8_t *data, size_t size) {
    ...
    // http2_window_update_frame(sockfd, data, size);
