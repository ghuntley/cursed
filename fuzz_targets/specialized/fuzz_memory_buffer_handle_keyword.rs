...
// Targets: handle_keyword in src/tools/formatter.rs:719
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // handle_keyword(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // handle_keyword(data, 0);        // Zero size
        // handle_keyword(data, 1);        // Single byte
        // handle_keyword(data, size);     // Full size
        // handle_keyword(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for handle_keyword
