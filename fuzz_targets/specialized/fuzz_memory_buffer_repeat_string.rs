...
// Targets: repeat_string in stdlib/stringz_enhanced/mod.csd:14
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // repeat_string(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // repeat_string(data, 0);        // Zero size
        // repeat_string(data, 1);        // Single byte
        // repeat_string(data, size);     // Full size
        // repeat_string(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for repeat_string
