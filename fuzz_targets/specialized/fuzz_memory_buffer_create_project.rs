...
// Targets: create_project in src/build_system/build_orchestrator.rs:370
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // create_project(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // create_project(data, 0);        // Zero size
        // create_project(data, 1);        // Single byte
        // create_project(data, size);     // Full size
        // create_project(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for create_project
