...
// Targets: check_single_constraint in src/type_system/advanced_constraints.rs:245
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // check_single_constraint(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // check_single_constraint(data, 0);        // Zero size
        // check_single_constraint(data, 1);        // Single byte
        // check_single_constraint(data, size);     // Full size
        // check_single_constraint(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for check_single_constraint
