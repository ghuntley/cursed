...
// Targets: write_memory_at_offset in stdlib/typez/mod.csd:627
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // write_memory_at_offset(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // write_memory_at_offset(data, 0);        // Zero size
        // write_memory_at_offset(data, 1);        // Single byte
        // write_memory_at_offset(data, size);     // Full size
        // write_memory_at_offset(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for write_memory_at_offset
