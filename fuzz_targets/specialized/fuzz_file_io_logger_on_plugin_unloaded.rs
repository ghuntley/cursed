...
// Targets: logger_on_plugin_unloaded in stdlib/plugin_system/example_plugin.csd:232
...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    // Test logger_on_plugin_unloaded with temp file
    // Example: logger_on_plugin_unloaded(temp_path);
    // Example: logger_on_plugin_unloaded(temp_path, data, size);
...
void test_path_operations(const uint8_t *data, size_t size) {
    ...
    // logger_on_plugin_unloaded(path);
}


// Additional test functions for logger_on_plugin_unloaded
