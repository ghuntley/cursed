...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    // Test generate_library_bindings with temp file
    // Example: generate_library_bindings(temp_path);
    // Example: generate_library_bindings(temp_path, data, size);
...
void test_path_operations(const uint8_t *data, size_t size) {
    ...
    // generate_library_bindings(path);
}
