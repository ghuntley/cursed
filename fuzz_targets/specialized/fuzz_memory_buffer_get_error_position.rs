...
// Targets: get_error_position in src/lsp/diagnostics.rs:294
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // get_error_position(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // get_error_position(data, 0);        // Zero size
        // get_error_position(data, 1);        // Single byte
        // get_error_position(data, size);     // Full size
        // get_error_position(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for get_error_position
