...
// Targets: compile_binary_expression in src/codegen/llvm/expression_compiler.rs:376
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // compile_binary_expression(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // compile_binary_expression(data, 0);        // Zero size
        // compile_binary_expression(data, 1);        // Single byte
        // compile_binary_expression(data, size);     // Full size
        // compile_binary_expression(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for compile_binary_expression
