...
// Targets: add_debug_data in src/runtime/debug_output.rs:1051
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // add_debug_data(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // add_debug_data(data, 0);        // Zero size
        // add_debug_data(data, 1);        // Single byte
        // add_debug_data(data, size);     // Full size
        // add_debug_data(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for add_debug_data
