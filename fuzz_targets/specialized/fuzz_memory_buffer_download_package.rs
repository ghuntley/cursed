...
// Targets: download_package in src/package_manager/downloader.rs:84
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // download_package(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // download_package(data, 0);        // Zero size
        // download_package(data, 1);        // Single byte
        // download_package(data, size);     // Full size
        // download_package(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for download_package
