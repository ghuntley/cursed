...
// Targets: record_call_stack in src/ffi/profiling.rs:267
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // record_call_stack(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // record_call_stack(data, 0);        // Zero size
        // record_call_stack(data, 1);        // Single byte
        // record_call_stack(data, size);     // Full size
        // record_call_stack(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for record_call_stack
