...
// Targets: run_file in src/lib.rs:558
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // run_file(data, 0);        // Zero size
        // run_file(data, 1);        // Single byte
        // run_file(data, size);     // Full size
        // run_file(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for run_file
