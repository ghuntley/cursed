...
// Targets: get_function_info in src/runtime/debug_manager.rs:557
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // get_function_info(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // get_function_info(data, 0);        // Zero size
        // get_function_info(data, 1);        // Single byte
        // get_function_info(data, size);     // Full size
        // get_function_info(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for get_function_info
