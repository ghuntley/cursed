...
// Targets: update_package in src/package_manager/mutable_state_old.rs:269
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // update_package(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // update_package(data, 0);        // Zero size
        // update_package(data, 1);        // Single byte
        // update_package(data, size);     // Full size
        // update_package(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for update_package
