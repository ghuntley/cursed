...
// Targets: demo_memory_management in examples/chaos_mode_demo.csd:65
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // demo_memory_management(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // demo_memory_management(data, 0);        // Zero size
        // demo_memory_management(data, 1);        // Single byte
        // demo_memory_management(data, size);     // Full size
        // demo_memory_management(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for demo_memory_management
