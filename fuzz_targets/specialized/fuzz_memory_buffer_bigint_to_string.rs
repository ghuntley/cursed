...
// Targets: bigint_to_string in stdlib/big_mood/mod.csd:169
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // bigint_to_string(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // bigint_to_string(data, 0);        // Zero size
        // bigint_to_string(data, 1);        // Single byte
        // bigint_to_string(data, size);     // Full size
        // bigint_to_string(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for bigint_to_string
