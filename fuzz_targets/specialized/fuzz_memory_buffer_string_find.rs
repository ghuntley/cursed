...
// Targets: string_find in stdlib/gob_encode_vibes/mod.csd:384
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // string_find(data, 0);        // Zero size
        // string_find(data, 1);        // Single byte
        // string_find(data, size);     // Full size
        // string_find(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for string_find
