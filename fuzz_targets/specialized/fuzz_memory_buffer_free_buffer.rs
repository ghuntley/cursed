...
// Targets: free_buffer in stdlib/net_real/mod.csd:781
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // free_buffer(data, 0);        // Zero size
        // free_buffer(data, 1);        // Single byte
        // free_buffer(data, size);     // Full size
        // free_buffer(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for free_buffer
