...
// Targets: match_keyword_sequence in src/lexer/mod.rs:551
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // match_keyword_sequence(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // match_keyword_sequence(data, 0);        // Zero size
        // match_keyword_sequence(data, 1);        // Single byte
        // match_keyword_sequence(data, size);     // Full size
        // match_keyword_sequence(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for match_keyword_sequence
