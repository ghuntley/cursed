...
// Targets: stream_create in stdlib/io/mod_comprehensive.csd:999
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // stream_create(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // stream_create(data, 0);        // Zero size
        // stream_create(data, 1);        // Single byte
        // stream_create(data, size);     // Full size
        // stream_create(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for stream_create
