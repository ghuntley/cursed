...
// Targets: to_file in src/formatter/config.rs:125
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // to_file(data, 0);        // Zero size
        // to_file(data, 1);        // Single byte
        // to_file(data, size);     // Full size
        // to_file(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for to_file
