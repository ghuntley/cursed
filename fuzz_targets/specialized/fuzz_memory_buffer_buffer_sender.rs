...
// Targets: buffer_sender in tests/e2e/concurrency/02_channels.csd:45
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // buffer_sender(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // buffer_sender(data, 0);        // Zero size
        // buffer_sender(data, 1);        // Single byte
        // buffer_sender(data, size);     // Full size
        // buffer_sender(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for buffer_sender
