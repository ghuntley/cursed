...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    // Test packet parsing
    // Test http2_update_window with network data
    // Example: http2_update_window(data, size);
    // Example: http2_update_window(sockfd, data, size);
...
void test_socket_operations(const uint8_t *data, size_t size) {
    ...
    // http2_update_window(sockfd, data, size);
