...
// Targets: check_wasm_feature in src/runtime/platform/runtime_detector.rs:507
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // check_wasm_feature(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // check_wasm_feature(data, 0);        // Zero size
        // check_wasm_feature(data, 1);        // Single byte
        // check_wasm_feature(data, size);     // Full size
        // check_wasm_feature(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for check_wasm_feature
