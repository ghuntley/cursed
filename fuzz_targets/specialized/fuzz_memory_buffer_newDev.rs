...
// Targets: newDev in stdlib/package_manager/mod.csd:116
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // newDev(data, 0);        // Zero size
        // newDev(data, 1);        // Single byte
        // newDev(data, size);     // Full size
        // newDev(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for newDev
