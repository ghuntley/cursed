...
// Targets: propagate_panic_to_goroutine in src/runtime/goroutine.rs:1261
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // propagate_panic_to_goroutine(buffer, data, size);
...
void test_string_operations(const uint8_t *data, size_t size) {
    ...
    // propagate_panic_to_goroutine(str);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // propagate_panic_to_goroutine(data, 0);        // Zero size
        // propagate_panic_to_goroutine(data, 1);        // Single byte
        // propagate_panic_to_goroutine(data, size);     // Full size
        // propagate_panic_to_goroutine(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for propagate_panic_to_goroutine
