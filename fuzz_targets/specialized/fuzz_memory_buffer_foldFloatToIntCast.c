...
// Targets: foldFloatToIntCast in src-zig/constant_folder.zig:328
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // foldFloatToIntCast(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // foldFloatToIntCast(data, 0);        // Zero size
        // foldFloatToIntCast(data, 1);        // Single byte
        // foldFloatToIntCast(data, size);     // Full size
        // foldFloatToIntCast(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for foldFloatToIntCast
