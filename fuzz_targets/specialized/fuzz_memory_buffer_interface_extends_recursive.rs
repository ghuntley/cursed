...
// Targets: interface_extends_recursive in src/codegen/llvm/interface_type_checking.rs:413
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // interface_extends_recursive(buffer, data, size);
...
void test_string_operations(const uint8_t *data, size_t size) {
    ...
    // interface_extends_recursive(str);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // interface_extends_recursive(data, 0);        // Zero size
        // interface_extends_recursive(data, 1);        // Single byte
        // interface_extends_recursive(data, size);     // Full size
        // interface_extends_recursive(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for interface_extends_recursive
