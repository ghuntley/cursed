...
// Targets: extract_function_metadata in src/coverage/collector.rs:534
...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    // Test extract_function_metadata with temp file
    // Example: extract_function_metadata(temp_path);
    // Example: extract_function_metadata(temp_path, data, size);
...
void test_path_operations(const uint8_t *data, size_t size) {
    ...
    // extract_function_metadata(path);
}


// Additional test functions for extract_function_metadata
