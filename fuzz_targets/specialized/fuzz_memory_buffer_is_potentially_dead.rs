...
// Targets: is_potentially_dead in src/optimization/simple_passes.rs:144
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // is_potentially_dead(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // is_potentially_dead(data, 0);        // Zero size
        // is_potentially_dead(data, 1);        // Single byte
        // is_potentially_dead(data, size);     // Full size
        // is_potentially_dead(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for is_potentially_dead
