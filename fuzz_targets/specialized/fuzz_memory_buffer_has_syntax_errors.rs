...
// Targets: has_syntax_errors in src/imports/resolver.rs:671
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // has_syntax_errors(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // has_syntax_errors(data, 0);        // Zero size
        // has_syntax_errors(data, 1);        // Single byte
        // has_syntax_errors(data, size);     // Full size
        // has_syntax_errors(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for has_syntax_errors
