...
// Targets: fold_fdiv in src/codegen/llvm/passes/constant_propagation.rs:807
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // fold_fdiv(data, 0);        // Zero size
        // fold_fdiv(data, 1);        // Single byte
        // fold_fdiv(data, size);     // Full size
        // fold_fdiv(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for fold_fdiv
