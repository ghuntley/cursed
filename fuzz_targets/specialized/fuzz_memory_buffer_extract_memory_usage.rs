...
// Targets: extract_memory_usage in src/execution_pipeline.rs:284
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // extract_memory_usage(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // extract_memory_usage(data, 0);        // Zero size
        // extract_memory_usage(data, 1);        // Single byte
        // extract_memory_usage(data, size);     // Full size
        // extract_memory_usage(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for extract_memory_usage
