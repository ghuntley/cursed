...
// Targets: runtime_print_string in src-zig/runtime_functions.zig:312
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // runtime_print_string(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // runtime_print_string(data, 0);        // Zero size
        // runtime_print_string(data, 1);        // Single byte
        // runtime_print_string(data, size);     // Full size
        // runtime_print_string(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for runtime_print_string
