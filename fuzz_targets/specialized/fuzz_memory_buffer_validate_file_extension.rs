...
// Targets: validate_file_extension in src/security/input_validation.rs:253
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // validate_file_extension(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // validate_file_extension(data, 0);        // Zero size
        // validate_file_extension(data, 1);        // Single byte
        // validate_file_extension(data, size);     // Full size
        // validate_file_extension(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for validate_file_extension
