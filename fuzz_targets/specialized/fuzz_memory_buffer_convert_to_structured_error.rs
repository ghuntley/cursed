...
// Targets: convert_to_structured_error in src/error/pipeline_integration.rs:219
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // convert_to_structured_error(buffer, data, size);
...
void test_string_operations(const uint8_t *data, size_t size) {
    ...
    // convert_to_structured_error(str);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // convert_to_structured_error(data, 0);        // Zero size
        // convert_to_structured_error(data, 1);        // Single byte
        // convert_to_structured_error(data, size);     // Full size
        // convert_to_structured_error(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for convert_to_structured_error
