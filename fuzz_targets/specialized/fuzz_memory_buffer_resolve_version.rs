...
// Targets: resolve_version in src/package_manager/resolver.rs:207
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // resolve_version(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // resolve_version(data, 0);        // Zero size
        // resolve_version(data, 1);        // Single byte
        // resolve_version(data, size);     // Full size
        // resolve_version(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for resolve_version
