...
// Targets: copy_memory in stdlib/memoryz/mod.csd:372
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // copy_memory(data, 0);        // Zero size
        // copy_memory(data, 1);        // Single byte
        // copy_memory(data, size);     // Full size
        // copy_memory(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for copy_memory
