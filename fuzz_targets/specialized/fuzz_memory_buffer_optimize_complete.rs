...
// Targets: optimize_complete in src/optimization/optimization_manager.rs:83
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // optimize_complete(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // optimize_complete(data, 0);        // Zero size
        // optimize_complete(data, 1);        // Single byte
        // optimize_complete(data, size);     // Full size
        // optimize_complete(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for optimize_complete
