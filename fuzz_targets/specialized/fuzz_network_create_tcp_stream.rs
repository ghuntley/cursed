...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    // Test packet parsing
    // Test create_tcp_stream with network data
    // Example: create_tcp_stream(data, size);
    // Example: create_tcp_stream(sockfd, data, size);
...
void test_socket_operations(const uint8_t *data, size_t size) {
    ...
    // create_tcp_stream(sockfd, data, size);
