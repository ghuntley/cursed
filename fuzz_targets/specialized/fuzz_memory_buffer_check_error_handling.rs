...
// Targets: check_error_handling in src/tools/linter.rs:1369
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // check_error_handling(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // check_error_handling(data, 0);        // Zero size
        // check_error_handling(data, 1);        // Single byte
        // check_error_handling(data, size);     // Full size
        // check_error_handling(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for check_error_handling
