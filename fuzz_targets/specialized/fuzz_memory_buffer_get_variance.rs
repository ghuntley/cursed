...
// Targets: get_variance in src/type_system/variance.rs:203
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // get_variance(data, 0);        // Zero size
        // get_variance(data, 1);        // Single byte
        // get_variance(data, size);     // Full size
        // get_variance(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for get_variance
