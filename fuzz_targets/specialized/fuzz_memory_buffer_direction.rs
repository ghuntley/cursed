...
// Targets: direction in src/runtime/channels/channel_direction.rs:109
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // direction(data, 0);        // Zero size
        // direction(data, 1);        // Single byte
        // direction(data, size);     // Full size
        // direction(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for direction
