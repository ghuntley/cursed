...
// Targets: split_parameters in src/ffi/header_parser.rs:278
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // split_parameters(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // split_parameters(data, 0);        // Zero size
        // split_parameters(data, 1);        // Single byte
        // split_parameters(data, size);     // Full size
        // split_parameters(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for split_parameters
