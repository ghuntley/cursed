...
// Targets: mul_constraint in src/stdlib/packages/crypto_zk/circuit_builder.rs:131
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // mul_constraint(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // mul_constraint(data, 0);        // Zero size
        // mul_constraint(data, 1);        // Single byte
        // mul_constraint(data, size);     // Full size
        // mul_constraint(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for mul_constraint
