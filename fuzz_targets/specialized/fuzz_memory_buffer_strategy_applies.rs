...
// Targets: strategy_applies in src/runtime/error_propagation.rs:406
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // strategy_applies(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // strategy_applies(data, 0);        // Zero size
        // strategy_applies(data, 1);        // Single byte
        // strategy_applies(data, size);     // Full size
        // strategy_applies(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for strategy_applies
