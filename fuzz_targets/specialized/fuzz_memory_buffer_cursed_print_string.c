...
// Targets: cursed_print_string in src-zig/syscall_interface.zig:38
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // cursed_print_string(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // cursed_print_string(data, 0);        // Zero size
        // cursed_print_string(data, 1);        // Single byte
        // cursed_print_string(data, size);     // Full size
        // cursed_print_string(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for cursed_print_string
