...
// Targets: sprintf in stdlib/panic_system/mod.csd:98
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // sprintf(data, 0);        // Zero size
        // sprintf(data, 1);        // Single byte
        // sprintf(data, size);     // Full size
        // sprintf(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for sprintf
