...
// Targets: calculate_struct_size in stdlib/typez/mod.csd:497
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // calculate_struct_size(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // calculate_struct_size(data, 0);        // Zero size
        // calculate_struct_size(data, 1);        // Single byte
        // calculate_struct_size(data, size);     // Full size
        // calculate_struct_size(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for calculate_struct_size
