...
// Targets: discovery_error in src/stdlib/testing/mod.rs:118
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // discovery_error(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // discovery_error(data, 0);        // Zero size
        // discovery_error(data, 1);        // Single byte
        // discovery_error(data, size);     // Full size
        // discovery_error(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for discovery_error
