...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    // Test packet parsing
    // Test tcp_receive_simple with network data
    // Example: tcp_receive_simple(data, size);
    // Example: tcp_receive_simple(sockfd, data, size);
...
void test_socket_operations(const uint8_t *data, size_t size) {
    ...
    // tcp_receive_simple(sockfd, data, size);
