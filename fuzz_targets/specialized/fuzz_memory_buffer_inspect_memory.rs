...
// Targets: inspect_memory in src/main.rs:3955
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // inspect_memory(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // inspect_memory(data, 0);        // Zero size
        // inspect_memory(data, 1);        // Single byte
        // inspect_memory(data, size);     // Full size
        // inspect_memory(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for inspect_memory
