...
// Targets: cleanup_thread_local_buffers in src/runtime/heap_optimizer_safe.rs:646
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // cleanup_thread_local_buffers(buffer, data, size);
...
void test_string_operations(const uint8_t *data, size_t size) {
    ...
    // cleanup_thread_local_buffers(str);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // cleanup_thread_local_buffers(data, 0);        // Zero size
        // cleanup_thread_local_buffers(data, 1);        // Single byte
        // cleanup_thread_local_buffers(data, size);     // Full size
        // cleanup_thread_local_buffers(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for cleanup_thread_local_buffers
