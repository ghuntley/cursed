...
// Targets: build_summary in src/tools/linter.rs:1190
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // build_summary(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // build_summary(data, 0);        // Zero size
        // build_summary(data, 1);        // Single byte
        // build_summary(data, size);     // Full size
        // build_summary(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for build_summary
