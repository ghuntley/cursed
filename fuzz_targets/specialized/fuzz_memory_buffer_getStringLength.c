...
// Targets: getStringLength in src-zig/reliable_llvm_ir_generator.zig:617
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // getStringLength(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // getStringLength(data, 0);        // Zero size
        // getStringLength(data, 1);        // Single byte
        // getStringLength(data, size);     // Full size
        // getStringLength(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for getStringLength
