...
// Targets: fatalf in src/stdlib/oglogging/mod.rs:43
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // fatalf(data, 0);        // Zero size
        // fatalf(data, 1);        // Single byte
        // fatalf(data, size);     // Full size
        // fatalf(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for fatalf
