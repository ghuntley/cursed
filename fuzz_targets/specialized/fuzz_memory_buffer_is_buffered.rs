...
// Targets: is_buffered in src/runtime/channels/simple_channel.rs:465
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // is_buffered(data, 0);        // Zero size
        // is_buffered(data, 1);        // Single byte
        // is_buffered(data, size);     // Full size
        // is_buffered(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for is_buffered
