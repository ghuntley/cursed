...
// Targets: string_capitalize in stdlib/string_simple/mod.csd:352
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // string_capitalize(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // string_capitalize(data, 0);        // Zero size
        // string_capitalize(data, 1);        // Single byte
        // string_capitalize(data, size);     // Full size
        // string_capitalize(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for string_capitalize
