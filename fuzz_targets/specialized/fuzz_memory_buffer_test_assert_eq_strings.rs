...
// Targets: test_assert_eq_strings in tests/testz/test_basic_assertions.csd:74
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // test_assert_eq_strings(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // test_assert_eq_strings(data, 0);        // Zero size
        // test_assert_eq_strings(data, 1);        // Single byte
        // test_assert_eq_strings(data, size);     // Full size
        // test_assert_eq_strings(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for test_assert_eq_strings
