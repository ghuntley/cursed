...
// Targets: to_lowercase in stdlib/envz/mod.csd:280
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // to_lowercase(data, 0);        // Zero size
        // to_lowercase(data, 1);        // Single byte
        // to_lowercase(data, size);     // Full size
        // to_lowercase(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for to_lowercase
