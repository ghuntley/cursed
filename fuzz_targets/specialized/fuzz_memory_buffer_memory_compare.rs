...
// Targets: memory_compare in stdlib/memory/mod.csd:133
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // memory_compare(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // memory_compare(data, 0);        // Zero size
        // memory_compare(data, 1);        // Single byte
        // memory_compare(data, size);     // Full size
        // memory_compare(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for memory_compare
