...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    // Test execute_shook_propagation with temp file
    // Example: execute_shook_propagation(temp_path);
    // Example: execute_shook_propagation(temp_path, data, size);
...
void test_path_operations(const uint8_t *data, size_t size) {
    ...
    // execute_shook_propagation(path);
}
