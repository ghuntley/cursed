...
// Targets: str_char_at in stdlib/networkz/simple_mod.csd:324
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // str_char_at(data, 0);        // Zero size
        // str_char_at(data, 1);        // Single byte
        // str_char_at(data, size);     // Full size
        // str_char_at(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for str_char_at
