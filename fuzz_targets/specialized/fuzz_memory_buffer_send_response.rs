...
// Targets: send_response in src/documentation/live_server.rs:389
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // send_response(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // send_response(data, 0);        // Zero size
        // send_response(data, 1);        // Single byte
        // send_response(data, size);     // Full size
        // send_response(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for send_response
