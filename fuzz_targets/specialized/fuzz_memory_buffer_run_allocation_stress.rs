...
// Targets: run_allocation_stress in tests/gc_concurrency_stress_test.csd:15
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // run_allocation_stress(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // run_allocation_stress(data, 0);        // Zero size
        // run_allocation_stress(data, 1);        // Single byte
        // run_allocation_stress(data, size);     // Full size
        // run_allocation_stress(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for run_allocation_stress
