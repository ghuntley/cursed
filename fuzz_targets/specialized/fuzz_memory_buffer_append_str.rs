...
// Targets: append_str in benchmarks/cursed/compiler/memory_usage_bench.csd:496
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // append_str(data, 0);        // Zero size
        // append_str(data, 1);        // Single byte
        // append_str(data, size);     // Full size
        // append_str(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for append_str
