...
// Targets: add_line in src/tools/formatter.rs:748
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // add_line(data, 0);        // Zero size
        // add_line(data, 1);        // Single byte
        // add_line(data, size);     // Full size
        // add_line(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for add_line
