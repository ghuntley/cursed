...
// Targets: recv in src/runtime/channels/buffer.rs:270
...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    // Test packet parsing
    // Test recv with network data
    // Example: recv(data, size);
    // Example: recv(sockfd, data, size);
...
void test_socket_operations(const uint8_t *data, size_t size) {
    ...
    // recv(sockfd, data, size);
    
    close(sockfd);
}


// Additional test functions for recv
