...
// Targets: OptimalBufferSize in stdlib/slay_io/mod.csd:245
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // OptimalBufferSize(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // OptimalBufferSize(data, 0);        // Zero size
        // OptimalBufferSize(data, 1);        // Single byte
        // OptimalBufferSize(data, size);     // Full size
        // OptimalBufferSize(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for OptimalBufferSize
