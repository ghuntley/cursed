...
// Targets: runtime_free_memory in src-zig/runtime_functions.zig:301
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // runtime_free_memory(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // runtime_free_memory(data, 0);        // Zero size
        // runtime_free_memory(data, 1);        // Single byte
        // runtime_free_memory(data, size);     // Full size
        // runtime_free_memory(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for runtime_free_memory
