...
// Targets: generate_variable_debug in src/tools/debug_info.rs:256
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // generate_variable_debug(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // generate_variable_debug(data, 0);        // Zero size
        // generate_variable_debug(data, 1);        // Single byte
        // generate_variable_debug(data, size);     // Full size
        // generate_variable_debug(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for generate_variable_debug
