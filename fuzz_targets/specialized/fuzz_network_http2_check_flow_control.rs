...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    // Test packet parsing
    // Test http2_check_flow_control with network data
    // Example: http2_check_flow_control(data, size);
    // Example: http2_check_flow_control(sockfd, data, size);
...
void test_socket_operations(const uint8_t *data, size_t size) {
    ...
    // http2_check_flow_control(sockfd, data, size);
