...
// Targets: parse_memory_size in src/runtime/pal/arm64.rs:483
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // parse_memory_size(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // parse_memory_size(data, 0);        // Zero size
        // parse_memory_size(data, 1);        // Single byte
        // parse_memory_size(data, size);     // Full size
        // parse_memory_size(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for parse_memory_size
