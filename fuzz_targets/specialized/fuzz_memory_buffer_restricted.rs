...
// Targets: restricted in src/runtime/process.rs:767
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // restricted(data, 0);        // Zero size
        // restricted(data, 1);        // Single byte
        // restricted(data, size);     // Full size
        // restricted(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for restricted
