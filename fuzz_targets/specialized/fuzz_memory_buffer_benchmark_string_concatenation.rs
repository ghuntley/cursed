...
// Targets: benchmark_string_concatenation in benchmarks/cursed/stdlib/string_bench.csd:43
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // benchmark_string_concatenation(buffer, data, size);
...
void test_string_operations(const uint8_t *data, size_t size) {
    ...
    // benchmark_string_concatenation(str);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // benchmark_string_concatenation(data, 0);        // Zero size
        // benchmark_string_concatenation(data, 1);        // Single byte
        // benchmark_string_concatenation(data, size);     // Full size
        // benchmark_string_concatenation(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for benchmark_string_concatenation
