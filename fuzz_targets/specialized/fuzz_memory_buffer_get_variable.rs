...
// Targets: get_variable in src/codegen/llvm/expression_compiler.rs:1149
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // get_variable(data, 0);        // Zero size
        // get_variable(data, 1);        // Single byte
        // get_variable(data, size);     // Full size
        // get_variable(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for get_variable
