...
// Targets: generate_dwarf_info in src/runtime/debug_info.rs:1805
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // generate_dwarf_info(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // generate_dwarf_info(data, 0);        // Zero size
        // generate_dwarf_info(data, 1);        // Single byte
        // generate_dwarf_info(data, size);     // Full size
        // generate_dwarf_info(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for generate_dwarf_info
