...
// Targets: on_function_entry in src/runtime/performance_hooks.rs:429
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // on_function_entry(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // on_function_entry(data, 0);        // Zero size
        // on_function_entry(data, 1);        // Single byte
        // on_function_entry(data, size);     // Full size
        // on_function_entry(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for on_function_entry
