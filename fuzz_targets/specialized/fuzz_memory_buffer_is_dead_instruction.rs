...
// Targets: is_dead_instruction in src/codegen/llvm/passes/dead_code_elimination.rs:58
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // is_dead_instruction(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // is_dead_instruction(data, 0);        // Zero size
        // is_dead_instruction(data, 1);        // Single byte
        // is_dead_instruction(data, size);     // Full size
        // is_dead_instruction(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for is_dead_instruction
