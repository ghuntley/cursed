...
// Targets: header in src/stdlib/net/websocket/client.rs:60
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // header(data, 0);        // Zero size
        // header(data, 1);        // Single byte
        // header(data, size);     // Full size
        // header(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for header
