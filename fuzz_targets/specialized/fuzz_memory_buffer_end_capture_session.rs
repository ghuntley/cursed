...
// Targets: end_capture_session in src/runtime/debug_info.rs:289
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // end_capture_session(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // end_capture_session(data, 0);        // Zero size
        // end_capture_session(data, 1);        // Single byte
        // end_capture_session(data, size);     // Full size
        // end_capture_session(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for end_capture_session
