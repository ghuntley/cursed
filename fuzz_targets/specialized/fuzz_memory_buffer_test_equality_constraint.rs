...
// Targets: test_equality_constraint in src/type_system/advanced_constraints.rs:552
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // test_equality_constraint(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // test_equality_constraint(data, 0);        // Zero size
        // test_equality_constraint(data, 1);        // Single byte
        // test_equality_constraint(data, size);     // Full size
        // test_equality_constraint(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for test_equality_constraint
