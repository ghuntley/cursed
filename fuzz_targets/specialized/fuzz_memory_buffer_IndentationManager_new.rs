...
// Targets: IndentationManager_new in stdlib/string_enhanced/mod.csd:355
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // IndentationManager_new(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // IndentationManager_new(data, 0);        // Zero size
        // IndentationManager_new(data, 1);        // Single byte
        // IndentationManager_new(data, size);     // Full size
        // IndentationManager_new(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for IndentationManager_new
