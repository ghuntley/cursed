...
// Targets: demonstrate_variables in examples/docs_demo.csd:40
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // demonstrate_variables(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // demonstrate_variables(data, 0);        // Zero size
        // demonstrate_variables(data, 1);        // Single byte
        // demonstrate_variables(data, size);     // Full size
        // demonstrate_variables(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for demonstrate_variables
