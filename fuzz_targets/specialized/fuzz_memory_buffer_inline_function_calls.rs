...
// Targets: inline_function_calls in src/optimization/link_time_optimization.rs:779
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // inline_function_calls(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // inline_function_calls(data, 0);        // Zero size
        // inline_function_calls(data, 1);        // Single byte
        // inline_function_calls(data, size);     // Full size
        // inline_function_calls(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for inline_function_calls
