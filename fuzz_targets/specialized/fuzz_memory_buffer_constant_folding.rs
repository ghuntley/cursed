...
// Targets: constant_folding in src/codegen/llvm/generic_optimization.rs:508
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // constant_folding(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // constant_folding(data, 0);        // Zero size
        // constant_folding(data, 1);        // Single byte
        // constant_folding(data, size);     // Full size
        // constant_folding(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for constant_folding
