...
// Targets: register_converter in src/ffi/type_mapping.rs:277
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // register_converter(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // register_converter(data, 0);        // Zero size
        // register_converter(data, 1);        // Single byte
        // register_converter(data, size);     // Full size
        // register_converter(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for register_converter
