...
// Targets: lint_source in src/linter/mod.rs:122
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // lint_source(data, 0);        // Zero size
        // lint_source(data, 1);        // Single byte
        // lint_source(data, size);     // Full size
        // lint_source(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for lint_source
