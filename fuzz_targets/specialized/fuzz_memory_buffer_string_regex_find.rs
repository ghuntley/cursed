...
// Targets: string_regex_find in src/execution/runtime_functions.rs:3571
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // string_regex_find(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // string_regex_find(data, 0);        // Zero size
        // string_regex_find(data, 1);        // Single byte
        // string_regex_find(data, size);     // Full size
        // string_regex_find(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for string_regex_find
