...
// Targets: remove_symbol in src/lsp/completion.rs:278
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // remove_symbol(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // remove_symbol(data, 0);        // Zero size
        // remove_symbol(data, 1);        // Single byte
        // remove_symbol(data, size);     // Full size
        // remove_symbol(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for remove_symbol
