...
// Targets: demonstrate_constants in examples/stdlib_examples/mathz_example.csd:8
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // demonstrate_constants(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // demonstrate_constants(data, 0);        // Zero size
        // demonstrate_constants(data, 1);        // Single byte
        // demonstrate_constants(data, size);     // Full size
        // demonstrate_constants(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for demonstrate_constants
