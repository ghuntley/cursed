...
// Targets: emit_deprecation_warning in src/parser_main.rs:4390
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // emit_deprecation_warning(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // emit_deprecation_warning(data, 0);        // Zero size
        // emit_deprecation_warning(data, 1);        // Single byte
        // emit_deprecation_warning(data, size);     // Full size
        // emit_deprecation_warning(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for emit_deprecation_warning
