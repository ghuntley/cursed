...
// Targets: demonstrate_collections in examples/generics/collections.csd:439
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // demonstrate_collections(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // demonstrate_collections(data, 0);        // Zero size
        // demonstrate_collections(data, 1);        // Single byte
        // demonstrate_collections(data, size);     // Full size
        // demonstrate_collections(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for demonstrate_collections
