...
// Targets: is_channel_ready_for_receive in src/runtime/channels/enhanced_select.rs:715
...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    // Test is_channel_ready_for_receive with temp file
    // Example: is_channel_ready_for_receive(temp_path);
    // Example: is_channel_ready_for_receive(temp_path, data, size);
...
void test_path_operations(const uint8_t *data, size_t size) {
    ...
    // is_channel_ready_for_receive(path);
}


// Additional test functions for is_channel_ready_for_receive
