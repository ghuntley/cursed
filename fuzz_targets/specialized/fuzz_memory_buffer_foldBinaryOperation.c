...
// Targets: foldBinaryOperation in src-zig/constant_folder.zig:145
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // foldBinaryOperation(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // foldBinaryOperation(data, 0);        // Zero size
        // foldBinaryOperation(data, 1);        // Single byte
        // foldBinaryOperation(data, size);     // Full size
        // foldBinaryOperation(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for foldBinaryOperation
