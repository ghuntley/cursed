...
// Targets: ends_with in stdlib/io/mod.csd:291
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // ends_with(data, 0);        // Zero size
        // ends_with(data, 1);        // Single byte
        // ends_with(data, size);     // Full size
        // ends_with(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for ends_with
