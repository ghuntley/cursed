...
// Targets: optimize_call in src/ffi/performance.rs:316
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // optimize_call(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // optimize_call(data, 0);        // Zero size
        // optimize_call(data, 1);        // Single byte
        // optimize_call(data, size);     // Full size
        // optimize_call(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for optimize_call
