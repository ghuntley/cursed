...
// Targets: show_memory_stats in examples/vibecheck_demo.csd:8
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // show_memory_stats(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // show_memory_stats(data, 0);        // Zero size
        // show_memory_stats(data, 1);        // Single byte
        // show_memory_stats(data, size);     // Full size
        // show_memory_stats(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for show_memory_stats
