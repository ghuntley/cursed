...
// Targets: should_log in src/runtime/debug_output.rs:474
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // should_log(data, 0);        // Zero size
        // should_log(data, 1);        // Single byte
        // should_log(data, size);     // Full size
        // should_log(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for should_log
