...
// Targets: analyze_type_usage in src/type_system/variance.rs:101
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // analyze_type_usage(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // analyze_type_usage(data, 0);        // Zero size
        // analyze_type_usage(data, 1);        // Single byte
        // analyze_type_usage(data, size);     // Full size
        // analyze_type_usage(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for analyze_type_usage
