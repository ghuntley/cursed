...
// Targets: large_struct_function in benchmarks/cursed/language_features/function_call_bench.csd:187
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // large_struct_function(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // large_struct_function(data, 0);        // Zero size
        // large_struct_function(data, 1);        // Single byte
        // large_struct_function(data, size);     // Full size
        // large_struct_function(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for large_struct_function
