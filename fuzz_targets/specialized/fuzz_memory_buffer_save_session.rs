...
// Targets: save_session in src/repl/session_manager.rs:44
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // save_session(data, 0);        // Zero size
        // save_session(data, 1);        // Single byte
        // save_session(data, size);     // Full size
        // save_session(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for save_session
