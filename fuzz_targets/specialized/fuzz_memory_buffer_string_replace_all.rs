...
// Targets: string_replace_all in stdlib/encoding_flex/mod_enhanced.csd:564
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // string_replace_all(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // string_replace_all(data, 0);        // Zero size
        // string_replace_all(data, 1);        // Single byte
        // string_replace_all(data, size);     // Full size
        // string_replace_all(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for string_replace_all
