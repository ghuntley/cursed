...
// Targets: error in src/type_system/test_result_simple.rs:209
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // error(data, 0);        // Zero size
        // error(data, 1);        // Single byte
        // error(data, size);     // Full size
        // error(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for error
