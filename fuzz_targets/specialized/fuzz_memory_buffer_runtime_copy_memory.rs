...
// Targets: runtime_copy_memory in stdlib/memoryz/mod.csd:480
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // runtime_copy_memory(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // runtime_copy_memory(data, 0);        // Zero size
        // runtime_copy_memory(data, 1);        // Single byte
        // runtime_copy_memory(data, size);     // Full size
        // runtime_copy_memory(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for runtime_copy_memory
