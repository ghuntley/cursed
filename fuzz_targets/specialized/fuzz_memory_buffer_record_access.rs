...
// Targets: record_access in src/runtime/gc_race_detector.rs:165
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // record_access(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // record_access(data, 0);        // Zero size
        // record_access(data, 1);        // Single byte
        // record_access(data, size);     // Full size
        // record_access(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for record_access
