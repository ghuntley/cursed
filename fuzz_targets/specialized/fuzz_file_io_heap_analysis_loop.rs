...
// Targets: heap_analysis_loop in src/memory/profiling.rs:533
...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    // Test heap_analysis_loop with temp file
    // Example: heap_analysis_loop(temp_path);
    // Example: heap_analysis_loop(temp_path, data, size);
...
void test_path_operations(const uint8_t *data, size_t size) {
    ...
    // heap_analysis_loop(path);
}


// Additional test functions for heap_analysis_loop
