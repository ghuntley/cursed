...
// Targets: text in src/stdlib/net/websocket/message.rs:11
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // text(data, 0);        // Zero size
        // text(data, 1);        // Single byte
        // text(data, size);     // Full size
        // text(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for text
