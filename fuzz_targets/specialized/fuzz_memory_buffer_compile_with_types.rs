...
// Targets: compile_with_types in src/type_system/compilation_integration.rs:82
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // compile_with_types(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // compile_with_types(data, 0);        // Zero size
        // compile_with_types(data, 1);        // Single byte
        // compile_with_types(data, size);     // Full size
        // compile_with_types(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for compile_with_types
