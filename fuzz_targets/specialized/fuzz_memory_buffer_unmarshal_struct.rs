...
// Targets: unmarshal_struct in src/ffi/type_mapping.rs:364
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // unmarshal_struct(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // unmarshal_struct(data, 0);        // Zero size
        // unmarshal_struct(data, 1);        // Single byte
        // unmarshal_struct(data, size);     // Full size
        // unmarshal_struct(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for unmarshal_struct
