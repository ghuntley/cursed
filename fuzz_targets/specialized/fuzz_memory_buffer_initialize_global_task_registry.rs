...
// Targets: initialize_global_task_registry in src/runtime/async/task.rs:413
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // initialize_global_task_registry(buffer, data, size);
...
void test_string_operations(const uint8_t *data, size_t size) {
    ...
    // initialize_global_task_registry(str);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // initialize_global_task_registry(data, 0);        // Zero size
        // initialize_global_task_registry(data, 1);        // Single byte
        // initialize_global_task_registry(data, size);     // Full size
        // initialize_global_task_registry(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for initialize_global_task_registry
