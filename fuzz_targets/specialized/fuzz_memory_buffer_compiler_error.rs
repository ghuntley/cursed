...
// Targets: compiler_error in src/error/mod.rs:214
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // compiler_error(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // compiler_error(data, 0);        // Zero size
        // compiler_error(data, 1);        // Single byte
        // compiler_error(data, size);     // Full size
        // compiler_error(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for compiler_error
