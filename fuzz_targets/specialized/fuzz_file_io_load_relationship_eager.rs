...
// Targets: load_relationship_eager in stdlib/database_orm/mod.csd:464
...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    // Test load_relationship_eager with temp file
    // Example: load_relationship_eager(temp_path);
    // Example: load_relationship_eager(temp_path, data, size);
...
void test_path_operations(const uint8_t *data, size_t size) {
    ...
    // load_relationship_eager(path);
}


// Additional test functions for load_relationship_eager
