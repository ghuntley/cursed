...
// Targets: create_struct_debug_type in src/tools/debug_info.rs:491
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // create_struct_debug_type(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // create_struct_debug_type(data, 0);        // Zero size
        // create_struct_debug_type(data, 1);        // Single byte
        // create_struct_debug_type(data, size);     // Full size
        // create_struct_debug_type(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for create_struct_debug_type
