...
// Targets: number_to_string_real in stdlib/vibez/enhanced_formatting.csd:70
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // number_to_string_real(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // number_to_string_real(data, 0);        // Zero size
        // number_to_string_real(data, 1);        // Single byte
        // number_to_string_real(data, size);     // Full size
        // number_to_string_real(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for number_to_string_real
