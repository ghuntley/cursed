...
// Targets: generatePrintfCallStringLiteral in src-zig/debug_enabled_codegen.zig:496
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // generatePrintfCallStringLiteral(buffer, data, size);
...
void test_string_operations(const uint8_t *data, size_t size) {
    ...
    // generatePrintfCallStringLiteral(str);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // generatePrintfCallStringLiteral(data, 0);        // Zero size
        // generatePrintfCallStringLiteral(data, 1);        // Single byte
        // generatePrintfCallStringLiteral(data, size);     // Full size
        // generatePrintfCallStringLiteral(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for generatePrintfCallStringLiteral
