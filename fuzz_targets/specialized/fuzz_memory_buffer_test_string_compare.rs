...
// Targets: test_string_compare in stdlib/sorta_fresh/test_basic.csd:17
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // test_string_compare(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // test_string_compare(data, 0);        // Zero size
        // test_string_compare(data, 1);        // Single byte
        // test_string_compare(data, size);     // Full size
        // test_string_compare(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for test_string_compare
