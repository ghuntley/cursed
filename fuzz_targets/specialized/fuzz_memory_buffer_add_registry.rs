...
// Targets: add_registry in src/package_manager/mutable_state_old.rs:181
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // add_registry(data, 0);        // Zero size
        // add_registry(data, 1);        // Single byte
        // add_registry(data, size);     // Full size
        // add_registry(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for add_registry
