...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // update_function_profile(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // update_function_profile(data, 0);        // Zero size
        // update_function_profile(data, 1);        // Single byte
        // update_function_profile(data, size);     // Full size
        // update_function_profile(NULL, 0);        // NULL pointer
    }
}
