...
// Targets: run_all_memory_benchmarks in benchmarks/cursed/compiler/memory_usage_bench.csd:518
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // run_all_memory_benchmarks(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // run_all_memory_benchmarks(data, 0);        // Zero size
        // run_all_memory_benchmarks(data, 1);        // Single byte
        // run_all_memory_benchmarks(data, size);     // Full size
        // run_all_memory_benchmarks(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for run_all_memory_benchmarks
