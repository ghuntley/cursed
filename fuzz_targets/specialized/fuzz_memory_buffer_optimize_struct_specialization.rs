...
// Targets: optimize_struct_specialization in src/codegen/llvm/generic_optimization.rs:374
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // optimize_struct_specialization(buffer, data, size);
...
void test_string_operations(const uint8_t *data, size_t size) {
    ...
    // optimize_struct_specialization(str);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // optimize_struct_specialization(data, 0);        // Zero size
        // optimize_struct_specialization(data, 1);        // Single byte
        // optimize_struct_specialization(data, size);     // Full size
        // optimize_struct_specialization(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for optimize_struct_specialization
