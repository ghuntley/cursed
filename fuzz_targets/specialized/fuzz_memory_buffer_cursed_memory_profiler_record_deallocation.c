...
// Targets: cursed_memory_profiler_record_deallocation in src-zig/memory_profiler_aggregation.zig:696
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // cursed_memory_profiler_record_deallocation(buffer, data, size);
...
void test_string_operations(const uint8_t *data, size_t size) {
    ...
    // cursed_memory_profiler_record_deallocation(str);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // cursed_memory_profiler_record_deallocation(data, 0);        // Zero size
        // cursed_memory_profiler_record_deallocation(data, 1);        // Single byte
        // cursed_memory_profiler_record_deallocation(data, size);     // Full size
        // cursed_memory_profiler_record_deallocation(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for cursed_memory_profiler_record_deallocation
