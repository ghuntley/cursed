...
// Targets: truncate_string in stdlib/stringz/mod.csd:796
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // truncate_string(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // truncate_string(data, 0);        // Zero size
        // truncate_string(data, 1);        // Single byte
        // truncate_string(data, size);     // Full size
        // truncate_string(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for truncate_string
