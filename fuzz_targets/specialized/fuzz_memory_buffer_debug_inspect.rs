...
// Targets: debug_inspect in src/stdlib/vibez/debug.rs:349
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // debug_inspect(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // debug_inspect(data, 0);        // Zero size
        // debug_inspect(data, 1);        // Single byte
        // debug_inspect(data, size);     // Full size
        // debug_inspect(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for debug_inspect
