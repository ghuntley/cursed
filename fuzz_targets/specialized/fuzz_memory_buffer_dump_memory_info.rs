...
// Targets: dump_memory_info in src/runtime/memory.rs:668
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // dump_memory_info(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // dump_memory_info(data, 0);        // Zero size
        // dump_memory_info(data, 1);        // Single byte
        // dump_memory_info(data, size);     // Full size
        // dump_memory_info(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for dump_memory_info
