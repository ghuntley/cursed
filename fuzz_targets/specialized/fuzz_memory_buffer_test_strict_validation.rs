...
// Targets: test_strict_validation in src/metrics/prometheus_test.rs:144
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // test_strict_validation(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // test_strict_validation(data, 0);        // Zero size
        // test_strict_validation(data, 1);        // Single byte
        // test_strict_validation(data, size);     // Full size
        // test_strict_validation(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for test_strict_validation
