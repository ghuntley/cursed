...
// Targets: cursed_memory_profiler_cleanup in src-zig/memory_profiler_aggregation.zig:705
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // cursed_memory_profiler_cleanup(buffer, data, size);
...
void test_string_operations(const uint8_t *data, size_t size) {
    ...
    // cursed_memory_profiler_cleanup(str);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // cursed_memory_profiler_cleanup(data, 0);        // Zero size
        // cursed_memory_profiler_cleanup(data, 1);        // Single byte
        // cursed_memory_profiler_cleanup(data, size);     // Full size
        // cursed_memory_profiler_cleanup(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for cursed_memory_profiler_cleanup
