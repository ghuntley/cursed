...
// Targets: digest_message in src/stdlib/packages/crypto_signatures/message_digest.rs:127
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // digest_message(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // digest_message(data, 0);        // Zero size
        // digest_message(data, 1);        // Single byte
        // digest_message(data, size);     // Full size
        // digest_message(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for digest_message
