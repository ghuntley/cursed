...
// Targets: get_function_count in src/optimization/pgo/profile_collector.rs:229
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // get_function_count(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // get_function_count(data, 0);        // Zero size
        // get_function_count(data, 1);        // Single byte
        // get_function_count(data, size);     // Full size
        // get_function_count(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for get_function_count
