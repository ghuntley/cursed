...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    // Test packet parsing
    // Test http2_send_response with network data
    // Example: http2_send_response(data, size);
    // Example: http2_send_response(sockfd, data, size);
...
void test_socket_operations(const uint8_t *data, size_t size) {
    ...
    // http2_send_response(sockfd, data, size);
