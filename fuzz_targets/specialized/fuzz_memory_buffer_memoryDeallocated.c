...
// Targets: memoryDeallocated in src-zig/performance_hooks.zig:677
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // memoryDeallocated(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // memoryDeallocated(data, 0);        // Zero size
        // memoryDeallocated(data, 1);        // Single byte
        // memoryDeallocated(data, size);     // Full size
        // memoryDeallocated(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for memoryDeallocated
