...
// Targets: vibez_sprintf in src/execution/runtime_functions.rs:5578
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // vibez_sprintf(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // vibez_sprintf(data, 0);        // Zero size
        // vibez_sprintf(data, 1);        // Single byte
        // vibez_sprintf(data, size);     // Full size
        // vibez_sprintf(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for vibez_sprintf
