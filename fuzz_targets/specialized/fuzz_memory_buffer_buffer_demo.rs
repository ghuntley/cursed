...
// Targets: buffer_demo in examples/bytefit_demo.csd:172
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // buffer_demo(data, 0);        // Zero size
        // buffer_demo(data, 1);        // Single byte
        // buffer_demo(data, size);     // Full size
        // buffer_demo(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for buffer_demo
