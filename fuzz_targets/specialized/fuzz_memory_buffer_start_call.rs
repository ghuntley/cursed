...
// Targets: start_call in src/ffi/profiling.rs:206
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // start_call(data, 0);        // Zero size
        // start_call(data, 1);        // Single byte
        // start_call(data, size);     // Full size
        // start_call(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for start_call
