...
// Targets: test_string_literals in tools/formatter/test_formatter.csd:88
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // test_string_literals(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // test_string_literals(data, 0);        // Zero size
        // test_string_literals(data, 1);        // Single byte
        // test_string_literals(data, size);     // Full size
        // test_string_literals(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for test_string_literals
