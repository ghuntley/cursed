...
// Targets: marshal_struct in src/ffi/type_mapping.rs:350
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // marshal_struct(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // marshal_struct(data, 0);        // Zero size
        // marshal_struct(data, 1);        // Single byte
        // marshal_struct(data, size);     // Full size
        // marshal_struct(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for marshal_struct
