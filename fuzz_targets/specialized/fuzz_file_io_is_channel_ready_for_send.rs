...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    // Test is_channel_ready_for_send with temp file
    // Example: is_channel_ready_for_send(temp_path);
    // Example: is_channel_ready_for_send(temp_path, data, size);
...
void test_path_operations(const uint8_t *data, size_t size) {
    ...
    // is_channel_ready_for_send(path);
}
