...
// Targets: remove_unused_variables in src/codegen/llvm/generic_optimization.rs:422
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // remove_unused_variables(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // remove_unused_variables(data, 0);        // Zero size
        // remove_unused_variables(data, 1);        // Single byte
        // remove_unused_variables(data, size);     // Full size
        // remove_unused_variables(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for remove_unused_variables
