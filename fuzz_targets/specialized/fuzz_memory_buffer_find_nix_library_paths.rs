...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // find_nix_library_paths(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // find_nix_library_paths(data, 0);        // Zero size
        // find_nix_library_paths(data, 1);        // Single byte
        // find_nix_library_paths(data, size);     // Full size
        // find_nix_library_paths(NULL, 0);        // NULL pointer
    }
}
