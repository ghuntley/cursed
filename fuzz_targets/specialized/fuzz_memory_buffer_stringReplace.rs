...
// Targets: stringReplace in stdlib/tag_core/mod.csd:42
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // stringReplace(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // stringReplace(data, 0);        // Zero size
        // stringReplace(data, 1);        // Single byte
        // stringReplace(data, size);     // Full size
        // stringReplace(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for stringReplace
