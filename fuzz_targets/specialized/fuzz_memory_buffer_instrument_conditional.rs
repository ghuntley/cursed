...
// Targets: instrument_conditional in src/coverage/instrumentation.rs:306
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // instrument_conditional(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // instrument_conditional(data, 0);        // Zero size
        // instrument_conditional(data, 1);        // Single byte
        // instrument_conditional(data, size);     // Full size
        // instrument_conditional(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for instrument_conditional
