...
// Targets: setBuffer in src-zig/windows_iocp_poller.zig:131
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // setBuffer(data, 0);        // Zero size
        // setBuffer(data, 1);        // Single byte
        // setBuffer(data, size);     // Full size
        // setBuffer(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for setBuffer
