...
// Targets: compile_to_wasm_with_optimizations in src/lib.rs:1341
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // compile_to_wasm_with_optimizations(buffer, data, size);
...
void test_string_operations(const uint8_t *data, size_t size) {
    ...
    // compile_to_wasm_with_optimizations(str);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // compile_to_wasm_with_optimizations(data, 0);        // Zero size
        // compile_to_wasm_with_optimizations(data, 1);        // Single byte
        // compile_to_wasm_with_optimizations(data, size);     // Full size
        // compile_to_wasm_with_optimizations(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for compile_to_wasm_with_optimizations
