...
// Targets: is_interface_method in src/codegen/llvm/passes/inlining_fixed.rs:253
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // is_interface_method(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // is_interface_method(data, 0);        // Zero size
        // is_interface_method(data, 1);        // Single byte
        // is_interface_method(data, size);     // Full size
        // is_interface_method(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for is_interface_method
