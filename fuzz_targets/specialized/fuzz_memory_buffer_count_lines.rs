...
// Targets: count_lines in src/bin/cursed_coverage.rs:269
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // count_lines(data, 0);        // Zero size
        // count_lines(data, 1);        // Single byte
        // count_lines(data, size);     // Full size
        // count_lines(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for count_lines
