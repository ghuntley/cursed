...
// Targets: remove_thread_root in src/memory/root_set.rs:245
...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    // Test remove_thread_root with temp file
    // Example: remove_thread_root(temp_path);
    // Example: remove_thread_root(temp_path, data, size);
...
void test_path_operations(const uint8_t *data, size_t size) {
    ...
    // remove_thread_root(path);
}


// Additional test functions for remove_thread_root
