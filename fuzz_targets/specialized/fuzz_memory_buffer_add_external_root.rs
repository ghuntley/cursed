...
// Targets: add_external_root in src/memory/root_set.rs:356
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // add_external_root(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // add_external_root(data, 0);        // Zero size
        // add_external_root(data, 1);        // Single byte
        // add_external_root(data, size);     // Full size
        // add_external_root(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for add_external_root
