...
// Targets: escape_json_string in stdlib/json/mod.csd:281
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // escape_json_string(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // escape_json_string(data, 0);        // Zero size
        // escape_json_string(data, 1);        // Single byte
        // escape_json_string(data, size);     // Full size
        // escape_json_string(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for escape_json_string
