...
// Targets: string_compare_pure in stdlib/self_hosted_builtins/mod.csd:38
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // string_compare_pure(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // string_compare_pure(data, 0);        // Zero size
        // string_compare_pure(data, 1);        // Single byte
        // string_compare_pure(data, size);     // Full size
        // string_compare_pure(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for string_compare_pure
