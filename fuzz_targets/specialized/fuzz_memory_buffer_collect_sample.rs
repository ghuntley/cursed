...
// Targets: collect_sample in src/codegen/llvm/jit_engine.rs:615
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // collect_sample(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // collect_sample(data, 0);        // Zero size
        // collect_sample(data, 1);        // Single byte
        // collect_sample(data, size);     // Full size
        // collect_sample(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for collect_sample
