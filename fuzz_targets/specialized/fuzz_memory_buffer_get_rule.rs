...
// Targets: get_rule in src/tools/linter.rs:543
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // get_rule(data, 0);        // Zero size
        // get_rule(data, 1);        // Single byte
        // get_rule(data, size);     // Full size
        // get_rule(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for get_rule
