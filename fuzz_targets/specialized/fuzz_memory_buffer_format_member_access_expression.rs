...
// Targets: format_member_access_expression in src/formatter/mod_complex.rs:680
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // format_member_access_expression(buffer, data, size);
...
void test_string_operations(const uint8_t *data, size_t size) {
    ...
    // format_member_access_expression(str);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // format_member_access_expression(data, 0);        // Zero size
        // format_member_access_expression(data, 1);        // Single byte
        // format_member_access_expression(data, size);     // Full size
        // format_member_access_expression(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for format_member_access_expression
