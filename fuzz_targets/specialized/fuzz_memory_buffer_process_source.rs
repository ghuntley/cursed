...
// Targets: process_source in src/preprocessor/mod.rs:26
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // process_source(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // process_source(data, 0);        // Zero size
        // process_source(data, 1);        // Single byte
        // process_source(data, size);     // Full size
        // process_source(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for process_source
