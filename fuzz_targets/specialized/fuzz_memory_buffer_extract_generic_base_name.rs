...
// Targets: extract_generic_base_name in src/codegen/llvm/passes/inlining.rs:397
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // extract_generic_base_name(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // extract_generic_base_name(data, 0);        // Zero size
        // extract_generic_base_name(data, 1);        // Single byte
        // extract_generic_base_name(data, size);     // Full size
        // extract_generic_base_name(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for extract_generic_base_name
