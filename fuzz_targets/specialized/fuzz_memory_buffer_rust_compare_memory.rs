...
// Targets: rust_compare_memory in src/runtime/memory_bridge.rs:420
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // rust_compare_memory(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // rust_compare_memory(data, 0);        // Zero size
        // rust_compare_memory(data, 1);        // Single byte
        // rust_compare_memory(data, size);     // Full size
        // rust_compare_memory(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for rust_compare_memory
