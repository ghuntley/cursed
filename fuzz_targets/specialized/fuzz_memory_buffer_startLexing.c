...
// Targets: startLexing in src-zig/performance_integration.zig:216
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // startLexing(data, 0);        // Zero size
        // startLexing(data, 1);        // Single byte
        // startLexing(data, size);     // Full size
        // startLexing(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for startLexing
