...
// Targets: init_registry in src/stdlib/plug_vibes/registry.rs:53
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // init_registry(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // init_registry(data, 0);        // Zero size
        // init_registry(data, 1);        // Single byte
        // init_registry(data, size);     // Full size
        // init_registry(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for init_registry
