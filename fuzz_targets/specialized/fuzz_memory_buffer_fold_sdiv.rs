...
// Targets: fold_sdiv in src/codegen/llvm/passes/constant_propagation.rs:340
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // fold_sdiv(data, 0);        // Zero size
        // fold_sdiv(data, 1);        // Single byte
        // fold_sdiv(data, size);     // Full size
        // fold_sdiv(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for fold_sdiv
