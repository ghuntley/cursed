...
// Targets: generate_type_checking_ir in src/codegen/llvm/interface_type_checking.rs:125
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // generate_type_checking_ir(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // generate_type_checking_ir(data, 0);        // Zero size
        // generate_type_checking_ir(data, 1);        // Single byte
        // generate_type_checking_ir(data, size);     // Full size
        // generate_type_checking_ir(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for generate_type_checking_ir
