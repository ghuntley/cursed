...
// Targets: remove_brace_spaces in src/formatter/simple.rs:389
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // remove_brace_spaces(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // remove_brace_spaces(data, 0);        // Zero size
        // remove_brace_spaces(data, 1);        // Single byte
        // remove_brace_spaces(data, size);     // Full size
        // remove_brace_spaces(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for remove_brace_spaces
