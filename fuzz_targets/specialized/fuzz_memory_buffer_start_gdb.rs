...
// Targets: start_gdb in src/debug/gdb_integration.rs:180
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // start_gdb(data, 0);        // Zero size
        // start_gdb(data, 1);        // Single byte
        // start_gdb(data, size);     // Full size
        // start_gdb(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for start_gdb
