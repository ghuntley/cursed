...
// Targets: single in src/stdlib/database/orm/query_builder.rs:510
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // single(data, 0);        // Zero size
        // single(data, 1);        // Single byte
        // single(data, size);     // Full size
        // single(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for single
