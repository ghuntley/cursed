...
// Targets: apply_rule in src/formatter/rules.rs:487
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // apply_rule(data, 0);        // Zero size
        // apply_rule(data, 1);        // Single byte
        // apply_rule(data, size);     // Full size
        // apply_rule(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for apply_rule
