...
// Targets: decode in src/stdlib/packages/crypto_asymmetric/x25519.rs:144
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // decode(data, 0);        // Zero size
        // decode(data, 1);        // Single byte
        // decode(data, size);     // Full size
        // decode(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for decode
