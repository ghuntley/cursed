...
// Targets: save_wasm_locals in src/runtime/goroutine_context.rs:1773
...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    // Test save_wasm_locals with temp file
    // Example: save_wasm_locals(temp_path);
    // Example: save_wasm_locals(temp_path, data, size);
...
void test_path_operations(const uint8_t *data, size_t size) {
    ...
    // save_wasm_locals(path);
}


// Additional test functions for save_wasm_locals
