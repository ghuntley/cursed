...
// Targets: load_startup_file in src/repl/enhanced_cursed_repl.rs:807
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // load_startup_file(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // load_startup_file(data, 0);        // Zero size
        // load_startup_file(data, 1);        // Single byte
        // load_startup_file(data, size);     // Full size
        // load_startup_file(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for load_startup_file
