...
// Targets: count_rows in stdlib/csv/mod.csd:27
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // count_rows(data, 0);        // Zero size
        // count_rows(data, 1);        // Single byte
        // count_rows(data, size);     // Full size
        // count_rows(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for count_rows
