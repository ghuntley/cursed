...
// Targets: package_exists in src/package_manager/registry.rs:370
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // package_exists(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // package_exists(data, 0);        // Zero size
        // package_exists(data, 1);        // Single byte
        // package_exists(data, size);     // Full size
        // package_exists(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for package_exists
