...
// Targets: generate_interface_member_access in src/codegen/llvm/main.rs:5724
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // generate_interface_member_access(buffer, data, size);
...
void test_string_operations(const uint8_t *data, size_t size) {
    ...
    // generate_interface_member_access(str);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // generate_interface_member_access(data, 0);        // Zero size
        // generate_interface_member_access(data, 1);        // Single byte
        // generate_interface_member_access(data, size);     // Full size
        // generate_interface_member_access(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for generate_interface_member_access
