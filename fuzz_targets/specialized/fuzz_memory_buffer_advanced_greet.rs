...
// Targets: advanced_greet in examples/docs_generation_demo.csd:197
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // advanced_greet(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // advanced_greet(data, 0);        // Zero size
        // advanced_greet(data, 1);        // Single byte
        // advanced_greet(data, size);     // Full size
        // advanced_greet(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for advanced_greet
