...
// Targets: compile_to_optimized_wasm_ir in src/lib.rs:1468
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // compile_to_optimized_wasm_ir(buffer, data, size);
...
void test_string_operations(const uint8_t *data, size_t size) {
    ...
    // compile_to_optimized_wasm_ir(str);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // compile_to_optimized_wasm_ir(data, 0);        // Zero size
        // compile_to_optimized_wasm_ir(data, 1);        // Single byte
        // compile_to_optimized_wasm_ir(data, size);     // Full size
        // compile_to_optimized_wasm_ir(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for compile_to_optimized_wasm_ir
