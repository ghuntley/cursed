...
// Targets: fold_udiv in src/codegen/llvm/passes/constant_propagation.rs:360
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // fold_udiv(data, 0);        // Zero size
        // fold_udiv(data, 1);        // Single byte
        // fold_udiv(data, size);     // Full size
        // fold_udiv(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for fold_udiv
