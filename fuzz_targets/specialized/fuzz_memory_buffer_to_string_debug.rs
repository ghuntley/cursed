...
// Targets: to_string_debug in src/runtime/runtime_value.rs:247
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // to_string_debug(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // to_string_debug(data, 0);        // Zero size
        // to_string_debug(data, 1);        // Single byte
        // to_string_debug(data, size);     // Full size
        // to_string_debug(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for to_string_debug
