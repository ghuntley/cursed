...
// Targets: check_long_string_literal in src/tools/linter.rs:2063
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // check_long_string_literal(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // check_long_string_literal(data, 0);        // Zero size
        // check_long_string_literal(data, 1);        // Single byte
        // check_long_string_literal(data, size);     // Full size
        // check_long_string_literal(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for check_long_string_literal
