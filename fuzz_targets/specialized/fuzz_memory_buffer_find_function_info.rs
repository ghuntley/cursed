...
// Targets: find_function_info in src/optimization/link_time_optimization.rs:625
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // find_function_info(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // find_function_info(data, 0);        // Zero size
        // find_function_info(data, 1);        // Single byte
        // find_function_info(data, size);     // Full size
        // find_function_info(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for find_function_info
