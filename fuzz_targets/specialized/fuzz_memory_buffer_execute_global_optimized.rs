...
// Targets: execute_global_optimized in src/runtime/jit_runtime.rs:1010
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // execute_global_optimized(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // execute_global_optimized(data, 0);        // Zero size
        // execute_global_optimized(data, 1);        // Single byte
        // execute_global_optimized(data, size);     // Full size
        // execute_global_optimized(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for execute_global_optimized
