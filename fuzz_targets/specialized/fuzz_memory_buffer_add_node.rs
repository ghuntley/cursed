...
// Targets: add_node in src/type_system/constraint_resolver.rs:843
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // add_node(data, 0);        // Zero size
        // add_node(data, 1);        // Single byte
        // add_node(data, size);     // Full size
        // add_node(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for add_node
