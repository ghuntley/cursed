...
// Targets: send_text in src/stdlib/net/websocket/client.rs:32
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // send_text(data, 0);        // Zero size
        // send_text(data, 1);        // Single byte
        // send_text(data, size);     // Full size
        // send_text(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for send_text
