...
// Targets: test_struct_types in tests/type_system_basic_test.rs:113
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // test_struct_types(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // test_struct_types(data, 0);        // Zero size
        // test_struct_types(data, 1);        // Single byte
        // test_struct_types(data, size);     // Full size
        // test_struct_types(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for test_struct_types
