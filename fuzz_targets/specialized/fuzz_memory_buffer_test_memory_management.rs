...
// Targets: test_memory_management in variadic_ffi_test.csd:156
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // test_memory_management(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // test_memory_management(data, 0);        // Zero size
        // test_memory_management(data, 1);        // Single byte
        // test_memory_management(data, size);     // Full size
        // test_memory_management(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for test_memory_management
