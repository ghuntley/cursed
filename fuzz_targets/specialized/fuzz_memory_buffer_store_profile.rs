...
// Targets: store_profile in src/optimization/pgo/profile_storage.rs:82
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // store_profile(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // store_profile(data, 0);        // Zero size
        // store_profile(data, 1);        // Single byte
        // store_profile(data, size);     // Full size
        // store_profile(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for store_profile
