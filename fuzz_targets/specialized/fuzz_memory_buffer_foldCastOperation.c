...
// Targets: foldCastOperation in src-zig/constant_folder.zig:279
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // foldCastOperation(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // foldCastOperation(data, 0);        // Zero size
        // foldCastOperation(data, 1);        // Single byte
        // foldCastOperation(data, size);     // Full size
        // foldCastOperation(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for foldCastOperation
