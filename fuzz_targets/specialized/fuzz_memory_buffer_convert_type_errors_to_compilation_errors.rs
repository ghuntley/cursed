...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // convert_type_errors_to_compilation_errors(buffer, data, size);
...
void test_string_operations(const uint8_t *data, size_t size) {
    ...
    // convert_type_errors_to_compilation_errors(str);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // convert_type_errors_to_compilation_errors(data, 0);        // Zero size
        // convert_type_errors_to_compilation_errors(data, 1);        // Single byte
        // convert_type_errors_to_compilation_errors(data, size);     // Full size
        // convert_type_errors_to_compilation_errors(NULL, 0);        // NULL pointer
    }
}
