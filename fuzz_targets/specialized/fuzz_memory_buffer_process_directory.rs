...
// Targets: process_directory in src/documentation/mod.rs:430
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // process_directory(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // process_directory(data, 0);        // Zero size
        // process_directory(data, 1);        // Single byte
        // process_directory(data, size);     // Full size
        // process_directory(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for process_directory
