...
// Targets: substring_range in stdlib/command_line/mod.csd:71
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // substring_range(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // substring_range(data, 0);        // Zero size
        // substring_range(data, 1);        // Single byte
        // substring_range(data, size);     // Full size
        // substring_range(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for substring_range
