...
// Targets: extract_quoted_string in stdlib/linter/ast_integration.csd:434
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // extract_quoted_string(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // extract_quoted_string(data, 0);        // Zero size
        // extract_quoted_string(data, 1);        // Single byte
        // extract_quoted_string(data, size);     // Full size
        // extract_quoted_string(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for extract_quoted_string
