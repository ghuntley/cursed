...
// Targets: with_public_key in src/stdlib/net/protocols/ssh.rs:136
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // with_public_key(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // with_public_key(data, 0);        // Zero size
        // with_public_key(data, 1);        // Single byte
        // with_public_key(data, size);     // Full size
        // with_public_key(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for with_public_key
