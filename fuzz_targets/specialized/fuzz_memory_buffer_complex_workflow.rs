...
// Targets: complex_workflow in examples/question_mark_demo.csd:90
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // complex_workflow(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // complex_workflow(data, 0);        // Zero size
        // complex_workflow(data, 1);        // Single byte
        // complex_workflow(data, size);     // Full size
        // complex_workflow(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for complex_workflow
