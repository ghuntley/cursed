...
// Targets: io_list_directory in src/execution/runtime_functions.rs:1633
...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    // Test io_list_directory with temp file
    // Example: io_list_directory(temp_path);
    // Example: io_list_directory(temp_path, data, size);
...
void test_path_operations(const uint8_t *data, size_t size) {
    ...
    // io_list_directory(path);
}


// Additional test functions for io_list_directory
