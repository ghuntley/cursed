...
// Targets: simplifySubtraction in src-zig/constant_folder.zig:514
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // simplifySubtraction(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // simplifySubtraction(data, 0);        // Zero size
        // simplifySubtraction(data, 1);        // Single byte
        // simplifySubtraction(data, size);     // Full size
        // simplifySubtraction(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for simplifySubtraction
