...
// Targets: has_side_effects in src/codegen/llvm/passes/dead_code_elimination.rs:72
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // has_side_effects(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // has_side_effects(data, 0);        // Zero size
        // has_side_effects(data, 1);        // Single byte
        // has_side_effects(data, size);     // Full size
        // has_side_effects(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for has_side_effects
