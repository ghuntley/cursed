...
// Targets: apply_all in src/formatter/rules.rs:474
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // apply_all(data, 0);        // Zero size
        // apply_all(data, 1);        // Single byte
        // apply_all(data, size);     // Full size
        // apply_all(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for apply_all
