...
// Targets: test_long_strings in stdlib/gob_encode_vibes/test_gob_encode_vibes.csd:331
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // test_long_strings(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // test_long_strings(data, 0);        // Zero size
        // test_long_strings(data, 1);        // Single byte
        // test_long_strings(data, size);     // Full size
        // test_long_strings(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for test_long_strings
