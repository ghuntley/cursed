...
// Targets: write_string in stdlib/serialization/mod.csd:200
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // write_string(data, 0);        // Zero size
        // write_string(data, 1);        // Single byte
        // write_string(data, size);     // Full size
        // write_string(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for write_string
