...
// Targets: string_slice in stdlib/string_simple/mod.csd:243
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // string_slice(data, 0);        // Zero size
        // string_slice(data, 1);        // Single byte
        // string_slice(data, size);     // Full size
        // string_slice(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for string_slice
