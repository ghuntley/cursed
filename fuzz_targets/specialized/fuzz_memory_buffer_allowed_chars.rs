...
// Targets: allowed_chars in src/security/input_validation.rs:35
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // allowed_chars(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // allowed_chars(data, 0);        // Zero size
        // allowed_chars(data, 1);        // Single byte
        // allowed_chars(data, size);     // Full size
        // allowed_chars(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for allowed_chars
