...
// Targets: parsePackageInfo in stdlib/package_manager/mod.csd:473
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // parsePackageInfo(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // parsePackageInfo(data, 0);        // Zero size
        // parsePackageInfo(data, 1);        // Single byte
        // parsePackageInfo(data, size);     // Full size
        // parsePackageInfo(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for parsePackageInfo
