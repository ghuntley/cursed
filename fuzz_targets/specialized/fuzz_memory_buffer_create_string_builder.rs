...
// Targets: create_string_builder in benchmarks/cursed/compiler/memory_usage_bench.csd:492
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // create_string_builder(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // create_string_builder(data, 0);        // Zero size
        // create_string_builder(data, 1);        // Single byte
        // create_string_builder(data, size);     // Full size
        // create_string_builder(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for create_string_builder
