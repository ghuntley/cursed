...
// Targets: apply_specialization_optimization in src/codegen/llvm/generic_optimization.rs:212
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // apply_specialization_optimization(buffer, data, size);
...
void test_string_operations(const uint8_t *data, size_t size) {
    ...
    // apply_specialization_optimization(str);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // apply_specialization_optimization(data, 0);        // Zero size
        // apply_specialization_optimization(data, 1);        // Single byte
        // apply_specialization_optimization(data, size);     // Full size
        // apply_specialization_optimization(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for apply_specialization_optimization
