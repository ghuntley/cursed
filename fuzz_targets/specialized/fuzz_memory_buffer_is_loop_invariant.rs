...
// Targets: is_loop_invariant in src/codegen/llvm/passes/licm.rs:91
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // is_loop_invariant(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // is_loop_invariant(data, 0);        // Zero size
        // is_loop_invariant(data, 1);        // Single byte
        // is_loop_invariant(data, size);     // Full size
        // is_loop_invariant(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for is_loop_invariant
