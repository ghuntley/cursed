...
// Targets: create in src-zig/enhanced_monomorphization.zig:291
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // create(data, 0);        // Zero size
        // create(data, 1);        // Single byte
        // create(data, size);     // Full size
        // create(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for create
