...
// Targets: sanitize_string in src/security/input_validation.rs:79
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // sanitize_string(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // sanitize_string(data, 0);        // Zero size
        // sanitize_string(data, 1);        // Single byte
        // sanitize_string(data, size);     // Full size
        // sanitize_string(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for sanitize_string
