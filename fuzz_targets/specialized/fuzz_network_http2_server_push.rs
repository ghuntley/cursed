...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    // Test packet parsing
    // Test http2_server_push with network data
    // Example: http2_server_push(data, size);
    // Example: http2_server_push(sockfd, data, size);
...
void test_socket_operations(const uint8_t *data, size_t size) {
    ...
    // http2_server_push(sockfd, data, size);
