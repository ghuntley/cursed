...
// Targets: getPackageInfo in stdlib/package_manager/registry_client.csd:170
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // getPackageInfo(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // getPackageInfo(data, 0);        // Zero size
        // getPackageInfo(data, 1);        // Single byte
        // getPackageInfo(data, size);     // Full size
        // getPackageInfo(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for getPackageInfo
