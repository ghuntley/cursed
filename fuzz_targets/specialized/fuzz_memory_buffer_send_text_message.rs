...
// Targets: send_text_message in src/runtime/process.rs:649
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // send_text_message(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // send_text_message(data, 0);        // Zero size
        // send_text_message(data, 1);        // Single byte
        // send_text_message(data, size);     // Full size
        // send_text_message(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for send_text_message
