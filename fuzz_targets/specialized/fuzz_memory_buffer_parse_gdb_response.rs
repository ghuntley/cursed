...
// Targets: parse_gdb_response in src/debug/gdb_integration.rs:730
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // parse_gdb_response(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // parse_gdb_response(data, 0);        // Zero size
        // parse_gdb_response(data, 1);        // Single byte
        // parse_gdb_response(data, size);     // Full size
        // parse_gdb_response(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for parse_gdb_response
