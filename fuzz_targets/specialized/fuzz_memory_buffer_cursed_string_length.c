...
// Targets: cursed_string_length in runtime/minimal_shims.c:56
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // cursed_string_length(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // cursed_string_length(data, 0);        // Zero size
        // cursed_string_length(data, 1);        // Single byte
        // cursed_string_length(data, size);     // Full size
        // cursed_string_length(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for cursed_string_length
