...
// Targets: decrypt_file_interactive in examples/file_encryption.csd:186
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // decrypt_file_interactive(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // decrypt_file_interactive(data, 0);        // Zero size
        // decrypt_file_interactive(data, 1);        // Single byte
        // decrypt_file_interactive(data, size);     // Full size
        // decrypt_file_interactive(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for decrypt_file_interactive
