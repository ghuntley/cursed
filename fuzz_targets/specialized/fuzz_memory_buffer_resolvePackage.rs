...
// Targets: resolvePackage in stdlib/package_manager/mod.csd:345
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // resolvePackage(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // resolvePackage(data, 0);        // Zero size
        // resolvePackage(data, 1);        // Single byte
        // resolvePackage(data, size);     // Full size
        // resolvePackage(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for resolvePackage
