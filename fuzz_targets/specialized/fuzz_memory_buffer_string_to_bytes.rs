...
// Targets: string_to_bytes in src/execution/runtime_functions.rs:4571
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // string_to_bytes(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // string_to_bytes(data, 0);        // Zero size
        // string_to_bytes(data, 1);        // Single byte
        // string_to_bytes(data, size);     // Full size
        // string_to_bytes(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for string_to_bytes
