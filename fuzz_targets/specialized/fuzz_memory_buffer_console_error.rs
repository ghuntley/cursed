...
// Targets: console_error in src/wasm/panic_handler.rs:24
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // console_error(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // console_error(data, 0);        // Zero size
        // console_error(data, 1);        // Single byte
        // console_error(data, size);     // Full size
        // console_error(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for console_error
