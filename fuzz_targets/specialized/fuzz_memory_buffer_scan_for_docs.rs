...
// Targets: scan_for_docs in src/bin/cursed_doc.rs:396
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // scan_for_docs(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // scan_for_docs(data, 0);        // Zero size
        // scan_for_docs(data, 1);        // Single byte
        // scan_for_docs(data, size);     // Full size
        // scan_for_docs(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for scan_for_docs
