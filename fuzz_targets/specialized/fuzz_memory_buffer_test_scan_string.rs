...
// Targets: test_scan_string in tests/vibez_test.csd:23
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // test_scan_string(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // test_scan_string(data, 0);        // Zero size
        // test_scan_string(data, 1);        // Single byte
        // test_scan_string(data, size);     // Full size
        // test_scan_string(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for test_scan_string
