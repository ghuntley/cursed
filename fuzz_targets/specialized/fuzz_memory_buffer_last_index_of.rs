...
// Targets: last_index_of in stdlib/io/mod.csd:319
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // last_index_of(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // last_index_of(data, 0);        // Zero size
        // last_index_of(data, 1);        // Single byte
        // last_index_of(data, size);     // Full size
        // last_index_of(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for last_index_of
