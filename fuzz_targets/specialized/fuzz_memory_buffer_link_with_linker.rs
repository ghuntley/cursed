...
// Targets: link_with_linker in src/lib.rs:2477
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // link_with_linker(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // link_with_linker(data, 0);        // Zero size
        // link_with_linker(data, 1);        // Single byte
        // link_with_linker(data, size);     // Full size
        // link_with_linker(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for link_with_linker
