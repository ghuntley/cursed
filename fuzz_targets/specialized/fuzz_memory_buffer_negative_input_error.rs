...
// Targets: negative_input_error in src/stdlib/math/mod.rs:149
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // negative_input_error(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // negative_input_error(data, 0);        // Zero size
        // negative_input_error(data, 1);        // Single byte
        // negative_input_error(data, size);     // Full size
        // negative_input_error(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for negative_input_error
