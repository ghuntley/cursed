...
// Targets: send_debugger_command in src/bin/cursed_debug.rs:676
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // send_debugger_command(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // send_debugger_command(data, 0);        // Zero size
        // send_debugger_command(data, 1);        // Single byte
        // send_debugger_command(data, size);     // Full size
        // send_debugger_command(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for send_debugger_command
