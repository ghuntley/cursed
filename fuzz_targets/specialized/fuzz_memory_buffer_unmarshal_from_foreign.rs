...
// Targets: unmarshal_from_foreign in src/ffi/type_mapping.rs:331
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // unmarshal_from_foreign(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // unmarshal_from_foreign(data, 0);        // Zero size
        // unmarshal_from_foreign(data, 1);        // Single byte
        // unmarshal_from_foreign(data, size);     // Full size
        // unmarshal_from_foreign(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for unmarshal_from_foreign
