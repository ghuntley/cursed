...
// Targets: named in src/type_system/mod.rs:642
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // named(data, 0);        // Zero size
        // named(data, 1);        // Single byte
        // named(data, size);     // Full size
        // named(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for named
