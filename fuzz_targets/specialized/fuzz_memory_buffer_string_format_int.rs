...
// Targets: string_format_int in stdlib/enhanced_error/mod.csd:420
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // string_format_int(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // string_format_int(data, 0);        // Zero size
        // string_format_int(data, 1);        // Single byte
        // string_format_int(data, size);     // Full size
        // string_format_int(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for string_format_int
