...
// Targets: category in src/error/structured.rs:222
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // category(data, 0);        // Zero size
        // category(data, 1);        // Single byte
        // category(data, size);     // Full size
        // category(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for category
