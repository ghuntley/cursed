...
// Targets: peek_ahead in src/lexer/mod.rs:899
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // peek_ahead(data, 0);        // Zero size
        // peek_ahead(data, 1);        // Single byte
        // peek_ahead(data, size);     // Full size
        // peek_ahead(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for peek_ahead
