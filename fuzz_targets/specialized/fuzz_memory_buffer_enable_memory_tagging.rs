...
// Targets: enable_memory_tagging in src/runtime/pal/arm64.rs:573
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // enable_memory_tagging(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // enable_memory_tagging(data, 0);        // Zero size
        // enable_memory_tagging(data, 1);        // Single byte
        // enable_memory_tagging(data, size);     // Full size
        // enable_memory_tagging(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for enable_memory_tagging
