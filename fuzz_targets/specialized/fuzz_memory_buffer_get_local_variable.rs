...
// Targets: get_local_variable in src/runtime/debug_runtime.rs:1214
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // get_local_variable(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // get_local_variable(data, 0);        // Zero size
        // get_local_variable(data, 1);        // Single byte
        // get_local_variable(data, size);     // Full size
        // get_local_variable(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for get_local_variable
