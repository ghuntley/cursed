...
// Targets: cursed_debug_print_interface_registry in runtime/interface_runtime.c:264
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // cursed_debug_print_interface_registry(buffer, data, size);
...
void test_string_operations(const uint8_t *data, size_t size) {
    ...
    // cursed_debug_print_interface_registry(str);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // cursed_debug_print_interface_registry(data, 0);        // Zero size
        // cursed_debug_print_interface_registry(data, 1);        // Single byte
        // cursed_debug_print_interface_registry(data, size);     // Full size
        // cursed_debug_print_interface_registry(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for cursed_debug_print_interface_registry
