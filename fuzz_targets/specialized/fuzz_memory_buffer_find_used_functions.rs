...
// Targets: find_used_functions in src/codegen/llvm/generic_optimization.rs:493
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // find_used_functions(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // find_used_functions(data, 0);        // Zero size
        // find_used_functions(data, 1);        // Single byte
        // find_used_functions(data, size);     // Full size
        // find_used_functions(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for find_used_functions
