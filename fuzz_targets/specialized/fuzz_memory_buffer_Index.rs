...
// Targets: Index in stdlib/string_energy/mod.csd:60
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // Index(data, 0);        // Zero size
        // Index(data, 1);        // Single byte
        // Index(data, size);     // Full size
        // Index(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for Index
