...
// Targets: reset_global_string_manager in src/codegen/llvm/string_constants.rs:123
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // reset_global_string_manager(buffer, data, size);
...
void test_string_operations(const uint8_t *data, size_t size) {
    ...
    // reset_global_string_manager(str);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // reset_global_string_manager(data, 0);        // Zero size
        // reset_global_string_manager(data, 1);        // Single byte
        // reset_global_string_manager(data, size);     // Full size
        // reset_global_string_manager(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for reset_global_string_manager
