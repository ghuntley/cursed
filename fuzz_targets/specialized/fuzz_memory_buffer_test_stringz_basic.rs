...
// Targets: test_stringz_basic in stdlib/test_enhanced_stdlib.csd:56
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // test_stringz_basic(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // test_stringz_basic(data, 0);        // Zero size
        // test_stringz_basic(data, 1);        // Single byte
        // test_stringz_basic(data, size);     // Full size
        // test_stringz_basic(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for test_stringz_basic
