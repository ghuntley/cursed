...
// Targets: fold_fmul in src/codegen/llvm/passes/constant_propagation.rs:785
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // fold_fmul(data, 0);        // Zero size
        // fold_fmul(data, 1);        // Single byte
        // fold_fmul(data, size);     // Full size
        // fold_fmul(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for fold_fmul
