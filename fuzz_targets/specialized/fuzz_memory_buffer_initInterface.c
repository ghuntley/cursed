...
// Targets: initInterface in src-zig/generics.zig:52
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // initInterface(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // initInterface(data, 0);        // Zero size
        // initInterface(data, 1);        // Single byte
        // initInterface(data, size);     // Full size
        // initInterface(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for initInterface
