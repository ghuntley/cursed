...
// Targets: apply_strength_reduction in src/optimization/production_llvm_optimization.rs:904
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // apply_strength_reduction(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // apply_strength_reduction(data, 0);        // Zero size
        // apply_strength_reduction(data, 1);        // Single byte
        // apply_strength_reduction(data, size);     // Full size
        // apply_strength_reduction(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for apply_strength_reduction
