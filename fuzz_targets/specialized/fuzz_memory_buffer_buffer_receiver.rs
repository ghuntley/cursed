...
// Targets: buffer_receiver in tests/e2e/concurrency/02_channels.csd:53
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // buffer_receiver(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // buffer_receiver(data, 0);        // Zero size
        // buffer_receiver(data, 1);        // Single byte
        // buffer_receiver(data, size);     // Full size
        // buffer_receiver(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for buffer_receiver
