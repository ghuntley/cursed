...
// Targets: string_contains_env_var in stdlib/configz/mod_complex.csd:1129
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // string_contains_env_var(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // string_contains_env_var(data, 0);        // Zero size
        // string_contains_env_var(data, 1);        // Single byte
        // string_contains_env_var(data, size);     // Full size
        // string_contains_env_var(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for string_contains_env_var
