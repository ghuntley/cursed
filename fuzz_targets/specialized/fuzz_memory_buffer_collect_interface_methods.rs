...
// Targets: collect_interface_methods in src/runtime/interface_dispatch.rs:298
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // collect_interface_methods(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // collect_interface_methods(data, 0);        // Zero size
        // collect_interface_methods(data, 1);        // Single byte
        // collect_interface_methods(data, size);     // Full size
        // collect_interface_methods(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for collect_interface_methods
