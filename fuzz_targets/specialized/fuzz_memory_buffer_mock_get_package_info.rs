...
// Targets: mock_get_package_info in src/package_manager/registry.rs:484
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // mock_get_package_info(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // mock_get_package_info(data, 0);        // Zero size
        // mock_get_package_info(data, 1);        // Single byte
        // mock_get_package_info(data, size);     // Full size
        // mock_get_package_info(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for mock_get_package_info
