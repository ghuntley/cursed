...
// Targets: parse_function_call in src/codegen/llvm/generic_optimization.rs:324
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // parse_function_call(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // parse_function_call(data, 0);        // Zero size
        // parse_function_call(data, 1);        // Single byte
        // parse_function_call(data, size);     // Full size
        // parse_function_call(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for parse_function_call
