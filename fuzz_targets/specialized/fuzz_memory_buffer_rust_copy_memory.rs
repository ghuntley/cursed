...
// Targets: rust_copy_memory in src/runtime/memory_bridge.rs:407
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // rust_copy_memory(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // rust_copy_memory(data, 0);        // Zero size
        // rust_copy_memory(data, 1);        // Single byte
        // rust_copy_memory(data, size);     // Full size
        // rust_copy_memory(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for rust_copy_memory
