...
// Targets: compile_and_run in src/codegen/llvm/jit_engine.rs:707
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // compile_and_run(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // compile_and_run(data, 0);        // Zero size
        // compile_and_run(data, 1);        // Single byte
        // compile_and_run(data, size);     // Full size
        // compile_and_run(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for compile_and_run
