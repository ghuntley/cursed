...
// Targets: discover_files_recursive in src/build_system/build_pipeline.rs:329
...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    // Test discover_files_recursive with temp file
    // Example: discover_files_recursive(temp_path);
    // Example: discover_files_recursive(temp_path, data, size);
...
void test_path_operations(const uint8_t *data, size_t size) {
    ...
    // discover_files_recursive(path);
}


// Additional test functions for discover_files_recursive
