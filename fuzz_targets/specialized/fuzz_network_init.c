...
// Targets: init in src-zig/http2_frame_parser.zig:625
...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    // Test packet parsing
    // Test init with network data
    // Example: init(data, size);
    // Example: init(sockfd, data, size);
...
void test_socket_operations(const uint8_t *data, size_t size) {
    ...
    // init(sockfd, data, size);
    
    close(sockfd);
}


// Additional test functions for init
