...
// Targets: debug_assert_eq_string in stdlib/debug_tea/mod.csd:107
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // debug_assert_eq_string(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // debug_assert_eq_string(data, 0);        // Zero size
        // debug_assert_eq_string(data, 1);        // Single byte
        // debug_assert_eq_string(data, size);     // Full size
        // debug_assert_eq_string(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for debug_assert_eq_string
