...
// Targets: unterminated_string in src/error/structured.rs:764
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // unterminated_string(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // unterminated_string(data, 0);        // Zero size
        // unterminated_string(data, 1);        // Single byte
        // unterminated_string(data, size);     // Full size
        // unterminated_string(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for unterminated_string
