...
// Targets: compile_with_debug in src/lib.rs:924
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // compile_with_debug(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // compile_with_debug(data, 0);        // Zero size
        // compile_with_debug(data, 1);        // Single byte
        // compile_with_debug(data, size);     // Full size
        // compile_with_debug(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for compile_with_debug
