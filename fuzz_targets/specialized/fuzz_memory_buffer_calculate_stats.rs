...
// Targets: calculate_stats in src/linter/mod.rs:166
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // calculate_stats(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // calculate_stats(data, 0);        // Zero size
        // calculate_stats(data, 1);        // Single byte
        // calculate_stats(data, size);     // Full size
        // calculate_stats(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for calculate_stats
