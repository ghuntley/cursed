...
// Targets: memory_health_check in stdlib/memory_core/mod.csd:344
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // memory_health_check(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // memory_health_check(data, 0);        // Zero size
        // memory_health_check(data, 1);        // Single byte
        // memory_health_check(data, size);     // Full size
        // memory_health_check(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for memory_health_check
