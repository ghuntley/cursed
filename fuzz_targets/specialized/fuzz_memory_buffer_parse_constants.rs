...
// Targets: parse_constants in src/ffi/header_parser.rs:627
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // parse_constants(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // parse_constants(data, 0);        // Zero size
        // parse_constants(data, 1);        // Single byte
        // parse_constants(data, size);     // Full size
        // parse_constants(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for parse_constants
