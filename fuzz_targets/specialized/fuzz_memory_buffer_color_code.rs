...
// Targets: color_code in src/runtime/debug_output.rs:47
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // color_code(data, 0);        // Zero size
        // color_code(data, 1);        // Single byte
        // color_code(data, size);     // Full size
        // color_code(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for color_code
