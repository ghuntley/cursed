...
// Targets: run_program in src/bin/cursed_debug.rs:371
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // run_program(data, 0);        // Zero size
        // run_program(data, 1);        // Single byte
        // run_program(data, size);     // Full size
        // run_program(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for run_program
