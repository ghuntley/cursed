...
// Targets: propagate_panic_to_siblings in src/runtime/goroutine.rs:1321
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // propagate_panic_to_siblings(buffer, data, size);
...
void test_string_operations(const uint8_t *data, size_t size) {
    ...
    // propagate_panic_to_siblings(str);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // propagate_panic_to_siblings(data, 0);        // Zero size
        // propagate_panic_to_siblings(data, 1);        // Single byte
        // propagate_panic_to_siblings(data, size);     // Full size
        // propagate_panic_to_siblings(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for propagate_panic_to_siblings
