...
// Targets: fold_ashr in src/codegen/llvm/passes/constant_propagation.rs:621
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // fold_ashr(data, 0);        // Zero size
        // fold_ashr(data, 1);        // Single byte
        // fold_ashr(data, size);     // Full size
        // fold_ashr(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for fold_ashr
