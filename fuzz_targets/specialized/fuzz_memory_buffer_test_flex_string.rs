...
// Targets: test_flex_string in stdlib/stat_flexin/test_stat_flexin.csd:48
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // test_flex_string(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // test_flex_string(data, 0);        // Zero size
        // test_flex_string(data, 1);        // Single byte
        // test_flex_string(data, size);     // Full size
        // test_flex_string(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for test_flex_string
