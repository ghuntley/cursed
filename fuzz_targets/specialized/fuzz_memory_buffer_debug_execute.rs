...
// Targets: debug_execute in src/execution_pipeline.rs:418
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // debug_execute(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // debug_execute(data, 0);        // Zero size
        // debug_execute(data, 1);        // Single byte
        // debug_execute(data, size);     // Full size
        // debug_execute(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for debug_execute
