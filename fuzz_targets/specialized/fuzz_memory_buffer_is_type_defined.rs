...
// Targets: is_type_defined in src/type_system/checker.rs:1997
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // is_type_defined(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // is_type_defined(data, 0);        // Zero size
        // is_type_defined(data, 1);        // Single byte
        // is_type_defined(data, size);     // Full size
        // is_type_defined(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for is_type_defined
