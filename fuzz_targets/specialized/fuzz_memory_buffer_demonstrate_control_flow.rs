...
// Targets: demonstrate_control_flow in examples/docs_demo.csd:257
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // demonstrate_control_flow(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // demonstrate_control_flow(data, 0);        // Zero size
        // demonstrate_control_flow(data, 1);        // Single byte
        // demonstrate_control_flow(data, size);     // Full size
        // demonstrate_control_flow(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for demonstrate_control_flow
