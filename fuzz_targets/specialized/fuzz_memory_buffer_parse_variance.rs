...
// Targets: parse_variance in src/type_system/variance.rs:239
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // parse_variance(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // parse_variance(data, 0);        // Zero size
        // parse_variance(data, 1);        // Single byte
        // parse_variance(data, size);     // Full size
        // parse_variance(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for parse_variance
