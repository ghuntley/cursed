...
// Targets: fold_mul in src/codegen/llvm/passes/constant_propagation.rs:279
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // fold_mul(data, 0);        // Zero size
        // fold_mul(data, 1);        // Single byte
        // fold_mul(data, size);     // Full size
        // fold_mul(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for fold_mul
