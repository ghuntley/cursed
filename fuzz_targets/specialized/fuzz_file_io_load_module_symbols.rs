...
// Targets: load_module_symbols in src/runtime/debug_info.rs:695
...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    // Test load_module_symbols with temp file
    // Example: load_module_symbols(temp_path);
    // Example: load_module_symbols(temp_path, data, size);
...
void test_path_operations(const uint8_t *data, size_t size) {
    ...
    // load_module_symbols(path);
}


// Additional test functions for load_module_symbols
