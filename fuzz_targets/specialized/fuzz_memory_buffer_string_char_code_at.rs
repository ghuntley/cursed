...
// Targets: string_char_code_at in stdlib/string/mod.csd:253
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // string_char_code_at(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // string_char_code_at(data, 0);        // Zero size
        // string_char_code_at(data, 1);        // Single byte
        // string_char_code_at(data, size);     // Full size
        // string_char_code_at(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for string_char_code_at
