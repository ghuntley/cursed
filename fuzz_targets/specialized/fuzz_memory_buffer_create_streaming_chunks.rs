...
// Targets: create_streaming_chunks in src/build_system/memory_optimizer.rs:151
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // create_streaming_chunks(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // create_streaming_chunks(data, 0);        // Zero size
        // create_streaming_chunks(data, 1);        // Single byte
        // create_streaming_chunks(data, size);     // Full size
        // create_streaming_chunks(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for create_streaming_chunks
