...
// Targets: visit_struct_statement in src/type_system/checker.rs:2300
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // visit_struct_statement(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // visit_struct_statement(data, 0);        // Zero size
        // visit_struct_statement(data, 1);        // Single byte
        // visit_struct_statement(data, size);     // Full size
        // visit_struct_statement(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for visit_struct_statement
