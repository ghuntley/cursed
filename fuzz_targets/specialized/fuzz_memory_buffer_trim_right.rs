...
// Targets: trim_right in src/stdlib/bytefit/mod.rs:737
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // trim_right(data, 0);        // Zero size
        // trim_right(data, 1);        // Single byte
        // trim_right(data, size);     // Full size
        // trim_right(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for trim_right
