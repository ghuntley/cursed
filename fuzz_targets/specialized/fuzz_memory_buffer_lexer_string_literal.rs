...
// Targets: lexer_string_literal in src-zig/lexer.csd:240
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // lexer_string_literal(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // lexer_string_literal(data, 0);        // Zero size
        // lexer_string_literal(data, 1);        // Single byte
        // lexer_string_literal(data, size);     // Full size
        // lexer_string_literal(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for lexer_string_literal
