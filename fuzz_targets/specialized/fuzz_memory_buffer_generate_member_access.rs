...
// Targets: generate_member_access in src/codegen/llvm/main.rs:3457
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // generate_member_access(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // generate_member_access(data, 0);        // Zero size
        // generate_member_access(data, 1);        // Single byte
        // generate_member_access(data, size);     // Full size
        // generate_member_access(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for generate_member_access
