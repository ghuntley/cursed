...
// Targets: format_source in src/tools/formatter.rs:127
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // format_source(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // format_source(data, 0);        // Zero size
        // format_source(data, 1);        // Single byte
        // format_source(data, size);     // Full size
        // format_source(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for format_source
