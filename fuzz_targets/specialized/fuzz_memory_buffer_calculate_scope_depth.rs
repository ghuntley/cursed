...
// Targets: calculate_scope_depth in src/runtime/debug_manager.rs:611
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // calculate_scope_depth(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // calculate_scope_depth(data, 0);        // Zero size
        // calculate_scope_depth(data, 1);        // Single byte
        // calculate_scope_depth(data, size);     // Full size
        // calculate_scope_depth(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for calculate_scope_depth
