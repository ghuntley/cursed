...
// Targets: get_package_metadata in src/package_manager/registry.rs:311
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // get_package_metadata(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // get_package_metadata(data, 0);        // Zero size
        // get_package_metadata(data, 1);        // Single byte
        // get_package_metadata(data, size);     // Full size
        // get_package_metadata(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for get_package_metadata
