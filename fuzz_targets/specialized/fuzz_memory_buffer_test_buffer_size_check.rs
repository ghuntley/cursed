...
// Targets: test_buffer_size_check in src/ffi/safety_checks.rs:257
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // test_buffer_size_check(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // test_buffer_size_check(data, 0);        // Zero size
        // test_buffer_size_check(data, 1);        // Single byte
        // test_buffer_size_check(data, size);     // Full size
        // test_buffer_size_check(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for test_buffer_size_check
