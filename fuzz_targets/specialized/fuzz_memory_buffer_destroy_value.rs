...
// Targets: destroy_value in src/runtime/runtime_value.rs:416
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // destroy_value(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // destroy_value(data, 0);        // Zero size
        // destroy_value(data, 1);        // Single byte
        // destroy_value(data, size);     // Full size
        // destroy_value(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for destroy_value
