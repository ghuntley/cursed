...
// Targets: register_struct_marshaller in src/ffi/type_mapping.rs:287
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // register_struct_marshaller(buffer, data, size);
...
void test_string_operations(const uint8_t *data, size_t size) {
    ...
    // register_struct_marshaller(str);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // register_struct_marshaller(data, 0);        // Zero size
        // register_struct_marshaller(data, 1);        // Single byte
        // register_struct_marshaller(data, size);     // Full size
        // register_struct_marshaller(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for register_struct_marshaller
