...
// Targets: create_channel_with_buffer in src/runtime/channels/lifecycle.rs:399
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // create_channel_with_buffer(buffer, data, size);
...
void test_string_operations(const uint8_t *data, size_t size) {
    ...
    // create_channel_with_buffer(str);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // create_channel_with_buffer(data, 0);        // Zero size
        // create_channel_with_buffer(data, 1);        // Single byte
        // create_channel_with_buffer(data, size);     // Full size
        // create_channel_with_buffer(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for create_channel_with_buffer
