...
// Targets: set_variable in src/bin/cursed_debug.rs:662
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // set_variable(data, 0);        // Zero size
        // set_variable(data, 1);        // Single byte
        // set_variable(data, size);     // Full size
        // set_variable(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for set_variable
