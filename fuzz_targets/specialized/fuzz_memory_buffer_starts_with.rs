...
// Targets: starts_with in stdlib/io/mod.csd:284
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // starts_with(data, 0);        // Zero size
        // starts_with(data, 1);        // Single byte
        // starts_with(data, size);     // Full size
        // starts_with(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for starts_with
