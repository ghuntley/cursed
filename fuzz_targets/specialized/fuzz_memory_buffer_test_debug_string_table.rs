...
// Targets: test_debug_string_table in src/debug/tests.rs:94
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // test_debug_string_table(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // test_debug_string_table(data, 0);        // Zero size
        // test_debug_string_table(data, 1);        // Single byte
        // test_debug_string_table(data, size);     // Full size
        // test_debug_string_table(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for test_debug_string_table
