...
// Targets: compile_native_only in src/lib.rs:2066
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // compile_native_only(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // compile_native_only(data, 0);        // Zero size
        // compile_native_only(data, 1);        // Single byte
        // compile_native_only(data, size);     // Full size
        // compile_native_only(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for compile_native_only
