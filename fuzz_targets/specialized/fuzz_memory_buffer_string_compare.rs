...
// Targets: string_compare in stdlib/string_simple/mod.csd:96
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // string_compare(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // string_compare(data, 0);        // Zero size
        // string_compare(data, 1);        // Single byte
        // string_compare(data, size);     // Full size
        // string_compare(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for string_compare
