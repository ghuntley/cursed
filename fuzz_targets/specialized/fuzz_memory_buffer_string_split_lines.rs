...
// Targets: string_split_lines in src/execution/runtime_functions.rs:4766
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // string_split_lines(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // string_split_lines(data, 0);        // Zero size
        // string_split_lines(data, 1);        // Single byte
        // string_split_lines(data, size);     // Full size
        // string_split_lines(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for string_split_lines
