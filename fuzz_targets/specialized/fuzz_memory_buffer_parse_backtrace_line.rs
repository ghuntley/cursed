...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // parse_backtrace_line(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // parse_backtrace_line(data, 0);        // Zero size
        // parse_backtrace_line(data, 1);        // Single byte
        // parse_backtrace_line(data, size);     // Full size
        // parse_backtrace_line(NULL, 0);        // NULL pointer
    }
}
