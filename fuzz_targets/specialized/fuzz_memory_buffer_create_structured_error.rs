...
// Targets: create_structured_error in stdlib/error_handling/mod.csd:118
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // create_structured_error(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // create_structured_error(data, 0);        // Zero size
        // create_structured_error(data, 1);        // Single byte
        // create_structured_error(data, size);     // Full size
        // create_structured_error(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for create_structured_error
