...
// Targets: process_string in tests/regression/parser/function_definitions.csd:10
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // process_string(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // process_string(data, 0);        // Zero size
        // process_string(data, 1);        // Single byte
        // process_string(data, size);     // Full size
        // process_string(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for process_string
