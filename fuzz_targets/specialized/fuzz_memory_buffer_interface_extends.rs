...
// Targets: interface_extends in src/codegen/llvm/interface_type_checking.rs:403
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // interface_extends(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // interface_extends(data, 0);        // Zero size
        // interface_extends(data, 1);        // Single byte
        // interface_extends(data, size);     // Full size
        // interface_extends(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for interface_extends
