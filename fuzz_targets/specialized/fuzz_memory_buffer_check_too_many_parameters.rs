...
// Targets: check_too_many_parameters in src/tools/linter.rs:2086
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // check_too_many_parameters(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // check_too_many_parameters(data, 0);        // Zero size
        // check_too_many_parameters(data, 1);        // Single byte
        // check_too_many_parameters(data, size);     // Full size
        // check_too_many_parameters(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for check_too_many_parameters
