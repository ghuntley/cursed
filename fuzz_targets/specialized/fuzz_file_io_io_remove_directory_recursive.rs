...
// Targets: io_remove_directory_recursive in src/execution/runtime_functions.rs:1029
...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    // Test io_remove_directory_recursive with temp file
    // Example: io_remove_directory_recursive(temp_path);
    // Example: io_remove_directory_recursive(temp_path, data, size);
...
void test_path_operations(const uint8_t *data, size_t size) {
    ...
    // io_remove_directory_recursive(path);
}


// Additional test functions for io_remove_directory_recursive
