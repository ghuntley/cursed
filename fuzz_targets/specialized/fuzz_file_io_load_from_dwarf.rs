...
// Targets: load_from_dwarf in src/runtime/dwarf_parser.rs:40
...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    // Test load_from_dwarf with temp file
    // Example: load_from_dwarf(temp_path);
    // Example: load_from_dwarf(temp_path, data, size);
...
void test_path_operations(const uint8_t *data, size_t size) {
    ...
    // load_from_dwarf(path);
}


// Additional test functions for load_from_dwarf
