...
// Targets: string_starts_with_char in stdlib/configz/mod_complex_syntax.csd:486
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // string_starts_with_char(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // string_starts_with_char(data, 0);        // Zero size
        // string_starts_with_char(data, 1);        // Single byte
        // string_starts_with_char(data, size);     // Full size
        // string_starts_with_char(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for string_starts_with_char
