...
// Targets: replace_string in stdlib/configz/mod_complex.csd:1024
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // replace_string(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // replace_string(data, 0);        // Zero size
        // replace_string(data, 1);        // Single byte
        // replace_string(data, size);     // Full size
        // replace_string(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for replace_string
