...
// Targets: is_recursive in src/codegen/llvm/passes/inlining.rs:1090
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // is_recursive(data, 0);        // Zero size
        // is_recursive(data, 1);        // Single byte
        // is_recursive(data, size);     // Full size
        // is_recursive(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for is_recursive
