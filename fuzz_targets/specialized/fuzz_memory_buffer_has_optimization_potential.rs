...
// Targets: has_optimization_potential in src/optimization/simple_passes.rs:90
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // has_optimization_potential(buffer, data, size);
...
void test_string_operations(const uint8_t *data, size_t size) {
    ...
    // has_optimization_potential(str);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // has_optimization_potential(data, 0);        // Zero size
        // has_optimization_potential(data, 1);        // Single byte
        // has_optimization_potential(data, size);     // Full size
        // has_optimization_potential(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for has_optimization_potential
