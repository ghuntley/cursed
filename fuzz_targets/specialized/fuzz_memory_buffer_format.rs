...
// Targets: format in src/formatter/simple.rs:39
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // format(data, 0);        // Zero size
        // format(data, 1);        // Single byte
        // format(data, size);     // Full size
        // format(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for format
