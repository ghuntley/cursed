...
// Targets: buffer_size in src/runtime/channels/mod.rs:361
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // buffer_size(data, 0);        // Zero size
        // buffer_size(data, 1);        // Single byte
        // buffer_size(data, size);     // Full size
        // buffer_size(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for buffer_size
