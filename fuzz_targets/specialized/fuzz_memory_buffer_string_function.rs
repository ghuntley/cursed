...
// Targets: string_function in benchmarks/cursed/language_features/function_call_bench.csd:28
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // string_function(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // string_function(data, 0);        // Zero size
        // string_function(data, 1);        // Single byte
        // string_function(data, size);     // Full size
        // string_function(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for string_function
