...
// Targets: create_stack_frame in src/debug/enhanced_debug.rs:492
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // create_stack_frame(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // create_stack_frame(data, 0);        // Zero size
        // create_stack_frame(data, 1);        // Single byte
        // create_stack_frame(data, size);     // Full size
        // create_stack_frame(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for create_stack_frame
