...
// Targets: apply_binary_operator in src/execution/mod.rs:1942
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // apply_binary_operator(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // apply_binary_operator(data, 0);        // Zero size
        // apply_binary_operator(data, 1);        // Single byte
        // apply_binary_operator(data, size);     // Full size
        // apply_binary_operator(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for apply_binary_operator
