...
// Targets: add_metric in src/metrics/prometheus_exporter.rs:199
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // add_metric(data, 0);        // Zero size
        // add_metric(data, 1);        // Single byte
        // add_metric(data, size);     // Full size
        // add_metric(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for add_metric
