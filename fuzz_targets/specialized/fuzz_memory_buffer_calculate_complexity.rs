...
// Targets: calculate_complexity in src/coverage/basic_coverage.rs:328
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // calculate_complexity(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // calculate_complexity(data, 0);        // Zero size
        // calculate_complexity(data, 1);        // Single byte
        // calculate_complexity(data, size);     // Full size
        // calculate_complexity(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for calculate_complexity
