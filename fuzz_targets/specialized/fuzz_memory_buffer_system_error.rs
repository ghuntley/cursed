...
// Targets: system_error in src/error/mod.rs:206
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // system_error(data, 0);        // Zero size
        // system_error(data, 1);        // Single byte
        // system_error(data, size);     // Full size
        // system_error(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for system_error
