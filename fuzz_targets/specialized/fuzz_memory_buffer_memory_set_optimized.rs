...
// Targets: memory_set_optimized in stdlib/memory/utils.csd:190
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // memory_set_optimized(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // memory_set_optimized(data, 0);        // Zero size
        // memory_set_optimized(data, 1);        // Single byte
        // memory_set_optimized(data, size);     // Full size
        // memory_set_optimized(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for memory_set_optimized
