...
// Targets: is_rule_enabled in src/linter/config.rs:84
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // is_rule_enabled(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // is_rule_enabled(data, 0);        // Zero size
        // is_rule_enabled(data, 1);        // Single byte
        // is_rule_enabled(data, size);     // Full size
        // is_rule_enabled(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for is_rule_enabled
