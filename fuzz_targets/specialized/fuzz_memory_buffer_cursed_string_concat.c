...
// Targets: cursed_string_concat in runtime/minimal_shims.c:60
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // cursed_string_concat(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // cursed_string_concat(data, 0);        // Zero size
        // cursed_string_concat(data, 1);        // Single byte
        // cursed_string_concat(data, size);     // Full size
        // cursed_string_concat(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for cursed_string_concat
