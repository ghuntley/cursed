...
// Targets: string_len in stdlib/grammar_drip/mod.csd:197
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // string_len(data, 0);        // Zero size
        // string_len(data, 1);        // Single byte
        // string_len(data, size);     // Full size
        // string_len(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for string_len
