...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    // Test test_thread_local_buffers with temp file
    // Example: test_thread_local_buffers(temp_path);
    // Example: test_thread_local_buffers(temp_path, data, size);
...
void test_path_operations(const uint8_t *data, size_t size) {
    ...
    // test_thread_local_buffers(path);
}
