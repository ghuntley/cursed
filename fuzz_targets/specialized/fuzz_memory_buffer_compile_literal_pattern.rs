...
// Targets: compile_literal_pattern in src/pattern_matching.rs:501
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // compile_literal_pattern(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // compile_literal_pattern(data, 0);        // Zero size
        // compile_literal_pattern(data, 1);        // Single byte
        // compile_literal_pattern(data, size);     // Full size
        // compile_literal_pattern(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for compile_literal_pattern
