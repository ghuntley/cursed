...
// Targets: load_dwarf_info in src/runtime/debug_info.rs:1673
...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    // Test load_dwarf_info with temp file
    // Example: load_dwarf_info(temp_path);
    // Example: load_dwarf_info(temp_path, data, size);
...
void test_path_operations(const uint8_t *data, size_t size) {
    ...
    // load_dwarf_info(path);
}


// Additional test functions for load_dwarf_info
