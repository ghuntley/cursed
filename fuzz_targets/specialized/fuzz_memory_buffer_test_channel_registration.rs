...
// Targets: test_channel_registration in src/runtime/channels/lifecycle.rs:1136
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // test_channel_registration(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // test_channel_registration(data, 0);        // Zero size
        // test_channel_registration(data, 1);        // Single byte
        // test_channel_registration(data, size);     // Full size
        // test_channel_registration(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for test_channel_registration
