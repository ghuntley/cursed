...
// Targets: simplifyAnd in src-zig/constant_folder.zig:553
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // simplifyAnd(data, 0);        // Zero size
        // simplifyAnd(data, 1);        // Single byte
        // simplifyAnd(data, size);     // Full size
        // simplifyAnd(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for simplifyAnd
