...
// Targets: get_or_create_basic_type in src/tools/debug_info.rs:617
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // get_or_create_basic_type(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // get_or_create_basic_type(data, 0);        // Zero size
        // get_or_create_basic_type(data, 1);        // Single byte
        // get_or_create_basic_type(data, size);     // Full size
        // get_or_create_basic_type(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for get_or_create_basic_type
