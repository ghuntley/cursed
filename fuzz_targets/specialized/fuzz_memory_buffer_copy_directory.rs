...
// Targets: copy_directory in stdlib/filez/mod.csd:694
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // copy_directory(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // copy_directory(data, 0);        // Zero size
        // copy_directory(data, 1);        // Single byte
        // copy_directory(data, size);     // Full size
        // copy_directory(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for copy_directory
