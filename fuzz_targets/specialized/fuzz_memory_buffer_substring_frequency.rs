...
// Targets: substring_frequency in stdlib/serialization/mod.csd:766
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // substring_frequency(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // substring_frequency(data, 0);        // Zero size
        // substring_frequency(data, 1);        // Single byte
        // substring_frequency(data, size);     // Full size
        // substring_frequency(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for substring_frequency
