...
// Targets: get_vtable in src/runtime/interface_dispatch.rs:273
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // get_vtable(data, 0);        // Zero size
        // get_vtable(data, 1);        // Single byte
        // get_vtable(data, size);     // Full size
        // get_vtable(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for get_vtable
