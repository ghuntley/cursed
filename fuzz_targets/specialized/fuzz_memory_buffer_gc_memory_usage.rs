...
// Targets: gc_memory_usage in stdlib/gc/mod.csd:358
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // gc_memory_usage(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // gc_memory_usage(data, 0);        // Zero size
        // gc_memory_usage(data, 1);        // Single byte
        // gc_memory_usage(data, size);     // Full size
        // gc_memory_usage(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for gc_memory_usage
