...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    // Test packet parsing
    // Test http2_handle_error with network data
    // Example: http2_handle_error(data, size);
    // Example: http2_handle_error(sockfd, data, size);
...
void test_socket_operations(const uint8_t *data, size_t size) {
    ...
    // http2_handle_error(sockfd, data, size);
