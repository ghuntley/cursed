...
// Targets: tcp_socket_receive in stdlib/networking_complete/mod.csd:125
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // tcp_socket_receive(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // tcp_socket_receive(data, 0);        // Zero size
        // tcp_socket_receive(data, 1);        // Single byte
        // tcp_socket_receive(data, size);     // Full size
        // tcp_socket_receive(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for tcp_socket_receive
