...
// Targets: read_source_context in src/runtime/enhanced_error_handling.rs:280
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // read_source_context(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // read_source_context(data, 0);        // Zero size
        // read_source_context(data, 1);        // Single byte
        // read_source_context(data, size);     // Full size
        // read_source_context(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for read_source_context
