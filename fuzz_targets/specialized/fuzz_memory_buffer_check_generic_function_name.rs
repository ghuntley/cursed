...
// Targets: check_generic_function_name in src/tools/linter.rs:2109
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // check_generic_function_name(buffer, data, size);
...
void test_string_operations(const uint8_t *data, size_t size) {
    ...
    // check_generic_function_name(str);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // check_generic_function_name(data, 0);        // Zero size
        // check_generic_function_name(data, 1);        // Single byte
        // check_generic_function_name(data, size);     // Full size
        // check_generic_function_name(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for check_generic_function_name
