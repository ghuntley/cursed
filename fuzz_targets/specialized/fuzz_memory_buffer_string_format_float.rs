...
// Targets: string_format_float in stdlib/string_simple/mod.csd:597
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // string_format_float(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // string_format_float(data, 0);        // Zero size
        // string_format_float(data, 1);        // Single byte
        // string_format_float(data, size);     // Full size
        // string_format_float(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for string_format_float
