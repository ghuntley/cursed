...
// Targets: startParsing in src-zig/performance_integration.zig:224
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // startParsing(data, 0);        // Zero size
        // startParsing(data, 1);        // Single byte
        // startParsing(data, size);     // Full size
        // startParsing(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for startParsing
