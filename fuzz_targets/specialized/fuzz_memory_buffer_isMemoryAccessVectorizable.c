...
// Targets: isMemoryAccessVectorizable in src-zig/loop_optimizer.zig:569
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // isMemoryAccessVectorizable(buffer, data, size);
...
void test_string_operations(const uint8_t *data, size_t size) {
    ...
    // isMemoryAccessVectorizable(str);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // isMemoryAccessVectorizable(data, 0);        // Zero size
        // isMemoryAccessVectorizable(data, 1);        // Single byte
        // isMemoryAccessVectorizable(data, size);     // Full size
        // isMemoryAccessVectorizable(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for isMemoryAccessVectorizable
