...
// Targets: log in src/stdlib/packages/test_vibes/core.rs:121
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // log(data, 0);        // Zero size
        // log(data, 1);        // Single byte
        // log(data, size);     // Full size
        // log(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for log
