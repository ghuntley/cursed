...
// Targets: where_in in src/stdlib/database/orm/query_builder.rs:157
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // where_in(data, 0);        // Zero size
        // where_in(data, 1);        // Single byte
        // where_in(data, size);     // Full size
        // where_in(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for where_in
