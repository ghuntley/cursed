...
// Targets: get_package_versions in src/package_manager/registry.rs:216
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // get_package_versions(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // get_package_versions(data, 0);        // Zero size
        // get_package_versions(data, 1);        // Single byte
        // get_package_versions(data, size);     // Full size
        // get_package_versions(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for get_package_versions
