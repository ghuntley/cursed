...
// Targets: update_config in src/optimization/optimization_full/distributed/mod.rs:174
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // update_config(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // update_config(data, 0);        // Zero size
        // update_config(data, 1);        // Single byte
        // update_config(data, size);     // Full size
        // update_config(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for update_config
