...
// Targets: prepare_statement in stdlib/database_drivers/mod.csd:210
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // prepare_statement(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // prepare_statement(data, 0);        // Zero size
        // prepare_statement(data, 1);        // Single byte
        // prepare_statement(data, size);     // Full size
        // prepare_statement(NULL, 0);        // NULL pointer
    }
}
