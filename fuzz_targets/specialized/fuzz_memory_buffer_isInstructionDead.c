...
// Targets: isInstructionDead in src-zig/dead_code_tracker.zig:119
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // isInstructionDead(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // isInstructionDead(data, 0);        // Zero size
        // isInstructionDead(data, 1);        // Single byte
        // isInstructionDead(data, size);     // Full size
        // isInstructionDead(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for isInstructionDead
