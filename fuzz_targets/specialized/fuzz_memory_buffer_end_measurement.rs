...
// Targets: end_measurement in src/optimization/performance_monitor.rs:41
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // end_measurement(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // end_measurement(data, 0);        // Zero size
        // end_measurement(data, 1);        // Single byte
        // end_measurement(data, size);     // Full size
        // end_measurement(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for end_measurement
