...
// Targets: runtime_write_file in src-zig/stdlib_core.zig:372
...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    // Test runtime_write_file with temp file
    // Example: runtime_write_file(temp_path);
    // Example: runtime_write_file(temp_path, data, size);
...
void test_path_operations(const uint8_t *data, size_t size) {
    ...
    // runtime_write_file(path);
}


// Additional test functions for runtime_write_file
