...
// Targets: array_length_string in stdlib/arrayz/mod.csd:747
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // array_length_string(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // array_length_string(data, 0);        // Zero size
        // array_length_string(data, 1);        // Single byte
        // array_length_string(data, size);     // Full size
        // array_length_string(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for array_length_string
