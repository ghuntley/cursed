...
// Targets: infer_type in src/type_helper.rs:4
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // infer_type(data, 0);        // Zero size
        // infer_type(data, 1);        // Single byte
        // infer_type(data, size);     // Full size
        // infer_type(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for infer_type
