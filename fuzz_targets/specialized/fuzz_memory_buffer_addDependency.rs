...
// Targets: addDependency in stdlib/package_manager/test_package_manager.csd:22
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // addDependency(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // addDependency(data, 0);        // Zero size
        // addDependency(data, 1);        // Single byte
        // addDependency(data, size);     // Full size
        // addDependency(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for addDependency
