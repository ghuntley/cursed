...
// Targets: allocate_with_strategy in src/runtime/heap_optimizer.rs:402
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // allocate_with_strategy(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // allocate_with_strategy(data, 0);        // Zero size
        // allocate_with_strategy(data, 1);        // Single byte
        // allocate_with_strategy(data, size);     // Full size
        // allocate_with_strategy(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for allocate_with_strategy
