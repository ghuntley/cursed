...
// Targets: delete_breakpoint in src/bin/cursed_debug.rs:623
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // delete_breakpoint(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // delete_breakpoint(data, 0);        // Zero size
        // delete_breakpoint(data, 1);        // Single byte
        // delete_breakpoint(data, size);     // Full size
        // delete_breakpoint(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for delete_breakpoint
