...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    // Test http2_server_handle_request with temp file
    // Example: http2_server_handle_request(temp_path);
    // Example: http2_server_handle_request(temp_path, data, size);
...
void test_path_operations(const uint8_t *data, size_t size) {
    ...
    // http2_server_handle_request(path);
}
