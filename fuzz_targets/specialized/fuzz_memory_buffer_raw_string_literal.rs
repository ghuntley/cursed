...
// Targets: raw_string_literal in src/lexer/mod.rs:654
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // raw_string_literal(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // raw_string_literal(data, 0);        // Zero size
        // raw_string_literal(data, 1);        // Single byte
        // raw_string_literal(data, size);     // Full size
        // raw_string_literal(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for raw_string_literal
