...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    // Test packet parsing
    // Test tcp_socket_receive with network data
    // Example: tcp_socket_receive(data, size);
    // Example: tcp_socket_receive(sockfd, data, size);
...
void test_socket_operations(const uint8_t *data, size_t size) {
    ...
    // tcp_socket_receive(sockfd, data, size);
