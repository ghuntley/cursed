...
// Targets: get_type_alignment in src/codegen/llvm/function_compilation.rs:1233
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // get_type_alignment(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // get_type_alignment(data, 0);        // Zero size
        // get_type_alignment(data, 1);        // Single byte
        // get_type_alignment(data, size);     // Full size
        // get_type_alignment(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for get_type_alignment
