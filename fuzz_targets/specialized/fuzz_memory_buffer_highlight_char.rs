...
// Targets: highlight_char in src/repl/enhanced_cursed_repl.rs:156
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // highlight_char(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // highlight_char(data, 0);        // Zero size
        // highlight_char(data, 1);        // Single byte
        // highlight_char(data, size);     // Full size
        // highlight_char(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for highlight_char
