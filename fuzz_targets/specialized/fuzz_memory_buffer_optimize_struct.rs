...
// Targets: optimize_struct in src/type_system/monomorphization_complete.rs:629
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // optimize_struct(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // optimize_struct(data, 0);        // Zero size
        // optimize_struct(data, 1);        // Single byte
        // optimize_struct(data, size);     // Full size
        // optimize_struct(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for optimize_struct
