...
// Targets: downloadPackage in stdlib/package_manager/registry_client.csd:201
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // downloadPackage(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // downloadPackage(data, 0);        // Zero size
        // downloadPackage(data, 1);        // Single byte
        // downloadPackage(data, size);     // Full size
        // downloadPackage(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for downloadPackage
