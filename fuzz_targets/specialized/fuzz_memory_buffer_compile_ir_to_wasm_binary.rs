...
// Targets: compile_ir_to_wasm_binary in src/lib.rs:1758
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // compile_ir_to_wasm_binary(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // compile_ir_to_wasm_binary(data, 0);        // Zero size
        // compile_ir_to_wasm_binary(data, 1);        // Single byte
        // compile_ir_to_wasm_binary(data, size);     // Full size
        // compile_ir_to_wasm_binary(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for compile_ir_to_wasm_binary
