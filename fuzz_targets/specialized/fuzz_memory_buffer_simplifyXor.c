...
// Targets: simplifyXor in src-zig/constant_folder.zig:595
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // simplifyXor(data, 0);        // Zero size
        // simplifyXor(data, 1);        // Single byte
        // simplifyXor(data, size);     // Full size
        // simplifyXor(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for simplifyXor
