...
// Targets: parse_includes in src/ffi/header_parser.rs:650
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // parse_includes(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // parse_includes(data, 0);        // Zero size
        // parse_includes(data, 1);        // Single byte
        // parse_includes(data, size);     // Full size
        // parse_includes(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for parse_includes
