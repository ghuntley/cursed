...
// Targets: extract_variable_name in src/semantic_error_recovery.rs:126
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // extract_variable_name(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // extract_variable_name(data, 0);        // Zero size
        // extract_variable_name(data, 1);        // Single byte
        // extract_variable_name(data, size);     // Full size
        // extract_variable_name(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for extract_variable_name
