...
// Targets: test_struct_formatting in tools/formatter/test_formatter.csd:26
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // test_struct_formatting(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // test_struct_formatting(data, 0);        // Zero size
        // test_struct_formatting(data, 1);        // Single byte
        // test_struct_formatting(data, size);     // Full size
        // test_struct_formatting(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for test_struct_formatting
