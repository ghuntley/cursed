...
// Targets: foldPhiOperation in src-zig/constant_folder.zig:254
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // foldPhiOperation(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // foldPhiOperation(data, 0);        // Zero size
        // foldPhiOperation(data, 1);        // Single byte
        // foldPhiOperation(data, size);     // Full size
        // foldPhiOperation(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for foldPhiOperation
