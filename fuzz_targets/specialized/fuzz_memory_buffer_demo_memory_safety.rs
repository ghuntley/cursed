...
// Targets: demo_memory_safety in examples/ffi_demo.rs:305
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // demo_memory_safety(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // demo_memory_safety(data, 0);        // Zero size
        // demo_memory_safety(data, 1);        // Single byte
        // demo_memory_safety(data, size);     // Full size
        // demo_memory_safety(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for demo_memory_safety
