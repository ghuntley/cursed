...
// Targets: set_rule_enabled in src/tools/linter.rs:555
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // set_rule_enabled(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // set_rule_enabled(data, 0);        // Zero size
        // set_rule_enabled(data, 1);        // Single byte
        // set_rule_enabled(data, size);     // Full size
        // set_rule_enabled(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for set_rule_enabled
