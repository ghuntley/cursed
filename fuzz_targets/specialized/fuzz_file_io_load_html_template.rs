...
// Targets: load_html_template in src/coverage/reporter.rs:436
...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    // Test load_html_template with temp file
    // Example: load_html_template(temp_path);
    // Example: load_html_template(temp_path, data, size);
...
void test_path_operations(const uint8_t *data, size_t size) {
    ...
    // load_html_template(path);
}


// Additional test functions for load_html_template
