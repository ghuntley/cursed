...
// Targets: sendValue in tests/minimal_channel_test.csd:24
...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    // Test packet parsing
    // Test sendValue with network data
    // Example: sendValue(data, size);
    // Example: sendValue(sockfd, data, size);
...
void test_socket_operations(const uint8_t *data, size_t size) {
    ...
    // sendValue(sockfd, data, size);
    
    close(sockfd);
}


// Additional test functions for sendValue
