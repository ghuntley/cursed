...
// Targets: handle_special_key in src/repl/advanced_multi_line_editor.rs:402
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // handle_special_key(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // handle_special_key(data, 0);        // Zero size
        // handle_special_key(data, 1);        // Single byte
        // handle_special_key(data, size);     // Full size
        // handle_special_key(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for handle_special_key
