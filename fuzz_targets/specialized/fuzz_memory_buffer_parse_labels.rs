...
// Targets: parse_labels in src/bin/cursed_metrics.rs:546
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // parse_labels(data, 0);        // Zero size
        // parse_labels(data, 1);        // Single byte
        // parse_labels(data, size);     // Full size
        // parse_labels(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for parse_labels
