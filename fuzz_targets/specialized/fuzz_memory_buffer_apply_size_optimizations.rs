...
// Targets: apply_size_optimizations in src/codegen/llvm/generic_optimization.rs:466
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // apply_size_optimizations(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // apply_size_optimizations(data, 0);        // Zero size
        // apply_size_optimizations(data, 1);        // Single byte
        // apply_size_optimizations(data, size);     // Full size
        // apply_size_optimizations(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for apply_size_optimizations
