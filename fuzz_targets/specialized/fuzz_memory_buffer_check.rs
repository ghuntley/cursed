...
// Targets: check in src/lib.rs:883
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // check(data, 0);        // Zero size
        // check(data, 1);        // Single byte
        // check(data, size);     // Full size
        // check(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for check
