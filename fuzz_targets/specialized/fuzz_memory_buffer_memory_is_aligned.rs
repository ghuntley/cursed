...
// Targets: memory_is_aligned in stdlib/memory/utils.csd:242
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // memory_is_aligned(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // memory_is_aligned(data, 0);        // Zero size
        // memory_is_aligned(data, 1);        // Single byte
        // memory_is_aligned(data, size);     // Full size
        // memory_is_aligned(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for memory_is_aligned
