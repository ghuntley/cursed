...
// Targets: type_check_program in src/tools/mod.rs:191
...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    // Test type_check_program with temp file
    // Example: type_check_program(temp_path);
    // Example: type_check_program(temp_path, data, size);
...
void test_path_operations(const uint8_t *data, size_t size) {
    ...
    // type_check_program(path);
}


// Additional test functions for type_check_program
