...
// Targets: save_results in src/bin/cursed_bench.rs:205
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // save_results(data, 0);        // Zero size
        // save_results(data, 1);        // Single byte
        // save_results(data, size);     // Full size
        // save_results(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for save_results
