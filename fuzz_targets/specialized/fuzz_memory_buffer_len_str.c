...
// Targets: len_str in src-zig/runtime_functions.zig:37
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // len_str(data, 0);        // Zero size
        // len_str(data, 1);        // Single byte
        // len_str(data, size);     // Full size
        // len_str(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for len_str
