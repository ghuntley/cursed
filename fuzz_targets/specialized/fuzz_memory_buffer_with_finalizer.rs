...
// Targets: with_finalizer in src/runtime/gc.rs:763
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // with_finalizer(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // with_finalizer(data, 0);        // Zero size
        // with_finalizer(data, 1);        // Single byte
        // with_finalizer(data, size);     // Full size
        // with_finalizer(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for with_finalizer
