...
// Targets: build_xor_instruction in src/codegen/llvm/inkwell_expression_compiler.rs:538
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // build_xor_instruction(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // build_xor_instruction(data, 0);        // Zero size
        // build_xor_instruction(data, 1);        // Single byte
        // build_xor_instruction(data, size);     // Full size
        // build_xor_instruction(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for build_xor_instruction
