...
// Targets: get_variable_info in src/repl/interactive_debugger.rs:333
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // get_variable_info(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // get_variable_info(data, 0);        // Zero size
        // get_variable_info(data, 1);        // Single byte
        // get_variable_info(data, size);     // Full size
        // get_variable_info(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for get_variable_info
