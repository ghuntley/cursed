...
// Targets: create_type_projection in src/types/full.rs:210
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // create_type_projection(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // create_type_projection(data, 0);        // Zero size
        // create_type_projection(data, 1);        // Single byte
        // create_type_projection(data, size);     // Full size
        // create_type_projection(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for create_type_projection
