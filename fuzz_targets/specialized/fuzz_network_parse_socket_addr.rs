...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    // Test packet parsing
    // Test parse_socket_addr with network data
    // Example: parse_socket_addr(data, size);
    // Example: parse_socket_addr(sockfd, data, size);
...
void test_socket_operations(const uint8_t *data, size_t size) {
    ...
    // parse_socket_addr(sockfd, data, size);
