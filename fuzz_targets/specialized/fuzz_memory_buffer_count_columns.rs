...
// Targets: count_columns in stdlib/csv/mod.csd:42
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // count_columns(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // count_columns(data, 0);        // Zero size
        // count_columns(data, 1);        // Single byte
        // count_columns(data, size);     // Full size
        // count_columns(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for count_columns
