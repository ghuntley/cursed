...
// Targets: demonstrate_concurrency in examples/stdlib_docs_demo.csd:289
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // demonstrate_concurrency(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // demonstrate_concurrency(data, 0);        // Zero size
        // demonstrate_concurrency(data, 1);        // Single byte
        // demonstrate_concurrency(data, size);     // Full size
        // demonstrate_concurrency(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for demonstrate_concurrency
