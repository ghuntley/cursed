...
// Targets: collect_string_literal in stdlib/formatter/mod.csd:264
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // collect_string_literal(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // collect_string_literal(data, 0);        // Zero size
        // collect_string_literal(data, 1);        // Single byte
        // collect_string_literal(data, size);     // Full size
        // collect_string_literal(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for collect_string_literal
