...
// Targets: execute_build_command in src/documentation/build_integration.rs:315
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // execute_build_command(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // execute_build_command(data, 0);        // Zero size
        // execute_build_command(data, 1);        // Single byte
        // execute_build_command(data, size);     // Full size
        // execute_build_command(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for execute_build_command
