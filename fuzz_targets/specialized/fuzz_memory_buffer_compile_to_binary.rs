...
// Targets: compile_to_binary in src/execution_pipeline.rs:304
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // compile_to_binary(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // compile_to_binary(data, 0);        // Zero size
        // compile_to_binary(data, 1);        // Single byte
        // compile_to_binary(data, size);     // Full size
        // compile_to_binary(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for compile_to_binary
