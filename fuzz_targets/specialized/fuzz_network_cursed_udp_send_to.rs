...
// Targets: cursed_udp_send_to in src/execution/cursed_bridge.rs:214
...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    // Test packet parsing
    // Test cursed_udp_send_to with network data
    // Example: cursed_udp_send_to(data, size);
    // Example: cursed_udp_send_to(sockfd, data, size);
...
void test_socket_operations(const uint8_t *data, size_t size) {
    ...
    // cursed_udp_send_to(sockfd, data, size);
    
    close(sockfd);
}


// Additional test functions for cursed_udp_send_to
