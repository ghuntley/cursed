...
// Targets: is_function_referenced in src/codegen/llvm/passes/inlining.rs:1113
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // is_function_referenced(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // is_function_referenced(data, 0);        // Zero size
        // is_function_referenced(data, 1);        // Single byte
        // is_function_referenced(data, size);     // Full size
        // is_function_referenced(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for is_function_referenced
