...
// Targets: enable_rule in src/linter/config.rs:126
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // enable_rule(data, 0);        // Zero size
        // enable_rule(data, 1);        // Single byte
        // enable_rule(data, size);     // Full size
        // enable_rule(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for enable_rule
