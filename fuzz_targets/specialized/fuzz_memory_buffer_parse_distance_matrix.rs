...
// Targets: parse_distance_matrix in src/runtime/pal/x86_64.rs:784
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // parse_distance_matrix(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // parse_distance_matrix(data, 0);        // Zero size
        // parse_distance_matrix(data, 1);        // Single byte
        // parse_distance_matrix(data, size);     // Full size
        // parse_distance_matrix(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for parse_distance_matrix
