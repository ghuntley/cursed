...
// Targets: is_executable_line in src/coverage/simple_collector.rs:397
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // is_executable_line(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // is_executable_line(data, 0);        // Zero size
        // is_executable_line(data, 1);        // Single byte
        // is_executable_line(data, size);     // Full size
        // is_executable_line(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for is_executable_line
