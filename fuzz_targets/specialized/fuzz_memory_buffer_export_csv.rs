...
// Targets: export_csv in src/ffi/profiling.rs:415
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // export_csv(data, 0);        // Zero size
        // export_csv(data, 1);        // Single byte
        // export_csv(data, size);     // Full size
        // export_csv(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for export_csv
