...
// Targets: repeat_fasta in benchmarks/rust/fasta.rs:66
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // repeat_fasta(data, 0);        // Zero size
        // repeat_fasta(data, 1);        // Single byte
        // repeat_fasta(data, size);     // Full size
        // repeat_fasta(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for repeat_fasta
