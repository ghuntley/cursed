...
// Targets: io_parse_bool in stdlib/io_simple/mod_broken.csd:598
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // io_parse_bool(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // io_parse_bool(data, 0);        // Zero size
        // io_parse_bool(data, 1);        // Single byte
        // io_parse_bool(data, size);     // Full size
        // io_parse_bool(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for io_parse_bool
