...
// Targets: create_mock_registry in src/package_manager/resolver_tests.rs:29
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // create_mock_registry(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // create_mock_registry(data, 0);        // Zero size
        // create_mock_registry(data, 1);        // Single byte
        // create_mock_registry(data, size);     // Full size
        // create_mock_registry(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for create_mock_registry
