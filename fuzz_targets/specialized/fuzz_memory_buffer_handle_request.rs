...
// Targets: handle_request in src/bin/cursed_doc_simple.rs:575
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // handle_request(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // handle_request(data, 0);        // Zero size
        // handle_request(data, 1);        // Single byte
        // handle_request(data, size);     // Full size
        // handle_request(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for handle_request
