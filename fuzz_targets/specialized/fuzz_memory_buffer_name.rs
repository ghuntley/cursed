...
// Targets: name in src/runtime/pal/common.rs:818
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // name(data, 0);        // Zero size
        // name(data, 1);        // Single byte
        // name(data, size);     // Full size
        // name(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for name
