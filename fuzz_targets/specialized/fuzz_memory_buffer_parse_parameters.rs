...
// Targets: parse_parameters in src/ffi/header_parser.rs:254
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // parse_parameters(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // parse_parameters(data, 0);        // Zero size
        // parse_parameters(data, 1);        // Single byte
        // parse_parameters(data, size);     // Full size
        // parse_parameters(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for parse_parameters
