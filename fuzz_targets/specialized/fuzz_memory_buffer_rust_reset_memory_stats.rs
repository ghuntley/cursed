...
// Targets: rust_reset_memory_stats in src/runtime/memory_bridge.rs:492
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // rust_reset_memory_stats(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // rust_reset_memory_stats(data, 0);        // Zero size
        // rust_reset_memory_stats(data, 1);        // Single byte
        // rust_reset_memory_stats(data, size);     // Full size
        // rust_reset_memory_stats(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for rust_reset_memory_stats
