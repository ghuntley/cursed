...
// Targets: disable_rule in src/linter/config.rs:140
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // disable_rule(data, 0);        // Zero size
        // disable_rule(data, 1);        // Single byte
        // disable_rule(data, size);     // Full size
        // disable_rule(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for disable_rule
