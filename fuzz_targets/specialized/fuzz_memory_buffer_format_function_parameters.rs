...
// Targets: format_function_parameters in src/formatter/simple.rs:510
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // format_function_parameters(buffer, data, size);
...
void test_string_operations(const uint8_t *data, size_t size) {
    ...
    // format_function_parameters(str);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // format_function_parameters(data, 0);        // Zero size
        // format_function_parameters(data, 1);        // Single byte
        // format_function_parameters(data, size);     // Full size
        // format_function_parameters(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for format_function_parameters
