...
// Targets: test_fit_buffer in tests/bytefit_test.csd:180
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // test_fit_buffer(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // test_fit_buffer(data, 0);        // Zero size
        // test_fit_buffer(data, 1);        // Single byte
        // test_fit_buffer(data, size);     // Full size
        // test_fit_buffer(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for test_fit_buffer
