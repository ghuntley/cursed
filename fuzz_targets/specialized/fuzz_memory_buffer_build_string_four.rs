...
// Targets: build_string_four in stdlib/stringz/mod.csd:52
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // build_string_four(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // build_string_four(data, 0);        // Zero size
        // build_string_four(data, 1);        // Single byte
        // build_string_four(data, size);     // Full size
        // build_string_four(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for build_string_four
