...
// Targets: reverse_string in stdlib/property_test/examples.csd:99
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // reverse_string(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // reverse_string(data, 0);        // Zero size
        // reverse_string(data, 1);        // Single byte
        // reverse_string(data, size);     // Full size
        // reverse_string(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for reverse_string
