...
// Targets: get_global in src/runtime/runtime.rs:722
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // get_global(data, 0);        // Zero size
        // get_global(data, 1);        // Single byte
        // get_global(data, size);     // Full size
        // get_global(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for get_global
