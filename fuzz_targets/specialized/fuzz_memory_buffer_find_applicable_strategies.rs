...
// Targets: find_applicable_strategies in src/runtime/recovery.rs:423
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // find_applicable_strategies(buffer, data, size);
...
void test_string_operations(const uint8_t *data, size_t size) {
    ...
    // find_applicable_strategies(str);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // find_applicable_strategies(data, 0);        // Zero size
        // find_applicable_strategies(data, 1);        // Single byte
        // find_applicable_strategies(data, size);     // Full size
        // find_applicable_strategies(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for find_applicable_strategies
