...
// Targets: route_request in src/documentation/live_server.rs:233
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // route_request(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // route_request(data, 0);        // Zero size
        // route_request(data, 1);        // Single byte
        // route_request(data, size);     // Full size
        // route_request(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for route_request
