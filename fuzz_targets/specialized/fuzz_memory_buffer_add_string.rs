...
// Targets: add_string in stdlib/collections_simple/mod.csd:37
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // add_string(data, 0);        // Zero size
        // add_string(data, 1);        // Single byte
        // add_string(data, size);     // Full size
        // add_string(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for add_string
