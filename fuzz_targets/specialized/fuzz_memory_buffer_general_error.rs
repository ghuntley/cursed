...
// Targets: general_error in src/error/mod.rs:218
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // general_error(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // general_error(data, 0);        // Zero size
        // general_error(data, 1);        // Single byte
        // general_error(data, size);     // Full size
        // general_error(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for general_error
