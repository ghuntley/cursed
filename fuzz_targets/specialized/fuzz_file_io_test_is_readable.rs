...
// Targets: test_is_readable in tests/dropz_file_test.csd:25
...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    // Test test_is_readable with temp file
    // Example: test_is_readable(temp_path);
    // Example: test_is_readable(temp_path, data, size);
...
void test_path_operations(const uint8_t *data, size_t size) {
    ...
    // test_is_readable(path);
}


// Additional test functions for test_is_readable
