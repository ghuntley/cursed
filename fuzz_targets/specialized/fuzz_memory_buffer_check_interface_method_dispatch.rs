...
// Targets: check_interface_method_dispatch in src/type_system/checker.rs:1777
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // check_interface_method_dispatch(buffer, data, size);
...
void test_string_operations(const uint8_t *data, size_t size) {
    ...
    // check_interface_method_dispatch(str);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // check_interface_method_dispatch(data, 0);        // Zero size
        // check_interface_method_dispatch(data, 1);        // Single byte
        // check_interface_method_dispatch(data, size);     // Full size
        // check_interface_method_dispatch(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for check_interface_method_dispatch
