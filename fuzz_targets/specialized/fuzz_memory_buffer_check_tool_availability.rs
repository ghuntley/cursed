...
// Targets: check_tool_availability in src/subprocess_utils.rs:45
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // check_tool_availability(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // check_tool_availability(data, 0);        // Zero size
        // check_tool_availability(data, 1);        // Single byte
        // check_tool_availability(data, size);     // Full size
        // check_tool_availability(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for check_tool_availability
