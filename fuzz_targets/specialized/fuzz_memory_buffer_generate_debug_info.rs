...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // generate_debug_info(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // generate_debug_info(data, 0);        // Zero size
        // generate_debug_info(data, 1);        // Single byte
        // generate_debug_info(data, size);     // Full size
        // generate_debug_info(NULL, 0);        // NULL pointer
    }
}
