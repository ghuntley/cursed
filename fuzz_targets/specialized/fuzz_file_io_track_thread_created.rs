...
// Targets: track_thread_created in src/runtime/performance_tracker.rs:240
...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    // Test track_thread_created with temp file
    // Example: track_thread_created(temp_path);
    // Example: track_thread_created(temp_path, data, size);
...
void test_path_operations(const uint8_t *data, size_t size) {
    ...
    // track_thread_created(path);
}


// Additional test functions for track_thread_created
