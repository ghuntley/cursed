...
// Targets: check_package_naming in src/tools/linter.rs:1253
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // check_package_naming(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // check_package_naming(data, 0);        // Zero size
        // check_package_naming(data, 1);        // Single byte
        // check_package_naming(data, size);     // Full size
        // check_package_naming(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for check_package_naming
