...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    // Test packet parsing
    // Test get_connection_quality with network data
    // Example: get_connection_quality(data, size);
    // Example: get_connection_quality(sockfd, data, size);
...
void test_socket_operations(const uint8_t *data, size_t size) {
    ...
    // get_connection_quality(sockfd, data, size);
