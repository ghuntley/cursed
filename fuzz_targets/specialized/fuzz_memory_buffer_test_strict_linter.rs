...
// Targets: test_strict_linter in src/tools/linter_backup.rs:2314
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // test_strict_linter(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // test_strict_linter(data, 0);        // Zero size
        // test_strict_linter(data, 1);        // Single byte
        // test_strict_linter(data, size);     // Full size
        // test_strict_linter(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for test_strict_linter
