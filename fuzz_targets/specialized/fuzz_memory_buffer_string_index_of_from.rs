...
// Targets: string_index_of_from in stdlib/config/mod.csd:411
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // string_index_of_from(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // string_index_of_from(data, 0);        // Zero size
        // string_index_of_from(data, 1);        // Single byte
        // string_index_of_from(data, size);     // Full size
        // string_index_of_from(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for string_index_of_from
