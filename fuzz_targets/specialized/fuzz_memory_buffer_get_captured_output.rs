...
// Targets: get_captured_output in stdlib/exec_vibez/mod.csd:551
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // get_captured_output(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // get_captured_output(data, 0);        // Zero size
        // get_captured_output(data, 1);        // Single byte
        // get_captured_output(data, size);     // Full size
        // get_captured_output(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for get_captured_output
