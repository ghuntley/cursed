...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // add_error_with_file(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // add_error_with_file(data, 0);        // Zero size
        // add_error_with_file(data, 1);        // Single byte
        // add_error_with_file(data, size);     // Full size
        // add_error_with_file(NULL, 0);        // NULL pointer
    }
}
