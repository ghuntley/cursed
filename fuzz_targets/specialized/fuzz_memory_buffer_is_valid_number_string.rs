...
// Targets: is_valid_number_string in stdlib/vibez/core_functions.csd:384
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // is_valid_number_string(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // is_valid_number_string(data, 0);        // Zero size
        // is_valid_number_string(data, 1);        // Single byte
        // is_valid_number_string(data, size);     // Full size
        // is_valid_number_string(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for is_valid_number_string
