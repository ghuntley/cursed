...
// Targets: parseType in src-zig/function_overloading.zig:333
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // parseType(data, 0);        // Zero size
        // parseType(data, 1);        // Single byte
        // parseType(data, size);     // Full size
        // parseType(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for parseType
