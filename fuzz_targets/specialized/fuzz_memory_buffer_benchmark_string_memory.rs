...
// Targets: benchmark_string_memory in benchmarks/cursed/compiler/memory_usage_bench.csd:86
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // benchmark_string_memory(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // benchmark_string_memory(data, 0);        // Zero size
        // benchmark_string_memory(data, 1);        // Single byte
        // benchmark_string_memory(data, size);     // Full size
        // benchmark_string_memory(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for benchmark_string_memory
