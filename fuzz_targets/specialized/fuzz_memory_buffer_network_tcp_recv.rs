...
// Targets: network_tcp_recv in src/execution/runtime_functions.rs:5196
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // network_tcp_recv(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // network_tcp_recv(data, 0);        // Zero size
        // network_tcp_recv(data, 1);        // Single byte
        // network_tcp_recv(data, size);     // Full size
        // network_tcp_recv(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for network_tcp_recv
