...
// Targets: inline_function in src/codegen/llvm/generic_optimization.rs:349
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // inline_function(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // inline_function(data, 0);        // Zero size
        // inline_function(data, 1);        // Single byte
        // inline_function(data, size);     // Full size
        // inline_function(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for inline_function
