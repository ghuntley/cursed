...
// Targets: resolve_type_string_to_expression in src/type_system/checker.rs:239
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // resolve_type_string_to_expression(buffer, data, size);
...
void test_string_operations(const uint8_t *data, size_t size) {
    ...
    // resolve_type_string_to_expression(str);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // resolve_type_string_to_expression(data, 0);        // Zero size
        // resolve_type_string_to_expression(data, 1);        // Single byte
        // resolve_type_string_to_expression(data, size);     // Full size
        // resolve_type_string_to_expression(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for resolve_type_string_to_expression
