...
// Targets: string_unescape_special_chars in stdlib/stringz/mod_complex.csd:629
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // string_unescape_special_chars(buffer, data, size);
...
void test_string_operations(const uint8_t *data, size_t size) {
    ...
    // string_unescape_special_chars(str);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // string_unescape_special_chars(data, 0);        // Zero size
        // string_unescape_special_chars(data, 1);        // Single byte
        // string_unescape_special_chars(data, size);     // Full size
        // string_unescape_special_chars(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for string_unescape_special_chars
