...
// Targets: foldSelectOperation in src-zig/constant_folder.zig:231
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // foldSelectOperation(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // foldSelectOperation(data, 0);        // Zero size
        // foldSelectOperation(data, 1);        // Single byte
        // foldSelectOperation(data, size);     // Full size
        // foldSelectOperation(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for foldSelectOperation
