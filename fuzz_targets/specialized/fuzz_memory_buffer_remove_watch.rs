...
// Targets: remove_watch in src/repl/interactive_debugger.rs:283
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // remove_watch(data, 0);        // Zero size
        // remove_watch(data, 1);        // Single byte
        // remove_watch(data, size);     // Full size
        // remove_watch(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for remove_watch
