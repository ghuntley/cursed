...
// Targets: contains_potential_credential in src/linter/rules.rs:379
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // contains_potential_credential(buffer, data, size);
...
void test_string_operations(const uint8_t *data, size_t size) {
    ...
    // contains_potential_credential(str);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // contains_potential_credential(data, 0);        // Zero size
        // contains_potential_credential(data, 1);        // Single byte
        // contains_potential_credential(data, size);     // Full size
        // contains_potential_credential(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for contains_potential_credential
