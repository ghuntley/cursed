...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    // Test packet parsing
    // Test http2_push_promise_frame with network data
    // Example: http2_push_promise_frame(data, size);
    // Example: http2_push_promise_frame(sockfd, data, size);
...
void test_socket_operations(const uint8_t *data, size_t size) {
    ...
    // http2_push_promise_frame(sockfd, data, size);
