...
// Targets: check_member_access in src/type_system/checker.rs:685
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // check_member_access(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // check_member_access(data, 0);        // Zero size
        // check_member_access(data, 1);        // Single byte
        // check_member_access(data, size);     // Full size
        // check_member_access(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for check_member_access
