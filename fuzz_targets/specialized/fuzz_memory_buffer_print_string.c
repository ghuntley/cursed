...
// Targets: print_string in src-zig/stdlib_core.zig:23
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // print_string(data, 0);        // Zero size
        // print_string(data, 1);        // Single byte
        // print_string(data, size);     // Full size
        // print_string(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for print_string
