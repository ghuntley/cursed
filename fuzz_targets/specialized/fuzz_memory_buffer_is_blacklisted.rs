...
// Targets: is_blacklisted in src/codegen/llvm/jit_compilation_stabilized.rs:413
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // is_blacklisted(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // is_blacklisted(data, 0);        // Zero size
        // is_blacklisted(data, 1);        // Single byte
        // is_blacklisted(data, size);     // Full size
        // is_blacklisted(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for is_blacklisted
