...
// Targets: build_mul_instruction in src/codegen/llvm/inkwell_expression_compiler.rs:314
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // build_mul_instruction(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // build_mul_instruction(data, 0);        // Zero size
        // build_mul_instruction(data, 1);        // Single byte
        // build_mul_instruction(data, size);     // Full size
        // build_mul_instruction(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for build_mul_instruction
