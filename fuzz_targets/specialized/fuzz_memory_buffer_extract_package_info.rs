...
// Targets: extract_package_info in src/documentation/api_extractor.rs:348
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // extract_package_info(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // extract_package_info(data, 0);        // Zero size
        // extract_package_info(data, 1);        // Single byte
        // extract_package_info(data, size);     // Full size
        // extract_package_info(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for extract_package_info
