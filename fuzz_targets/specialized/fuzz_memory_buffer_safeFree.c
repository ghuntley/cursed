...
// Targets: safeFree in src-zig/safe_operations.zig:74
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // safeFree(data, 0);        // Zero size
        // safeFree(data, 1);        // Single byte
        // safeFree(data, size);     // Full size
        // safeFree(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for safeFree
