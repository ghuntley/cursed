...
// Targets: check_export_patterns in src/tools/linter.rs:1571
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // check_export_patterns(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // check_export_patterns(data, 0);        // Zero size
        // check_export_patterns(data, 1);        // Single byte
        // check_export_patterns(data, size);     // Full size
        // check_export_patterns(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for check_export_patterns
