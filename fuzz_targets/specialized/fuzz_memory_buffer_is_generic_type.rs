...
// Targets: is_generic_type in src/type_system/monomorphisation.rs:995
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // is_generic_type(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // is_generic_type(data, 0);        // Zero size
        // is_generic_type(data, 1);        // Single byte
        // is_generic_type(data, size);     // Full size
        // is_generic_type(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for is_generic_type
