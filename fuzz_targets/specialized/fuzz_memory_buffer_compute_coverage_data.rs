...
// Targets: compute_coverage_data in src/coverage/simple_collector.rs:208
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // compute_coverage_data(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // compute_coverage_data(data, 0);        // Zero size
        // compute_coverage_data(data, 1);        // Single byte
        // compute_coverage_data(data, size);     // Full size
        // compute_coverage_data(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for compute_coverage_data
