...
// Targets: evaluate_member_access in src/execution/mod.rs:2900
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // evaluate_member_access(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // evaluate_member_access(data, 0);        // Zero size
        // evaluate_member_access(data, 1);        // Single byte
        // evaluate_member_access(data, size);     // Full size
        // evaluate_member_access(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for evaluate_member_access
