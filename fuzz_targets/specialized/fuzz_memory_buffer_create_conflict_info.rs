...
// Targets: create_conflict_info in src/package_manager/resolver.rs:246
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // create_conflict_info(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // create_conflict_info(data, 0);        // Zero size
        // create_conflict_info(data, 1);        // Single byte
        // create_conflict_info(data, size);     // Full size
        // create_conflict_info(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for create_conflict_info
