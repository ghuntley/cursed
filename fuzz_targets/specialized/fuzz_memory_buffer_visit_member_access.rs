...
// Targets: visit_member_access in src/type_system/checker.rs:2264
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // visit_member_access(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // visit_member_access(data, 0);        // Zero size
        // visit_member_access(data, 1);        // Single byte
        // visit_member_access(data, size);     // Full size
        // visit_member_access(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for visit_member_access
