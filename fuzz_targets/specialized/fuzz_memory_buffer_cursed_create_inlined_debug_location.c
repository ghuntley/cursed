...
// Targets: cursed_create_inlined_debug_location in src-zig/inlined_function_debug.zig:478
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // cursed_create_inlined_debug_location(buffer, data, size);
...
void test_string_operations(const uint8_t *data, size_t size) {
    ...
    // cursed_create_inlined_debug_location(str);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // cursed_create_inlined_debug_location(data, 0);        // Zero size
        // cursed_create_inlined_debug_location(data, 1);        // Single byte
        // cursed_create_inlined_debug_location(data, size);     // Full size
        // cursed_create_inlined_debug_location(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for cursed_create_inlined_debug_location
