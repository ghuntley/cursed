...
// Targets: test_lexer_errors in src/bin/test_file_errors.rs:32
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // test_lexer_errors(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // test_lexer_errors(data, 0);        // Zero size
        // test_lexer_errors(data, 1);        // Single byte
        // test_lexer_errors(data, size);     // Full size
        // test_lexer_errors(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for test_lexer_errors
