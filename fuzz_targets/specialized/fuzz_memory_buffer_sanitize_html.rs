...
// Targets: sanitize_html in stdlib/htmlrizzler/mod.csd:360
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // sanitize_html(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // sanitize_html(data, 0);        // Zero size
        // sanitize_html(data, 1);        // Single byte
        // sanitize_html(data, size);     // Full size
        // sanitize_html(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for sanitize_html
