...
// Targets: parse_struct in stdlib/formatter/mod.csd:631
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // parse_struct(data, 0);        // Zero size
        // parse_struct(data, 1);        // Single byte
        // parse_struct(data, size);     // Full size
        // parse_struct(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for parse_struct
