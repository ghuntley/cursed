...
// Targets: get_var in src/runtime/process.rs:789
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // get_var(data, 0);        // Zero size
        // get_var(data, 1);        // Single byte
        // get_var(data, size);     // Full size
        // get_var(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for get_var
