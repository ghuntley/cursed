...
// Targets: worker_main in src/runtime/advanced_goroutine_scheduler.rs:692
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // worker_main(data, 0);        // Zero size
        // worker_main(data, 1);        // Single byte
        // worker_main(data, size);     // Full size
        // worker_main(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for worker_main
