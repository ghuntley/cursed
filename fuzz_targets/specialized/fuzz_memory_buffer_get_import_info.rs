...
// Targets: get_import_info in src/imports/resolver.rs:925
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // get_import_info(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // get_import_info(data, 0);        // Zero size
        // get_import_info(data, 1);        // Single byte
        // get_import_info(data, size);     // Full size
        // get_import_info(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for get_import_info
