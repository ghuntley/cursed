...
// Targets: string_contains in stdlib/encode_mood/test_encode_mood.csd:237
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // string_contains(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // string_contains(data, 0);        // Zero size
        // string_contains(data, 1);        // Single byte
        // string_contains(data, size);     // Full size
        // string_contains(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for string_contains
