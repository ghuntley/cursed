...
// Targets: remove_worker in src/optimization/optimization_full/distributed/mod.rs:171
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // remove_worker(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // remove_worker(data, 0);        // Zero size
        // remove_worker(data, 1);        // Single byte
        // remove_worker(data, size);     // Full size
        // remove_worker(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for remove_worker
