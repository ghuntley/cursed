...
// Targets: find_test_files in src/main.rs:2796
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // find_test_files(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // find_test_files(data, 0);        // Zero size
        // find_test_files(data, 1);        // Single byte
        // find_test_files(data, size);     // Full size
        // find_test_files(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for find_test_files
