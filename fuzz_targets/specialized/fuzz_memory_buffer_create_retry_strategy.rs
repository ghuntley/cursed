...
// Targets: create_retry_strategy in src/runtime/recovery.rs:719
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // create_retry_strategy(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // create_retry_strategy(data, 0);        // Zero size
        // create_retry_strategy(data, 1);        // Single byte
        // create_retry_strategy(data, size);     // Full size
        // create_retry_strategy(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for create_retry_strategy
