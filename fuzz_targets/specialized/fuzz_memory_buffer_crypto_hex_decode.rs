...
// Targets: crypto_hex_decode in stdlib/crypto_production/mod.csd:473
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // crypto_hex_decode(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // crypto_hex_decode(data, 0);        // Zero size
        // crypto_hex_decode(data, 1);        // Single byte
        // crypto_hex_decode(data, size);     // Full size
        // crypto_hex_decode(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for crypto_hex_decode
