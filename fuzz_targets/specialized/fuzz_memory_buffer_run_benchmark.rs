...
// Targets: run_benchmark in src/bin/cursed_bench.rs:62
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // run_benchmark(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // run_benchmark(data, 0);        // Zero size
        // run_benchmark(data, 1);        // Single byte
        // run_benchmark(data, size);     // Full size
        // run_benchmark(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for run_benchmark
