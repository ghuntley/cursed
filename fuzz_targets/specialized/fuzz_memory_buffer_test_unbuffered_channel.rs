...
// Targets: test_unbuffered_channel in src/runtime/channels/buffer.rs:472
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // test_unbuffered_channel(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // test_unbuffered_channel(data, 0);        // Zero size
        // test_unbuffered_channel(data, 1);        // Single byte
        // test_unbuffered_channel(data, size);     // Full size
        // test_unbuffered_channel(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for test_unbuffered_channel
