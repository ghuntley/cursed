...
// Targets: load_profile in src/optimization/pgo/profile_storage.rs:127
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // load_profile(data, 0);        // Zero size
        // load_profile(data, 1);        // Single byte
        // load_profile(data, size);     // Full size
        // load_profile(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for load_profile
