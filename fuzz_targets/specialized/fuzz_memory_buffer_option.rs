...
// Targets: option in src/stdlib/database/postgres/config.rs:144
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // option(data, 0);        // Zero size
        // option(data, 1);        // Single byte
        // option(data, size);     // Full size
        // option(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for option
