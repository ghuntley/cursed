...
// Targets: string_ends_with_extension in stdlib/configz/mod_complex_syntax.csd:500
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // string_ends_with_extension(buffer, data, size);
...
void test_string_operations(const uint8_t *data, size_t size) {
    ...
    // string_ends_with_extension(str);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // string_ends_with_extension(data, 0);        // Zero size
        // string_ends_with_extension(data, 1);        // Single byte
        // string_ends_with_extension(data, size);     // Full size
        // string_ends_with_extension(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for string_ends_with_extension
