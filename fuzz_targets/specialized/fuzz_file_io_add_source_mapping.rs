...
// Targets: add_source_mapping in src/runtime/debug_info.rs:304
...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    // Test add_source_mapping with temp file
    // Example: add_source_mapping(temp_path);
    // Example: add_source_mapping(temp_path, data, size);
...
void test_path_operations(const uint8_t *data, size_t size) {
    ...
    // add_source_mapping(path);
}


// Additional test functions for add_source_mapping
