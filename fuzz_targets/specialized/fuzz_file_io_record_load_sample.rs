...
// Targets: record_load_sample in src/runtime/preemptive_scheduler.rs:454
...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    // Test record_load_sample with temp file
    // Example: record_load_sample(temp_path);
    // Example: record_load_sample(temp_path, data, size);
...
void test_path_operations(const uint8_t *data, size_t size) {
    ...
    // record_load_sample(path);
}


// Additional test functions for record_load_sample
