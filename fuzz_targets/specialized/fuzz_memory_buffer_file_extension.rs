...
// Targets: file_extension in src/stdlib/squish_core/zlib.rs:187
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // file_extension(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // file_extension(data, 0);        // Zero size
        // file_extension(data, 1);        // Single byte
        // file_extension(data, size);     // Full size
        // file_extension(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for file_extension
