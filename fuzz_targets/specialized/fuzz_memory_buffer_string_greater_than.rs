...
// Targets: string_greater_than in stdlib/string_simple/mod.csd:124
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // string_greater_than(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // string_greater_than(data, 0);        // Zero size
        // string_greater_than(data, 1);        // Single byte
        // string_greater_than(data, size);     // Full size
        // string_greater_than(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for string_greater_than
