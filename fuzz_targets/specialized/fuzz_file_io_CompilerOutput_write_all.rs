...
// Targets: CompilerOutput_write_all in stdlib/io_enhanced/mod.csd:267
...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    // Test CompilerOutput_write_all with temp file
    // Example: CompilerOutput_write_all(temp_path);
    // Example: CompilerOutput_write_all(temp_path, data, size);
...
void test_path_operations(const uint8_t *data, size_t size) {
    ...
    // CompilerOutput_write_all(path);
}


// Additional test functions for CompilerOutput_write_all
