...
// Targets: publish_package in src/package_manager/mod.rs:239
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // publish_package(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // publish_package(data, 0);        // Zero size
        // publish_package(data, 1);        // Single byte
        // publish_package(data, size);     // Full size
        // publish_package(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for publish_package
