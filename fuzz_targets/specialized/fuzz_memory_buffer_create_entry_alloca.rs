...
// Targets: create_entry_alloca in src/codegen/llvm/improved_codegen.rs:493
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // create_entry_alloca(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // create_entry_alloca(data, 0);        // Zero size
        // create_entry_alloca(data, 1);        // Single byte
        // create_entry_alloca(data, size);     // Full size
        // create_entry_alloca(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for create_entry_alloca
