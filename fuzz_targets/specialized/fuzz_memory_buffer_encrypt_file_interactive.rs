...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // encrypt_file_interactive(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // encrypt_file_interactive(data, 0);        // Zero size
        // encrypt_file_interactive(data, 1);        // Single byte
        // encrypt_file_interactive(data, size);     // Full size
        // encrypt_file_interactive(NULL, 0);        // NULL pointer
    }
}
