...
// Targets: write_to_file in src/ffi/error_handling.rs:471
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // write_to_file(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // write_to_file(data, 0);        // Zero size
        // write_to_file(data, 1);        // Single byte
        // write_to_file(data, size);     // Full size
        // write_to_file(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for write_to_file
