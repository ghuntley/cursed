...
// Targets: into in src/stdlib/database/builder.rs:234
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // into(data, 0);        // Zero size
        // into(data, 1);        // Single byte
        // into(data, size);     // Full size
        // into(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for into
