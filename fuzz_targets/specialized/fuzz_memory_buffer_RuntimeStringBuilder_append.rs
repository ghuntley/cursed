...
// Targets: RuntimeStringBuilder_append in stdlib/runtime_core/mod.csd:160
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // RuntimeStringBuilder_append(buffer, data, size);
...
void test_string_operations(const uint8_t *data, size_t size) {
    ...
    // RuntimeStringBuilder_append(str);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // RuntimeStringBuilder_append(data, 0);        // Zero size
        // RuntimeStringBuilder_append(data, 1);        // Single byte
        // RuntimeStringBuilder_append(data, size);     // Full size
        // RuntimeStringBuilder_append(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for RuntimeStringBuilder_append
