...
// Targets: getAsyncCapabilities in src-zig/windows_async_integration.zig:315
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // getAsyncCapabilities(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // getAsyncCapabilities(data, 0);        // Zero size
        // getAsyncCapabilities(data, 1);        // Single byte
        // getAsyncCapabilities(data, size);     // Full size
        // getAsyncCapabilities(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for getAsyncCapabilities
