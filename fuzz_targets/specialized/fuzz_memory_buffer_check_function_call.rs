...
// Targets: check_function_call in src/type_system/checker.rs:638
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // check_function_call(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // check_function_call(data, 0);        // Zero size
        // check_function_call(data, 1);        // Single byte
        // check_function_call(data, size);     // Full size
        // check_function_call(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for check_function_call
