...
// Targets: is_object_string in stdlib/configz/mod_complex_syntax.csd:597
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // is_object_string(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // is_object_string(data, 0);        // Zero size
        // is_object_string(data, 1);        // Single byte
        // is_object_string(data, size);     // Full size
        // is_object_string(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for is_object_string
