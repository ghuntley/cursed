...
// Targets: AssertHasSuffix in stdlib/test_vibes/mod.csd:430
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // AssertHasSuffix(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // AssertHasSuffix(data, 0);        // Zero size
        // AssertHasSuffix(data, 1);        // Single byte
        // AssertHasSuffix(data, size);     // Full size
        // AssertHasSuffix(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for AssertHasSuffix
