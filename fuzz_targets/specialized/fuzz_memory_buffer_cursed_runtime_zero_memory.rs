...
// Targets: cursed_runtime_zero_memory in src/runtime/goroutine_context.rs:2814
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // cursed_runtime_zero_memory(buffer, data, size);
...
void test_string_operations(const uint8_t *data, size_t size) {
    ...
    // cursed_runtime_zero_memory(str);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // cursed_runtime_zero_memory(data, 0);        // Zero size
        // cursed_runtime_zero_memory(data, 1);        // Single byte
        // cursed_runtime_zero_memory(data, size);     // Full size
        // cursed_runtime_zero_memory(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for cursed_runtime_zero_memory
