...
// Targets: memory_usage in stdlib/sys_core/mod.csd:93
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // memory_usage(data, 0);        // Zero size
        // memory_usage(data, 1);        // Single byte
        // memory_usage(data, size);     // Full size
        // memory_usage(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for memory_usage
