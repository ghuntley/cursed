...
// Targets: active_count in src/runtime/runtime.rs:223
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // active_count(data, 0);        // Zero size
        // active_count(data, 1);        // Single byte
        // active_count(data, size);     // Full size
        // active_count(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for active_count
