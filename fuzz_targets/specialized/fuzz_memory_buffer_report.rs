...
// Targets: report in src/subprocess_utils.rs:86
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // report(data, 0);        // Zero size
        // report(data, 1);        // Single byte
        // report(data, size);     // Full size
        // report(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for report
