...
// Targets: contains_substring in stdlib/sys_core/mod.csd:61
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // contains_substring(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // contains_substring(data, 0);        // Zero size
        // contains_substring(data, 1);        // Single byte
        // contains_substring(data, size);     // Full size
        // contains_substring(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for contains_substring
