...
// Targets: effective_name in src/stdlib/json_tea/mod.rs:88
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // effective_name(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // effective_name(data, 0);        // Zero size
        // effective_name(data, 1);        // Single byte
        // effective_name(data, size);     // Full size
        // effective_name(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for effective_name
