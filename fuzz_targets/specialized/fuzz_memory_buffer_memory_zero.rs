...
// Targets: memory_zero in stdlib/memory/utils.csd:217
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // memory_zero(data, 0);        // Zero size
        // memory_zero(data, 1);        // Single byte
        // memory_zero(data, size);     // Full size
        // memory_zero(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for memory_zero
