...
// Targets: parse_structs in src/ffi/header_parser.rs:370
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // parse_structs(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // parse_structs(data, 0);        // Zero size
        // parse_structs(data, 1);        // Single byte
        // parse_structs(data, size);     // Full size
        // parse_structs(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for parse_structs
