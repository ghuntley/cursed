...
// Targets: AssertHasPrefix in stdlib/test_vibes/mod.csd:424
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // AssertHasPrefix(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // AssertHasPrefix(data, 0);        // Zero size
        // AssertHasPrefix(data, 1);        // Single byte
        // AssertHasPrefix(data, size);     // Full size
        // AssertHasPrefix(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for AssertHasPrefix
