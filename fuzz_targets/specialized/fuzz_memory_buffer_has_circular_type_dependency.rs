...
// Targets: has_circular_type_dependency in src/type_system/monomorphisation.rs:676
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // has_circular_type_dependency(buffer, data, size);
...
void test_string_operations(const uint8_t *data, size_t size) {
    ...
    // has_circular_type_dependency(str);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // has_circular_type_dependency(data, 0);        // Zero size
        // has_circular_type_dependency(data, 1);        // Single byte
        // has_circular_type_dependency(data, size);     // Full size
        // has_circular_type_dependency(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for has_circular_type_dependency
