...
// Targets: call_count in src/stdlib/packages/test_vibes/mocking.rs:51
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // call_count(data, 0);        // Zero size
        // call_count(data, 1);        // Single byte
        // call_count(data, size);     // Full size
        // call_count(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for call_count
