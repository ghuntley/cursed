...
// Targets: find_cursed_files_recursive in src/bin/cursed_coverage.rs:183
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // find_cursed_files_recursive(buffer, data, size);
...
void test_string_operations(const uint8_t *data, size_t size) {
    ...
    // find_cursed_files_recursive(str);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // find_cursed_files_recursive(data, 0);        // Zero size
        // find_cursed_files_recursive(data, 1);        // Single byte
        // find_cursed_files_recursive(data, size);     // Full size
        // find_cursed_files_recursive(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for find_cursed_files_recursive
