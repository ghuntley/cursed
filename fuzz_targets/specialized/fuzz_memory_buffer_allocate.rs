...
// Targets: allocate in src/runtime/gc.rs:464
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // allocate(data, 0);        // Zero size
        // allocate(data, 1);        // Single byte
        // allocate(data, size);     // Full size
        // allocate(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for allocate
