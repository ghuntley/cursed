...
// Targets: to_string_repr in src/runtime/value/mod.rs:134
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // to_string_repr(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // to_string_repr(data, 0);        // Zero size
        // to_string_repr(data, 1);        // Single byte
        // to_string_repr(data, size);     // Full size
        // to_string_repr(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for to_string_repr
