...
// Targets: string_escape in stdlib/string_pure/mod.csd:668
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // string_escape(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // string_escape(data, 0);        // Zero size
        // string_escape(data, 1);        // Single byte
        // string_escape(data, size);     // Full size
        // string_escape(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for string_escape
