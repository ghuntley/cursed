...
// Targets: discover_tests in src/bin/cursed_test.rs:111
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // discover_tests(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // discover_tests(data, 0);        // Zero size
        // discover_tests(data, 1);        // Single byte
        // discover_tests(data, size);     // Full size
        // discover_tests(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for discover_tests
