...
// Targets: remove_root in src/runtime/memory.rs:474
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // remove_root(data, 0);        // Zero size
        // remove_root(data, 1);        // Single byte
        // remove_root(data, size);     // Full size
        // remove_root(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for remove_root
