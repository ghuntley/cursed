...
// Targets: topological_sort in src/type_system/constraint_resolver.rs:847
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // topological_sort(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // topological_sort(data, 0);        // Zero size
        // topological_sort(data, 1);        // Single byte
        // topological_sort(data, size);     // Full size
        // topological_sort(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for topological_sort
