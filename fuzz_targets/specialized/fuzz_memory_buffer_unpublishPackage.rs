...
// Targets: unpublishPackage in stdlib/package_manager/registry_client.csd:297
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // unpublishPackage(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // unpublishPackage(data, 0);        // Zero size
        // unpublishPackage(data, 1);        // Single byte
        // unpublishPackage(data, size);     // Full size
        // unpublishPackage(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for unpublishPackage
