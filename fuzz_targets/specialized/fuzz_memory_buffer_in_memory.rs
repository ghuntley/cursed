...
// Targets: in_memory in src/stdlib/packages/web_vibez/ratelimit/store.rs:103
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // in_memory(data, 0);        // Zero size
        // in_memory(data, 1);        // Single byte
        // in_memory(data, size);     // Full size
        // in_memory(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for in_memory
