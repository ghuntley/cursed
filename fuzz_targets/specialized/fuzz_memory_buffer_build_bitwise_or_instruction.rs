...
// Targets: build_bitwise_or_instruction in src/codegen/llvm/inkwell_expression_compiler.rs:526
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // build_bitwise_or_instruction(buffer, data, size);
...
void test_string_operations(const uint8_t *data, size_t size) {
    ...
    // build_bitwise_or_instruction(str);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // build_bitwise_or_instruction(data, 0);        // Zero size
        // build_bitwise_or_instruction(data, 1);        // Single byte
        // build_bitwise_or_instruction(data, size);     // Full size
        // build_bitwise_or_instruction(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for build_bitwise_or_instruction
