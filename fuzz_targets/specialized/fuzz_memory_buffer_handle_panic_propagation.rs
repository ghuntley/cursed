...
// Targets: handle_panic_propagation in src/runtime/goroutine.rs:1218
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // handle_panic_propagation(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // handle_panic_propagation(data, 0);        // Zero size
        // handle_panic_propagation(data, 1);        // Single byte
        // handle_panic_propagation(data, size);     // Full size
        // handle_panic_propagation(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for handle_panic_propagation
