...
// Targets: demo_struct_field_analysis in examples/lookin_glass_demo.csd:92
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // demo_struct_field_analysis(buffer, data, size);
...
void test_string_operations(const uint8_t *data, size_t size) {
    ...
    // demo_struct_field_analysis(str);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // demo_struct_field_analysis(data, 0);        // Zero size
        // demo_struct_field_analysis(data, 1);        // Single byte
        // demo_struct_field_analysis(data, size);     // Full size
        // demo_struct_field_analysis(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for demo_struct_field_analysis
