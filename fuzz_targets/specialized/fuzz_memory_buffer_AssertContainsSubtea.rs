...
// Targets: AssertContainsSubtea in stdlib/test_vibes/mod.csd:418
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // AssertContainsSubtea(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // AssertContainsSubtea(data, 0);        // Zero size
        // AssertContainsSubtea(data, 1);        // Single byte
        // AssertContainsSubtea(data, size);     // Full size
        // AssertContainsSubtea(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for AssertContainsSubtea
