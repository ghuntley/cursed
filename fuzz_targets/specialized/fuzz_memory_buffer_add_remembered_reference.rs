...
// Targets: add_remembered_reference in src/memory/generational.rs:300
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // add_remembered_reference(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // add_remembered_reference(data, 0);        // Zero size
        // add_remembered_reference(data, 1);        // Single byte
        // add_remembered_reference(data, size);     // Full size
        // add_remembered_reference(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for add_remembered_reference
