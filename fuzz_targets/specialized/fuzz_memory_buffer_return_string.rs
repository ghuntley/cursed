...
// Targets: return_string in benchmarks/cursed/language_features/function_call_bench.csd:295
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // return_string(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // return_string(data, 0);        // Zero size
        // return_string(data, 1);        // Single byte
        // return_string(data, size);     // Full size
        // return_string(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for return_string
