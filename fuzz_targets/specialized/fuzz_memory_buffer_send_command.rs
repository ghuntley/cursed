...
// Targets: send_command in src/debug/lldb_integration.rs:290
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // send_command(data, 0);        // Zero size
        // send_command(data, 1);        // Single byte
        // send_command(data, size);     // Full size
        // send_command(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for send_command
