...
// Targets: check_binary_operation in src/type_system/mod.rs:315
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // check_binary_operation(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // check_binary_operation(data, 0);        // Zero size
        // check_binary_operation(data, 1);        // Single byte
        // check_binary_operation(data, size);     // Full size
        // check_binary_operation(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for check_binary_operation
