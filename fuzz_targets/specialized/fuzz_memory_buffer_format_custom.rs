...
// Targets: format_custom in src/runtime/debug_output.rs:909
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // format_custom(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // format_custom(data, 0);        // Zero size
        // format_custom(data, 1);        // Single byte
        // format_custom(data, size);     // Full size
        // format_custom(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for format_custom
