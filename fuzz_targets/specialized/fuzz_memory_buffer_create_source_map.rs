...
// Targets: create_source_map in src/debug/enhanced_debug.rs:169
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // create_source_map(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // create_source_map(data, 0);        // Zero size
        // create_source_map(data, 1);        // Single byte
        // create_source_map(data, size);     // Full size
        // create_source_map(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for create_source_map
