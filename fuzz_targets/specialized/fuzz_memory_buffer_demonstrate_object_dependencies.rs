...
// Targets: demonstrate_object_dependencies in examples/adaptive_gc_demo.rs:137
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // demonstrate_object_dependencies(buffer, data, size);
...
void test_string_operations(const uint8_t *data, size_t size) {
    ...
    // demonstrate_object_dependencies(str);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // demonstrate_object_dependencies(data, 0);        // Zero size
        // demonstrate_object_dependencies(data, 1);        // Single byte
        // demonstrate_object_dependencies(data, size);     // Full size
        // demonstrate_object_dependencies(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for demonstrate_object_dependencies
