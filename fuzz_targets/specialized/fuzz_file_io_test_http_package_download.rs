...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    // Test test_http_package_download with temp file
    // Example: test_http_package_download(temp_path);
    // Example: test_http_package_download(temp_path, data, size);
...
void test_path_operations(const uint8_t *data, size_t size) {
    ...
    // test_http_package_download(path);
}
