...
// Targets: ping_worker in src/optimization/distributed/network_optimizer.rs:265
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // ping_worker(data, 0);        // Zero size
        // ping_worker(data, 1);        // Single byte
        // ping_worker(data, size);     // Full size
        // ping_worker(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for ping_worker
