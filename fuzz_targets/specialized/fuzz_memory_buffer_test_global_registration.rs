...
// Targets: test_global_registration in stdlib/gob_encode_vibes/test_gob_encode_vibes.csd:375
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // test_global_registration(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // test_global_registration(data, 0);        // Zero size
        // test_global_registration(data, 1);        // Single byte
        // test_global_registration(data, size);     // Full size
        // test_global_registration(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for test_global_registration
