...
// Targets: which in examples/simple_process_demo.rs:145
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // which(data, 0);        // Zero size
        // which(data, 1);        // Single byte
        // which(data, size);     // Full size
        // which(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for which
