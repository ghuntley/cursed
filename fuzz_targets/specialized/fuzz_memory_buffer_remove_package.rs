...
// Targets: remove_package in src/package_manager/cache.rs:153
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // remove_package(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // remove_package(data, 0);        // Zero size
        // remove_package(data, 1);        // Single byte
        // remove_package(data, size);     // Full size
        // remove_package(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for remove_package
