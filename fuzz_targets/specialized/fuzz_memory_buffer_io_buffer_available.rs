...
// Targets: io_buffer_available in src/execution/runtime_functions.rs:1417
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // io_buffer_available(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // io_buffer_available(data, 0);        // Zero size
        // io_buffer_available(data, 1);        // Single byte
        // io_buffer_available(data, size);     // Full size
        // io_buffer_available(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for io_buffer_available
