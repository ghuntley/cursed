...
// Targets: get_location in src/type_system/compilation_integration.rs:440
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // get_location(data, 0);        // Zero size
        // get_location(data, 1);        // Single byte
        // get_location(data, size);     // Full size
        // get_location(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for get_location
