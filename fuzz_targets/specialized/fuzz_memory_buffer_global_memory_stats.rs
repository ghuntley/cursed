...
// Targets: global_memory_stats in src/runtime/channels/memory.rs:523
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // global_memory_stats(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // global_memory_stats(data, 0);        // Zero size
        // global_memory_stats(data, 1);        // Single byte
        // global_memory_stats(data, size);     // Full size
        // global_memory_stats(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for global_memory_stats
