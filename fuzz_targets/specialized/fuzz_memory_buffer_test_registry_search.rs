...
// Targets: test_registry_search in src/package_manager/registry.rs:579
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // test_registry_search(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // test_registry_search(data, 0);        // Zero size
        // test_registry_search(data, 1);        // Single byte
        // test_registry_search(data, size);     // Full size
        // test_registry_search(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for test_registry_search
