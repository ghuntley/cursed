...
// Targets: compile_or_pattern in src/pattern_matching.rs:855
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // compile_or_pattern(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // compile_or_pattern(data, 0);        // Zero size
        // compile_or_pattern(data, 1);        // Single byte
        // compile_or_pattern(data, size);     // Full size
        // compile_or_pattern(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for compile_or_pattern
