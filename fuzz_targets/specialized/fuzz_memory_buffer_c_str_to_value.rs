...
// Targets: c_str_to_value in src/execution/cursed_bridge.rs:36
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // c_str_to_value(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // c_str_to_value(data, 0);        // Zero size
        // c_str_to_value(data, 1);        // Single byte
        // c_str_to_value(data, size);     // Full size
        // c_str_to_value(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for c_str_to_value
