...
// Targets: send_webhook_alert in src/bin/cursed_metrics.rs:562
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // send_webhook_alert(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // send_webhook_alert(data, 0);        // Zero size
        // send_webhook_alert(data, 1);        // Single byte
        // send_webhook_alert(data, size);     // Full size
        // send_webhook_alert(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for send_webhook_alert
