...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // record_line_hit(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // record_line_hit(data, 0);        // Zero size
        // record_line_hit(data, 1);        // Single byte
        // record_line_hit(data, size);     // Full size
        // record_line_hit(NULL, 0);        // NULL pointer
    }
}
