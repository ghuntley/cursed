...
// Targets: try_allocate_thread_local in src/memory/pool_optimization.rs:704
...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    // Test try_allocate_thread_local with temp file
    // Example: try_allocate_thread_local(temp_path);
    // Example: try_allocate_thread_local(temp_path, data, size);
...
void test_path_operations(const uint8_t *data, size_t size) {
    ...
    // try_allocate_thread_local(path);
}


// Additional test functions for try_allocate_thread_local
