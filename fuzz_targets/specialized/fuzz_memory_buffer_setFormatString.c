...
// Targets: setFormatString in src-zig/variadic_ffi_bridge.zig:226
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // setFormatString(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // setFormatString(data, 0);        // Zero size
        // setFormatString(data, 1);        // Single byte
        // setFormatString(data, size);     // Full size
        // setFormatString(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for setFormatString
