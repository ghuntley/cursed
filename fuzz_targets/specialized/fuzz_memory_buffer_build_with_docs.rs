...
// Targets: build_with_docs in src/documentation/build_integration.rs:615
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // build_with_docs(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // build_with_docs(data, 0);        // Zero size
        // build_with_docs(data, 1);        // Single byte
        // build_with_docs(data, size);     // Full size
        // build_with_docs(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for build_with_docs
