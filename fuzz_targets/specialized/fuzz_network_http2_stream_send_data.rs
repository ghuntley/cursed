...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    // Test packet parsing
    // Test http2_stream_send_data with network data
    // Example: http2_stream_send_data(data, size);
    // Example: http2_stream_send_data(sockfd, data, size);
...
void test_socket_operations(const uint8_t *data, size_t size) {
    ...
    // http2_stream_send_data(sockfd, data, size);
