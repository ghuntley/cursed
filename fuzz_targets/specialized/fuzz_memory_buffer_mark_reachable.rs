...
// Targets: mark_reachable in src/optimization/link_time_optimization.rs:458
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // mark_reachable(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // mark_reachable(data, 0);        // Zero size
        // mark_reachable(data, 1);        // Single byte
        // mark_reachable(data, size);     // Full size
        // mark_reachable(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for mark_reachable
