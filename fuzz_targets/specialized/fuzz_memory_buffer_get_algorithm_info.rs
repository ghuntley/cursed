...
// Targets: get_algorithm_info in src/stdlib/packages/crypto_signatures/mod.rs:261
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // get_algorithm_info(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // get_algorithm_info(data, 0);        // Zero size
        // get_algorithm_info(data, 1);        // Single byte
        // get_algorithm_info(data, size);     // Full size
        // get_algorithm_info(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for get_algorithm_info
