...
// Targets: get_worker in src/optimization/distributed/worker_node.rs:211
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // get_worker(data, 0);        // Zero size
        // get_worker(data, 1);        // Single byte
        // get_worker(data, size);     // Full size
        // get_worker(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for get_worker
