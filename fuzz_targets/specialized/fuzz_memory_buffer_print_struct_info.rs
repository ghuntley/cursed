...
// Targets: print_struct_info in stdlib/typez/mod.csd:581
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // print_struct_info(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // print_struct_info(data, 0);        // Zero size
        // print_struct_info(data, 1);        // Single byte
        // print_struct_info(data, size);     // Full size
        // print_struct_info(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for print_struct_info
