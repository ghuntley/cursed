...
// Targets: add_operator_spaces in src/formatter/simple.rs:157
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // add_operator_spaces(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // add_operator_spaces(data, 0);        // Zero size
        // add_operator_spaces(data, 1);        // Single byte
        // add_operator_spaces(data, size);     // Full size
        // add_operator_spaces(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for add_operator_spaces
