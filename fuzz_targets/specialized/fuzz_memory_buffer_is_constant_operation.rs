...
// Targets: is_constant_operation in src/optimization/simple_passes.rs:55
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // is_constant_operation(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // is_constant_operation(data, 0);        // Zero size
        // is_constant_operation(data, 1);        // Single byte
        // is_constant_operation(data, size);     // Full size
        // is_constant_operation(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for is_constant_operation
