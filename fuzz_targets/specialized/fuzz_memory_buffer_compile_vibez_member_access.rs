...
// Targets: compile_vibez_member_access in src/codegen/llvm/expression_compiler.rs:1095
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // compile_vibez_member_access(buffer, data, size);
...
void test_string_operations(const uint8_t *data, size_t size) {
    ...
    // compile_vibez_member_access(str);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // compile_vibez_member_access(data, 0);        // Zero size
        // compile_vibez_member_access(data, 1);        // Single byte
        // compile_vibez_member_access(data, size);     // Full size
        // compile_vibez_member_access(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for compile_vibez_member_access
