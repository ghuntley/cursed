...
// Targets: parse_error_code in src/error/cli.rs:215
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // parse_error_code(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // parse_error_code(data, 0);        // Zero size
        // parse_error_code(data, 1);        // Single byte
        // parse_error_code(data, size);     // Full size
        // parse_error_code(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for parse_error_code
