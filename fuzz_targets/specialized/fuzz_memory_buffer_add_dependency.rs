...
// Targets: add_dependency in src/tools/package_manager.rs:147
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // add_dependency(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // add_dependency(data, 0);        // Zero size
        // add_dependency(data, 1);        // Single byte
        // add_dependency(data, size);     // Full size
        // add_dependency(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for add_dependency
