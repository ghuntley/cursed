...
// Targets: compile_with_optimization in src/lib.rs:2040
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // compile_with_optimization(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // compile_with_optimization(data, 0);        // Zero size
        // compile_with_optimization(data, 1);        // Single byte
        // compile_with_optimization(data, size);     // Full size
        // compile_with_optimization(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for compile_with_optimization
