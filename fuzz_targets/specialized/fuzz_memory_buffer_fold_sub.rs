...
// Targets: fold_sub in src/codegen/llvm/passes/constant_propagation.rs:227
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // fold_sub(data, 0);        // Zero size
        // fold_sub(data, 1);        // Single byte
        // fold_sub(data, size);     // Full size
        // fold_sub(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for fold_sub
