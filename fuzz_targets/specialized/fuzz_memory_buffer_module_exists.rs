...
// Targets: module_exists in src/imports/resolver.rs:1037
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // module_exists(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // module_exists(data, 0);        // Zero size
        // module_exists(data, 1);        // Single byte
        // module_exists(data, size);     // Full size
        // module_exists(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for module_exists
