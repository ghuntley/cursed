...
// Targets: with_limits in src/optimization/parallel.rs:85
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // with_limits(data, 0);        // Zero size
        // with_limits(data, 1);        // Single byte
        // with_limits(data, size);     // Full size
        // with_limits(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for with_limits
