...
// Targets: test_string_comparison in tests/testz/test_advanced_features.csd:199
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // test_string_comparison(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // test_string_comparison(data, 0);        // Zero size
        // test_string_comparison(data, 1);        // Single byte
        // test_string_comparison(data, size);     // Full size
        // test_string_comparison(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for test_string_comparison
