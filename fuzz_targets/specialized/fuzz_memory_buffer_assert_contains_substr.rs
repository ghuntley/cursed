...
// Targets: assert_contains_substr in src/stdlib/test_vibes/assertions.rs:94
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // assert_contains_substr(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // assert_contains_substr(data, 0);        // Zero size
        // assert_contains_substr(data, 1);        // Single byte
        // assert_contains_substr(data, size);     // Full size
        // assert_contains_substr(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for assert_contains_substr
