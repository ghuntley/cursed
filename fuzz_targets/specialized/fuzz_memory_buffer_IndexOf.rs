...
// Targets: IndexOf in stdlib/stringz/mod_original.csd:246
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // IndexOf(data, 0);        // Zero size
        // IndexOf(data, 1);        // Single byte
        // IndexOf(data, size);     // Full size
        // IndexOf(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for IndexOf
