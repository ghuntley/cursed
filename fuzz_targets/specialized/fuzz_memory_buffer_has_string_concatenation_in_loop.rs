...
// Targets: has_string_concatenation_in_loop in src/linter/rules.rs:252
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // has_string_concatenation_in_loop(buffer, data, size);
...
void test_string_operations(const uint8_t *data, size_t size) {
    ...
    // has_string_concatenation_in_loop(str);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // has_string_concatenation_in_loop(data, 0);        // Zero size
        // has_string_concatenation_in_loop(data, 1);        // Single byte
        // has_string_concatenation_in_loop(data, size);     // Full size
        // has_string_concatenation_in_loop(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for has_string_concatenation_in_loop
