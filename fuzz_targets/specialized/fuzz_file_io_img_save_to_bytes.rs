...
// Targets: img_save_to_bytes in stdlib/image_processing/mod.csd:145
...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    // Test img_save_to_bytes with temp file
    // Example: img_save_to_bytes(temp_path);
    // Example: img_save_to_bytes(temp_path, data, size);
...
void test_path_operations(const uint8_t *data, size_t size) {
    ...
    // img_save_to_bytes(path);
}


// Additional test functions for img_save_to_bytes
