...
// Targets: cursed_memory_profiler_record_allocation in src-zig/memory_profiler_aggregation.zig:691
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // cursed_memory_profiler_record_allocation(buffer, data, size);
...
void test_string_operations(const uint8_t *data, size_t size) {
    ...
    // cursed_memory_profiler_record_allocation(str);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // cursed_memory_profiler_record_allocation(data, 0);        // Zero size
        // cursed_memory_profiler_record_allocation(data, 1);        // Single byte
        // cursed_memory_profiler_record_allocation(data, size);     // Full size
        // cursed_memory_profiler_record_allocation(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for cursed_memory_profiler_record_allocation
