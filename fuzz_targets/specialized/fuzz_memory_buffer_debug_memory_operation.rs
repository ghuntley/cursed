...
// Targets: debug_memory_operation in src/ffi/debug_tools.rs:425
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // debug_memory_operation(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // debug_memory_operation(data, 0);        // Zero size
        // debug_memory_operation(data, 1);        // Single byte
        // debug_memory_operation(data, size);     // Full size
        // debug_memory_operation(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for debug_memory_operation
