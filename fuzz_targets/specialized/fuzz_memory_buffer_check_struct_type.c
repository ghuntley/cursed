...
// Targets: check_struct_type in runtime/pattern_matching_runtime.c:124
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // check_struct_type(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // check_struct_type(data, 0);        // Zero size
        // check_struct_type(data, 1);        // Single byte
        // check_struct_type(data, size);     // Full size
        // check_struct_type(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for check_struct_type
