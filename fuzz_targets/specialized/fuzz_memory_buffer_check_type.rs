...
// Targets: check_type in src/core/type_checker.rs:37
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // check_type(data, 0);        // Zero size
        // check_type(data, 1);        // Single byte
        // check_type(data, size);     // Full size
        // check_type(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for check_type
