...
// Targets: bool_to_string_pure in stdlib/self_hosted_builtins/mod.csd:202
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // bool_to_string_pure(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // bool_to_string_pure(data, 0);        // Zero size
        // bool_to_string_pure(data, 1);        // Single byte
        // bool_to_string_pure(data, size);     // Full size
        // bool_to_string_pure(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for bool_to_string_pure
