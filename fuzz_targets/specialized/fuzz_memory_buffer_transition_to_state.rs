...
// Targets: transition_to_state in src/runtime/recovery.rs:261
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // transition_to_state(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // transition_to_state(data, 0);        // Zero size
        // transition_to_state(data, 1);        // Single byte
        // transition_to_state(data, size);     // Full size
        // transition_to_state(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for transition_to_state
