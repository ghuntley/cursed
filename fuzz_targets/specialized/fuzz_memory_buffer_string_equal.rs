...
// Targets: string_equal in stdlib/pure_cursed_runtime/mod.csd:272
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // string_equal(data, 0);        // Zero size
        // string_equal(data, 1);        // Single byte
        // string_equal(data, size);     // Full size
        // string_equal(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for string_equal
