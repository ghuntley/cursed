...
// Targets: new_slay_command in src/stdlib/exec_slay/mod.rs:116
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // new_slay_command(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // new_slay_command(data, 0);        // Zero size
        // new_slay_command(data, 1);        // Single byte
        // new_slay_command(data, size);     // Full size
        // new_slay_command(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for new_slay_command
