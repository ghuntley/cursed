...
// Targets: signature_string in src/runtime/stack_trace.rs:124
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // signature_string(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // signature_string(data, 0);        // Zero size
        // signature_string(data, 1);        // Single byte
        // signature_string(data, size);     // Full size
        // signature_string(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for signature_string
