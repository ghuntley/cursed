...
// Targets: check_breakpoint in src/ffi/debug_tools.rs:614
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // check_breakpoint(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // check_breakpoint(data, 0);        // Zero size
        // check_breakpoint(data, 1);        // Single byte
        // check_breakpoint(data, size);     // Full size
        // check_breakpoint(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for check_breakpoint
