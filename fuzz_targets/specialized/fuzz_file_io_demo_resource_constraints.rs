...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    // Test demo_resource_constraints with temp file
    // Example: demo_resource_constraints(temp_path);
    // Example: demo_resource_constraints(temp_path, data, size);
...
void test_path_operations(const uint8_t *data, size_t size) {
    ...
    // demo_resource_constraints(path);
}
