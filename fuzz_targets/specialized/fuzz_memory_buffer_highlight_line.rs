...
// Targets: highlight_line in src/repl/advanced_syntax_highlighter.rs:144
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // highlight_line(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // highlight_line(data, 0);        // Zero size
        // highlight_line(data, 1);        // Single byte
        // highlight_line(data, size);     // Full size
        // highlight_line(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for highlight_line
