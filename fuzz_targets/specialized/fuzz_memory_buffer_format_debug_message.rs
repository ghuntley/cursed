...
// Targets: format_debug_message in src/stdlib/vibez/debug.rs:271
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // format_debug_message(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // format_debug_message(data, 0);        // Zero size
        // format_debug_message(data, 1);        // Single byte
        // format_debug_message(data, size);     // Full size
        // format_debug_message(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for format_debug_message
