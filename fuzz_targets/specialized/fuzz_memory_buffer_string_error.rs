...
// Targets: string_error in src/error/mod.rs:234
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // string_error(data, 0);        // Zero size
        // string_error(data, 1);        // Single byte
        // string_error(data, size);     // Full size
        // string_error(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for string_error
