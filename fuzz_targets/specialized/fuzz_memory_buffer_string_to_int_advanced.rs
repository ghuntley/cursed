...
// Targets: string_to_int_advanced in stdlib/stringz/mod.csd:842
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // string_to_int_advanced(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // string_to_int_advanced(data, 0);        // Zero size
        // string_to_int_advanced(data, 1);        // Single byte
        // string_to_int_advanced(data, size);     // Full size
        // string_to_int_advanced(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for string_to_int_advanced
