...
// Targets: test_string_lengths in tests/quick_test_test.csd:99
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // test_string_lengths(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // test_string_lengths(data, 0);        // Zero size
        // test_string_lengths(data, 1);        // Single byte
        // test_string_lengths(data, size);     // Full size
        // test_string_lengths(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for test_string_lengths
