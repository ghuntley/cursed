...
// Targets: fold_and in src/codegen/llvm/passes/constant_propagation.rs:417
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // fold_and(data, 0);        // Zero size
        // fold_and(data, 1);        // Single byte
        // fold_and(data, size);     // Full size
        // fold_and(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for fold_and
