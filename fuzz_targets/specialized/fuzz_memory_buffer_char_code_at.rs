...
// Targets: char_code_at in stdlib/runtime_core/mod.csd:341
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // char_code_at(data, 0);        // Zero size
        // char_code_at(data, 1);        // Single byte
        // char_code_at(data, size);     // Full size
        // char_code_at(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for char_code_at
