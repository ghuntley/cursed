...
// Targets: from_cli_level in src/optimization/config.rs:464
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // from_cli_level(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // from_cli_level(data, 0);        // Zero size
        // from_cli_level(data, 1);        // Single byte
        // from_cli_level(data, size);     // Full size
        // from_cli_level(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for from_cli_level
