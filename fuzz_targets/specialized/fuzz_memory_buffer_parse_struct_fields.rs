...
// Targets: parse_struct_fields in src/parser/generic_parser.rs:348
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // parse_struct_fields(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // parse_struct_fields(data, 0);        // Zero size
        // parse_struct_fields(data, 1);        // Single byte
        // parse_struct_fields(data, size);     // Full size
        // parse_struct_fields(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for parse_struct_fields
