...
// Targets: test_grouped_imports in tests/grouped_imports_test.csd:6
...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    // Test test_grouped_imports with temp file
    // Example: test_grouped_imports(temp_path);
    // Example: test_grouped_imports(temp_path, data, size);
...
void test_path_operations(const uint8_t *data, size_t size) {
    ...
    // test_grouped_imports(path);
}


// Additional test functions for test_grouped_imports
