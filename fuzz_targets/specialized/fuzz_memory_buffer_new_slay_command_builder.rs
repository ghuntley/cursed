...
// Targets: new_slay_command_builder in src/stdlib/exec_slay/mod.rs:119
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // new_slay_command_builder(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // new_slay_command_builder(data, 0);        // Zero size
        // new_slay_command_builder(data, 1);        // Single byte
        // new_slay_command_builder(data, size);     // Full size
        // new_slay_command_builder(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for new_slay_command_builder
