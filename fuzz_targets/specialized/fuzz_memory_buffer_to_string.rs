...
// Targets: to_string in benchmarks/cursed/compiler/memory_usage_bench.csd:501
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // to_string(data, 0);        // Zero size
        // to_string(data, 1);        // Single byte
        // to_string(data, size);     // Full size
        // to_string(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for to_string
