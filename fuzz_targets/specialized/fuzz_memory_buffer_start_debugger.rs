...
// Targets: start_debugger in src/bin/cursed_debug.rs:166
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // start_debugger(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // start_debugger(data, 0);        // Zero size
        // start_debugger(data, 1);        // Single byte
        // start_debugger(data, size);     // Full size
        // start_debugger(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for start_debugger
