...
// Targets: demo_string_test in tests/testz/test_runner_demo.csd:36
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // demo_string_test(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // demo_string_test(data, 0);        // Zero size
        // demo_string_test(data, 1);        // Single byte
        // demo_string_test(data, size);     // Full size
        // demo_string_test(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for demo_string_test
