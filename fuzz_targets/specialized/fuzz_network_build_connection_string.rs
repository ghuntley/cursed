...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    // Test packet parsing
    // Test build_connection_string with network data
    // Example: build_connection_string(data, size);
    // Example: build_connection_string(sockfd, data, size);
...
void test_socket_operations(const uint8_t *data, size_t size) {
    ...
    // build_connection_string(sockfd, data, size);
