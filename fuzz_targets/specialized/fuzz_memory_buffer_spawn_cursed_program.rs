...
// Targets: spawn_cursed_program in src/runtime/process.rs:401
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // spawn_cursed_program(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // spawn_cursed_program(data, 0);        // Zero size
        // spawn_cursed_program(data, 1);        // Single byte
        // spawn_cursed_program(data, size);     // Full size
        // spawn_cursed_program(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for spawn_cursed_program
