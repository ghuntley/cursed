...
// Targets: compile_ir_to_object in src/build_system/build_pipeline.rs:563
...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    // Test compile_ir_to_object with temp file
    // Example: compile_ir_to_object(temp_path);
    // Example: compile_ir_to_object(temp_path, data, size);
...
void test_path_operations(const uint8_t *data, size_t size) {
    ...
    // compile_ir_to_object(path);
}


// Additional test functions for compile_ir_to_object
