...
// Targets: remove_global in src/runtime/runtime.rs:727
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // remove_global(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // remove_global(data, 0);        // Zero size
        // remove_global(data, 1);        // Single byte
        // remove_global(data, size);     // Full size
        // remove_global(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for remove_global
