...
// Targets: to_vibe_string in src/runtime/unicode_char.rs:543
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // to_vibe_string(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // to_vibe_string(data, 0);        // Zero size
        // to_vibe_string(data, 1);        // Single byte
        // to_vibe_string(data, size);     // Full size
        // to_vibe_string(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for to_vibe_string
