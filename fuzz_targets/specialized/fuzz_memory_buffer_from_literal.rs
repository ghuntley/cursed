...
// Targets: from_literal in src/runtime/runtime_value.rs:484
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // from_literal(data, 0);        // Zero size
        // from_literal(data, 1);        // Single byte
        // from_literal(data, size);     // Full size
        // from_literal(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for from_literal
