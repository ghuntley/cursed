...
// Targets: capture_groups in src/stdlib/string/regex.rs:31
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // capture_groups(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // capture_groups(data, 0);        // Zero size
        // capture_groups(data, 1);        // Single byte
        // capture_groups(data, size);     // Full size
        // capture_groups(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for capture_groups
