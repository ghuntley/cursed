...
// Targets: test_string_searching in tests/string_helpers_test.csd:24
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // test_string_searching(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // test_string_searching(data, 0);        // Zero size
        // test_string_searching(data, 1);        // Single byte
        // test_string_searching(data, size);     // Full size
        // test_string_searching(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for test_string_searching
