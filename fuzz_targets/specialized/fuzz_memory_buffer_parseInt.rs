...
// Targets: parseInt in src-zig/parser.csd:660
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // parseInt(data, 0);        // Zero size
        // parseInt(data, 1);        // Single byte
        // parseInt(data, size);     // Full size
        // parseInt(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for parseInt
