...
// Targets: full in src/stdlib/database/orm/query_builder.rs:483
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // full(data, 0);        // Zero size
        // full(data, 1);        // Single byte
        // full(data, size);     // Full size
        // full(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for full
