...
// Targets: get_cached_metadata in src/package_manager/mutable_state_old.rs:129
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // get_cached_metadata(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // get_cached_metadata(data, 0);        // Zero size
        // get_cached_metadata(data, 1);        // Single byte
        // get_cached_metadata(data, size);     // Full size
        // get_cached_metadata(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for get_cached_metadata
