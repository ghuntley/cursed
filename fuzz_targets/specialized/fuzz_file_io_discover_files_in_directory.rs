...
// Targets: discover_files_in_directory in src/coverage/simple_collector.rs:72
...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    // Test discover_files_in_directory with temp file
    // Example: discover_files_in_directory(temp_path);
    // Example: discover_files_in_directory(temp_path, data, size);
...
void test_path_operations(const uint8_t *data, size_t size) {
    ...
    // discover_files_in_directory(path);
}


// Additional test functions for discover_files_in_directory
