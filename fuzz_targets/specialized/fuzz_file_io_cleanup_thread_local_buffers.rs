...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    // Test cleanup_thread_local_buffers with temp file
    // Example: cleanup_thread_local_buffers(temp_path);
    // Example: cleanup_thread_local_buffers(temp_path, data, size);
...
void test_path_operations(const uint8_t *data, size_t size) {
    ...
    // cleanup_thread_local_buffers(path);
}
