...
// Targets: columns in src/stdlib/database/query.rs:304
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // columns(data, 0);        // Zero size
        // columns(data, 1);        // Single byte
        // columns(data, size);     // Full size
        // columns(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for columns
