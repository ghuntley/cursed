...
// Targets: create_struct_value in runtime/pattern_matching_runtime.c:223
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // create_struct_value(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // create_struct_value(data, 0);        // Zero size
        // create_struct_value(data, 1);        // Single byte
        // create_struct_value(data, size);     // Full size
        // create_struct_value(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for create_struct_value
