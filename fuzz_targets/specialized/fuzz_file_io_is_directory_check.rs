...
// Targets: is_directory_check in stdlib/ioz_enhanced/mod.csd:145
...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    // Test is_directory_check with temp file
    // Example: is_directory_check(temp_path);
    // Example: is_directory_check(temp_path, data, size);
...
void test_path_operations(const uint8_t *data, size_t size) {
    ...
    // is_directory_check(path);
}


// Additional test functions for is_directory_check
