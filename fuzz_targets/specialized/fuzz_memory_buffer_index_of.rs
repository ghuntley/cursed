...
// Targets: index_of in stdlib/io/mod.csd:303
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // index_of(data, 0);        // Zero size
        // index_of(data, 1);        // Single byte
        // index_of(data, size);     // Full size
        // index_of(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for index_of
