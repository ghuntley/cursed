...
// Targets: demonstrate_rsa_crypto in examples/crypto_asymmetric_demo.csd:44
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // demonstrate_rsa_crypto(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // demonstrate_rsa_crypto(data, 0);        // Zero size
        // demonstrate_rsa_crypto(data, 1);        // Single byte
        // demonstrate_rsa_crypto(data, size);     // Full size
        // demonstrate_rsa_crypto(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for demonstrate_rsa_crypto
