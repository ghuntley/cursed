...
// Targets: get_variable_memory_address in src/runtime/debug_manager.rs:595
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // get_variable_memory_address(buffer, data, size);
...
void test_string_operations(const uint8_t *data, size_t size) {
    ...
    // get_variable_memory_address(str);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // get_variable_memory_address(data, 0);        // Zero size
        // get_variable_memory_address(data, 1);        // Single byte
        // get_variable_memory_address(data, size);     // Full size
        // get_variable_memory_address(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for get_variable_memory_address
