...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    // Test compile_to_wasm_with_optimizations with temp file
    // Example: compile_to_wasm_with_optimizations(temp_path);
    // Example: compile_to_wasm_with_optimizations(temp_path, data, size);
...
void test_path_operations(const uint8_t *data, size_t size) {
    ...
    // compile_to_wasm_with_optimizations(path);
}
