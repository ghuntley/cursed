...
// Targets: compile_string_concatenation in src/codegen/llvm/expression_compiler.rs:535
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // compile_string_concatenation(buffer, data, size);
...
void test_string_operations(const uint8_t *data, size_t size) {
    ...
    // compile_string_concatenation(str);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // compile_string_concatenation(data, 0);        // Zero size
        // compile_string_concatenation(data, 1);        // Single byte
        // compile_string_concatenation(data, size);     // Full size
        // compile_string_concatenation(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for compile_string_concatenation
