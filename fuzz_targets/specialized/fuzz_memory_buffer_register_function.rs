...
// Targets: register_function in src/ffi/wasm_bridge.rs:211
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // register_function(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // register_function(data, 0);        // Zero size
        // register_function(data, 1);        // Single byte
        // register_function(data, size);     // Full size
        // register_function(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for register_function
