...
// Targets: foldFloatCastOperation in src-zig/constant_folder.zig:304
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // foldFloatCastOperation(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // foldFloatCastOperation(data, 0);        // Zero size
        // foldFloatCastOperation(data, 1);        // Single byte
        // foldFloatCastOperation(data, size);     // Full size
        // foldFloatCastOperation(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for foldFloatCastOperation
