...
// Targets: by_name in src/optimization/config.rs:858
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // by_name(data, 0);        // Zero size
        // by_name(data, 1);        // Single byte
        // by_name(data, size);     // Full size
        // by_name(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for by_name
