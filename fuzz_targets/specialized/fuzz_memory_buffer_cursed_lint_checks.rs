...
// Targets: cursed_lint_checks in src/lsp/diagnostics.rs:138
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // cursed_lint_checks(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // cursed_lint_checks(data, 0);        // Zero size
        // cursed_lint_checks(data, 1);        // Single byte
        // cursed_lint_checks(data, size);     // Full size
        // cursed_lint_checks(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for cursed_lint_checks
