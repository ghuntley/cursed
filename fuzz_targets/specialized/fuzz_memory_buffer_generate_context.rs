...
// Targets: generate_context in src/error_recovery.rs:363
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // generate_context(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // generate_context(data, 0);        // Zero size
        // generate_context(data, 1);        // Single byte
        // generate_context(data, size);     // Full size
        // generate_context(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for generate_context
