...
// Targets: find in stdlib/stringz/mod_enhanced.csd:116
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // find(data, 0);        // Zero size
        // find(data, 1);        // Single byte
        // find(data, size);     // Full size
        // find(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for find
