...
// Targets: print_to_string in src/codegen/llvm/improved_codegen.rs:534
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // print_to_string(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // print_to_string(data, 0);        // Zero size
        // print_to_string(data, 1);        // Single byte
        // print_to_string(data, size);     // Full size
        // print_to_string(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for print_to_string
