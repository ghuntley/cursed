...
// Targets: copy_with_metadata in stdlib/filesystem_complete/mod.csd:1025
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // copy_with_metadata(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // copy_with_metadata(data, 0);        // Zero size
        // copy_with_metadata(data, 1);        // Single byte
        // copy_with_metadata(data, size);     // Full size
        // copy_with_metadata(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for copy_with_metadata
