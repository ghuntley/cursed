...
// Targets: test_llvm_codegen_export_available in tests/test_lib_exports.rs:57
...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    // Test test_llvm_codegen_export_available with temp file
    // Example: test_llvm_codegen_export_available(temp_path);
    // Example: test_llvm_codegen_export_available(temp_path, data, size);
...
void test_path_operations(const uint8_t *data, size_t size) {
    ...
    // test_llvm_codegen_export_available(path);
}


// Additional test functions for test_llvm_codegen_export_available
