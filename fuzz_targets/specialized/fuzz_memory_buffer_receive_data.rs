...
// Targets: receive_data in src/optimization/distributed/network_optimizer.rs:235
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // receive_data(data, 0);        // Zero size
        // receive_data(data, 1);        // Single byte
        // receive_data(data, size);     // Full size
        // receive_data(NULL, 0);        // NULL pointer
    }
}
