...
// Targets: parse_single_constraint in src/parser/generic_parser.rs:253
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // parse_single_constraint(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // parse_single_constraint(data, 0);        // Zero size
        // parse_single_constraint(data, 1);        // Single byte
        // parse_single_constraint(data, size);     // Full size
        // parse_single_constraint(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for parse_single_constraint
