...
// Targets: type_name in src/runtime/value/mod.rs:118
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // type_name(data, 0);        // Zero size
        // type_name(data, 1);        // Single byte
        // type_name(data, size);     // Full size
        // type_name(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for type_name
