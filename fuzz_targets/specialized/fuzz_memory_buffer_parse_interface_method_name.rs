...
// Targets: parse_interface_method_name in src/codegen/llvm/interface_optimization.rs:301
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // parse_interface_method_name(buffer, data, size);
...
void test_string_operations(const uint8_t *data, size_t size) {
    ...
    // parse_interface_method_name(str);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // parse_interface_method_name(data, 0);        // Zero size
        // parse_interface_method_name(data, 1);        // Single byte
        // parse_interface_method_name(data, size);     // Full size
        // parse_interface_method_name(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for parse_interface_method_name
