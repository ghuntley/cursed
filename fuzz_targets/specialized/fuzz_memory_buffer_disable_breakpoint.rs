...
// Targets: disable_breakpoint in src/bin/cursed_debug.rs:649
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // disable_breakpoint(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // disable_breakpoint(data, 0);        // Zero size
        // disable_breakpoint(data, 1);        // Single byte
        // disable_breakpoint(data, size);     // Full size
        // disable_breakpoint(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for disable_breakpoint
