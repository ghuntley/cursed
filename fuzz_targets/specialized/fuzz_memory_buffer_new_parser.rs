...
// Targets: new_parser in src/parser_main.rs:4403
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // new_parser(data, 0);        // Zero size
        // new_parser(data, 1);        // Single byte
        // new_parser(data, size);     // Full size
        // new_parser(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for new_parser
