...
// Targets: call_function in src/ffi/wasm_bridge.rs:222
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // call_function(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // call_function(data, 0);        // Zero size
        // call_function(data, 1);        // Single byte
        // call_function(data, size);     // Full size
        // call_function(NULL, 0);        // NULL pointer
    }
}
