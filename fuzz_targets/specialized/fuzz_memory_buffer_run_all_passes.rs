...
// Targets: run_all_passes in src/codegen/llvm/optimization_passes.rs:120
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // run_all_passes(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // run_all_passes(data, 0);        // Zero size
        // run_all_passes(data, 1);        // Single byte
        // run_all_passes(data, size);     // Full size
        // run_all_passes(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for run_all_passes
