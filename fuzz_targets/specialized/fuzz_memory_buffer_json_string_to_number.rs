...
// Targets: json_string_to_number in stdlib/pure_json/mod.csd:510
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // json_string_to_number(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // json_string_to_number(data, 0);        // Zero size
        // json_string_to_number(data, 1);        // Single byte
        // json_string_to_number(data, size);     // Full size
        // json_string_to_number(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for json_string_to_number
