...
// Targets: was_called in src/stdlib/packages/test_vibes/mocking.rs:47
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // was_called(data, 0);        // Zero size
        // was_called(data, 1);        // Single byte
        // was_called(data, size);     // Full size
        // was_called(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for was_called
