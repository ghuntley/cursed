...
// Targets: classifyStructReturn in src-zig/robust_llvm_backend.zig:70
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // classifyStructReturn(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // classifyStructReturn(data, 0);        // Zero size
        // classifyStructReturn(data, 1);        // Single byte
        // classifyStructReturn(data, size);     // Full size
        // classifyStructReturn(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for classifyStructReturn
