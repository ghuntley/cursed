...
// Targets: binary_op in src/runtime/runtime_value.rs:507
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // binary_op(data, 0);        // Zero size
        // binary_op(data, 1);        // Single byte
        // binary_op(data, size);     // Full size
        // binary_op(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for binary_op
