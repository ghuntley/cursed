...
// Targets: cursed_channel_create in src/runtime/channels/select_runtime.rs:248
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // cursed_channel_create(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // cursed_channel_create(data, 0);        // Zero size
        // cursed_channel_create(data, 1);        // Single byte
        // cursed_channel_create(data, size);     // Full size
        // cursed_channel_create(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for cursed_channel_create
