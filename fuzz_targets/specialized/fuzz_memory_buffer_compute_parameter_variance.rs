...
// Targets: compute_parameter_variance in src/type_system/variance.rs:71
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // compute_parameter_variance(buffer, data, size);
...
void test_string_operations(const uint8_t *data, size_t size) {
    ...
    // compute_parameter_variance(str);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // compute_parameter_variance(data, 0);        // Zero size
        // compute_parameter_variance(data, 1);        // Single byte
        // compute_parameter_variance(data, size);     // Full size
        // compute_parameter_variance(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for compute_parameter_variance
