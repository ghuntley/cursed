...
// Targets: str_index_of_from in stdlib/networkz/mod.csd:425
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // str_index_of_from(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // str_index_of_from(data, 0);        // Zero size
        // str_index_of_from(data, 1);        // Single byte
        // str_index_of_from(data, size);     // Full size
        // str_index_of_from(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for str_index_of_from
