...
// Targets: get_function_profile in src/ffi/debug_tools.rs:971
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // get_function_profile(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // get_function_profile(data, 0);        // Zero size
        // get_function_profile(data, 1);        // Single byte
        // get_function_profile(data, size);     // Full size
        // get_function_profile(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for get_function_profile
