...
// Targets: unescape_string in stdlib/fmt/mod.csd:317
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // unescape_string(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // unescape_string(data, 0);        // Zero size
        // unescape_string(data, 1);        // Single byte
        // unescape_string(data, size);     // Full size
        // unescape_string(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for unescape_string
