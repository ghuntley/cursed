...
// Targets: call_string in src/ffi/c_bridge.rs:144
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // call_string(data, 0);        // Zero size
        // call_string(data, 1);        // Single byte
        // call_string(data, size);     // Full size
        // call_string(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for call_string
