...
// Targets: tcp_receive in stdlib/networkz/mod.csd:225
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // tcp_receive(data, 0);        // Zero size
        // tcp_receive(data, 1);        // Single byte
        // tcp_receive(data, size);     // Full size
        // tcp_receive(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for tcp_receive
