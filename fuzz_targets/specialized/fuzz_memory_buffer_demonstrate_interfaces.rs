...
// Targets: demonstrate_interfaces in examples/docs_demo.csd:186
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // demonstrate_interfaces(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // demonstrate_interfaces(data, 0);        // Zero size
        // demonstrate_interfaces(data, 1);        // Single byte
        // demonstrate_interfaces(data, size);     // Full size
        // demonstrate_interfaces(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for demonstrate_interfaces
