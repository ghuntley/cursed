...
// Targets: dispatch_global_method in src/runtime/interface_dispatch.rs:620
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // dispatch_global_method(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // dispatch_global_method(data, 0);        // Zero size
        // dispatch_global_method(data, 1);        // Single byte
        // dispatch_global_method(data, size);     // Full size
        // dispatch_global_method(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for dispatch_global_method
