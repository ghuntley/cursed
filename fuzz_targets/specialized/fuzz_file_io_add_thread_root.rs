...
// Targets: add_thread_root in src/memory/root_set.rs:202
...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    // Test add_thread_root with temp file
    // Example: add_thread_root(temp_path);
    // Example: add_thread_root(temp_path, data, size);
...
void test_path_operations(const uint8_t *data, size_t size) {
    ...
    // add_thread_root(path);
}


// Additional test functions for add_thread_root
