...
// Targets: parse_async_record in src/debug/gdb_integration.rs:816
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // parse_async_record(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // parse_async_record(data, 0);        // Zero size
        // parse_async_record(data, 1);        // Single byte
        // parse_async_record(data, size);     // Full size
        // parse_async_record(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for parse_async_record
