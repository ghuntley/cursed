...
// Targets: core_string_from_int in stdlib/core/test_core_working.csd:87
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // core_string_from_int(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // core_string_from_int(data, 0);        // Zero size
        // core_string_from_int(data, 1);        // Single byte
        // core_string_from_int(data, size);     // Full size
        // core_string_from_int(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for core_string_from_int
