...
// Targets: list_directory in src/stdlib/net/protocols/ftp.rs:35
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // list_directory(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // list_directory(data, 0);        // Zero size
        // list_directory(data, 1);        // Single byte
        // list_directory(data, size);     // Full size
        // list_directory(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for list_directory
