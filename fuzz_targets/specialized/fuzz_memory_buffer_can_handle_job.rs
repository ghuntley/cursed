...
// Targets: can_handle_job in src/optimization/distributed/worker_node.rs:108
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // can_handle_job(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // can_handle_job(data, 0);        // Zero size
        // can_handle_job(data, 1);        // Single byte
        // can_handle_job(data, size);     // Full size
        // can_handle_job(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for can_handle_job
