...
// Targets: compile_job in src/optimization/parallel.rs:308
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // compile_job(data, 0);        // Zero size
        // compile_job(data, 1);        // Single byte
        // compile_job(data, size);     // Full size
        // compile_job(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for compile_job
