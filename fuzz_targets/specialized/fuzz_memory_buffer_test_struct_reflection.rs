...
// Targets: test_struct_reflection in tests/reflectz_test.csd:117
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // test_struct_reflection(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // test_struct_reflection(data, 0);        // Zero size
        // test_struct_reflection(data, 1);        // Single byte
        // test_struct_reflection(data, size);     // Full size
        // test_struct_reflection(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for test_struct_reflection
