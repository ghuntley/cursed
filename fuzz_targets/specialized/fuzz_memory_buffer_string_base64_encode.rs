...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // string_base64_encode(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // string_base64_encode(data, 0);        // Zero size
        // string_base64_encode(data, 1);        // Single byte
        // string_base64_encode(data, size);     // Full size
        // string_base64_encode(NULL, 0);        // NULL pointer
    }
}
