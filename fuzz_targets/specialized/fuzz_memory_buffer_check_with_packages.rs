...
// Targets: check_with_packages in src/lib.rs:888
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // check_with_packages(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // check_with_packages(data, 0);        // Zero size
        // check_with_packages(data, 1);        // Single byte
        // check_with_packages(data, size);     // Full size
        // check_with_packages(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for check_with_packages
