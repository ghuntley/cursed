...
// Targets: test_string_properties in stdlib/testz/comprehensive_test_runner.csd:160
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // test_string_properties(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // test_string_properties(data, 0);        // Zero size
        // test_string_properties(data, 1);        // Single byte
        // test_string_properties(data, size);     // Full size
        // test_string_properties(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for test_string_properties
