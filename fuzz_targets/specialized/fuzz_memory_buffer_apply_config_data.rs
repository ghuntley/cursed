...
// Targets: apply_config_data in examples/type_switch_practical_use_cases.csd:106
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // apply_config_data(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // apply_config_data(data, 0);        // Zero size
        // apply_config_data(data, 1);        // Single byte
        // apply_config_data(data, size);     // Full size
        // apply_config_data(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for apply_config_data
