...
// Targets: print_variable in src/bin/cursed_debug.rs:506
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // print_variable(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // print_variable(data, 0);        // Zero size
        // print_variable(data, 1);        // Single byte
        // print_variable(data, size);     // Full size
        // print_variable(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for print_variable
