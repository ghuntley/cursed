...
// Targets: string_trim_right in stdlib/encode_mood/mod.csd:433
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // string_trim_right(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // string_trim_right(data, 0);        // Zero size
        // string_trim_right(data, 1);        // Single byte
        // string_trim_right(data, size);     // Full size
        // string_trim_right(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for string_trim_right
