...
// Targets: demonstrate_key_exchange in examples/crypto_asymmetric_demo.csd:177
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // demonstrate_key_exchange(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // demonstrate_key_exchange(data, 0);        // Zero size
        // demonstrate_key_exchange(data, 1);        // Single byte
        // demonstrate_key_exchange(data, size);     // Full size
        // demonstrate_key_exchange(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for demonstrate_key_exchange
