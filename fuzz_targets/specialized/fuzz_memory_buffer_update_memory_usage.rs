...
// Targets: update_memory_usage in src/runtime/debug_runtime.rs:330
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // update_memory_usage(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // update_memory_usage(data, 0);        // Zero size
        // update_memory_usage(data, 1);        // Single byte
        // update_memory_usage(data, size);     // Full size
        // update_memory_usage(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for update_memory_usage
