...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    // Test cursed_memory_profiler_destroy with temp file
    // Example: cursed_memory_profiler_destroy(temp_path);
    // Example: cursed_memory_profiler_destroy(temp_path, data, size);
...
void test_path_operations(const uint8_t *data, size_t size) {
    ...
    // cursed_memory_profiler_destroy(path);
}
