...
// Targets: validation_error in src/error/mod.rs:226
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // validation_error(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // validation_error(data, 0);        // Zero size
        // validation_error(data, 1);        // Single byte
        // validation_error(data, size);     // Full size
        // validation_error(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for validation_error
