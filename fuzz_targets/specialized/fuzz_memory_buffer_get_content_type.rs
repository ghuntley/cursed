...
// Targets: get_content_type in src/bin/cursed_doc.rs:888
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // get_content_type(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // get_content_type(data, 0);        // Zero size
        // get_content_type(data, 1);        // Single byte
        // get_content_type(data, size);     // Full size
        // get_content_type(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for get_content_type
