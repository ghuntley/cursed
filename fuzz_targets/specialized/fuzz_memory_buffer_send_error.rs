...
// Targets: send_error in examples/http_server.csd:145
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // send_error(data, 0);        // Zero size
        // send_error(data, 1);        // Single byte
        // send_error(data, size);     // Full size
        // send_error(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for send_error
