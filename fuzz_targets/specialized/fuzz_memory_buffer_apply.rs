...
// Targets: apply in src/formatter/rules.rs:421
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // apply(data, 0);        // Zero size
        // apply(data, 1);        // Single byte
        // apply(data, size);     // Full size
        // apply(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for apply
