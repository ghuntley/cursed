...
// Targets: get_rule_options in src/linter/config.rs:118
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // get_rule_options(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // get_rule_options(data, 0);        // Zero size
        // get_rule_options(data, 1);        // Single byte
        // get_rule_options(data, size);     // Full size
        // get_rule_options(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for get_rule_options
