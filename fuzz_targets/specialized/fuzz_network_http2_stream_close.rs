...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    // Test packet parsing
    // Test http2_stream_close with network data
    // Example: http2_stream_close(data, size);
    // Example: http2_stream_close(sockfd, data, size);
...
void test_socket_operations(const uint8_t *data, size_t size) {
    ...
    // http2_stream_close(sockfd, data, size);
