...
// Targets: demonstrate_query_builder in examples/type_switch_practical_use_cases.csd:569
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // demonstrate_query_builder(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // demonstrate_query_builder(data, 0);        // Zero size
        // demonstrate_query_builder(data, 1);        // Single byte
        // demonstrate_query_builder(data, size);     // Full size
        // demonstrate_query_builder(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for demonstrate_query_builder
