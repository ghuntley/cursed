...
// Targets: get_by_name in src/codegen/llvm/jit_engine.rs:233
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // get_by_name(data, 0);        // Zero size
        // get_by_name(data, 1);        // Single byte
        // get_by_name(data, size);     // Full size
        // get_by_name(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for get_by_name
