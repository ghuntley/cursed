...
// Targets: cache_package in src/package_manager/cache.rs:114
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // cache_package(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // cache_package(data, 0);        // Zero size
        // cache_package(data, 1);        // Single byte
        // cache_package(data, size);     // Full size
        // cache_package(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for cache_package
