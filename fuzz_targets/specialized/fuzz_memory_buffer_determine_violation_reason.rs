...
// Targets: determine_violation_reason in src/type_system/constraint_resolver.rs:332
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // determine_violation_reason(buffer, data, size);
...
void test_string_operations(const uint8_t *data, size_t size) {
    ...
    // determine_violation_reason(str);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // determine_violation_reason(data, 0);        // Zero size
        // determine_violation_reason(data, 1);        // Single byte
        // determine_violation_reason(data, size);     // Full size
        // determine_violation_reason(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for determine_violation_reason
