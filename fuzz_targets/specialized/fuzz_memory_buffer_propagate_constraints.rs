...
// Targets: propagate_constraints in src/type_system/constraint_resolver.rs:71
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // propagate_constraints(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // propagate_constraints(data, 0);        // Zero size
        // propagate_constraints(data, 1);        // Single byte
        // propagate_constraints(data, size);     // Full size
        // propagate_constraints(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for propagate_constraints
