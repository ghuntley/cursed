...
// Targets: right in src/stdlib/database/orm/query_builder.rs:479
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // right(data, 0);        // Zero size
        // right(data, 1);        // Single byte
        // right(data, size);     // Full size
        // right(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for right
