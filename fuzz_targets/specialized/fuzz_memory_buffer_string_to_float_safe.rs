...
// Targets: string_to_float_safe in stdlib/vibez/enhanced_formatting.csd:229
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // string_to_float_safe(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // string_to_float_safe(data, 0);        // Zero size
        // string_to_float_safe(data, 1);        // Single byte
        // string_to_float_safe(data, size);     // Full size
        // string_to_float_safe(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for string_to_float_safe
