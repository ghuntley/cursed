...
// Targets: invalidate in src/build_system/incremental_cache.rs:147
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // invalidate(data, 0);        // Zero size
        // invalidate(data, 1);        // Single byte
        // invalidate(data, size);     // Full size
        // invalidate(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for invalidate
