...
// Targets: calculate_backoff_delay in src/runtime/recovery.rs:646
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // calculate_backoff_delay(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // calculate_backoff_delay(data, 0);        // Zero size
        // calculate_backoff_delay(data, 1);        // Single byte
        // calculate_backoff_delay(data, size);     // Full size
        // calculate_backoff_delay(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for calculate_backoff_delay
