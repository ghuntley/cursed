...
// Targets: serve_static_file in src/documentation/live_server.rs:331
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // serve_static_file(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // serve_static_file(data, 0);        // Zero size
        // serve_static_file(data, 1);        // Single byte
        // serve_static_file(data, size);     // Full size
        // serve_static_file(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for serve_static_file
