...
// Targets: test_string_assertions in stdlib/test_vibes/test_test_vibes.csd:167
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // test_string_assertions(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // test_string_assertions(data, 0);        // Zero size
        // test_string_assertions(data, 1);        // Single byte
        // test_string_assertions(data, size);     // Full size
        // test_string_assertions(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for test_string_assertions
