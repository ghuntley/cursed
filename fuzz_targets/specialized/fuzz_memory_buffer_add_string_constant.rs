...
// Targets: add_string_constant in src/codegen/llvm/string_constants.rs:39
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // add_string_constant(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // add_string_constant(data, 0);        // Zero size
        // add_string_constant(data, 1);        // Single byte
        // add_string_constant(data, size);     // Full size
        // add_string_constant(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for add_string_constant
