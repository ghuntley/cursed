...
// Targets: demonstrate_rounding in examples/stdlib_examples/mathz_example.csd:69
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // demonstrate_rounding(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // demonstrate_rounding(data, 0);        // Zero size
        // demonstrate_rounding(data, 1);        // Single byte
        // demonstrate_rounding(data, size);     // Full size
        // demonstrate_rounding(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for demonstrate_rounding
