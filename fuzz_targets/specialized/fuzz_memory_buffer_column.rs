...
// Targets: column in src/stdlib/packages/db_sql/builder.rs:108
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // column(data, 0);        // Zero size
        // column(data, 1);        // Single byte
        // column(data, size);     // Full size
        // column(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for column
