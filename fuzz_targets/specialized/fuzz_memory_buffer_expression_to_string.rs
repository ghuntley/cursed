...
// Targets: expression_to_string in src/ast.rs:1158
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // expression_to_string(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // expression_to_string(data, 0);        // Zero size
        // expression_to_string(data, 1);        // Single byte
        // expression_to_string(data, size);     // Full size
        // expression_to_string(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for expression_to_string
