...
// Targets: run_command_timeout in examples/simple_process_demo.rs:235
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // run_command_timeout(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // run_command_timeout(data, 0);        // Zero size
        // run_command_timeout(data, 1);        // Single byte
        // run_command_timeout(data, size);     // Full size
        // run_command_timeout(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for run_command_timeout
