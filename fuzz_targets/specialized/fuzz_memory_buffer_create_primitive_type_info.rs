...
// Targets: create_primitive_type_info in src/runtime/type_switch.rs:194
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // create_primitive_type_info(buffer, data, size);
...
void test_string_operations(const uint8_t *data, size_t size) {
    ...
    // create_primitive_type_info(str);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // create_primitive_type_info(data, 0);        // Zero size
        // create_primitive_type_info(data, 1);        // Single byte
        // create_primitive_type_info(data, size);     // Full size
        // create_primitive_type_info(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for create_primitive_type_info
