...
// Targets: benchmark_string_comparison in benchmarks/cursed/stdlib/string_bench.csd:97
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // benchmark_string_comparison(buffer, data, size);
...
void test_string_operations(const uint8_t *data, size_t size) {
    ...
    // benchmark_string_comparison(str);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // benchmark_string_comparison(data, 0);        // Zero size
        // benchmark_string_comparison(data, 1);        // Single byte
        // benchmark_string_comparison(data, size);     // Full size
        // benchmark_string_comparison(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for benchmark_string_comparison
