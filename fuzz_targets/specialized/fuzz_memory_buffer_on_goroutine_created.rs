...
// Targets: on_goroutine_created in src/runtime/performance_hooks.rs:551
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // on_goroutine_created(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // on_goroutine_created(data, 0);        // Zero size
        // on_goroutine_created(data, 1);        // Single byte
        // on_goroutine_created(data, size);     // Full size
        // on_goroutine_created(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for on_goroutine_created
