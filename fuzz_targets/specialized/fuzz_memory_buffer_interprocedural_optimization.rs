...
// Targets: interprocedural_optimization in src/codegen/llvm/generic_optimization.rs:528
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // interprocedural_optimization(buffer, data, size);
...
void test_string_operations(const uint8_t *data, size_t size) {
    ...
    // interprocedural_optimization(str);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // interprocedural_optimization(data, 0);        // Zero size
        // interprocedural_optimization(data, 1);        // Single byte
        // interprocedural_optimization(data, size);     // Full size
        // interprocedural_optimization(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for interprocedural_optimization
