...
// Targets: fold_fsub in src/codegen/llvm/passes/constant_propagation.rs:763
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // fold_fsub(data, 0);        // Zero size
        // fold_fsub(data, 1);        // Single byte
        // fold_fsub(data, size);     // Full size
        // fold_fsub(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for fold_fsub
