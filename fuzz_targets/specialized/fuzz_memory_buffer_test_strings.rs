...
// Targets: test_strings in performance_test.csd:33
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // test_strings(data, 0);        // Zero size
        // test_strings(data, 1);        // Single byte
        // test_strings(data, size);     // Full size
        // test_strings(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for test_strings
