...
// Targets: shouldInlineFunction in src-zig/enhanced_minimal_compiler.zig:1202
...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    // Test shouldInlineFunction with temp file
    // Example: shouldInlineFunction(temp_path);
    // Example: shouldInlineFunction(temp_path, data, size);
...
void test_path_operations(const uint8_t *data, size_t size) {
    ...
    // shouldInlineFunction(path);
}


// Additional test functions for shouldInlineFunction
