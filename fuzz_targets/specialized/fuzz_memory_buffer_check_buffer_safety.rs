...
// Targets: check_buffer_safety in src/ffi/safety_checks.rs:182
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // check_buffer_safety(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // check_buffer_safety(data, 0);        // Zero size
        // check_buffer_safety(data, 1);        // Single byte
        // check_buffer_safety(data, size);     // Full size
        // check_buffer_safety(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for check_buffer_safety
