...
// Targets: generate_function in src/codegen/llvm/main.rs:2412
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // generate_function(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // generate_function(data, 0);        // Zero size
        // generate_function(data, 1);        // Single byte
        // generate_function(data, size);     // Full size
        // generate_function(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for generate_function
