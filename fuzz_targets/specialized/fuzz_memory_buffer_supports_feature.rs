...
// Targets: supports_feature in src/stdlib/packages/db_sql/sqlite.rs:74
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // supports_feature(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // supports_feature(data, 0);        // Zero size
        // supports_feature(data, 1);        // Single byte
        // supports_feature(data, size);     // Full size
        // supports_feature(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for supports_feature
