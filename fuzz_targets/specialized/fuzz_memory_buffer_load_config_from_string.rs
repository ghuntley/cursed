...
// Targets: load_config_from_string in stdlib/formatter/mod.csd:1417
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // load_config_from_string(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // load_config_from_string(data, 0);        // Zero size
        // load_config_from_string(data, 1);        // Single byte
        // load_config_from_string(data, size);     // Full size
        // load_config_from_string(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for load_config_from_string
