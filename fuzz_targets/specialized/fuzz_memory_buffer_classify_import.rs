...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // classify_import(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // classify_import(data, 0);        // Zero size
        // classify_import(data, 1);        // Single byte
        // classify_import(data, size);     // Full size
        // classify_import(NULL, 0);        // NULL pointer
    }
}
