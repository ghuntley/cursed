...
// Targets: parse_edge_name in src/optimization/pgo/optimization_engine.rs:425
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // parse_edge_name(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // parse_edge_name(data, 0);        // Zero size
        // parse_edge_name(data, 1);        // Single byte
        // parse_edge_name(data, size);     // Full size
        // parse_edge_name(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for parse_edge_name
