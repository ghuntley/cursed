...
// Targets: cursed_format_error in src-zig/error_runtime_support.zig:162
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // cursed_format_error(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // cursed_format_error(data, 0);        // Zero size
        // cursed_format_error(data, 1);        // Single byte
        // cursed_format_error(data, size);     // Full size
        // cursed_format_error(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for cursed_format_error
