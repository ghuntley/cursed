...
// Targets: logger_init_buffer in stdlib/plugin_system/example_plugin.csd:307
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // logger_init_buffer(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // logger_init_buffer(data, 0);        // Zero size
        // logger_init_buffer(data, 1);        // Single byte
        // logger_init_buffer(data, size);     // Full size
        // logger_init_buffer(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for logger_init_buffer
