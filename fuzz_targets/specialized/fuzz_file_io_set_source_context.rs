...
// Targets: set_source_context in src/parser_error_recovery.rs:329
...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    // Test set_source_context with temp file
    // Example: set_source_context(temp_path);
    // Example: set_source_context(temp_path, data, size);
...
void test_path_operations(const uint8_t *data, size_t size) {
    ...
    // set_source_context(path);
}


// Additional test functions for set_source_context
