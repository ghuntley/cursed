...
// Targets: parse_interface_call in src/codegen/llvm/interface_optimization.rs:521
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // parse_interface_call(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // parse_interface_call(data, 0);        // Zero size
        // parse_interface_call(data, 1);        // Single byte
        // parse_interface_call(data, size);     // Full size
        // parse_interface_call(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for parse_interface_call
