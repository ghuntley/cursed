...
// Targets: get_last_error_message in src/runtime/goroutine_context.rs:2962
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // get_last_error_message(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // get_last_error_message(data, 0);        // Zero size
        // get_last_error_message(data, 1);        // Single byte
        // get_last_error_message(data, size);     // Full size
        // get_last_error_message(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for get_last_error_message
