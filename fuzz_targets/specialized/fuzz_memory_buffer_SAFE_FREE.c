...
// Targets: SAFE_FREE in src-zig/safe_operations.zig:370
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // SAFE_FREE(data, 0);        // Zero size
        // SAFE_FREE(data, 1);        // Single byte
        // SAFE_FREE(data, size);     // Full size
        // SAFE_FREE(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for SAFE_FREE
