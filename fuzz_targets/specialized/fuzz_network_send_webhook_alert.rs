...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    // Test packet parsing
    // Test send_webhook_alert with network data
    // Example: send_webhook_alert(data, size);
    // Example: send_webhook_alert(sockfd, data, size);
...
void test_socket_operations(const uint8_t *data, size_t size) {
    ...
    // send_webhook_alert(sockfd, data, size);
