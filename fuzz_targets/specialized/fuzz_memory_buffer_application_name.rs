...
// Targets: application_name in src/stdlib/database/postgres/config.rs:126
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // application_name(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // application_name(data, 0);        // Zero size
        // application_name(data, 1);        // Single byte
        // application_name(data, size);     // Full size
        // application_name(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for application_name
