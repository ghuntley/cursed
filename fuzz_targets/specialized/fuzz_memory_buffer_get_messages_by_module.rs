...
// Targets: get_messages_by_module in src/runtime/debug_output.rs:519
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // get_messages_by_module(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // get_messages_by_module(data, 0);        // Zero size
        // get_messages_by_module(data, 1);        // Single byte
        // get_messages_by_module(data, size);     // Full size
        // get_messages_by_module(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for get_messages_by_module
