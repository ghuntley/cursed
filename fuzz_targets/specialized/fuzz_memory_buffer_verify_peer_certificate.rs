...
// Targets: verify_peer_certificate in src/security/network_secure.rs:351
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // verify_peer_certificate(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // verify_peer_certificate(data, 0);        // Zero size
        // verify_peer_certificate(data, 1);        // Single byte
        // verify_peer_certificate(data, size);     // Full size
        // verify_peer_certificate(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for verify_peer_certificate
