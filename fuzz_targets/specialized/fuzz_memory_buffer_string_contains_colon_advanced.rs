...
// Targets: string_contains_colon_advanced in stdlib/configz/mod_complex.csd:1119
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // string_contains_colon_advanced(buffer, data, size);
...
void test_string_operations(const uint8_t *data, size_t size) {
    ...
    // string_contains_colon_advanced(str);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // string_contains_colon_advanced(data, 0);        // Zero size
        // string_contains_colon_advanced(data, 1);        // Single byte
        // string_contains_colon_advanced(data, size);     // Full size
        // string_contains_colon_advanced(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for string_contains_colon_advanced
