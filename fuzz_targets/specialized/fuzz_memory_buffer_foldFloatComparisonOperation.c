...
// Targets: foldFloatComparisonOperation in src-zig/constant_folder.zig:209
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // foldFloatComparisonOperation(buffer, data, size);
...
void test_string_operations(const uint8_t *data, size_t size) {
    ...
    // foldFloatComparisonOperation(str);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // foldFloatComparisonOperation(data, 0);        // Zero size
        // foldFloatComparisonOperation(data, 1);        // Single byte
        // foldFloatComparisonOperation(data, size);     // Full size
        // foldFloatComparisonOperation(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for foldFloatComparisonOperation
