...
// Targets: string_left in stdlib/string_simple/mod.csd:286
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // string_left(data, 0);        // Zero size
        // string_left(data, 1);        // Single byte
        // string_left(data, size);     // Full size
        // string_left(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for string_left
