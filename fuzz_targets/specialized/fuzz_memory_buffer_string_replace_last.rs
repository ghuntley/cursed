...
// Targets: string_replace_last in stdlib/string_simple/mod.csd:545
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // string_replace_last(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // string_replace_last(data, 0);        // Zero size
        // string_replace_last(data, 1);        // Single byte
        // string_replace_last(data, size);     // Full size
        // string_replace_last(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for string_replace_last
