...
// Targets: export_binary in src/ffi/profiling.rs:438
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // export_binary(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // export_binary(data, 0);        // Zero size
        // export_binary(data, 1);        // Single byte
        // export_binary(data, size);     // Full size
        // export_binary(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for export_binary
