...
// Targets: goroutine_panic in src/runtime/panic_recover.rs:568
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // goroutine_panic(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // goroutine_panic(data, 0);        // Zero size
        // goroutine_panic(data, 1);        // Single byte
        // goroutine_panic(data, size);     // Full size
        // goroutine_panic(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for goroutine_panic
