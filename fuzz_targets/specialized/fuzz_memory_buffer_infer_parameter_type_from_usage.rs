...
// Targets: infer_parameter_type_from_usage in src/codegen/llvm/function_compilation.rs:1818
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // infer_parameter_type_from_usage(buffer, data, size);
...
void test_string_operations(const uint8_t *data, size_t size) {
    ...
    // infer_parameter_type_from_usage(str);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // infer_parameter_type_from_usage(data, 0);        // Zero size
        // infer_parameter_type_from_usage(data, 1);        // Single byte
        // infer_parameter_type_from_usage(data, size);     // Full size
        // infer_parameter_type_from_usage(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for infer_parameter_type_from_usage
