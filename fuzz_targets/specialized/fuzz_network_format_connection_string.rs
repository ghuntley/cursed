...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    // Test packet parsing
    // Test format_connection_string with network data
    // Example: format_connection_string(data, size);
    // Example: format_connection_string(sockfd, data, size);
...
void test_socket_operations(const uint8_t *data, size_t size) {
    ...
    // format_connection_string(sockfd, data, size);
