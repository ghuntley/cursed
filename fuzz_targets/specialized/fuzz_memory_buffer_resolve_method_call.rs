...
// Targets: resolve_method_call in src/codegen/llvm/interface_dispatch.rs:677
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // resolve_method_call(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // resolve_method_call(data, 0);        // Zero size
        // resolve_method_call(data, 1);        // Single byte
        // resolve_method_call(data, size);     // Full size
        // resolve_method_call(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for resolve_method_call
