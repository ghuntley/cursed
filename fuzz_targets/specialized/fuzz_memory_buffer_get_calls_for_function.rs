...
// Targets: get_calls_for_function in src/ffi/debug_tools.rs:737
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // get_calls_for_function(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // get_calls_for_function(data, 0);        // Zero size
        // get_calls_for_function(data, 1);        // Single byte
        // get_calls_for_function(data, size);     // Full size
        // get_calls_for_function(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for get_calls_for_function
