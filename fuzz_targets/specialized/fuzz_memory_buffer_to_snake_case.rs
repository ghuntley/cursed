...
// Targets: to_snake_case in stdlib/string_enhanced/mod.csd:388
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // to_snake_case(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // to_snake_case(data, 0);        // Zero size
        // to_snake_case(data, 1);        // Single byte
        // to_snake_case(data, size);     // Full size
        // to_snake_case(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for to_snake_case
