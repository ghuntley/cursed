...
// Targets: record_sample in src/profiling/core.rs:87
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // record_sample(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // record_sample(data, 0);        // Zero size
        // record_sample(data, 1);        // Single byte
        // record_sample(data, size);     // Full size
        // record_sample(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for record_sample
