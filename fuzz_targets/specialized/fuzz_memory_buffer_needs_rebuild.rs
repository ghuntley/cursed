...
// Targets: needs_rebuild in src/build_system/incremental_cache.rs:83
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // needs_rebuild(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // needs_rebuild(data, 0);        // Zero size
        // needs_rebuild(data, 1);        // Single byte
        // needs_rebuild(data, size);     // Full size
        // needs_rebuild(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for needs_rebuild
