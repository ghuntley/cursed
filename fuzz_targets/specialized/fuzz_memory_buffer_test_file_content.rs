...
// Targets: test_file_content in src/bin/test_errors.rs:161
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // test_file_content(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // test_file_content(data, 0);        // Zero size
        // test_file_content(data, 1);        // Single byte
        // test_file_content(data, size);     // Full size
        // test_file_content(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for test_file_content
