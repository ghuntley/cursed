...
// Targets: string_reverse in stdlib/string_simple/mod.csd:298
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // string_reverse(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // string_reverse(data, 0);        // Zero size
        // string_reverse(data, 1);        // Single byte
        // string_reverse(data, size);     // Full size
        // string_reverse(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for string_reverse
