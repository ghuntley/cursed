...
// Targets: convert_to in src/runtime/runtime_value.rs:297
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // convert_to(data, 0);        // Zero size
        // convert_to(data, 1);        // Single byte
        // convert_to(data, size);     // Full size
        // convert_to(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for convert_to
