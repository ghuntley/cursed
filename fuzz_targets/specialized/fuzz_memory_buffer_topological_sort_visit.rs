...
// Targets: topological_sort_visit in src/optimization/function_inlining_complete.rs:662
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // topological_sort_visit(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // topological_sort_visit(data, 0);        // Zero size
        // topological_sort_visit(data, 1);        // Single byte
        // topological_sort_visit(data, size);     // Full size
        // topological_sort_visit(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for topological_sort_visit
