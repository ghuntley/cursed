...
// Targets: with_buffer_type in src/runtime/channels/channel.rs:100
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // with_buffer_type(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // with_buffer_type(data, 0);        // Zero size
        // with_buffer_type(data, 1);        // Single byte
        // with_buffer_type(data, size);     // Full size
        // with_buffer_type(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for with_buffer_type
