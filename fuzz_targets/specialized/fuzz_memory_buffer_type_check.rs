...
// Targets: type_check in src/core/type_checker.rs:45
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // type_check(data, 0);        // Zero size
        // type_check(data, 1);        // Single byte
        // type_check(data, size);     // Full size
        // type_check(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for type_check
