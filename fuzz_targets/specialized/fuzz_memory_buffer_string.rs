...
// Targets: string in stdlib/runtime_core/mod.csd:336
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // string(data, 0);        // Zero size
        // string(data, 1);        // Single byte
        // string(data, size);     // Full size
        // string(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for string
