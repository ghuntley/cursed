...
// Targets: parameter in src/type_system/mod.rs:651
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // parameter(data, 0);        // Zero size
        // parameter(data, 1);        // Single byte
        // parameter(data, size);     // Full size
        // parameter(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for parameter
