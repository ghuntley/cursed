...
// Targets: channel_destroy in stdlib/select_core/mod.csd:265
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // channel_destroy(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // channel_destroy(data, 0);        // Zero size
        // channel_destroy(data, 1);        // Single byte
        // channel_destroy(data, size);     // Full size
        // channel_destroy(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for channel_destroy
