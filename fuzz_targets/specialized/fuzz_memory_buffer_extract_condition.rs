...
// Targets: extract_condition in src/coverage/basic_coverage.rs:317
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // extract_condition(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // extract_condition(data, 0);        // Zero size
        // extract_condition(data, 1);        // Single byte
        // extract_condition(data, size);     // Full size
        // extract_condition(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for extract_condition
