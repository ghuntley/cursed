...
// Targets: is_cached in src/package_manager/cache.rs:85
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // is_cached(data, 0);        // Zero size
        // is_cached(data, 1);        // Single byte
        // is_cached(data, size);     // Full size
        // is_cached(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for is_cached
