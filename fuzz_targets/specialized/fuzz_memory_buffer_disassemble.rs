...
// Targets: disassemble in src/bin/cursed_debug.rs:600
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // disassemble(data, 0);        // Zero size
        // disassemble(data, 1);        // Single byte
        // disassemble(data, size);     // Full size
        // disassemble(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for disassemble
