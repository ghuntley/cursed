...
// Targets: execute_function_value in src/runtime/goroutine_context.rs:1158
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // execute_function_value(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // execute_function_value(data, 0);        // Zero size
        // execute_function_value(data, 1);        // Single byte
        // execute_function_value(data, size);     // Full size
        // execute_function_value(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for execute_function_value
