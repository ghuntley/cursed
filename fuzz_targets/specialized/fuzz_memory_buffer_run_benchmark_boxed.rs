...
// Targets: run_benchmark_boxed in src/profiling/benchmarking.rs:126
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // run_benchmark_boxed(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // run_benchmark_boxed(data, 0);        // Zero size
        // run_benchmark_boxed(data, 1);        // Single byte
        // run_benchmark_boxed(data, size);     // Full size
        // run_benchmark_boxed(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for run_benchmark_boxed
