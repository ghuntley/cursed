...
// Targets: run_stage in src/codegen/llvm/optimization_pipeline.rs:225
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // run_stage(data, 0);        // Zero size
        // run_stage(data, 1);        // Single byte
        // run_stage(data, size);     // Full size
        // run_stage(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for run_stage
