...
// Targets: fold_icmp in src/codegen/llvm/passes/constant_propagation.rs:637
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // fold_icmp(data, 0);        // Zero size
        // fold_icmp(data, 1);        // Single byte
        // fold_icmp(data, size);     // Full size
        // fold_icmp(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for fold_icmp
