...
// Targets: load in src/stdlib/database/orm/relationships.rs:70
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // load(data, 0);        // Zero size
        // load(data, 1);        // Single byte
        // load(data, size);     // Full size
        // load(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for load
