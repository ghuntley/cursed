...
// Targets: test_stringz_parsing in stdlib/test_enhanced_stdlib.csd:97
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // test_stringz_parsing(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // test_stringz_parsing(data, 0);        // Zero size
        // test_stringz_parsing(data, 1);        // Single byte
        // test_stringz_parsing(data, size);     // Full size
        // test_stringz_parsing(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for test_stringz_parsing
