...
// Targets: with_location in src/runtime/stack_trace.rs:632
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // with_location(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // with_location(data, 0);        // Zero size
        // with_location(data, 1);        // Single byte
        // with_location(data, size);     // Full size
        // with_location(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for with_location
