...
// Targets: update_session_state in src/repl/jit_repl.rs:376
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // update_session_state(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // update_session_state(data, 0);        // Zero size
        // update_session_state(data, 1);        // Single byte
        // update_session_state(data, size);     // Full size
        // update_session_state(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for update_session_state
