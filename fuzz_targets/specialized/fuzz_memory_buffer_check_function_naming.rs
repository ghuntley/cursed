...
// Targets: check_function_naming in src/linter/rules.rs:89
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // check_function_naming(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // check_function_naming(data, 0);        // Zero size
        // check_function_naming(data, 1);        // Single byte
        // check_function_naming(data, size);     // Full size
        // check_function_naming(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for check_function_naming
