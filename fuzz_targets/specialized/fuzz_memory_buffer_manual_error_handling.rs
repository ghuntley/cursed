...
// Targets: manual_error_handling in examples/question_mark_demo.csd:66
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // manual_error_handling(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // manual_error_handling(data, 0);        // Zero size
        // manual_error_handling(data, 1);        // Single byte
        // manual_error_handling(data, size);     // Full size
        // manual_error_handling(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for manual_error_handling
