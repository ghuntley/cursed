...
// Targets: extract_function_name_simple in src/coverage/simple_collector.rs:157
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // extract_function_name_simple(buffer, data, size);
...
void test_string_operations(const uint8_t *data, size_t size) {
    ...
    // extract_function_name_simple(str);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // extract_function_name_simple(data, 0);        // Zero size
        // extract_function_name_simple(data, 1);        // Single byte
        // extract_function_name_simple(data, size);     // Full size
        // extract_function_name_simple(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for extract_function_name_simple
