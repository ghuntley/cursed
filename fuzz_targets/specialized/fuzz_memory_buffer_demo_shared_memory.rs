...
// Targets: demo_shared_memory in examples/process_ipc_showcase.csd:363
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // demo_shared_memory(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // demo_shared_memory(data, 0);        // Zero size
        // demo_shared_memory(data, 1);        // Single byte
        // demo_shared_memory(data, size);     // Full size
        // demo_shared_memory(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for demo_shared_memory
