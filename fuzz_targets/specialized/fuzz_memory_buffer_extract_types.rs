...
// Targets: extract_types in src/documentation/api_extractor.rs:186
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // extract_types(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // extract_types(data, 0);        // Zero size
        // extract_types(data, 1);        // Single byte
        // extract_types(data, size);     // Full size
        // extract_types(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for extract_types
