...
// Targets: check_member_access_patterns in src/tools/linter.rs:1907
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // check_member_access_patterns(buffer, data, size);
...
void test_string_operations(const uint8_t *data, size_t size) {
    ...
    // check_member_access_patterns(str);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // check_member_access_patterns(data, 0);        // Zero size
        // check_member_access_patterns(data, 1);        // Single byte
        // check_member_access_patterns(data, size);     // Full size
        // check_member_access_patterns(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for check_member_access_patterns
