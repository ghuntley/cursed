...
// Targets: left in src/stdlib/database/orm/query_builder.rs:475
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // left(data, 0);        // Zero size
        // left(data, 1);        // Single byte
        // left(data, size);     // Full size
        // left(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for left
