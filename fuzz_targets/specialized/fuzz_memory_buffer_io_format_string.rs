...
// Targets: io_format_string in stdlib/io_simple/mod_broken.csd:47
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // io_format_string(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // io_format_string(data, 0);        // Zero size
        // io_format_string(data, 1);        // Single byte
        // io_format_string(data, size);     // Full size
        // io_format_string(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for io_format_string
