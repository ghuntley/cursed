...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    // Test packet parsing
    // Test http2_goaway_frame with network data
    // Example: http2_goaway_frame(data, size);
    // Example: http2_goaway_frame(sockfd, data, size);
...
void test_socket_operations(const uint8_t *data, size_t size) {
    ...
    // http2_goaway_frame(sockfd, data, size);
