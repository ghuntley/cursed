...
// Targets: callSprintfNative in src-zig/variadic_ffi_bridge.zig:494
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // callSprintfNative(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // callSprintfNative(data, 0);        // Zero size
        // callSprintfNative(data, 1);        // Single byte
        // callSprintfNative(data, size);     // Full size
        // callSprintfNative(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for callSprintfNative
