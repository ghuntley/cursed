...
// Targets: format_string_real in stdlib/vibez/enhanced_formatting.csd:8
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // format_string_real(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // format_string_real(data, 0);        // Zero size
        // format_string_real(data, 1);        // Single byte
        // format_string_real(data, size);     // Full size
        // format_string_real(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for format_string_real
