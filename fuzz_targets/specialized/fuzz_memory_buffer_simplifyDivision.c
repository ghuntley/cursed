...
// Targets: simplifyDivision in src-zig/constant_folder.zig:538
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // simplifyDivision(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // simplifyDivision(data, 0);        // Zero size
        // simplifyDivision(data, 1);        // Single byte
        // simplifyDivision(data, size);     // Full size
        // simplifyDivision(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for simplifyDivision
