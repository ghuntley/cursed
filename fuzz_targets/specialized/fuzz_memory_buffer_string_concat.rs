...
// Targets: string_concat in stdlib/asn1_mood/mod.csd:388
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // string_concat(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // string_concat(data, 0);        // Zero size
        // string_concat(data, 1);        // Single byte
        // string_concat(data, size);     // Full size
        // string_concat(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for string_concat
