...
// Targets: runtime_string_char_at in stdlib/stringz/mod_enhanced.csd:988
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // runtime_string_char_at(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // runtime_string_char_at(data, 0);        // Zero size
        // runtime_string_char_at(data, 1);        // Single byte
        // runtime_string_char_at(data, size);     // Full size
        // runtime_string_char_at(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for runtime_string_char_at
