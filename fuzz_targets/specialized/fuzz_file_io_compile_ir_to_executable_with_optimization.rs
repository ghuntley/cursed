...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    // Test compile_ir_to_executable_with_optimization with temp file
    // Example: compile_ir_to_executable_with_optimization(temp_path);
    // Example: compile_ir_to_executable_with_optimization(temp_path, data, size);
...
void test_path_operations(const uint8_t *data, size_t size) {
    ...
    // compile_ir_to_executable_with_optimization(path);
}
