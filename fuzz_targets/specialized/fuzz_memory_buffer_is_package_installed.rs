...
// Targets: is_package_installed in src/imports/package_resolver.rs:421
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // is_package_installed(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // is_package_installed(data, 0);        // Zero size
        // is_package_installed(data, 1);        // Single byte
        // is_package_installed(data, size);     // Full size
        // is_package_installed(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for is_package_installed
