...
// Targets: test_async_streams in stdlib/asyncz/test_asyncz.csd:229
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // test_async_streams(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // test_async_streams(data, 0);        // Zero size
        // test_async_streams(data, 1);        // Single byte
        // test_async_streams(data, size);     // Full size
        // test_async_streams(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for test_async_streams
