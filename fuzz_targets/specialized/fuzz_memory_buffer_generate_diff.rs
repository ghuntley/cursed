...
// Targets: generate_diff in src/formatter/mod_complex.rs:1183
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // generate_diff(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // generate_diff(data, 0);        // Zero size
        // generate_diff(data, 1);        // Single byte
        // generate_diff(data, size);     // Full size
        // generate_diff(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for generate_diff
