...
// Targets: test_production_gc_under_load in src/runtime/production_gc_test.rs:125
...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    // Test test_production_gc_under_load with temp file
    // Example: test_production_gc_under_load(temp_path);
    // Example: test_production_gc_under_load(temp_path, data, size);
...
void test_path_operations(const uint8_t *data, size_t size) {
    ...
    // test_production_gc_under_load(path);
}


// Additional test functions for test_production_gc_under_load
