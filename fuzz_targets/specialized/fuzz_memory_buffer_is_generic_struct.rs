...
// Targets: is_generic_struct in src/type_system/monomorphisation.rs:990
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // is_generic_struct(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // is_generic_struct(data, 0);        // Zero size
        // is_generic_struct(data, 1);        // Single byte
        // is_generic_struct(data, size);     // Full size
        // is_generic_struct(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for is_generic_struct
