...
// Targets: visit_program in src/ast.rs:20
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // visit_program(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // visit_program(data, 0);        // Zero size
        // visit_program(data, 1);        // Single byte
        // visit_program(data, size);     // Full size
        // visit_program(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for visit_program
