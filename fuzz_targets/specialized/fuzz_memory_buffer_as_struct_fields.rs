...
// Targets: as_struct_fields in examples/ffi_demo.rs:536
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // as_struct_fields(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // as_struct_fields(data, 0);        // Zero size
        // as_struct_fields(data, 1);        // Single byte
        // as_struct_fields(data, size);     // Full size
        // as_struct_fields(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for as_struct_fields
