...
// Targets: count_changes in src/tools/formatter.rs:865
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // count_changes(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // count_changes(data, 0);        // Zero size
        // count_changes(data, 1);        // Single byte
        // count_changes(data, size);     // Full size
        // count_changes(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for count_changes
