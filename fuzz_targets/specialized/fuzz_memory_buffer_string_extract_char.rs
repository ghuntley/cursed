...
// Targets: string_extract_char in stdlib/vibez/mod_complex.csd:240
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // string_extract_char(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // string_extract_char(data, 0);        // Zero size
        // string_extract_char(data, 1);        // Single byte
        // string_extract_char(data, size);     // Full size
        // string_extract_char(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for string_extract_char
