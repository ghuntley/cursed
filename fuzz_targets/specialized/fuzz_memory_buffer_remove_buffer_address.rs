...
// Targets: remove_buffer_address in src/runtime/channels/lifecycle.rs:564
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // remove_buffer_address(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // remove_buffer_address(data, 0);        // Zero size
        // remove_buffer_address(data, 1);        // Single byte
        // remove_buffer_address(data, size);     // Full size
        // remove_buffer_address(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for remove_buffer_address
