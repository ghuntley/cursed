...
// Targets: rust_zero_memory in src/runtime/memory_bridge.rs:394
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // rust_zero_memory(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // rust_zero_memory(data, 0);        // Zero size
        // rust_zero_memory(data, 1);        // Single byte
        // rust_zero_memory(data, size);     // Full size
        // rust_zero_memory(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for rust_zero_memory
