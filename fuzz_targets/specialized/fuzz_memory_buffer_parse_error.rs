...
// Targets: parse_error in src/error/mod.rs:222
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // parse_error(data, 0);        // Zero size
        // parse_error(data, 1);        // Single byte
        // parse_error(data, size);     // Full size
        // parse_error(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for parse_error
