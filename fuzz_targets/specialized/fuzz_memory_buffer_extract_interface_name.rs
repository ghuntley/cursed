...
// Targets: extract_interface_name in src/codegen/llvm/passes/inlining.rs:408
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // extract_interface_name(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // extract_interface_name(data, 0);        // Zero size
        // extract_interface_name(data, 1);        // Single byte
        // extract_interface_name(data, size);     // Full size
        // extract_interface_name(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for extract_interface_name
