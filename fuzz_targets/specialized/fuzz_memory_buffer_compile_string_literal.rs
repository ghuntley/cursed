...
// Targets: compile_string_literal in src/codegen/llvm/expression_compiler.rs:338
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // compile_string_literal(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // compile_string_literal(data, 0);        // Zero size
        // compile_string_literal(data, 1);        // Single byte
        // compile_string_literal(data, size);     // Full size
        // compile_string_literal(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for compile_string_literal
