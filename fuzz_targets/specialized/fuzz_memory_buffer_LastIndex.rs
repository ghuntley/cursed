...
// Targets: LastIndex in stdlib/string_energy/mod.csd:93
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // LastIndex(data, 0);        // Zero size
        // LastIndex(data, 1);        // Single byte
        // LastIndex(data, size);     // Full size
        // LastIndex(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for LastIndex
