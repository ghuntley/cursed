...
// Targets: parse_program in src/ast.rs:1276
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // parse_program(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // parse_program(data, 0);        // Zero size
        // parse_program(data, 1);        // Single byte
        // parse_program(data, size);     // Full size
        // parse_program(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for parse_program
