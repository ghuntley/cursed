...
// Targets: record_metric in examples/test_example.csd:215
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // record_metric(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // record_metric(data, 0);        // Zero size
        // record_metric(data, 1);        // Single byte
        // record_metric(data, size);     // Full size
        // record_metric(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for record_metric
