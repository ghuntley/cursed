...
// Targets: create_diagnostic_error in src/error/diagnostics.rs:228
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // create_diagnostic_error(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // create_diagnostic_error(data, 0);        // Zero size
        // create_diagnostic_error(data, 1);        // Single byte
        // create_diagnostic_error(data, size);     // Full size
        // create_diagnostic_error(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for create_diagnostic_error
