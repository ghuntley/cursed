...
// Targets: run_file_enhanced in src/lib.rs:586
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // run_file_enhanced(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // run_file_enhanced(data, 0);        // Zero size
        // run_file_enhanced(data, 1);        // Single byte
        // run_file_enhanced(data, size);     // Full size
        // run_file_enhanced(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for run_file_enhanced
