...
// Targets: record_adaptation_event in src/memory/adaptive_gc.rs:448
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // record_adaptation_event(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // record_adaptation_event(data, 0);        // Zero size
        // record_adaptation_event(data, 1);        // Single byte
        // record_adaptation_event(data, size);     // Full size
        // record_adaptation_event(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for record_adaptation_event
