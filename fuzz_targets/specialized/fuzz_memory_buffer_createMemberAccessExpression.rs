...
// Targets: createMemberAccessExpression in src-zig/ast.csd:501
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // createMemberAccessExpression(buffer, data, size);
...
void test_string_operations(const uint8_t *data, size_t size) {
    ...
    // createMemberAccessExpression(str);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // createMemberAccessExpression(data, 0);        // Zero size
        // createMemberAccessExpression(data, 1);        // Single byte
        // createMemberAccessExpression(data, size);     // Full size
        // createMemberAccessExpression(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for createMemberAccessExpression
