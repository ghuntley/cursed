...
// Targets: remove_trusted_certificate in src/stdlib/packages/src/stdlib/packages/crypto_pki/trust_store.rs:13
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // remove_trusted_certificate(buffer, data, size);
...
void test_string_operations(const uint8_t *data, size_t size) {
    ...
    // remove_trusted_certificate(str);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // remove_trusted_certificate(data, 0);        // Zero size
        // remove_trusted_certificate(data, 1);        // Single byte
        // remove_trusted_certificate(data, size);     // Full size
        // remove_trusted_certificate(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for remove_trusted_certificate
