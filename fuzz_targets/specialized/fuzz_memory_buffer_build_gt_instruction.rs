...
// Targets: build_gt_instruction in src/codegen/llvm/inkwell_expression_compiler.rs:439
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // build_gt_instruction(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // build_gt_instruction(data, 0);        // Zero size
        // build_gt_instruction(data, 1);        // Single byte
        // build_gt_instruction(data, size);     // Full size
        // build_gt_instruction(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for build_gt_instruction
