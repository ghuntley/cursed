...
// Targets: initChannelRegistry in src-zig/concurrency.zig:1854
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // initChannelRegistry(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // initChannelRegistry(data, 0);        // Zero size
        // initChannelRegistry(data, 1);        // Single byte
        // initChannelRegistry(data, size);     // Full size
        // initChannelRegistry(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for initChannelRegistry
