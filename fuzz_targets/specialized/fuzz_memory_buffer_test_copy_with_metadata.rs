...
// Targets: test_copy_with_metadata in stdlib/filesystem_complete/test_filesystem_complete.csd:484
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // test_copy_with_metadata(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // test_copy_with_metadata(data, 0);        // Zero size
        // test_copy_with_metadata(data, 1);        // Single byte
        // test_copy_with_metadata(data, size);     // Full size
        // test_copy_with_metadata(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for test_copy_with_metadata
