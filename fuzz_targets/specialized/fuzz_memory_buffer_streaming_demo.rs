...
// Targets: streaming_demo in examples/csv_mood_demo.csd:108
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // streaming_demo(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // streaming_demo(data, 0);        // Zero size
        // streaming_demo(data, 1);        // Single byte
        // streaming_demo(data, size);     // Full size
        // streaming_demo(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for streaming_demo
