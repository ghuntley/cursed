...
// Targets: memory_object_exists in stdlib/memory_core/mod.csd:148
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // memory_object_exists(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // memory_object_exists(data, 0);        // Zero size
        // memory_object_exists(data, 1);        // Single byte
        // memory_object_exists(data, size);     // Full size
        // memory_object_exists(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for memory_object_exists
