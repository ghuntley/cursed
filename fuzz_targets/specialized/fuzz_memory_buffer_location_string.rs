...
// Targets: location_string in src/runtime/stack_trace.rs:114
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // location_string(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // location_string(data, 0);        // Zero size
        // location_string(data, 1);        // Single byte
        // location_string(data, size);     // Full size
        // location_string(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for location_string
