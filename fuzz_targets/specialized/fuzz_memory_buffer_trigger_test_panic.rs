...
// Targets: trigger_test_panic in src/runtime/panic.rs:264
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // trigger_test_panic(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // trigger_test_panic(data, 0);        // Zero size
        // trigger_test_panic(data, 1);        // Single byte
        // trigger_test_panic(data, size);     // Full size
        // trigger_test_panic(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for trigger_test_panic
