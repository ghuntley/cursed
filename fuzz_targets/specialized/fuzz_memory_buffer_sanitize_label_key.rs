...
// Targets: sanitize_label_key in src/metrics/prometheus_exporter.rs:680
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // sanitize_label_key(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // sanitize_label_key(data, 0);        // Zero size
        // sanitize_label_key(data, 1);        // Single byte
        // sanitize_label_key(data, size);     // Full size
        // sanitize_label_key(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for sanitize_label_key
