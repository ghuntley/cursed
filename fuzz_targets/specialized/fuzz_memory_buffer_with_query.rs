...
// Targets: with_query in src/stdlib/database/orm/relationships.rs:323
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // with_query(data, 0);        // Zero size
        // with_query(data, 1);        // Single byte
        // with_query(data, size);     // Full size
        // with_query(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for with_query
