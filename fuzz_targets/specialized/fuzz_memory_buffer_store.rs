...
// Targets: store in src/build_system/advanced_cache.rs:107
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // store(data, 0);        // Zero size
        // store(data, 1);        // Single byte
        // store(data, size);     // Full size
        // store(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for store
