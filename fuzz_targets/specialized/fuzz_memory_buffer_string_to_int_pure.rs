...
// Targets: string_to_int_pure in stdlib/self_hosted_builtins/mod.csd:197
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // string_to_int_pure(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // string_to_int_pure(data, 0);        // Zero size
        // string_to_int_pure(data, 1);        // Single byte
        // string_to_int_pure(data, size);     // Full size
        // string_to_int_pure(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for string_to_int_pure
