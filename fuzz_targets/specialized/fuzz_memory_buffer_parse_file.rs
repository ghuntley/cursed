...
// Targets: parse_file in src/ffi/header_parser.rs:162
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // parse_file(data, 0);        // Zero size
        // parse_file(data, 1);        // Single byte
        // parse_file(data, size);     // Full size
        // parse_file(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for parse_file
