...
// Targets: test_allocator_stress in tests/gc_safety_basic.rs:136
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // test_allocator_stress(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // test_allocator_stress(data, 0);        // Zero size
        // test_allocator_stress(data, 1);        // Single byte
        // test_allocator_stress(data, size);     // Full size
        // test_allocator_stress(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for test_allocator_stress
