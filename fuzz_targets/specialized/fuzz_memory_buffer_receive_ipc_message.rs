...
// Targets: receive_ipc_message in src/runtime/process.rs:524
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // receive_ipc_message(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // receive_ipc_message(data, 0);        // Zero size
        // receive_ipc_message(data, 1);        // Single byte
        // receive_ipc_message(data, size);     // Full size
        // receive_ipc_message(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for receive_ipc_message
