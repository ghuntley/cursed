...
// Targets: remove_named_root in src/memory/roots.rs:87
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // remove_named_root(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // remove_named_root(data, 0);        // Zero size
        // remove_named_root(data, 1);        // Single byte
        // remove_named_root(data, size);     // Full size
        // remove_named_root(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for remove_named_root
