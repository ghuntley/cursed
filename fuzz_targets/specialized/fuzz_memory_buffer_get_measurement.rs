...
// Targets: get_measurement in src/optimization/performance_monitor.rs:51
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // get_measurement(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // get_measurement(data, 0);        // Zero size
        // get_measurement(data, 1);        // Single byte
        // get_measurement(data, size);     // Full size
        // get_measurement(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for get_measurement
