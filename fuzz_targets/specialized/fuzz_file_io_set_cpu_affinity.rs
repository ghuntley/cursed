...
// Targets: set_cpu_affinity in src/runtime/pal/x86_64.rs:1986
...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    // Test set_cpu_affinity with temp file
    // Example: set_cpu_affinity(temp_path);
    // Example: set_cpu_affinity(temp_path, data, size);
...
void test_path_operations(const uint8_t *data, size_t size) {
    ...
    // set_cpu_affinity(path);
}


// Additional test functions for set_cpu_affinity
