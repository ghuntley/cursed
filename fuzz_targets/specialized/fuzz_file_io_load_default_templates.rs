...
// Targets: load_default_templates in src/performance/visualization.rs:146
...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    // Test load_default_templates with temp file
    // Example: load_default_templates(temp_path);
    // Example: load_default_templates(temp_path, data, size);
...
void test_path_operations(const uint8_t *data, size_t size) {
    ...
    // load_default_templates(path);
}


// Additional test functions for load_default_templates
