...
// Targets: gen_non_empty_string in stdlib/property_test/mod_complex.csd:123
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // gen_non_empty_string(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // gen_non_empty_string(data, 0);        // Zero size
        // gen_non_empty_string(data, 1);        // Single byte
        // gen_non_empty_string(data, size);     // Full size
        // gen_non_empty_string(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for gen_non_empty_string
