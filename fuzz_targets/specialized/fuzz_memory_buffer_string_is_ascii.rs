...
// Targets: string_is_ascii in stdlib/string_simple/mod.csd:875
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // string_is_ascii(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // string_is_ascii(data, 0);        // Zero size
        // string_is_ascii(data, 1);        // Single byte
        // string_is_ascii(data, size);     // Full size
        // string_is_ascii(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for string_is_ascii
