...
// Targets: handle_error in src/runtime/production_error.rs:158
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // handle_error(data, 0);        // Zero size
        // handle_error(data, 1);        // Single byte
        // handle_error(data, size);     // Full size
        // handle_error(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for handle_error
