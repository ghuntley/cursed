...
// Targets: try_fold_instruction in src/codegen/llvm/passes/constant_propagation.rs:121
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // try_fold_instruction(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // try_fold_instruction(data, 0);        // Zero size
        // try_fold_instruction(data, 1);        // Single byte
        // try_fold_instruction(data, size);     // Full size
        // try_fold_instruction(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for try_fold_instruction
