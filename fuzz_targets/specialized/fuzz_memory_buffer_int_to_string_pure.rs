...
// Targets: int_to_string_pure in stdlib/self_hosted_builtins/mod.csd:192
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // int_to_string_pure(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // int_to_string_pure(data, 0);        // Zero size
        // int_to_string_pure(data, 1);        // Single byte
        // int_to_string_pure(data, size);     // Full size
        // int_to_string_pure(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for int_to_string_pure
