...
// Targets: optimize_project in src/optimization/performance_integration.rs:112
...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    // Test optimize_project with temp file
    // Example: optimize_project(temp_path);
    // Example: optimize_project(temp_path, data, size);
...
void test_path_operations(const uint8_t *data, size_t size) {
    ...
    // optimize_project(path);
}


// Additional test functions for optimize_project
