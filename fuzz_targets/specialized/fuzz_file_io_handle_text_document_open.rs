...
// Targets: handle_text_document_open in tools/lsp/lsp_server.csd:118
...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    // Test handle_text_document_open with temp file
    // Example: handle_text_document_open(temp_path);
    // Example: handle_text_document_open(temp_path, data, size);
...
void test_path_operations(const uint8_t *data, size_t size) {
    ...
    // handle_text_document_open(path);
}


// Additional test functions for handle_text_document_open
