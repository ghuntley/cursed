...
// Targets: compile_ir_to_wasm in src/lib.rs:1969
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // compile_ir_to_wasm(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // compile_ir_to_wasm(data, 0);        // Zero size
        // compile_ir_to_wasm(data, 1);        // Single byte
        // compile_ir_to_wasm(data, size);     // Full size
        // compile_ir_to_wasm(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for compile_ir_to_wasm
