...
// Targets: validate_string in src/security/input_validation.rs:51
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // validate_string(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // validate_string(data, 0);        // Zero size
        // validate_string(data, 1);        // Single byte
        // validate_string(data, size);     // Full size
        // validate_string(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for validate_string
