...
// Targets: benchmark_memory_usage in stdlib/collections/performance_benchmarks.csd:320
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // benchmark_memory_usage(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // benchmark_memory_usage(data, 0);        // Zero size
        // benchmark_memory_usage(data, 1);        // Single byte
        // benchmark_memory_usage(data, size);     // Full size
        // benchmark_memory_usage(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for benchmark_memory_usage
