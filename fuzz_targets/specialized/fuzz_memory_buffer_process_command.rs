...
// Targets: process_command in src/bin/cursed_debug.rs:230
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // process_command(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // process_command(data, 0);        // Zero size
        // process_command(data, 1);        // Single byte
        // process_command(data, size);     // Full size
        // process_command(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for process_command
