...
// Targets: simplifyMultiplication in src-zig/constant_folder.zig:481
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // simplifyMultiplication(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // simplifyMultiplication(data, 0);        // Zero size
        // simplifyMultiplication(data, 1);        // Single byte
        // simplifyMultiplication(data, size);     // Full size
        // simplifyMultiplication(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for simplifyMultiplication
