...
// Targets: has_capability in src/runtime/pal/common.rs:363
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // has_capability(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // has_capability(data, 0);        // Zero size
        // has_capability(data, 1);        // Single byte
        // has_capability(data, size);     // Full size
        // has_capability(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for has_capability
