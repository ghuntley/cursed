...
// Targets: analyze_document in src/lsp/diagnostics.rs:25
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // analyze_document(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // analyze_document(data, 0);        // Zero size
        // analyze_document(data, 1);        // Single byte
        // analyze_document(data, size);     // Full size
        // analyze_document(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for analyze_document
