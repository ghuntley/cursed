...
// Targets: initialize in src/tools/debug_info.rs:108
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // initialize(data, 0);        // Zero size
        // initialize(data, 1);        // Single byte
        // initialize(data, size);     // Full size
        // initialize(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for initialize
