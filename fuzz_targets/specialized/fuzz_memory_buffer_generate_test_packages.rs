...
// Targets: generate_test_packages in tests/package_manager_integration_test.rs:139
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // generate_test_packages(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // generate_test_packages(data, 0);        // Zero size
        // generate_test_packages(data, 1);        // Single byte
        // generate_test_packages(data, size);     // Full size
        // generate_test_packages(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for generate_test_packages
