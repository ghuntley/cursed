...
// Targets: memory_cleanup_demo in examples/vibecheck_demo.csd:253
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // memory_cleanup_demo(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // memory_cleanup_demo(data, 0);        // Zero size
        // memory_cleanup_demo(data, 1);        // Single byte
        // memory_cleanup_demo(data, size);     // Full size
        // memory_cleanup_demo(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for memory_cleanup_demo
