...
// Targets: parse_results in src/debug/gdb_integration.rs:850
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // parse_results(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // parse_results(data, 0);        // Zero size
        // parse_results(data, 1);        // Single byte
        // parse_results(data, size);     // Full size
        // parse_results(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for parse_results
