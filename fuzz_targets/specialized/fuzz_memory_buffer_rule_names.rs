...
// Targets: rule_names in src/formatter/rules.rs:497
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // rule_names(data, 0);        // Zero size
        // rule_names(data, 1);        // Single byte
        // rule_names(data, size);     // Full size
        // rule_names(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for rule_names
