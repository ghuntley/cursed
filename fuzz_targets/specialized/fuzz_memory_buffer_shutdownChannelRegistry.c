...
// Targets: shutdownChannelRegistry in src-zig/concurrency_old.zig:931
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // shutdownChannelRegistry(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // shutdownChannelRegistry(data, 0);        // Zero size
        // shutdownChannelRegistry(data, 1);        // Single byte
        // shutdownChannelRegistry(data, size);     // Full size
        // shutdownChannelRegistry(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for shutdownChannelRegistry
