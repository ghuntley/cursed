...
// Targets: is_cross_generational_write in src/runtime/gc/barrier.rs:349
...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    // Test is_cross_generational_write with temp file
    // Example: is_cross_generational_write(temp_path);
    // Example: is_cross_generational_write(temp_path, data, size);
...
void test_path_operations(const uint8_t *data, size_t size) {
    ...
    // is_cross_generational_write(path);
}


// Additional test functions for is_cross_generational_write
