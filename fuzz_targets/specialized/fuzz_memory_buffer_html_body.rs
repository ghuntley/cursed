...
// Targets: html_body in src/stdlib/net/protocols/smtp.rs:87
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // html_body(data, 0);        // Zero size
        // html_body(data, 1);        // Single byte
        // html_body(data, size);     // Full size
        // html_body(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for html_body
