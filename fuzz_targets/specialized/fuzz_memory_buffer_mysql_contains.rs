...
// Targets: mysql_contains in stdlib/database_drivers/mysql.csd:336
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // mysql_contains(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // mysql_contains(data, 0);        // Zero size
        // mysql_contains(data, 1);        // Single byte
        // mysql_contains(data, size);     // Full size
        // mysql_contains(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for mysql_contains
