...
// Targets: string_upper in stdlib/template_engine/mod.csd:433
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // string_upper(data, 0);        // Zero size
        // string_upper(data, 1);        // Single byte
        // string_upper(data, size);     // Full size
        // string_upper(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for string_upper
