...
// Targets: parseVersion in stdlib/package_manager/mod.csd:75
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // parseVersion(data, 0);        // Zero size
        // parseVersion(data, 1);        // Single byte
        // parseVersion(data, size);     // Full size
        // parseVersion(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for parseVersion
