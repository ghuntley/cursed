...
// Targets: bench_string_concatenation in examples/test_example.csd:197
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // bench_string_concatenation(buffer, data, size);
...
void test_string_operations(const uint8_t *data, size_t size) {
    ...
    // bench_string_concatenation(str);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // bench_string_concatenation(data, 0);        // Zero size
        // bench_string_concatenation(data, 1);        // Single byte
        // bench_string_concatenation(data, size);     // Full size
        // bench_string_concatenation(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for bench_string_concatenation
