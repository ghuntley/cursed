...
// Targets: example_string_concatenation in stdlib/property_test/examples.csd:62
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // example_string_concatenation(buffer, data, size);
...
void test_string_operations(const uint8_t *data, size_t size) {
    ...
    // example_string_concatenation(str);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // example_string_concatenation(data, 0);        // Zero size
        // example_string_concatenation(data, 1);        // Single byte
        // example_string_concatenation(data, size);     // Full size
        // example_string_concatenation(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for example_string_concatenation
