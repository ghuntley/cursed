...
// Targets: max_string_length in src/security/input_validation.rs:29
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // max_string_length(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // max_string_length(data, 0);        // Zero size
        // max_string_length(data, 1);        // Single byte
        // max_string_length(data, size);     // Full size
        // max_string_length(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for max_string_length
