...
// Targets: demonstrate_statistics in examples/math_comprehensive_demo.csd:179
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // demonstrate_statistics(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // demonstrate_statistics(data, 0);        // Zero size
        // demonstrate_statistics(data, 1);        // Single byte
        // demonstrate_statistics(data, size);     // Full size
        // demonstrate_statistics(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for demonstrate_statistics
