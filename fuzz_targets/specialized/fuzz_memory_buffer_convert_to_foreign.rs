...
// Targets: convert_to_foreign in src/ffi/type_mapping.rs:57
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // convert_to_foreign(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // convert_to_foreign(data, 0);        // Zero size
        // convert_to_foreign(data, 1);        // Single byte
        // convert_to_foreign(data, size);     // Full size
        // convert_to_foreign(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for convert_to_foreign
