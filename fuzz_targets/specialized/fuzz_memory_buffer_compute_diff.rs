...
// Targets: compute_diff in src/formatter/output.rs:84
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // compute_diff(data, 0);        // Zero size
        // compute_diff(data, 1);        // Single byte
        // compute_diff(data, size);     // Full size
        // compute_diff(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for compute_diff
