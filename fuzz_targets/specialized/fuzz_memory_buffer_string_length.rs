...
// Targets: string_length in stdlib/compression/mod.csd:92
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // string_length(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // string_length(data, 0);        // Zero size
        // string_length(data, 1);        // Single byte
        // string_length(data, size);     // Full size
        // string_length(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for string_length
