...
// Targets: automatic_error_handling in examples/question_mark_demo.csd:83
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // automatic_error_handling(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // automatic_error_handling(data, 0);        // Zero size
        // automatic_error_handling(data, 1);        // Single byte
        // automatic_error_handling(data, size);     // Full size
        // automatic_error_handling(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for automatic_error_handling
