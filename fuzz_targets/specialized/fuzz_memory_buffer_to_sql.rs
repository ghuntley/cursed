...
// Targets: to_sql in src/security/database_secure.rs:247
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // to_sql(data, 0);        // Zero size
        // to_sql(data, 1);        // Single byte
        // to_sql(data, size);     // Full size
        // to_sql(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for to_sql
