...
// Targets: demonstrate_unified_api in examples/crypto_asymmetric_demo.csd:244
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // demonstrate_unified_api(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // demonstrate_unified_api(data, 0);        // Zero size
        // demonstrate_unified_api(data, 1);        // Single byte
        // demonstrate_unified_api(data, size);     // Full size
        // demonstrate_unified_api(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for demonstrate_unified_api
