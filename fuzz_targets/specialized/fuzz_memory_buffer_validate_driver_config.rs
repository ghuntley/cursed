...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // validate_driver_config(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // validate_driver_config(data, 0);        // Zero size
        // validate_driver_config(data, 1);        // Single byte
        // validate_driver_config(data, size);     // Full size
        // validate_driver_config(NULL, 0);        // NULL pointer
    }
}
