...
// Targets: string_equals in stdlib/collections_core/mod.csd:726
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // string_equals(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // string_equals(data, 0);        // Zero size
        // string_equals(data, 1);        // Single byte
        // string_equals(data, size);     // Full size
        // string_equals(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for string_equals
