...
// Targets: string_before in stdlib/configz/mod_complex.csd:707
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // string_before(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // string_before(data, 0);        // Zero size
        // string_before(data, 1);        // Single byte
        // string_before(data, size);     // Full size
        // string_before(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for string_before
