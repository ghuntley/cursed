...
// Targets: get_rule_category in src/tools/linter.rs:548
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // get_rule_category(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // get_rule_category(data, 0);        // Zero size
        // get_rule_category(data, 1);        // Single byte
        // get_rule_category(data, size);     // Full size
        // get_rule_category(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for get_rule_category
