...
// Targets: trace_call in src/ffi/debug_tools.rs:384
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // trace_call(data, 0);        // Zero size
        // trace_call(data, 1);        // Single byte
        // trace_call(data, size);     // Full size
        // trace_call(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for trace_call
