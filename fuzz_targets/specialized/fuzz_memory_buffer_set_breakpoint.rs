...
// Targets: set_breakpoint in src/bin/cursed_debug.rs:384
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // set_breakpoint(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // set_breakpoint(data, 0);        // Zero size
        // set_breakpoint(data, 1);        // Single byte
        // set_breakpoint(data, size);     // Full size
        // set_breakpoint(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for set_breakpoint
