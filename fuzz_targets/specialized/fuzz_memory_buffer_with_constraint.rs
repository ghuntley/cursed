...
// Targets: with_constraint in src/stdlib/database/orm/relationships.rs:351
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // with_constraint(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // with_constraint(data, 0);        // Zero size
        // with_constraint(data, 1);        // Single byte
        // with_constraint(data, size);     // Full size
        // with_constraint(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for with_constraint
