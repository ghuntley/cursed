...
// Targets: with_relationship in src/stdlib/database/orm/relationships.rs:346
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // with_relationship(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // with_relationship(data, 0);        // Zero size
        // with_relationship(data, 1);        // Single byte
        // with_relationship(data, size);     // Full size
        // with_relationship(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for with_relationship
