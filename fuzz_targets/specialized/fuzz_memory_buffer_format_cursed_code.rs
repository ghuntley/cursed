...
// Targets: format_cursed_code in src/lsp/minimal_server.rs:160
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // format_cursed_code(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // format_cursed_code(data, 0);        // Zero size
        // format_cursed_code(data, 1);        // Single byte
        // format_cursed_code(data, size);     // Full size
        // format_cursed_code(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for format_cursed_code
