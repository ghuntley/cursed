...
// Targets: string_after in stdlib/configz/mod_complex.csd:721
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // string_after(data, 0);        // Zero size
        // string_after(data, 1);        // Single byte
        // string_after(data, size);     // Full size
        // string_after(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for string_after
