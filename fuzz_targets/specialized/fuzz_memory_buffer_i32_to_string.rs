...
// Targets: i32_to_string in src/execution/runtime_functions.rs:3837
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // i32_to_string(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // i32_to_string(data, 0);        // Zero size
        // i32_to_string(data, 1);        // Single byte
        // i32_to_string(data, size);     // Full size
        // i32_to_string(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for i32_to_string
