...
// Targets: build_call_graph in src/optimization/pgo/optimization_engine.rs:411
...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    // Test build_call_graph with temp file
    // Example: build_call_graph(temp_path);
    // Example: build_call_graph(temp_path, data, size);
...
void test_path_operations(const uint8_t *data, size_t size) {
    ...
    // build_call_graph(path);
}


// Additional test functions for build_call_graph
