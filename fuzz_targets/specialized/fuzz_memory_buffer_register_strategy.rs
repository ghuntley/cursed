...
// Targets: register_strategy in src/runtime/error_propagation.rs:333
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // register_strategy(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // register_strategy(data, 0);        // Zero size
        // register_strategy(data, 1);        // Single byte
        // register_strategy(data, size);     // Full size
        // register_strategy(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for register_strategy
