...
// Targets: extract_example_description in src/tools/doc_generator.rs:467
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // extract_example_description(buffer, data, size);
...
void test_string_operations(const uint8_t *data, size_t size) {
    ...
    // extract_example_description(str);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // extract_example_description(data, 0);        // Zero size
        // extract_example_description(data, 1);        // Single byte
        // extract_example_description(data, size);     // Full size
        // extract_example_description(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for extract_example_description
