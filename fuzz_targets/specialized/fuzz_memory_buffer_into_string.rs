...
// Targets: into_string in src/stdlib/string/mod.rs:55
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // into_string(data, 0);        // Zero size
        // into_string(data, 1);        // Single byte
        // into_string(data, size);     // Full size
        // into_string(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for into_string
