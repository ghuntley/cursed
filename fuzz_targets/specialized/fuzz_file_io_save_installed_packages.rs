...
// Targets: save_installed_packages in src/package_manager/installer.rs:453
...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    // Test save_installed_packages with temp file
    // Example: save_installed_packages(temp_path);
    // Example: save_installed_packages(temp_path, data, size);
...
void test_path_operations(const uint8_t *data, size_t size) {
    ...
    // save_installed_packages(path);
}


// Additional test functions for save_installed_packages
