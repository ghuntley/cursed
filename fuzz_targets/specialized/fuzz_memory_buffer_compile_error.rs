...
// Targets: compile_error in src/error/structured.rs:304
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // compile_error(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // compile_error(data, 0);        // Zero size
        // compile_error(data, 1);        // Single byte
        // compile_error(data, size);     // Full size
        // compile_error(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for compile_error
