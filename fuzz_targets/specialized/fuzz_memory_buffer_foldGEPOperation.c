...
// Targets: foldGEPOperation in src-zig/constant_folder.zig:376
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // foldGEPOperation(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // foldGEPOperation(data, 0);        // Zero size
        // foldGEPOperation(data, 1);        // Single byte
        // foldGEPOperation(data, size);     // Full size
        // foldGEPOperation(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for foldGEPOperation
