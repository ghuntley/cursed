...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    // Test packet parsing
    // Test create_tcp_listener with network data
    // Example: create_tcp_listener(data, size);
    // Example: create_tcp_listener(sockfd, data, size);
...
void test_socket_operations(const uint8_t *data, size_t size) {
    ...
    // create_tcp_listener(sockfd, data, size);
