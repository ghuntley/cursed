...
// Targets: isValidStreamId in src-zig/http2_network_integration.zig:509
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // isValidStreamId(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // isValidStreamId(data, 0);        // Zero size
        // isValidStreamId(data, 1);        // Single byte
        // isValidStreamId(data, size);     // Full size
        // isValidStreamId(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for isValidStreamId
