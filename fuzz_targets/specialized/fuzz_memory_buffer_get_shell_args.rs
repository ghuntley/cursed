...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // get_shell_args(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // get_shell_args(data, 0);        // Zero size
        // get_shell_args(data, 1);        // Single byte
        // get_shell_args(data, size);     // Full size
        // get_shell_args(NULL, 0);        // NULL pointer
    }
}
