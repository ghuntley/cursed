...
// Targets: array_length_str in stdlib/sorta_fresh/mod.csd:622
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // array_length_str(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // array_length_str(data, 0);        // Zero size
        // array_length_str(data, 1);        // Single byte
        // array_length_str(data, size);     // Full size
        // array_length_str(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for array_length_str
