...
// Targets: string_right in stdlib/string_simple/mod.csd:290
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // string_right(data, 0);        // Zero size
        // string_right(data, 1);        // Single byte
        // string_right(data, size);     // Full size
        // string_right(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for string_right
