...
// Targets: check_mutable_borrow in src/type_system/checker.rs:2548
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // check_mutable_borrow(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // check_mutable_borrow(data, 0);        // Zero size
        // check_mutable_borrow(data, 1);        // Single byte
        // check_mutable_borrow(data, size);     // Full size
        // check_mutable_borrow(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for check_mutable_borrow
