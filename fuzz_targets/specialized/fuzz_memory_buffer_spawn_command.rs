...
// Targets: spawn_command in src/runtime/process.rs:610
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // spawn_command(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // spawn_command(data, 0);        // Zero size
        // spawn_command(data, 1);        // Single byte
        // spawn_command(data, size);     // Full size
        // spawn_command(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for spawn_command
