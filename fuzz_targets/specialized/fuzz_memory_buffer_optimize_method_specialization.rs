...
// Targets: optimize_method_specialization in src/codegen/llvm/generic_optimization.rs:384
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // optimize_method_specialization(buffer, data, size);
...
void test_string_operations(const uint8_t *data, size_t size) {
    ...
    // optimize_method_specialization(str);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // optimize_method_specialization(data, 0);        // Zero size
        // optimize_method_specialization(data, 1);        // Single byte
        // optimize_method_specialization(data, size);     // Full size
        // optimize_method_specialization(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for optimize_method_specialization
