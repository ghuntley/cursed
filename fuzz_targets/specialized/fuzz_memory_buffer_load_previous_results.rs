...
// Targets: load_previous_results in src/bin/cursed_bench.rs:215
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // load_previous_results(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // load_previous_results(data, 0);        // Zero size
        // load_previous_results(data, 1);        // Single byte
        // load_previous_results(data, size);     // Full size
        // load_previous_results(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for load_previous_results
