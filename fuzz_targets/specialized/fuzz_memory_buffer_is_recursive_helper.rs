...
// Targets: is_recursive_helper in src/codegen/llvm/passes/inlining.rs:1094
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // is_recursive_helper(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // is_recursive_helper(data, 0);        // Zero size
        // is_recursive_helper(data, 1);        // Single byte
        // is_recursive_helper(data, size);     // Full size
        // is_recursive_helper(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for is_recursive_helper
