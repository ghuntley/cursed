...
// Targets: instructionHasSideEffects in src-zig/loop_optimizer.zig:665
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // instructionHasSideEffects(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // instructionHasSideEffects(data, 0);        // Zero size
        // instructionHasSideEffects(data, 1);        // Single byte
        // instructionHasSideEffects(data, size);     // Full size
        // instructionHasSideEffects(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for instructionHasSideEffects
