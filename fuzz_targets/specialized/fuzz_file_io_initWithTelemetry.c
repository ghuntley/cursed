...
// Targets: initWithTelemetry in src-zig/parser.zig:117
...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    // Test initWithTelemetry with temp file
    // Example: initWithTelemetry(temp_path);
    // Example: initWithTelemetry(temp_path, data, size);
...
void test_path_operations(const uint8_t *data, size_t size) {
    ...
    // initWithTelemetry(path);
}


// Additional test functions for initWithTelemetry
