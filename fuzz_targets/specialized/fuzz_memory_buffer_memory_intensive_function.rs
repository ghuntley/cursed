...
// Targets: memory_intensive_function in performance_hooks_test.csd:31
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // memory_intensive_function(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // memory_intensive_function(data, 0);        // Zero size
        // memory_intensive_function(data, 1);        // Single byte
        // memory_intensive_function(data, size);     // Full size
        // memory_intensive_function(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for memory_intensive_function
