...
// Targets: compile_with_full_recovery in src/error_recovery_simple.rs:177
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // compile_with_full_recovery(buffer, data, size);
...
void test_string_operations(const uint8_t *data, size_t size) {
    ...
    // compile_with_full_recovery(str);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // compile_with_full_recovery(data, 0);        // Zero size
        // compile_with_full_recovery(data, 1);        // Single byte
        // compile_with_full_recovery(data, size);     // Full size
        // compile_with_full_recovery(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for compile_with_full_recovery
