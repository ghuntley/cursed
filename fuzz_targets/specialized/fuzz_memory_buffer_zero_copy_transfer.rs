...
// Targets: zero_copy_transfer in src/ffi/performance.rs:400
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // zero_copy_transfer(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // zero_copy_transfer(data, 0);        // Zero size
        // zero_copy_transfer(data, 1);        // Single byte
        // zero_copy_transfer(data, size);     // Full size
        // zero_copy_transfer(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for zero_copy_transfer
