...
// Targets: delete_profile in src/optimization/pgo/profile_storage.rs:162
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // delete_profile(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // delete_profile(data, 0);        // Zero size
        // delete_profile(data, 1);        // Single byte
        // delete_profile(data, size);     // Full size
        // delete_profile(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for delete_profile
