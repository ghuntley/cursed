...
// Targets: glob_match in src/coverage/simple_collector.rs:424
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // glob_match(data, 0);        // Zero size
        // glob_match(data, 1);        // Single byte
        // glob_match(data, size);     // Full size
        // glob_match(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for glob_match
