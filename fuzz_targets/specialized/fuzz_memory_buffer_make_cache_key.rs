...
// Targets: make_cache_key in src/package_manager/cache.rs:307
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // make_cache_key(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // make_cache_key(data, 0);        // Zero size
        // make_cache_key(data, 1);        // Single byte
        // make_cache_key(data, size);     // Full size
        // make_cache_key(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for make_cache_key
