...
// Targets: check_variable_shadowing in src/tools/linter.rs:1409
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // check_variable_shadowing(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // check_variable_shadowing(data, 0);        // Zero size
        // check_variable_shadowing(data, 1);        // Single byte
        // check_variable_shadowing(data, size);     // Full size
        // check_variable_shadowing(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for check_variable_shadowing
