...
// Targets: test_object_registration in src/memory/mark_sweep.rs:997
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // test_object_registration(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // test_object_registration(data, 0);        // Zero size
        // test_object_registration(data, 1);        // Single byte
        // test_object_registration(data, size);     // Full size
        // test_object_registration(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for test_object_registration
