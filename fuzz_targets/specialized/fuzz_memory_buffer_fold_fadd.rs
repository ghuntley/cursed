...
// Targets: fold_fadd in src/codegen/llvm/passes/constant_propagation.rs:740
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // fold_fadd(data, 0);        // Zero size
        // fold_fadd(data, 1);        // Single byte
        // fold_fadd(data, size);     // Full size
        // fold_fadd(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for fold_fadd
