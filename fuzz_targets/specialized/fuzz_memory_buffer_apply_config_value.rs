...
// Targets: apply_config_value in examples/type_switch_practical_use_cases.csd:120
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // apply_config_value(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // apply_config_value(data, 0);        // Zero size
        // apply_config_value(data, 1);        // Single byte
        // apply_config_value(data, size);     // Full size
        // apply_config_value(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for apply_config_value
