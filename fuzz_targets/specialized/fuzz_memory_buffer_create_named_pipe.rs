...
// Targets: create_named_pipe in stdlib/ipc/mod_simple.csd:32
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // create_named_pipe(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // create_named_pipe(data, 0);        // Zero size
        // create_named_pipe(data, 1);        // Single byte
        // create_named_pipe(data, size);     // Full size
        // create_named_pipe(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for create_named_pipe
