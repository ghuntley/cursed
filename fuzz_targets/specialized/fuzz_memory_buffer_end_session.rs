...
// Targets: end_session in src/runtime/debug_runtime.rs:291
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // end_session(data, 0);        // Zero size
        // end_session(data, 1);        // Single byte
        // end_session(data, size);     // Full size
        // end_session(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for end_session
