...
// Targets: char_at in stdlib/encoding_flex/mod_enhanced.csd:455
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // char_at(data, 0);        // Zero size
        // char_at(data, 1);        // Single byte
        // char_at(data, size);     // Full size
        // char_at(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for char_at
