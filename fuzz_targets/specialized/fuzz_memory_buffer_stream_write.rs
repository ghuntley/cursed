...
// Targets: stream_write in stdlib/io/mod_comprehensive.csd:1041
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // stream_write(data, 0);        // Zero size
        // stream_write(data, 1);        // Single byte
        // stream_write(data, size);     // Full size
        // stream_write(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for stream_write
