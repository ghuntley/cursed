...
// Targets: add_watch in src/ffi/debug_tools.rs:495
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // add_watch(data, 0);        // Zero size
        // add_watch(data, 1);        // Single byte
        // add_watch(data, size);     // Full size
        // add_watch(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for add_watch
