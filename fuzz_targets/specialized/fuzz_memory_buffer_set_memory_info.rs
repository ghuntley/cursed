...
// Targets: set_memory_info in src/runtime/debug_runtime.rs:1219
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // set_memory_info(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // set_memory_info(data, 0);        // Zero size
        // set_memory_info(data, 1);        // Single byte
        // set_memory_info(data, size);     // Full size
        // set_memory_info(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for set_memory_info
