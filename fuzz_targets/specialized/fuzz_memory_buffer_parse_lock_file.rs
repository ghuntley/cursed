...
// Targets: parse_lock_file in src/package_manager/lock_file.rs:52
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // parse_lock_file(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // parse_lock_file(data, 0);        // Zero size
        // parse_lock_file(data, 1);        // Single byte
        // parse_lock_file(data, size);     // Full size
        // parse_lock_file(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for parse_lock_file
