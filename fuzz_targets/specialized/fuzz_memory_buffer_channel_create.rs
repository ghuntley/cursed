...
// Targets: channel_create in stdlib/select_core/mod.csd:164
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // channel_create(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // channel_create(data, 0);        // Zero size
        // channel_create(data, 1);        // Single byte
        // channel_create(data, size);     // Full size
        // channel_create(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for channel_create
