...
// Targets: handle_multiline_command in src/repl/enhanced_cursed_repl.rs:527
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // handle_multiline_command(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // handle_multiline_command(data, 0);        // Zero size
        // handle_multiline_command(data, 1);        // Single byte
        // handle_multiline_command(data, size);     // Full size
        // handle_multiline_command(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for handle_multiline_command
