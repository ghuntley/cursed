...
// Targets: list_source in src/bin/cursed_debug.rs:577
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // list_source(data, 0);        // Zero size
        // list_source(data, 1);        // Single byte
        // list_source(data, size);     // Full size
        // list_source(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for list_source
