...
// Targets: squish_stream_decompress in stdlib/squish_core/mod.csd:134
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // squish_stream_decompress(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // squish_stream_decompress(data, 0);        // Zero size
        // squish_stream_decompress(data, 1);        // Single byte
        // squish_stream_decompress(data, size);     // Full size
        // squish_stream_decompress(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for squish_stream_decompress
