...
// Targets: verify_checksum in src/package_manager/downloader.rs:195
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // verify_checksum(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // verify_checksum(data, 0);        // Zero size
        // verify_checksum(data, 1);        // Single byte
        // verify_checksum(data, size);     // Full size
        // verify_checksum(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for verify_checksum
