...
// Targets: string_of in src/stdlib/packages/quick_test/mod.rs:896
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // string_of(data, 0);        // Zero size
        // string_of(data, 1);        // Single byte
        // string_of(data, size);     // Full size
        // string_of(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for string_of
