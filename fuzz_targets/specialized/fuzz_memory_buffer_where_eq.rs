...
// Targets: where_eq in src/security/database_secure.rs:316
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // where_eq(data, 0);        // Zero size
        // where_eq(data, 1);        // Single byte
        // where_eq(data, size);     // Full size
        // where_eq(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for where_eq
