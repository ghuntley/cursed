...
// Targets: runtime_string_builder_free in stdlib/string_simple/mod.csd:867
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // runtime_string_builder_free(buffer, data, size);
...
void test_string_operations(const uint8_t *data, size_t size) {
    ...
    // runtime_string_builder_free(str);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // runtime_string_builder_free(data, 0);        // Zero size
        // runtime_string_builder_free(data, 1);        // Single byte
        // runtime_string_builder_free(data, size);     // Full size
        // runtime_string_builder_free(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for runtime_string_builder_free
