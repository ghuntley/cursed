...
// Targets: create_generic_type in src/types/full.rs:468
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // create_generic_type(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // create_generic_type(data, 0);        // Zero size
        // create_generic_type(data, 1);        // Single byte
        // create_generic_type(data, size);     // Full size
        // create_generic_type(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for create_generic_type
