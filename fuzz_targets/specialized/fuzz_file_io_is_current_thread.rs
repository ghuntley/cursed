...
// Targets: is_current_thread in src/runtime/heap_optimizer_safe.rs:274
...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    // Test is_current_thread with temp file
    // Example: is_current_thread(temp_path);
    // Example: is_current_thread(temp_path, data, size);
...
void test_path_operations(const uint8_t *data, size_t size) {
    ...
    // is_current_thread(path);
}


// Additional test functions for is_current_thread
