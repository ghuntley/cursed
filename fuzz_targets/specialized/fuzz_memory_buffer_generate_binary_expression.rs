...
// Targets: generate_binary_expression in src/codegen/llvm/main.rs:1957
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // generate_binary_expression(buffer, data, size);
...
void test_string_operations(const uint8_t *data, size_t size) {
    ...
    // generate_binary_expression(str);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // generate_binary_expression(data, 0);        // Zero size
        // generate_binary_expression(data, 1);        // Single byte
        // generate_binary_expression(data, size);     // Full size
        // generate_binary_expression(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for generate_binary_expression
