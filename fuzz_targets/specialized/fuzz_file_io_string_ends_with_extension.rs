...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    // Test string_ends_with_extension with temp file
    // Example: string_ends_with_extension(temp_path);
    // Example: string_ends_with_extension(temp_path, data, size);
...
void test_path_operations(const uint8_t *data, size_t size) {
    ...
    // string_ends_with_extension(path);
}
