...
// Targets: allocate_string in variadic_ffi_test.csd:216
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // allocate_string(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // allocate_string(data, 0);        // Zero size
        // allocate_string(data, 1);        // Single byte
        // allocate_string(data, size);     // Full size
        // allocate_string(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for allocate_string
