...
// Targets: generate_shook_propagation in src/codegen/llvm/error_runtime_codegen.rs:97
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // generate_shook_propagation(buffer, data, size);
...
void test_string_operations(const uint8_t *data, size_t size) {
    ...
    // generate_shook_propagation(str);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // generate_shook_propagation(data, 0);        // Zero size
        // generate_shook_propagation(data, 1);        // Single byte
        // generate_shook_propagation(data, size);     // Full size
        // generate_shook_propagation(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for generate_shook_propagation
