...
// Targets: test_thread_info in src/debug/tests.rs:278
...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    // Test test_thread_info with temp file
    // Example: test_thread_info(temp_path);
    // Example: test_thread_info(temp_path, data, size);
...
void test_path_operations(const uint8_t *data, size_t size) {
    ...
    // test_thread_info(path);
}


// Additional test functions for test_thread_info
