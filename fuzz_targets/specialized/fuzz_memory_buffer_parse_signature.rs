...
// Targets: parse_signature in src/bin/advanced_signature_demo.rs:86
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // parse_signature(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // parse_signature(data, 0);        // Zero size
        // parse_signature(data, 1);        // Single byte
        // parse_signature(data, size);     // Full size
        // parse_signature(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for parse_signature
