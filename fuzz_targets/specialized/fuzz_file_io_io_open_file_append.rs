...
// Targets: io_open_file_append in src/execution/runtime_functions.rs:1157
...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    // Test io_open_file_append with temp file
    // Example: io_open_file_append(temp_path);
    // Example: io_open_file_append(temp_path, data, size);
...
void test_path_operations(const uint8_t *data, size_t size) {
    ...
    // io_open_file_append(path);
}


// Additional test functions for io_open_file_append
