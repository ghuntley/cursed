...
// Targets: remove_from_gc in src/runtime/channels/lifecycle.rs:1055
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // remove_from_gc(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // remove_from_gc(data, 0);        // Zero size
        // remove_from_gc(data, 1);        // Single byte
        // remove_from_gc(data, size);     // Full size
        // remove_from_gc(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for remove_from_gc
