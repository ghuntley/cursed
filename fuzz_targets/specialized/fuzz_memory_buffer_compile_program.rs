...
// Targets: compile_program in src/tools/mod.rs:156
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // compile_program(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // compile_program(data, 0);        // Zero size
        // compile_program(data, 1);        // Single byte
        // compile_program(data, size);     // Full size
        // compile_program(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for compile_program
