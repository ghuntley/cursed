...
// Targets: quote_string in src/coverage/instrumentation.rs:334
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // quote_string(data, 0);        // Zero size
        // quote_string(data, 1);        // Single byte
        // quote_string(data, size);     // Full size
        // quote_string(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for quote_string
