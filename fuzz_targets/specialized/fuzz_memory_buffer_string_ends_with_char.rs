...
// Targets: string_ends_with_char in stdlib/configz/mod_complex_syntax.csd:493
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // string_ends_with_char(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // string_ends_with_char(data, 0);        // Zero size
        // string_ends_with_char(data, 1);        // Single byte
        // string_ends_with_char(data, size);     // Full size
        // string_ends_with_char(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for string_ends_with_char
