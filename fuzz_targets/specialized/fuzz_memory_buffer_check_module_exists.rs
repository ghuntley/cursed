...
// Targets: check_module_exists in src/imports/resolver.rs:907
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // check_module_exists(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // check_module_exists(data, 0);        // Zero size
        // check_module_exists(data, 1);        // Single byte
        // check_module_exists(data, size);     // Full size
        // check_module_exists(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for check_module_exists
