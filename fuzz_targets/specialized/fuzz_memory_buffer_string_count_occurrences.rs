...
// Targets: string_count_occurrences in stdlib/string_pure/mod.csd:331
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // string_count_occurrences(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // string_count_occurrences(data, 0);        // Zero size
        // string_count_occurrences(data, 1);        // Single byte
        // string_count_occurrences(data, size);     // Full size
        // string_count_occurrences(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for string_count_occurrences
