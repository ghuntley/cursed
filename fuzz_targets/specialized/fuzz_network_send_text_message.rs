...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    // Test packet parsing
    // Test send_text_message with network data
    // Example: send_text_message(data, size);
    // Example: send_text_message(sockfd, data, size);
...
void test_socket_operations(const uint8_t *data, size_t size) {
    ...
    // send_text_message(sockfd, data, size);
