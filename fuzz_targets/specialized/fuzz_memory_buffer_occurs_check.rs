...
// Targets: occurs_check in src/type_system/constraint_resolver.rs:718
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // occurs_check(data, 0);        // Zero size
        // occurs_check(data, 1);        // Single byte
        // occurs_check(data, size);     // Full size
        // occurs_check(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for occurs_check
