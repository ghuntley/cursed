...
// Targets: test_string_indexing in stdlib/string_pure/test_string_pure.csd:66
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // test_string_indexing(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // test_string_indexing(data, 0);        // Zero size
        // test_string_indexing(data, 1);        // Single byte
        // test_string_indexing(data, size);     // Full size
        // test_string_indexing(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for test_string_indexing
