...
// Targets: get_example in src/documentation/examples.rs:44
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // get_example(data, 0);        // Zero size
        // get_example(data, 1);        // Single byte
        // get_example(data, size);     // Full size
        // get_example(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for get_example
