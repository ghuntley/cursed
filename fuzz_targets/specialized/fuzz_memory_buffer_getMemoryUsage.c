...
// Targets: getMemoryUsage in src-zig/gc.zig:798
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // getMemoryUsage(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // getMemoryUsage(data, 0);        // Zero size
        // getMemoryUsage(data, 1);        // Single byte
        // getMemoryUsage(data, size);     // Full size
        // getMemoryUsage(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for getMemoryUsage
