...
// Targets: is_package_locked in src/package_manager/lock_file.rs:160
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // is_package_locked(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // is_package_locked(data, 0);        // Zero size
        // is_package_locked(data, 1);        // Single byte
        // is_package_locked(data, size);     // Full size
        // is_package_locked(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for is_package_locked
