...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    // Test generate_shook_propagation with temp file
    // Example: generate_shook_propagation(temp_path);
    // Example: generate_shook_propagation(temp_path, data, size);
...
void test_path_operations(const uint8_t *data, size_t size) {
    ...
    // generate_shook_propagation(path);
}
