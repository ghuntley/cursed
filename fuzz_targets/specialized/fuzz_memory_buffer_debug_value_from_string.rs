...
// Targets: debug_value_from_string in src/runtime/debug_output.rs:1029
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // debug_value_from_string(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // debug_value_from_string(data, 0);        // Zero size
        // debug_value_from_string(data, 1);        // Single byte
        // debug_value_from_string(data, size);     // Full size
        // debug_value_from_string(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for debug_value_from_string
