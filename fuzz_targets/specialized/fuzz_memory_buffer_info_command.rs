...
// Targets: info_command in src/bin/cursed_debug.rs:542
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // info_command(data, 0);        // Zero size
        // info_command(data, 1);        // Single byte
        // info_command(data, size);     // Full size
        // info_command(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for info_command
