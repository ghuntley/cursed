...
// Targets: codegen_error in src/error/mod.rs:238
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // codegen_error(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // codegen_error(data, 0);        // Zero size
        // codegen_error(data, 1);        // Single byte
        // codegen_error(data, size);     // Full size
        // codegen_error(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for codegen_error
