...
// Targets: collect_garbage in src/runtime/memory.rs:484
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // collect_garbage(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // collect_garbage(data, 0);        // Zero size
        // collect_garbage(data, 1);        // Single byte
        // collect_garbage(data, size);     // Full size
        // collect_garbage(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for collect_garbage
