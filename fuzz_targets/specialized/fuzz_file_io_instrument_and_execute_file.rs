...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    // Test instrument_and_execute_file with temp file
    // Example: instrument_and_execute_file(temp_path);
    // Example: instrument_and_execute_file(temp_path, data, size);
...
void test_path_operations(const uint8_t *data, size_t size) {
    ...
    // instrument_and_execute_file(path);
}
