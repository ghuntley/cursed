...
// Targets: resolve_program_imports in src/imports/resolver.rs:1031
...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    // Test resolve_program_imports with temp file
    // Example: resolve_program_imports(temp_path);
    // Example: resolve_program_imports(temp_path, data, size);
...
void test_path_operations(const uint8_t *data, size_t size) {
    ...
    // resolve_program_imports(path);
}


// Additional test functions for resolve_program_imports
