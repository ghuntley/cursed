...
// Targets: break_line in src/formatter/rules.rs:140
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // break_line(data, 0);        // Zero size
        // break_line(data, 1);        // Single byte
        // break_line(data, size);     // Full size
        // break_line(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for break_line
