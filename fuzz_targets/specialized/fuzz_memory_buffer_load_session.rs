...
// Targets: load_session in src/repl/session_manager.rs:49
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // load_session(data, 0);        // Zero size
        // load_session(data, 1);        // Single byte
        // load_session(data, size);     // Full size
        // load_session(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for load_session
