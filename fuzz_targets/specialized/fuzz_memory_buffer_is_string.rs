...
// Targets: is_string in stdlib/json_tea/mod.csd:495
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // is_string(data, 0);        // Zero size
        // is_string(data, 1);        // Single byte
        // is_string(data, size);     // Full size
        // is_string(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for is_string
