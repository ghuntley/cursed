...
// Targets: string_char_at_safe in stdlib/jit_vibes/mod.csd:189
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // string_char_at_safe(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // string_char_at_safe(data, 0);        // Zero size
        // string_char_at_safe(data, 1);        // Single byte
        // string_char_at_safe(data, size);     // Full size
        // string_char_at_safe(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for string_char_at_safe
