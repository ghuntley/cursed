...
// Targets: export_debug_data in src/ffi/debug_tools.rs:566
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // export_debug_data(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // export_debug_data(data, 0);        // Zero size
        // export_debug_data(data, 1);        // Single byte
        // export_debug_data(data, size);     // Full size
        // export_debug_data(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for export_debug_data
