...
// Targets: type_mismatch in src/error/structured.rs:772
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // type_mismatch(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // type_mismatch(data, 0);        // Zero size
        // type_mismatch(data, 1);        // Single byte
        // type_mismatch(data, size);     // Full size
        // type_mismatch(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for type_mismatch
