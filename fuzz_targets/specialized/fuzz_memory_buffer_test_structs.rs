...
// Targets: test_structs in tests/composite_types_test.csd:53
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // test_structs(data, 0);        // Zero size
        // test_structs(data, 1);        // Single byte
        // test_structs(data, size);     // Full size
        // test_structs(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for test_structs
