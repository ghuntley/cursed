...
// Targets: io_change_directory in src/execution/runtime_functions.rs:1077
...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    // Test io_change_directory with temp file
    // Example: io_change_directory(temp_path);
    // Example: io_change_directory(temp_path, data, size);
...
void test_path_operations(const uint8_t *data, size_t size) {
    ...
    // io_change_directory(path);
}


// Additional test functions for io_change_directory
