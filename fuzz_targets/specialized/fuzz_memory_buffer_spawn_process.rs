...
// Targets: spawn_process in src/runtime/process.rs:361
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // spawn_process(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // spawn_process(data, 0);        // Zero size
        // spawn_process(data, 1);        // Single byte
        // spawn_process(data, size);     // Full size
        // spawn_process(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for spawn_process
