...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // validate_memory_block(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // validate_memory_block(data, 0);        // Zero size
        // validate_memory_block(data, 1);        // Single byte
        // validate_memory_block(data, size);     // Full size
        // validate_memory_block(NULL, 0);        // NULL pointer
    }
}
