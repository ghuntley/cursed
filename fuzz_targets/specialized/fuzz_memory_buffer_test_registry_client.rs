...
// Targets: test_registry_client in stdlib/package_manager/test_package_manager.csd:364
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // test_registry_client(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // test_registry_client(data, 0);        // Zero size
        // test_registry_client(data, 1);        // Single byte
        // test_registry_client(data, size);     // Full size
        // test_registry_client(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for test_registry_client
