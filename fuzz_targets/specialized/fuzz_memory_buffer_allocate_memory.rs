...
// Targets: allocate_memory in stdlib/memory_core/mod.csd:93
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // allocate_memory(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // allocate_memory(data, 0);        // Zero size
        // allocate_memory(data, 1);        // Single byte
        // allocate_memory(data, size);     // Full size
        // allocate_memory(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for allocate_memory
