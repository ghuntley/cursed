...
// Targets: is_feature_available in src/stdlib/database/sqlite/features.rs:13
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // is_feature_available(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // is_feature_available(data, 0);        // Zero size
        // is_feature_available(data, 1);        // Single byte
        // is_feature_available(data, size);     // Full size
        // is_feature_available(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for is_feature_available
