...
// Targets: archive_add_directory in stdlib/archive_handling/mod.csd:101
...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    // Test archive_add_directory with temp file
    // Example: archive_add_directory(temp_path);
    // Example: archive_add_directory(temp_path, data, size);
...
void test_path_operations(const uint8_t *data, size_t size) {
    ...
    // archive_add_directory(path);
}


// Additional test functions for archive_add_directory
