...
// Targets: memory_copy in stdlib/memory/mod.csd:108
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // memory_copy(data, 0);        // Zero size
        // memory_copy(data, 1);        // Single byte
        // memory_copy(data, size);     // Full size
        // memory_copy(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for memory_copy
