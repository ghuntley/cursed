...
// Targets: test_string_converter in src/ffi/type_mapping.rs:880
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // test_string_converter(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // test_string_converter(data, 0);        // Zero size
        // test_string_converter(data, 1);        // Single byte
        // test_string_converter(data, size);     // Full size
        // test_string_converter(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for test_string_converter
