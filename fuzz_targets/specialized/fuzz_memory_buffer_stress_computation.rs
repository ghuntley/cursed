...
// Targets: stress_computation in final_validation/stress_test.csd:1
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // stress_computation(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // stress_computation(data, 0);        // Zero size
        // stress_computation(data, 1);        // Single byte
        // stress_computation(data, size);     // Full size
        // stress_computation(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for stress_computation
