...
// Targets: test_spillstr in tests/vibez_test.csd:17
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // test_spillstr(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // test_spillstr(data, 0);        // Zero size
        // test_spillstr(data, 1);        // Single byte
        // test_spillstr(data, size);     // Full size
        // test_spillstr(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for test_spillstr
