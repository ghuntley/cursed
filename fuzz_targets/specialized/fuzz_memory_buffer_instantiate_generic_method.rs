...
// Targets: instantiate_generic_method in src/type_system/monomorphisation.rs:578
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // instantiate_generic_method(buffer, data, size);
...
void test_string_operations(const uint8_t *data, size_t size) {
    ...
    // instantiate_generic_method(str);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // instantiate_generic_method(data, 0);        // Zero size
        // instantiate_generic_method(data, 1);        // Single byte
        // instantiate_generic_method(data, size);     // Full size
        // instantiate_generic_method(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for instantiate_generic_method
