...
// Targets: to_camel_case in src/linter/rules.rs:628
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // to_camel_case(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // to_camel_case(data, 0);        // Zero size
        // to_camel_case(data, 1);        // Single byte
        // to_camel_case(data, size);     // Full size
        // to_camel_case(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for to_camel_case
