...
// Targets: parse_value in stdlib/json/mod.csd:24
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // parse_value(data, 0);        // Zero size
        // parse_value(data, 1);        // Single byte
        // parse_value(data, size);     // Full size
        // parse_value(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for parse_value
