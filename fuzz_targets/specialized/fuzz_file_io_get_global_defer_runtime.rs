...
// Targets: get_global_defer_runtime in src/runtime/defer_runtime.rs:208
...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    // Test get_global_defer_runtime with temp file
    // Example: get_global_defer_runtime(temp_path);
    // Example: get_global_defer_runtime(temp_path, data, size);
...
void test_path_operations(const uint8_t *data, size_t size) {
    ...
    // get_global_defer_runtime(path);
}


// Additional test functions for get_global_defer_runtime
