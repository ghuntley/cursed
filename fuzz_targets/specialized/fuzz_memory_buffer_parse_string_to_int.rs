...
// Targets: parse_string_to_int in stdlib/configz/mod_complex_syntax.csd:394
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // parse_string_to_int(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // parse_string_to_int(data, 0);        // Zero size
        // parse_string_to_int(data, 1);        // Single byte
        // parse_string_to_int(data, size);     // Full size
        // parse_string_to_int(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for parse_string_to_int
