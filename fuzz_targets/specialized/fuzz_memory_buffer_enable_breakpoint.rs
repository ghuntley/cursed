...
// Targets: enable_breakpoint in src/bin/cursed_debug.rs:636
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // enable_breakpoint(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // enable_breakpoint(data, 0);        // Zero size
        // enable_breakpoint(data, 1);        // Single byte
        // enable_breakpoint(data, size);     // Full size
        // enable_breakpoint(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for enable_breakpoint
