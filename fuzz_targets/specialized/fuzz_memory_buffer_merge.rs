...
// Targets: merge in src/optimization/profile_guided_optimization.rs:629
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // merge(data, 0);        // Zero size
        // merge(data, 1);        // Single byte
        // merge(data, size);     // Full size
        // merge(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for merge
