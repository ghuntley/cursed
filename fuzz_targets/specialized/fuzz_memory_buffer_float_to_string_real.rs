...
// Targets: float_to_string_real in stdlib/vibez/enhanced_formatting.csd:114
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // float_to_string_real(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // float_to_string_real(data, 0);        // Zero size
        // float_to_string_real(data, 1);        // Single byte
        // float_to_string_real(data, size);     // Full size
        // float_to_string_real(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for float_to_string_real
