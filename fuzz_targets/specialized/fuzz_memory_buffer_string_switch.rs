...
// Targets: string_switch in examples/string_switch_fixed.csd:3
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // string_switch(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // string_switch(data, 0);        // Zero size
        // string_switch(data, 1);        // Single byte
        // string_switch(data, size);     // Full size
        // string_switch(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for string_switch
