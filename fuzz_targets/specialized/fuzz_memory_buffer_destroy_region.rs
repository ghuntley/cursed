...
// Targets: destroy_region in src/memory/regions.rs:281
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // destroy_region(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // destroy_region(data, 0);        // Zero size
        // destroy_region(data, 1);        // Single byte
        // destroy_region(data, size);     // Full size
        // destroy_region(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for destroy_region
