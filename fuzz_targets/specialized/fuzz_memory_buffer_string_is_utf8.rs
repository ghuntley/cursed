...
// Targets: string_is_utf8 in stdlib/string_simple/mod.csd:885
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // string_is_utf8(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // string_is_utf8(data, 0);        // Zero size
        // string_is_utf8(data, 1);        // Single byte
        // string_is_utf8(data, size);     // Full size
        // string_is_utf8(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for string_is_utf8
