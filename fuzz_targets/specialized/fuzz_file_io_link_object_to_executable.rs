...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    // Test link_object_to_executable with temp file
    // Example: link_object_to_executable(temp_path);
    // Example: link_object_to_executable(temp_path, data, size);
...
void test_path_operations(const uint8_t *data, size_t size) {
    ...
    // link_object_to_executable(path);
}
