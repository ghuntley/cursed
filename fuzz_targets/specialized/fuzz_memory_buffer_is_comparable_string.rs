...
// Targets: is_comparable_string in stdlib/reflection/mod.csd:74
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // is_comparable_string(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // is_comparable_string(data, 0);        // Zero size
        // is_comparable_string(data, 1);        // Single byte
        // is_comparable_string(data, size);     // Full size
        // is_comparable_string(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for is_comparable_string
