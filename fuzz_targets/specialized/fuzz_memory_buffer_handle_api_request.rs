...
// Targets: handle_api_request in src/documentation/live_server.rs:300
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // handle_api_request(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // handle_api_request(data, 0);        // Zero size
        // handle_api_request(data, 1);        // Single byte
        // handle_api_request(data, size);     // Full size
        // handle_api_request(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for handle_api_request
