...
// Targets: test_string_highlighting in src/repl/advanced_syntax_highlighter.rs:523
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // test_string_highlighting(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // test_string_highlighting(data, 0);        // Zero size
        // test_string_highlighting(data, 1);        // Single byte
        // test_string_highlighting(data, size);     // Full size
        // test_string_highlighting(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for test_string_highlighting
