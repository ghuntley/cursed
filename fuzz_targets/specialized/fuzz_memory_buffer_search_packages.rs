...
// Targets: search_packages in src/package_manager/registry.rs:116
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // search_packages(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // search_packages(data, 0);        // Zero size
        // search_packages(data, 1);        // Single byte
        // search_packages(data, size);     // Full size
        // search_packages(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for search_packages
