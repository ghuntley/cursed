...
// Targets: string_compare_values in stdlib/arrayz/mod_enhanced.csd:886
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // string_compare_values(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // string_compare_values(data, 0);        // Zero size
        // string_compare_values(data, 1);        // Single byte
        // string_compare_values(data, size);     // Full size
        // string_compare_values(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for string_compare_values
