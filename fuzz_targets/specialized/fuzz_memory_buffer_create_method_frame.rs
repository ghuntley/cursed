...
// Targets: create_method_frame in src/runtime/stack_trace.rs:549
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // create_method_frame(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // create_method_frame(data, 0);        // Zero size
        // create_method_frame(data, 1);        // Single byte
        // create_method_frame(data, size);     // Full size
        // create_method_frame(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for create_method_frame
