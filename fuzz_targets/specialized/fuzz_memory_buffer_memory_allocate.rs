...
// Targets: memory_allocate in stdlib/core/mod.csd:106
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // memory_allocate(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // memory_allocate(data, 0);        // Zero size
        // memory_allocate(data, 1);        // Single byte
        // memory_allocate(data, size);     // Full size
        // memory_allocate(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for memory_allocate
