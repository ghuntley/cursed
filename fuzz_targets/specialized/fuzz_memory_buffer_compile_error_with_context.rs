...
// Targets: compile_error_with_context in src/error/types.rs:104
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // compile_error_with_context(buffer, data, size);
...
void test_string_operations(const uint8_t *data, size_t size) {
    ...
    // compile_error_with_context(str);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // compile_error_with_context(data, 0);        // Zero size
        // compile_error_with_context(data, 1);        // Single byte
        // compile_error_with_context(data, size);     // Full size
        // compile_error_with_context(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for compile_error_with_context
