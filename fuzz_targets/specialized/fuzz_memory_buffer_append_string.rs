...
// Targets: append_string in src/stdlib/bytefit/fitbuffer.rs:28
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // append_string(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // append_string(data, 0);        // Zero size
        // append_string(data, 1);        // Single byte
        // append_string(data, size);     // Full size
        // append_string(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for append_string
