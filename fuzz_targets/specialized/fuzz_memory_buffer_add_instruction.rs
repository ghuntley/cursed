...
// Targets: add_instruction in src/bootstrap/stage2/codegen.csd:206
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // add_instruction(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // add_instruction(data, 0);        // Zero size
        // add_instruction(data, 1);        // Single byte
        // add_instruction(data, size);     // Full size
        // add_instruction(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for add_instruction
