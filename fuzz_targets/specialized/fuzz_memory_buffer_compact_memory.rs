...
// Targets: compact_memory in stdlib/memory_core/mod.csd:288
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // compact_memory(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // compact_memory(data, 0);        // Zero size
        // compact_memory(data, 1);        // Single byte
        // compact_memory(data, size);     // Full size
        // compact_memory(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for compact_memory
