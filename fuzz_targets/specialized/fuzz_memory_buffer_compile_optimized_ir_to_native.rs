...
// Targets: compile_optimized_ir_to_native in src/lib.rs:2032
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // compile_optimized_ir_to_native(buffer, data, size);
...
void test_string_operations(const uint8_t *data, size_t size) {
    ...
    // compile_optimized_ir_to_native(str);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // compile_optimized_ir_to_native(data, 0);        // Zero size
        // compile_optimized_ir_to_native(data, 1);        // Single byte
        // compile_optimized_ir_to_native(data, size);     // Full size
        // compile_optimized_ir_to_native(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for compile_optimized_ir_to_native
