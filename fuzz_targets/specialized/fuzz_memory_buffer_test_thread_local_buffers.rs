...
// Targets: test_thread_local_buffers in src/runtime/heap_optimizer.rs:964
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // test_thread_local_buffers(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // test_thread_local_buffers(data, 0);        // Zero size
        // test_thread_local_buffers(data, 1);        // Single byte
        // test_thread_local_buffers(data, size);     // Full size
        // test_thread_local_buffers(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for test_thread_local_buffers
