...
// Targets: import_error in src/error/mod.rs:210
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // import_error(data, 0);        // Zero size
        // import_error(data, 1);        // Single byte
        // import_error(data, size);     // Full size
        // import_error(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for import_error
