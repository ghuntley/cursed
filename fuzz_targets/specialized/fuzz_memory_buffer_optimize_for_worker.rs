...
// Targets: optimize_for_worker in src/optimization/distributed/network_optimizer.rs:312
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // optimize_for_worker(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // optimize_for_worker(data, 0);        // Zero size
        // optimize_for_worker(data, 1);        // Single byte
        // optimize_for_worker(data, size);     // Full size
        // optimize_for_worker(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for optimize_for_worker
