...
// Targets: foldIntToFloatCast in src-zig/constant_folder.zig:352
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // foldIntToFloatCast(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // foldIntToFloatCast(data, 0);        // Zero size
        // foldIntToFloatCast(data, 1);        // Single byte
        // foldIntToFloatCast(data, size);     // Full size
        // foldIntToFloatCast(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for foldIntToFloatCast
