...
// Targets: substitute_types_in_statement in src/type_system/monomorphisation.rs:749
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // substitute_types_in_statement(buffer, data, size);
...
void test_string_operations(const uint8_t *data, size_t size) {
    ...
    // substitute_types_in_statement(str);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // substitute_types_in_statement(data, 0);        // Zero size
        // substitute_types_in_statement(data, 1);        // Single byte
        // substitute_types_in_statement(data, size);     // Full size
        // substitute_types_in_statement(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for substitute_types_in_statement
