...
// Targets: check_return_type in src/tools/linter.rs:1346
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // check_return_type(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // check_return_type(data, 0);        // Zero size
        // check_return_type(data, 1);        // Single byte
        // check_return_type(data, size);     // Full size
        // check_return_type(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for check_return_type
