...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // read_cache_line_size(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // read_cache_line_size(data, 0);        // Zero size
        // read_cache_line_size(data, 1);        // Single byte
        // read_cache_line_size(data, size);     // Full size
        // read_cache_line_size(NULL, 0);        // NULL pointer
    }
}
