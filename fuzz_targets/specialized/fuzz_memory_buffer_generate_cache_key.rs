...
// Targets: generate_cache_key in src/optimization/optimization_full/ml/mod.rs:152
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // generate_cache_key(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // generate_cache_key(data, 0);        // Zero size
        // generate_cache_key(data, 1);        // Single byte
        // generate_cache_key(data, size);     // Full size
        // generate_cache_key(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for generate_cache_key
