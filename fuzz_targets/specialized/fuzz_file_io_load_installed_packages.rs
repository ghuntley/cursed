...
// Targets: load_installed_packages in src/package_manager/installer.rs:437
...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    // Test load_installed_packages with temp file
    // Example: load_installed_packages(temp_path);
    // Example: load_installed_packages(temp_path, data, size);
...
void test_path_operations(const uint8_t *data, size_t size) {
    ...
    // load_installed_packages(path);
}


// Additional test functions for load_installed_packages
