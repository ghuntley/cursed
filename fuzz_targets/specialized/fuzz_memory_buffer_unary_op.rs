...
// Targets: unary_op in src/runtime/runtime_value.rs:589
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // unary_op(data, 0);        // Zero size
        // unary_op(data, 1);        // Single byte
        // unary_op(data, size);     // Full size
        // unary_op(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for unary_op
