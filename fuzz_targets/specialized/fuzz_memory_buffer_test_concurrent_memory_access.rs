...
// Targets: test_concurrent_memory_access in tests/gc_safety_basic.rs:9
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // test_concurrent_memory_access(buffer, data, size);
...
void test_string_operations(const uint8_t *data, size_t size) {
    ...
    // test_concurrent_memory_access(str);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // test_concurrent_memory_access(data, 0);        // Zero size
        // test_concurrent_memory_access(data, 1);        // Single byte
        // test_concurrent_memory_access(data, size);     // Full size
        // test_concurrent_memory_access(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for test_concurrent_memory_access
