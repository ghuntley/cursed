...
// Targets: load_module in src/ffi/wasm_bridge.rs:67
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // load_module(data, 0);        // Zero size
        // load_module(data, 1);        // Single byte
        // load_module(data, size);     // Full size
        // load_module(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for load_module
