...
// Targets: contains_function in src/runtime/stack_trace.rs:254
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // contains_function(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // contains_function(data, 0);        // Zero size
        // contains_function(data, 1);        // Single byte
        // contains_function(data, size);     // Full size
        // contains_function(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for contains_function
