...
// Targets: test_goroutine_member_access in tests/goroutine_parser_test.rs:61
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // test_goroutine_member_access(buffer, data, size);
...
void test_string_operations(const uint8_t *data, size_t size) {
    ...
    // test_goroutine_member_access(str);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // test_goroutine_member_access(data, 0);        // Zero size
        // test_goroutine_member_access(data, 1);        // Single byte
        // test_goroutine_member_access(data, size);     // Full size
        // test_goroutine_member_access(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for test_goroutine_member_access
