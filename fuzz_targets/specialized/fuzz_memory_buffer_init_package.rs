...
// Targets: init_package in src/tools/package_manager.rs:87
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // init_package(data, 0);        // Zero size
        // init_package(data, 1);        // Single byte
        // init_package(data, size);     // Full size
        // init_package(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for init_package
