...
// Targets: export_json in src/ffi/profiling.rs:405
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // export_json(data, 0);        // Zero size
        // export_json(data, 1);        // Single byte
        // export_json(data, size);     // Full size
        // export_json(NULL, 0);        // NULL pointer
    }
}
