...
// Targets: get_struct_field in runtime/pattern_matching_runtime.c:147
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // get_struct_field(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // get_struct_field(data, 0);        // Zero size
        // get_struct_field(data, 1);        // Single byte
        // get_struct_field(data, size);     // Full size
        // get_struct_field(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for get_struct_field
