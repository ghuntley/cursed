...
// Targets: string_count in stdlib/string_simple/mod.csd:215
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // string_count(data, 0);        // Zero size
        // string_count(data, 1);        // Single byte
        // string_count(data, size);     // Full size
        // string_count(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for string_count
