...
// Targets: call in src/ffi/binding_generator.rs:670
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // call(data, 0);        // Zero size
        // call(data, 1);        // Single byte
        // call(data, size);     // Full size
        // call(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for call
