...
// Targets: is_installed in src/package_manager/mod.rs:526
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // is_installed(data, 0);        // Zero size
        // is_installed(data, 1);        // Single byte
        // is_installed(data, size);     // Full size
        // is_installed(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for is_installed
