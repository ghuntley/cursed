...
// Targets: test_struct_field_access in stdlib/lookin_glass/test_enhanced_lookin_glass.csd:184
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // test_struct_field_access(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // test_struct_field_access(data, 0);        // Zero size
        // test_struct_field_access(data, 1);        // Single byte
        // test_struct_field_access(data, size);     // Full size
        // test_struct_field_access(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for test_struct_field_access
