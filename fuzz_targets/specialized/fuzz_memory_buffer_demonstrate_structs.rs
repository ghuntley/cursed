...
// Targets: demonstrate_structs in examples/docs_demo.csd:110
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // demonstrate_structs(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // demonstrate_structs(data, 0);        // Zero size
        // demonstrate_structs(data, 1);        // Single byte
        // demonstrate_structs(data, size);     // Full size
        // demonstrate_structs(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for demonstrate_structs
