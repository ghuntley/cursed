...
// Targets: perform_cleanup in src/runtime/channels/lifecycle.rs:866
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // perform_cleanup(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // perform_cleanup(data, 0);        // Zero size
        // perform_cleanup(data, 1);        // Single byte
        // perform_cleanup(data, size);     // Full size
        // perform_cleanup(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for perform_cleanup
