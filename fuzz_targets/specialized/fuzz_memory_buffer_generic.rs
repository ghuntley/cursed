...
// Targets: generic in src/stdlib/squish_core/error.rs:54
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // generic(data, 0);        // Zero size
        // generic(data, 1);        // Single byte
        // generic(data, size);     // Full size
        // generic(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for generic
