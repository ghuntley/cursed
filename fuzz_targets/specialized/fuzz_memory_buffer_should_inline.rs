...
// Targets: should_inline in src/codegen/llvm/passes/loop_optimization.rs:78
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // should_inline(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // should_inline(data, 0);        // Zero size
        // should_inline(data, 1);        // Single byte
        // should_inline(data, size);     // Full size
        // should_inline(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for should_inline
