...
// Targets: apply_mirrors in src/package_manager/config.rs:274
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // apply_mirrors(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // apply_mirrors(data, 0);        // Zero size
        // apply_mirrors(data, 1);        // Single byte
        // apply_mirrors(data, size);     // Full size
        // apply_mirrors(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for apply_mirrors
