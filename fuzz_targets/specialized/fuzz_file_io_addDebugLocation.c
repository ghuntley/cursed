...
// Targets: addDebugLocation in src-zig/error_codegen_integration.zig:363
...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    // Test addDebugLocation with temp file
    // Example: addDebugLocation(temp_path);
    // Example: addDebugLocation(temp_path, data, size);
...
void test_path_operations(const uint8_t *data, size_t size) {
    ...
    // addDebugLocation(path);
}


// Additional test functions for addDebugLocation
