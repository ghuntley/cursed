...
// Targets: generate_interface_cast_ir in src/codegen/llvm/interface_type_checking.rs:169
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // generate_interface_cast_ir(buffer, data, size);
...
void test_string_operations(const uint8_t *data, size_t size) {
    ...
    // generate_interface_cast_ir(str);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // generate_interface_cast_ir(data, 0);        // Zero size
        // generate_interface_cast_ir(data, 1);        // Single byte
        // generate_interface_cast_ir(data, size);     // Full size
        // generate_interface_cast_ir(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for generate_interface_cast_ir
