...
// Targets: io_create_buffer in src/execution/runtime_functions.rs:1311
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // io_create_buffer(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // io_create_buffer(data, 0);        // Zero size
        // io_create_buffer(data, 1);        // Single byte
        // io_create_buffer(data, size);     // Full size
        // io_create_buffer(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for io_create_buffer
