...
// Targets: test_sprintf in variadic_ffi_test.csd:33
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // test_sprintf(data, 0);        // Zero size
        // test_sprintf(data, 1);        // Single byte
        // test_sprintf(data, size);     // Full size
        // test_sprintf(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for test_sprintf
