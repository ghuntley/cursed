...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    // Test compile_to_wasm_with_optimizations with fuzzed input
    // Example: compile_to_wasm_with_optimizations(allocator, input);
    // Example: compile_to_wasm_with_optimizations(input, size);
