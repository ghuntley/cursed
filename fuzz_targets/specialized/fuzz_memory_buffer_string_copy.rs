...
// Targets: string_copy in stdlib/compression/mod.csd:112
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // string_copy(data, 0);        // Zero size
        // string_copy(data, 1);        // Single byte
        // string_copy(data, size);     // Full size
        // string_copy(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for string_copy
