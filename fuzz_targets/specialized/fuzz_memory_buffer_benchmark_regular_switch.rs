...
// Targets: benchmark_regular_switch in examples/type_switch_vs_regular_switch.csd:258
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // benchmark_regular_switch(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // benchmark_regular_switch(data, 0);        // Zero size
        // benchmark_regular_switch(data, 1);        // Single byte
        // benchmark_regular_switch(data, size);     // Full size
        // benchmark_regular_switch(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for benchmark_regular_switch
