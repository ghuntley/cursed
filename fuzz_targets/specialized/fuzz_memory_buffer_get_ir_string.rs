...
// Targets: get_ir_string in src/codegen/llvm/inkwell_codegen.rs:567
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // get_ir_string(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // get_ir_string(data, 0);        // Zero size
        // get_ir_string(data, 1);        // Single byte
        // get_ir_string(data, size);     // Full size
        // get_ir_string(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for get_ir_string
