...
// Targets: is_critical_instruction in src/codegen/llvm/passes/dead_code_elimination.rs:224
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // is_critical_instruction(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // is_critical_instruction(data, 0);        // Zero size
        // is_critical_instruction(data, 1);        // Single byte
        // is_critical_instruction(data, size);     // Full size
        // is_critical_instruction(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for is_critical_instruction
