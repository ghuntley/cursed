...
// Targets: demonstrate_basic_math in examples/math_comprehensive_demo.csd:8
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // demonstrate_basic_math(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // demonstrate_basic_math(data, 0);        // Zero size
        // demonstrate_basic_math(data, 1);        // Single byte
        // demonstrate_basic_math(data, size);     // Full size
        // demonstrate_basic_math(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for demonstrate_basic_math
