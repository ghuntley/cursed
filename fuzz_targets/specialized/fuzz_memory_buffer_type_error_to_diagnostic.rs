...
// Targets: type_error_to_diagnostic in src/lsp/diagnostics.rs:112
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // type_error_to_diagnostic(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // type_error_to_diagnostic(data, 0);        // Zero size
        // type_error_to_diagnostic(data, 1);        // Single byte
        // type_error_to_diagnostic(data, size);     // Full size
        // type_error_to_diagnostic(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for type_error_to_diagnostic
