...
// Targets: tokenize in src/bin/minimal_parser_test.rs:63
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // tokenize(data, 0);        // Zero size
        // tokenize(data, 1);        // Single byte
        // tokenize(data, size);     // Full size
        // tokenize(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for tokenize
