...
// Targets: get_lines in src/linter/mod.rs:278
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // get_lines(data, 0);        // Zero size
        // get_lines(data, 1);        // Single byte
        // get_lines(data, size);     // Full size
        // get_lines(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for get_lines
