...
// Targets: compile_vibez_method_call in src/codegen/llvm/expression_compiler.rs:999
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // compile_vibez_method_call(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // compile_vibez_method_call(data, 0);        // Zero size
        // compile_vibez_method_call(data, 1);        // Single byte
        // compile_vibez_method_call(data, size);     // Full size
        // compile_vibez_method_call(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for compile_vibez_method_call
