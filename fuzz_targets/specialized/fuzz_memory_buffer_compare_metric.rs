...
// Targets: compare_metric in src/optimization/performance_regression_detection.rs:393
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // compare_metric(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // compare_metric(data, 0);        // Zero size
        // compare_metric(data, 1);        // Single byte
        // compare_metric(data, size);     // Full size
        // compare_metric(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for compare_metric
