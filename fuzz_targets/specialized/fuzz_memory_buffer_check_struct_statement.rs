...
// Targets: check_struct_statement in src/type_system/checker.rs:1343
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // check_struct_statement(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // check_struct_statement(data, 0);        // Zero size
        // check_struct_statement(data, 1);        // Single byte
        // check_struct_statement(data, size);     // Full size
        // check_struct_statement(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for check_struct_statement
