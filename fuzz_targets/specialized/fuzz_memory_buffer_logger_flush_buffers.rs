...
// Targets: logger_flush_buffers in stdlib/plugin_system/example_plugin.csd:311
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // logger_flush_buffers(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // logger_flush_buffers(data, 0);        // Zero size
        // logger_flush_buffers(data, 1);        // Single byte
        // logger_flush_buffers(data, size);     // Full size
        // logger_flush_buffers(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for logger_flush_buffers
