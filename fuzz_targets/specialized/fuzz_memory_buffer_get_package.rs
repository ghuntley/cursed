...
// Targets: get_package in src/package_manager/lock_file.rs:165
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // get_package(data, 0);        // Zero size
        // get_package(data, 1);        // Single byte
        // get_package(data, size);     // Full size
        // get_package(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for get_package
