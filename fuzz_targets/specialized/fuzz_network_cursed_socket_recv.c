...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    // Test packet parsing
    // Test cursed_socket_recv with network data
    // Example: cursed_socket_recv(data, size);
    // Example: cursed_socket_recv(sockfd, data, size);
...
void test_socket_operations(const uint8_t *data, size_t size) {
    ...
    // cursed_socket_recv(sockfd, data, size);
