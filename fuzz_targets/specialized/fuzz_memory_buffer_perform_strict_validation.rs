...
// Targets: perform_strict_validation in src/lib.rs:1948
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // perform_strict_validation(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // perform_strict_validation(data, 0);        // Zero size
        // perform_strict_validation(data, 1);        // Single byte
        // perform_strict_validation(data, size);     // Full size
        // perform_strict_validation(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for perform_strict_validation
