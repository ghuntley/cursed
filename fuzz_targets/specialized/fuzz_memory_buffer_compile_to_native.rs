...
// Targets: compile_to_native in src/lib.rs:2085
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // compile_to_native(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // compile_to_native(data, 0);        // Zero size
        // compile_to_native(data, 1);        // Single byte
        // compile_to_native(data, size);     // Full size
        // compile_to_native(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for compile_to_native
