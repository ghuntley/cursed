...
// Targets: validate_email in src/security/input_validation.rs:274
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // validate_email(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // validate_email(data, 0);        // Zero size
        // validate_email(data, 1);        // Single byte
        // validate_email(data, size);     // Full size
        // validate_email(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for validate_email
