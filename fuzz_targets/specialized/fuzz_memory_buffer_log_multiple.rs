...
// Targets: log_multiple in src/stdlib/test_vibes/core.rs:93
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // log_multiple(data, 0);        // Zero size
        // log_multiple(data, 1);        // Single byte
        // log_multiple(data, size);     // Full size
        // log_multiple(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for log_multiple
