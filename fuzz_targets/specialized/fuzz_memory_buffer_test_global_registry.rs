...
// Targets: test_global_registry in stdlib/stat_flexin/test_stat_flexin.csd:92
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // test_global_registry(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // test_global_registry(data, 0);        // Zero size
        // test_global_registry(data, 1);        // Single byte
        // test_global_registry(data, size);     // Full size
        // test_global_registry(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for test_global_registry
