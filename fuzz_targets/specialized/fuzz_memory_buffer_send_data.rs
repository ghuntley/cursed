...
// Targets: send_data in src/optimization/distributed/network_optimizer.rs:208
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // send_data(data, 0);        // Zero size
        // send_data(data, 1);        // Single byte
        // send_data(data, size);     // Full size
        // send_data(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for send_data
