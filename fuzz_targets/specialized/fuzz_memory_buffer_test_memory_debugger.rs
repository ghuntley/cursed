...
// Targets: test_memory_debugger in src/ffi/debug_tools.rs:1054
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // test_memory_debugger(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // test_memory_debugger(data, 0);        // Zero size
        // test_memory_debugger(data, 1);        // Single byte
        // test_memory_debugger(data, size);     // Full size
        // test_memory_debugger(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for test_memory_debugger
