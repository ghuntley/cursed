...
// Targets: show_project_structure in examples/watch_build_example.csd:170
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // show_project_structure(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // show_project_structure(data, 0);        // Zero size
        // show_project_structure(data, 1);        // Single byte
        // show_project_structure(data, size);     // Full size
        // show_project_structure(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for show_project_structure
