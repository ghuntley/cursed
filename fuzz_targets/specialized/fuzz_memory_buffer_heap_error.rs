...
// Targets: heap_error in src/stdlib/collections/heap_slay/mod.rs:50
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // heap_error(data, 0);        // Zero size
        // heap_error(data, 1);        // Single byte
        // heap_error(data, size);     // Full size
        // heap_error(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for heap_error
