...
// Targets: string_replace_first in stdlib/string_simple/mod.csd:533
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // string_replace_first(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // string_replace_first(data, 0);        // Zero size
        // string_replace_first(data, 1);        // Single byte
        // string_replace_first(data, size);     // Full size
        // string_replace_first(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for string_replace_first
