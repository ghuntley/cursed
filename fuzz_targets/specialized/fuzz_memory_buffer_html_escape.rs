...
// Targets: html_escape in src/coverage/reporter.rs:453
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // html_escape(data, 0);        // Zero size
        // html_escape(data, 1);        // Single byte
        // html_escape(data, size);     // Full size
        // html_escape(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for html_escape
