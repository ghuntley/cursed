...
// Targets: center_string in stdlib/stringz_enhanced/mod.csd:296
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // center_string(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // center_string(data, 0);        // Zero size
        // center_string(data, 1);        // Single byte
        // center_string(data, size);     // Full size
        // center_string(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for center_string
