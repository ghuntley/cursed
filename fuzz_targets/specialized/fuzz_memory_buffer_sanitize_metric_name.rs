...
// Targets: sanitize_metric_name in src/metrics/prometheus_exporter.rs:638
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // sanitize_metric_name(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // sanitize_metric_name(data, 0);        // Zero size
        // sanitize_metric_name(data, 1);        // Single byte
        // sanitize_metric_name(data, size);     // Full size
        // sanitize_metric_name(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for sanitize_metric_name
