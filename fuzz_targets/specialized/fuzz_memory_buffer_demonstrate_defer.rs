...
// Targets: demonstrate_defer in defer_compilation_demo.csd:1
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // demonstrate_defer(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // demonstrate_defer(data, 0);        // Zero size
        // demonstrate_defer(data, 1);        // Single byte
        // demonstrate_defer(data, size);     // Full size
        // demonstrate_defer(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for demonstrate_defer
