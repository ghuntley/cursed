...
// Targets: pretty_print_json in stdlib/json/mod.csd:235
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // pretty_print_json(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // pretty_print_json(data, 0);        // Zero size
        // pretty_print_json(data, 1);        // Single byte
        // pretty_print_json(data, size);     // Full size
        // pretty_print_json(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for pretty_print_json
