...
// Targets: build_constraint_graph in src/type_system/constraint_resolver.rs:780
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // build_constraint_graph(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // build_constraint_graph(data, 0);        // Zero size
        // build_constraint_graph(data, 1);        // Single byte
        // build_constraint_graph(data, size);     // Full size
        // build_constraint_graph(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for build_constraint_graph
