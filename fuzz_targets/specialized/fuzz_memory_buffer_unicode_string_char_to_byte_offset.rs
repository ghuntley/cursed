...
// Targets: unicode_string_char_to_byte_offset in stdlib/unicode/string_integration.csd:687
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // unicode_string_char_to_byte_offset(buffer, data, size);
...
void test_string_operations(const uint8_t *data, size_t size) {
    ...
    // unicode_string_char_to_byte_offset(str);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // unicode_string_char_to_byte_offset(data, 0);        // Zero size
        // unicode_string_char_to_byte_offset(data, 1);        // Single byte
        // unicode_string_char_to_byte_offset(data, size);     // Full size
        // unicode_string_char_to_byte_offset(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for unicode_string_char_to_byte_offset
