...
// Targets: fold_zext in src/codegen/llvm/passes/constant_propagation.rs:669
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // fold_zext(data, 0);        // Zero size
        // fold_zext(data, 1);        // Single byte
        // fold_zext(data, size);     // Full size
        // fold_zext(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for fold_zext
