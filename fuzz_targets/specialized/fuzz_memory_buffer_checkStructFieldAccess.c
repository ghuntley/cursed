...
// Targets: checkStructFieldAccess in src-zig/type_system_runtime.zig:641
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // checkStructFieldAccess(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // checkStructFieldAccess(data, 0);        // Zero size
        // checkStructFieldAccess(data, 1);        // Single byte
        // checkStructFieldAccess(data, size);     // Full size
        // checkStructFieldAccess(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for checkStructFieldAccess
