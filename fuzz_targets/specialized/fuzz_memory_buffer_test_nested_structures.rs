...
// Targets: test_nested_structures in stdlib/formatter/test_formatter.csd:203
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // test_nested_structures(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // test_nested_structures(data, 0);        // Zero size
        // test_nested_structures(data, 1);        // Single byte
        // test_nested_structures(data, size);     // Full size
        // test_nested_structures(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for test_nested_structures
