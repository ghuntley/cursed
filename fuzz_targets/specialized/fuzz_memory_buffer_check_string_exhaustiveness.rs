...
// Targets: check_string_exhaustiveness in src/pattern_matching.rs:306
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // check_string_exhaustiveness(buffer, data, size);
...
void test_string_operations(const uint8_t *data, size_t size) {
    ...
    // check_string_exhaustiveness(str);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // check_string_exhaustiveness(data, 0);        // Zero size
        // check_string_exhaustiveness(data, 1);        // Single byte
        // check_string_exhaustiveness(data, size);     // Full size
        // check_string_exhaustiveness(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for check_string_exhaustiveness
