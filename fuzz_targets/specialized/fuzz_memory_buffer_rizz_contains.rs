...
// Targets: rizz_contains in stdlib/rizz_template/mod.csd:248
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // rizz_contains(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // rizz_contains(data, 0);        // Zero size
        // rizz_contains(data, 1);        // Single byte
        // rizz_contains(data, size);     // Full size
        // rizz_contains(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for rizz_contains
