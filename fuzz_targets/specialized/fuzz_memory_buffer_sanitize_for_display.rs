...
// Targets: sanitize_for_display in src/runtime/unicode_char.rs:653
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // sanitize_for_display(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // sanitize_for_display(data, 0);        // Zero size
        // sanitize_for_display(data, 1);        // Single byte
        // sanitize_for_display(data, size);     // Full size
        // sanitize_for_display(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for sanitize_for_display
