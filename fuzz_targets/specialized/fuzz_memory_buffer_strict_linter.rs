...
// Targets: strict_linter in src/tools/linter.rs:279
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // strict_linter(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // strict_linter(data, 0);        // Zero size
        // strict_linter(data, 1);        // Single byte
        // strict_linter(data, size);     // Full size
        // strict_linter(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for strict_linter
