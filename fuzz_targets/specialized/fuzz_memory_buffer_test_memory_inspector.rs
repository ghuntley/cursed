...
// Targets: test_memory_inspector in tests/debug_engine_tests.rs:373
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // test_memory_inspector(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // test_memory_inspector(data, 0);        // Zero size
        // test_memory_inspector(data, 1);        // Single byte
        // test_memory_inspector(data, size);     // Full size
        // test_memory_inspector(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for test_memory_inspector
