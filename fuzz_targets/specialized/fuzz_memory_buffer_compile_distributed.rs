...
// Targets: compile_distributed in src/optimization/optimization_full/distributed/mod.rs:123
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // compile_distributed(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // compile_distributed(data, 0);        // Zero size
        // compile_distributed(data, 1);        // Single byte
        // compile_distributed(data, size);     // Full size
        // compile_distributed(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for compile_distributed
