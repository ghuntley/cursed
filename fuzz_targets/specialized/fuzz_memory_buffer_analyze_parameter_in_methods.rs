...
// Targets: analyze_parameter_in_methods in src/type_system/variance.rs:82
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // analyze_parameter_in_methods(buffer, data, size);
...
void test_string_operations(const uint8_t *data, size_t size) {
    ...
    // analyze_parameter_in_methods(str);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // analyze_parameter_in_methods(data, 0);        // Zero size
        // analyze_parameter_in_methods(data, 1);        // Single byte
        // analyze_parameter_in_methods(data, size);     // Full size
        // analyze_parameter_in_methods(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for analyze_parameter_in_methods
