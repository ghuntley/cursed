...
// Targets: string_literal in src/lexer/mod.rs:571
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // string_literal(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // string_literal(data, 0);        // Zero size
        // string_literal(data, 1);        // Single byte
        // string_literal(data, size);     // Full size
        // string_literal(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for string_literal
