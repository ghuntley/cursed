...
// Targets: get_latest_version in src/package_manager/registry.rs:277
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // get_latest_version(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // get_latest_version(data, 0);        // Zero size
        // get_latest_version(data, 1);        // Single byte
        // get_latest_version(data, size);     // Full size
        // get_latest_version(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for get_latest_version
