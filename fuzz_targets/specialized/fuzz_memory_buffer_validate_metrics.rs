...
// Targets: validate_metrics in src/bin/cursed_metrics.rs:272
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // validate_metrics(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // validate_metrics(data, 0);        // Zero size
        // validate_metrics(data, 1);        // Single byte
        // validate_metrics(data, size);     // Full size
        // validate_metrics(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for validate_metrics
