...
// Targets: check_struct_literal_expression in src/type_system/checker.rs:1941
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // check_struct_literal_expression(buffer, data, size);
...
void test_string_operations(const uint8_t *data, size_t size) {
    ...
    // check_struct_literal_expression(str);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // check_struct_literal_expression(data, 0);        // Zero size
        // check_struct_literal_expression(data, 1);        // Single byte
        // check_struct_literal_expression(data, size);     // Full size
        // check_struct_literal_expression(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for check_struct_literal_expression
