...
// Targets: compile_channel_send in src/codegen/llvm/expression_compiler.rs:1197
...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    // Test packet parsing
    // Test compile_channel_send with network data
    // Example: compile_channel_send(data, size);
    // Example: compile_channel_send(sockfd, data, size);
...
void test_socket_operations(const uint8_t *data, size_t size) {
    ...
    // compile_channel_send(sockfd, data, size);
    
    close(sockfd);
}


// Additional test functions for compile_channel_send
