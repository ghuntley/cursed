...
// Targets: archive_context in src/runtime/error_context.rs:616
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // archive_context(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // archive_context(data, 0);        // Zero size
        // archive_context(data, 1);        // Single byte
        // archive_context(data, size);     // Full size
        // archive_context(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for archive_context
