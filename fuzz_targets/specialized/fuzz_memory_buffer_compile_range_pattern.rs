...
// Targets: compile_range_pattern in src/pattern_matching.rs:552
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // compile_range_pattern(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // compile_range_pattern(data, 0);        // Zero size
        // compile_range_pattern(data, 1);        // Single byte
        // compile_range_pattern(data, size);     // Full size
        // compile_range_pattern(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for compile_range_pattern
