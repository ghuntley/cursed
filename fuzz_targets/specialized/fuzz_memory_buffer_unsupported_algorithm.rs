...
// Targets: unsupported_algorithm in src/error/mod.rs:194
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // unsupported_algorithm(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // unsupported_algorithm(data, 0);        // Zero size
        // unsupported_algorithm(data, 1);        // Single byte
        // unsupported_algorithm(data, size);     // Full size
        // unsupported_algorithm(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for unsupported_algorithm
