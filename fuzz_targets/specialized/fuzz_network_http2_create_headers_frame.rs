...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    // Test packet parsing
    // Test http2_create_headers_frame with network data
    // Example: http2_create_headers_frame(data, size);
    // Example: http2_create_headers_frame(sockfd, data, size);
...
void test_socket_operations(const uint8_t *data, size_t size) {
    ...
    // http2_create_headers_frame(sockfd, data, size);
