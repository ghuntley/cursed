...
// Targets: call_method_string in stdlib/reflection/mod.csd:141
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // call_method_string(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // call_method_string(data, 0);        // Zero size
        // call_method_string(data, 1);        // Single byte
        // call_method_string(data, size);     // Full size
        // call_method_string(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for call_method_string
