...
// Targets: format_variance in src/type_system/variance.rs:230
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // format_variance(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // format_variance(data, 0);        // Zero size
        // format_variance(data, 1);        // Single byte
        // format_variance(data, size);     // Full size
        // format_variance(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for format_variance
