...
// Targets: parse_cstring in src/debug/gdb_integration.rs:903
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // parse_cstring(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // parse_cstring(data, 0);        // Zero size
        // parse_cstring(data, 1);        // Single byte
        // parse_cstring(data, size);     // Full size
        // parse_cstring(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for parse_cstring
