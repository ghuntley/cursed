...
// Targets: register_capability in tests/lsp_integration_tests.rs:19
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // register_capability(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // register_capability(data, 0);        // Zero size
        // register_capability(data, 1);        // Single byte
        // register_capability(data, size);     // Full size
        // register_capability(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for register_capability
