...
// Targets: from in src/runtime/value/mod.rs:274
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // from(data, 0);        // Zero size
        // from(data, 1);        // Single byte
        // from(data, size);     // Full size
        // from(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for from
