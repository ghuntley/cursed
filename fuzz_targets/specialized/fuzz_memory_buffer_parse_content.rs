...
// Targets: parse_content in src/ffi/header_parser.rs:175
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // parse_content(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // parse_content(data, 0);        // Zero size
        // parse_content(data, 1);        // Single byte
        // parse_content(data, size);     // Full size
        // parse_content(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for parse_content
