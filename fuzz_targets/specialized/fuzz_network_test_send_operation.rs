...
// Targets: test_send_operation in src/runtime/channels/operations.rs:812
...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    // Test packet parsing
    // Test test_send_operation with network data
    // Example: test_send_operation(data, size);
    // Example: test_send_operation(sockfd, data, size);
...
void test_socket_operations(const uint8_t *data, size_t size) {
    ...
    // test_send_operation(sockfd, data, size);
    
    close(sockfd);
}


// Additional test functions for test_send_operation
