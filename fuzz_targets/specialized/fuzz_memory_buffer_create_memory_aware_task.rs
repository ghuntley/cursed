...
// Targets: create_memory_aware_task in src/build_system/memory_optimizer.rs:206
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // create_memory_aware_task(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // create_memory_aware_task(data, 0);        // Zero size
        // create_memory_aware_task(data, 1);        // Single byte
        // create_memory_aware_task(data, size);     // Full size
        // create_memory_aware_task(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for create_memory_aware_task
