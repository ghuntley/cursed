...
// Targets: is_ascii_only in src/runtime/unicode_char.rs:591
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // is_ascii_only(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // is_ascii_only(data, 0);        // Zero size
        // is_ascii_only(data, 1);        // Single byte
        // is_ascii_only(data, size);     // Full size
        // is_ascii_only(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for is_ascii_only
