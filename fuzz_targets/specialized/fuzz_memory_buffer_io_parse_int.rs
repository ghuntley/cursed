...
// Targets: io_parse_int in stdlib/io_simple/mod_broken.csd:580
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // io_parse_int(data, 0);        // Zero size
        // io_parse_int(data, 1);        // Single byte
        // io_parse_int(data, size);     // Full size
        // io_parse_int(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for io_parse_int
