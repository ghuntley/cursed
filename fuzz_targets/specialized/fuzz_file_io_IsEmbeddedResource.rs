...
// Targets: IsEmbeddedResource in stdlib/embed_that/mod.csd:341
...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    // Test IsEmbeddedResource with temp file
    // Example: IsEmbeddedResource(temp_path);
    // Example: IsEmbeddedResource(temp_path, data, size);
...
void test_path_operations(const uint8_t *data, size_t size) {
    ...
    // IsEmbeddedResource(path);
}


// Additional test functions for IsEmbeddedResource
