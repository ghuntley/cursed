...
// Targets: send_ipc_message in src/runtime/process.rs:220
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // send_ipc_message(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // send_ipc_message(data, 0);        // Zero size
        // send_ipc_message(data, 1);        // Single byte
        // send_ipc_message(data, size);     // Full size
        // send_ipc_message(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for send_ipc_message
