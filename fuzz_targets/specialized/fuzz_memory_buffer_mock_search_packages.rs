...
// Targets: mock_search_packages in src/package_manager/registry.rs:462
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // mock_search_packages(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // mock_search_packages(data, 0);        // Zero size
        // mock_search_packages(data, 1);        // Single byte
        // mock_search_packages(data, size);     // Full size
        // mock_search_packages(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for mock_search_packages
