...
// Targets: update_stats in src/stdlib/packages/crypto_signatures/mod.rs:182
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // update_stats(data, 0);        // Zero size
        // update_stats(data, 1);        // Single byte
        // update_stats(data, size);     // Full size
        // update_stats(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for update_stats
