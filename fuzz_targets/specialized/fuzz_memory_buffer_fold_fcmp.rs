...
// Targets: fold_fcmp in src/codegen/llvm/passes/constant_propagation.rs:663
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // fold_fcmp(data, 0);        // Zero size
        // fold_fcmp(data, 1);        // Single byte
        // fold_fcmp(data, size);     // Full size
        // fold_fcmp(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for fold_fcmp
