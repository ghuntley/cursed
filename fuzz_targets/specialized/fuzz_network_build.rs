...
// Targets: build in src/stdlib/database/sqlite/transaction.rs:115
...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    // Test packet parsing
    // Test build with network data
    // Example: build(data, size);
    // Example: build(sockfd, data, size);
...
void test_socket_operations(const uint8_t *data, size_t size) {
    ...
    // build(sockfd, data, size);
    
    close(sockfd);
}


// Additional test functions for build
