...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // set_memory_profiler(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // set_memory_profiler(data, 0);        // Zero size
        // set_memory_profiler(data, 1);        // Single byte
        // set_memory_profiler(data, size);     // Full size
        // set_memory_profiler(NULL, 0);        // NULL pointer
    }
}
