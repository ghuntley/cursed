...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // io_parse_float(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // io_parse_float(data, 0);        // Zero size
        // io_parse_float(data, 1);        // Single byte
        // io_parse_float(data, size);     // Full size
        // io_parse_float(NULL, 0);        // NULL pointer
    }
}
