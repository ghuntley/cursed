...
// Targets: string_last_index_of in stdlib/string_simple/mod.csd:170
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // string_last_index_of(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // string_last_index_of(data, 0);        // Zero size
        // string_last_index_of(data, 1);        // Single byte
        // string_last_index_of(data, size);     // Full size
        // string_last_index_of(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for string_last_index_of
