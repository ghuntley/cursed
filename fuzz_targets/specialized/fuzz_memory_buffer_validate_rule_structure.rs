...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // validate_rule_structure(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // validate_rule_structure(data, 0);        // Zero size
        // validate_rule_structure(data, 1);        // Single byte
        // validate_rule_structure(data, size);     // Full size
        // validate_rule_structure(NULL, 0);        // NULL pointer
    }
}
