...
// Targets: export_to_file in src/ffi/debug_tools.rs:975
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // export_to_file(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // export_to_file(data, 0);        // Zero size
        // export_to_file(data, 1);        // Single byte
        // export_to_file(data, size);     // Full size
        // export_to_file(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for export_to_file
