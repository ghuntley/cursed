...
// Targets: test_buffered_output in stdlib/sus_log/test_sus_log.csd:188
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // test_buffered_output(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // test_buffered_output(data, 0);        // Zero size
        // test_buffered_output(data, 1);        // Single byte
        // test_buffered_output(data, size);     // Full size
        // test_buffered_output(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for test_buffered_output
