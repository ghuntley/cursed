...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    // Test packet parsing
    // Test send_debugger_command with network data
    // Example: send_debugger_command(data, size);
    // Example: send_debugger_command(sockfd, data, size);
...
void test_socket_operations(const uint8_t *data, size_t size) {
    ...
    // send_debugger_command(sockfd, data, size);
