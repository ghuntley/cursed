...
// Targets: call_c_function in src/wasm/ffi_stub.rs:34
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // call_c_function(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // call_c_function(data, 0);        // Zero size
        // call_c_function(data, 1);        // Single byte
        // call_c_function(data, size);     // Full size
        // call_c_function(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for call_c_function
