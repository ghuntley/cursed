...
// Targets: generate_cpu_summary in src/tools/profiler.rs:319
...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    // Test generate_cpu_summary with temp file
    // Example: generate_cpu_summary(temp_path);
    // Example: generate_cpu_summary(temp_path, data, size);
...
void test_path_operations(const uint8_t *data, size_t size) {
    ...
    // generate_cpu_summary(path);
}


// Additional test functions for generate_cpu_summary
