...
// Targets: string_base64_decode in src/execution/runtime_functions.rs:3909
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // string_base64_decode(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // string_base64_decode(data, 0);        // Zero size
        // string_base64_decode(data, 1);        // Single byte
        // string_base64_decode(data, size);     // Full size
        // string_base64_decode(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for string_base64_decode
