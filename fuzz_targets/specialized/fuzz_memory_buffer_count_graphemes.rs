...
// Targets: count_graphemes in src/runtime/unicode_char.rs:596
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // count_graphemes(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // count_graphemes(data, 0);        // Zero size
        // count_graphemes(data, 1);        // Single byte
        // count_graphemes(data, size);     // Full size
        // count_graphemes(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for count_graphemes
