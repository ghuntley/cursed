...
// Targets: debug_print_memory_usage in stdlib/debug_tea/mod.csd:159
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // debug_print_memory_usage(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // debug_print_memory_usage(data, 0);        // Zero size
        // debug_print_memory_usage(data, 1);        // Single byte
        // debug_print_memory_usage(data, size);     // Full size
        // debug_print_memory_usage(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for debug_print_memory_usage
