...
// Targets: validate_debug_info in src/cli/debug_cli.rs:682
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // validate_debug_info(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // validate_debug_info(data, 0);        // Zero size
        // validate_debug_info(data, 1);        // Single byte
        // validate_debug_info(data, size);     // Full size
        // validate_debug_info(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for validate_debug_info
