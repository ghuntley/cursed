...
// Targets: update_variable in src/runtime/debug_runtime.rs:1112
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // update_variable(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // update_variable(data, 0);        // Zero size
        // update_variable(data, 1);        // Single byte
        // update_variable(data, size);     // Full size
        // update_variable(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for update_variable
