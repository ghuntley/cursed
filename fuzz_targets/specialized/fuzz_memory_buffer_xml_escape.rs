...
// Targets: xml_escape in src/coverage/reporter.rs:462
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // xml_escape(data, 0);        // Zero size
        // xml_escape(data, 1);        // Single byte
        // xml_escape(data, size);     // Full size
        // xml_escape(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for xml_escape
