...
// Targets: demonstrate_asymmetric_crypto in examples/crypto_asymmetric_demo.csd:13
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // demonstrate_asymmetric_crypto(buffer, data, size);
...
void test_string_operations(const uint8_t *data, size_t size) {
    ...
    // demonstrate_asymmetric_crypto(str);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // demonstrate_asymmetric_crypto(data, 0);        // Zero size
        // demonstrate_asymmetric_crypto(data, 1);        // Single byte
        // demonstrate_asymmetric_crypto(data, size);     // Full size
        // demonstrate_asymmetric_crypto(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for demonstrate_asymmetric_crypto
