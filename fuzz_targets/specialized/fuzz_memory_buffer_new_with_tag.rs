...
// Targets: new_with_tag in src/runtime/gc.rs:752
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // new_with_tag(data, 0);        // Zero size
        // new_with_tag(data, 1);        // Single byte
        // new_with_tag(data, size);     // Full size
        // new_with_tag(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for new_with_tag
