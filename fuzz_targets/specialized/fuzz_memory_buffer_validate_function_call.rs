...
// Targets: validate_function_call in src/ffi/safety_checks.rs:60
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // validate_function_call(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // validate_function_call(data, 0);        // Zero size
        // validate_function_call(data, 1);        // Single byte
        // validate_function_call(data, size);     // Full size
        // validate_function_call(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for validate_function_call
