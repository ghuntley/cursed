...
// Targets: worker_thread_main in stdlib/async/executor.csd:100
...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    // Test worker_thread_main with temp file
    // Example: worker_thread_main(temp_path);
    // Example: worker_thread_main(temp_path, data, size);
...
void test_path_operations(const uint8_t *data, size_t size) {
    ...
    // worker_thread_main(path);
}


// Additional test functions for worker_thread_main
