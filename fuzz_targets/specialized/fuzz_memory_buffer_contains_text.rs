...
// Targets: contains_text in stdlib/rpc_vibes/test_rpc_vibes.csd:127
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // contains_text(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // contains_text(data, 0);        // Zero size
        // contains_text(data, 1);        // Single byte
        // contains_text(data, size);     // Full size
        // contains_text(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for contains_text
