...
// Targets: handle_special_command in src/repl/cursed_repl.rs:190
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // handle_special_command(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // handle_special_command(data, 0);        // Zero size
        // handle_special_command(data, 1);        // Single byte
        // handle_special_command(data, size);     // Full size
        // handle_special_command(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for handle_special_command
