...
// Targets: compile_with_packages in src/codegen/llvm/main.rs:3129
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // compile_with_packages(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // compile_with_packages(data, 0);        // Zero size
        // compile_with_packages(data, 1);        // Single byte
        // compile_with_packages(data, size);     // Full size
        // compile_with_packages(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for compile_with_packages
