...
// Targets: benchmark_function_memory in benchmarks/cursed/compiler/memory_usage_bench.csd:206
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // benchmark_function_memory(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // benchmark_function_memory(data, 0);        // Zero size
        // benchmark_function_memory(data, 1);        // Single byte
        // benchmark_function_memory(data, size);     // Full size
        // benchmark_function_memory(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for benchmark_function_memory
