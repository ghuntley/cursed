...
// Targets: cursed_is_string_type in runtime/type_checking.c:133
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // cursed_is_string_type(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // cursed_is_string_type(data, 0);        // Zero size
        // cursed_is_string_type(data, 1);        // Single byte
        // cursed_is_string_type(data, size);     // Full size
        // cursed_is_string_type(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for cursed_is_string_type
