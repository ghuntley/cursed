...
// Targets: length in src/stdlib/packages/crypto_random/random_strings.rs:38
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // length(data, 0);        // Zero size
        // length(data, 1);        // Single byte
        // length(data, size);     // Full size
        // length(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for length
