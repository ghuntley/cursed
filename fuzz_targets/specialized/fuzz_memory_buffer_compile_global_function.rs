...
// Targets: compile_global_function in src/runtime/jit_runtime.rs:985
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // compile_global_function(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // compile_global_function(data, 0);        // Zero size
        // compile_global_function(data, 1);        // Single byte
        // compile_global_function(data, size);     // Full size
        // compile_global_function(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for compile_global_function
