...
// Targets: string_index in stdlib/template_engine/mod.csd:505
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // string_index(data, 0);        // Zero size
        // string_index(data, 1);        // Single byte
        // string_index(data, size);     // Full size
        // string_index(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for string_index
