...
// Targets: create_target in src/debug/lldb_integration.rs:316
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // create_target(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // create_target(data, 0);        // Zero size
        // create_target(data, 1);        // Single byte
        // create_target(data, size);     // Full size
        // create_target(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for create_target
