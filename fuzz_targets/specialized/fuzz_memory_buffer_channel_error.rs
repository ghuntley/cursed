...
// Targets: channel_error in src/stdlib/async/mod.rs:213
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // channel_error(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // channel_error(data, 0);        // Zero size
        // channel_error(data, 1);        // Single byte
        // channel_error(data, size);     // Full size
        // channel_error(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for channel_error
