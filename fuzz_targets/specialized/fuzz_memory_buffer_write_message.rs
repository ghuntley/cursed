...
// Targets: write_message in src/runtime/debug_output.rs:227
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // write_message(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // write_message(data, 0);        // Zero size
        // write_message(data, 1);        // Single byte
        // write_message(data, size);     // Full size
        // write_message(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for write_message
