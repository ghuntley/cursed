...
// Targets: update_global_stats in src/stdlib/packages/crypto_signatures/mod.rs:276
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // update_global_stats(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // update_global_stats(data, 0);        // Zero size
        // update_global_stats(data, 1);        // Single byte
        // update_global_stats(data, size);     // Full size
        // update_global_stats(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for update_global_stats
