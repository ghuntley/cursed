...
// Targets: format_json_with_indent in stdlib/json_tea/mod.csd:290
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // format_json_with_indent(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // format_json_with_indent(data, 0);        // Zero size
        // format_json_with_indent(data, 1);        // Single byte
        // format_json_with_indent(data, size);     // Full size
        // format_json_with_indent(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for format_json_with_indent
