...
// Targets: validate_metric_name in src/metrics/prometheus_exporter.rs:786
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // validate_metric_name(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // validate_metric_name(data, 0);        // Zero size
        // validate_metric_name(data, 1);        // Single byte
        // validate_metric_name(data, size);     // Full size
        // validate_metric_name(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for validate_metric_name
