...
// Targets: register_mapping in src/ffi/type_mapping.rs:262
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // register_mapping(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // register_mapping(data, 0);        // Zero size
        // register_mapping(data, 1);        // Single byte
        // register_mapping(data, size);     // Full size
        // register_mapping(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for register_mapping
