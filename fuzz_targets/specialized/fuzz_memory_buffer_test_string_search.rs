...
// Targets: test_string_search in stdlib/string_simple/test_mod.csd:39
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // test_string_search(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // test_string_search(data, 0);        // Zero size
        // test_string_search(data, 1);        // Single byte
        // test_string_search(data, size);     // Full size
        // test_string_search(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for test_string_search
