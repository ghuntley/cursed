...
// Targets: test_constrained_generics in runtime_generic_integration_test.csd:275
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // test_constrained_generics(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // test_constrained_generics(data, 0);        // Zero size
        // test_constrained_generics(data, 1);        // Single byte
        // test_constrained_generics(data, size);     // Full size
        // test_constrained_generics(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for test_constrained_generics
