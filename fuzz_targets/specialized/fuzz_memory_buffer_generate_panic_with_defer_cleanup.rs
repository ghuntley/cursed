...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // generate_panic_with_defer_cleanup(buffer, data, size);
...
void test_string_operations(const uint8_t *data, size_t size) {
    ...
    // generate_panic_with_defer_cleanup(str);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // generate_panic_with_defer_cleanup(data, 0);        // Zero size
        // generate_panic_with_defer_cleanup(data, 1);        // Single byte
        // generate_panic_with_defer_cleanup(data, size);     // Full size
        // generate_panic_with_defer_cleanup(NULL, 0);        // NULL pointer
    }
}
