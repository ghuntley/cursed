...
// Targets: processData in examples/formatter/formatted.csd:71
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // processData(data, 0);        // Zero size
        // processData(data, 1);        // Single byte
        // processData(data, size);     // Full size
        // processData(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for processData
