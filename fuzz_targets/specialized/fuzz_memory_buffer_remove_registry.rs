...
// Targets: remove_registry in src/package_manager/config.rs:246
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // remove_registry(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // remove_registry(data, 0);        // Zero size
        // remove_registry(data, 1);        // Single byte
        // remove_registry(data, size);     // Full size
        // remove_registry(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for remove_registry
