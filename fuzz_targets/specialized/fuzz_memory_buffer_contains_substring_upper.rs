...
// Targets: contains_substring_upper in stdlib/dbz/mod.csd:470
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // contains_substring_upper(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // contains_substring_upper(data, 0);        // Zero size
        // contains_substring_upper(data, 1);        // Single byte
        // contains_substring_upper(data, size);     // Full size
        // contains_substring_upper(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for contains_substring_upper
