...
// Targets: demonstrate_io_operations in examples/stdlib_docs_demo.csd:171
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // demonstrate_io_operations(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // demonstrate_io_operations(data, 0);        // Zero size
        // demonstrate_io_operations(data, 1);        // Single byte
        // demonstrate_io_operations(data, size);     // Full size
        // demonstrate_io_operations(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for demonstrate_io_operations
