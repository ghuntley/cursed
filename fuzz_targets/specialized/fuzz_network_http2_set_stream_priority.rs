...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    // Test packet parsing
    // Test http2_set_stream_priority with network data
    // Example: http2_set_stream_priority(data, size);
    // Example: http2_set_stream_priority(sockfd, data, size);
...
void test_socket_operations(const uint8_t *data, size_t size) {
    ...
    // http2_set_stream_priority(sockfd, data, size);
