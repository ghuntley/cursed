...
// Targets: LastIndexOf in stdlib/stringz/mod_original.csd:265
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // LastIndexOf(data, 0);        // Zero size
        // LastIndexOf(data, 1);        // Single byte
        // LastIndexOf(data, size);     // Full size
        // LastIndexOf(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for LastIndexOf
