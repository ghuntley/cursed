...
// Targets: mark_operands_live in src/codegen/llvm/passes/dead_code_elimination.rs:251
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // mark_operands_live(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // mark_operands_live(data, 0);        // Zero size
        // mark_operands_live(data, 1);        // Single byte
        // mark_operands_live(data, size);     // Full size
        // mark_operands_live(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for mark_operands_live
