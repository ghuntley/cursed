...
// Targets: resizeFn in src-zig/arena_allocator.zig:501
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // resizeFn(data, 0);        // Zero size
        // resizeFn(data, 1);        // Single byte
        // resizeFn(data, size);     // Full size
        // resizeFn(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for resizeFn
