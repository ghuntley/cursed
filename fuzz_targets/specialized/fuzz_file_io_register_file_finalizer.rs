...
// Targets: register_file_finalizer in src/runtime/gc/mod.rs:180
...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    // Test register_file_finalizer with temp file
    // Example: register_file_finalizer(temp_path);
    // Example: register_file_finalizer(temp_path, data, size);
...
void test_path_operations(const uint8_t *data, size_t size) {
    ...
    // register_file_finalizer(path);
}


// Additional test functions for register_file_finalizer
