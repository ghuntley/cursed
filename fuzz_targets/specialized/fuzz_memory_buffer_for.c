...
// Targets: for in runtime/wasm_runtime.c:70
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // for(data, 0);        // Zero size
        // for(data, 1);        // Single byte
        // for(data, size);     // Full size
        // for(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for for
