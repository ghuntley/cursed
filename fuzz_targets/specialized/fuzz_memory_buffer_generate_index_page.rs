...
// Targets: generate_index_page in src/bin/cursed_doc_simple.rs:310
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // generate_index_page(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // generate_index_page(data, 0);        // Zero size
        // generate_index_page(data, 1);        // Single byte
        // generate_index_page(data, size);     // Full size
        // generate_index_page(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for generate_index_page
