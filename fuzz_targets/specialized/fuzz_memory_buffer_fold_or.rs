...
// Targets: fold_or in src/codegen/llvm/passes/constant_propagation.rs:474
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // fold_or(data, 0);        // Zero size
        // fold_or(data, 1);        // Single byte
        // fold_or(data, size);     // Full size
        // fold_or(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for fold_or
