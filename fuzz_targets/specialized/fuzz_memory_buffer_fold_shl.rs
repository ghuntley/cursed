...
// Targets: fold_shl in src/codegen/llvm/passes/constant_propagation.rs:589
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // fold_shl(data, 0);        // Zero size
        // fold_shl(data, 1);        // Single byte
        // fold_shl(data, size);     // Full size
        // fold_shl(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for fold_shl
