...
// Targets: inspect_variable in src/runtime/debug_runtime.rs:1054
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // inspect_variable(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // inspect_variable(data, 0);        // Zero size
        // inspect_variable(data, 1);        // Single byte
        // inspect_variable(data, size);     // Full size
        // inspect_variable(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for inspect_variable
