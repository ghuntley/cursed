...
// Targets: get_algorithm in src/stdlib/packages/crypto_signatures/mod.rs:158
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // get_algorithm(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // get_algorithm(data, 0);        // Zero size
        // get_algorithm(data, 1);        // Single byte
        // get_algorithm(data, size);     // Full size
        // get_algorithm(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for get_algorithm
