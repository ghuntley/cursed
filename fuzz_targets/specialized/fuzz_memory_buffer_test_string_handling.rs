...
// Targets: test_string_handling in src/repl/advanced_multi_line_editor.rs:457
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // test_string_handling(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // test_string_handling(data, 0);        // Zero size
        // test_string_handling(data, 1);        // Single byte
        // test_string_handling(data, size);     // Full size
        // test_string_handling(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for test_string_handling
