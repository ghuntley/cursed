...
// Targets: RuntimeStringBuilder_to_string in stdlib/runtime_core/mod.csd:171
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // RuntimeStringBuilder_to_string(buffer, data, size);
...
void test_string_operations(const uint8_t *data, size_t size) {
    ...
    // RuntimeStringBuilder_to_string(str);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // RuntimeStringBuilder_to_string(data, 0);        // Zero size
        // RuntimeStringBuilder_to_string(data, 1);        // Single byte
        // RuntimeStringBuilder_to_string(data, size);     // Full size
        // RuntimeStringBuilder_to_string(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for RuntimeStringBuilder_to_string
