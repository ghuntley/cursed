...
// Targets: trigger_global_test_panic in src/runtime/panic.rs:624
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // trigger_global_test_panic(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // trigger_global_test_panic(data, 0);        // Zero size
        // trigger_global_test_panic(data, 1);        // Single byte
        // trigger_global_test_panic(data, size);     // Full size
        // trigger_global_test_panic(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for trigger_global_test_panic
