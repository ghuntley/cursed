...
// Targets: intern in src/runtime/advanced_memory_optimizer.rs:556
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // intern(data, 0);        // Zero size
        // intern(data, 1);        // Single byte
        // intern(data, size);     // Full size
        // intern(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for intern
