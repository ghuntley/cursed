...
// Targets: context_value_from_str in src/runtime/error_context.rs:755
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // context_value_from_str(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // context_value_from_str(data, 0);        // Zero size
        // context_value_from_str(data, 1);        // Single byte
        // context_value_from_str(data, size);     // Full size
        // context_value_from_str(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for context_value_from_str
