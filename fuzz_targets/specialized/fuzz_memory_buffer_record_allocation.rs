...
// Targets: record_allocation in src/runtime/memory_profiler.rs:568
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // record_allocation(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // record_allocation(data, 0);        // Zero size
        // record_allocation(data, 1);        // Single byte
        // record_allocation(data, size);     // Full size
        // record_allocation(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for record_allocation
