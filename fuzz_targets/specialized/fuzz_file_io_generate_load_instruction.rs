...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    // Test generate_load_instruction with temp file
    // Example: generate_load_instruction(temp_path);
    // Example: generate_load_instruction(temp_path, data, size);
...
void test_path_operations(const uint8_t *data, size_t size) {
    ...
    // generate_load_instruction(path);
}
