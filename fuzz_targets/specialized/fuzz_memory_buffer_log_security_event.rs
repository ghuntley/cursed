...
// Targets: log_security_event in src/security/mod.rs:100
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // log_security_event(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // log_security_event(data, 0);        // Zero size
        // log_security_event(data, 1);        // Single byte
        // log_security_event(data, size);     // Full size
        // log_security_event(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for log_security_event
