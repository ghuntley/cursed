...
// Targets: string_array_contains in stdlib/arrayz/mod.csd:238
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // string_array_contains(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // string_array_contains(data, 0);        // Zero size
        // string_array_contains(data, 1);        // Single byte
        // string_array_contains(data, size);     // Full size
        // string_array_contains(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for string_array_contains
