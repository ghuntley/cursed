...
// Targets: parse_functions in src/ffi/header_parser.rs:222
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // parse_functions(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // parse_functions(data, 0);        // Zero size
        // parse_functions(data, 1);        // Single byte
        // parse_functions(data, size);     // Full size
        // parse_functions(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for parse_functions
