...
// Targets: detect_hardware_capability in src/runtime/pal/common.rs:326
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // detect_hardware_capability(buffer, data, size);
...
void test_string_operations(const uint8_t *data, size_t size) {
    ...
    // detect_hardware_capability(str);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // detect_hardware_capability(data, 0);        // Zero size
        // detect_hardware_capability(data, 1);        // Single byte
        // detect_hardware_capability(data, size);     // Full size
        // detect_hardware_capability(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for detect_hardware_capability
