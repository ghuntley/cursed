...
// Targets: deserialize_string in stdlib/serialization/mod.csd:86
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // deserialize_string(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // deserialize_string(data, 0);        // Zero size
        // deserialize_string(data, 1);        // Single byte
        // deserialize_string(data, size);     // Full size
        // deserialize_string(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for deserialize_string
