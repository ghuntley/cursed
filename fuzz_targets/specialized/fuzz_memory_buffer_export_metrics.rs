...
// Targets: export_metrics in src/bin/cursed_metrics.rs:213
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // export_metrics(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // export_metrics(data, 0);        // Zero size
        // export_metrics(data, 1);        // Single byte
        // export_metrics(data, size);     // Full size
        // export_metrics(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for export_metrics
