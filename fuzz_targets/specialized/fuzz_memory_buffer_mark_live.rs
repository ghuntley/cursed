...
// Targets: mark_live in src/codegen/llvm/passes/dead_code_elimination.rs:244
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // mark_live(data, 0);        // Zero size
        // mark_live(data, 1);        // Single byte
        // mark_live(data, size);     // Full size
        // mark_live(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for mark_live
