...
// Targets: preview_highlighting in src/repl/advanced_syntax_highlighter.rs:381
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // preview_highlighting(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // preview_highlighting(data, 0);        // Zero size
        // preview_highlighting(data, 1);        // Single byte
        // preview_highlighting(data, size);     // Full size
        // preview_highlighting(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for preview_highlighting
