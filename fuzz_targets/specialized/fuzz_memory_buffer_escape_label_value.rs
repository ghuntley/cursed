...
// Targets: escape_label_value in src/metrics/prometheus_exporter.rs:744
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // escape_label_value(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // escape_label_value(data, 0);        // Zero size
        // escape_label_value(data, 1);        // Single byte
        // escape_label_value(data, size);     // Full size
        // escape_label_value(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for escape_label_value
