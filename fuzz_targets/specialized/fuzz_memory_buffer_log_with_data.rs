...
// Targets: log_with_data in src/runtime/debug_output.rs:390
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // log_with_data(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // log_with_data(data, 0);        // Zero size
        // log_with_data(data, 1);        // Single byte
        // log_with_data(data, size);     // Full size
        // log_with_data(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for log_with_data
