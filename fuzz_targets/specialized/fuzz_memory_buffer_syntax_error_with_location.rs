...
// Targets: syntax_error_with_location in src/error/types.rs:85
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // syntax_error_with_location(buffer, data, size);
...
void test_string_operations(const uint8_t *data, size_t size) {
    ...
    // syntax_error_with_location(str);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // syntax_error_with_location(data, 0);        // Zero size
        // syntax_error_with_location(data, 1);        // Single byte
        // syntax_error_with_location(data, size);     // Full size
        // syntax_error_with_location(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for syntax_error_with_location
