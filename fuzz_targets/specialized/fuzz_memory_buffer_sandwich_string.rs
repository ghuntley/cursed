...
// Targets: sandwich_string in stdlib/stringz/mod.csd:144
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // sandwich_string(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // sandwich_string(data, 0);        // Zero size
        // sandwich_string(data, 1);        // Single byte
        // sandwich_string(data, size);     // Full size
        // sandwich_string(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for sandwich_string
