...
// Targets: validate_argument_types in src/ffi/safety_checks.rs:104
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // validate_argument_types(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // validate_argument_types(data, 0);        // Zero size
        // validate_argument_types(data, 1);        // Single byte
        // validate_argument_types(data, size);     // Full size
        // validate_argument_types(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for validate_argument_types
