...
// Targets: ast_node_to_string in stdlib/ast_mood/mod.csd:425
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // ast_node_to_string(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // ast_node_to_string(data, 0);        // Zero size
        // ast_node_to_string(data, 1);        // Single byte
        // ast_node_to_string(data, size);     // Full size
        // ast_node_to_string(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for ast_node_to_string
