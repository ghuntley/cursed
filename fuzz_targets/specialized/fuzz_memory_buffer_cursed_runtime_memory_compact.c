...
// Targets: cursed_runtime_memory_compact in runtime/memory_runtime.c:268
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // cursed_runtime_memory_compact(buffer, data, size);
...
void test_string_operations(const uint8_t *data, size_t size) {
    ...
    // cursed_runtime_memory_compact(str);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // cursed_runtime_memory_compact(data, 0);        // Zero size
        // cursed_runtime_memory_compact(data, 1);        // Single byte
        // cursed_runtime_memory_compact(data, size);     // Full size
        // cursed_runtime_memory_compact(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for cursed_runtime_memory_compact
