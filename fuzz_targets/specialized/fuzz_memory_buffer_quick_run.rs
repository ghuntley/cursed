...
// Targets: quick_run in src/tools/mod.rs:256
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // quick_run(data, 0);        // Zero size
        // quick_run(data, 1);        // Single byte
        // quick_run(data, size);     // Full size
        // quick_run(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for quick_run
