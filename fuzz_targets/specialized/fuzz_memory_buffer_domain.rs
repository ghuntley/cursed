...
// Targets: domain in src/stdlib/net/http/cookies.rs:30
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // domain(data, 0);        // Zero size
        // domain(data, 1);        // Single byte
        // domain(data, size);     // Full size
        // domain(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for domain
