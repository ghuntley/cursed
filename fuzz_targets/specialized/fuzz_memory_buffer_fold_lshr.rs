...
// Targets: fold_lshr in src/codegen/llvm/passes/constant_propagation.rs:605
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // fold_lshr(data, 0);        // Zero size
        // fold_lshr(data, 1);        // Single byte
        // fold_lshr(data, size);     // Full size
        // fold_lshr(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for fold_lshr
