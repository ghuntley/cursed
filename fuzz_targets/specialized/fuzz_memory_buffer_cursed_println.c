...
// Targets: cursed_println in runtime/minimal_shims.c:28
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // cursed_println(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // cursed_println(data, 0);        // Zero size
        // cursed_println(data, 1);        // Single byte
        // cursed_println(data, size);     // Full size
        // cursed_println(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for cursed_println
