...
// Targets: demonstrate_message_queue in examples/type_switch_practical_use_cases.csd:394
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // demonstrate_message_queue(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // demonstrate_message_queue(data, 0);        // Zero size
        // demonstrate_message_queue(data, 1);        // Single byte
        // demonstrate_message_queue(data, size);     // Full size
        // demonstrate_message_queue(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for demonstrate_message_queue
