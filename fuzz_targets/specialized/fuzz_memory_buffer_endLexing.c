...
// Targets: endLexing in src-zig/performance_integration.zig:220
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // endLexing(data, 0);        // Zero size
        // endLexing(data, 1);        // Single byte
        // endLexing(data, size);     // Full size
        // endLexing(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for endLexing
