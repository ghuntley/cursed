...
// Targets: startEvaluation in src-zig/performance_integration.zig:232
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // startEvaluation(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // startEvaluation(data, 0);        // Zero size
        // startEvaluation(data, 1);        // Single byte
        // startEvaluation(data, size);     // Full size
        // startEvaluation(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for startEvaluation
