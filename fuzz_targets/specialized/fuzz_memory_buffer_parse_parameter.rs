...
// Targets: parse_parameter in src/ffi/header_parser.rs:313
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // parse_parameter(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // parse_parameter(data, 0);        // Zero size
        // parse_parameter(data, 1);        // Single byte
        // parse_parameter(data, size);     // Full size
        // parse_parameter(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for parse_parameter
