...
// Targets: isAllocatedString in src-zig/cross_compilation.zig:151
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // isAllocatedString(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // isAllocatedString(data, 0);        // Zero size
        // isAllocatedString(data, 1);        // Single byte
        // isAllocatedString(data, size);     // Full size
        // isAllocatedString(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for isAllocatedString
