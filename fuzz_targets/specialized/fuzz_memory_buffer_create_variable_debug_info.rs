...
// Targets: create_variable_debug_info in src/debug/enhanced_debug.rs:502
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // create_variable_debug_info(buffer, data, size);
...
void test_string_operations(const uint8_t *data, size_t size) {
    ...
    // create_variable_debug_info(str);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // create_variable_debug_info(data, 0);        // Zero size
        // create_variable_debug_info(data, 1);        // Single byte
        // create_variable_debug_info(data, size);     // Full size
        // create_variable_debug_info(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for create_variable_debug_info
