...
// Targets: format_channel_operations in src/formatter/simple.rs:520
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // format_channel_operations(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // format_channel_operations(data, 0);        // Zero size
        // format_channel_operations(data, 1);        // Single byte
        // format_channel_operations(data, size);     // Full size
        // format_channel_operations(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for format_channel_operations
