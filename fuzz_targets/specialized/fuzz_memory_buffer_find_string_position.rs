...
// Targets: find_string_position in stdlib/configz/mod_complex_syntax.csd:626
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // find_string_position(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // find_string_position(data, 0);        // Zero size
        // find_string_position(data, 1);        // Single byte
        // find_string_position(data, size);     // Full size
        // find_string_position(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for find_string_position
