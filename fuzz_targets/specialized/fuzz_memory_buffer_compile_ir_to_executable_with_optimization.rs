...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // compile_ir_to_executable_with_optimization(buffer, data, size);
...
void test_string_operations(const uint8_t *data, size_t size) {
    ...
    // compile_ir_to_executable_with_optimization(str);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // compile_ir_to_executable_with_optimization(data, 0);        // Zero size
        // compile_ir_to_executable_with_optimization(data, 1);        // Single byte
        // compile_ir_to_executable_with_optimization(data, size);     // Full size
        // compile_ir_to_executable_with_optimization(NULL, 0);        // NULL pointer
    }
}
