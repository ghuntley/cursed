...
// Targets: runtime_allocate_memory in src-zig/runtime_functions.zig:295
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // runtime_allocate_memory(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // runtime_allocate_memory(data, 0);        // Zero size
        // runtime_allocate_memory(data, 1);        // Single byte
        // runtime_allocate_memory(data, size);     // Full size
        // runtime_allocate_memory(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for runtime_allocate_memory
