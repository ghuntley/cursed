...
// Targets: getUsage in src-zig/arena_allocator.zig:463
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // getUsage(data, 0);        // Zero size
        // getUsage(data, 1);        // Single byte
        // getUsage(data, size);     // Full size
        // getUsage(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for getUsage
