...
// Targets: is_type in src/runtime/runtime_value.rs:151
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // is_type(data, 0);        // Zero size
        // is_type(data, 1);        // Single byte
        // is_type(data, size);     // Full size
        // is_type(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for is_type
