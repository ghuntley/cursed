...
// Targets: send_with_priority in src/runtime/channels/production_channel.rs:718
...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    // Test packet parsing
    // Test send_with_priority with network data
    // Example: send_with_priority(data, size);
    // Example: send_with_priority(sockfd, data, size);
...
void test_socket_operations(const uint8_t *data, size_t size) {
    ...
    // send_with_priority(sockfd, data, size);
    
    close(sockfd);
}


// Additional test functions for send_with_priority
