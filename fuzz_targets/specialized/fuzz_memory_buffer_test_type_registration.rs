...
// Targets: test_type_registration in src/types/full.rs:531
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // test_type_registration(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // test_type_registration(data, 0);        // Zero size
        // test_type_registration(data, 1);        // Single byte
        // test_type_registration(data, size);     // Full size
        // test_type_registration(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for test_type_registration
