...
// Targets: is_valid_email in examples/docs_generation_demo.csd:269
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // is_valid_email(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // is_valid_email(data, 0);        // Zero size
        // is_valid_email(data, 1);        // Single byte
        // is_valid_email(data, size);     // Full size
        // is_valid_email(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for is_valid_email
