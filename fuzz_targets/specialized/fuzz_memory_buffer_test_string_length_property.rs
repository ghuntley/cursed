...
// Targets: test_string_length_property in examples/quick_test_demo.csd:93
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // test_string_length_property(buffer, data, size);
...
void test_string_operations(const uint8_t *data, size_t size) {
    ...
    // test_string_length_property(str);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // test_string_length_property(data, 0);        // Zero size
        // test_string_length_property(data, 1);        // Single byte
        // test_string_length_property(data, size);     // Full size
        // test_string_length_property(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for test_string_length_property
