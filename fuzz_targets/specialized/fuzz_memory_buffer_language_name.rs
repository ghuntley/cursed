...
// Targets: language_name in src/ffi/multi_language.rs:549
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // language_name(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // language_name(data, 0);        // Zero size
        // language_name(data, 1);        // Single byte
        // language_name(data, size);     // Full size
        // language_name(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for language_name
