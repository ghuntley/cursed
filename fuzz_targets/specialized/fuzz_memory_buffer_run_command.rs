...
// Targets: run_command in src/performance/benchmark.rs:565
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // run_command(data, 0);        // Zero size
        // run_command(data, 1);        // Single byte
        // run_command(data, size);     // Full size
        // run_command(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for run_command
