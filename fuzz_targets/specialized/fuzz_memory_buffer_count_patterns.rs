...
// Targets: count_patterns in examples/cursed_optimization_demo.rs:234
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // count_patterns(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // count_patterns(data, 0);        // Zero size
        // count_patterns(data, 1);        // Single byte
        // count_patterns(data, size);     // Full size
        // count_patterns(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for count_patterns
