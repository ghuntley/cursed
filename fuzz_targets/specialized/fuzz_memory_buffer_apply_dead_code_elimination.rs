...
// Targets: apply_dead_code_elimination in src/codegen/llvm/generic_optimization.rs:252
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // apply_dead_code_elimination(buffer, data, size);
...
void test_string_operations(const uint8_t *data, size_t size) {
    ...
    // apply_dead_code_elimination(str);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // apply_dead_code_elimination(data, 0);        // Zero size
        // apply_dead_code_elimination(data, 1);        // Single byte
        // apply_dead_code_elimination(data, size);     // Full size
        // apply_dead_code_elimination(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for apply_dead_code_elimination
