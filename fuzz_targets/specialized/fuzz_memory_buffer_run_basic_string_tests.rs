...
// Targets: run_basic_string_tests in stdlib/test_all_stdlib.csd:133
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // run_basic_string_tests(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // run_basic_string_tests(data, 0);        // Zero size
        // run_basic_string_tests(data, 1);        // Single byte
        // run_basic_string_tests(data, size);     // Full size
        // run_basic_string_tests(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for run_basic_string_tests
