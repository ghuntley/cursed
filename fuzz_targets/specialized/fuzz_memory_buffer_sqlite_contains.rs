...
// Targets: sqlite_contains in stdlib/database_drivers/sqlite.csd:380
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // sqlite_contains(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // sqlite_contains(data, 0);        // Zero size
        // sqlite_contains(data, 1);        // Single byte
        // sqlite_contains(data, size);     // Full size
        // sqlite_contains(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for sqlite_contains
