...
// Targets: string_starts_with_prefix in stdlib/configz/mod_complex_syntax.csd:645
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // string_starts_with_prefix(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // string_starts_with_prefix(data, 0);        // Zero size
        // string_starts_with_prefix(data, 1);        // Single byte
        // string_starts_with_prefix(data, size);     // Full size
        // string_starts_with_prefix(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for string_starts_with_prefix
