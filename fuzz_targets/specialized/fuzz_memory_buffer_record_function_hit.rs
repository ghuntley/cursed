...
// Targets: record_function_hit in src/coverage/simple_collector.rs:374
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // record_function_hit(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // record_function_hit(data, 0);        // Zero size
        // record_function_hit(data, 1);        // Single byte
        // record_function_hit(data, size);     // Full size
        // record_function_hit(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for record_function_hit
