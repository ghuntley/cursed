...
// Targets: background_worker in examples/stdlib_docs_demo.csd:308
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // background_worker(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // background_worker(data, 0);        // Zero size
        // background_worker(data, 1);        // Single byte
        // background_worker(data, size);     // Full size
        // background_worker(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for background_worker
