...
// Targets: is_variable_mutable in src/runtime/debug_manager.rs:622
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // is_variable_mutable(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // is_variable_mutable(data, 0);        // Zero size
        // is_variable_mutable(data, 1);        // Single byte
        // is_variable_mutable(data, size);     // Full size
        // is_variable_mutable(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for is_variable_mutable
