...
// Targets: download_with_retries in src/package_manager/downloader.rs:175
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // download_with_retries(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // download_with_retries(data, 0);        // Zero size
        // download_with_retries(data, 1);        // Single byte
        // download_with_retries(data, size);     // Full size
        // download_with_retries(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for download_with_retries
