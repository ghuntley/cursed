...
// Targets: io_copy_file in src/execution/runtime_functions.rs:909
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // io_copy_file(data, 0);        // Zero size
        // io_copy_file(data, 1);        // Single byte
        // io_copy_file(data, size);     // Full size
        // io_copy_file(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for io_copy_file
