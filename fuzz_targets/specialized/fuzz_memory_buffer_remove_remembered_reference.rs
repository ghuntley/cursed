...
// Targets: remove_remembered_reference in src/memory/generational.rs:306
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // remove_remembered_reference(buffer, data, size);
...
void test_string_operations(const uint8_t *data, size_t size) {
    ...
    // remove_remembered_reference(str);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // remove_remembered_reference(data, 0);        // Zero size
        // remove_remembered_reference(data, 1);        // Single byte
        // remove_remembered_reference(data, size);     // Full size
        // remove_remembered_reference(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for remove_remembered_reference
