...
// Targets: runtime_get_string_byte_at in stdlib/string_simple/mod.csd:949
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // runtime_get_string_byte_at(buffer, data, size);
...
void test_string_operations(const uint8_t *data, size_t size) {
    ...
    // runtime_get_string_byte_at(str);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // runtime_get_string_byte_at(data, 0);        // Zero size
        // runtime_get_string_byte_at(data, 1);        // Single byte
        // runtime_get_string_byte_at(data, size);     // Full size
        // runtime_get_string_byte_at(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for runtime_get_string_byte_at
