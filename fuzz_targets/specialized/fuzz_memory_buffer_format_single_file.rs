...
// Targets: format_single_file in src/formatter/simple.rs:681
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // format_single_file(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // format_single_file(data, 0);        // Zero size
        // format_single_file(data, 1);        // Single byte
        // format_single_file(data, size);     // Full size
        // format_single_file(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for format_single_file
