...
// Targets: format_struct in stdlib/formatter/mod.csd:1126
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // format_struct(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // format_struct(data, 0);        // Zero size
        // format_struct(data, 1);        // Single byte
        // format_struct(data, size);     // Full size
        // format_struct(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for format_struct
