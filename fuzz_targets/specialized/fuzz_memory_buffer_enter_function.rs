...
// Targets: enter_function in src/codegen/llvm/debug_integration.rs:63
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // enter_function(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // enter_function(data, 0);        // Zero size
        // enter_function(data, 1);        // Single byte
        // enter_function(data, size);     // Full size
        // enter_function(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for enter_function
