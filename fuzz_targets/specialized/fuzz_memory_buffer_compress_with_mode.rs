...
// Targets: compress_with_mode in src/stdlib/squish_core/enhanced.rs:14
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // compress_with_mode(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // compress_with_mode(data, 0);        // Zero size
        // compress_with_mode(data, 1);        // Single byte
        // compress_with_mode(data, size);     // Full size
        // compress_with_mode(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for compress_with_mode
