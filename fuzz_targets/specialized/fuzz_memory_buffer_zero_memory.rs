...
// Targets: zero_memory in stdlib/memoryz/mod.csd:366
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // zero_memory(data, 0);        // Zero size
        // zero_memory(data, 1);        // Single byte
        // zero_memory(data, size);     // Full size
        // zero_memory(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for zero_memory
