...
// Targets: watch_variable in src/bin/cursed_debug.rs:520
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // watch_variable(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // watch_variable(data, 0);        // Zero size
        // watch_variable(data, 1);        // Single byte
        // watch_variable(data, size);     // Full size
        // watch_variable(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for watch_variable
