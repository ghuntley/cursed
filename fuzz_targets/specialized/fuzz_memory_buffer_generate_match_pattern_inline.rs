...
// Targets: generate_match_pattern_inline in src/codegen/llvm/expression_compiler.rs:1549
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // generate_match_pattern_inline(buffer, data, size);
...
void test_string_operations(const uint8_t *data, size_t size) {
    ...
    // generate_match_pattern_inline(str);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // generate_match_pattern_inline(data, 0);        // Zero size
        // generate_match_pattern_inline(data, 1);        // Single byte
        // generate_match_pattern_inline(data, size);     // Full size
        // generate_match_pattern_inline(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for generate_match_pattern_inline
