...
// Targets: match_whitespace_and_keyword in src/lexer/mod.rs:416
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // match_whitespace_and_keyword(buffer, data, size);
...
void test_string_operations(const uint8_t *data, size_t size) {
    ...
    // match_whitespace_and_keyword(str);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // match_whitespace_and_keyword(data, 0);        // Zero size
        // match_whitespace_and_keyword(data, 1);        // Single byte
        // match_whitespace_and_keyword(data, size);     // Full size
        // match_whitespace_and_keyword(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for match_whitespace_and_keyword
