...
// Targets: record_stage_time in src/codegen/llvm/enhanced_optimization.rs:656
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // record_stage_time(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // record_stage_time(data, 0);        // Zero size
        // record_stage_time(data, 1);        // Single byte
        // record_stage_time(data, size);     // Full size
        // record_stage_time(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for record_stage_time
