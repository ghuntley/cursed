...
// Targets: build_alternation in stdlib/regex/mod.csd:616
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // build_alternation(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // build_alternation(data, 0);        // Zero size
        // build_alternation(data, 1);        // Single byte
        // build_alternation(data, size);     // Full size
        // build_alternation(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for build_alternation
