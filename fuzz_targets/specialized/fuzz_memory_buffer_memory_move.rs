...
// Targets: memory_move in stdlib/memory/utils.csd:131
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // memory_move(data, 0);        // Zero size
        // memory_move(data, 1);        // Single byte
        // memory_move(data, size);     // Full size
        // memory_move(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for memory_move
