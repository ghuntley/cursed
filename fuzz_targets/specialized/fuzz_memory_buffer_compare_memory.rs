...
// Targets: compare_memory in stdlib/memoryz/mod.csd:378
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // compare_memory(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // compare_memory(data, 0);        // Zero size
        // compare_memory(data, 1);        // Single byte
        // compare_memory(data, size);     // Full size
        // compare_memory(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for compare_memory
