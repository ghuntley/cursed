...
// Targets: parse_with_recovery_step in src/error_recovery_simple.rs:200
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // parse_with_recovery_step(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // parse_with_recovery_step(data, 0);        // Zero size
        // parse_with_recovery_step(data, 1);        // Single byte
        // parse_with_recovery_step(data, size);     // Full size
        // parse_with_recovery_step(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for parse_with_recovery_step
