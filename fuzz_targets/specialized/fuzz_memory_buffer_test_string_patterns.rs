...
// Targets: test_string_patterns in examples/testing_framework_demo.csd:171
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // test_string_patterns(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // test_string_patterns(data, 0);        // Zero size
        // test_string_patterns(data, 1);        // Single byte
        // test_string_patterns(data, size);     // Full size
        // test_string_patterns(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for test_string_patterns
