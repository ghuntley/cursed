...
// Targets: load_directory_as_map in src/stdlib/embed_that/mod.rs:130
...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    // Test load_directory_as_map with temp file
    // Example: load_directory_as_map(temp_path);
    // Example: load_directory_as_map(temp_path, data, size);
...
void test_path_operations(const uint8_t *data, size_t size) {
    ...
    // load_directory_as_map(path);
}


// Additional test functions for load_directory_as_map
