...
// Targets: parseCType in src-zig/extern_abi.zig:339
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // parseCType(data, 0);        // Zero size
        // parseCType(data, 1);        // Single byte
        // parseCType(data, size);     // Full size
        // parseCType(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for parseCType
