...
// Targets: build_sub_instruction in src/codegen/llvm/inkwell_expression_compiler.rs:283
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // build_sub_instruction(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // build_sub_instruction(data, 0);        // Zero size
        // build_sub_instruction(data, 1);        // Single byte
        // build_sub_instruction(data, size);     // Full size
        // build_sub_instruction(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for build_sub_instruction
