...
// Targets: generate_index in src/bin/cursed_doc_basic.rs:186
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // generate_index(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // generate_index(data, 0);        // Zero size
        // generate_index(data, 1);        // Single byte
        // generate_index(data, size);     // Full size
        // generate_index(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for generate_index
