...
// Targets: compare_strings in stdlib/stringz/mod.csd:607
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // compare_strings(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // compare_strings(data, 0);        // Zero size
        // compare_strings(data, 1);        // Single byte
        // compare_strings(data, size);     // Full size
        // compare_strings(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for compare_strings
