...
// Targets: CopyData in stdlib/slay_io/mod.csd:213
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // CopyData(data, 0);        // Zero size
        // CopyData(data, 1);        // Single byte
        // CopyData(data, size);     // Full size
        // CopyData(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for CopyData
