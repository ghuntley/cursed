...
// Targets: get_variant_index in src/pattern_matching.rs:40
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // get_variant_index(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // get_variant_index(data, 0);        // Zero size
        // get_variant_index(data, 1);        // Single byte
        // get_variant_index(data, size);     // Full size
        // get_variant_index(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for get_variant_index
