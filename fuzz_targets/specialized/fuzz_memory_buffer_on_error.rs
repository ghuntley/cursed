...
// Targets: on_error in src/runtime/performance_hooks.rs:612
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // on_error(data, 0);        // Zero size
        // on_error(data, 1);        // Single byte
        // on_error(data, size);     // Full size
        // on_error(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for on_error
