...
// Targets: remove_unused_functions in src/codegen/llvm/generic_optimization.rs:404
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // remove_unused_functions(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // remove_unused_functions(data, 0);        // Zero size
        // remove_unused_functions(data, 1);        // Single byte
        // remove_unused_functions(data, size);     // Full size
        // remove_unused_functions(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for remove_unused_functions
