...
// Targets: test_core_exports_available in tests/test_lib_exports.rs:14
...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    // Test test_core_exports_available with temp file
    // Example: test_core_exports_available(temp_path);
    // Example: test_core_exports_available(temp_path, data, size);
...
void test_path_operations(const uint8_t *data, size_t size) {
    ...
    // test_core_exports_available(path);
}


// Additional test functions for test_core_exports_available
