...
// Targets: parseProfileMode in build_optimized.zig:482
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // parseProfileMode(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // parseProfileMode(data, 0);        // Zero size
        // parseProfileMode(data, 1);        // Single byte
        // parseProfileMode(data, size);     // Full size
        // parseProfileMode(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for parseProfileMode
