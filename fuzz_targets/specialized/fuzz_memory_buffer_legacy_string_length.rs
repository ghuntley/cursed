...
// Targets: legacy_string_length in stdlib/self_hosted_builtins/mod.csd:425
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // legacy_string_length(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // legacy_string_length(data, 0);        // Zero size
        // legacy_string_length(data, 1);        // Single byte
        // legacy_string_length(data, size);     // Full size
        // legacy_string_length(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for legacy_string_length
