...
// Targets: io_buffer_flush in src/execution/runtime_functions.rs:1373
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // io_buffer_flush(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // io_buffer_flush(data, 0);        // Zero size
        // io_buffer_flush(data, 1);        // Single byte
        // io_buffer_flush(data, size);     // Full size
        // io_buffer_flush(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for io_buffer_flush
