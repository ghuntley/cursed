...
// Targets: classify_word in src/lsp/semantic_highlighting.rs:316
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // classify_word(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // classify_word(data, 0);        // Zero size
        // classify_word(data, 1);        // Single byte
        // classify_word(data, size);     // Full size
        // classify_word(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for classify_word
