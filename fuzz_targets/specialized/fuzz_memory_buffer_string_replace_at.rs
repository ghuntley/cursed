...
// Targets: string_replace_at in stdlib/vibez/mod_enhanced.csd:321
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // string_replace_at(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // string_replace_at(data, 0);        // Zero size
        // string_replace_at(data, 1);        // Single byte
        // string_replace_at(data, size);     // Full size
        // string_replace_at(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for string_replace_at
