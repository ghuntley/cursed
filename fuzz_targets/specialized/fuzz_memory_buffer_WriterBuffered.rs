...
// Targets: WriterBuffered in stdlib/slay_io/mod.csd:101
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // WriterBuffered(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // WriterBuffered(data, 0);        // Zero size
        // WriterBuffered(data, 1);        // Single byte
        // WriterBuffered(data, size);     // Full size
        // WriterBuffered(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for WriterBuffered
