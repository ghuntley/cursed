...
// Targets: add_variable in src/codegen/llvm/debug_integration.rs:82
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // add_variable(data, 0);        // Zero size
        // add_variable(data, 1);        // Single byte
        // add_variable(data, size);     // Full size
        // add_variable(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for add_variable
