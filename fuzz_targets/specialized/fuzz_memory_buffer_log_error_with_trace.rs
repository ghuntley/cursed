...
// Targets: log_error_with_trace in src/runtime/debug_output.rs:1060
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // log_error_with_trace(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // log_error_with_trace(data, 0);        // Zero size
        // log_error_with_trace(data, 1);        // Single byte
        // log_error_with_trace(data, size);     // Full size
        // log_error_with_trace(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for log_error_with_trace
