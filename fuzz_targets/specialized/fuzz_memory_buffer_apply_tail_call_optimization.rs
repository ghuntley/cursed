...
// Targets: apply_tail_call_optimization in src/codegen/llvm/passes/tail_call.rs:132
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // apply_tail_call_optimization(buffer, data, size);
...
void test_string_operations(const uint8_t *data, size_t size) {
    ...
    // apply_tail_call_optimization(str);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // apply_tail_call_optimization(data, 0);        // Zero size
        // apply_tail_call_optimization(data, 1);        // Single byte
        // apply_tail_call_optimization(data, size);     // Full size
        // apply_tail_call_optimization(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for apply_tail_call_optimization
