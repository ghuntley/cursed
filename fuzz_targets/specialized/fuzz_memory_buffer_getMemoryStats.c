...
// Targets: getMemoryStats in src-zig/safe_operations.zig:115
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // getMemoryStats(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // getMemoryStats(data, 0);        // Zero size
        // getMemoryStats(data, 1);        // Single byte
        // getMemoryStats(data, size);     // Full size
        // getMemoryStats(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for getMemoryStats
