...
// Targets: into_send_only in src/runtime/channels/channel_direction.rs:89
...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    // Test packet parsing
    // Test into_send_only with network data
    // Example: into_send_only(data, size);
    // Example: into_send_only(sockfd, data, size);
...
void test_socket_operations(const uint8_t *data, size_t size) {
    ...
    // into_send_only(sockfd, data, size);
    
    close(sockfd);
}


// Additional test functions for into_send_only
