...
// Targets: compile_snippet in src/repl/types.rs:103
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // compile_snippet(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // compile_snippet(data, 0);        // Zero size
        // compile_snippet(data, 1);        // Single byte
        // compile_snippet(data, size);     // Full size
        // compile_snippet(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for compile_snippet
