...
// Targets: create_member_access_node in stdlib/ast_mood/mod.csd:102
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // create_member_access_node(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // create_member_access_node(data, 0);        // Zero size
        // create_member_access_node(data, 1);        // Single byte
        // create_member_access_node(data, size);     // Full size
        // create_member_access_node(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for create_member_access_node
