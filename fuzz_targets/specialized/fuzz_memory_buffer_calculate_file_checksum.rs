...
// Targets: calculate_file_checksum in src/tools/debug_info.rs:651
...
void test_buffer_operations(const uint8_t *data, size_t size) {
    ...
    // calculate_file_checksum(buffer, data, size);
...
void test_boundary_conditions(const uint8_t *data, size_t size) {
    // Test edge cases
    if (size > 0) {
        // calculate_file_checksum(data, 0);        // Zero size
        // calculate_file_checksum(data, 1);        // Single byte
        // calculate_file_checksum(data, size);     // Full size
        // calculate_file_checksum(NULL, 0);        // NULL pointer
    }
}


// Additional test functions for calculate_file_checksum
