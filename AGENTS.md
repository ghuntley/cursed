...
1. **Package Clause**: Every `.csd` file MUST start with `vibe <package_name>`
...
- **Stdlib modules must also follow proper structure**: Each stdlib module's `mod.csd` file must start with `vibe <module_name>`
...
- Test files should be named descriptively: `validation_basic_syntax.csd`, `feature_stdlib_imports.csd`
...
#### Interpreter Mode
```bash
./zig-out/bin/cursed-compiler --interpret <file.csd>
```

#### Compiled Mode
```bash
# Compile CURSED code to native binary
./zig-out/bin/cursed-compiler --compile <file.csd> -o <output_binary>
...
./zig-out/bin/cursed-compiler --compile <file.csd> -o <binary> --emit-ir

# Verbose output
./zig-out/bin/cursed-compiler --interpret <file.csd> --verbose
```
...
- Use descriptive names: `test_parser_arithmetic.csd`, `test_stdlib_mathz.csd`
- Include purpose in filename: `validation_`, `regression_`, `feature_`
- Use `.csd` extension for CURSED source files
...
1. **Write the test**: Create a `.csd` file with correct CURSED syntax
...
### Standard Library
- `stdlib/*/mod.csd` - CURSED standard library implementations
...
- IMPORTANT: always check that programs authored in CURSED (*.csd) are valid cursed programs as per the programming language specifications (specs/*.md) before making changes to the compiler by passing the specifications and the program over to the oracle for analysis.
