...
mod tests {
    ...
    fn test_default_config() {
        ...
        assert!(config.has_debug_info());
        assert!(config.has_line_info());
    }

    #[test]
    fn test_preset_configs() {
        let none = DebugConfig::none();
        assert!(!none.generate_debug_info);
        assert!(!none.has_debug_info());
        
        let minimal = DebugConfig::minimal();
        assert!(minimal.generate_debug_info);
        assert_eq!(minimal.debug_level, 1);
        assert!(minimal.optimized_debug);
        
        let full = DebugConfig::full();
        assert!(full.generate_debug_info);
        assert_eq!(full.debug_level, 3);
        assert!(full.include_source);
        
        let release = DebugConfig::release();
        assert!(release.generate_debug_info);
        assert!(release.split_debug_info);
        assert!(release.compress_debug);
    }
    ...
    fn test_config_validation() {
        ...
        config.dwarf_version = 1;
        assert!(config.validate().is_err());
        
        config.dwarf_version = 4;
    ...
    #[test]
    fn test_debug_flags() {
        let none = DebugFlags::none();
        assert!(!none.line_debug);
        assert!(!none.variable_debug);
        
        let minimal = DebugFlags::minimal();
        assert!(minimal.line_debug);
        assert!(!minimal.variable_debug);
        
        let full = DebugFlags::full();
        assert!(full.line_debug);
        assert!(full.variable_debug);
        assert!(full.type_debug);
    }
    ...
    fn test_config_builder() {
        ...
        assert!(!config.optimized_debug);
        assert!(config.debug_flags.line_debug);
        assert!(config.debug_flags.variable_debug);
    }

    #[test]
    fn test_config_merge() {
        let base = DebugConfig::minimal();
        let override_config = DebugConfig::full();
        
        let merged = base.merge_with(&override_config);
        assert_eq!(merged.debug_level, 3);
        assert!(merged.include_source);
        assert!(merged.debug_flags.variable_debug);
    }

    #[test]
    fn test_llvm_debug_kind() {
        assert_eq!(DebugConfig::none().llvm_debug_kind(), "NoDebug");
        assert_eq!(DebugConfig::minimal().llvm_debug_kind(), "LineTablesOnly");
        assert_eq!(DebugConfig::default().llvm_debug_kind(), "FullDebug");
        assert_eq!(DebugConfig::full().llvm_debug_kind(), "FullDebug");
    }

    #[test]
    fn test_debug_file_extension() {
        let config = DebugConfig::default();
        assert_eq!(config.debug_file_extension(), "");
        
        let split_config = DebugConfigBuilder::new()
            .split_debug(true)
            .dwarf_version(4)
            .build();
        assert_eq!(split_config.debug_file_extension(), ".dwo");
    }
}
