...
mod tests {
    ...
    impl Traceable for TestObject {
        fn trace(&self, _visitor: &mut dyn Visitor) {
            // Simple test object - no references to trace
        }
    }
    ...
    fn create_test_store() -> Arc<ObjectStore> {
        ...
        let heap = Arc::new(RwLock::new(HeapManager::new(config, registry.clone())));
    ...
    fn test_object_storage() {
        ...
        assert_eq!(obj_ref.data, vec![1, 2, 3, 4]);
    ...
    #[test]
    fn test_object_handle_retrieval() {
        let store = create_test_store();
        ...
        let handle1 = store.store(test_obj).unwrap();
        let object_id = handle1.object_id();
        
        // Get another handle to the same object
        let handle2: ObjectHandle<TestObject> = store.get_handle(object_id).unwrap();
        
        // Both handles should point to the same object
        assert_eq!(handle1.object_id(), handle2.object_id());
    ...
    fn test_object_store_stats() {
        ...
        let obj1 = TestObject { value: 1, data: vec![1; 10] };
        let obj2 = TestObject { value: 2, data: vec![2; 20] };
        ...
        // Should have TestObject entries in type counts
        assert!(stats.type_counts.contains_key("cursed::memory::object_store::tests::TestObject"));
    }
