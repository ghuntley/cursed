...
mod tests {
    ...
    #[test]
    fn test_pressure_detection_creation() {
        let detector = MemoryPressureDetector::with_default_config();
        let stats = detector.get_statistics().unwrap();
        assert_eq!(stats.current_pressure, PressureLevel::None);
        assert!(stats.detection_active);
    }

    #[test]
    fn test_memory_pressure_calculation() {
        let detector = MemoryPressureDetector::with_default_config();
        let config = PressureDetectionConfig::default();

        // Test low memory usage
        let heap_stats = HeapStats {
            total_blocks: 10,
            total_capacity: 1000,
            total_used: 500, // 50% usage
            total_free: 500,
            average_utilization: 0.5,
            active_objects: 10,
            object_registry_count: 10,
            fragmentation_ratio: 0.25,
            memory_pressure: crate::memory::heap_manager::MemoryPressure::Low,
            metrics: crate::memory::heap_manager::AllocationMetrics {
                ...
                average_allocation_time: std::time::Duration::from_millis(20),
                gc_triggers: 2,
                compaction_events: 0,
            },
        };

        let pressure = detector.calculate_memory_pressure(&heap_stats, &config).unwrap();
        assert!(pressure < 1.0); // Should be below low threshold

        // Test high memory usage
        let heap_stats = HeapStats {
            total_blocks: 10,
            total_capacity: 1000,
            total_used: 900, // 90% usage
            total_free: 100,
            average_utilization: 0.9,
            active_objects: 10,
            object_registry_count: 10,
            fragmentation_ratio: 0.8,
            memory_pressure: crate::memory::heap_manager::MemoryPressure::High,
            metrics: crate::memory::heap_manager::AllocationMetrics {
                total_allocations: 200,
                total_deallocations: 20,
                bytes_allocated: 2000,
                bytes_deallocated: 200,
                peak_memory_usage: 950,
                allocation_failures: 5,
                average_allocation_time: std::time::Duration::from_millis(45),
                gc_triggers: 10,
                compaction_events: 2,
            },
        };
        ...
        let pressure = detector.calculate_memory_pressure(&heap_stats, &config).unwrap();
        assert!(pressure > 3.0); // Should be high pressure
    }
    ...
    fn test_pressure_level_determination() {
        ...
        assert_eq!(detector.determine_pressure_level(0.3, &config).unwrap(), PressureLevel::None);
        assert_eq!(detector.determine_pressure_level(0.8, &config).unwrap(), PressureLevel::Low);
        assert_eq!(detector.determine_pressure_level(2.0, &config).unwrap(), PressureLevel::Moderate);
        assert_eq!(detector.determine_pressure_level(3.0, &config).unwrap(), PressureLevel::High);
        assert_eq!(detector.determine_pressure_level(4.0, &config).unwrap(), PressureLevel::Critical);
        assert_eq!(detector.determine_pressure_level(5.0, &config).unwrap(), PressureLevel::Emergency);
    }
    ...
    fn test_trend_slope_calculation() {
        ...
        let samples = vec![
            ...
            PressureSample {
                ...
                allocation_rate: 3000,
                collection_failure_rate: 0.3,
        ...
        let slope = detector.calculate_trend_slope(&sample_refs).unwrap();
        assert!(slope > 0.0); // Should be positive trend
    }
    ...
    fn test_adaptive_threshold_adjustment() {
        ...
        // Test adjustment for emergency pressure
        detector.update_adaptive_thresholds(PressureLevel::Emergency, 5.0).unwrap();
        let stats = detector.get_statistics().unwrap();
        assert!(stats.adaptive_adjustment_factor > 1.0);

        // Test adjustment for no pressure
        for _ in 0..1000 {
            detector.update_adaptive_thresholds(PressureLevel::None, 0.0).unwrap();
        }
        let stats = detector.get_statistics().unwrap();
        assert!(stats.adaptive_adjustment_factor < 1.0);
    }
    ...
    fn test_configuration_update() {
        ...
        let mut new_config = PressureDetectionConfig::default();
        new_config.detection_interval_ms = 200;
        new_config.trend_window_seconds = 120;

        detector.update_config(new_config).unwrap();
        
        // Verify configuration was updated (indirect test through behavior)
        assert!(true); // If we get here, update_config succeeded
    }
    ...
    fn test_detection_enable_disable() {
        ...
        detector.set_detection_active(false);
        let stats = detector.get_statistics().unwrap();
        assert!(!stats.detection_active);

        // Test enabling detection
        detector.set_detection_active(true);
        let stats = detector.get_statistics().unwrap();
        assert!(stats.detection_active);
    }
}
