...
mod tests {
    ...
    fn create_test_collector() -> (MarkSweepCollector, SharedObjectRegistry) {
        let registry = Arc::new(ObjectRegistry::new());
        let collector = MarkSweepCollector::new(registry.clone());
        (collector, registry)
    }
    ...
    fn test_mark_object() {
        ...
        // Since object doesn't exist in registry, marking should fail
        let result = collector.mark_object(object_id).unwrap();
        assert!(!result);
    }
    ...
    fn test_config_update() {
        ...
        let new_config = MarkSweepConfig {
    ...
    fn test_write_barrier() {
        ...
        collector.write_barrier(object_id, 0, new_value).unwrap();
    ...
    #[test]
    fn test_finalization_priority() {
        let (collector, registry) = create_test_collector();
        
        // Test that different object types get appropriate finalization priorities
        let channel_id = ObjectId::new(1);
        let function_id = ObjectId::new(2);
        let custom_id = ObjectId::new(3);
        
        // Create mock objects with different tags
        // Note: This test assumes the registry supports tag-based metadata
        
        // Test finalization queue ordering
        collector.add_to_finalization_queue(custom_id).unwrap();
        collector.add_to_finalization_queue(channel_id).unwrap();
        collector.add_to_finalization_queue(function_id).unwrap();
        
        // The queue should be ordered by priority (channels first, then functions, then custom)
        // In a real implementation, we'd verify the queue order
    }
    ...
    fn test_emergency_collection() {
        ...
        // Test that emergency collection applies aggressive settings
        let original_config = collector.config.read().unwrap().clone();
        
        // Emergency collection should succeed even if no objects to collect
        let result = collector.emergency_collect();
        
        // Should not fail even with empty heap
        assert!(result.is_ok() || result.err().unwrap().contains("Failed to get"));
        
        // Configuration should be restored
        let restored_config = collector.config.read().unwrap().clone();
        assert_eq!(original_config.marking_threads, restored_config.marking_threads);
    }
    ...
    fn test_heap_compaction() {
        ...
        // Test compaction with empty heap
        let result = collector.compact_heap();
        
        // Should succeed with 0 bytes moved or fail gracefully
        match result {
            Ok(bytes_moved) => assert_eq!(bytes_moved, 0),
            Err(_) => {}, // Acceptable if registry doesn't support compaction
        }
    }
    ...
    fn test_fragmentation_stats() {
        ...
        let result = collector.get_fragmentation_stats();
        
        // Should either return stats or fail gracefully
        match result {
            Ok(_stats) => {}, // Stats returned successfully
            Err(_) => {},     // Acceptable if registry doesn't support fragmentation stats
        }
    }
}
