...
mod tests {
    ...
    fn test_memory_layout() {
        ...
        assert!(layout.header_size >= mem::size_of::<ObjectHeader>());
        ...
        assert_eq!(layout.object_offset, layout.header_size);
        
        // Test with larger alignment
        let layout16 = MemoryLayout::calculate(128, 16);
        assert!(layout16.alignment >= 16);
    }
    
    #[test]
    fn test_metadata_manager() {
        // This test is simplified since the metadata manager is designed
        // to work with the full heap system. In the production system,
        // metadata would be managed through the Heap interface.
        let manager = MetadataManager::new(8).unwrap();
        
        // Get statistics
        let stats = manager.get_statistics().unwrap();
        assert_eq!(stats.current_objects, 0);
        assert_eq!(stats.total_initialized, 0);
    }
    ...
    fn test_metadata_statistics() {
        ...
        let stats_initial = manager.get_statistics().unwrap();
        assert_eq!(stats_initial.current_objects, 0);
        assert_eq!(stats_initial.total_initialized, 0);
        
        // Test basic statistics functionality
        let stats_final = manager.get_statistics().unwrap();
        assert_eq!(stats_final.current_objects, 0);
        assert_eq!(stats_final.total_removed, 0);
    }
    ...
    fn test_object_aging() {
        ...
        // Test aging functionality with simplified test
        // Get objects older than 1000ms (should be empty)
        let old_objects = manager.get_objects_older_than(1000).unwrap();
        assert!(old_objects.is_empty());
        
        // Get objects older than 0ms (should be empty since no objects)
        let all_objects = manager.get_objects_older_than(0).unwrap();
