...
impl GarbageCollector {
    ...
    pub fn allocate<T: Traceable + Clone + 'static>(&self, value: T) -> Gc<T> {
        ...
        if !state.type_map.contains_key(&type_id) {
            state.type_map.insert(type_id, std::any::type_name::<T>().to_string());
        }
    ...
    fn collect_garbage_internal(&self, trigger: CollectionTrigger) {
        ...
        if state.collection_in_progress {
            if state.options.verbose {
                state.debug_logs.push("Skipping collection: already in progress".to_string());
            }
            return;
        }
        ...
        if state.options.verbose {
            state.debug_logs.push(format!("Starting garbage collection: trigger={:?}", trigger));
        }
    ...
    pub fn debug_info(&self) -> GcDebugInfo {
        ...
        for obj in state.objects.values() {
            let type_name = state.type_map.get(&obj.type_id)
                .cloned()
                .unwrap_or_else(|| "Unknown".to_string());
            
            let entry = type_map.entry(type_name.clone()).or_insert_with(|| TypeUsage {
