...
impl Default for CopyingConfig {
    fn default() -> Self {
        Self {
            ...
            copying_time_limit: Some(Duration::from_millis(20)),
            enable_aging: true,
...
pub struct CopyingStats {
    ...
    pub copying_threads_used: usize,
    pub space_utilization_before: f64,
    pub space_utilization_after: f64,
}
...
impl CopyingCollector {
    ...
    pub fn with_config(object_registry: SharedObjectRegistry, config: CopyingConfig) -> Result<Self, String> {
        ...
        Ok(Self {
            ...
            object_registry,
            root_manager: None,
            from_space: Mutex::new(from_space_mut),
            to_space: Mutex::new(to_space),
            object_ages: RwLock::new(HashMap::new()),
            forwarding_table: RwLock::new(HashMap::new()),
            work_queue: Mutex::new(VecDeque::new()),
            stats: RwLock::new(CopyingStats {
                collection_number: 0,
                total_duration: Duration::ZERO,
                copying_duration: Duration::ZERO,
                objects_copied: 0,
                objects_promoted: 0,
                bytes_copied: 0,
                bytes_promoted: 0,
                ...
                promotion_rate: 0.0,
                survival_rate: 0.0,
                copying_threads_used: 0,
                space_utilization_before: 0.0,
                space_utilization_after: 0.0,
            }),
    ...
    pub fn collect(&self, promote_callback: Option<Box<dyn Fn(ObjectId, &[u8]) -> Result<(), String>>>) -> Result<CopyingStats, String> {
        ...
        let stats = CopyingStats {
            ...
            bytes_copied,
            bytes_promoted,
            bytes_reclaimed: ((space_utilization_before - space_utilization_after) * config.semispace_size as f64) as usize,
            allocation_rate,
            promotion_rate,
            survival_rate,
            copying_threads_used: if config.parallel_copying { config.copying_threads } else { 1 },
            space_utilization_before,
            space_utilization_after,
        };
    ...
    fn parallel_copy_objects(
        ...
        for thread_id in 0..config.copying_threads {
            ...
            let handle = std::thread::spawn(move || {
                ...
                collector_ref.copying_worker_thread(
                    objects_copied_clone,
                    bytes_copied_clone,
                    objects_promoted_clone,
                    bytes_promoted_clone,
                )
            });
            handles.push(handle);
        }
        ...
        debug!("Parallel copying completed: {} objects copied, {} promoted", 
    ...
    fn copying_worker_thread(
        &self,
        objects_copied: Arc<std::sync::atomic::AtomicUsize>,
        bytes_copied: Arc<std::sync::atomic::AtomicUsize>,
        objects_promoted: Arc<std::sync::atomic::AtomicUsize>,
        bytes_promoted: Arc<std::sync::atomic::AtomicUsize>,
    ) -> Result<(), String> {
        let config = self.config.read()
            .map_err(|_| "Failed to acquire read lock on config")?;
        let start_time = Instant::now();
        ...
        if let Some(new_ptr) = to_space.allocate(work_item.size, 8) {
            ...
            if let Ok(Some(old_metadata)) = self.object_registry.get(work_item.object_id) {
                let new_metadata = ObjectMetadata {
                    ...
                    type_name: old_metadata.type_name.clone(),
            ...
            debug!("Copied object {} from {:?} to {:?} (new ID: {})", 
    ...
    fn get_object_references_traceable(&self, object_id: ObjectId) -> Result<Vec<ObjectId>, String> {
        ...
        let mut visitor = RefCollector {
            refs: Vec::new(),
            registry: Arc::clone(&self.object_registry),
        };
...
mod tests {
    ...
    fn create_test_collector() -> Result<(CopyingCollector, SharedObjectRegistry), String> {
    ...
    fn test_collector_creation() {
        ...
        assert_eq!(stats.collection_number, 0);
    }
    ...
    fn test_space_utilization() {
        ...
        // Should have plenty of available space initially
        let available = collector.available_space().unwrap();
        assert!(available > 0);
        
        // Should not need collection initially
        assert!(!collector.should_collect().unwrap());
    }
    ...
    fn test_allocation() {
        ...
        // Try allocating small object
        let result = collector.allocate(64, 8).unwrap();
    ...
    fn test_config_update() {
        ...
        let new_config = CopyingConfig {
            semispace_size: 16 * 1024 * 1024, // 16MB
    ...
    fn test_object_forwarding() {
        ...
        // Initially, object should not be forwarded
        assert!(!collector.is_object_forwarded(obj_id).unwrap());
        assert!(collector.get_forwarded_object(obj_id).unwrap().is_none());
        
        // Object size should be retrievable
        assert_eq!(collector.get_object_size(obj_id).unwrap(), Some(64));
    }
    ...
    fn test_efficiency_estimation() {
        ...
        // Should be able to estimate efficiency
        let efficiency = collector.estimate_copying_efficiency().unwrap();
        assert!(efficiency >= 0.0 && efficiency <= 1.0);
    }
    
    #[test]
    fn test_collection_with_mock_objects() {
        let (collector, registry) = create_test_collector().unwrap();
        ...
        for i in 0..5 {
            let obj_id = id_gen.next();
            let metadata = ObjectMetadata::new(obj_id, 64 + i * 16, format!("TestObject{}", i));
            registry.register(metadata).unwrap();
        }
        
        // Try to perform collection
        let result = collector.force_collect();
        // This might fail due to mock implementation, but it should not panic
        match result {
            Ok(stats) => {
