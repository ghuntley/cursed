...
mod tests {
    ...
    fn test_expand_simple_variables() {
        set_env("TEST_USER", "alice").unwrap();
        set_env("TEST_HOME", "/home/alice").unwrap();
        
        let result = expand_env_vars("User: $TEST_USER, Home: $TEST_HOME").unwrap();
        assert_eq!(result, "User: alice, Home: /home/alice");
        
        remove_env("TEST_USER").unwrap();
        remove_env("TEST_HOME").unwrap();
    }

    #[test]
    fn test_expand_braced_variables() {
        set_env("TEST_VAR", "value").unwrap();
        
        let result = expand_env_vars("Before ${TEST_VAR} after").unwrap();
        assert_eq!(result, "Before value after");
        
        remove_env("TEST_VAR").unwrap();
    }
    ...
    fn test_expand_with_defaults() {
        ...
        set_env("TEST_EXISTS", "existing").unwrap();
        let result = expand_env_vars("${TEST_EXISTS:-default_value}").unwrap();
        assert_eq!(result, "existing");
        remove_env("TEST_EXISTS").unwrap();
    }

    #[test]
    fn test_expand_conditional() {
        set_env("TEST_SET", "value").unwrap();
        
        let result = expand_env_vars("${TEST_SET:+alternative}").unwrap();
        assert_eq!(result, "alternative");
        
        let result = expand_env_vars("${NONEXISTENT:+alternative}").unwrap();
        assert_eq!(result, "");
        
        remove_env("TEST_SET").unwrap();
    }
    ...
    fn test_has_env_vars() {
        ...
        assert!(!has_env_vars("No variables"));
    }

    #[test]
    fn test_validate_env_syntax() {
        assert!(validate_env_syntax("${HOME}/docs").is_ok());
        assert!(validate_env_syntax("$USER works").is_ok());
    ...
    fn test_substitute_env_vars() {
        ...
        let result = substitute_env_vars("$USER: ${HOME}/docs", &replacements).unwrap();
        assert_eq!(result, "bob: /home/bob/docs");
    }
    ...
    #[test]
    fn test_expand_with_custom_defaults() {
        let mut defaults = HashMap::new();
        defaults.insert("CUSTOM_VAR".to_string(), "custom_value".to_string());
        
        let result = expand_env_vars_with_defaults("Value: $CUSTOM_VAR", &defaults).unwrap();
