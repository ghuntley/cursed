...
mod tests {
    ...
    #[test]
    fn test_shutdown_task_addition() {
        ...
        let tasks = shutdown.tasks.lock().unwrap();
        assert!(tasks.contains_key("test_task"));
    }

    #[test]
    fn test_shutdown_task_execution() {
        let mut shutdown = GracefulShutdown::new();
        ...
        // Configure for synchronous shutdown
        shutdown.options.sync_shutdown = true;
        shutdown.options.timeout = Duration::from_secs(1);
        
        // Start and trigger shutdown
        ...
        assert!(task_executed.load(Ordering::SeqCst));
    }

    #[test]
    fn test_shutdown_task_ordering() {
        let mut shutdown = GracefulShutdown::new();
        ...
        shutdown.add_with_order("task_3", 3, move || {
            order_clone.lock().unwrap().push(3);
            Ok(())
        });
        
        let order_clone = execution_order.clone();
        shutdown.add_with_order("task_1", 1, move || {
            order_clone.lock().unwrap().push(1);
            Ok(())
        });
        
        let order_clone = execution_order.clone();
        shutdown.add_with_order("task_2", 2, move || {
    ...
    fn test_shutdown_timeout() {
        ...
        shutdown.add("slow_task", || {
            thread::sleep(Duration::from_millis(500));
            Ok(())
        });
        
        shutdown.options.sync_shutdown = true;
        shutdown.options.timeout = Duration::from_millis(100); // Short timeout
        
        assert!(shutdown.start().is_ok());
        assert!(shutdown.shutdown().is_ok());
        
        // Should timeout
        let result = shutdown.wait();
        assert!(result.is_err());
    }

    #[test]
    fn test_shutdown_error_handling() {
        let mut shutdown = GracefulShutdown::new();
        
        let error_handled = Arc::new(AtomicBool::new(false));
        let error_handled_clone = error_handled.clone();
        
        shutdown.options.error_handler = Some(Arc::new(move |_| {
            error_handled_clone.store(true, Ordering::SeqCst);
        }));
        
        shutdown.add("failing_task", || {
            Err(SignalBoostError::SystemError("Test error".to_string()))
        });
        ...
        // Check status has error recorded
        let status = shutdown.status();
        assert!(status.errors.contains_key("failing_task"));
    }

    #[test]
    fn test_shutdown_group_tasks() {
        let mut shutdown = GracefulShutdown::new();
        ...
        shutdown.add_group("test_group", tasks);
    ...
    #[test]
    fn test_shutdown_status_tracking() {
        let mut shutdown = GracefulShutdown::new();
        
        shutdown.add("status_task", || {
            thread::sleep(Duration::from_millis(50));
            Ok(())
        });
        
        shutdown.options.sync_shutdown = false; // Async
        shutdown.options.timeout = Duration::from_secs(1);
        
        assert!(shutdown.start().is_ok());
        
        // Check initial status
        let initial_status = shutdown.status();
        ...
        thread::sleep(Duration::from_millis(10));
        let progress_status = shutdown.status();
        ...
        // Check final status
        let final_status = shutdown.status();
        assert!(!final_status.in_progress);
        assert_eq!(final_status.completed_tasks.len(), 1);
        assert!(final_status.completed_tasks.contains(&"status_task".to_string()));
    }
}
