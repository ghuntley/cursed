...
mod tests {
    ...
    fn test_mem_stats() {
        ...
        let result = mem_stats();
        assert!(result.is_ok());
        
        let stats = result.unwrap();
        // Basic sanity checks
        assert!(stats.alloc >= 0);
        assert!(stats.total_alloc >= stats.alloc);
    ...
    #[test]
    fn test_set_gc_enabled() {
        assert!(initialize().is_ok());
        
        let result = set_gc_enabled(false);
        assert!(result.is_ok());
        let old = result.unwrap();
        assert!(old); // Should default to true
        
        let result2 = set_gc_enabled(true);
        assert!(result2.is_ok());
        let old2 = result2.unwrap();
        assert!(!old2); // Should be false from previous call
    ...
    #[test]
    fn test_set_mem_profile_rate() {
        assert!(initialize().is_ok());
        
        let result = set_mem_profile_rate(1024);
        assert!(result.is_ok());
    ...
    #[test]
    fn test_allocation_size_histogram() {
        assert!(initialize().is_ok());
        
        let result = allocation_size_histogram();
        assert!(result.is_ok());
        
        let histogram = result.unwrap();
        assert!(!histogram.is_empty());
        assert!(histogram.contains_key(&8));
        assert!(histogram.contains_key(&1024));
        
        assert!(cleanup().is_ok());
    }

    #[test]
    fn test_top_allocated_types() {
        assert!(initialize().is_ok());
        
        let result = top_allocated_types(5);
        assert!(result.is_ok());
    ...
    fn test_is_valid_pointer() {
        ...
        let result = is_valid_pointer(null_ptr);
        assert!(result.is_ok());
        assert!(!result.unwrap()); // Null pointer should be invalid
        
        let valid_ptr = &42u8 as *const u8;
        let result2 = is_valid_pointer(valid_ptr);
        assert!(result2.is_ok());
        assert!(result2.unwrap()); // Non-null pointer should be valid
    }
    ...
    fn test_get_object_size() {
        ...
        let valid_ptr = &42u8 as *const u8;
        let result2 = get_object_size(valid_ptr);
        assert!(result2.is_ok());
        assert!(result2.unwrap() > 0);
    }

    #[test]
    fn test_get_pointer_info() {
        let null_ptr: *const u8 = std::ptr::null();
        let result = get_pointer_info(null_ptr);
        assert!(result.is_err());
        
        let valid_ptr = &42u8 as *const u8;
        let result2 = get_pointer_info(valid_ptr);
        assert!(result2.is_ok());
        
        let info = result2.unwrap();
        assert_eq!(info.address, valid_ptr as usize);
        assert!(info.size > 0);
