...
pub fn sleep(args: &[Rc<Object>]) -> Result<Rc<Object>, Error> {
    if args.is_empty() {
        return Err(Error::Runtime(
            "sleep requires 1 argument: duration in milliseconds".to_string(),
        ));
    }
...
pub fn duration_from_secs(args: &[Rc<Object>]) -> Result<Rc<Object>, Error> {
    if args.is_empty() {
        return Err(Error::Runtime(
            "duration_from_secs requires 1 argument: seconds".to_string(),
        ));
    }
    ...
    let secs = match &*args[0] {
        ...
        _ => {
            return Err(Error::Runtime(
                "Argument to duration_from_secs must be a number".to_string(),
            ))
        }
    };
