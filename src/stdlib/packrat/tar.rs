...
mod tests {
    ...
    #[test]
    fn test_rat_header_creation() {
        info!("Testing RatHeader creation");
        ...
        info!("RatHeader creation test passed");
    }
    
    #[test]
    fn test_rat_header_validation() {
        info!("Testing RatHeader validation");
        
        let mut header = RatHeader::default();
        header.name = "test.txt".to_string();
        ...
        info!("RatHeader validation tests passed");
    }
    
    #[test]
    fn test_rat_header_serialization() {
        info!("Testing RatHeader serialization");
        
        let header = RatHeader::from_file_info("test.txt", 100, 0o644);
        let bytes = header.to_bytes().unwrap();
        assert_eq!(bytes.len(), TAR_BLOCK_SIZE);
        
        // Parse back
        let parsed = RatHeader::from_bytes(&bytes).unwrap();
    ...
    #[test]
    fn test_rat_pack_creation() {
        info!("Testing RatPack creation");
        
        let data = vec![0u8; 1024];
        let cursor = Cursor::new(data);
        let _rat_pack = RatPack::new(cursor);
        
        info!("RatPack creation test passed");
    }
    
    #[test]
    fn test_rat_stash_creation() {
        info!("Testing RatStash creation");
        
        let buffer = Vec::new();
        let mut rat_stash = RatStash::new(buffer);
        
        let header = RatHeader::from_file_info("test.txt", 5, 0o644);
        rat_stash.write_header(&header).unwrap();
        rat_stash.write_all(b"hello").unwrap();
        rat_stash.close().unwrap();
        
        info!("RatStash creation test passed");
    }
    
    #[test]
    fn test_format_types() {
        info!("Testing TAR format types");
        
        assert_eq!(Format::default(), Format::FormatPOSIX);
        
        let format = Format::FormatGNU;
        assert_ne!(format, Format::FormatLegacy);
        
        info!("TAR format types tests passed");
    }
