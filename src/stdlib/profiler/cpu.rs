//! Functional implementation for cpu
...
impl ModuleHandler {
    ...
    pub fn info(&self) -> String {
        format!("Module: cpu, Enabled: {}", self.enabled)
    }
}
...
pub fn init_cpu() -> ModuleResult<()> {
    let handler = ModuleHandler::new();
    let result = handler.process("test")?;
    if !result.contains("test") {
        return Err(CursedError::runtime_error("Module test failed"));
    }
    println!("⚙️  Module processing (cpu) initialized");
    Ok(())
}

/// Test cpu functionality
pub fn test_cpu() -> ModuleResult<()> {
    let handler = ModuleHandler::new();
