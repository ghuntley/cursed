...
mod tests {
    ...
    fn test_function_recording() {
        ...
        assert!(profile.data.sample_count > 0);
    }

    #[test]
    fn test_global_profiling_functions() {
        // Test global start/stop functions
        let start_result = start_cpu_profiling();
        // Note: This might fail if profiler is already running
        // In a real test environment, we'd ensure clean state
        
        let profile_result = get_cpu_profile();
        assert!(profile_result.is_ok());
        
        // Clean up
        let _ = stop_cpu_profiling();
    }

    #[test]
    fn test_profiler_statistics() {
        let initial_count = get_profile_count();
        let initial_samples = get_total_samples();
    ...
    fn test_cpu_sample_creation() {
        let sample = CpuSample {
            ...
            stack_trace: vec!["main".to_string(), "test_func".to_string()],
            cpu_time_ns: 1000000,
            instruction_pointer: 0x1000,
        };

        assert_eq!(sample.function_name, "test_func");
        assert_eq!(sample.thread_id, 12345);
        assert_eq!(sample.cpu_time_ns, 1000000);
    }
    ...
    fn test_function_profile() {
        let mut profile = FunctionProfile {
            ...
            self_time_ns: 500000,
            call_count: 10,
            average_time_ns: 100000,
            min_time_ns: 50000,
            max_time_ns: 200000,
            child_functions: HashMap::new(),
        };

        profile.child_functions.insert("child1".to_string(), 5);
        assert_eq!(profile.child_functions.len(), 1);
