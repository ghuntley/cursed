...
mod tests {
    ...
    fn test_allocation_tracking() {
        ...
        // Track allocation
        let allocation_id = tracker.track_allocation(1024, "test_function", "test.rs", 42).unwrap();
        ...
        // Track deallocation
        tracker.track_deallocation(allocation_id).unwrap();
    ...
    fn test_memory_profiler_lifecycle() {
        ...
        profiler.record_gc_event("minor", 1000000, 512, 10, 2048, 1536, "allocation_threshold").unwrap();
    ...
    fn test_memory_stats_efficiency() {
        ...
        assert_eq!(efficiency.average_allocation_size, 100.0);
        assert!((efficiency.memory_utilization - 0.244140625).abs() < 0.001);
        assert_eq!(efficiency.fragmentation_ratio, 0.15);
        assert_eq!(efficiency.live_object_ratio, 0.2);
    }
    ...
    fn test_allocation_site() {
        let site = AllocationSite {
            ...
            line_number: 42,
            stack_trace: vec!["main".to_string(), "test_function".to_string()],
            allocation_count: 10,
            total_bytes: 10240,
            peak_bytes: 2048,
            average_size: 1024,
            first_allocation: Instant::now(),
            last_allocation: Instant::now(),
        };

        assert_eq!(site.function_name, "test_function");
        assert_eq!(site.line_number, 42);
        assert_eq!(site.allocation_count, 10);
        assert_eq!(site.average_size, 1024);
    }

    #[test]
    fn test_global_memory_profiling_functions() {
        // Test memory tracking functions
        let allocation_id = track_allocation(512, "test", "test.rs", 10).unwrap();
        assert_eq!(allocation_id, 0); // No profiler running
        
        track_deallocation(allocation_id).unwrap();
        
        let stats = get_memory_stats().unwrap();
        assert_eq!(stats.allocation_count, 0);
        
        let count = get_profile_count();
        let samples = get_total_samples();
        assert!(count >= 0);
        assert!(samples >= 0);
    }

    #[test]
    fn test_memory_profile_analysis() {
        ...
        let leaks = profile.potential_leaks();
        assert_eq!(leaks.len(), 1);

        let leak_stats = profile.leak_statistics();
        assert_eq!(leak_stats.potential_leak_count, 1);
        assert_eq!(leak_stats.potential_leak_bytes, 1024);
        assert_eq!(leak_stats.largest_leak_size, 1024);
    }
}
