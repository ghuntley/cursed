...
mod tests {
    ...
    fn test_file_type_from_extension() {
        assert_eq!(file_type_from_extension("document.txt"), Some("text".to_string()));
        assert_eq!(file_type_from_extension("data.json"), Some("data".to_string()));
        assert_eq!(file_type_from_extension("photo.jpg"), Some("image".to_string()));
        assert_eq!(file_type_from_extension("movie.mp4"), Some("video".to_string()));
        assert_eq!(file_type_from_extension("song.mp3"), Some("audio".to_string()));
        assert_eq!(file_type_from_extension("archive.zip"), Some("archive".to_string()));
        assert_eq!(file_type_from_extension("program.exe"), Some("binary".to_string()));
        assert_eq!(file_type_from_extension("source.rs"), Some("code".to_string()));
        assert_eq!(file_type_from_extension("unknown.xyz"), Some("unknown".to_string()));
        assert_eq!(file_type_from_extension("no_extension"), None);
    }

    #[test]
    fn test_is_safe_path() {
        assert!(is_safe_path("normal/path/file.txt"));
        assert!(is_safe_path("./current/dir/file.txt"));
        assert!(!is_safe_path("../../../etc/passwd"));
        assert!(!is_safe_path("path/with/../traversal"));
        assert!(!is_safe_path("path/with\0null"));
    }

    #[test]
    fn test_is_safe_file_size() {
        assert!(is_safe_file_size(1024)); // 1KB
        assert!(is_safe_file_size(50 * 1024 * 1024)); // 50MB
        assert!(!is_safe_file_size(200 * 1024 * 1024)); // 200MB
    }
    ...
    fn test_safe_file_operations() {
        ...
        let content = "This is safe content";
        
        // Test safe write
        write_text_file_safe(&file_path_str, content).unwrap();
        
        // Test safe read
        let read_content = read_text_file_safe(&file_path_str).unwrap();
    ...
    fn test_integration_example() {
        ...
        // Create a file
        let file_path = join_path(vec![base_path.clone(), "test.txt".to_string()]);
        write_file(&file_path, "Hello, CURSED!").unwrap();
        ...
        assert!(meta.is_file);
        assert_eq!(meta.size, 14); // "Hello, CURSED!" is 14 bytes
        ...
        // Copy file
        let copy_path = join_path(vec![base_path.clone(), "copy.txt".to_string()]);
        copy_file(&file_path, &copy_path).unwrap();
        
        // Verify copy
        let copy_content = read_file(&copy_path).unwrap();
        assert_eq!(copy_content, "Hello, CURSED!");
        
        // Clean up
        delete_file(&file_path).unwrap();
        delete_file(&copy_path).unwrap();
    }
}
