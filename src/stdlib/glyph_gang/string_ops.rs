...
mod tests {
    ...
    fn test_case_conversion() {
        assert_eq!(to_upper_string("hello"), "HELLO");
        assert_eq!(to_lower_string("WORLD"), "world");
        assert_eq!(to_title_string("hello world"), "Hello World");
        assert_eq!(to_title_string("hello-world test"), "Hello-World Test");
    }

    #[test]
    fn test_rune_operations() {
        let s = "Hello, 世界";
        assert_eq!(rune_count(s), 8);
        
        let (first, size) = first_rune(s);
        assert_eq!(first, 'H');
        assert_eq!(size, 1);
        
        let (last, size) = last_rune(s);
        assert_eq!(last, '界');
        assert_eq!(size, 3);
        
        assert_eq!(rune_at(s, 0), 'H');
        assert_eq!(rune_at(s, 7), '界');
        assert_eq!(rune_at(s, 100), '\0');
    }

    #[test]
    fn test_string_width() {
        assert_eq!(string_width("Hello"), 5);
        assert_eq!(string_width("世界"), 4); // Two double-width characters
        assert_eq!(string_width("Hello世界"), 9); // 5 + 4
        
        assert_eq!(get_char_width('A'), 1);
        assert_eq!(get_char_width('世'), 2);
        assert_eq!(get_char_width('\u{0300}'), 0); // Combining grave accent
    }

    #[test]
    fn test_truncation() {
        assert_eq!(truncate_string("Hello, World!", 5), "Hello");
        assert_eq!(truncate_string("世界", 3), "世");
    ...
    fn test_reverse() {
        ...
        assert_eq!(reverse_string("世界"), "界世");
        assert_eq!(reverse_string(""), "");
    }

    #[test]
    fn test_boundaries() {
        let boundaries = word_boundaries("Hello, world!");
        assert!(boundaries.contains(&0)); // Start
        assert!(boundaries.contains(&5)); // After "Hello"
        assert!(boundaries.contains(&6)); // After ","
        assert!(boundaries.contains(&7)); // After " "
        assert!(boundaries.contains(&12)); // After "world"
        
        let boundaries = sentence_boundaries("Hello world. How are you?");
        assert!(boundaries.contains(&0)); // Start
        assert!(boundaries.len() >= 2); // At least start and one sentence break
    }

    #[test]
    fn test_case_folding() {
        assert_eq!(fold_string("Hello"), "hello");
        assert_eq!(fold_string("WORLD"), "world");
        
        assert!(equal_fold("Hello", "HELLO"));
        assert!(equal_fold("world", "WORLD"));
        assert!(!equal_fold("hello", "world"));
        assert!(equal_fold("", ""));
    }

    #[test]
    fn test_normalization() {
        // Basic test - in a real implementation this would test actual normalization
        let result = normalize_string("hello", NFC).unwrap();
        assert_eq!(result, "hello");
        
        let result = normalize_string("test", NFD).unwrap();
