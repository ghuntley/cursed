...
mod tests {
    ...
    fn test_character_direction() {
        assert_eq!(get_direction('A'), Direction::LTR);
        assert_eq!(get_direction('a'), Direction::LTR);
        assert_eq!(get_direction('1'), Direction::LTR);
        assert_eq!(get_direction(' '), Direction::LTR);
        
        // Hebrew characters
        assert_eq!(get_direction('א'), Direction::RTL); // Hebrew letter alef
        assert_eq!(get_direction('ב'), Direction::RTL); // Hebrew letter bet
        
        // Arabic characters
        assert_eq!(get_direction('ا'), Direction::RTL); // Arabic letter alif
        assert_eq!(get_direction('ب'), Direction::RTL); // Arabic letter beh
    }

    #[test]
    fn test_string_direction() {
        assert_eq!(get_string_direction("Hello World"), Direction::LTR);
        assert_eq!(get_string_direction("123"), Direction::LTR);
        assert_eq!(get_string_direction(""), Direction::LTR);
        
        // Hebrew text
        assert_eq!(get_string_direction("שלום"), Direction::RTL);
        
        // Arabic text
        assert_eq!(get_string_direction("السلام"), Direction::RTL);
        
        // Mixed text
        assert_eq!(get_string_direction("Hello שלום"), Direction::Mixed);
        assert_eq!(get_string_direction("English العربية"), Direction::Mixed);
    }

    #[test]
    fn test_direction_checks() {
        assert!(is_ltr("Hello World"));
        assert!(!is_rtl("Hello World"));
        assert!(!is_mixed("Hello World"));
        
        assert!(!is_ltr("שלום"));
        assert!(is_rtl("שלום"));
        assert!(!is_mixed("שלום"));
        
        assert!(!is_ltr("Hello שלום"));
        assert!(!is_rtl("Hello שלום"));
        assert!(is_mixed("Hello שלום"));
    }

    #[test]
    fn test_text_reordering() {
        let result = reorder_text("Hello").unwrap();
        assert_eq!(result, "Hello");
        
        // For RTL text, this simplified implementation just reverses
        // In a real implementation, this would be more sophisticated
        let result = reorder_text("שלום").unwrap();
        assert_eq!(result, "םולש");
        
        let result = logical_to_visual("Hello").unwrap();
        assert_eq!(result, "Hello");
        
        let result = visual_to_logical("Hello").unwrap();
        assert_eq!(result, "Hello");
    }

    #[test]
    fn test_bidi_analysis() {
        let analysis = analyze_bidi("Hello World");
        assert_eq!(analysis.get("direction").unwrap(), "LTR");
        assert_eq!(analysis.get("is_ltr").unwrap(), "true");
        assert_eq!(analysis.get("is_rtl").unwrap(), "false");
        assert_eq!(analysis.get("is_mixed").unwrap(), "false");
        
        let analysis = analyze_bidi("Hello שלום");
        assert_eq!(analysis.get("direction").unwrap(), "Mixed");
        assert_eq!(analysis.get("is_mixed").unwrap(), "true");
    }

    #[test]
    fn test_bidi_types() {
        assert_eq!(get_bidi_type('A'), BidiType::L);
        assert_eq!(get_bidi_type('1'), BidiType::EN);
        assert_eq!(get_bidi_type(' '), BidiType::WS);
        assert_eq!(get_bidi_type('+'), BidiType::ES);
        assert_eq!(get_bidi_type(','), BidiType::CS);
        assert_eq!(get_bidi_type('!'), BidiType::ON);
    }
}
