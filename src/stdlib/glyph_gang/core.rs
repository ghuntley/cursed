...
mod tests {
    ...
    fn test_ascii_conversion() {
        ...
        assert_eq!(to_ascii('a'), 'a');
        assert_eq!(to_ascii('1'), '1');
        
        // Test some common diacritics
        assert_eq!(to_ascii('à'), 'a');
        assert_eq!(to_ascii('À'), 'A');
    ...
    fn test_character_names() {
        assert_eq!(get_character_name('A'), "LATIN CAPITAL LETTER A");
        assert_eq!(get_character_name('a'), "LATIN SMALL LETTER A");
        assert_eq!(get_character_name(' '), "SPACE");
        assert_eq!(get_character_name('!'), "EXCLAMATION MARK");
        assert_eq!(get_character_name('0'), "DIGIT ZERO");
        assert_eq!(get_character_name('9'), "DIGIT NINE");
    }
    
    #[test]
    fn test_find_character_by_name() {
        assert_eq!(find_character_by_name("LATIN CAPITAL LETTER A").unwrap(), 'A');
        assert_eq!(find_character_by_name("SPACE").unwrap(), ' ');
        assert_eq!(find_character_by_name("U+0041").unwrap(), 'A');
        assert_eq!(find_character_by_name("U+0020").unwrap(), ' ');
        
        assert!(find_character_by_name("NONEXISTENT CHARACTER").is_err());
    }
    
    #[test]
    fn test_block_names() {
        assert_eq!(get_block_name('A'), "Basic Latin");
        assert_eq!(get_block_name('α'), "Greek and Coptic");
        assert_eq!(get_block_name('А'), "Cyrillic");
    }
    
    #[test]
    fn test_categories() {
        assert_eq!(get_category('A'), "Lu");
        assert_eq!(get_category('a'), "Ll");
        assert_eq!(get_category('1'), "Nd");
        assert_eq!(get_category(' '), "Zs");
        assert_eq!(get_category('!'), "Po");
    }
    
    #[test]
    fn test_code_points() {
        assert_eq!(get_code_point('A'), "U+0041");
        assert_eq!(get_code_point(' '), "U+0020");
        assert_eq!(get_code_point('\0'), "U+0000");
    }
    
    #[test]
    fn test_range_operations() {
        // Test with basic letter ranges
        assert!(is_in_range('A', &LETTER));
        assert!(is_in_range('a', &LETTER));
        assert!(!is_in_range('1', &LETTER));
        
        let tables = vec![&*LETTER, &*NUMBER];
        assert!(is_in_ranges('A', &tables));
        assert!(is_in_ranges('1', &tables));
        assert!(!is_in_ranges('!', &tables));
        
        assert!(is_one_of(&["Letter"], 'A').unwrap());
        assert!(is_one_of(&["Number"], '1').unwrap());
        assert!(!is_one_of(&["Letter"], '1').unwrap());
        assert!(is_one_of(&["UnknownTable"], 'A').is_err());
    }
    
    #[test]
    fn test_properties() {
        let props = get_properties('A');
        assert_eq!(props.get("name").unwrap(), "LATIN CAPITAL LETTER A");
        assert_eq!(props.get("category").unwrap(), "Lu");
        assert_eq!(props.get("is_letter").unwrap(), "true");
        assert_eq!(props.get("is_upper").unwrap(), "true");
        assert_eq!(props.get("is_lower").unwrap(), "false");
        assert_eq!(props.get("to_lower").unwrap(), "a");
    }
}
