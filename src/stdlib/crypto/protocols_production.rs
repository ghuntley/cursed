//! I/O functionality for protocols_production
...
pub fn init_protocols_production() -> IOResult<()> {
    let handler = IOHandler::new();
    let test_data = b"test data";
    let mut cursor = std::io::Cursor::new(test_data);
    let result = handler.read_all(&mut cursor)?;
    if result != test_data {
        return Err(IOError::Other("I/O test failed"));
    }
    println!("📁 I/O processing (protocols_production) initialized");
    Ok(())
}

/// Test I/O functionality
pub fn test_protocols_production() -> IOResult<()> {
    let handler = IOHandler::new();
    ...
    if result != test_string {
        return Err(IOError::Other("I/O string test failed"));
    }
    Ok(())
}
