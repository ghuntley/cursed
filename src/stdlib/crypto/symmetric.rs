//! Cryptographic functionality for symmetric
...
pub fn init_symmetric() -> CryptoResult<()> {
    let handler = CryptoHandler::new();
    let key = handler.generate_key()?;
    if key.len() != 32 {
        return Err(CursedError::runtime_error("Crypto key generation test failed"));
    }
    println!("🔐 Crypto processing (symmetric) initialized");
    Ok(())
}

/// Test crypto functionality
pub fn test_symmetric() -> CryptoResult<()> {
    let handler = CryptoHandler::new();
