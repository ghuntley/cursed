//! Cryptographic functionality for xchacha20_poly1305
...
pub fn init_xchacha20_poly1305() -> CryptoResult<()> {
    let handler = CryptoHandler::new();
    let key = handler.generate_key()?;
    if key.len() != 32 {
        return Err(CursedError::runtime_error("Crypto key generation test failed"));
    }
    println!("🔐 Crypto processing (xchacha20_poly1305) initialized");
    Ok(())
}

/// Test crypto functionality
pub fn test_xchacha20_poly1305() -> CryptoResult<()> {
    let handler = CryptoHandler::new();
