//! Cryptographic functionality for protocols_advanced
...
pub fn init_protocols_advanced() -> CryptoResult<()> {
    let handler = CryptoHandler::new();
    let key = handler.generate_key()?;
    if key.len() != 32 {
        return Err(CursedError::runtime_error("Crypto key generation test failed"));
    }
    println!("🔐 Crypto processing (protocols_advanced) initialized");
    Ok(())
}

/// Test crypto functionality
pub fn test_protocols_advanced() -> CryptoResult<()> {
    let handler = CryptoHandler::new();
