//! Cryptographic functionality for encoding
...
pub fn init_encoding() -> CryptoResult<()> {
    let handler = CryptoHandler::new();
    let key = handler.generate_key()?;
    if key.len() != 32 {
        return Err(CursedError::runtime_error("Crypto key generation test failed"));
    }
    println!("🔐 Crypto processing (encoding) initialized");
    Ok(())
}

/// Test crypto functionality
pub fn test_encoding() -> CryptoResult<()> {
    let handler = CryptoHandler::new();
