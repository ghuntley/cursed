//! Cryptographic functionality for unified_api
...
pub fn init_unified_api() -> CryptoResult<()> {
    let handler = CryptoHandler::new();
    let key = handler.generate_key()?;
    if key.len() != 32 {
        return Err(CursedError::runtime_error("Crypto key generation test failed"));
    }
    println!("🔐 Crypto processing (unified_api) initialized");
    Ok(())
}

/// Test crypto functionality
pub fn test_unified_api() -> CryptoResult<()> {
    let handler = CryptoHandler::new();
