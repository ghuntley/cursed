//! Cryptographic functionality for ed25519_keys
...
pub fn init_ed25519_keys() -> CryptoResult<()> {
    let handler = CryptoHandler::new();
    let key = handler.generate_key()?;
    if key.len() != 32 {
        return Err(CursedError::runtime_error("Crypto key generation test failed"));
    }
    println!("🔐 Crypto processing (ed25519_keys) initialized");
    Ok(())
}

/// Test crypto functionality
pub fn test_ed25519_keys() -> CryptoResult<()> {
    let handler = CryptoHandler::new();
