//! Cryptographic functionality for protocols_comprehensive
...
pub fn init_protocols_comprehensive() -> CryptoResult<()> {
    let handler = CryptoHandler::new();
    let key = handler.generate_key()?;
    if key.len() != 32 {
        return Err(CursedError::runtime_error("Crypto key generation test failed"));
    }
    println!("🔐 Crypto processing (protocols_comprehensive) initialized");
    Ok(())
}

/// Test crypto functionality
pub fn test_protocols_comprehensive() -> CryptoResult<()> {
    let handler = CryptoHandler::new();
