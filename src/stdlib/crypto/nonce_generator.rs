//! Cryptographic functionality for nonce_generator
...
pub fn init_nonce_generator() -> CryptoResult<()> {
    let handler = CryptoHandler::new();
    let key = handler.generate_key()?;
    if key.len() != 32 {
        return Err(CursedError::runtime_error("Crypto key generation test failed"));
    }
    println!("🔐 Crypto processing (nonce_generator) initialized");
    Ok(())
}

/// Test crypto functionality
pub fn test_nonce_generator() -> CryptoResult<()> {
    let handler = CryptoHandler::new();
