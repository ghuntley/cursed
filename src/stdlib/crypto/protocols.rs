//! Cryptographic functionality for protocols
...
pub fn init_protocols() -> CryptoResult<()> {
    let handler = CryptoHandler::new();
    let key = handler.generate_key()?;
    if key.len() != 32 {
        return Err(CryptoError::KeyGenerationFailed);
    }
    println!("🔐 Crypto processing (protocols) initialized");
    Ok(())
}

/// Test crypto functionality
pub fn test_protocols() -> CryptoResult<()> {
    let handler = CryptoHandler::new();
