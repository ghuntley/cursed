...
mod tests {
    ...
    fn test_fixture_creation() {
        let fixture = FixtureVibe::new(
            |_t| DatabaseFixture::new("test_db"),
            |_t, mut db| db.disconnect(),
        );
    ...
    fn test_test_case_creation() {
        let test_case = TestCase::new(
            ...
            |t, (a, b), expected| {
                let result = a + b;
                if result != expected {
                    t.fail_vibe(&format!("Expected {}, got {}", expected, result));
                }
            },
        );

        let mut test = VibeTest::new("case_test".to_string());
        test_case.run(&mut test);
        assert!(!test.failed());
    }
    ...
    fn test_parameterized_test() {
        ...
        let param_test = ParameterizedTest::new(
            ...
            |t, x| {
                let result = x * x;
                t.log(&[&format!("{} squared is {}", x, result)]);
            },
        );

        // This would print results in a real run
        // param_test.run();
    }
    ...
    fn test_test_suite() {
        ...
        suite.add_test(|t| {
            let result = 5 * 3;
            if result != 15 {
                t.fail_vibe("Multiplication failed");
            }
        });

        suite.set_setup(|t| {
            t.log(&["Setting up math test environment"]);
        });

        suite.set_teardown(|t| {
            t.log(&["Cleaning up math test environment"]);
        });

        // This would run the suite in a real scenario
        // suite.run();
    }

    #[test]
    fn test_database_fixture() {
        let mut db = DatabaseFixture::new("test://localhost");
        assert!(!db.connected);
        ...
        let query_result = db.execute_query("SELECT * FROM test");
    ...
    fn test_http_server_fixture() {
        let mut server = HttpServerFixture::new(8080);
