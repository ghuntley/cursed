...
mod tests {
    use super::*;
    use crate::stdlib::packages::test_vibes::core::{VibeTest, VibeBench};
    ...
    fn test_runner_add_test() {
        ...
        runner.add_test("test_example", |t| {
    ...
    #[test]
    fn test_runner_add_benchmark() {
        let config = TestRunnerConfig::default();
        let mut runner = TestRunner::new(config);
        
        runner.add_benchmark("bench_example", |b| {
    ...
    fn test_filter_functionality() {
        ...
        config.filter = Some("math".to_string());
        
        let runner = TestRunner::new(config);
        
        assert!(runner.should_run_test("test_math_addition"));
        assert!(runner.should_run_test("math_subtraction"));
        assert!(!runner.should_run_test("string_operations"));
    }

    #[test]
    fn test_registry_creation() {
        let registry = TestRegistry::new();
        assert_eq!(registry.tests.len(), 0);
        assert_eq!(registry.benchmarks.len(), 0);
    }

    #[test]
    fn test_registry_register_test() {
        let mut registry = TestRegistry::new();
        
        registry.register_test("test_example", |t| {
            t.log(&["Test executed"]);
        });
        
        assert_eq!(registry.tests.len(), 1);
        assert!(registry.tests.contains_key("test_example"));
    }

    #[test]
    fn test_registry_register_benchmark() {
        let mut registry = TestRegistry::new();
        
        registry.register_benchmark("bench_example", |b| {
            for _ in 0..b.iterations() {
                let _ = 3 * 3;
            }
        });
        ...
        assert!(registry.benchmarks.contains_key("bench_example"));
    }
    ...
    fn test_summary_creation() {
        ...
        let summary = TestRunSummary::new(test_results, bench_results, total_duration);
    ...
    #[test]
    fn test_global_test_registration() {
        // Clear the global registry first
        GLOBAL_TEST_REGISTRY.lock().unwrap().tests.clear();
        GLOBAL_TEST_REGISTRY.lock().unwrap().benchmarks.clear();
        
        register_test("global_test", |t| {
            t.log(&["Global test executed"]);
        });
        
        register_benchmark("global_bench", |b| {
            for _ in 0..b.iterations() {
                let _ = 4 * 4;
            }
        });
        
        let registry = GLOBAL_TEST_REGISTRY.lock().unwrap();
        assert_eq!(registry.tests.len(), 1);
        assert_eq!(registry.benchmarks.len(), 1);
    }
}
