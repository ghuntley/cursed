//! Cryptographic functionality for ring_signatures
...
pub fn init_ring_signatures() -> CryptoResult<()> {
    let handler = CryptoHandler::new();
    let key = handler.generate_key()?;
    if key.len() != 32 {
        return Err(CursedError::runtime_error("Crypto key generation test failed"));
    }
    println!("🔐 Crypto processing (ring_signatures) initialized");
    Ok(())
}

/// Test crypto functionality
pub fn test_ring_signatures() -> CryptoResult<()> {
    let handler = CryptoHandler::new();
