//! Cryptographic functionality for verifiers
...
pub fn init_verifiers() -> CryptoResult<()> {
    let handler = CryptoHandler::new();
    let key = handler.generate_key()?;
    if key.len() != 32 {
        return Err(CursedError::runtime_error("Crypto key generation test failed"));
    }
    println!("🔐 Crypto processing (verifiers) initialized");
    Ok(())
}

/// Test crypto functionality
pub fn test_verifiers() -> CryptoResult<()> {
    let handler = CryptoHandler::new();
