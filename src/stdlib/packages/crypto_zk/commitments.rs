...
impl PedersenParams {
    ...
    pub fn from_value(value: &Value) -> AdvancedCryptoResult<Self> {
        let obj = match value {
            ...
            _ => return Err(CryptoError::InvalidInput("Expected object for Pedersen params".to_string())),
        };

        let g_str = match obj.get("g") {
            Some(Value::String(s)) => s,
            _ => return Err(CryptoError::InvalidInput("Invalid g parameter".to_string())),
        };

        let h_str = match obj.get("h") {
            Some(Value::String(s)) => s,
            _ => return Err(CryptoError::InvalidInput("Invalid h parameter".to_string())),
        };

        let field_size = match obj.get("field_size") {
            Some(Value::Integer(i)) => *i as usize,
            _ => return Err(CryptoError::InvalidInput("Invalid field_size parameter".to_string())),
        };
...
impl PedersenCommitment {
    ...
    pub fn from_value(value: &Value) -> AdvancedCryptoResult<Self> {
        let obj = match value {
            Value::Object(map) => map,
            _ => return Err(CryptoError::InvalidInput("Expected object for commitment".to_string())),
        };

        let commitment_str = match obj.get("commitment") {
            Some(Value::String(s)) => s,
            _ => return Err(CryptoError::InvalidInput("Invalid commitment field".to_string())),
        };

        let randomness_str = match obj.get("randomness") {
            Some(Value::String(s)) => s,
            _ => return Err(CryptoError::InvalidInput("Invalid randomness field".to_string())),
        };
...
impl HashCommitment {
    ...
    pub fn from_value(value: &Value) -> AdvancedCryptoResult<Self> {
        let obj = match value {
            Value::Object(map) => map,
            _ => return Err(CryptoError::InvalidInput("Expected object for hash commitment".to_string())),
        };

        let commitment = match obj.get("commitment") {
            Some(Value::String(s)) => hex::decode(s)
                .map_err(|_| CryptoError::InvalidInput("Invalid commitment hex".to_string()))?,
            _ => return Err(CryptoError::InvalidInput("Invalid commitment field".to_string())),
        };

        let nonce = match obj.get("nonce") {
            Some(Value::String(s)) => hex::decode(s)
                .map_err(|_| CryptoError::InvalidInput("Invalid nonce hex".to_string()))?,
            _ => return Err(CryptoError::InvalidInput("Invalid nonce field".to_string())),
        };
...
impl KateCommitment {
    ...
    pub fn commit_polynomial(coefficients: &[FieldElement], setup_g: &FieldElement) -> AdvancedCryptoResult<Self> {
        ...
        Ok(Self {
            commitment,
            polynomial_degree: coefficients.len().saturating_sub(1),
        })
    }
...
impl Commitments {
    ...
    fn value_to_field_vector(value: &Value) -> AdvancedCryptoResult<Vec<FieldElement>> {
        match value {
            ...
            _ => Err(CryptoError::InvalidInput("Expected array for field vector".to_string())),
        }
    }
