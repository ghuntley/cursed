//! Functional implementation for orm
...
impl ModuleHandler {
    ...
    pub fn info(&self) -> String {
        format!("Module: orm, Enabled: {}", self.enabled)
    }
}
...
/// Initialize orm processing
pub fn init_orm() -> ModuleResult<()> {
    let handler = ModuleHandler::new();
    let result = handler.process("test")?;
    if !result.contains("test") {
        return Err(CursedError::runtime_error(&"Module test failed".to_string()));
    }
    println!("⚙️  Module processing (orm) initialized");
    Ok(())
}

/// Test orm functionality
pub fn test_orm() -> ModuleResult<()> {
    let handler = ModuleHandler::new();
