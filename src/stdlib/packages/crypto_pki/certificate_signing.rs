...
pub struct CertificateSigner {
    ...
    pub signing_policy: CertificateSigningPolicy,
    /// ASN.1 encoder for certificate structures
    pub asn1_encoder: Asn1Encoder,
    /// Signing statistics
    pub statistics: SigningStatistics,
}
...
pub struct CertificateSigningPolicy {
    ...
    pub min_validity_period: Duration,
    ...
    pub allowed_subject_fields: Vec<String>,
    /// Certificate serial number management
    pub serial_number_policy: SerialNumberPolicy,
...
pub struct ExtensionPolicy {
    ...
    pub auto_basic_constraints: bool,
    /// Automatically add key identifiers
    pub auto_key_identifiers: bool,
}
...
pub struct CertificateSigningRequest {
    ...
    pub template: Option<CertificateTemplate>,
    /// Certificate purpose
...
pub struct CertificateTemplate {
    ...
    /// Required extensions
    pub required_extensions: Vec<X509Extension>,
    /// Extension overrides
    pub extension_overrides: HashMap<String, Vec<u8>>,
}
...
pub struct BatchSigningRequest {
    ...
    pub batch_template: Option<CertificateTemplate>,
    /// Signing options
    pub options: BatchSigningOptions,
}
...
pub struct BatchSigningResult {
    ...
    pub successful_certificates: Vec<X509Certificate>,
    /// Failed signing attempts
    pub failed_requests: Vec<(CertificateSigningRequest, PkiError)>,
    /// Signing statistics
    pub statistics: BatchSigningStatistics,
    /// Detailed report
...
pub struct BatchSigningStatistics {
    ...
    /// Successful signings
    pub successful_signings: usize,
    /// Failed signings
    pub failed_signings: usize,
...
pub struct SigningStatistics {
    ...
    pub certificates_by_algorithm: HashMap<String, u64>,
    /// Certificates by purpose
    pub certificates_by_purpose: HashMap<String, u64>,
    /// Failed signing attempts
    pub failed_signings: u64,
    /// Average signing time (milliseconds)
    pub avg_signing_time_ms: f64,
    /// Total serial numbers issued
    pub serial_numbers_issued: u64,
}
...
pub struct CertificateRenewalRequest {
    ...
    pub original_certificate: X509Certificate,
    /// New public key (if key rotation required)
    pub new_public_key: Option<SubjectPublicKeyInfo>,
    /// New validity period
    pub new_validity_period: Option<Duration>,
    /// Extensions to update
    pub extension_updates: HashMap<String, Vec<u8>>,
    /// Keep original serial number
    pub keep_serial_number: bool,
}
...
impl CertificateSigner {
    ...
    pub fn new(
        ...
        let mut signer = Self {
            ...
            signing_policy: CertificateSigningPolicy::default(),
            asn1_encoder: Asn1Encoder::new(),
            statistics: SigningStatistics::default(),
        };
    ...
    pub fn sign_certificate(&mut self, request: CertificateSigningRequest) -> PkiResult<X509Certificate> {
        ...
        let mut certificate = X509Certificate {
            version: 3, // X.509v3
            serial_number,
            signature_algorithm: self.ca_certificate.signature_algorithm.clone(),
    ...
    pub fn sign_batch(&mut self, batch_request: BatchSigningRequest) -> PkiResult<BatchSigningResult> {
        ...
        let statistics = BatchSigningStatistics {
            total_requests,
            successful_signings: successful_certificates.len(),
            failed_signings: failed_requests.len(),
    ...
    pub fn renew_certificate(&mut self, renewal_request: CertificateRenewalRequest) -> PkiResult<X509Certificate> {
        ...
        let signing_request = CertificateSigningRequest {
            csr,
            validity_period: renewal_request.new_validity_period,
            additional_extensions: Vec::new(),
            template: None,
            purpose: self.determine_certificate_purpose(&renewal_request.original_certificate)?,
        };
    ...
    fn validate_csr_signature(&self, csr: &super::types::CertificateSigningRequest) -> PkiResult<()> {
        ...
        let provider = self.signature_providers.get(&csr.signature_algorithm)
            .ok_or_else(|| PkiError::crypto_error(
                "No signature provider for CSR algorithm",
                "validation"
            ))?;
    ...
    fn validate_against_policy(&self, request: &CertificateSigningRequest) -> PkiResult<()> {
        ...
        if let Some(validity_period) = request.validity_period {
            if validity_period > self.signing_policy.max_validity_period {
                return Err(PkiError::certificate_error(
                    ...
                    CertificateErrorCode::PolicyConstraintsViolation,
                ));
            }
            if validity_period < self.signing_policy.min_validity_period {
                return Err(PkiError::certificate_error(
                    "Requested validity period is below minimum allowed",
                    CertificateErrorCode::PolicyConstraintsViolation,
                ));
            }
        }
        ...
        if self.signing_policy.require_san {
            ...
            if !has_san {
                return Err(PkiError::certificate_error(
                    "Subject Alternative Names are required by policy",
                    CertificateErrorCode::PolicyConstraintsViolation,
                ));
            }
        }
    ...
    fn validate_subject_dn(&self, subject: &DistinguishedName) -> PkiResult<()> {
        ...
        if subject.common_name.is_none() {
            return Err(PkiError::certificate_error(
                "Subject Common Name is required",
        ...
        if let Some(cn) = &subject.common_name {
            if cn.is_empty() || cn.len() > 64 {
                return Err(PkiError::certificate_error(
                    "Subject Common Name length is invalid",
                    CertificateErrorCode::MalformedCertificate,
                ));
            }
        }
    ...
    fn add_standard_extensions(
        ...
        if self.signing_policy.extension_policy.auto_basic_constraints {
            let is_ca = matches!(request.purpose, 
            ...
            certificate.extensions.push(X509Extension {
                ...
                critical: true,
                value: self.encode_basic_constraints(is_ca, None)?,
        ...
        certificate.extensions.push(X509Extension {
            ...
            critical: true,
            value: self.encode_key_usage(&key_usage)?,
            parsed_data: Some(ExtensionData::KeyUsage(key_usage)),
        });
        ...
        if self.has_extended_key_usage(&extended_key_usage) {
            certificate.extensions.push(X509Extension {
                ...
                critical: false,
                value: self.encode_extended_key_usage(&extended_key_usage)?,
                parsed_data: Some(ExtensionData::ExtendedKeyUsage(extended_key_usage)),
            });
        }
    ...
    fn determine_key_usage(&self, purpose: &CertificatePurpose) -> KeyUsage {
        match purpose {
            ...
            CertificatePurpose::Custom(_) => KeyUsage::default(),
        }
    }
    ...
    fn sign_certificate_info(&self, cert_info: &[u8]) -> PkiResult<Vec<u8>> {
        let provider = self.signature_providers.get(&self.ca_certificate.signature_algorithm)
            .ok_or_else(|| PkiError::crypto_error(
                "No signature provider for CA algorithm",
                "signing"
            ))?;
    ...
    fn create_csr_from_renewal(&self, renewal: &CertificateRenewalRequest) -> PkiResult<super::types::CertificateSigningRequest> {
        ...
        Ok(super::types::CertificateSigningRequest {
            ...
            attributes: Vec::new(),
            signature_algorithm: renewal.original_certificate.signature_algorithm.clone(),
            signature: Vec::new(),
            raw_data: Vec::new(),
        })
    }
    ...
    fn generate_batch_report(
        &self,
        statistics: &BatchSigningStatistics,
        failed_requests: &[(CertificateSigningRequest, PkiError)],
    ) -> String {
    ...
    fn encode_algorithm_identifier(&self, algorithm: &PublicKeyAlgorithm) -> PkiResult<Vec<u8>> {
        ...
        let oid = match algorithm {
            ...
            PublicKeyAlgorithm::EllipticCurve { .. } => "1.2.840.10045.2.1",
