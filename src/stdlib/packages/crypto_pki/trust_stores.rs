//! Cryptographic functionality for trust_stores
...
pub fn init_trust_stores() -> CryptoResult<()> {
    let handler = CryptoHandler::new();
    let key = handler.generate_key()?;
    if key.len() != 32 {
        return Err(CursedError::runtime_error("Crypto key generation test failed"));
    }
    println!("🔐 Crypto processing (trust_stores) initialized");
    Ok(())
}

/// Test crypto functionality
pub fn test_trust_stores() -> CryptoResult<()> {
    let handler = CryptoHandler::new();
