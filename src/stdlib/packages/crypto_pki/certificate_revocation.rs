...
/// - Error condition and security validation testing
...
mod tests {
    ...
    fn create_test_serial_number(value: u64) -> SerialNumber {
        SerialNumber::from_big_int(value)
    }
    
    fn create_test_distinguished_name() -> DistinguishedName {
        DistinguishedName {
            common_name: Some("Test CA".to_string()),
            organization: Some("Test Org".to_string()),
            organizational_unit: Some("Test OU".to_string()),
            country: Some("US".to_string()),
            state_or_province: Some("CA".to_string()),
    ...
    fn create_test_audit_info() -> RevocationAuditInfo {
        RevocationAuditInfo {
            initiated_by: "test_user".to_string(),
            authorization: "test_auth_token".to_string(),
    ...
    #[test]
    fn test_in_memory_revocation_database() {
        let mut database = InMemoryRevocationDatabase::new();
        
        let serial_number = create_test_serial_number(12345);
        let issuer = create_test_distinguished_name();
        
        // Initially not revoked
        let result = database.is_revoked(&serial_number, &issuer).unwrap();
        ...
        let entry = RevocationEntry {
            ...
            entry_created: SystemTime::now(),
            entry_modified: SystemTime::now(),
            audit_info: create_test_audit_info(),
        };
        
        database.add_revocation(entry.clone()).unwrap();
    ...
    #[test]
    fn test_revocation_manager_basic_operations() {
        let database = Arc::new(Mutex::new(InMemoryRevocationDatabase::new()));
        let crl_config = CrlConfig::default();
        let ocsp_config = OcspConfig::default();
        
        let mut manager = CertificateRevocationManager::new(database, crl_config, ocsp_config);
        
        let serial_number = create_test_serial_number(54321);
        let issuer = create_test_distinguished_name();
        
        // Initially not revoked
        let status = manager.check_revocation_status(&serial_number, &issuer).unwrap();
        ...
        match status {
            CertificateRevocationStatus::Revoked { reason, .. } => {
                assert_eq!(reason, RevocationReason::CessationOfOperation);
            }
    ...
    fn test_crl_generation() {
        ...
        for i in 1..=5 {
            manager.revoke_certificate(
                ...
                create_test_distinguished_name(),
    ...
    #[test]
    fn test_ocsp_request_processing() {
        let database = Arc::new(Mutex::new(InMemoryRevocationDatabase::new()));
        let crl_config = CrlConfig::default();
        let ocsp_config = OcspConfig::default();
        ...
        let serial_number = create_test_serial_number(99999);
        let issuer = create_test_distinguished_name();
        
        // Test OCSP for valid certificate
        let request = OcspRequest {
            certificate_serial: serial_number.clone(),
            issuer: issuer.clone(),
        ...
        let response = manager.process_ocsp_request(request.clone()).unwrap();
        ...
        match response.certificate_status {
            OcspCertificateStatus::Revoked { reason, .. } => {
                assert_eq!(reason, Some(RevocationReason::KeyCompromise));
            }
            _ => panic!("Certificate should be revoked in OCSP response"),
        }
    ...
    fn test_batch_revocation() {
        ...
        let revocations = vec![
            (create_test_serial_number(1001), create_test_distinguished_name(), RevocationReason::KeyCompromise, None, create_test_audit_info()),
            (create_test_serial_number(1002), create_test_distinguished_name(), RevocationReason::Superseded, None, create_test_audit_info()),
            (create_test_serial_number(1003), create_test_distinguished_name(), RevocationReason::CessationOfOperation, None, create_test_audit_info()),
        ];
        
        let result = manager.revoke_certificates_batch(revocations).unwrap();
        
        assert_eq!(result.successful_revocations.len(), 3);
        assert_eq!(result.failed_revocations.len(), 0);
        assert_eq!(result.total_processed, 3);
        
        // Verify all certificates are revoked
        for i in 1001..=1003 {
            let status = manager.check_revocation_status(&create_test_serial_number(i), &create_test_distinguished_name()).unwrap();
            assert!(matches!(status, CertificateRevocationStatus::Revoked { .. }));
        }
    }
    ...
    fn test_emergency_revocation() {
        ...
        let certificates = vec![
            (create_test_serial_number(2001), create_test_distinguished_name()),
            (create_test_serial_number(2002), create_test_distinguished_name()),
        ];
        
        manager.emergency_revoke_certificates(
            certificates,
            RevocationReason::CaCompromise,
            "EMERGENCY_AUTH_12345".to_string(),
        ).unwrap();
        ...
        for i in 2001..=2002 {
            ...
            match status {
                ...
                _ => panic!("Certificate should be revoked in emergency procedure"),
            }
        }
        
        // Check statistics include emergency revocations
        let stats = manager.get_statistics().unwrap();
        assert_eq!(stats.emergency_revocations, 2);
    }
    
    #[test]
    fn test_revocation_reasons_display() {
    ...
    #[test]
    fn test_serial_number_conversions() {
        let original_value = 0x123456789ABCDEF0u64;
        let serial = SerialNumber::from_big_int(original_value);
