//! Functional implementation for query
...
impl ModuleHandler {
    ...
    pub fn info(&self) -> String {
        format!("Module: query, Enabled: {}", self.enabled)
    }
}
...
pub fn init_query() -> ModuleResult<()> {
    let handler = ModuleHandler::new();
    let result = handler.process("test")?;
    if !result.contains("test") {
        return Err(CursedError::runtime_error(&"Module test failed".to_string()));
    }
    println!("⚙️  Module processing (query) initialized");
    Ok(())
}

/// Test query functionality
pub fn test_query() -> ModuleResult<()> {
    let handler = ModuleHandler::new();
