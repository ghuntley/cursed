...
mod tests {
    ...
    fn test_siphash_empty() {
        ...
        let mut hasher2 = SipHash::new(&key);
        let result2 = hasher2.hash(b"");
    ...
    fn test_siphash_known_vector() {
        ...
        let key = [0u8; 16];
        let result = siphash_24(b"", &key);
        
        // Different key should give different result
        let key2 = [1u8; 16];
        let result2 = siphash_24(b"", &key2);
    ...
    fn test_siphash_incremental() {
        ...
        let mut hasher1 = SipHash::new(&key);
    ...
    #[test]
    fn test_siphash_variants() {
        let key = [123u8; 16];
        let data = b"test data";
        
        let result_24 = siphash_24(data, &key);
        let result_13 = siphash_13(data, &key);
        
        // Different variants should produce different results
        assert_ne!(result_24, result_13);
        
        // But each should be consistent
        assert_eq!(result_24, siphash_24(data, &key));
        assert_eq!(result_13, siphash_13(data, &key));
    }
    ...
    fn test_siphash_key_sensitivity() {
        ...
        let result1 = siphash_24(data, &key1);
        let result2 = siphash_24(data, &key2);
        
        assert_ne!(result1, result2);
    }

    #[test]
    fn test_siphash_set_key() {
        let key1 = [1u8; 16];
        let key2 = [2u8; 16];
        
        let mut hasher = SipHash::new(&key1);
        let result1 = hasher.hash(b"test");
        
        hasher.set_key(&key2).unwrap();
        let result2 = hasher.hash(b"test");
        
        assert_ne!(result1, result2);
    }

    #[test]
    fn test_siphash_invalid_key() {
        let mut hasher = SipHash::new(&[0u8; 16]);
        
        // Too short key
        assert!(hasher.set_key(&[1, 2, 3]).is_err());
        
        // Too long key
        assert!(hasher.set_key(&[0u8; 20]).is_err());
    }

    #[test]
    fn test_siphash_hashmap() {
        let key = [42u8; 16];
        let mut hasher = SipHashHashMap::with_key(&key);
        
        let result1 = hasher.hash_u64(12345);
        let result2 = hasher.hash_u64(12345);
        assert_eq!(result1, result2);
        
        let result3 = hasher.hash_u64(67890);
        assert_ne!(result1, result3);
        
        let str_result = hasher.hash_str("hello");
        assert_ne!(str_result, 0);
    }

    #[test]
    fn test_generate_siphash_key() {
        let key1 = generate_siphash_key().unwrap();
        let key2 = generate_siphash_key().unwrap();
    ...
    #[test]
    fn test_siphash_large_data() {
        let key = [0u8; 16];
        let large_data = vec![42u8; 10000];
        
        let result1 = siphash_24(&large_data, &key);
        let result2 = siphash_24(&large_data, &key);
        assert_eq!(result1, result2);
        
        // Modify one byte
        let mut modified_data = large_data.clone();
        modified_data[5000] = 43;
        let result3 = siphash_24(&modified_data, &key);
        assert_ne!(result1, result3);
    }
}
