...
mod tests {
    ...
    fn test_keccak256_hello() {
        ...
        let result2 = keccak256(b"hello");
        assert_eq!(result, result2);
        
        // Different from empty
        let empty_result = keccak256(b"");
    ...
    fn test_keccak_variants() {
        ...
        let result224 = keccak224(data);
        let result256 = keccak256(data);
        let result384 = keccak384(data);
        let result512 = keccak512(data);
        
        assert_eq!(result224.len(), 28);
        assert_eq!(result256.len(), 32);
        assert_eq!(result384.len(), 48);
        assert_eq!(result512.len(), 64);
        
        // All should be different
        assert_ne!(result224, result256[..28]);
        assert_ne!(result256, result384[..32]);
        assert_ne!(result384, result512[..48]);
    }

    #[test]
    fn test_keccak_incremental() {
        let mut hasher1 = KeccakHasher::keccak256();
        hasher1.update(b"hello");
        hasher1.update(b"world");
        let result1 = hasher1.finalize();
        
        let result2 = keccak256(b"helloworld");
        
        assert_eq!(result1, result2);
    }

    #[test]
    fn test_keccak_reset() {
        let mut hasher = KeccakHasher::keccak256();
        hasher.update(b"some data");
        hasher.reset();
        
        let result = hasher.hash(b"different data");
        let expected = keccak256(b"different data");
        
        assert_eq!(result, expected);
    }

    #[test]
    fn test_ethereum_keccak256() {
        let result = ethereum_keccak256(b"hello");
        assert_eq!(result.len(), 32);
        
        // Should match regular keccak256
        let regular_result = keccak256(b"hello");
        assert_eq!(result.to_vec(), regular_result);
    }
    ...
    fn test_shake128() {
        ...
        let output1 = shake.read(32);
        assert_eq!(output1.len(), 32);
        
        // Different lengths should give different results (when truncated)
        let mut shake2 = Shake128::new();
        shake2.update(b"hello");
        let output2 = shake2.read(16);
        assert_eq!(output2.len(), 16);
        assert_eq!(output1[..16], output2);
    }
    ...
    fn test_shake256() {
        ...
        let output = shake.read(64);
        assert_eq!(output.len(), 64);
        
        // Should be deterministic
        let mut shake2 = Shake256::new();
        shake2.update(b"test");
        let output2 = shake2.read(64);
        assert_eq!(output, output2);
    }

    #[test]
    fn test_keccak_large_data() {
        let large_data = vec![42u8; 10000];
        let result1 = keccak256(&large_data);
        let result2 = keccak256(&large_data);
        assert_eq!(result1, result2);
    ...
    #[test]
    fn test_keccak_security_levels() {
        let hasher256 = KeccakHasher::keccak256();
        assert_eq!(hasher256.security_level(), 128);
        assert_eq!(hasher256.collision_resistance(), SecurityLevel::Strong);
        assert!(!hasher256.is_quantum_resistant());
        
        let hasher512 = KeccakHasher::keccak512();
        assert_eq!(hasher512.security_level(), 256);
        assert_eq!(hasher512.collision_resistance(), SecurityLevel::QuantumResistant);
    }
}
