//! Cryptographic functionality for hardware_entropy
...
pub fn init_hardware_entropy() -> CryptoResult<()> {
    let handler = CryptoHandler::new();
    let key = handler.generate_key()?;
    if key.len() != 32 {
        return Err(CursedError::runtime_error("Crypto key generation test failed"));
    }
    println!("🔐 Crypto processing (hardware_entropy) initialized");
    Ok(())
}

/// Test crypto functionality
pub fn test_hardware_entropy() -> CryptoResult<()> {
    let handler = CryptoHandler::new();
