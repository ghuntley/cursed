...
mod tests {
    ...
    fn test_keypair_generation() {
        ...
        for curve in [EcdhCurve::P256, EcdhCurve::P384, EcdhCurve::X25519] {
            let keypair = manager.generate_keypair(curve.clone()).unwrap();
    ...
    fn test_shared_secret_computation() {
        ...
        let keypair1 = manager.generate_keypair(EcdhCurve::X25519).unwrap();
        let keypair2 = manager.generate_keypair(EcdhCurve::X25519).unwrap();
        ...
        // Shared secrets should be equal (ECDH property)
    ...
    fn test_public_key_validation() {
        ...
        let keypair = manager.generate_keypair(EcdhCurve::P256).unwrap();
        
        // Valid key
        assert!(manager.validate_public_key(&EcdhCurve::P256, &keypair.public_key).unwrap());
        
        // Invalid key (wrong size)
        assert!(!manager.validate_public_key(&EcdhCurve::P256, &[1, 2, 3]).unwrap());
        
        // Empty key
        assert!(!manager.validate_public_key(&EcdhCurve::P256, &[]).unwrap());
    }
    ...
    fn test_curve_info() {
        ...
        let p256_info = manager.curve_info(&EcdhCurve::P256);
        assert_eq!(p256_info.name, "P-256");
        assert_eq!(p256_info.security_level, 128);
        
        let x25519_info = manager.curve_info(&EcdhCurve::X25519);
        assert_eq!(x25519_info.name, "X25519");
        assert_eq!(x25519_info.key_size, 32);
    }
    ...
    fn test_all_curves() {
        ...
        for curve in curves {
            ...
            let info = manager.curve_info(&curve);
            assert_eq!(keypair.private_key.len(), info.key_size);
            assert_eq!(keypair.public_key.len(), info.public_key_size);
        }
    }
    ...
    fn test_display_formatting() {
        ...
        assert!(display.contains("P-256"));
        assert!(display.contains("EcdhKeyPair"));
        
        let curve_display = format!("{}", EcdhCurve::X25519);
        assert_eq!(curve_display, "X25519");
    }

    #[test]
    fn test_benchmark() {
        let benchmark = EcdhBenchmark::new().unwrap();
        
        let keygen_time = benchmark.benchmark_keygen(EcdhCurve::X25519, 5).unwrap();
        assert!(keygen_time.as_millis() < 1000);
        
        let shared_secret_time = benchmark.benchmark_shared_secret(EcdhCurve::X25519, 5).unwrap();
