...
impl DiffieHellmanManager {
    ...
    pub fn compute_shared_secret(&self, private_key: &DhKeyPair, peer_public_key: &[u8]) -> AdvancedCryptoResult<DhSharedSecret> {
        ...
        let secret = self.modular_exponentiation(
            ...
            &private_key.params.prime,
        )?;

        Ok(DhSharedSecret {
            secret,
            group: private_key.params.group.clone(),
            bit_length: private_key.params.bit_length,
        })
    }
    ...
    pub fn group_info(&self, group: &DhGroup) -> GroupInfo {
        match group {
            ...
            DhGroup::Group5 => GroupInfo {
                name: "Group 5".to_string(),
                bit_length: 1536,
                security_level: 112,
                recommended: true,
                rfc: "RFC 3526".to_string(),
            },
            DhGroup::Group14 => GroupInfo {
                name: "Group 14".to_string(),
                bit_length: 2048,
                security_level: 112,
                recommended: true,
                rfc: "RFC 3526".to_string(),
...
impl fmt::Display for DhGroup {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            DhGroup::Group1 => write!(f, "Group 1 (768-bit)"),
            DhGroup::Group2 => write!(f, "Group 2 (1024-bit)"),
            DhGroup::Group5 => write!(f, "Group 5 (1536-bit)"),
            DhGroup::Group14 => write!(f, "Group 14 (2048-bit)"),
            DhGroup::Group15 => write!(f, "Group 15 (3072-bit)"),
            DhGroup::Group16 => write!(f, "Group 16 (4096-bit)"),
            DhGroup::Group17 => write!(f, "Group 17 (6144-bit)"),
            DhGroup::Group18 => write!(f, "Group 18 (8192-bit)"),
        }
    }
}
...
impl fmt::Display for DhKeyPair {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "DhKeyPair(group: {}, private: {}B, public: {}B)", 
...
pub struct DhBenchmark {
    manager: DiffieHellmanManager,
}

impl DhBenchmark {
    pub fn new() -> AdvancedCryptoResult<Self> {
        Ok(Self {
            manager: DiffieHellmanManager::new()?,
        })
    }
...
mod tests {
    ...
    fn test_group_params() {
        ...
        let params = manager.get_group_params(DhGroup::Group14).unwrap();
        assert_eq!(params.group, DhGroup::Group14);
        assert_eq!(params.bit_length, 2048);
        assert_eq!(params.generator, vec![2]);
        assert_eq!(params.prime.len(), 256); // 2048 bits = 256 bytes
    }
    ...
    fn test_keypair_generation() {
        ...
        let keypair = manager.generate_keypair(DhGroup::Group14).unwrap();
        assert_eq!(keypair.params.group, DhGroup::Group14);
    ...
    fn test_shared_secret_computation() {
        ...
        let keypair1 = manager.generate_keypair(DhGroup::Group14).unwrap();
        let keypair2 = manager.generate_keypair(DhGroup::Group14).unwrap();
        
        let secret1 = manager.compute_shared_secret(&keypair1, &keypair2.public_key).unwrap();
        let secret2 = manager.compute_shared_secret(&keypair2, &keypair1.public_key).unwrap();
    ...
    fn test_public_key_validation() {
        ...
        assert!(manager.validate_public_key(&params, &valid_key).unwrap());
        
        // Invalid key (empty)
        assert!(!manager.validate_public_key(&params, &[]).unwrap());
        
        // Invalid key (all zeros)
        assert!(!manager.validate_public_key(&params, &vec![0; 32]).unwrap());
        
        // Invalid key (value 1)
        assert!(!manager.validate_public_key(&params, &[1]).unwrap());
    }
    ...
    fn test_group_info() {
        ...
        let group14_info = manager.group_info(&DhGroup::Group14);
        assert_eq!(group14_info.name, "Group 14");
        assert_eq!(group14_info.bit_length, 2048);
        assert!(group14_info.recommended);
        
        let group1_info = manager.group_info(&DhGroup::Group1);
        assert!(!group1_info.recommended); // Deprecated
    }
    ...
    fn test_all_groups() {
        ...
        for group in groups {
            let params = manager.get_group_params(group.clone()).unwrap();
            assert_eq!(params.group, group);
            
            let info = manager.group_info(&group);
            assert_eq!(params.bit_length, info.bit_length);
        }
    }
    ...
    fn test_display_formatting() {
        ...
        let display = format!("{}", keypair);
        assert!(display.contains("Group 14"));
        assert!(display.contains("DhKeyPair"));
        
        let group_display = format!("{}", DhGroup::Group14);
        assert!(group_display.contains("2048-bit"));
    }

    #[test]
    fn test_benchmark() {
        let benchmark = DhBenchmark::new().unwrap();
        
        let keygen_time = benchmark.benchmark_keygen(DhGroup::Group14, 3).unwrap();
        assert!(keygen_time.as_millis() < 1000);
        
        let shared_secret_time = benchmark.benchmark_shared_secret(DhGroup::Group14, 3).unwrap();
        assert!(shared_secret_time.as_millis() < 1000);
    }
}
