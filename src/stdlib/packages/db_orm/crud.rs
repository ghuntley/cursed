//! Functional implementation for crud
...
impl ModuleHandler {
    ...
    pub fn info(&self) -> String {
        format!("Module: crud, Enabled: {}", self.enabled)
    }
}
...
/// Initialize crud processing
pub fn init_crud() -> ModuleResult<()> {
    let handler = ModuleHandler::new();
    let result = handler.process("test")?;
    if !result.contains("test") {
        return Err(CursedError::runtime_error(&"Module test failed".to_string()));
    }
    println!("⚙️  Module processing (crud) initialized");
    Ok(())
}

/// Test crud functionality
pub fn test_crud() -> ModuleResult<()> {
    let handler = ModuleHandler::new();
