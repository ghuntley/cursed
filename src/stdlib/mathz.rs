...
mod tests {
    ...
    fn test_thicc_operations() {
        assert_eq!(abs_thicc(-1000000000), 1000000000);
    ...
    fn test_utility_functions() {
        ...
        let geo_values = [1.0, 4.0, 9.0];
        assert!((geometric_mean_chonky(&geo_values).unwrap() - (1.0 * 4.0 * 9.0).powf(1.0/3.0)).abs() < 1e-10);
        
        let harm_values = [1.0, 2.0, 4.0];
        let expected_harmonic = 3.0 / (1.0/1.0 + 1.0/2.0 + 1.0/4.0);
        assert!((harmonic_mean_chonky(&harm_values).unwrap() - expected_harmonic).abs() < 1e-10);
    }
    ...
    fn test_random_functions() {
        ...
        let _r_int = random_normie();
        let _r_bool = random_bool();
    }
    
    #[test]
    fn test_constants() {
        assert!((PI_CHONKY - 3.141592653589793).abs() < 1e-10);
        assert!((TAU_CHONKY - 2.0 * PI_CHONKY).abs() < 1e-10);
        assert!((E_CHONKY - 2.718281828459045).abs() < 1e-10);
        assert!((PHI_CHONKY - 1.618033988749895).abs() < 1e-10);
        assert!((SQRT_2_CHONKY - 1.4142135623730951).abs() < 1e-10);
        assert!((LN_2_CHONKY - 0.6931471805599453).abs() < 1e-10);
        assert!((LN_10_CHONKY - 2.302585092994046).abs() < 1e-10);
    }
}
