...
mod tests {
    ...
    fn test_sqlite_flags() {
        ...
        assert!(SqliteFlags::has_flag(combined, SqliteFlags::Create));
        assert!(!SqliteFlags::has_flag(combined, SqliteFlags::ReadOnly));

        assert_eq!(SqliteFlags::default_flags(), 0x06);
        assert_eq!(SqliteFlags::readonly_flags(), 0x01);
    }

    #[test]
    fn test_journal_mode() {
        assert_eq!(SqliteJournalMode::Wal.as_str(), "WAL");
        assert_eq!(SqliteJournalMode::Delete.as_str(), "DELETE");

        assert_eq!(SqliteJournalMode::from_str("WAL").unwrap(), SqliteJournalMode::Wal);
        assert_eq!(SqliteJournalMode::from_str("wal").unwrap(), SqliteJournalMode::Wal);
        assert!(SqliteJournalMode::from_str("invalid").is_err());

        assert_eq!(SqliteJournalMode::Wal.pragma_statement(), "PRAGMA journal_mode = WAL");
    }

    #[test]
    fn test_synchronous_mode() {
        assert_eq!(SqliteSynchronous::Full.as_str(), "FULL");
        assert_eq!(SqliteSynchronous::Normal.value(), 1);

        assert_eq!(SqliteSynchronous::from_str("NORMAL").unwrap(), SqliteSynchronous::Normal);
        assert_eq!(SqliteSynchronous::from_str("2").unwrap(), SqliteSynchronous::Full);
        assert!(SqliteSynchronous::from_str("invalid").is_err());

        assert_eq!(SqliteSynchronous::Normal.pragma_statement(), "PRAGMA synchronous = NORMAL");
    }
    ...
    fn test_config_creation() {
        ...
        let readonly_config = SqliteConfig::readonly("readonly.db");
        assert!(readonly_config.is_readonly());
    ...
    fn test_config_validation() {
        let mut config = SqliteConfig::new("test.db");
        assert!(config.validate().is_ok());

        config.database_path = String::new();
        assert!(config.validate().is_err());

        config.database_path = "test.db".to_string();
        config.page_size = 1000; // Not power of 2
        assert!(config.validate().is_err());
    ...
    fn test_initialization_sql() {
        ...
        let sql_statements = config.initialization_sql();
        
        assert!(sql_statements.iter().any(|s| s.contains("PRAGMA journal_mode = WAL")));
        assert!(sql_statements.iter().any(|s| s.contains("PRAGMA page_size = 4096")));
        assert!(sql_statements.iter().any(|s| s.contains("PRAGMA foreign_keys = ON")));
    }
    ...
    fn test_connection_string_parsing() {
        ...
        // Memory database
        let conn_str = SqliteConnectionString::parse(":memory:").unwrap();
        assert!(conn_str.config.is_memory_database());

        // URI format
        let conn_str = SqliteConnectionString::parse("file:test.db?mode=ro&cache=shared").unwrap();
        assert_eq!(conn_str.config.database_path, "test.db");
        assert!(conn_str.config.is_readonly());

        // Data source format
        let conn_str = SqliteConnectionString::parse("Data Source=test.db;Journal Mode=WAL;Foreign Keys=true").unwrap();
        assert_eq!(conn_str.config.database_path, "test.db");
        assert_eq!(conn_str.config.journal_mode, SqliteJournalMode::Wal);
        assert!(conn_str.config.foreign_keys);
    }
    ...
    fn test_config_helpers() {
        ...
        config.add_pragma("PRAGMA temp_store = memory");
        assert_eq!(config.custom_pragmas.len(), 1);

        config.add_collation("NOCASE", "SELECT 1");
        assert_eq!(config.custom_collations.len(), 1);

        config.add_extension("fts5");
        assert_eq!(config.extensions.len(), 1);

        config.add_function("custom_func");
        assert_eq!(config.custom_functions.len(), 1);

        config.add_parameter("custom", "value");
    ...
    fn test_config_presets() {
        ...
        assert_eq!(high_perf.journal_mode, SqliteJournalMode::Wal);
