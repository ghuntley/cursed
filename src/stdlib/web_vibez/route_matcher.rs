...
mod tests {
    ...
    #[test]
    fn test_static_route_compilation() {
        let pattern = RoutePattern::compile("/users/profile").unwrap();
        assert_eq!(pattern.segments.len(), 2);
        assert_eq!(pattern.segments[0], PathSegment::Static("users".to_string()));
        assert_eq!(pattern.segments[1], PathSegment::Static("profile".to_string()));
    ...
    #[test]
    fn test_wildcard_route_compilation() {
    ...
    fn test_route_matching() {
        ...
        let route_match = pattern.matches("/users/123").unwrap();
        assert_eq!(route_match.param("id"), Some("123"));
    }

    #[test]
    fn test_route_matcher_basic() {
        let mut matcher = RouteMatcher::new(100);
        matcher.add_route("/users/:id").unwrap();
        matcher.add_route("/users/profile").unwrap();
        
        let route_match = matcher.find_match("/users/123").unwrap();
        assert_eq!(route_match.param("id"), Some("123"));
        
        let profile_match = matcher.find_match("/users/profile").unwrap();
        assert_eq!(profile_match.pattern.pattern, "/users/profile");
    }

    #[test]
    fn test_wildcard_matching() {
        let mut matcher = RouteMatcher::new(100);
        matcher.add_route("/files/*").unwrap();
        matcher.add_route("/api/**").unwrap();
        
        let file_match = matcher.find_match("/files/document.pdf").unwrap();
        assert!(file_match.pattern.has_wildcards);
        
        let api_match = matcher.find_match("/api/v1/users/123").unwrap();
        assert!(api_match.pattern.has_wildcards);
    }
}
