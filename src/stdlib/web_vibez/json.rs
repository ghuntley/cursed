//! Functional implementation for json
...
impl ModuleHandler {
    ...
    pub fn info(&self) -> String {
        format!("Module: json, Enabled: {}", self.enabled)
    }
}
...
pub fn init_json() -> ModuleResult<()> {
    let handler = ModuleHandler::new();
    let result = handler.process("test")?;
    if !result.contains("test") {
        return Err(CursedError::runtime_error("Module test failed"));
    }
    println!("⚙️  Module processing (json) initialized");
    Ok(())
}

/// Test json functionality
pub fn test_json() -> ModuleResult<()> {
    let handler = ModuleHandler::new();
