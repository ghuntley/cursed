//! Functional implementation for parallel
...
impl ModuleHandler {
    ...
    pub fn info(&self) -> String {
        format!("Module: parallel, Enabled: {}", self.enabled)
    }
}
...
pub fn init_parallel() -> ModuleResult<()> {
    let handler = ModuleHandler::new();
    let result = handler.process("test")?;
    if !result.contains("test") {
        return Err(CursedError::runtime_error("Module test failed"));
    }
    println!("⚙️  Module processing (parallel) initialized");
    Ok(())
}

/// Test parallel functionality
pub fn test_parallel() -> ModuleResult<()> {
    let handler = ModuleHandler::new();
