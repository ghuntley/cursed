...
mod tests {
    ...
    #[test]
    fn test_thread_local_cleanup() {
        let key: ThreadLocalKey<String> = ThreadLocalKey::new();
        let cleanup_called = Arc::new(AtomicI32::new(0));
        
        {
            let cleanup_counter = Arc::clone(&cleanup_called);
            key.register_cleanup(move |_value: &mut String| {
                cleanup_counter.fetch_add(1, Ordering::Relaxed);
            }).unwrap();
        }

        key.set("test".to_string()).unwrap();
        
        // Cleanup current thread
        cleanup_current_thread().unwrap();
        
        // Note: In this simple test, cleanup won't be called because
        // the thread hasn't actually ended. In a real scenario,
        // cleanup would be called when threads terminate.
    }

    #[test]
    fn test_thread_local_cell() {
        let cell = ThreadLocalCell::<i32>::new();
    ...
    fn test_thread_local_value() {
        let value = ThreadLocalValue::<i32>::new();
    ...
    fn test_thread_local_statistics() {
        let _key1: ThreadLocalKey<i32> = ThreadLocalKey::new();
        let _key2: ThreadLocalKey<String> = ThreadLocalKey::named("test");
        
        let stats = get_thread_local_statistics().unwrap();
