...
mod tests {
    ...
    fn test_trie_operations() {
        ...
        let prefixed = trie.keys_with_prefix("hel");
        assert!(prefixed.contains(&"hello".to_string()));
        assert!(prefixed.contains(&"help".to_string()));
        
        assert_eq!(trie.longest_prefix("helping"), Some("help".to_string()));
    ...
    fn test_graph_operations() {
        ...
        graph.add_edge(v2, v3, 2.0).unwrap();
        graph.add_edge(v1, v3, 4.0).unwrap();
        
        assert_eq!(graph.vertex_count(), 3);
        assert_eq!(graph.edge_count(), 3);
        
        let dfs_result = graph.dfs(v1);
        assert!(!dfs_result.is_empty());
        
        let (path, distance) = graph.shortest_path(v1, v3).unwrap();
        assert_eq!(distance, 3.0); // Via v2
    ...
    fn test_bloom_filter() {
        ...
        assert!(!filter.contains(&"missing")); // Might give false positive but unlikely
    ...
    fn test_quadtree() {
        ...
        qtree.insert(Point2D::new(10.0, 10.0), "Point1");
        qtree.insert(Point2D::new(90.0, 90.0), "Point2");
        qtree.insert(Point2D::new(10.0, 90.0), "Point3");
        qtree.insert(Point2D::new(90.0, 10.0), "Point4");
        
        let query_rect = Rectangle::new(0.0, 0.0, 50.0, 50.0);
        let results = qtree.query_range(&query_rect);
        assert_eq!(results.len(), 2); // Point1 and Point4
        
        let nearest = qtree.nearest_neighbor(&Point2D::new(12.0, 12.0));
    ...
    fn test_point_operations() {
        ...
        let rect = Rectangle::new(1.0, 1.0, 5.0, 5.0);
        assert!(rect.contains(&Point2D::new(3.0, 3.0)));
