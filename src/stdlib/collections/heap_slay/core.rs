...
mod tests {
    ...
    // Simple test implementation of Interface for integers
    struct TestHeap {
        data: Vec<i32>,
    }
    ...
    #[test]
    fn test_init_single_element() {
        let mut heap = TestHeap::from(vec![42]);
        init(&mut heap);
        assert!(is_heap(&heap));
    }
    
    #[test]
    fn test_init_multiple_elements() {
        let mut heap = TestHeap::from(vec![3, 1, 4, 1, 5, 9, 2, 6]);
        init(&mut heap);
        assert!(is_heap(&heap));
    }
    
    #[test]
    fn test_push_to_empty_heap() {
        let mut heap = TestHeap::new();
        push(&mut heap, Value::Integer(42));
    ...
    fn test_pop_empty_heap() {
        let mut heap = TestHeap::new();
        let result = pop(&mut heap).unwrap();
    ...
    #[test]
    fn test_pop_maintains_heap_property() {
        let mut heap = TestHeap::from(vec![1, 3, 2, 7, 4, 6, 5]);
        init(&mut heap);
    ...
    fn test_remove_last_element() {
        let mut heap = TestHeap::from(vec![1, 3, 2]);
        init(&mut heap);
        
        let removed = remove(&mut heap, 2).unwrap();
        assert_eq!(removed, Value::Integer(3)); // Element at index 2 after heapify
    ...
    fn test_remove_middle_element() {
        let mut heap = TestHeap::from(vec![1, 2, 3, 4, 5]);
        init(&mut heap);
        
        let removed = remove(&mut heap, 1).unwrap();
    ...
    fn test_remove_invalid_index() {
        let mut heap = TestHeap::from(vec![1, 2, 3]);
        let result = remove(&mut heap, 5);
    ...
    fn test_fix_element() {
        ...
        // Manually modify an element (simulating priority change)
        heap.data[1] = 0;
        fix(&mut heap, 1).unwrap();
        
        assert!(is_heap(&heap));
    }
    
    #[test]
    fn test_is_heap_valid() {
        let mut heap = TestHeap::from(vec![1, 2, 3, 4, 5]);
        init(&mut heap);
        assert!(is_heap(&heap));
    }
    ...
    #[test]
    fn test_is_heap_invalid() {
        let heap = TestHeap::from(vec![3, 1, 2]); // Not a valid min-heap
        assert!(!is_heap(&heap));
    }
    
    #[test]
    fn test_heap_sort_behavior() {
        let mut heap = TestHeap::from(vec![3, 1, 4, 1, 5, 9, 2, 6]);
        init(&mut heap);
