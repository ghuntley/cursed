...
mod tests {
    ...
    fn test_int32_operations() {
        let atomic = Int32::new(42);
        assert_eq!(atomic.load(), 42);
        
        atomic.store(100);
        assert_eq!(atomic.load(), 100);
        
        let old = atomic.add(10);
        assert_eq!(old, 100);
        assert_eq!(atomic.load(), 110);
        
        let swapped = atomic.swap(200);
        assert_eq!(swapped, 110);
    ...
    fn test_float64_operations() {
        let atomic = Float64::new(3.14);
        assert_eq!(atomic.load(), 3.14);
        
        atomic.store(2.71);
        assert_eq!(atomic.load(), 2.71);
        
        let old = atomic.swap(1.41);
        assert_eq!(old, 2.71);
        assert_eq!(atomic.load(), 1.41);
    }

    #[test]
    fn test_string_operations() {
        let atomic = String::new("hello".to_string());
        assert_eq!(atomic.load(), "hello");
        
        atomic.store("world".to_string());
        assert_eq!(atomic.load(), "world");
        
        let old = atomic.swap("foo".to_string());
        assert_eq!(old, "world");
        assert_eq!(atomic.load(), "foo");
        
        assert!(atomic.compare_and_swap("foo".to_string(), "bar".to_string()));
        assert_eq!(atomic.load(), "bar");
        assert!(!atomic.compare_and_swap("foo".to_string(), "baz".to_string()));
        assert_eq!(atomic.load(), "bar");
    }
    ...
    fn test_pointer_operations() {
        ...
        let atomic = Pointer::new(ptr);
        
        assert_eq!(atomic.load(), ptr);
        
        let mut other = 24i32;
        let other_ptr = &mut other as *mut i32;
        atomic.store(other_ptr);
        assert_eq!(atomic.load(), other_ptr);
        
        let old = atomic.swap(ptr);
        assert_eq!(old, other_ptr);
        assert_eq!(atomic.load(), ptr);
    }

    #[test]
    fn test_memory_ordering() {
        let atomic = Int32::new(0);
        
        atomic.store_ordered(10, MemoryOrder::Release);
        let value = atomic.load_ordered(MemoryOrder::Acquire);
        assert_eq!(value, 10);
        
        let old = atomic.add_ordered(5, MemoryOrder::Relaxed);
        assert_eq!(old, 10);
