//! Cryptographic functionality for kyber_real
...
pub fn init_kyber_real() -> CryptoResult<()> {
    let handler = CryptoHandler::new();
    let key = handler.generate_key()?;
    if key.len() != 32 {
        return Err(CursedError::runtime_error("Crypto key generation test failed"));
    }
    println!("🔐 Crypto processing (kyber_real) initialized");
    Ok(())
}

/// Test crypto functionality
pub fn test_kyber_real() -> CryptoResult<()> {
    let handler = CryptoHandler::new();
