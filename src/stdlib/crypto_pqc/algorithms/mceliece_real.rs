//! Cryptographic functionality for mceliece_real
...
pub fn init_mceliece_real() -> CryptoResult<()> {
    let handler = CryptoHandler::new();
    let key = handler.generate_key()?;
    if key.len() != 32 {
        return Err(CursedError::runtime_error("Crypto key generation test failed"));
    }
    println!("🔐 Crypto processing (mceliece_real) initialized");
    Ok(())
}

/// Test crypto functionality
pub fn test_mceliece_real() -> CryptoResult<()> {
    let handler = CryptoHandler::new();
