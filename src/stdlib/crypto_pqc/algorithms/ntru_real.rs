//! Cryptographic functionality for ntru_real
...
pub fn init_ntru_real() -> CryptoResult<()> {
    let handler = CryptoHandler::new();
    let key = handler.generate_key()?;
    if key.len() != 32 {
        return Err(CursedError::runtime_error("Crypto key generation test failed"));
    }
    println!("🔐 Crypto processing (ntru_real) initialized");
    Ok(())
}

/// Test crypto functionality
pub fn test_ntru_real() -> CryptoResult<()> {
    let handler = CryptoHandler::new();
