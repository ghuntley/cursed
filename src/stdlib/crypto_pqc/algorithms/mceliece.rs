//! Cryptographic functionality for mceliece
...
pub fn init_mceliece() -> CryptoResult<()> {
    let handler = CryptoHandler::new();
    let key = handler.generate_key()?;
    if key.len() != 32 {
        return Err(CursedError::runtime_error("Crypto key generation test failed"));
    }
    println!("🔐 Crypto processing (mceliece) initialized");
    Ok(())
}

/// Test crypto functionality
pub fn test_mceliece() -> CryptoResult<()> {
    let handler = CryptoHandler::new();
