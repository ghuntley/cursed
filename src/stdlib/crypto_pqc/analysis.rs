//! Cryptographic functionality for analysis
...
pub fn init_analysis() -> CryptoResult<()> {
    let handler = CryptoHandler::new();
    let key = handler.generate_key()?;
    if key.len() != 32 {
        return Err(CursedError::runtime_error("Crypto key generation test failed"));
    }
    println!("🔐 Crypto processing (analysis) initialized");
    Ok(())
}

/// Test crypto functionality
pub fn test_analysis() -> CryptoResult<()> {
    let handler = CryptoHandler::new();
