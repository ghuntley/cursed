...
pub fn file_exists(args: &[Rc<Object>]) -> Result<Rc<Object>, Error> {
    if args.is_empty() {
        return Err(Error::Runtime("file_exists requires 1 argument: path".to_string()));
    }
    
    let path = match &*args[0] {
        Object::String(p) => p.clone(),
        _ => return Err(Error::Runtime("Argument to file_exists must be a string".to_string())),
    };
...
pub fn is_readable(args: &[Rc<Object>]) -> Result<Rc<Object>, Error> {
    if args.is_empty() {
        return Err(Error::Runtime("is_readable requires 1 argument: path".to_string()));
    }
    
    let path = match &*args[0] {
        Object::String(p) => p.clone(),
        _ => return Err(Error::Runtime("Argument to is_readable must be a string".to_string())),
    };
...
pub fn is_writable(args: &[Rc<Object>]) -> Result<Rc<Object>, Error> {
    if args.is_empty() {
        return Err(Error::Runtime("is_writable requires 1 argument: path".to_string()));
    }
    
    let path = match &*args[0] {
        Object::String(p) => p.clone(),
        _ => return Err(Error::Runtime("Argument to is_writable must be a string".to_string())),
    };
...
pub fn file_info(args: &[Rc<Object>]) -> Result<Rc<Object>, Error> {
    ...
    let path = match &*args[0] {
        Object::String(p) => p.clone(),
        _ => return Err(Error::Runtime("Argument to file_info must be a string".to_string())),
    };
        ...
        Ok(meta) => {
            ...
            if let Ok(modified) = meta.modified() {
                if let Ok(since_epoch) = modified.duration_since(std::time::UNIX_EPOCH) {
                    info.insert("modified".to_string(), Object::Integer(since_epoch.as_secs() as i64));
                }
            }
            ...
            if let Ok(created) = meta.created() {
                if let Ok(since_epoch) = created.duration_since(std::time::UNIX_EPOCH) {
                    info.insert("created".to_string(), Object::Integer(since_epoch.as_secs() as i64));
                }
            }
            ...
            if let Ok(accessed) = meta.accessed() {
                if let Ok(since_epoch) = accessed.duration_since(std::time::UNIX_EPOCH) {
                    info.insert("accessed".to_string(), Object::Integer(since_epoch.as_secs() as i64));
                }
            }
...
pub fn remove_file(args: &[Rc<Object>]) -> Result<Rc<Object>, Error> {
    if args.is_empty() {
        return Err(Error::Runtime("remove_file requires 1 argument: path".to_string()));
    }
    
    let path = match &*args[0] {
        Object::String(p) => p.clone(),
        _ => return Err(Error::Runtime("Argument to remove_file must be a string".to_string())),
    };
    
    match fs::remove_file(&path) {
        Ok(_) => Ok(Rc::new(Object::Boolean(true))),
        Err(e) => Err(Error::Runtime(format!("Failed to remove file {}: {}", path, e))),
    }
}
...
pub fn append_file(args: &[Rc<Object>]) -> Result<Rc<Object>, Error> {
    if args.len() < 2 {
        return Err(Error::Runtime("append_file requires 2 arguments: path and data".to_string()));
    }
    
    let path = match &*args[0] {
        Object::String(p) => p.clone(),
        _ => return Err(Error::Runtime("First argument to append_file must be a string".to_string())),
    };
    ...
    let data = match &*args[1] {
        ...
        Object::String(s) => s.as_bytes().to_vec(),
        _ => return Err(Error::Runtime("Second argument to append_file must be an array of bytes or a string".to_string())),
    };
    ...
    match OpenOptions::new().create(true).append(true).open(&path) {
        Ok(mut file) => {
            match file.write_all(&data) {
                Ok(_) => Ok(Rc::new(Object::Boolean(true))),
                Err(e) => Err(Error::Runtime(format!("Failed to append to file {}: {}", path, e))),
            }
        },
        Err(e) => Err(Error::Runtime(format!("Failed to open file {} for appending: {}", path, e))),
    }
}