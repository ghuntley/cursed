...
impl CursedExecutor {
    ...
    pub fn new_with_config(config: ExecutionConfig) -> Result<(), Error> {
        ...
        let jit_engine = CursedJitEngine::new(leaked_context, jit_config)
            .map_err(|e| Error::CompilationError(format!("Failed to create JIT engine: {}", e)))?;
    ...
    fn execute_with_jit(&mut self, func_name: &str, ir: &str, expected_type: ValueType) -> Result<(), Error> {
        ...
        let mut jit_engine = self.jit_engine.lock().map_err(|_| {
            Error::CompilationError("Failed to acquire JIT engine lock".to_string())
        })?;

        // Compile the function
        jit_engine.compile_function(func_name, ir).map_err(|e| {
            self.stats.failed_executions += 1;
            Error::CompilationError(format!("JIT compilation failed: {}", e))
        })?;
        ...
        let result = jit_engine.execute_function(func_name).map_err(|e| {
            self.stats.failed_executions += 1;
            Error::RuntimeError(format!("JIT execution failed: {}", e))
        })?;
    ...
    fn extract_function_name(&self, ir: &str) -> Result<(), Error> {
        if let Some(captures) = regex::Regex::new(r"define.*@([a-zA-Z_][a-zA-Z0-9_]*)")
            ...
            .captures(ir) {
            Ok(captures.get(1)
                .ok_or_else(|| Error::Parse("Could not extract function name".to_string()))?
                .as_str()
                .to_string())
        } else {
            Err(Error::Parse("No function definition found in IR".to_string()))
        }
    }
...
impl From<ExecutionError> for Error {
    fn from(err: ExecutionError) -> Self {
        match err {
            ExecutionError::CompilationFailed(msg) => Error::CompilationError(msg),
            ExecutionError::ExecutionFailed(msg) => Error::RuntimeError(msg),
            ExecutionError::TimeoutError(msg) => Error::RuntimeError(format!("Timeout: {}", msg)),
            ExecutionError::InvalidIR(msg) => Error::Parse(format!("Invalid IR: {}", msg)),
            ExecutionError::FunctionNotFound(msg) => Error::CompilationError(format!("Function not found: {}", msg)),
            ExecutionError::RuntimeError(msg) => Error::RuntimeError(msg),
        }
    }
}
...
mod tests {
    ...
    fn test_executor_creation() {
        let executor = CursedExecutor::new();
        assert!(executor.is_ok());
    }

    #[test]
    fn test_integer_literal_extraction() {
        let executor = CursedExecutor::new().unwrap();
        
        let ir = "ret i32 42";
        let result = executor.extract_integer_literal(ir);
        assert_eq!(result, Some(42));
    }

    #[test]
    fn test_float_literal_extraction() {
        let executor = CursedExecutor::new().unwrap();
        
        let ir = "ret double 3.14";
        let result = executor.extract_float_literal(ir);
        assert_eq!(result, Some(3.14));
    }

    #[test]
    fn test_boolean_literal_extraction() {
        let executor = CursedExecutor::new().unwrap();
        
        let ir = "ret i1 true";
        let result = executor.extract_boolean_literal(ir);
