...
mod tests {
    use super::*;
    use crate::runtime::stack_trace::CallFrame;
    use std::time::SystemTime;

    #[test]
    fn test_debug_formatter_creation() {
    ...
    fn test_config_customization() {
        ...
        let formatter = DebugFormatter::with_config(config);
        assert!(!formatter.config.use_gen_z_slang);
    ...
    fn test_cursed_function_detection() {
        ...
        assert!(formatter.is_cursed_function("yolo_function"));
    ...
    fn test_value_truncation() {
        ...
        let truncated = formatter.truncate_value(&long_value);
        
        assert!(truncated.len() <= formatter.config.max_variable_length);
        assert!(truncated.ends_with("..."));
    }

    #[test]
    fn test_gen_z_messages() {
        let panic_msg = GenZMessages::panic_message("null_pointer");
        assert!(panic_msg.contains("bestie"));
        assert!(panic_msg.contains("💀"));
        
        let success_msg = GenZMessages::success_message("compilation");
        assert!(success_msg.contains("✨"));
        
        let warning_msg = GenZMessages::warning_message("performance");
        assert!(warning_msg.contains("⚠️"));
    }
    ...
    fn test_colorization() {
        ...
        let colored = formatter.colorize_function_name("slay_function");
        // Can't easily test actual colors, but verify it returns a string
        assert!(!colored.is_empty());
        
        let colored_module = formatter.colorize_module_name("test_module");
        assert!(!colored_module.is_empty());
    }

    #[test]
    fn test_format_convenience_functions() {
        let mut trace = StackTrace::new(10);
        let frame = CallFrame::new("panic_function".to_string(), 0);
        trace.add_frame(frame);
        
        let result = format_panic_trace(&trace);
        assert!(result.is_ok());
        
        let output = result.unwrap();
        assert!(output.contains("PANIC") || output.contains("panic"));
    }

    #[test]
    fn test_gen_z_message_printing() {
        // This just tests that the function doesn't panic
        print_gen_z_message("success", "compilation");
        print_gen_z_message("panic", "null_pointer");
        print_gen_z_message("warning", "performance");
    }
}
