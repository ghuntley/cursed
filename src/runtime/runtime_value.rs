...
mod tests {
    ...
    fn test_runtime_value_creation() {
        let rv = RuntimeValue::new(Value::integer(42));
        assert_eq!(rv.get_value(), &Value::integer(42));
        assert!(!rv.has_type_info());
    }

    #[test]
    fn test_runtime_value_with_type_info() {
        let type_info = TypeInfo::new("i64".to_string());
        let rv = RuntimeValue::with_type_info(Value::integer(42), type_info);
        assert!(rv.has_type_info());
        assert_eq!(rv.type_name(), "i64");
    }

    #[test]
    fn test_runtime_value_constructors() {
        assert!(RuntimeValue::null().is_null());
        assert_eq!(RuntimeValue::bool(true).get_value(), &Value::bool(true));
        assert_eq!(RuntimeValue::integer(42).get_value(), &Value::integer(42));
        assert_eq!(RuntimeValue::string("hello").get_value(), &Value::string("hello"));
    }

    #[test]
    fn test_specialized_runtime_values() {
        let rv = RuntimeValue::integer(42);
        let specialized = SpecializedRuntimeValue::Regular(rv);
        assert!(specialized.as_runtime_value().is_some());

        let func = SpecializedRuntimeValue::Function {
            name: "test".to_string(),
            parameters: vec!["x".to_string()],
            return_type: Some("i64".to_string()),
            body: None,
        };
        assert_eq!(func.to_string(), "function test");
    }
}
