...
impl JitMemoryManager {
    ...
    pub fn allocate(&self, function_name: &str, size: usize) -> Result<(), Error> {
        let mut allocated = self.allocated_memory.lock().unwrap();
        let mut total = self.total_allocated.lock().unwrap();

        if *total + size > self.max_memory {
            return Err(Error::from_str("JIT memory limit exceeded"));
        }
    ...
    pub fn deallocate(&self, function_name: &str) -> Result<(), Error> {
        let mut allocated = self.allocated_memory.lock().unwrap();
        let mut total = self.total_allocated.lock().unwrap();
    ...
    pub fn trigger_gc_if_needed(&self) -> Result<(), Error> {
...
impl<'ctx> JitRuntime<'ctx> {
    ...
    fn execute_with_panic_recovery(
        ...
        match result {
            ...
            Err(_) => {
                ...
                Err(Error::from_str(&format!("Panic occurred during execution of function '{}'", function_name)))
            }
        }
    }

    /// Pre-execution setup
    fn pre_execution_setup(&self, function_name: &str) -> Result<(), Error> {
        // Check memory limits
    ...
    fn start_background_optimization(&mut self) -> Result<(), Error> {
    ...
    pub fn shutdown(&mut self) -> Result<(), Error> {
        ...
        if let Some(handle) = self.optimization_thread.take() {
            if let Err(e) = handle.join() {
                tracing::error!(error = ?e, "Error joining background optimization thread");
            }
        }
    ...
    pub fn compile_function(&self, function_name: &str, source: &str) -> Result<(), Error> {
        let mut jit_interface = self.jit_interface.lock().unwrap();
...
mod tests {
    use super::*;
    use inkwell::context::Context;
    use crate::codegen::llvm::{LlvmCodeGenerator, jit_engine::CursedJitEngine, jit_compilation::JitCompilationInterface};
    ...
    fn test_performance_monitor() {
        ...
        let (avg_time, _) = monitor.get_function_performance("test_function").unwrap();
        assert!(avg_time > Duration::ZERO);
    }

    #[test]
    fn test_memory_manager() {
        let manager = JitMemoryManager::new(1024, false);
        
        // Test allocation
        assert!(manager.allocate("test_function", 512).is_ok());
        assert_eq!(manager.get_total_allocated(), 512);
        
        // Test deallocation
        assert!(manager.deallocate("test_function").is_ok());
        assert_eq!(manager.get_total_allocated(), 0);
        
        // Test memory limit
        assert!(manager.allocate("big_function", 2048).is_err());
    }
    ...
    fn test_jit_runtime_creation() {
        ...
        assert_eq!(jit_runtime.get_stats().total_jit_executions, 0);
    }
    ...
    fn test_jit_runtime_function_execution() {
        ...
        assert!(jit_runtime.compile_function("test_function", "").is_ok());
        assert!(jit_runtime.has_function("test_function"));
        
        let result = jit_runtime.execute_function("test_function");
        assert!(result.is_ok());
        assert_eq!(result.unwrap(), 0);
        
        // Check statistics
        let stats = jit_runtime.get_stats();
        assert_eq!(stats.total_jit_executions, 1);
    }
    ...
    fn test_optimization_opportunity_detection() {
        ...
        // Record many slow executions to trigger optimization
        for _ in 0..15 {
            monitor.record_execution("slow_function", Duration::from_millis(100));
        }
        ...
        assert_eq!(opportunity.function_name, "slow_function");
        assert!(matches!(opportunity.reason, OptimizationReason::PoorPerformance | OptimizationReason::HotPath));
    }
