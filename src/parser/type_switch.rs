...
impl Parser {
    ...
    /// }
    /// ```
    pub fn parse_type_switch_statement(&mut self) -> crate::error::Result<()> {
        ...
        while !self.current_token_is(&TokenType::RightBrace) && !self.current_token_is(&TokenType::Eof) {
            ...
            if self.current_token_is(&TokenType::Mood) {
                ...
                return Err(CursedError::Parse("Expected 'mood' or 'basic' in type switch statement".to_string()));
            }
        }
    ...
    fn parse_type_assertion_expression(&mut self) -> crate::error::Result<()> {
        ...
        if self.current_token_is(&TokenType::Dot) {
            ...
            Err(CursedError::Parse("Expected type assertion syntax '.(type)' in type switch".to_string()))
        }
    }
    
    /// Parse type case label (just the type name in mood clauses)
    fn parse_type_case_label(&mut self) -> crate::error::Result<()> {
        if self.current_token_is(&TokenType::Identifier) {
            let type_name = self.current_token.literal.clone();
            self.advance_token()?;
            Ok(Box::new(TypeLiteral::new(type_name)))
        } else {
            Err(CursedError::Parse("Expected type name in type switch case".to_string()))
        }
    }
