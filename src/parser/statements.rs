...
impl<'a> Parser<'a> {
    ...
    /// Parse an import statement (yeet [alias] "package_path")
    #[tracing::instrument(skip(self), fields(token = ?self.current_token), level = "debug")]
    pub(super) fn parse_import_statement(&mut self) -> Result<Box<dyn Statement>, Error> {
        let token = self.current_token.clone();
        self.next_token()?; // Advance past 'yeet'

        let mut alias = None;
        
        // Check if we have an alias (identifier before string literal)
        if matches!(self.current_token, Token::Identifier(_)) {
            // Look ahead to see if this identifier is followed by a string literal
            if matches!(self.peek_token, Token::String(_)) {
                // Parse the alias identifier
                let alias_name = match &self.current_token {
                    Token::Identifier(ident) => ast::expressions::Identifier {
                        token: self.current_token.token_literal(),
                        value: ident.clone(),
                    },
                    _ => unreachable!(),
                ...
                self.next_token()?; // Advance past alias to string literal
            }
        }

        // Parse the package path (string literal)
        ...
        let path = match &self.current_token {
            Token::String(path_str) => crate::ast::expressions::StringLiteral {
                token: self.current_token.token_literal(),
        ...
        self.next_token()?; // Advance past string literal

        // Expect semicolon
        self.expect_semicolon()?;

        Ok(Box::new(ast::statements::declarations::ImportStatement {
            token: token.token_literal(),
            path,
            alias,
        }))
