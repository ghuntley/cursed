...
fn test_variable_declarations() -> Result<(), Error> {
    ...
    assert_eq!(program.statements.len(), 3, "Program doesn't contain 3 statements.");
...
fn test_identifier_expression() -> Result<(), Error> {
    ...
    assert_eq!(parser.errors().len(), 0, "Parser had errors: {:?}", parser.errors());
    assert_eq!(program.statements.len(), 1, "Program doesn't contain 1 statement.");
...
// Skipping this test as our implementation doesn't fully support hash literals yet
#[test]
#[ignore]
