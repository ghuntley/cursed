...
impl LiveDocumentationServer {
    ...
    async fn regenerate_documentation(
        ...
        let doc_config = DocumentationConfig {
            input_dirs: source_paths,
            output_dir: output_path,
            formats: config.output_formats.clone(),
            options: crate::documentation::DocOptions {
                include_private: config.include_private,
                include_source: config.include_source,
                generate_cross_refs: config.generate_cross_refs,
                include_examples: config.include_examples,
                max_type_depth: 10,
                custom_css: config.custom_css_path.clone(),
                template_dir: None,
            },
            metadata: crate::documentation::DocMetadata {
                title: "CURSED Documentation".to_string(),
        ...
        timeout(
            ...
            async {
                for input_dir in &generator.config.input_dirs {
