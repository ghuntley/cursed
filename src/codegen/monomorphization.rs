...
impl MonomorphizationManager {
        ...
        let type_suffix = type_args.iter()
            .map(|t| {
                ...
                match t {
                    ...
                    _ => t.to_string().replace("[", "_").replace("]", "_").replace(", ", "_")
                }
            })
            .collect::<Vec<String>>()
            .join("_");
    ...
    pub fn get_specialized_function_name(&self, generic_name: &str, type_args: &[Type]) -> Option<String> {
        let key = self.get_instantiation_key(generic_name, type_args);
    ...
    pub fn specialize_function(
        ...
        let specialized_function = instantiator.monomorphize_function(
            generic_function,
            type_args
        )?;
        ...
        self.instantiated_functions.insert(key, specialized_name.clone());
        self.specialization_types.insert(specialized_name.clone(), type_args.to_vec());
    ...
    pub fn specialize_struct(
        ...
        let specialized_struct = instantiator.monomorphize_struct(
            generic_struct,
            type_args
        )?;
    ...
    fn is_traceable_type(&self, type_name: &str, type_args: &[Type]) -> bool {
        ...
        match type_name {
            "normie" | "smol" | "mid" | "thicc" | "snack" | "meal" | "lit" | "byte" | "rune" => false,
            _ => true
        }
    }
