...
impl crate::codegen::llvm::LlvmCodeGenerator {
    ...
    pub fn compile_literal_real(&self, literal: &Literal) -> crate::error::Result<()> {
    ...
    pub fn compile_function_real(&self, func: &FunctionStatement) -> crate::error::Result<()> {
        let module_guard = self.module.lock().map_err(|_| {
            CursedError::CompilationError("Failed to acquire module lock".to_string())
        })?;
        ...
        if let Some(ref body) = func.body {
            ...
            if return_type.is_void_type() {
                builder_guard.build_return(None).map_err(|e| {
                    CursedError::CompilationError(format!("Failed to build void return: {:?}", e))
                })?;
            } else {
                ...
                builder_guard.build_return(Some(&zero_value)).map_err(|e| {
                    CursedError::CompilationError(format!("Failed to build return: {:?}", e))
                })?;
            }
        }
    ...
    pub fn compile_identifier_real(&self, identifier: &Identifier) -> crate::error::Result<()> {
        let builder_guard = self.builder.lock().map_err(|_| {
            CursedError::CompilationError("Failed to acquire builder lock".to_string())
        })?;
    ...
    pub fn compile_variable_declaration_real(&mut self, let_stmt: &LetStatement) -> crate::error::Result<()> {
        ...
        let alloca_ptr = builder_guard.build_alloca(var_type, &let_stmt.to_string().value)
            .map_err(|e| CursedError::CompilationError(format!("Failed to build alloca: {:?}", e)))?;
    ...
    pub fn compile_program_real(&mut self, program: &crate::ast::Program) -> crate::error::Result<()> {
    ...
    pub fn optimize_module(&self, optimization_level: crate::common::optimization_level::OptimizationLevel) -> crate::error::Result<()> {
        ...
        let pass_manager = LlvmPassManager::new(&self.context, config)
            .map_err(|e| CursedError::CompilationError(format!("Failed to create pass manager: {}", e)))?;
        ...
        pass_manager.optimize_module(&*module_guard)
            .map_err(|e| CursedError::CompilationError(format!("Module optimization failed: {}", e)))?;
    ...
    pub fn run_optimization_pipeline(
        ...
        if enable_lto {
            let module_guard = self.module.lock().map_err(|_| {
                CursedError::CompilationError("Failed to acquire module lock for LTO".to_string())
            })?;
    ...
    pub fn compile_unary_expression_real(&mut self, unary: &UnaryExpression) -> crate::error::Result<()> {
        let builder_guard = self.builder.lock().map_err(|_| {
            CursedError::CompilationError("Failed to acquire builder lock".to_string())
        })?;
    ...
    pub fn compile_call_expression_real(&mut self, call: &CallExpression) -> crate::error::Result<()> {
        let builder_guard = self.builder.lock().map_err(|_| {
            CursedError::CompilationError("Failed to acquire builder lock".to_string())
        })?;
    ...
    pub fn compile_assignment_expression_real(&mut self, assignment: &AssignmentExpression) -> crate::error::Result<()> {
        ...
        if let Some(identifier) = assignment.to_string().as_any().downcast_ref::<Identifier>() {
            ...
            Err(CursedError::CompilationError("Assignment target must be an identifier".to_string()))
        }
    }
    
    /// Compile index expressions (array access)
    pub fn compile_index_expression_real(&mut self, index: &IndexExpression) -> crate::error::Result<()> {
        let builder_guard = self.builder.lock().map_err(|_| {
            CursedError::CompilationError("Failed to acquire builder lock".to_string())
        })?;
                ...
                if let Some(symbol) = symbol_table.lookup(&identifier.value) {
                    ...
                    if let Some(alloca_ptr) = symbol.alloca_pointer.as_ref() {
                        let builder_guard_inner = self.builder.lock().map_err(|_| {
                            CursedError::CompilationError("Failed to acquire builder lock for store".to_string())
                        })?;
    ...
    pub fn compile_print_statement_real(&mut self, print_stmt: &PrintStatement) -> crate::error::Result<()> {
        ...
        builder_guard.build_call(printf_fn, &print_args, "printf_call")
            .map_err(|e| CursedError::CompilationError(format!("Failed to build printf call: {:?}", e)))?;
