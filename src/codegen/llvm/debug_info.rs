...
mod tests {
    ...
    fn test_simple_debug_location() {
        let location = SimpleDebugLocation::new(10, 5, Some(PathBuf::from("test.csd")));
        assert_eq!(location.get_line(), 10);
        assert_eq!(location.get_column(), 5);
    }
    ...
    fn test_debug_generator_creation() {
        ...
        let result = LlvmDebugGenerator::new(&context, &module, source_file, true);
        assert!(result.is_ok());
    }

    #[test]
    fn test_debug_statistics() {
        let stats = DebugStatistics {
            functions: 5,
            variables: 10,
            types: 3,
            scopes: 2,
