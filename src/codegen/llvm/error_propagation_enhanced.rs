...
impl<'ctx> ErrorPropagationCompiler for LlvmCodeGenerator<'ctx> {
    fn compile_error_propagation(&mut self, expr: &ErrorPropagation) -> Result<BasicValueEnum<'static>, CursedError> {
        let context = ErrorPropagationContext::new(expr.location.clone())
            .with_tail_position(expr.is_tail_position);
    ...
    fn compile_error_propagation_enhanced(
        ...
        // Check if it's a Result or Option type
        if self.is_result_type(&inner_type) {
            self.compile_result_propagation(inner_value, context)
        } else if self.is_option_type(&inner_type) {
            self.compile_option_propagation(inner_value, context)
        } else {
            // For other types, assume they might be error-like values
            self.compile_generic_error_propagation(inner_value, context)
        }
    }
