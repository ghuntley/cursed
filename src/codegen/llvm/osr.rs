...
impl<'ctx> OSRManager<'ctx> {
    ...
    /// Execute recovery strategy
    fn execute_recovery_strategy(&self, deopt_info: &DeoptimizationInfo) -> Result<(), Error> {
    ...
    pub fn record_stack_frame_entry(&self, frame: StackFrame) -> Result<(), Error> {
        ...
        if tracker.current_depth > self.config.max_stack_depth {
            return Err(Error::from_str("Stack depth exceeded maximum limit"));
        }
    ...
    pub fn record_stack_frame_exit(&self, function_name: &str) -> Result<(), Error> {
        let mut tracker = self.stack_frame_tracker.write().unwrap();
...
mod tests {
    ...
    fn test_osr_manager_creation() {
        ...
        let manager = OSRManager::new(&context, config);
        
        assert_eq!(manager.get_current_stack_depth(), 0);
        assert!(manager.get_pending_osr_functions().is_empty());
    }

    #[test]
    fn test_stack_frame_tracking() {
        let context = Context::create();
        let manager = OSRManager::new(&context, OSRConfig::default());
        ...
        manager.record_stack_frame_entry(frame).unwrap();
        assert_eq!(manager.get_current_stack_depth(), 1);
        
        manager.record_stack_frame_exit("test_function").unwrap();
        assert_eq!(manager.get_current_stack_depth(), 0);
    }
    ...
    fn test_osr_trigger_conditions() {
        ...
        assert!(!manager.should_trigger_osr("test_function", 50));
        
        // Would trigger after threshold (but no replacement prepared)
        assert!(!manager.should_trigger_osr("test_function", 150));
    }

    #[test]
    fn test_deoptimization_reason_recovery_mapping() {
        let context = Context::create();
        let manager = OSRManager::new(&context, OSRConfig::default());
        
        let recovery = manager.determine_recovery_strategy(&DeoptimizationReason::TypeAssumptionViolated);
        assert!(matches!(recovery, RecoveryStrategy::ReturnToOriginal));
        
        let recovery = manager.determine_recovery_strategy(&DeoptimizationReason::SpeculativeOptimizationFailed);
        assert!(matches!(recovery, RecoveryStrategy::ReOptimizeWithDifferentAssumptions));
    }
