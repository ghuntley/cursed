...
impl<'a> IntegratedTypeSwitchCompiler<'a> {
    ...
    /// Extract type ID from interface value
    fn extract_integrated_type_id(&mut self, interface_value: &LlvmValue) -> crate::error::Result<()> {
...
impl LlvmTypeSwitchCompiler {
    ...
    fn get_expected_type_id(
        ...
        match type_name {
            _ => Err(CursedError::TypeCompilation(format!("Unknown type for type switch: {}", type_name)))
        }
    }
        ...
        match cursed_type {
            _ => {
                ...
                if let Some(struct_type) = type_registry.get_struct(cursed_type) {
                    ...
                    Err(CursedError::TypeCompilation(format!("Unsupported type in type switch: {}", cursed_type)))
                }
            }
        }
    }
    ...
    fn parse_llvm_type<'ctx>(
        ...
        match type_str {
            ...
            _ if type_str.starts_with("%struct.") && type_str.ends_with('*') => {
                ...
                let ptr_type = context.i8_type().ptr_type(inkwell::AddressSpace::Generic);
                Ok(ptr_type.into())
            _ => Err(CursedError::TypeCompilation(format!("Cannot parse LLVM type: {}", type_str)))
        }
    }
...
impl TypeSwitchUtils {
    ...
    pub fn parse_type_switch_expr(expr_str: &str) -> crate::error::Result<()> {
        ...
        Err(CursedError::TypeCompilation(format!("Invalid type switch expression: {}", expr_str)))
