...
impl SymbolTable {
    ...
    pub fn exit_scope(&mut self) -> Result<(), Error> {
        if self.current_scope_level == 0 {
            return Err(Error::CompilationError("Cannot exit global scope".to_string()));
        }
    ...
    pub fn declare_variable(&mut self, name: String, var_type: LlvmType, llvm_name: String) -> Result<(), Error> {
        let symbol = Symbol::local_variable(name.clone(), var_type, llvm_name, self.current_scope_level);
        self.declare(name, symbol)
    }
    
    /// Declare a global variable
    pub fn declare_global_variable(&mut self, name: String, var_type: LlvmType, llvm_name: String) -> Result<(), Error> {
    ...
    /// Declare a constant
    pub fn declare_constant(&mut self, name: String, const_type: LlvmType, llvm_name: String) -> Result<(), Error> {
...
mod tests {
    ...
    fn test_symbol_declaration() {
        ...
        assert_eq!(symbol.symbol_type, LlvmType::Int64);
        assert_eq!(symbol.llvm_name, "%x");
    }
