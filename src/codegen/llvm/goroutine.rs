...
impl<'ctx> GoroutineCompiler<'ctx> for crate::codegen::llvm::LlvmCodeGeneratorReal<'ctx> {
    fn compile_goroutine_spawn(&mut self, spawn: &GoroutineSpawn) -> crate::error::Result<()> {
        ...
        let spawn_fn = self.module().get_function("cursed_spawn_goroutine")
            .ok_or_else(|| CursedError::Compile("cursed_spawn_goroutine function not found".to_string()))?;
    ...
    fn generate_yield_point(&mut self, location: &str) -> crate::error::Result<()> {
        ...
        let current_block = self.builder().get_insert_block()
            .ok_or_else(|| CursedError::Compile("No current basic block for yield point".to_string()))?;
        let function = current_block.get_parent()
            .ok_or_else(|| CursedError::Compile("No parent function for yield point".to_string()))?;
        
        // Check if GC coordination is requested
        let gc_requested_fn = self.module().get_function("cursed_gc_requested")
            .ok_or_else(|| CursedError::Compile("cursed_gc_requested function not found".to_string()))?;
        ...
        let gc_requested = gc_check_result.try_as_basic_value().left()
            ...
            .ok_or_else(|| CursedError::Compile("Failed to get GC request check result".to_string()))?;
    ...
    fn generate_safe_point(&mut self, location: &str) -> crate::error::Result<()> {
        ...
        let safe_point_fn = self.module().get_function("cursed_safe_point")
            .ok_or_else(|| CursedError::Compile("cursed_safe_point function not found".to_string()))?;
...
impl<'ctx> crate::codegen::llvm::LlvmCodeGeneratorReal<'ctx> {
    ...
    fn extract_function_name(&self, spawn: &GoroutineSpawn) -> crate::error::Result<()> {
        ...
        if parts.len() >= 2 && parts[0] == "stan" {
            ...
            Err(CursedError::Compile(format!("Invalid goroutine spawn expression: {}", spawn_str)))
        }
    }
    
    /// Get runtime scheduler pointer
    fn get_runtime_scheduler_ptr(&self) -> crate::error::Result<()> {
        ...
        let scheduler_ptr_raw = scheduler_opt
            .ok_or_else(|| CursedError::Runtime("No runtime scheduler available".to_string()))?;
...
pub mod runtime_integration {
    ...
    pub fn initialize_scheduler() -> crate::error::Result<()> {
        let mut scheduler = Box::new(crate::runtime::goroutine::GoroutineScheduler::new());
        scheduler.start()
            .map_err(|e| CursedError::Runtime(format!("Failed to start scheduler: {}", e)))?;
