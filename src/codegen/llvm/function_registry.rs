...
impl FunctionSignature {
    ...
    pub fn check_argument_types(&self, arg_types: &[LlvmType]) -> Result<(), Error> {
        if self.is_variadic {
...
impl FunctionRegistry {
    ...
    pub fn register_function(&mut self, signature: FunctionSignature) -> Result<(), Error> {
        ...
        if self.builtins.contains_key(&signature.name) {
            return Err(Error::FunctionRedefinition(format!(
...
mod tests {
    ...
    fn test_type_parsing() {
        assert_eq!(FunctionSignature::parse_cursed_type_to_llvm("normie").unwrap(), LlvmType::Int64);
        assert_eq!(FunctionSignature::parse_cursed_type_to_llvm("facts").unwrap(), LlvmType::Boolean);
        assert_eq!(FunctionSignature::parse_cursed_type_to_llvm("tea").unwrap(), LlvmType::String);
        assert_eq!(FunctionSignature::parse_cursed_type_to_llvm("vibes").unwrap(), LlvmType::Float64);
    }
    ...
    fn test_argument_type_checking() {
        ...
        // Compatible arguments (int32 -> int64)
    ...
    fn test_variadic_functions() {
        let mut signature = FunctionSignature::new_variadic(
            "printf".to_string(),
