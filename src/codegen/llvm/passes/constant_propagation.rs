...
use inkwell::{
    context::Context,
    module::Module,
    values::{FunctionValue, BasicValueEnum, InstructionValue, IntValue, FloatValue},
...
impl<'ctx> ConstantPropagationPass<'ctx> {
    ...
    fn fold_add(&self, instruction: &InstructionValue<'ctx>) -> Result<Option<BasicValueEnum<'ctx>>> {
        ...
        if operands.len() == 2 {
            if let (Some(lhs), Some(rhs)) = (&operands[0], &operands[1]) {
                if let (Some(lhs_int), Some(rhs_int)) = (lhs.as_int_value(), rhs.as_int_value()) {
    ...
    fn check_add_identities(&self, instruction: &InstructionValue<'ctx>) -> Option<BasicValueEnum<'ctx>> {
        if instruction.get_num_operands() >= 2 {
            ...
            // Check for x + 0
            if let Some(rhs_int) = rhs.as_int_value() {
            ...
            // Check for 0 + x
            if let Some(lhs_int) = lhs.as_int_value() {
    ...
    fn check_sub_identities(&self, instruction: &InstructionValue<'ctx>) -> Option<BasicValueEnum<'ctx>> {
        if instruction.get_num_operands() >= 2 {
            ...
            // Check for x - x = 0
            if lhs == rhs {
                if let Some(int_type) = lhs.get_type().as_int_type() {
            ...
            if let (Some(lhs), Some(rhs)) = (&operands[0], &operands[1]) {
                if let (Some(lhs_int), Some(rhs_int)) = (lhs.as_int_value(), rhs.as_int_value()) {
                    ...
                    if rhs_int.get_sign_extended_constant() != Some(0) {
                        let result = lhs_int.const_signed_div(rhs_int);
                        return Ok(Some(result.as_basic_value_enum()));
                    }
                }
            }
        ...
        if operands.len() == 2 {
            if let (Some(lhs), Some(rhs)) = (&operands[0], &operands[1]) {
                if let (Some(lhs_int), Some(rhs_int)) = (lhs.as_int_value(), rhs.as_int_value()) {
                    // Check for division by zero
                    if rhs_int.get_zero_extended_constant() != Some(0) {
                        let result = lhs_int.const_unsigned_div(rhs_int);
                        return Ok(Some(result.as_basic_value_enum()));
                    }
                }
            }
        ...
        if operands.len() == 2 {
            if let (Some(lhs), Some(rhs)) = (&operands[0], &operands[1]) {
                if let (Some(lhs_int), Some(rhs_int)) = (lhs.as_int_value(), rhs.as_int_value()) {
                    // Check for division by zero
                    if rhs_int.get_sign_extended_constant() != Some(0) {
                        let result = lhs_int.const_signed_remainder(rhs_int);
                        return Ok(Some(result.as_basic_value_enum()));
                    }
                }
            }
        ...
        if operands.len() == 2 {
            if let (Some(lhs), Some(rhs)) = (&operands[0], &operands[1]) {
                if let (Some(lhs_int), Some(rhs_int)) = (lhs.as_int_value(), rhs.as_int_value()) {
                    // Check for division by zero
                    if rhs_int.get_zero_extended_constant() != Some(0) {
                        let result = lhs_int.const_unsigned_remainder(rhs_int);
                        return Ok(Some(result.as_basic_value_enum()));
                    }
                }
            }
    ...
    fn fold_shl(&self, instruction: &InstructionValue<'ctx>) -> Result<Option<BasicValueEnum<'ctx>>> {
        ...
        if operands.len() == 2 {
            if let (Some(lhs), Some(rhs)) = (&operands[0], &operands[1]) {
                if let (Some(lhs_int), Some(rhs_int)) = (lhs.as_int_value(), rhs.as_int_value()) {
                    let result = lhs_int.const_left_shift(rhs_int);
                    return Ok(Some(result.as_basic_value_enum()));
                }
            }
        }
    ...
    fn fold_lshr(&self, instruction: &InstructionValue<'ctx>) -> Result<Option<BasicValueEnum<'ctx>>> {
        ...
        if operands.len() == 2 {
            if let (Some(lhs), Some(rhs)) = (&operands[0], &operands[1]) {
                if let (Some(lhs_int), Some(rhs_int)) = (lhs.as_int_value(), rhs.as_int_value()) {
                    let result = lhs_int.const_right_shift(rhs_int, false);
                    return Ok(Some(result.as_basic_value_enum()));
                }
            }
        }
    ...
    fn fold_ashr(&self, instruction: &InstructionValue<'ctx>) -> Result<Option<BasicValueEnum<'ctx>>> {
        ...
        if operands.len() == 2 {
            if let (Some(lhs), Some(rhs)) = (&operands[0], &operands[1]) {
                if let (Some(lhs_int), Some(rhs_int)) = (lhs.as_int_value(), rhs.as_int_value()) {
                    let result = lhs_int.const_right_shift(rhs_int, true);
                    return Ok(Some(result.as_basic_value_enum()));
                }
            }
        }
    ...
    fn fold_icmp(&self, instruction: &InstructionValue<'ctx>) -> Result<Option<BasicValueEnum<'ctx>>> {
        if instruction.get_num_operands() >= 3 {
            let predicate_operand = instruction.get_operand(0)?;
            let lhs = instruction.get_operand(1)?.left()?;
            let rhs = instruction.get_operand(2)?.left()?;
    ...
    fn fold_zext(&self, instruction: &InstructionValue<'ctx>) -> Result<Option<BasicValueEnum<'ctx>>> {
        if let Some(operand) = instruction.get_operand(0) {
            if let Some(value) = operand.left() {
                if let Some(int_val) = value.as_int_value() {
                    if let Some(target_type) = instruction.get_type().left() {
                        if let Some(target_int_type) = target_type.as_int_type() {
                            let result = int_val.const_zero_extend(target_int_type);
                            return Ok(Some(result.as_basic_value_enum()));
                        }
                    }
                }
        ...
        if let Some(operand) = instruction.get_operand(0) {
            if let Some(value) = operand.left() {
                if let Some(int_val) = value.as_int_value() {
                    if let Some(target_type) = instruction.get_type().left() {
                        if let Some(target_int_type) = target_type.as_int_type() {
                            let result = int_val.const_sign_extend(target_int_type);
                            return Ok(Some(result.as_basic_value_enum()));
                        }
                    }
                }
        ...
        if let Some(operand) = instruction.get_operand(0) {
            if let Some(value) = operand.left() {
                if let Some(int_val) = value.as_int_value() {
                    if let Some(target_type) = instruction.get_type().left() {
                        if let Some(target_int_type) = target_type.as_int_type() {
                            let result = int_val.const_truncate(target_int_type);
                            return Ok(Some(result.as_basic_value_enum()));
                        }
                    }
                }
    ...
    fn fold_fadd(&self, instruction: &InstructionValue<'ctx>) -> Result<Option<BasicValueEnum<'ctx>>> {
        ...
        if operands.len() == 2 {
            if let (Some(lhs), Some(rhs)) = (&operands[0], &operands[1]) {
                if let (Some(lhs_float), Some(rhs_float)) = (lhs.as_float_value(), rhs.as_float_value()) {
                    let result = lhs_float.const_add(rhs_float);
                    return Ok(Some(result.as_basic_value_enum()));
                }
            }
        }
    ...
    fn fold_fsub(&self, instruction: &InstructionValue<'ctx>) -> Result<Option<BasicValueEnum<'ctx>>> {
        ...
        if operands.len() == 2 {
            if let (Some(lhs), Some(rhs)) = (&operands[0], &operands[1]) {
                if let (Some(lhs_float), Some(rhs_float)) = (lhs.as_float_value(), rhs.as_float_value()) {
                    let result = lhs_float.const_sub(rhs_float);
                    return Ok(Some(result.as_basic_value_enum()));
                }
            }
        }
    ...
    fn fold_fmul(&self, instruction: &InstructionValue<'ctx>) -> Result<Option<BasicValueEnum<'ctx>>> {
        ...
        if operands.len() == 2 {
            if let (Some(lhs), Some(rhs)) = (&operands[0], &operands[1]) {
                if let (Some(lhs_float), Some(rhs_float)) = (lhs.as_float_value(), rhs.as_float_value()) {
                    let result = lhs_float.const_mul(rhs_float);
                    return Ok(Some(result.as_basic_value_enum()));
                }
            }
        }
    ...
    fn fold_fdiv(&self, instruction: &InstructionValue<'ctx>) -> Result<Option<BasicValueEnum<'ctx>>> {
        ...
        if operands.len() == 2 {
            if let (Some(lhs), Some(rhs)) = (&operands[0], &operands[1]) {
                if let (Some(lhs_float), Some(rhs_float)) = (lhs.as_float_value(), rhs.as_float_value()) {
                    let result = lhs_float.const_div(rhs_float);
                    return Ok(Some(result.as_basic_value_enum()));
                }
            }
        }
...
impl<'ctx> SccpAnalyzer<'ctx> {
    ...
    pub fn apply_constants(&self) -> Result<u32> {
        ...
        for (original, constant) in &self.constant_values {
            original.replace_all_uses_with(constant);
            folded_count += 1;
        }
