...
mod tests {
    ...
    use crate::codegen::llvm::passes::OptimizationLevel;
    
    #[test]
    fn test_dead_code_elimination_pass_creation() {
        let config = PassConfiguration::default();
        let pass = DeadCodeEliminationPass::new(config);
        
        assert_eq!(pass.name(), "dead_code_elimination");
        assert!(pass.description().contains("dead"));
    }
    ...
    fn test_dead_code_analyzer() {
        ...
        assert_eq!(analyzer.function_call_graph.len(), 0);
        assert_eq!(analyzer.instruction_use_counts.len(), 0);
    }
    
    #[test]
    fn test_pass_result_merge() {
        let mut result1 = PassResult::changed();
        result1.instructions_eliminated = 5;
        result1.functions_inlined = 2;
        
        let mut result2 = PassResult::changed();
        result2.instructions_eliminated = 3;
        result2.loops_unrolled = 1;
        
        let merged = result1.merge(result2);
        assert_eq!(merged.instructions_eliminated, 8);
        assert_eq!(merged.functions_inlined, 2);
        assert_eq!(merged.loops_unrolled, 1);
