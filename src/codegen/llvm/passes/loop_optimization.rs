...
mod tests {
    ...
    #[test]
    fn test_loop_optimization_pass_creation() {
        let config = PassConfiguration::default();
        let pass = LoopOptimizationPass::new(config);
        
        assert_eq!(pass.name(), "loop_optimization");
        assert!(pass.description().contains("loop"));
    }
    ...
    fn test_loop_unroller() {
        ...
        assert_eq!(unroller.get_statistics().loops_unrolled, 0);
    ...
    fn test_loop_vectorizer() {
        ...
        assert_eq!(vectorizer.get_statistics().loops_vectorized, 0);
    ...
    fn test_loop_analysis_default() {
        ...
        assert_eq!(analysis.estimated_iterations, 0);
        assert!(!analysis.has_function_calls);
        assert!(!analysis.is_vectorizable);
    }
    
    #[test]
    fn test_loop_optimization_config() {
        ...
        let pass = LoopOptimizationPass::new(config);
        assert!(pass.config.enable_unrolling);
        assert!(pass.config.enable_vectorization);
        assert_eq!(pass.config.max_unroll_count, 8);
        assert!(pass.config.aggressive_unrolling);
    }
}
