                    ...
                    InstructionOpcode::Br | InstructionOpcode::CondBr => {
                        ...
                        for i in 0..terminator.get_num_operands() {
                            if let Some(operand) = terminator.get_operand(i) {
                                if let Some(operand_block) = operand.as_any_value_enum().into_basic_block() {
                        ...
                        for (alloca, _) in &self.promotable_allocas {
                            if store_ptr.as_any_value_enum() == alloca.as_any_value_enum() {
                                if let Some(value) = instruction.get_operand(0) {
                                    let val = value.as_any_value_enum().into_basic_value_enum();
        ...
        if let Some(terminator) = block.get_terminator() {
            match terminator.get_opcode() {
                InstructionOpcode::Br | InstructionOpcode::CondBr => {
                    for i in 0..terminator.get_num_operands() {
                        if let Some(operand) = terminator.get_operand(i) {
                            if let Some(succ_block) = operand.as_any_value_enum().into_basic_block() {
                                for (alloca, _) in &self.promotable_allocas {
