...
impl<'ctx> WebVibezLlvmIntegration<'ctx> {
    ...
    fn register_server_functions(&mut self) -> crate::error::Result<()> {
        ...
        // ListenAndServe(addr: string, handler: Handler) -> CursedError
        ...
        // ListenAndServeTLS(addr: string, certFile: string, keyFile: string, handler: Handler) -> CursedError
    ...
    fn register_client_functions(&mut self) -> crate::error::Result<()> {
        ...
        // Post(url: string, contentType: string, body: string) -> (Response, CursedError)
        ...
        // Head(url: string) -> (Response, CursedError)
        ...
        // Delete(url: string) -> (Response, CursedError)
    ...
    fn compile_listen_and_serve(&self, args: &[BasicValueEnum<'ctx>]) -> crate::error::Result<()> {
        ...
        let func = self.runtime_functions
            ...
            .ok_or_else(|| CursedError::Compile("web_vibez_listen_and_serve runtime function not found".to_string()))?;
        ...
        let call_result = self.builder.build_call(*func, &converted_args, "listen_serve_runtime_call")
            .map_err(|e| CursedError::Compile(format!("Failed to build ListenAndServe runtime call: {:?}", e)))?;
    ...
    fn compile_http_get(&self, args: &[BasicValueEnum<'ctx>]) -> crate::error::Result<()> {
        ...
        let func = self.runtime_functions
            .get("web_vibez_get")
            .ok_or_else(|| CursedError::Compile("web_vibez_get runtime function not found".to_string()))?;
        ...
        let call_result = self.builder.build_call(*func, &converted_args, "http_get_runtime_call")
            .map_err(|e| CursedError::Compile(format!("Failed to build Get runtime call: {:?}", e)))?;
    ...
    fn compile_client_timeout(&self, args: &[BasicValueEnum<'ctx>]) -> crate::error::Result<()> {
        let func = self.runtime_functions
            ...
            .ok_or_else(|| CursedError::Compile("web_vibez_client_timeout runtime function not found".to_string()))?;
        
        let converted_args = self.convert_args(args);
        let call_result = self.builder.build_call(*func, &converted_args, "client_timeout_runtime_call")
            .map_err(|e| CursedError::Compile(format!("Failed to build client_timeout runtime call: {:?}", e)))?;
    ...
    fn compile_http_post(&self, args: &[BasicValueEnum<'ctx>]) -> crate::error::Result<()> {
        ...
        let call_result = self.builder.build_call(*func, &converted_args, "http_post_call")
            .map_err(|e| CursedError::Compile(format!("Failed to build Post call: {:?}", e)))?;
    ...
    fn compile_simple_http_method(&self, method: &str, args: &[BasicValueEnum<'ctx>]) -> crate::error::Result<()> {
        let func = self.function_declarations
            .get(method)
            .ok_or_else(|| CursedError::Compile(format!("{} function not found", method)))?;
        
        let converted_args = self.convert_args(args);
        let call_result = self.builder.build_call(*func, &converted_args, &format!("http_{}_call", method.to_lowercase()))
            .map_err(|e| CursedError::Compile(format!("Failed to build {} call: {:?}", method, e)))?;
    ...
    fn compile_handle_func(&self, args: &[BasicValueEnum<'ctx>]) -> crate::error::Result<()> {
        ...
        let call_result = self.builder.build_call(*func, &converted_args, "handle_func_call")
            .map_err(|e| CursedError::Compile(format!("Failed to build HandleFunc call: {:?}", e)))?;
    ...
    fn compile_response_write(&self, args: &[BasicValueEnum<'ctx>]) -> crate::error::Result<()> {
        ...
        let call_result = self.builder.build_call(*func, &converted_args, "response_write_call")
            .map_err(|e| CursedError::Compile(format!("Failed to build ResponseWriter.Write call: {:?}", e)))?;
    ...
    fn compile_request_property(&self, property: &str, args: &[BasicValueEnum<'ctx>]) -> crate::error::Result<()> {
        ...
        let call_result = self.builder.build_call(*func, &converted_args, &format!("{}_call", property.replace(".", "_").to_lowercase()))
            .map_err(|e| CursedError::Compile(format!("Failed to build {} call: {:?}", property, e)))?;
    ...
    fn allocate_gc_object(&self, type_name: &str) -> crate::error::Result<()> {
        ...
        let ptr = self.builder.build_call(malloc_func, &[size_value.into()], "gc_alloc")
            .map_err(|e| CursedError::Compile(format!("Failed to allocate GC object: {:?}", e)))?
            .try_as_basic_value()
            .left()
            .unwrap()
            .into_pointer_value();
    ...
    fn register_gc_object(&self, ptr: PointerValue<'ctx>, type_name: &str) -> crate::error::Result<()> {
...
// CursedError handling for web_vibez LLVM integration
// impl From<Box<dyn std::error::CursedError>> for CursedError {
//     fn from(err: Box<dyn std::error::CursedError>) -> Self {
//         CursedError::Compile(format!("LLVM integration error: {}", err))
//     }
// }
