...
impl<'ctx> ImprovedFieldAccessors<'ctx> for LlvmCodeGenerator<'ctx> {
    ...
    fn generate_improved_field_accessors(
        ...
        for (i, field) in generic_struct.fields.iter().enumerate() {
            ...
            // Check if field accessors already exist before generating them
            let getter_exists = self.field_accessor_exists(specialized_name, field_name, "get");
            let setter_exists = self.field_accessor_exists(specialized_name, field_name, "set");
            
            if getter_exists && setter_exists {
                debug!("Field accessors for {}.{} already exist, skipping generation", specialized_name, field_name);
                continue;
            }
...
impl<'ctx> LlvmCodeGenerator<'ctx> {
    /// Check if a field accessor already exists for the given specialized struct field
    pub fn field_accessor_exists(&self, specialized_name: &str, field_name: &str, accessor_type: &str) -> bool {
