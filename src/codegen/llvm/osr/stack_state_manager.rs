...
mod tests {
    ...
    #[test]
    fn test_stack_state_manager_creation() {
        let context = Context::create();
        let manager = StackStateManager::new(&context);
        
        assert_eq!(manager.osr_metrics.lock().unwrap().total_osr_attempts, 0);
    }
    
    #[test]
    fn test_platform_register_detection() {
        let register_info = StackStateManager::detect_platform_registers();
        
        assert!(!register_info.general_registers.is_empty());
        assert!(!register_info.float_registers.is_empty());
    }
    ...
    fn test_variable_tracker() {
        ...
        let live_vars = tracker.analyze_live_variables("test_function", 100).unwrap();
        assert!(!live_vars.is_empty());
        
        for (name, var_state) in &live_vars {
            assert_eq!(var_state.name, *name);
            assert!(var_state.is_live);
        }
    }
    
    #[test]
    fn test_register_analyzer() {
        let platform_info = StackStateManager::detect_platform_registers();
        let analyzer = RegisterAnalyzer::new(platform_info);
        
        let register_state = analyzer.capture_register_state("test_function").unwrap();
        
        assert!(!register_state.general_registers.is_empty());
        assert!(!register_state.float_registers.is_empty());
    }
    
    #[test]
    fn test_osr_value_types() {
        let int_val = OSRValue::Integer(42);
        let float_val = OSRValue::Float(3.14);
        let bool_val = OSRValue::Boolean(true);
        
        match int_val {
            OSRValue::Integer(value) => assert_eq!(value, 42),
            _ => panic!("Expected integer value"),
        }
        
        match float_val {
            OSRValue::Float(value) => assert!((value - 3.14).abs() < 0.001),
            _ => panic!("Expected float value"),
        }
        
        match bool_val {
            OSRValue::Boolean(value) => assert!(value),
