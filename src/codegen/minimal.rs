...
impl<'ctx> MinimalCodegen<'ctx> {
    ...
    fn compile_expression(&mut self, expression: &Expression) -> Result<BasicValueEnum<'ctx>, Error> {
        match expression {
            Expression::String(s) => {
    ...
    pub fn write_to_file(&self, filename: &str) -> Result<(), Error> {
        self.module.print_to_file(filename).map_err(|e| {
            Error::Compile(format!("Failed to write LLVM IR to file: {}", e))
        })
    }
    
    pub fn compile_to_object_file(&self, target_triple: &str, filename: &str) -> Result<(), Error> {
        Target::initialize_all(&InitializationConfig::default());
        
        let target = Target::from_triple(&TargetTriple::create(target_triple))
            .map_err(|e| Error::Compile(format!("Failed to create target: {}", e)))?;
        ...
        let target_machine = target.create_target_machine(
            ...
            RelocMode::Default,
            CodeModel::Default,
        ).ok_or_else(|| Error::Compile("Failed to create target machine".to_string()))?;
        
        target_machine.write_to_file(&self.module, FileType::Object, filename.as_ref())
            .map_err(|e| Error::Compile(format!("Failed to write object file: {}", e)))
    }
}

pub fn compile_cursed_to_llvm(program: &Program, module_name: &str) -> Result<String, Error> {
...
pub fn compile_cursed_to_object(program: &Program, module_name: &str, output_file: &str) -> Result<(), Error> {
    let context = Context::create();
    let mut codegen = MinimalCodegen::new(&context, module_name);
