...
impl GcAwareOptimizer {
    ...
    fn monitor_memory_pressure(&mut self) -> Result<()> {
        let gc = self.gc.lock().map_err(|_| Error::Internal("GC lock failed".to_string()))?;
    ...
    fn apply_emergency_optimizations(&mut self) -> Result<()> {
        ...
        // Force immediate GC
        let mut gc = self.gc.lock().map_err(|_| Error::Internal("GC lock failed".to_string()))?;
        gc.collect_garbage();
        drop(gc);
...
mod tests {
    ...
    fn test_memory_pressure_thresholds() {
        ...
        assert_eq!(thresholds.low_threshold, 0.5);
        assert_eq!(thresholds.medium_threshold, 1.0);
    }

    #[test]
    fn test_object_lifetime_analyzer() {
        let analyzer = ObjectLifetimeAnalyzer::new();
        assert_eq!(analyzer.objects_analyzed, 0);
        assert!(analyzer.object_lifetimes.is_empty());
    }

    #[test]
    fn test_memory_pressure_monitor() {
        let monitor = MemoryPressureMonitor::new();
        assert_eq!(monitor.pressure_level, MemoryPressureLevel::Low);
        assert_eq!(monitor.samples_taken, 0);
    }

    #[test]
    fn test_write_barrier_optimizer() {
        let optimizer = WriteBarrierOptimizer::new();
        assert_eq!(optimizer.barriers_analyzed, 0);
        assert!(optimizer.write_barriers.is_empty());
    }

    #[test]
    fn test_allocation_optimizer() {
        let optimizer = AllocationOptimizer::new();
        assert_eq!(optimizer.allocations_analyzed, 0);
        assert!(optimizer.allocation_patterns.is_empty());
    }
}
