    ...
    fn wait_for_completion(&self) -> Result<Vec<PassExecutionResult>> {
        ...
        loop {
            ...
            if start_time.elapsed() > timeout {
                return Err(Error::OptimizationError(
...
impl<'ctx> Worker<'ctx> {
    ...
    fn join(&mut self) -> Result<()> {
        if let Some(handle) = self.handle.take() {
            match handle.join() {
                Ok(result) => result,
                Err(_) => Err(Error::OptimizationError(
...
mod tests {
    ...
    #[test]
    fn test_parallel_pass_config_default() {
        let config = ParallelPassConfig::default();
    ...
    fn test_work_stealing_queue() {
        ...
        queue.add_tasks(vec![1, 2, 3, 4, 5]);
        
        assert!(!queue.all_tasks_completed());
        assert_eq!(queue.total_count.load(Ordering::Relaxed), 5);
    }
    
    #[test]
    fn test_parallel_statistics() {
        let mut stats = ParallelPassStatistics::default();
        stats.total_functions = 100;
        stats.functions_optimized = 95;
