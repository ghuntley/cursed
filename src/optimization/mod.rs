...
pub use coordinator::{
    ...
    RealTimeSavings, CoordinatorStatistics, CacheBenefits,
...
pub use adaptive::{
    ...
    OptimizationStrategy as AdaptiveOptimizationStrategy, 
    OptimizationRecommendation as AdaptiveOptimizationRecommendation,
};
// AdaptiveStrategy comes from optimization_result module
pub use crate::optimization::optimization_result::{AdaptiveStrategy};
...
pub mod analysis {
    ...
    pub use super::real_compilation_profiler::CompilationProfiler;
    pub use super::benchmarks::BenchmarkRunner;
}
...
impl LocalOptimizationCoordinator {
    ...
    pub fn generate_recommendations(&self, source_code: &str) -> Vec<LocalOptimizationRecommendation> {
        ...
        match self.generate_intelligent_recommendations(source_code) {
            Ok(detailed_recommendations) => {
                ...
                detailed_recommendations.into_iter().map(|detailed| {
                    LocalOptimizationRecommendation {
...
// OptimizationRecommendation is now exported from adaptive module as AdaptiveOptimizationRecommendation
// and aliased as OptimizationRecommendation for backward compatibility

/// Local optimization recommendation for the coordinator
#[derive(Debug, Clone)]
pub struct LocalOptimizationRecommendation {
...
/// Category of optimization recommendation (for local coordinator)
#[derive(Debug, Clone)]
...
/// Priority level for recommendations (for local coordinator)
#[derive(Debug, Clone)]
...
// Type aliases for compatibility with different import patterns
pub type AdaptiveOptimizationFeedback = OptimizationFeedback;
// Re-export the main adaptive optimization strategy for backward compatibility
pub type OptimizationStrategy = AdaptiveOptimizationStrategy;
pub type OptimizationRecommendation = AdaptiveOptimizationRecommendation;
