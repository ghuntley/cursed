...
impl OptimizationPredictor {
    ...
    fn combine_predictions_ensemble(
        ...
        if total_weight == 0.0 {
            return Err(Error::InvalidInput("No valid predictions available".to_string()));
        }
        ...
        let best_strategy_key = weighted_scores
            ...
            .map(|(key, _)| key.clone())
            .ok_or_else(|| Error::InvalidInput("No strategy selected".to_string()))?;
    ...
    fn select_best_prediction(&self, predictions: &HashMap<ModelType, ModelPrediction>) -> Result<CombinedPrediction> {
        let best_prediction = predictions
            ...
            .ok_or_else(|| Error::InvalidInput("No predictions available".to_string()))?;
