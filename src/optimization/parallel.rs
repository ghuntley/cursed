...
impl ParallelCompiler {
    ...
    pub fn add_job(&self, job: CompilationJob) -> Result<()> {
        if let Some(ref sender) = self.job_sender {
            sender.send(job.clone())
                .map_err(|e| CursedError::General(format!("Failed to queue job: {}", e)))?;
            ...
            Err(CursedError::General("Parallel compiler not started".to_string()))
        }
    }
            ...
            loop {
                ...
                if let Some(timeout_duration) = timeout {
                    if start_time.elapsed() > timeout_duration {
                        return Err(CursedError::General("Compilation timeout".to_string()));
                    }
                }
    ...
    fn resolve_dependencies(&self, jobs: Vec<CompilationJob>) -> Result<Vec<CompilationJob>> {
        ...
        if result.len() != job_map.len() + result.len() {
            return Err(CursedError::General("Circular dependencies detected".to_string()));
        Ok(result)
    ...
    fn compile_job(worker_id: usize, job: CompilationJob) -> Result<()> {
        ...
        if !job.source_path.exists() {
            return Err(CursedError::General(format!("Source file not found: {}", job.source_path.display())));
    ...
    fn perform_integrated_cursed_compilation(worker_id: usize, job: &CompilationJob) -> Result<()> {
        ...
        let ast = parser.parse()
            .map_err(|e| CursedError::General(format!("Failed to parse CURSED source: {}", e)))?;
        ...
        let llvm_module = codegen.compile_program(&ast)
            .map_err(|e| CursedError::General(format!("Failed to generate LLVM IR: {}", e)))?;
    ...
    fn generate_object_file(
        ...
        let target = Target::from_triple(&target_triple)
            .map_err(|e| CursedError::General(format!("Failed to get target: {}", e)))?;
    ...
    fn fallback_cursed_compilation(worker_id: usize, job: &CompilationJob) -> Result<()> {
        ...
        let output = cmd.output()
            .map_err(|e| CursedError::General(format!("Failed to execute CURSED compiler: {}", e)))?;
        
        if !output.status.success() {
            let stderr = String::from_utf8_lossy(&output.stderr);
            return Err(CursedError::General(format!("CURSED compilation failed: {}", stderr)));
    ...
    fn compile_rust_file(worker_id: usize, job: &CompilationJob) -> Result<()> {
        ...
        for flag in &job.compile_flags {
            cmd.arg(flag);
        let output = cmd.output()
            .map_err(|e| CursedError::General(format!("Failed to execute rustc: {}", e)))?;
        
        if !output.status.success() {
            let stderr = String::from_utf8_lossy(&output.stderr);
            return Err(CursedError::General(format!("Rust compilation failed: {}", stderr)));
    ...
    fn compile_c_file(worker_id: usize, job: &CompilationJob) -> Result<()> {
        ...
        if !output.status.success() {
            let stderr = String::from_utf8_lossy(&output.stderr);
            return Err(CursedError::General(format!("C/C++ compilation failed: {}", stderr)));
    ...
    fn compile_generic_file(worker_id: usize, job: &CompilationJob) -> Result<()> {
        ...
        fs::copy(&job.source_path, &job.output_path)
            .map_err(|e| CursedError::General(format!("Failed to copy file: {}", e)))?;
