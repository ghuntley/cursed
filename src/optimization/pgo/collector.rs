...
impl LlvmProfileWriter {
    fn new(file_path: &Path) -> Result<Self> {
        let file = File::create(file_path).map_err(|e| {
            CursedError::General(format!("Failed to create profile file: {}", e))
        })?;
    ...
    fn flush(&mut self) -> Result<()> {
        if !self.buffer.is_empty() {
            self.file.write_all(&self.buffer).map_err(|e| {
                CursedError::General(format!("Failed to write profile data: {}", e))
            })?;
            self.buffer.clear();
        }
        
        self.file.flush().map_err(|e| {
            CursedError::General(format!("Failed to flush profile data: {}", e))
        })?;
...
impl PerformanceCounterCollector {
    ...
    fn start_collection(&mut self, pid: u32) -> Result<()> {
        ...
        let child = cmd
            ...
            .stderr(Stdio::null())
            .spawn()
            .map_err(|e| CursedError::General(format!("Failed to start perf: {}", e)))?;
    ...
    fn stop_collection(&mut self) -> Result<HashMap<String, u64>> {
        if let Some(mut child) = self.perf_process.take() {
            child.terminate().map_err(|e| {
                CursedError::General(format!("Failed to terminate perf process: {}", e))
            })?;

            child.wait().map_err(|e| {
                CursedError::General(format!("Failed to wait for perf process: {}", e))
            })?;
        }
    ...
    fn parse_perf_output(&self) -> Result<HashMap<String, u64>> {
        ...
        let file = File::open(&self.output_file).map_err(|e| {
            CursedError::General(format!("Failed to open perf output: {}", e))
        })?;
        ...
        for line in reader.split("\n") {
            let line = line.map_err(|e| {
                CursedError::General(format!("Failed to read perf output line: {}", e))
            })?;
...
impl SamplingProfiler {
    ...
    fn start_sampling(&mut self, pid: u32) -> Result<()> {
        ...
        let child = cmd
            ...
            .stderr(Stdio::null())
            .spawn()
            .map_err(|e| CursedError::General(format!("Failed to start perf record: {}", e)))?;
    ...
    fn stop_sampling(&mut self) -> Result<()> {
        if let Some(mut child) = self.profiler_process.take() {
            child.terminate().map_err(|e| {
                CursedError::General(format!("Failed to terminate sampling profiler: {}", e))
            })?;

            child.wait().map_err(|e| {
                CursedError::General(format!("Failed to wait for sampling profiler: {}", e))
            })?;
        }
    ...
    fn process_samples(&self) -> Result<Vec<SampleData>> {
        ...
        let output = Command::new("perf")
            ...
            .arg(&self.output_file)
            .output()
            .map_err(|e| CursedError::General(format!("Failed to run perf script: {}", e)))?;
...
impl ProfileCollector {
    ...
    pub fn load_profile_data(&self, profile_path: &Path) -> Result<ProfileData> {
        ...
        let data = std::fs::read_to_string(profile_path).map_err(|e| {
            CursedError::General(format!("Failed to read profile data: {}", e))
        })?;

        if profile_path.extension().and_then(|s| s.to_str()) == Some("json") {
            // JSON format
            serde_json::from_str(&data).map_err(|e| {
                CursedError::General(format!("Failed to parse JSON profile data: {}", e))
            })
        } else {
    ...
    pub fn save_profile_data(&self, profile_path: &Path, profile_data: &ProfileData) -> Result<()> {
        ...
        if profile_path.extension().and_then(|s| s.to_str()) == Some("json") {
            ...
            let data = serde_json::to_string_pretty(profile_data).map_err(|e| {
                CursedError::General(format!("Failed to serialize profile data: {}", e))
            })?;
    ...
    fn load_raw_profile_data(&self, session_id: &str) -> Result<RawProfileData> {
        ...
        let content = std::fs::read_to_string(&profile_file).map_err(|e| {
            CursedError::General(format!("Failed to read raw profile data: {}", e))
        })?;
