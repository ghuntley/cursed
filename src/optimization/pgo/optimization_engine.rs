...
mod tests {
    use super::*;
    use crate::optimization::pgo::{HotFunction, OptimizationPotential, MemoryAccessPattern};

    #[test]
    fn test_optimization_engine_creation() {
        let config = PgoConfig::default();
        let engine = PgoOptimizationEngine::new(config);
        assert!(engine.is_ok());
    }

    #[test]
    fn test_should_inline_function() {
        let config = PgoConfig::default();
        let engine = PgoOptimizationEngine::new(config).unwrap();
        
        let hot_function = HotFunction {
            name: "test_func".to_string(),
            execution_count: 10000,
            average_size: 50, // Small function
            call_count: 5000, // High call count
        ...
        let analysis = ProfileAnalysis {
            hot_functions: vec![hot_function.clone()],
            cold_functions: vec![],
            loop_profiles: vec![],
            branch_profiles: vec![],
            memory_profiles: vec![],
        ...
        assert!(engine.should_inline_function(&hot_function, &analysis));
    }
    ...
    fn test_improvement_calculation() {
        ...
        let improvement = engine.calculate_improvement_percentage(&before, &after);
        assert!((improvement - 20.0).abs() < 1.0); // Should be approximately 20%
    }
}
