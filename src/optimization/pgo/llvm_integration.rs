...
mod tests {
    ...
    #[test]
    fn test_llvm_pgo_integration_creation() {
        let config = PgoConfig::default();
        let integration = LlvmPgoIntegration::new(config);
        assert!(integration.is_ok());
    }

    #[test]
    fn test_instrumentation_runtime_addition() {
        let config = PgoConfig::default();
        let mut integration = LlvmPgoIntegration::new(config).unwrap();
        
        let context = Context::create();
        let module = context.create_module("test");
        
        let result = integration.add_instrumentation_runtime(&module);
        assert!(result.is_ok());
        
        // Check that runtime functions were added
        assert!(module.get_function("__llvm_profile_increment_step").is_some());
        assert!(module.get_function("__llvm_profile_write_file").is_some());
    }

    #[test]
    fn test_should_instrument_function() {
        let config = PgoConfig::default();
        let integration = LlvmPgoIntegration::new(config).unwrap();
        ...
        // Regular function should be instrumented
        let regular_fn = module.add_function("regular_function", fn_type, None);
        assert!(integration.should_instrument_function(&regular_fn));
        
        // LLVM intrinsic should not be instrumented
        let intrinsic_fn = module.add_function("llvm.memcpy", fn_type, None);
        assert!(!integration.should_instrument_function(&intrinsic_fn));
    }
}
