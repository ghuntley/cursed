...
mod tests {
    ...
    fn test_profile_collector() {
        ...
        let profile_data = collector.stop_collection().unwrap();
        assert!(profile_data.execution_counts.is_empty()); // No data collected yet
    }
    
    #[test]
    fn test_profile_storage() {
        let temp_dir = TempDir::new().unwrap();
        let storage = ProfileStorage::new(temp_dir.path()).unwrap();
        ...
        assert!(storage.store_profile("test_profile", profile_data).is_ok());
        assert!(storage.load_profile("test_profile").is_ok());
    }
    ...
    fn test_optimization_opportunity_creation() {
        let opportunity = OptimizationOpportunity {
            opportunity_type: OptimizationType::FunctionInlining,
            target: OptimizationTarget::Function(FunctionId {
        ...
        assert_eq!(opportunity.confidence, 0.9);
        assert_eq!(opportunity.estimated_improvement, 1.5);
    }
}
