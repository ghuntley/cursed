...
mod tests {
    ...
    fn test_incremental_compiler_creation() {
        ...
        let config = IncrementalConfig {
            state_file_path: temp_dir.join("test_incremental_state.json"),
            ..Default::default()
        };
    ...
    #[test]
    fn test_unit_hash_computation() {
        let temp_dir = env::temp_dir();
        let config = IncrementalConfig {
            state_file_path: temp_dir.join("test_incremental_state_2.json"),
            ..Default::default()
        };
        ...
        let hash1 = compiler.compute_unit_hash(&unit).unwrap();
        let hash2 = compiler.compute_unit_hash(&unit).unwrap();
        
        assert_eq!(hash1, hash2);
        
        // Change unit and verify hash changes
        unit.source_files.push("test2.csd".to_string());
        let hash3 = compiler.compute_unit_hash(&unit).unwrap();
    ...
    fn test_change_detection() {
        let temp_dir = env::temp_dir();
        let config = IncrementalConfig {
            state_file_path: temp_dir.join("test_incremental_state_3.json"),
            ..Default::default()
        };

        let mut compiler = IncrementalCompiler::new(config).unwrap();
        ...
        let changes = compiler.detect_changes(&units);
        assert!(changes.is_ok());
        
        let changes = changes.unwrap();
        // All units should be new on first run
        assert_eq!(changes.new_units.len(), 2);
    }
}
