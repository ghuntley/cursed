...
impl TargetOptimizationManager {
    ...
    pub fn optimize(&mut self, code_unit: &mut CodeUnit) -> Result<TargetOptimizationStatistics> {
        ...
        let profile = self.optimization_profiles
            ...
            .ok_or_else(|| Error::OptimizationError(
...
mod tests {
    ...
    #[test]
    fn test_target_optimization_manager_creation() {
        let config = TargetOptimizationConfig::default();
        let manager = TargetOptimizationManager::new(config);
        assert!(manager.is_ok());
    }
    
    #[test]
    fn test_cpu_info_detection() {
        let cpu_info = TargetOptimizationManager::detect_cpu_info(&CpuArchitecture::X86_64).unwrap();
        assert_eq!(cpu_info.architecture, CpuArchitecture::X86_64);
        assert!(cpu_info.features.contains(&CpuFeature::AVX2));
        assert_eq!(cpu_info.simd_capabilities.max_vector_width, 256);
    }
    
    #[test]
    fn test_optimization_profiles() {
        let profiles = TargetOptimizationManager::create_optimization_profiles();
        assert!(profiles.contains_key(&CpuArchitecture::X86_64));
        assert!(profiles.contains_key(&CpuArchitecture::Arm64));
        
        let x86_profile = &profiles[&CpuArchitecture::X86_64];
        assert_eq!(x86_profile.vectorization_preferences.preferred_vector_width, 256);
    }
    
    #[test]
    fn test_code_unit_analysis() {
        let mut code_unit = CodeUnit::new("test".to_string());
        ...
        assert!(code_unit.has_loops_longer_than(8));
        assert!(code_unit.uses_data_type(&SimdType::Float32));
        assert!(!code_unit.uses_data_type(&SimdType::Int64));
    }
}
