...
mod tests {
    use super::*;
    use super::super::worker_node::{WorkerCapabilities};
    ...
    fn create_test_job() -> CompilationJob {
        CompilationJob::new(
            ...
            super::super::distributed_compiler::OutputType::Object,
        )
    }
    ...
    #[tokio::test]
    async fn test_worker_addition() {
        let mut lb = LoadBalancer::new(LoadBalancingStrategy::RoundRobin, 4).unwrap();
        let worker = create_test_worker("test_worker", 4, 0);
        
        let result = lb.add_worker(worker).await;
        assert!(result.is_ok());

        let workers = lb.get_active_workers().await.unwrap();
        assert_eq!(workers.len(), 1);
        assert_eq!(workers[0].id, "test_worker");
    }
    ...
    #[tokio::test]
    async fn test_worker_removal() {
        let mut lb = LoadBalancer::new(LoadBalancingStrategy::RoundRobin, 4).unwrap();
        let worker = create_test_worker("test_worker", 4, 0);
        
        lb.add_worker(worker).await.unwrap();
        assert_eq!(lb.get_active_workers().await.unwrap().len(), 1);
        
        lb.remove_worker("test_worker").await.unwrap();
        assert_eq!(lb.get_active_workers().await.unwrap().len(), 0);
    }
