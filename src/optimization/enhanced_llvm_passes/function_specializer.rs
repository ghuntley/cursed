...
mod tests {
    ...
    #[test]
    fn test_function_specializer_creation() {
        let stats = Arc::new(Mutex::new(EnhancedOptimizationStatistics::default()));
        let specializer = FunctionSpecializer::new(stats);
        assert_eq!(specializer.specialization_threshold, 10);
    }
    
    #[test]
    fn test_call_site_analysis() {
        let analysis = CallSiteAnalysis::default();
        assert!(analysis.constant_call_sites.is_empty());
        assert!(analysis.call_frequencies.is_empty());
    }
    
    #[test]
    fn test_call_site_info() {
        let info = CallSiteInfo {
            constant_args: vec![0, 2],
            frequency: 5,
            location: "test.csd:10:5".to_string(),
        };
        assert_eq!(info.constant_args.len(), 2);
