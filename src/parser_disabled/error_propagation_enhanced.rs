...
impl Parser {
    ...
    pub fn parse_error_propagation_enhanced(&mut self, left: Box<dyn Expression>) -> crate::error::Result<()> {
    ...
    fn validate_error_propagation_context_enhanced(&self, location: &SourceLocation) -> crate::error::Result<()> {
        ...
        if self.is_in_const_context() {
            return Err(CursedError::ErrorPropagation {
                message: "CursedError propagation with '?' is not allowed in constant expressions".to_string(),
                line: Some(location.line),
                column: Some(location.column),
            });
        }
    ...
    pub fn parse_chained_error_propagations(&mut self, mut expr: Box<dyn Expression>) -> crate::error::Result<()> {
        ...
        if propagation_count >= MAX_PROPAGATION_CHAIN {
            return Err(CursedError::ParseError {
                message: "CursedError propagation chain too long (maximum 50 chained '?' operators)".to_string(),
                line: Some(self.current_token.location.line),
                column: Some(self.current_token.location.column),
            });
        }
    ...
    pub fn parse_error_propagation_with_recovery(&mut self, left: Box<dyn Expression>) -> crate::error::Result<()> {
        match self.parse_error_propagation_enhanced(left.clone()) {
            ...
            Err(err) => {
                ...
                eprintln!("Warning: CursedError propagation parsing failed, continuing without '?' operator: {}", err);
                Ok(left)
            }
        }
    }
    ...
    fn recover_from_error_propagation_error(&mut self) -> crate::error::Result<()> {
...
pub enum ErrorPropagationParseError {
    ...
    /// CursedError propagation in invalid context
    InvalidContext {
        context: String,
        line: usize,
        column: usize,
    ...
    /// CursedError propagation in constant expression
...
//                 write!(f, "CursedError propagation with '?' not allowed in {} context at line {}, column {}", 
//                        context, line, column)
//             },
...
//                 write!(f, "CursedError propagation chain too long ({} operators) at line {}, column {}", 
//                        chain_length, line, column)
//             },
//             ErrorPropagationParseError::InConstantExpression { line, column } => {
//                 write!(f, "CursedError propagation with '?' not allowed in constant expressions at line {}, column {}", 
//                        line, column)
//             },
//         }
//     }
...
// impl std::error::CursedError for ErrorPropagationParseError {}
// 
...
pub struct ErrorPropagationParseContext {
    ...
    /// CursedError propagation chain length
