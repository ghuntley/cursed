# heap_slay module comprehensive test suite
yeet "testz"
yeet "heap_slay"
...
assert_true(heap_parent(0) == 0)  # Root parent is itself in integer division

# Test min heap insertion
...
# Test min heap peek (should be minimum)
test_start("heap_min_peek")
sus min_root normie = heap_peek(min_h)
assert_true(min_root == 1)  # Minimum element should be at root

# Test min heap extraction
test_start("heap_min_extract")
sus extracted normie = heap_extract(min_h)
assert_true(extracted == 1)  # Should extract minimum
assert_true(heap_size(min_h) == 3)
sus next_min normie = heap_peek(min_h)
assert_true(next_min == 3)  # Next minimum should be 3

# Test max heap insertion
...
# Test max heap peek (should be maximum)
test_start("heap_max_peek")
sus max_root normie = heap_peek(max_h)
assert_true(max_root == 8)  # Maximum element should be at root

# Test max heap extraction
test_start("heap_max_extract")
sus max_extracted normie = heap_extract(max_h)
assert_true(max_extracted == 8)  # Should extract maximum
assert_true(heap_size(max_h) == 3)
sus next_max normie = heap_peek(max_h)
assert_true(next_max == 5)  # Next maximum should be 5
...
assert_false(heap_insert(small_h, 15))  # Should fail when full
...
assert_true(empty_peek == -1)  # Error return for empty heap
sus empty_extract normie = heap_extract(empty_h)
assert_true(empty_extract == -1)  # Error return for empty heap
...
assert_true(heap_validate(build_h))  # Should maintain heap property
...
# Test priority queue peek and dequeue
test_start("pq_peek_dequeue")
sus pq_peek_val normie = pq_peek(pq_test)
assert_true(pq_peek_val == 1)  # Highest priority (lowest value)
...
assert_true(pq_next_peek == 2)  # Next highest priority
...
assert_true(kth_3 == 5)  # 3rd largest should be 5 (9, 6, 5)
sus kth_1 normie = heap_kth_largest(kth_arr, 9, 1)
assert_true(kth_1 == 9)  # 1st largest should be 9

# Test heap compare function
test_start("heap_compare")
sus min_heap *Heap = heap_min_new(5)
sus max_heap *Heap = heap_max_new(5)
assert_true(heap_compare(min_heap, 3, 5))   # 3 < 5 for min heap
assert_false(heap_compare(min_heap, 5, 3))  # 5 > 3 for min heap
assert_false(heap_compare(max_heap, 3, 5))  # 3 < 5 for max heap
assert_true(heap_compare(max_heap, 5, 3))   # 5 > 3 for max heap
...
# Test heap with reverse sorted input
...
assert_true(heap_peek(rev_h) == 5)  # Minimum should be 5

# Test heap with already sorted input
...
assert_true(heap_peek(sorted_h) == 5)  # Maximum should be 5

# Test error conditions
test_start("heap_error_conditions")
sus err_h *Heap = heap_min_new(0)  # Zero capacity
assert_true(heap_is_empty(err_h))
assert_true(heap_is_full(err_h))
assert_false(heap_insert(err_h, 1))  # Should fail

sus invalid_kth normie = heap_kth_largest(kth_arr, 9, 0)
assert_true(invalid_kth == -1)  # Invalid k
sus invalid_kth2 normie = heap_kth_largest(kth_arr, 9, 10)
assert_true(invalid_kth2 == -1)  # k > array size

# Test heap swap functionality
...
# Test complex heap operations sequence
