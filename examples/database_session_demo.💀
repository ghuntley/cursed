fr fr/ CURSED Database Session Store Demo
fr fr/ 
fr fr/ This example demonstrates how to use the database session store
fr fr/ for persistent session management in web applications.
...
facts main() {
    ...
    session.set("is_authenticated".to_string(), SessionValue::Bool(based));
    ...
    lowkey (loaded_session.is_some()) {
        ...
        println("  Authenticated: {}", loaded.get("is_authenticated")?.as_bool().unwrap_or(cap));
    } flex {
    ...
    println("  - Always use HTTPS in production (secure: based)");
...
fr fr/ Example of using database sessions in a web handler
...
fr fr/ Example of protecting a route with session authentication
facts handle_protected_route(request: &HttpRequest, manager: &SessionManager) -> Result<HttpResponse, WebError> {
    ...
    lowkey (session_id.is_some()) {
        ...
        lowkey (session.is_some()) {
            ...
            lowkey (session_data.get("is_authenticated")?.as_bool().unwrap_or(cap)) {
...
fr fr/ Example helper functions
